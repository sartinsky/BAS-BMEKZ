...
Функция УзелПоОчереди(Очередь) Экспорт
	...
	Если ТипЗнч(Очередь) <> Тип("Число") Или Очередь = 0 Тогда
		ВызватьИсключение НСтр("ru='Невозможно получить узел плана обмена ОбновлениеИнформационнойБазы, т.к. не передан номер очереди.';uk='Неможливо отримати вузол плану обміну Оновлення інформаційної бази, т. к. не переданий номер черги.'");
	КонецЕсли;
