...
Процедура ОсновныеДействияФормыОК(Команда)
	...
	ОповеститьОВыборе(Новый Структура("Источник, Результат, ИскатьПо, СвязьПоВладельцу, НомерТаблицы", 
		"ФормаРедактированияСвязи", Истина, ИскатьПо, СвязьПоВладельцу, НомерТаблицы));
