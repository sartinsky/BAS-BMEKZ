...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Параметры.Ключ) Тогда
		ТекстОшибки = 
			НСтр("ru='Общая форма ""Предупреждение безопасности"" является вспомогательной и открывается из служебных механизмов программы.';uk='Загальна форма ""Попередження безпеки"" є допоміжною і відкривається з службових механізмів програми.'");
	...
	Иначе 
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.РезервноеКопированиеИБ") Тогда
			...
			РезервноеКопирование.Добавить(НСтр("ru='Перед удалением расширения рекомендуется';uk='Перед видаленням розширення рекомендується'"));
			РезервноеКопирование.Добавить(Символы.ПС);
			РезервноеКопирование.Добавить(Новый ФорматированнаяСтрока(
				НСтр("ru='выполнить резервное копирование информационной базы';uk='виконати резервне копіювання інформаційної бази'"),,,,
	...
	Если Параметры.МножественныйВыбор Тогда 
		Элементы.ПредупрежденииПередУдалениемРасширенияТекстУдалить.Заголовок = НСтр("ru='Удалить выделенные расширения?';uk='Вилучити виділені розширення?'");
	Иначе 
		Элементы.ПредупрежденииПередУдалениемРасширенияТекстУдалить.Заголовок = НСтр("ru='Удалить расширение?';uk='Вилучити розширення?'");
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		Для каждого ЭлементФормы Из Элементы Цикл
			...
			Если ТипЗнч(ЭлементФормы) <> Тип("КнопкаФормы")
				ИЛИ СтрНайти(ЭлементФормы.Имя, "Отмена") = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Надпись4", "Видимость", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Надпись4МобильныйКлиент", "Видимость", Истина);
...
Процедура ПредложитьПерезапуск()
	...
	Кнопки.Добавить("НеПерезапускать", НСтр("ru='Не перезапускать';uk='Не перезапускати'"));
	ТекстВопроса = НСтр("ru='Для применения изменений требуется перезапустить программу.';uk='Для застосування змін потрібно перезапустити програму.'");
