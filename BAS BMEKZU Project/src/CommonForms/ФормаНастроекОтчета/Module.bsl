...
Процедура СортировкаПослеВыбораПоля(ДоступноеПолеКД, ПараметрыВыполнения) Экспорт
	...
	СтрокаТаблицы.ТипНастройки   = "ЭлементПорядка";
...
Процедура ОбновитьПараметрыУниверсальногоОтчета(ПользовательскаяНастройкаКД)
	...
	Если ПараметрТипОбъектаМетаданных <> Неопределено Тогда
		Если ПользовательскаяНастройкаКД.Параметр = ПараметрТипОбъектаМетаданных.Параметр
			...
			НеобходимоОбновитьНастройки = Истина;
			ОчищаемыеПараметры.Добавить("ИсточникДанных");
			ОчищаемыеПараметры.Добавить("ИмяОбъектаМетаданных");
			ОчищаемыеПараметры.Добавить("ИмяТаблицы");
		КонецЕсли;
	КонецЕсли;
	ПараметрИмяОбъектаМетаданных = Отчет.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ИмяОбъектаМетаданных");
	Если ПараметрИмяОбъектаМетаданных <> Неопределено Тогда
		Если ПользовательскаяНастройкаКД.Параметр = ПараметрИмяОбъектаМетаданных.Параметр
				И ПараметрИмяОбъектаМетаданных.Значение <> ПользовательскаяНастройкаКД.Значение Тогда
			ПараметрИмяОбъектаМетаданных.Значение = ПользовательскаяНастройкаКД.Значение;
			НеобходимоОбновитьНастройки = Истина;
			ОчищаемыеПараметры.Добавить("ИсточникДанных");
			ОчищаемыеПараметры.Добавить("ИмяТаблицы");
		КонецЕсли;
...
Процедура ОпределитьВыделенныеСтроки(Контекст)
	...
	Если Контекст.СтрокиДерева.Количество() = 0 Тогда
		Контекст.ПричинаОтказа = НСтр("ru='Выберите элементы.';uk='Оберіть елементи.'");
		Возврат;
	КонецЕсли;
	Если Контекст.ТекущаяСтрока = Неопределено Тогда
		Если Контекст.Действие = "ИзменитьГруппу" Тогда
			Контекст.ПричинаОтказа = НСтр("ru='Выберите группу.';uk='Оберіть групу.'");
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Функция ДобавитьВыделеннуюСтроку(Строки, СтрокаДерева, Особенности)
	...
	Если Особенности.МогутБытьПараметры И СтрокаДерева.ЭтоПараметр Тогда
		Если Строки.Действие = "Переместить" Тогда
			Строки.ПричинаОтказа = НСтр("ru='Параметры не могут быть перемещены.';uk='Параметри не можуть бути переміщені.'");
		ИначеЕсли Строки.Действие = "Сгруппировать" Тогда
			Строки.ПричинаОтказа = НСтр("ru='Параметры не могут быть участниками групп.';uk='Параметри не можуть бути учасниками груп.'");
		ИначеЕсли Строки.Действие = "Удалить" Тогда
			Строки.ПричинаОтказа = НСтр("ru='Параметры не могут быть удалены.';uk='Параметри не можуть бути вилучені.'");
		КонецЕсли;
	...
	Если Особенности.ТребоватьОдногоРодителя Тогда
		...
		ИначеЕсли Строки.ТекущийРодитель <> Родитель Тогда
			Если Строки.Действие = "Переместить" Тогда
				Строки.ПричинаОтказа = НСтр("ru='Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';uk='Вибрані елементи не можуть бути переміщені, оскільки вони належать різним батькам.'");
			ИначеЕсли Строки.Действие = "Сгруппировать" Тогда
				Строки.ПричинаОтказа = НСтр("ru='Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';uk='Вибрані елементи не можуть бути згруповані, оскільки вони належать різним батькам.'");
			КонецЕсли;
			Возврат Ложь; 
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьЗначение(СтрокаТаблицы, УзелКД, ЭлементКД, НовоеЗначение)
	...
	Если Не ПустаяСтрока(ФорматРедактирования) Тогда
		СтрокаТаблицы.ПредставлениеЗначения = Формат(НовоеЗначение, ФорматРедактирования);
	Иначе	
		СтрокаТаблицы.ПредставлениеЗначения = НовоеЗначение;
	КонецЕсли;
КонецПроцедуры
...
Процедура ТаблицыПолейИзменитьУровеньДоступа(ТаблицаИмя, СтрокаИдентификатор, ПоказыватьРежимыВвода, ПоказыватьРежимыФлажков, УровеньДоступа = Неопределено)
	...
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить("ПоказыватьВШапкеОтчета", НСтр("ru='В шапке отчета';uk='В шапці звіту'"), , БиблиотекаКартинок.БыстрыйДоступ);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить("ПоказыватьТолькоФлажокВШапкеОтчета", НСтр("ru='Только флажок в шапке отчета';uk='Тільки прапорець в шапці звіту'"), , БиблиотекаКартинок.БыстрыйДоступСФлажком);
	КонецЕсли;
	Если ПоказыватьРежимыВвода Тогда
		Список.Добавить("ПоказыватьВНастройкахОтчета", НСтр("ru='В настройках отчета';uk='В настройках звіту'"), , БиблиотекаКартинок.Реквизит);
	КонецЕсли;
	Если ПоказыватьРежимыФлажков Тогда
		Список.Добавить("ПоказыватьТолькоФлажокВНастройкахОтчета", НСтр("ru='Только флажок в настройках отчета';uk='Тільки прапорець в настройках звіту'"), , БиблиотекаКартинок.ОбычныйДоступСФлажком);
	КонецЕсли;
	Список.Добавить("НеПоказывать", НСтр("ru='Не показывать';uk='Не показувати'"), , БиблиотекаКартинок.СкрытаяНастройкаОтчета);
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения(ПараметрыЗаполнения, Информация)
	...
	Для Каждого ОписаниеЗакладки Из ГруппыВывода.ДополнительныеЗакладки Цикл
		...
		Если ГруппаВывода.Размер > 0 Тогда
			...
			Колонки.Группировка         = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
