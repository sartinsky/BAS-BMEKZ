...
Процедура КнопкаОк(Команда)
	Если СтрокаСсылочногоТипа Тогда
		...
		КонецЕсли;
		РезультатСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, Ссылка, Представление);
		РезультатФорматированнаяСтрока = СтроковыеФункцииКлиентСервер.ФорматированнаяСтрока(РезультатСтрока);
...
Функция ПустаяФорматированнаяСтрока()
	ПредставлениеЗначения= НСтр("ru='не задано';uk='не задано'");
