...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(СсылкаНаОбъект) Тогда
		ВызватьИсключение НСтр("ru='Не указан владелец настроек прав.';uk='Не вказаний власник настройок прав.'");
	КонецЕсли;
	...
	Если ВозможныеПрава.ПоТипамСсылок.Получить(ТипСсылкиНаОбъект) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Права доступа для каждого объекта не настраиваются
                       |для объектов типа ""%1"".'
                       |;uk='Права доступу для кожного об''єкта не настроюються
                       |для об''єктів типу ""%1"".'"),
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.НайтиПоТипу(ТипСсылкиНаОбъект)) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Нет права Просмотр на объекты типа ""%1"".';uk='Немає права Перегляд на об''єкти типу ""%1"".'"), Строка(ТипСсылкиНаОбъект));
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаЗаписатьИЗакрыть", "Заголовок", НСтр("ru='ОК';uk='ОК'"));
...
Процедура ДобавитьРеквизитыИлиЭлементыФормы(НовыеРеквизиты = Неопределено)
	...
	Если НовыеРеквизиты = Неопределено И Параметры.СсылкаНаОбъект.Метаданные().Иерархический Тогда
		...
		Элемент.Подсказка = НСтр("ru='Права не только для текущей папки,
                                       ...
                                       |але і для її підпорядкованих папок'");
...
Процедура ЗаписатьПрава()
	...
	Попытка
		...
		Иначе
			...
			ВызватьИсключение НСтр("ru='После записи настройка прав станет недоступной.';uk='Після запису настройка прав стане недоступною.'");
		КонецЕсли;
