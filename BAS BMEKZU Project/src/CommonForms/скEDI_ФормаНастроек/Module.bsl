...
Процедура ЗаполнитьСоставГруппыПодписейНаСервере()
	...
	Если ЗначениеЗаполнено(ГруппаПодписи) Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	ВЫБОР
		               |		КОГДА скEDI_Подписи.ПометкаУдаления
		               |			ТОГДА 2
		               |		ИНАЧЕ 1
		               |	КОНЕЦ КАК Картинка
		               |ИЗ
		...
		Пока ВыборкаРезультатаЗапроса.Следующий() Цикл
			...
			НоваяСтрока.Картинка = ВыборкаРезультатаЗапроса.Картинка;
...
Процедура ОткрытьСтраницуНастроек(Команда)
	...
	Если Результат <> Неопределено Тогда
		Для Каждого П Из Результат.ТаблицаНеобходимыхПодписей Цикл
            ...
            ДоступКСтраницеНастроекЗащищенныйНосительIsRemoteSlot = П.ЗащищенныйНосительIsRemoteSlot;
            ДоступКСтраницеНастроекЗащищенныйНосительUseNamedKeys = П.ЗащищенныйНосительUseNamedKeys;
            ДоступКСтраницеНастроекЗащищенныйНосительTokenPassword = П.ЗащищенныйНосительTokenPassword;
            ДоступКСтраницеНастроекЗащищенныйНосительСтрокаПодключения = П.ЗащищенныйНосительСтрокаПодключения;
			Флаг = Истина;
		КонецЦикла;
	КонецЕсли;
	Если Флаг Тогда
...
Процедура СтраницаНастроекДокументСформирован(Элемент)
	Если ДоступКСтраницеНастроекСостояние = "ВводКлюча" Тогда
		...
		Если Токен <> "" Тогда
            ...
            лДанныеПодписи.Вставить("ЗащищенныйНосительIsRemoteSlot", ДоступКСтраницеНастроекЗащищенныйНосительIsRemoteSlot);
            лДанныеПодписи.Вставить("ЗащищенныйНосительUseNamedKeys", ДоступКСтраницеНастроекЗащищенныйНосительUseNamedKeys);
            лДанныеПодписи.Вставить("ЗащищенныйНосительTokenPassword", ДоступКСтраницеНастроекЗащищенныйНосительTokenPassword);
            лДанныеПодписи.Вставить("ЗащищенныйНосительСтрокаПодключения", ДоступКСтраницеНастроекЗащищенныйНосительСтрокаПодключения);
