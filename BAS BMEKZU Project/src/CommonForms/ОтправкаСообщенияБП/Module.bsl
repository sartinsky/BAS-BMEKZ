...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПочтовыеАдресаПолучателей.Количество() = 0 Тогда
		...
		НоваяСтрока.ВариантОтправки = НСтр("ru='Кому:';uk='Кому:'");
	...
	КонецЕсли;
	Элементы.ГруппаПодписьИПечатьБП.Видимость = Ложь;
...
Процедура ПочтовыйАдресаПолучателейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если СтрНайти(Адрес, "@") = 0 ИЛИ СтрНайти(Адрес, ".") = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Необходимо ввести адрес электронной почты';uk='Потрібно ввести адресу електронної пошти'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Для каждого ПолучательПисьма Из ПочтовыеАдресаПолучателей Цикл
		...
		Иначе
			ПоказатьПредупреждение(, НСтр("ru='Такой адрес электронной почты уже есть в списке.';uk='Така адреса електронної пошти вже є в списку.'"));
			Отказ = Истина;
			Возврат;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОтправитьПисьмо()
	...
	Если ПоляЗаполненыКорректно() И ОтправитьПочтовоеСообщение() Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Сообщение отправлено:';uk='Повідомлення надіслано:'"), ,
			?(ПустаяСтрока(ТемаПисьма), НСтр("ru='<Без темы>';uk='<Без теми>'"), ТемаПисьма), БиблиотекаКартинок.Информация32);
...
Функция ПоляЗаполненыКорректно()
	...
	Если ПочтовыеАдресаПолучателей.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Необходимо заполнить получателя письма';uk='Необхідно заповнити одержувача листа'"), , "ПочтовыеАдресаПолучателей");
		Результат = Ложь;
	КонецЕсли;
	Для каждого ПолучательПочты Из ПочтовыеАдресаПолучателей Цикл
		...
		Если ПустаяСтрока(Адрес) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Необходимо заполнить получателя письма';uk='Необхідно заповнити одержувача листа'"),, "ПочтовыеАдресаПолучателей[" + Формат(ПочтовыеАдресаПолучателей.Индекс(ПолучательПочты), "ЧГ=0") + "].Представление");
			Результат = Ложь;
		ИначеЕсли СтрНайти(Адрес, "@") = 0 Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Неверный адрес электронной почты';uk='Неправильна адреса електронної пошти'"),, "ПочтовыеАдресаПолучателей[" + Формат(ПочтовыеАдресаПолучателей.Индекс(ПолучательПочты), "ЧГ=0") + "].Представление");
			Результат = Ложь;
		КонецЕсли;
	КонецЦикла;
...
Функция СформироватьПараметрыПисьма()
	...
	Для каждого Получатель Из ПочтовыеАдресаПолучателей Цикл
		...
		Для каждого ПочтаПолучателя Из ПочтаПолучателей Цикл
			Если Получатель.ВариантОтправки = НСтр("ru='Скрытая копия:';uk='Прихована копія:'") Тогда
				СкрытыеКопии.Добавить(Новый Структура("Адрес, Представление", ПочтаПолучателя.Адрес, ПочтаПолучателя.Псевдоним));
			ИначеЕсли Получатель.ВариантОтправки = НСтр("ru='Копия:';uk='Копія:'") Тогда
...
Функция ПолучитьПриведенныйПочтовыйАдресВФормате(Текст)
	...
	Если Адреса.Количество() > 1 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Можно указывать только один адрес для ответа.';uk='Можна вказувати тільки одну адресу для відповіді.'"), , "АдресОтвета");
		СтандартнаяОбработка = Ложь;
		Возврат Текст;
	КонецЕсли;
...
Процедура ПоказатьВопросПередЗакрытиемФормы()
	ТекстВопроса = НСтр("ru='Сообщение еще не отправлено. Закрыть форму?';uk='Повідомлення ще не відправлене. Закрити форму?'");
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки,,
		КодВозвратаДиалога.Отмена, НСтр("ru='Отправка сообщения';uk='Відправлення повідомлення'"));
КонецПроцедуры
	...
	Если Результат <> Неопределено Тогда
		...
		Если ТипЗнч(Результат.Получатель) = Тип("СписокЗначений") Тогда
			Для Каждого Получатель Из Результат.Получатель Цикл
				...
				АдресПолучателя.ВариантОтправки = НСтр("ru='Кому:';uk='Кому:'");
...
Функция ВариантОтправкиПоПолучателям()
	...
	Для Каждого ПочтовыйАдрес Из ПочтовыеАдресаПолучателей Цикл
		Если ПочтовыйАдрес.ВариантОтправки = НСтр("ru='Скрытая копия:';uk='Прихована копія:'") Тогда
...
Процедура ДобавитьАдресаПолучателей(АдресаПолучателей, СкрытыеКопии = Ложь)
	...
	ВариантОтправки = ?(СкрытыеКопии, НСтр("ru='Скрытая копия:';uk='Прихована копія:'"), НСтр("ru='Кому:';uk='Кому:'"));
...
Процедура ЗаполнитьПараметрыШаблоновПисьмаПриЗапуске()
	...
	Элементы.ФормаВыбратьПолучателей.Видимость = Ложь;
