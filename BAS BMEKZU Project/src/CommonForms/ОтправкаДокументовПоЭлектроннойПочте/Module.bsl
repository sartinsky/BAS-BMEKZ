...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Параметры.ОбъектыОтправки) ИЛИ НЕ ЗначениеЗаполнено(Параметры.ИмяФормыДокумента) Тогда
		ВызватьИсключение НСтр("ru='Непосредственное открытие этой формы не предусмотрено.';uk='Безпосереднє відкриття цієї форми не передбачено.'");
	КонецЕсли;
...
Процедура ОтправитьПоЭлектроннойПочтеНастройкаУчетнойЗаписиПредложена(УчетнаяЗаписьНастроена, ДополнительныеПараметры) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru='Необходимо выбрать, как минимум, одну печатную форму.';uk='Необхідно вибрати, як мінімум, одну друковану форму.'");
