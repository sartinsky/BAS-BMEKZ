...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		Элементы.Переместить(Элементы.СписокРасширенийДобавить, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.СписокРасширенийУдалить, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.КнопкиСписка, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ФормаУстановленныеПатчи, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ФормаОбновитьКэшиРасширений, Элементы.ФормаКоманднаяПанель);
		Элементы.Переместить(Элементы.ФормаУдалитьУстаревшиеКэшиРасширений, Элементы.ФормаКоманднаяПанель);
		
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КомандыФормы", "Видимость", Ложь);
...
Процедура СохранитьКакПослеУстановкиРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		НачатьПолучениеФайлов(Новый ОписаниеОповещения, ПолучаемыеФайлы, Диалог);
	Иначе
...
Процедура ОбновитьКэшиРасширений(Команда)
	...
	ПоказатьПредупреждение(,
		НСтр("ru='Выполнено обновление версии параметров работы расширений,
                   ...
                   |;uk='Виконано оновлення версії параметрів роботи розширень,
                   |підключених в цьому сеансі.'"));
...
Процедура ОбновитьСписок(ПослеДобавления = Ложь)
	...
	Элементы.ГруппаПредупреждение.Видимость =
		ПараметрыСеанса.УстановленныеРасширения.Основные <> УстановленныеРасширения.Основные;
	УстановитьПривилегированныйРежим(Ложь);
	...
	Исключение
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось удалить расширение ""%1"" по причине:
                       |
                       |%2'
                       |;uk='Не вдалося видалити розширення ""%1"" з причини:
                       |
                       |%2'"),
			УдаляемоеРасширение,
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='После удаления, при подготовке оставшихся расширений к работе, произошла ошибка:
                           |
                           |%1'
                           |;uk='Після вилучення, при підготовці залишившихся розширень до роботи, виникла помилка:
                           |
...
Процедура ЗагрузитьРасширениеПослеПодтверждения(Ответ, Контекст) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения("ЗагрузитьРасширениеПослеУстановкиРасширения", ЭтотОбъект, Контекст);
...
Процедура ЗагрузитьРасширениеПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.ОбновлениеКонфигурации") Тогда
		...
		ИначеЕсли ВыбранныеФайлыСодержатИсправления(ПомещенныеФайлы, МодульОбновлениеКонфигурацииКлиент) Тогда 
			ПоказатьПредупреждение(,
				НСтр("ru='Выбранные файлы не должны одновременно содержать исправления (патчи) и другие виды расширений.';uk='Вибрані файли не повинні одночасно містити виправлення (патчі) та інші види розширень.'"));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(НеподключенныеРасширения) Тогда
		Если Контекст.ПомещенныеФайлы.Количество() > 1 Тогда
			Если СтрНайти(НеподключенныеРасширения, ",") > 0 Тогда
				Пояснение = НСтр("ru='Некоторые расширения не подключаются:';uk='Деякі розширення не підключаються:'");
			Иначе
				Пояснение = НСтр("ru='Одно расширение не подключается:';uk='Одне розширення не підключається:'");
			КонецЕсли;
			Пояснение = Пояснение + " " + НеподключенныеРасширения;
		Иначе
			Пояснение = НСтр("ru='Расширение не подключается.';uk='Розширення не підключається.'");
		КонецЕсли;
	Иначе
		Пояснение = "";
	КонецЕсли;
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Новое расширение не может быть применено по причине:
                               |%1'
                               |;uk='Нове розширення не може бути застосовано з причини:
                               |%1'"),
					ТекстОшибки);
			Иначе
				...
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Нельзя заменить расширение ""%1"" на ""%2"".';uk='Не можна замінити розширення ""%1"" на ""%2"".'"),
		...
		Если ИдентификаторРасширения <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось обновить расширение по причине:
                           |
                           |%1'
                           |;uk='Не вдалося оновити розширення з причини:
                           |
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		...
		ИначеЕсли ЗначениеЗаполнено(ИмяФайлаДобавляемогоРасширения) Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось добавить расширение из файла
                           ...
                           |
                           |%2'
                           |;uk='Не вдалося додати розширення файлу
                           |""%1""
                           |з причини:
                           |
                           |%2'"),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось добавить по причине:
                           |
                           |%1'
                           |;uk='Не вдалося додати з причини:
                           |
                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
		КонецЕсли;
	КонецПопытки;
		...
		Если ВосстановлениеВыполнялось
		   И ИнформацияОбОшибкеВосстановления = Неопределено Тогда
			...
			Если ИдентификаторРасширения <> Неопределено Тогда
				Если НовыеДвоичныеДанныеЗаписаны Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
						+ НСтр("ru='Измененное расширение было восстановлено.';uk='Змінене розширення було відновлено.'");
				Иначе
					ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
						+ НСтр("ru='Расширение не было изменено.';uk='Розширення не було змінено.'");
				КонецЕсли;
			Иначе
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
				...
				Если ВременноеРасширение = Неопределено Тогда
					ВызватьИсключение
						НСтр("ru='Текущее расширение не найдено в базе данных,
                                   ...
                                   |;uk='Поточне розширення не знайдено в базі даних
                                   |можливо воно було вилучено в іншому сеансі.'");
				КонецЕсли;
				...
				Если ПомещенныеФайлы.Количество() > 1 Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='При попытке временно добавить расширение из файла
                                       ...
                                       |возникла ошибка:
                                       |%2'
                                       |;uk='При спробі тимчасово додати розширення з файлу
                                       |""%1""
                                       |виникла помилка:
                                       |%2'"),
							ПомещенныйФайл.Имя,
				...
				Иначе
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='При попытке временно добавить расширение возникла ошибка:
                                       |%1'
                                       |;uk='При спробі тимчасово додати розширення виникла помилка:
                                       |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
				КонецЕсли;
			Иначе
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='При попытке временно обновить расширение возникла ошибка:
                                   |%1'
                                   |;uk='При спробі тимчасово оновити розширення виникла помилка:
                                   |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
			КонецЕсли;
							...
							ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='При попытке удалить временно добавленное расширение из файла
                                               ...
                                               |%2'
                                               |;uk='При спробі видалити тимчасово доданий розширення файлу
                                               |%1
                                               |виникла ще одна помилка:
                                               |%2'"),
									ПомещенныйФайл.Имя,
									КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
						Иначе
							ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
								+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='При попытке удалить временно добавленное расширение возникла еще одна ошибка:
                                               |%1'
                                               |;uk='При спробі видалити тимчасово доданий розширення виникла ще одна помилка:
                                               |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
						КонецЕсли;
					Иначе
						ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='При попытке восстановить временно измененное расширение возникла еще одна ошибка:
                                           |%1'
                                           |;uk='При спробі відновити тимчасово змінене розширення виникла ще одна помилка:
                                           |%1'"), КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
					КонецЕсли;
				КонецПопытки;
			КонецЕсли;
