	...
	Если Не ЗначениеЗаполнено(Параметры.ИсточникДанных) Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(ТипЗнч(Параметры.ПараметрКоманды) = Тип("Массив") Или ОбщегоНазначения.ЗначениеСсылочногоТипа(Параметры.ПараметрКоманды),
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Недопустимое значение параметра ПараметрКоманды при вызове метода УправлениеПечатьюКлиент.ВыполнитьКомандуПечати.
                ...
                |Очікувалося: Массив, ЛюбаяСсылка.
	...
	Если ВРег(ИсточникВыбора.ИмяФормы) = ВРег("ОбщаяФорма.СохранениеПечатнойФормы") Тогда
		...
		Если ВыбранноеЗначение <> Неопределено И ВыбранноеЗначение <> КодВозвратаДиалога.Отмена Тогда
			...
			Иначе
				...
				КонецЦикла;
				ПоказатьОповещениеПользователя(, , НСтр("ru='Сохранение завершено';uk='Збереження завершено'"), БиблиотекаКартинок.Информация32);
			КонецЕсли;
		КонецЕсли;
...
Функция СформироватьПечатныеФормы(ИменаМакетов, Отказ)
	...
	Если ПустаяСтрока(КодЯзыкаДляМногоязычныхПечатныхФорм) Тогда
		Если Метаданные.ОбщиеМодули.Найти("Локализация") = Неопределено Тогда
			КодЯзыкаДляМногоязычныхПечатныхФорм = "ru";	
		Иначе	
		    МодульЛокализация = ОбщегоНазначения.ОбщийМодуль("Локализация");
			КодЯзыкаДляМногоязычныхПечатныхФорм = МодульЛокализация.ПолучитьЯзыкФормированияПечатныхФорм();
		КонецЕсли;
	КонецЕсли;
...
Функция ИмяФайлаПечатнойФормыПоУмолчанию(ОбъектПечати, НазваниеПечатнойФормы)
	...
	Если ОбщегоНазначения.ЭтоДокумент(Метаданные.НайтиПоТипу(ТипЗнч(ОбъектПечати))) Тогда
		...
		Иначе
			...
			Шаблон = НСтр("ru='[НазваниеПечатнойФормы] от [Дата]';uk='[НазваниеПечатнойФормы] від [Дата]'");
		КонецЕсли;
...
Процедура СохранитьПечатнуюФормуВФайл()
	...
	ДиалогВыбора.Заголовок = НСтр("ru='Выбор папки для сохранения сформированного документа';uk='Вибір папки для збереження сформованого документа'");
...
Процедура СохранитьПечатныеФормыВПапку(СписокФайловВоВременномХранилище, Знач Папка = "")
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Сохранение успешно завершено';uk='Збереження успішно завершене'"), "file:///" + Папка, НСтр("ru='в папку:'") + " " + Папка, БиблиотекаКартинок.Информация32);
