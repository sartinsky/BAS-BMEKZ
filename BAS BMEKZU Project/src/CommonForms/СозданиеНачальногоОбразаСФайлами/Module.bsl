...
Процедура ЗапуститьСозданиеНачальногоОбраза()
	...
	Иначе
		ВызватьИсключение НСтр("ru='Не удалось создать начальный образ по причине:';uk='Не вдалося створити початковий образ з причини:'") + " " + Результат.КраткоеПредставлениеОшибки; 
	КонецЕсли;
...
Процедура СоздатьНачальныйОбразНаСервереЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		ПроцентВыполнения = 0;
		ДопИнформацияВыполнение = НСтр("ru='Действие отменено администратором.';uk='Дія скасована адміністратором.'");
	...
	Если Результат.Статус = "Ошибка" Тогда
		ПроцентВыполнения = 0;
		Элементы.СтатусГотово.Заголовок = НСтр("ru='Не удалось создать начальный образ по причине:';uk='Не вдалося створити початковий образ з причини:'") + " " + Результат.КраткоеПредставлениеОшибки;
		ЗапуститьПереходРезультат();
		Возврат;
	КонецЕсли;
