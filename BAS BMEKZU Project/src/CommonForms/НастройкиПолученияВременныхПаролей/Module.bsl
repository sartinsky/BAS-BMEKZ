...
Процедура УстановитьНадписи()
	...
	ПояснениеПодтвержденияИзменений = НСтр("ru='Изменения необходимо будет подтвердить, введя код отправленный на %1.
                                            ...
                                            |;uk='Зміни необхідно буде підтвердити, ввівши код відправлений на %1.
                                            |Якщо цей телефон більше не доступний, то скористайтесь <a href = ""#Інструкція"">інструкцією</a>.'");
		...
		ИначеЕсли ПроверкаТелефонСтарый.ВыполняетсяПроверка Тогда
			...
			Если Результат.Выполнено Тогда
				...
				Если ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено И ПроверкаЭлектроннаяПочтаДляПаролей.ПодтверждениеВыполнено Тогда
					ТекстПредупреждения = НСтр("ru='Ваш номер телефона и адрес электронной почты успешно изменены';uk='Ваш номер телефону та адресу електронної пошти успішно змінено'");
				ИначеЕсли ПроверкаТелефонДляПаролей.ПодтверждениеВыполнено Тогда
					ТекстПредупреждения = НСтр("ru='Ваш номер телефона успешно изменен';uk='Ваш номер телефону успішно змінено'");
				Иначе
					ТекстПредупреждения = НСтр("ru='Ваш адрес электронной почты успешно изменен';uk='Ваша адреса електронної пошти успішно змінена'");
				КонецЕсли;				
