...
Процедура Подтвердить(Команда)
	...
	ИначеЕсли ЗначениеЗаполнено(Пароль) И СтрДлина(Пароль) <> 6 Тогда
		ТекстОшибки = НСтр("ru='Пароль должен состоять из 6 цифр';uk='Пароль повинен складатися з 6 цифр'");
	Иначе
		ТекстОшибки = НСтр("ru='Пароль не указан';uk='Пароль не вказано'");
	КонецЕсли;
...
Процедура ПодтвердитьПарольПослеВыполнения(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		Если СтрНайти(ТекстИсключения, "УказанНеверныйПароль") Тогда
			ТекстОшибки = НСтр("ru='Указан неверный пароль';uk='Вказаний невірний пароль'");
		ИначеЕсли СтрНайти(ТекстИсключения, "ПревышенЛимитПопытокВводаПароля") Тогда
			ЗакрытьОткрытую(Новый Структура("Состояние, ОписаниеОшибки", "ПарольНеПринят", НСтр("ru='Превышен лимит попыток ввода пароля';uk='Перевищено ліміт спроб введення пароля'"))); 
		ИначеЕсли СтрНайти(ТекстИсключения, "СрокДействияПароляИстек") Тогда
			ЗакрытьОткрытую(Новый Структура("Состояние, ОписаниеОшибки", "ПарольНеПринят", НСтр("ru='Срок действия пароля истек';uk='Термін дії пароля закінчився '")));
		Иначе 
			ЗакрытьОткрытую(Новый Структура("Состояние, ОписаниеОшибки", "ПарольНеПринят", НСтр("ru='Выполнение операции временно невозможно';uk='Виконання операції тимчасово неможливе'")));
		КонецЕсли;		
	КонецЕсли;
...
Процедура ПолучитьНастройкиПолученияВременныхПаролейПослеВыполнения(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		ПоказатьПредупреждение(Оповещение, НСтр("ru='Сервис отправки SMS-сообщений временно недоступен. Повторите попытку позже.';uk='Сервіс відправки SMS-повідомлень тимчасово недоступний. Повторіть спробу пізніше.'"));		
	КонецЕсли;
...
Процедура Подключаемый_ОбработчикОбратногоОтсчета()
	...
	Таймер = Таймер - 1;
	Если Таймер >= 0 Тогда
		НадписьОбратногоОтсчета = СтрШаблон(НСтр("ru='Запросить пароль повторно можно будет через %1 сек.';uk='Запитати пароль повторно можна буде через %1 сек.'"), Таймер);
		ПодключитьОбработчикОжидания("Подключаемый_ОбработчикОбратногоОтсчета", 1, Истина);		
	Иначе
		...
		Элементы.ИзменитьПолучателя.Заголовок = НСтр("ru='Изменить адрес';uk='Змінити адресу'");
...
Процедура УправлениеФормой(Форма)
	...
	ТекстКоманды = НСтр("ru='Отправить пароль на %1';uk='Відправити пароль на %1'");
	Если Форма.СпособДоставкиПаролей = "phone" Тогда
		Если Форма.ПарольОтправляется Тогда
			Элементы.Пояснение.Заголовок = НСтр("ru='Выполняется отправка пароля в SMS-сообщении на номер';uk='Надсилання пароля у SMS-повідомленні на номер'");
		Иначе
			Элементы.Пояснение.Заголовок = НСтр("ru='Пароль отправлен в SMS-сообщении на номер';uk='Пароль надіслано у SMS-повідомленні на номер'");
		КонецЕсли;
		Элементы.ИзменитьПолучателя.Заголовок = НСтр("ru='Изменить номер';uk='Змінити номер'");
		...
		Элементы.Пароль.ПодсказкаВвода = НСтр("ru='Введите пароль из SMS';uk='Введіть пароль з SMS'");
	ИначеЕсли Форма.СпособДоставкиПаролей = "email" Тогда
		Если Форма.ПарольОтправляется Тогда
			Элементы.Пояснение.Заголовок = НСтр("ru='Выполняется отправка пароля в письме на адрес';uk='Надсилання пароля в листі на адресу'");
		Иначе
			Элементы.Пояснение.Заголовок = НСтр("ru='Пароль отправлен в письме на адрес';uk='Пароль надіслано у листі на адресу'");
		КонецЕсли;	
		Элементы.ИзменитьПолучателя.Заголовок = НСтр("ru='Изменить адрес';uk='Змінити адресу'");
		...
		Элементы.Пароль.ПодсказкаВвода = НСтр("ru='Введите пароль из письма';uk='Введіть пароль з листа'");
	КонецЕсли;
