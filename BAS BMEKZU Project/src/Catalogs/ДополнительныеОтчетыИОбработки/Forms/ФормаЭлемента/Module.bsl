...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		...
		Отказ = Истина;
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Работа с печатными формами не поддерживается.';uk='Робота з друкованими формами не підтримується.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьКоманду(Команда)
	...
	Если Объект.Ссылка.Пустая() Или Модифицированность Тогда
		ТекстВопроса = НСтр("ru='Для выполнения команды необходимо записать данные.';uk='Для виконання команди необхідно записати дані.'");
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить("ЗаписатьИПродолжить", НСтр("ru='Записать и продолжить';uk='Записати й продовжити'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
...
Процедура ОбновлениеИзФайлаПоказатьКонфликты(ПараметрыРегистрации)
	...
	Иначе
		...
		Иначе
            ...
            |;uk='Внутрішнє ім''я обробки ""[Name]""
            |вже зайнято існуючою додатковою обробкою [List].
            |
            |Оберіть:
...
Процедура ОбновлениеИзФайлаРазрешениеКонфликтов(Ответ, ПараметрыРегистрации) Экспорт
	...
	ИначеЕсли Ответ = "ОтменитьИОткрыть" Тогда
		...
		Если ПоказатьСписок Тогда // Форма списка с отбором по конфликтующим.
			...
			ФормаЗаголовок = НСтр("ru='Дополнительные отчеты и обработки с внутреннем именем ""%1""';uk='Додаткові звіти й обробки із внутрішнім  ім''ям ""%1""'");
...
Процедура ПослеОкончанияРедактированияРасписания(Расписание, Контекст) Экспорт
	...
	Иначе
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = НСтр("ru='Не заполнено';uk='Не заповнено'");
			Если ЭлементКоманда.РегламентноеЗаданиеИспользование Тогда
				ЭлементКоманда.РегламентноеЗаданиеИспользование = Ложь;
				ПоказатьОповещениеПользователя(
					НСтр("ru='Запуск по расписанию отключен';uk='Запуск за розкладом відключений'"),
					,
					НСтр("ru='Расписание не заполнено';uk='Розклад не заповнено'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
	...
	Если Задание.Статус = "Ошибка" Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось выполнить команду по причине:
                |%1.'
                |;uk='Не вдалося виконати команду з причини:
...
Функция ПредставлениеБыстрогоДоступаПользователей(КоличествоПользователей)
	...
	БыстрыйДоступПредставление = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
		НСтр("ru=';%1 пользователь;;%1 пользователя;%1 пользователей;%1 пользователя';uk=';%1 користувач;;%1 користувача;%1 користувачів;%1 користувача'"), КоличествоПользователей);
...
Функция НачатьВыполнениеСервернойКомандыВФоне(ВыполняемаяКоманда, УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru='Дополнительные отчеты и обработки: Выполнение серверного метода обработки';uk='Додаткові звіти й обробки: Виконання серверного методу обробки'");
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 не является корректным режимом подключения для дополнительных отчетов и обработок,
                    ...
                    |;uk='%1 не є коректним режимом підключення для додаткових звітів і обробок,
                    |які потребують дозволів на використання профілів безпеки.'"),
				БезопасныйРежим);
		КонецЕсли;
	...
	Для Каждого ЭлементКоманда Из Объект.Команды Цикл
		...
		Иначе
			ЭлементКоманда.РегламентноеЗаданиеПредставление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Неприменимо для команд с вариантом запуска ""%1""';uk='Застосовується для команд з варіантом запуску ""%1""'"),
