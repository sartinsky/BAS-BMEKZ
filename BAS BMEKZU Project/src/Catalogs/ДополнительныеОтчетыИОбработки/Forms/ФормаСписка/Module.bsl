...
Процедура ИзменитьПубликацию(ВариантПубликации)
	...
	Если КоличествоСтрок = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Не выбран ни один дополнительный отчет (обработка)';uk='Не обраний жоден додатковий звіт (обробка)'"));
		Возврат;
	КонецЕсли;
	...
	Иначе
		ТекстСообщения = НСтр("ru='Изменена публикация у дополнительных отчетов (обработок): %1';uk='Змінено публікація у додаткових звітів (обробок): %1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, КоличествоСтрок);
	КонецЕсли;
	
	ПоказатьОповещениеПользователя(НСтр("ru='Изменена публикация';uk='Змінена публікація'"),, ТекстСообщения);
