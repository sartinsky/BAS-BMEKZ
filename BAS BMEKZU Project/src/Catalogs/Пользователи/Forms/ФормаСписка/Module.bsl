...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПравоДоступа("Добавление", Метаданные.Справочники.Пользователи) Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"СоздатьПользователя", "Видимость", Ложь);
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппыПользователей", "ПоложениеЗаголовка", ПоложениеЗаголовкаЭлементаФормы.Авто);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппыПользователей", "ВариантУправленияВысотой", ВариантУправленияВысотойТаблицы.ВСтрокахТаблицы);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппыПользователей", "РастягиватьПоВертикали", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппыПользователей", "Высота", 4);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппыПользователей", "АвтоМаксимальнаяВысота", Ложь);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ГруппыПользователей", "МаксимальнаяВысота", 4);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПользователиСписок", "ПоложениеЗаголовка", ПоложениеЗаголовкаЭлементаФормы.Авто);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ЗавершитьИЗакрыть", "Заголовок", НСтр("ru='Завершить';uk='Завершити'"));
	...
	Если ВыбиратьИерархически Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='Для перетаскивания пользователя в группы необходимо отключить
                       ...
                       |прапорець ""Показувати користувачів підпорядкованих груп"".'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьЗаголовокСпискаВыбранныхПользователейИГрупп()
	...
	Если ХранимыеПараметры.ИспользоватьГруппы Тогда
		ЗаголовокВыбранныеПользователиИГруппы = НСтр("ru='Выбранные пользователи и группы (%1)';uk='Обрані користувачі та групи (%1)'");
	Иначе
	...
	Если Параметры.РежимВыбора Тогда
		...
		Если Параметры.ЗакрыватьПриВыборе = Ложь Тогда
			...
			Если ВыборГруппПользователей Тогда
				...
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьПользователя", "Заголовок", НСтр("ru='Выбрать пользователей';uk='Вибрати користувачів'"));
				
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьГруппуПользователей", "Заголовок", НСтр("ru='Выбрать группы';uk='Вибрати групи'"));
			Иначе
		...
		Иначе
			...
			Если ВыборГруппПользователей Тогда
				...
				ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
					"ВыбратьПользователя", "Заголовок", НСтр("ru='Выбрать пользователя';uk='Вибрати користувача'"));
			Иначе
