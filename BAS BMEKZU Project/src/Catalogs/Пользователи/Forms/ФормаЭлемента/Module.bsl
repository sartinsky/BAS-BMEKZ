...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда // Временное решение для работы в мобильном клиенте, будет удалено в следующих версиях
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаЗаписатьИЗакрыть", "Картинка", БиблиотекаКартинок.ЗаписатьИЗакрыть);
		...
		Элементы.Комментарий.ПодсказкаВвода = НСтр("ru='Произвольный текст';uk='Довільний текст'");
	   ...
	   И (Не ПараметрыЗаписи.Свойство("НеКопироватьПраваПользователя")
		...
		ПоказатьВопрос(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Скопировать также права пользователя ""%1""?';uk='Скопіювати також права користувача ""%1""?'"), Строка(ЗначениеКопирования)),
		   ...
		   И ПользовательИБРоли.Количество() = 0 Тогда
			...
			Если НЕ ПараметрыЗаписи.Свойство("СПустымСпискомРолей") Тогда
				...
				ПоказатьВопрос(
					...
					НСтр("ru='Пользователю информационной базы не установлено ни одной роли. Продолжить?';uk='Користувачу інформаційної бази не встановлено жодної ролі. Продовжити?'"),
					РежимДиалогаВопрос.ДаНет,
					,
					,
					ЗаголовокВопросов);
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Роль ""%1"" в строке %%1 недоступна для пользователей.';uk='Роль ""%1"" у рядку %%1 недоступна для користувачів.'"), Строка.Синоним));
			КонецЕсли;
		КонецЦикла;
...
Процедура ПроверитьПервогоАдминистратора() 
	...
	ЗаголовокВопроса = НСтр("ru='Запись пользователя информационной базы';uk='Запис користувача інформаційної бази'");
	ПоказатьВопрос(
...
Процедура ОпределитьНесоответствияПользователяСПользователемИБ(ПараметрыЗаписи = Неопределено)
	...
	Элементы.СопоставлениеПояснениеНесоответствия.Заголовок =
		НСтр("ru='Пользователь информационной базы не найден.';uk='Користувач інформаційної бази не знайдений.'") + Рекомендация;
