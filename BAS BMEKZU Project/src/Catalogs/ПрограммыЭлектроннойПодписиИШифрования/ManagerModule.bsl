...
Процедура ОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка)
	Если Не ЭлектроннаяПодписьСлужебныйВызовСервера.ИспользоватьЭлектроннуюПодписьВМоделиСервиса() Тогда
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		...
		НСтр("ru='Процедуре Справочники.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось обработать некоторые программы электронной подписи (пропущены): %1';uk='Процедурі Довідники.ОбработатьДанныеДляПереходаНаНовуюверсию.ОбработатьДанныеДляПереходаНаНовуюверсию не вдалося обробити деякі програми електронного підпису (пропущені): %1'"), 
		ПроблемныхОбъектов);
	...
	Иначе
		...
		НСтр("ru='Процедура Справочники.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию обработала очередную порцию программ электронной подписи: %1';uk='Процедура Справочники.ОбработатьДанныеДляПереходаНаНовуюверсию.ОбработатьДанныеДляПереходаНаНовуюверсию опрацювала чергову порцію програм електронного підпису: %1'"),
		ОбъектовОбработано));
	КонецЕсли;
...
Процедура ДозаполнитьНачальныеНастройкиОтложенно(ОбъектовОбработано, ПроблемныхОбъектов)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%1'
            |;uk='Не вдалося дозаповнити початкові настройки програм з причини:
