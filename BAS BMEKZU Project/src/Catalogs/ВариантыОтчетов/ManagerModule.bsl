...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		Если Не ЗначениеЗаполнено(СсылкаВарианта) Тогда
			ВызватьИсключение НСтр("ru='Новый вариант отчета можно создать только из формы отчета';uk='Новий варіант звіту можна створити тільки з форми звіту'");
		КонецЕсли;
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт 
	...
	Пока Вариант.Следующий() Цикл 
		...
		Исключение
			...
			ШаблонКомментария = НСтр("ru='Не удалось установить источник данных варианта отчета %1.
                ...
                |;uk='Не вдалося встановити джерело даних варіанту звіту %1.
                |Можливо, він пошкоджений і не підлягає відновленню.
                |
                |Технічна інформація про проблему: %2'");
	...
	Если Обработано = 0 И Отказано <> 0 Тогда
		ШаблонСообщения = НСтр("ru='Процедуре УстановитьИсточникДанныхВарианта не удалось обработать некоторые варианты отчетов: %1';uk='Процедурі УстановитьИсточникДанныхВарианта не вдалося обробити деякі варіанти звітів: %1'");
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонСообщения, Отказано);
		
		ВызватьИсключение ТекстСообщения;
	Иначе
		ШаблонКомментария = НСтр("ru='Процедура УстановитьИсточникДанныхВарианта обработала очередной пакет вариантов отчетов: %1';uk='Процедура УстановитьИсточникДанныхВарианта опрацювала черговий пакет варіантів звітів: %1'");
