	...
	Иначе
		...
		Если ПараметрыРазмещения.Действие = "Перемещение" Тогда
			Шаблон = НСтр("ru='Перемещено вариантов отчетов: %1 из %2.
                |Подробности:
                |%3'
                |;uk='Переміщено варіантів звітів: %1 з %2.
                |Подробиці:
                |%3'");
		Иначе
			Шаблон = НСтр("ru='Размещено вариантов отчетов: %1 из %2.
                |Подробности:
                |%3'
                |;uk='Розміщено варіантів звітів: %1 з %2.
                |Подробиці:
                |%3'");
		КонецЕсли;
...
Процедура ОбновитьСодержимоеСписка(Знач Событие = "")
	...
	ВариантыПользователя = ?(РезультатПоиска = Неопределено, Новый Массив, РезультатПоиска.Ссылки);
	Список.Параметры.УстановитьЗначениеПараметра("ЕстьОтборПоВариантам", РезультатПоиска <> Неопределено);
