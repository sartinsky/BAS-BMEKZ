...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда // Временное решение для работы в мобильном клиенте, будет удалено в следующих версиях
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Описание", "ПодсказкаВвода", НСтр("ru='Краткое описание';uk='Короткий опис'"));
...
Процедура ОбновитьЗаголовок()
	...
	Если ТипЗнч(Объект.ВладелецФайла) = Тип("СправочникСсылка.ПапкиФайлов") Тогда
		ТипФайла = НСтр("ru='Файл';uk='Файл'");
	Иначе
		ТипФайла = НСтр("ru='Присоединенный файл';uk='Приєднаний файл'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (%2)';uk='%1 (%2)'"), Строка(ЭтотОбъект.Объект.Ссылка), ТипФайла);
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (Создание)';uk='%1 (Створення)'"), ТипФайла);
	КонецЕсли;
