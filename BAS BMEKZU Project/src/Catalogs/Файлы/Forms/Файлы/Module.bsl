...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Сравнить.Видимость = Не ОбщегоНазначенияКлиентСервер.ЭтоLinuxКлиент()
				...
				Если НайденоЗацикливание = Истина Тогда
					Если ЗначениеПеретаскивания.Количество() = 1 Тогда
						ТекстСообщения = НСтр("ru='Перемещение невозможно.
                            ...
                            |;uk='Переміщення неможливо.
                            |Папка ""%1"" є дочірньою для папки ""%2"", що переміщується.'");
					Иначе
						ТекстСообщения = НСтр("ru='Перемещение невозможно.
                            |Папка ""%1"" является дочерней для одной из перемещаемых папок.'
                            |;uk='Переміщення неможливо.
                            |Папка ""%1"" є дочірньою для однієї з папок, що переміщуються.'");
					КонецЕсли;
...
Процедура УстановитьВидимостьПоясненияСинхронизацииФайлов()
	...
	Иначе
		...
		Если Элементы.Папки.ТекущиеДанные.ПапкаСинхронизируется Тогда
			...
			ЧастиСтроки.Добавить(НСтр("ru='Работа с файлами этой папки ведется в облачном сервисе';uk='Робота з файлами цієї папки ведеться в хмарному сервісі'"));
			...
			Если ЗначениеЗаполнено(СведенияОСинхронизации) Тогда
				...
				ЧастиСтроки.Добавить(НСтр("ru='Синхронизировано';uk='Синхронізовано'"));
				ЧастиСтроки.Добавить(": ");
...
Процедура ОбновитьПредпросмотр()
	...
	Если ТекущиеДанные <> Неопределено И РасширенияПоддерживающиеПредпросмотр.НайтиПоЗначению(ТекущиеДанные.Расширение) <> Неопределено Тогда
		...
		Исключение
			...
			ТекстНевыбраннойКартинки = НСтр("ru='Предварительный просмотр недоступен по причине:';uk='Попередній перегляд недоступний через:'") + Символы.ПС + КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
		КонецПопытки;
		
	Иначе
		
		АдресДанныхФайла         = Неопределено;
		ТекстНевыбраннойКартинки = НСтр("ru='Нет данных для предварительного просмотра';uk='Немає даних для попереднього перегляду'");
