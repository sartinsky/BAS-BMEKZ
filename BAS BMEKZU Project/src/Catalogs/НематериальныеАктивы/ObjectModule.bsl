...
Функция СуществуютСсылки()
	...
	|ГДЕ
	|	ХозрасчетныйСубконто.Вид = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.НематериальныеАктивы)
...
Процедура ПроверитьВозможностьЗаписиЭлемента(Отказ)
	...
	Если ИзмененНалоговоеНазначение Тогда
		...
		Если СуществуютСсылки() Тогда
			...
			ТекстСообщения = НСтр("ru='Нематериальный актив ""%1"" участвует в движениях по регистру бухгалтерии ""Хозрасчетный"".
|Нельзя изменять налоговое назначение ""%2""!';uk='Нематеріальний актив ""%1"" бере участь у рухах по регістру бухгалтерії ""Госпрозрахунковий"".
