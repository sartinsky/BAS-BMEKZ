...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ПериодичностьРасписания) Тогда
		ТекстСообщения = ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(, , НСтр("ru='Когда';uk='Коли'"));
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Для Каждого Получатель Из СписокПолучателей Цикл
		Если Не ПустаяСтрока(Получатель.ОписаниеОшибки) Тогда
			ПоказатьПредупреждение(, Получатель.ОписаниеОшибки, , НСтр("ru='Рассылка отчета';uk='Розсилання звіту'"));
			Отказ = Истина;
...
Процедура ЗаполнитьПустыеШаблоныСтандартными(ТекущийОбъект, ФорматированныйДокумент)
	...
	Если ПустаяСтрока(ТекущийОбъект.ТемаПисьма) Тогда
		ТекущийОбъект.ТемаПисьма = ШаблонТемы(ЯзыкШаблона);
	КонецЕсли;
	Если ПустаяСтрока(ТекущийОбъект.ТекстПисьма) Тогда
		ТекущийОбъект.ТекстПисьма = ШаблонТекста(ЯзыкШаблона);
	КонецЕсли;
	Если ПустаяСтрока(ТекущийОбъект.ИмяАрхива) Тогда
		ТекущийОбъект.ИмяАрхива = ШаблонИмениАрхива(ЯзыкШаблона);
	КонецЕсли;
	// Данные формы
...
&НаСервереБезКонтекста
Функция ШаблонТемы(ЯзыкШаблона = "ru") Экспорт
	Если ПустаяСтрока(ЯзыкШаблона) Тогда
		ЯзыкШаблона = Локализация.КодЯзыкаИнформационнойБазы();
	КонецЕсли;
	Возврат НСтр("ru='[ПредставлениеОтчета]';uk='[ПредставлениеОтчета]'",ЯзыкШаблона);
КонецФункции

&НаСервереБезКонтекста
Функция ШаблонТекста(ЯзыкШаблона = "ru") Экспорт
	Если ПустаяСтрока(ЯзыкШаблона) Тогда
		ЯзыкШаблона = Локализация.КодЯзыкаИнформационнойБазы();
	КонецЕсли;
	Возврат НСтр("ru='К письму приложены файлы:
        |[СформированныеОтчетыМаркированныйСписок]
        |
        |[ПодписьАвтора]'
        |;uk='До листа включені файли:
        |[СформированныеОтчетыМаркированныйСписок]
        |
        |[ПодписьАвтора]'",ЯзыкШаблона);
КонецФункции

&НаСервереБезКонтекста
Функция ШаблонИмениАрхива(ЯзыкШаблона = "ru") Экспорт
	Если ПустаяСтрока(ЯзыкШаблона) Тогда
		ЯзыкШаблона = Локализация.КодЯзыкаИнформационнойБазы();
	КонецЕсли;
	Возврат НСтр("ru='[НаименованиеРассылки]_[ДатаВыполнения(ДФ=''yyyy-MM-dd'')]';uk='[НаименованиеРассылки]_[ДатаВыполнения(ДФ=''yyyy-MM-dd'')]'",ЯзыкШаблона);
КонецФункции
...
Функция ТекстПредупрежденияНеобходимоНастроитьУчетнуюЗапись()
	...
	МассивСтрок.Добавить(НСтр("ru='Для отправки отчета настройте ';uk='Для відправки звіту виконайте настройку  '"));
	МассивСтрок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='учетную запись электронной почты';uk='обліковий запис електронної пошти'"), , , , "НастроитьУчетнуюЗапись"));
