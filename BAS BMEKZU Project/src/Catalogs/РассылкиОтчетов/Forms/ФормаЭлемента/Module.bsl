...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда // Временное решение для работы в мобильном клиенте, будет удалено в следующих версиях
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КомандаЗаписатьИЗакрыть", "Отображение", ОтображениеКнопки.Картинка);
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КомандаЗаписатьИЗакрыть", "Картинка", БиблиотекаКартинок.ЗаписатьИЗакрыть);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если РассылкаНеПерсонализирована И ОтчетПерсонализирован Тогда
			Отказ = Истина;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В настройках отчета ''%1'' задан отбор по получателю рассылки. Необходимо отключить этот отбор или изменить вид рассылки на ''Свой отчет для каждого получателя''.';uk='В настройках звіту ''%1'' заданий відбір за одержувачем розсилки. Необхідно відключити цей відбір або змінити вид розсилки на ''Свій звіт для кожного одержувача''.'"),
				Строка(СтрокаОтчеты.Отчет));
			Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
		КонецЕсли;
		...
		Если Объект.Персонализирована И	НЕ ОтчетПерсонализирован Тогда
			Отказ = Истина;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В настройках отчета ""%1"" не указан отбор по получателю рассылки.';uk='В настройках звіту ""%1"" не вказаний відбір за одержувачем розсилки.'"),
				Строка(СтрокаОтчеты.Отчет));
			Поле = "Отчеты["+ Формат(ТекущийОбъект.Отчеты.Индекс(ОбъектСтрокаОтчеты), "ЧН=0; ЧГ=0") +"].Представление";
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ТекущийОбъект, Поле);
		КонецЕсли;
...
Процедура FTPСерверИКаталогНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыПроизвольнойФормы.Вставить("Заголовок", НСтр("ru='<Укажите получателя>';uk='<Зазначте одержувача>'"));
		...
		Если СтруктураВыбора.Выбрано.Количество = 1 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось включить отчет в рассылку по причине:
                    |%1'
                    |;uk='Не вдалося включити звіт у розсилку з причини:
		...
		ИначеЕсли СтруктураВыбора.Успешно.Количество = 0 Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось включить отчеты в рассылку по причине:
                    |%1'
                    |;uk='Не вдалося включити звіти у розсилку з причини:
                    |%1'"), СтруктураВыбора.СОшибками.Текст);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В рассылку включено отчетов: %1 из %2
                    |Подробности:
                    |%3'
                    |;uk='В розсилку включено звітів: %1 з %2
                    |Подробиці:
                    |%3'"),
...
Процедура УказатьПолучателяРассылки(Команда)
	...
	Если Не Настройка.ОписаниеДоступныхТипов.СодержитТип(ОсновнойТипПолучателей) Тогда
		ТекстПредупреждения = НСтр("ru='Тип ""%1"" не подходит по типу к выбранной настройке.
            ...
            |Необхідно вибрати інший тип одержувачів розсилки або іншу настройку.'");
...
Функция ОпределитьНастройку()
	...
	Если СКД Тогда
		...
		Если ИдентификаторНастройки = Неопределено Тогда
			ПоказатьПредупреждение(, НСтр("ru='Не выбрана настройка отчета.';uk='Не обрана настройка звіту.'"));
...
Процедура ДобавитьШаблонПоУмолчанию(Команда)
	...
	ИначеЕсли ЗначениеПредмета = ШаблонПоУмолчанию Тогда
		...
		Если ПерезаполняетсяТема Тогда
			ТекстПредупреждения = НСтр("ru='Тема письма уже соответствует шаблону по умолчанию.';uk='Тема листа вже відповідає шаблону по умовчанню.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Текст письма уже соответствует шаблону по умолчанию.';uk='Текст листа вже відповідає шаблону по умовчанню.'");
		КонецЕсли;
		ПоказатьПредупреждение(, ТекстПредупреждения);
	...
	Иначе
		...
		Если ПерезаполняетсяТема Тогда
			ЗаголовокВопроса = НСтр("ru='Добавить в тему письма шаблон по умолчанию';uk='Додати в тему листа шаблон по умовчанню'");
			ТекстВопроса = НСтр("ru='Заменить тему письма на шаблон по умолчанию?';uk='Замінити тему листа на шаблон по умовчанню?'");
		Иначе
			ЗаголовокВопроса = НСтр("ru='Добавить в текст письма шаблон по умолчанию';uk='Додати в текст листа шаблон по умовчанню'");
			ТекстВопроса = НСтр("ru='Заменить текст письма на шаблон по умолчанию?';uk='Замінити текст листа на шаблон по умовчанню?'");
		КонецЕсли;
...
Процедура ПредпросмотрШаблона(Команда)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Предварительный просмотр шаблона доступен в толстом, тонком или ВЕБ-клиенте.';uk='Попередній перегляд шаблону доступний в товстому, тонкому або ВЕБ-клієнті.'"));
		Возврат;
...
Процедура ПроверитьРассылкуПослеВыбораПолучателей(РезультатВыбора, ПараметрыДоставки) Экспорт
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ПараметрыДоставки.Вставить("ШаблонТемы",    НСтр("ru='Тестовое сообщение BAS';uk='Тестове повідомлення BAS'"));
		ПараметрыДоставки.Вставить("ШаблонТекста",  НСтр("ru='Это сообщение отправлено системой рассылок BAS.';uk='Це повідомлення надіслано системою розсилок BAS.'") + Символы.ПС + Кэш.ЗаголовокСистемы);
...
Функция ВыбратьФормат(СсылкаНаОтчет, ОбработчикРезультата)
	...
	Если ЭтоФорматПоУмолчанию Тогда
		ЗаголовокДиалога = НСтр("ru='Выберите форматы по умолчанию';uk='Виберіть формати по умовчанню'");
	Иначе
...
Процедура ПроверитьРассылку(ПараметрыДоставки)
	...
	Если Не Объект.Подготовлена Или Объект.Ссылка.Пустая() Тогда
		...
		Если Не Объект.Подготовлена Тогда
            ...
            |Натисніть ""Продовжити"", щоб включити прапорець ""Підготовлена"" і записати розсилку.'");
		Иначе
...
Функция ПредставлениеПолучателей(Форма)
	...
	Если Представление.Всего = 0 Тогда
		Представление.Краткое = НСтр("ru='<Укажите получателей>';uk='<Зазначте одержувачів>'");
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='<Пустое значение>';uk='<Порожнє значення>'");
	
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='[Получатель]';uk='[Получатель]'"));
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
Функция ПолучитьКэш()
	...
	Шаблоны = Новый ФиксированнаяСтруктура("Тема, Текст, ИмяАрхива",
		РассылкаОтчетовКлиентСервер.ШаблонТемы(ЯзыкШаблона),
		РассылкаОтчетовКлиентСервер.ШаблонТекста(ЯзыкШаблона),
		РассылкаОтчетовКлиентСервер.ШаблонИмениАрхива(ЯзыкШаблона));
...
Процедура ЗаполнитьСведенияТаблицыОтчеты()
	...
	Для Каждого СтрокаОтчеты Из Объект.Отчеты Цикл
		...
		Если ИнформацияОбОтчете = Неопределено Тогда
			...
			СтрокаОтчеты.Представление = НСтр("ru='<Недостаточно прав для работы с отчетом>';uk='<Недостатньо прав для роботи із звітом>'");
		Иначе
...
Процедура ДобавитьНастройкиОтчетов(ПрисоединяемыеОтчеты)
	...
	Для Каждого СтрокаОтчетыПараметры Из ПрисоединяемыеОтчеты Цикл
		...
		Если Не ЭтоНовый Тогда
			...
			Иначе
				ШаблонСтрокиСообщения = НСтр("ru='Добавлен отчет ""%1"".';uk='Додано звіт ""%1"".'");
			КонецЕсли;
...
Процедура СобытияРассылки()
	...
	Если ПараметрыФормыЖурналаРегистрации = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Рассылка еще не выполнялась.';uk='Розсилка ще не виконувалась.'"));
		Возврат;
	КонецЕсли;
