...
Процедура ПутьКПапкеДляКопированияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Новый ОписаниеОповещения("ПодключениеРасширенияРаботыСФайламиВыборФайлаЗавершение", ЭтотОбъект), , Ложь);
	
КонецПроцедуры

&НаКлиенте
Процедура ПодключениеРасширенияРаботыСФайламиВыборФайлаЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	ДиалогОткрытияФайла.Показать(Новый ОписаниеОповещения("ПутьКПапкеДляКопированияНачалоВыбораЗавершение", ЭтотОбъект, Новый Структура("ДиалогОткрытияФайла", ДиалогОткрытияФайла)));
	
КонецПроцедуры

&НаКлиенте
Процедура ПутьКПапкеДляКопированияНачалоВыбораЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
...
Процедура УдалитьЛишниеФайлы(Команда)
	...
	ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(Новый ОписаниеОповещения("ПодключениеРасширенияРаботыСФайламиЗавершение", ЭтотОбъект), , Ложь);
...
Процедура ПодключениеРасширенияРаботыСФайламиЗавершение(РасширениеПодключено, ДополнительныеПараметры) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		ПоказатьПредупреждение(, НСтр("ru='Расширение работы с файлами не установлено. Работа с файлами с неустановленным расширением в веб клиенте невозможна.';uk='Розширення роботи з файлами не встановлено. Робота з файлами з невстановленим розширенням веб клієнта неможлива.'"));
		Возврат;
	КонецЕсли;
...
Процедура ПроверкаСуществованияКаталогаЗавершение(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Не Существует Тогда
		ПоказатьПредупреждение(, НСтр("ru='Путь к каталогу копирования некорректен.';uk='Шлях до каталогу копіювання некоректний.'"));
	Иначе
...
Процедура ОбработатьОчереднойФайл(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		СтрокиФайлов        = ЛишниеФайлы.НайтиСтроки(Новый Структура("Имя", ТекущийФайл.Имя));
...
Процедура ОбработатьОшибкуПравДоступа(ИнформацияОбОшибке, ИсходноеОповещение)
	...
	ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , , "ПутьКПапкеДляКопирования");
