...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ТипЗнч(Параметры.МассивСсылок) <> Тип("Массив") 
		...
		ВызватьИсключение НСтр("ru='Должна быть хоть одна ссылка указана.';uk='Повинне бути хоч одне посилання вказане.'");
	КонецЕсли;
	...
	Для Каждого Ссылка Из Параметры.МассивСсылок Цикл 
		...
		ТекстПояснения = ТекстПояснения
			...
			+ ?(Реквизиты.ОбновлятьСПортала1СИТС, "", " - " + НСтр("ru='обновление отключено';uk='оновлення вимкнено'") + ".")
			+ Символы.ПС;
	...
	Если ПредложитьОбновление Тогда 
		...
		Элементы.ДекорацияПояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1
                       ...
                       |;uk='%1
                       |Перевірити оновлення компоненти?'"),
			ТекстПояснения);
...
Процедура Загрузить(Команда)
	...
	Если ДлительнаяОперация = Неопределено Тогда 
		КраткоеПредставлениеОшибки = НСтр("ru='Не удалось создать фоновое задание обновления компоненты.';uk='Не вдалося створити фонове завдання оновлення компоненти.'");
...
Функция НачатьОбновлениеКомпонентСПортала()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Обновление внешней компоненты.';uk='Оновлення зовнішньої компоненти.'");
