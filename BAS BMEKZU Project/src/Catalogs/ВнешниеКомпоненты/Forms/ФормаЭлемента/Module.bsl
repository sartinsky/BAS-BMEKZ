...
Процедура ОбновитьСПортала(Команда)
	...
	Если Модифицированность Тогда
		...
		ПоказатьВопрос(Оповещение, 
			НСтр("ru='Для проверки обновления необходимо записать объект. Записать?';uk='Для перевірки оновлень необхідно записати об''єкт. Записати?'"),
...
Процедура СохранитьКак(Команда)
	...
	Если ЭтоАдресВременногоХранилища(АдресДвоичныхДанныхКомпоненты) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Перед сохранение компоненты в файл элемент справочника нужно записать.';uk='Перед збереження компоненти в файл елемент довідника потрібно записати.'"));
	Иначе 
		ОчиститьСообщения();
...
Процедура ЗагрузитьКомпонентуПослеПредупрежденияБезопасности(Ответ, Контекст) Экспорт
	...
	ПараметрыДиалога.Вставить("Заголовок", НСтр("ru='Выберите файл внешней компоненты';uk='Виберіть файл зовнішньої компоненти'"));
	ПараметрыДиалога.Вставить("Фильтр",    НСтр("ru='Внешняя компонента (*.zip)|*.zip|Все файлы(*.*)|*.*';uk='Зовнішня компонента (*.zip)|*.zip|Всі файли(*.*)|*.*'"));
...
Процедура ЗагрузитьКомпонентуПослеПомещенияФайла(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ПомещенныеФайлы = Неопределено Или ПомещенныеФайлы.Количество() = 0 Тогда
		ЗагрузитьКомпонентуПриОтображенииОшибки(НСтр("ru='Файл не удалось загрузить на сервер.';uk='Файл не вдалося завантажити на сервер.'"));
		Возврат;
	КонецЕсли;
		...
		СтрокаСПредупреждением = Новый ФорматированнаяСтрока(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1
                           ...
                           |Подробнее:'
                           |;uk='%1
                           |Необхідно вказати zip-архів з зовнішньою компонентою.
                           |Детальніше:'"),
				ОписаниеОшибки),
			Новый ФорматированнаяСтрока("https://its.1c.ua/db/metoddevu#content:8021071",,,, 
				"https://its.1c.ua/db/metoddevu#content:8021071"), ".");			
...
Функция ЗагрузитьКомпонентуИзФайлаНаСервере(ПараметрыЗагрузки)
		...
		Если ОбъектСправочника.Идентификатор <> Информация.Реквизиты.Идентификатор Тогда 
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Текущий идентификатор %1 отличается от загружаемого %2
                         ...
                         |;uk='Поточний ідентифікатор %1 відрізняється від завантажуваного %2
                         |Оновлення неможливо.'"),
	...
	ОбъектСправочника.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Загружена из файла %1. %2.';uk='Завантажена з файлу %1. %2.'"),
