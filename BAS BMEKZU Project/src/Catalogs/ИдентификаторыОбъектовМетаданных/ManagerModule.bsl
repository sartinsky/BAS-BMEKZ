...
Функция ИдентификаторыОбъектовМетаданных(ПолныеИменаОбъектовМетаданных, КромеНесуществующих = Ложь, ОдинЭлемент = Ложь) Экспорт
	...
	Для Каждого ПолноеИмя Из ПолныеИменаОбъектовМетаданных Цикл
...
Функция ОбъектМетаданныхПоИдентификатору(Идентификатор, КромеНесуществующихИНедоступных = Ложь) Экспорт
	...
	ОбъектыМетаданных = ОбъектыМетаданныхПоИдентификаторам(Идентификаторы, КромеНесуществующихИНедоступных);
...
Функция ОбъектыМетаданныхПоИдентификаторам(Идентификаторы, КромеНесуществующихИНедоступных = Ложь) Экспорт
	...
	Для Каждого Идентификатор Из Идентификаторы Цикл
		...
		Если Описание = Неопределено
		 Или Не КромеНесуществующихИНедоступных
	...
	ОбъектыМетаданныхПоИдентификаторам = ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(ИдентификаторыБезКэша,
		КромеНесуществующихИНедоступных);
...
// Возвращает ссылки на объекты метаданных, найденный по полному имени удаленного объекта метаданных.
	   ...
	   И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ТекстОшибки =
			НСтр("ru='Невозможно обновить идентификаторы объектов расширений
                       ...
                       |;uk='Неможливо оновити ідентифікатори об''єктів розширень
                       |в сеансі нерозподіленого користувача, так як
                       |розширення для розділених користувачів не підключені.'");
...
Функция ИдентификаторОбъектаМетаданных(ОписаниеОбъектаМетаданных) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных().
                       |
                       |Указан недопустимый тип параметра ОписаниеОбъектаМетаданных:
                       |""%1"".'
                       |;uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторОбъектаМетаданных().
                       |
                       |Вказаний некоректний параметр ОписаниеОбъектаМетаданных:
                       |""%1"".'"),
	...
	Если СтароеИмяКоллекции <> НовоеИмяКоллекции Тогда
		ВызватьИсключение ЗаголовокОшибки + Символы.ПС + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не совпадают имена типов переименованного объекта метаданных.
                       |Прежний тип: ""%1"",
                       |новый тип: ""%2"".'
                       |;uk='Не збігаються імена типів переіменованого об''єкта метаданих.
                       |Колишній тип: ""%1"",
                       |новий тип: ""%2"".'"),
			СтароеПолноеИмя,
			НовоеПолноеИмя);
		   ...
		   И ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ() Тогда
			...
			ВызватьИсключениеПоОшибке(ОбъектыРасширений,
				НСтр("ru='Критичные изменения могут быть выполнены только
                           ...
                           |;uk='Критичні зміни можуть бути виконані тільки
                           |в головному вузлі розподіленої інформаційної бази.
                           |Склад необхідних змін див. у журналі реєстрації.'"));
		КонецЕсли;
...
Функция ИдентификаторыОбъектовМетаданныхБезПопыткиПовтора(ПолныеИменаОбъектовМетаданных, КромеНесуществующих, ОдинЭлемент)
		...
		Если НайденныеСтроки.Количество() = 0 Тогда
			   ...
			   И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Идентификаторы объектов метаданных расширений не поддерживаются в неразделенном режиме.
                               ...
                               |;uk='Ідентифікатори об''єктів метаданих розширень не підтримуються в невідомою режимі.
                               |Неможливо повернути ідентифікатор об''єкта метаданих ""%1""
                               |розширення ""%2"" версії ""%3"".'"),
			...
			ШаблонОшибки = ?(Расширение <> Неопределено,
				НСтр("ru='Для объекта метаданных ""%1""
                           ...
                           |не знайдено ідентифікатор в регістрі відомостей ""Ідентифікатори об''єктів розширень"".'"),
				НСтр("ru='Для объекта метаданных ""%1""
	...
	Если КоличествоОшибок > 0 Тогда
		...
		ИначеЕсли КоличествоОшибок = 1 Тогда
			ЗаголовокОшибки = НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().';uk='Помилка при виконанні функції ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().'");
		Иначе
			ЗаголовокОшибки = НСтр("ru='Ошибки при выполнении функции ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().';uk='Помилки при виконанні функції ОбщегоНазначения.ИдентификаторыОбъектаМетаданных().'");
		КонецЕсли;
		...
		Для Каждого ОписаниеОшибки Из Ошибки Цикл
			...
			Если НомерОшибки = 3 И КоличествоОшибок > 5 Тогда
				ПредставлениеКоличества = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
					НСтр("ru=';%1 ошибка;; %1 ошибки; %1 ошибок; %1 ошибки';uk=';%1 помилка;; %1 помилки; %1 помилок; %1 помилки'"), (КоличествоОшибок - НомерОшибки));
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='... И еще %1';uk='... І ще %1'"),
...
Функция ОбъектыМетаданныхПоИдентификаторамСПопыткойПовтора(Идентификаторы, КромеНесуществующихИНедоступных)
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().
                           ...
                           |""%1"".'
                           |;uk='Помилка при виконанні функції ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору().
                           |
                           |Невірний тип ідентифікатора об''єкта метаданих:
                           |""%1"".'"),
	...
	Попытка
		ОбъектыМетаданных = ОбъектыМетаданныхПоИдентификаторамБезПопыткиПовтора(Идентификаторы,
			ИдентификаторыКонфигурации, ИдентификаторыРасширений, КромеНесуществующихИНедоступных);
	Исключение
		Если Не ОбщегоНазначения.РазделениеВключено()
		 Или Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			// В случае ошибки, если есть возможность обновления справочника,
	...
	Если Выгрузка.Количество() < ВсегоИдентификаторов Тогда
		Для Каждого Идентификатор Из Идентификаторы Цикл
			Если Выгрузка.Найти(Идентификатор, "Ссылка") = Неопределено Тогда
				Если КромеНесуществующихИНедоступных Тогда
			...
			КонецЕсли;
		КонецЦикла;
		Если Не КромеНесуществующихИНедоступных Тогда
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Для Каждого Свойства Из Выгрузка Цикл
		...
		Если РезультатПроверки.НеСоответствует Тогда
			...
			Если РезультатПроверки.ОбъектМетаданных = Неопределено Тогда
						...
						Если РасширенияКонфигурации.Получить(Отбор, ИсточникРасширенийКонфигурации.СеансОтключенные).Количество() > 0 Тогда
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Расширение конфигурации %1 установлено, но отключено при запуске сеанса.
                                           ...
                                           |;uk='Розширення конфігурації %1 встановлено, але відключено при запуску сеансу.
                                           |Зазвичай, це означає, що при підключенні розширення конфігурації виникла помилка.'"),
								Свойства.ИмяРасширения);
						Иначе
							ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Расширение конфигурации %1 установлено, но не подключено.
                                           |Требуется перезапустить сеанс.'
                                           |;uk='Розширення конфігурації %1 встановлено, але не підключено.
                                           |Потрібно перезапустити сеанс.'"),
								Свойства.ИмяРасширения);
						КонецЕсли;
					КонецЕсли;
				...
				ИначеЕсли КромеНесуществующихИНедоступных Тогда
					// Объект метаданных не существует.
		...
		Если ТекущаяПопытка > 10 Тогда
			ВызватьИсключение
				НСтр("ru='Не удалось выполнить замену дублей идентификаторов объектов метаданных.
                           ...
                           |;uk='Не вдалося виконати заміну дублів ідентифікаторів об''єктів метаданих.
                           |Після 10 спроб все ще є дані, які потребують заміни.
                           |Виконайте дію в монопольному режимі.'");
		КонецЕсли;
		...
		Если Не БезОшибок Тогда
			ВызватьИсключение
				НСтр("ru='Не удалось выполнить замену дублей идентификаторов объектов метаданных.
                           ...
                           |;uk='Не вдалося виконати заміну дублів ідентифікаторів об''єктів метаданих.
                           |Детальніше див. помилки при заміні ідентифікатора в журналі реєстрації.'");
		КонецЕсли;
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='При записи объекта ""%1"" возникла ошибка:
                                       |%2'
                                       |;uk='При записі об''єкта ""%1"" виникла помилка:
                                       |%2'"),
						...
						Исключение
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='При записи движений объекта ""%1"" в ""%2"" возникла ошибка:
                                           |%3'
                                           |;uk='При записі рухів об''єкта ""%1"" в ""%2"" виникла помилка:
                                           |%3'"),
								ПолучитьНавигационнуюСсылку(Параметры.Объект.Ссылка),
								...
								Исключение
									...
									ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru='При записи по регистратору ""%1"" в ""%2"" возникла ошибка:
                                                       |%3'
                                                       |;uk='При записі по реєстратору ""%1"" в ""%2"" виникла помилка:
                                                       |%3'"),
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='При удалении записи ""%1"" возникла ошибка:
                                       |%2'
                                       |;uk='При вилученні запису ""%1"" виникла помилка:
                                       |%2'"),
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='При добавлении записи ""%1"" возникла ошибка:
                                       |%2'
                                       |;uk='При додаванні запису ""%1"" виникла помилка:
                                       |%2'"),
							ПолучитьНавигационнуюСсылку(МенеджерРегистра.СоздатьКлючЗаписи(СтруктураИзмерений)),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
						ВызватьИсключение;
