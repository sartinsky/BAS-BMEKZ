...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.ШаблонЗаполнения.НайтиСтроки(Новый Структура("ДеньВключенВГрафик", Истина)).Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не отмечены дни, включаемые в график работы';uk='Не відмічені дні, що включаються в графік роботи'"), , "Объект.ШаблонЗаполнения", , Отказ);
	КонецЕсли;
...
Процедура УточнитьЗаполненностьГрафика(Форма)
	...
	Иначе	
		...
		КонецЕсли;
	КонецЕсли;
	Форма.Элементы.ЗаполненностьДекорация.Картинка = ?(Форма.ТребуетЗаполнения, БиблиотекаКартинок.Предупреждение, БиблиотекаКартинок.ИнформацияБСП);
...
Процедура ЗаполнитьИнформационныйТекстРезультатаЗаполнения(Форма)
	...
	Если Форма.РучноеРедактирование Тогда
		ИнформационныйТекст = НСтр("ru='График работы на текущий год изменен вручную. 
                                    ...
                                    |;uk='Графік роботи на поточний рік змінений вручну. 
                                    |Натисніть «Заповнити за шаблоном» щоб повернутися до автоматичного заповнення.'");
	...
	Иначе
		Если Форма.РезультатЗаполненПоШаблону Тогда
			Если ЗначениеЗаполнено(Форма.Объект.ПроизводственныйКалендарь) Тогда
				...
				ИнформационнаяКартинка = БиблиотекаКартинок.ИнформацияБСП;
			КонецЕсли;
		Иначе
			ИнформационныйТекст = НСтр("ru='Отображаемый результат не соответствует настройке шаблона. 
                                        ...
                                        |;uk='Відображений результат не відповідає настройці шаблону. 
                                        |Натисніть «Заповнити за шаблоном», щоб побачити як виглядає графік роботи з урахуванням змін шаблону.'");
			ИнформационнаяКартинка = БиблиотекаКартинок.Предупреждение;
			ДоступноЗаполнениеПоШаблону = Истина;
		КонецЕсли;
	КонецЕсли;
