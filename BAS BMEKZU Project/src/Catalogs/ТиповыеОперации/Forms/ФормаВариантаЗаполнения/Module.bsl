...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтображатьНеИзменять Тогда
		Элементы.ГруппаНеИзменять.ТекущаяСтраница = Элементы.СтраницаНеИзменять;
	Иначе
		Элементы.ГруппаНеИзменять.ТекущаяСтраница = Элементы.СтраницаПустаяСтрока;
	КонецЕсли;
	...
	Если ТипЗнч(Параметры.Значение) = Тип("СправочникСсылка.ПараметрыТиповойОперации")
		...
		И Найти(Параметры.Значение," *") <> 0) Тогда
...
Процедура ПараметрНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru='Параметр не может быть изменен.';uk='Параметр не може бути змінений.'")
			+ Символы.ПС
			+ НСтр("ru='Для выбора параметра вручную необходимо записать типовую операцию';uk='Для вибору параметра вручну необхідно записати типову операцію'");
