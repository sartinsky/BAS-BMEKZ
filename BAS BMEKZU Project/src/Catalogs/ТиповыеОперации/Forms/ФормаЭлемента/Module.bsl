...
Процедура ВвестиОперацию(Команда)
	...
	Если ОбщегоНазначенияБПКлиент.НадоЗаписать(ЭтотОбъект) Тогда
		...
		Текст = НСтр("ru='Ввод операции возможен после записи данных.';uk='Введення операції можливе після запису даних.'")
			+ Символы.ПС
			+ НСтр("ru='Записать данные?';uk='Записати дані?'");
...
Процедура ХозрасчетныйПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	ТекущаяСтрока = Элементы.Хозрасчетный.ТекущиеДанные;
	
	// Создадим параметр Сумма для новой строки
	Если НоваяСтрока И НЕ ЗначениеЗаполнено(ТекущаяСтрока.Сумма) Тогда
		Параметр									 = ПараметрПоИмениТипу("Сумма", ОписаниеТиповСумма, Ложь);
		ТекущаяСтрока.Сумма							 = Параметр;
		ТекущаяСтрока.СуммаОтображение				 = "<" + Параметр + ">";
		Элементы.ХозрасчетныйСумма.ОграничениеТипа	 = Новый ОписаниеТипов;
	КонецЕсли;
	
	Если НоваяСтрока Тогда
		ЗаполнитьНадписиВПроводке(ТекущаяСтрока);
	КонецЕсли;
...
Процедура ХозрасчетныйНачалоВыбораЗначения(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИначеЕсли Найти(ИмяПоля,"НалоговоеНазначение") > 0 Тогда
		ТипЗначения = ОписаниеТиповНалоговоеНазначение;
	ИначеЕсли Найти(ИмяПоля,"Субконто") > 0 Тогда
...
Процедура ПараметрыТиповойОперацииПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Параметры.Ключ.Пустая() Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВопросЗаписатьФормуЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		Режим = РежимДиалогаВопрос.ДаНет;
		Текст = НСтр("ru='Работа с параметрами возможна после записи данных.';uk='Робота з параметрами можлива після запису даних.'")
			+ Символы.ПС
			+ НСтр("ru='Записать данные?';uk='Записати дані?'");
		ПоказатьВопрос(ОписаниеОповещения, Текст, Режим, 0);
	КонецЕсли;
...
Процедура ПодготовитьФормуНаСервере()
	...
	ОписаниеТиповНалоговоеНазначение = Новый ОписаниеТипов("СправочникСсылка.НалоговыеНазначенияАктивовИЗатрат");
...
Процедура УстановитьДоступностьИОтображениеПолейХозрасчетный()
	...
	Для Каждого Строка Из Объект.Хозрасчетный Цикл
		...
		Для Каждого ДтКт Из МассивДтКт Цикл
			...
			КонецЦикла;
			БухгалтерскийУчетКлиентСервер.УстановитьДоступностьСубконто(Строка["Счет"+ДтКт], Строка, ПоляОбъекта);
			...
			УстановитьОтображениеКолонки(Строка, "НалоговоеНазначение" + ДтКт);
...
Процедура УстановитьОтображениеКолонки(Строка, ИмяКолонки)
	...
	Если ЭтоПараметр(Значение) ИЛИ (ТипЗнч(Значение) = Тип("Строка") И Найти(Значение," *") <> 0) Тогда
		Значение = "<" + Значение + ">";
	КонецЕсли;
...
Процедура ЗаполнитьНадписиВПроводке(Проводка)
	...
	Проводка.НадписьНУДт = ?(Проводка.НалоговаяСуммаДтДоступность, НСтр("ru='НУ:';uk='ПО:'"), "");
	Проводка.НадписьНУКт = ?(Проводка.НалоговаяСуммаКтДоступность, НСтр("ru='НУ:';uk='ПО:'"), "");
...
Процедура ЗаполнитьТаблицуРегистров()
	...
	Для Каждого МетаданныеРегистра Из ДвиженияОперации Цикл
		...
		Если МетаданныеРегистра.Имя = "Хозрасчетный" Тогда
			Продолжить;
		КонецЕсли;
		...
		ИначеЕсли ИмяТипаРегистра = "РегистрСведений" Тогда
...
Процедура ОбработатьИзменениеСчета(ИдСтроки, ДтКт);
	...
	Если ДанныеСчета.НалоговыйУчет Тогда
		...
		Параметр = ПараметрПоИмениТипу("НалоговоеНазначение", ОписаниеТиповНалоговоеНазначение, Ложь);
		Строка["НалоговоеНазначение" + ДтКт]				= Параметр;
		Строка["НалоговоеНазначение" + ДтКт + "Отображение"]= "<"+Параметр+">";
	Иначе
		Строка["СуммаНУ" + ДтКт]						 = 0;
		Строка["СуммаНУ" + ДтКт + "Отображение"]		 = "";
		Строка["НалоговоеНазначение" + ДтКт]						 = 0;
		Строка["НалоговоеНазначение" + ДтКт + "Отображение"]		 = "";
	КонецЕсли;
	...
	Для Каждого СтрокаРегистра Из Регистры Цикл;
		...
		Для Каждого СтрокаТаблицы Из Таблица Цикл
			...
			Для Каждого Колонка Из Таблица.Колонки Цикл
				Если Найти(Колонка.Имя, "Отображение") <> 0 
					ИЛИ Колонка.Имя = "ВидДвижения" 
					ИЛИ Найти(Колонка.Имя, "НеИзменятьЗначение") <> 0 Тогда
					Продолжить;
				КонецЕсли;
...
Функция ОписаниеТипаВладельца(ТипПараметра)
	...
	Если ТипПараметра.СодержитТип(Тип("СправочникСсылка.ДоговорыКонтрагентов"))
		ИЛИ ТипПараметра.СодержитТип(Тип("СправочникСсылка.БанковскиеСчета")) Тогда
...
Процедура СоздатьНовыеПараметры()
		...
		Для Каждого ДтКт Из МассивДтКт Цикл;
			...
			Если ДанныеСчета.НалоговыйУчет Тогда
				Если ТипЗнч(Строка["СуммаНУ"+ДтКт]) = Тип("Строка") Тогда
					...
					ТребуетсяПерезапись			 = Истина;
				КонецЕсли;
				Если ТипЗнч(Строка["НалоговоеНазначение"+ДтКт]) = Тип("Строка") И ЗначениеЗаполнено(Строка["НалоговоеНазначение"+ДтКт]) Тогда
					Параметр					 = НайтиСоздатьПараметр("НалоговоеНазначение", ОписаниеТиповНалоговоеНазначение);
					Поле						 = "НалоговоеНазначение" + ДтКт;
					Строка[Поле]				 = Параметр;
					Строка[Поле+"Отображение"]	 = "<" + Параметр + ">";
					ТребуетсяПерезапись			 = Истина;
				КонецЕсли;
	...
	Для Каждого Строка Из Объект.ДвиженияРегистров Цикл
		Если ТипЗнч(Строка.Значение) = Тип("Строка") И Найти(Строка.Значение, " *")<>0 Тогда
...
Процедура УстановитьВыбранноеЗначениеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ТабличнаяЧасть = "Хозрасчетный" Тогда
		...
		Если Аналитика = "Сумма" Тогда
			...
			КонецЕсли;
		ИначеЕсли Найти(Аналитика, "Субконто") > 0 Тогда
			...
			Если ТипЗначения = Тип("СправочникСсылка.Контрагенты") Тогда
				// Очистим ДоговорКонтрагента или БанковскийСчет
				Для НомерСубконто = 1 По 3 Цикл
					Субконто = "Субконто" + ДтКт + НомерСубконто;
					ТипСубконто = ТипЗнч(Строка[Субконто]);
					Если ТипСубконто = Тип("СправочникСсылка.ДоговорыКонтрагентов")
						ИЛИ ТипСубконто = Тип("СправочникСсылка.БанковскиеСчета") Тогда
