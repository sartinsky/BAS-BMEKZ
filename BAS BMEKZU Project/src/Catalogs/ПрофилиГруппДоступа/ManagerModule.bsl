	...
	Если СвойстваПрофиля = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Профиль c идентификатором ""%1""
                       ...
                       |;uk='Профіль з ідентифікатором ""%1""
                       |не поставляється в програмі.'"),
	...
	Если ВызыватьИсключениеЕслиНетВБазеДанных Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Поставляемый профиль с идентификатором ""%1""
                       ...
                       |;uk='Поставляється профіль з ідентифікатором ""%1""
                       |не знайдений в інформаційній базі.'"),
			Строка(Идентификатор));
	КонецЕсли;
...
Функция ПоставляемыеПрофили()
	...
	ОписаниеПрофиляАдминистратор.Описание =
		НСтр("ru='Предназначен для:
                   ...
                   |;uk='Призначений для:
                   |- настройки параметрів роботи та обслуговування інформаційної системи,
                   |- настройки прав доступу інших користувачів,
                   |- вилучення відмічених об''єктів,
                   |- в окремих випадках для внесення змін у конфігурацію.
                   |
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		Если Не ЗначениеЗаполнено(ОписаниеПрофиля.Идентификатор) Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В описании профиля ""%1"" не заполнено свойство Идентификатор.';uk='В описі профілю ""%1"" не заповнено властивість Ідентифікатор.'"),
		...
		ИначеЕсли Не СтроковыеФункцииКлиентСервер.ЭтоУникальныйИдентификатор(ОписаниеПрофиля.Идентификатор) Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В описании профиля ""%1"" указан некорректный идентификатор: ""%2"".';uk='В описі профілю ""%1"" вказано некоректний ідентифікатор: ""%2"".'"),
				?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Наименование),
