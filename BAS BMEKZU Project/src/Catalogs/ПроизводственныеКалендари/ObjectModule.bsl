...
Процедура ПроверитьИспользованиеБазовогоКалендаря(Отказ)
	...
	Если Ссылка = БазовыйКалендарь Тогда
		ТекстСообщения = НСтр("ru='В качестве базового не может быть выбран тот же самый календарь.';uk='В якості базового не може бути обраний той же самий календар.'");
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Календарь уже является базовым для календаря «%1» и не может зависеть от другого.';uk='Календар вже є базовим для календаря «%1» і не може залежати від іншого.'"),
		Выборка.Ссылка);
