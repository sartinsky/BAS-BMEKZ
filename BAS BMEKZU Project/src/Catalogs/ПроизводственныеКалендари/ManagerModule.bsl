...
Процедура ОбновитьПроизводственныеКалендари(ТаблицаКалендарей) Экспорт
	...
	Иначе	
		...
		КодЯзыкаИБ = МодульЛокализация.КодЯзыкаИнформационнойБазы();
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|	ВЫРАЗИТЬ(КлассификаторТаблица.DescriptionUkr КАК СТРОКА(100)) КАК НаименованиеУкр
		...
		|	КлассификаторТаблица.НаименованиеУкр КАК НаименованиеУкр,
	...
	Пока Выборка.Следующий() Цикл
		Если СокрЛП(Выборка.Код) = СокрЛП(Выборка.ПроизводственныйКалендарьКод)
			И СокрЛП(Выборка.Наименование) = СокрЛП(Выборка.ПроизводственныйКалендарьНаименование) Тогда
			Продолжить;
		КонецЕсли;
		...
		Если КодЯзыкаИБ = "uk" Тогда
			СправочникОбъект.Наименование = СокрЛП(Выборка.НаименованиеУкр);
		Иначе
...
Функция ПроизводственныеКалендариИзМакета() Экспорт
	...
	ТекстовыйДокумент = ПолучитьМакет("ПроизводственныеКалендари");
...
Функция ПраздничныеДниПроизводственногоКалендаря(КодПроизводственногоКалендаря, НомерГода)
	...
	Если КодПроизводственногоКалендаря = "УК" Тогда		
		...
		// 1 января - Новый год
		ДобавитьПраздничныйДень(ПраздничныеДни, 1,	1, НомерГода);
		
		// 7 января - Рождество Христово
		ДобавитьПраздничныйДень(ПраздничныеДни, 7,	1, НомерГода);
		...
		Если НомерГода <= 2017 Тогда
			ДобавитьПраздничныйДень(ПраздничныеДни, 2,	5, НомерГода);
		КонецЕсли; 
		...
		Иначе
			ДобавитьПраздничныйДень(ПраздничныеДни, 8,	5, НомерГода);
		КонецЕсли; 
		
		// 28 июня - День Конституции
		ДобавитьПраздничныйДень(ПраздничныеДни, 28,	6, НомерГода);
		
		Если НомерГода >= 2022 Тогда
			// 28 липня - День Української Державності
			ДобавитьПраздничныйДень(ПраздничныеДни, 28, 7, НомерГода);
		КонецЕсли;

		// 24 августа - День Независимости
		ДобавитьПраздничныйДень(ПраздничныеДни, 24,	8, НомерГода);
...
Процедура ДобавитьПасхуТроицу(ПраздничныеДни, ПраздничныйДень, НомерГода, ПереноситьВыходной = Истина, ДобавлятьПредпраздничный = Истина, ТолькоНерабочий = Ложь)
