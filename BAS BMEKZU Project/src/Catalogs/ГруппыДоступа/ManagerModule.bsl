	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось
                       ...
                       |;uk='Процедурі Довідники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюверсию не вдалося
                       |оновити допоміжні дані деяких груп доступу (пропущені): %1'"), 
			ПроблемныхОбъектов);
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			НСтр("ru='Процедура Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию
                       ...
                       |;uk='Процедура Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюверсию
                       |опрацювала чергову порцію груп доступу: %1'"),
		ОбъектовОбработано));
	КонецЕсли;
...
Функция СвойстваВидаСсылок(ВидСсылок)
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Недопустимое значение ""%1"" параметра ВидСсылок функции СвойстваВидаСсылок.';uk='Неприпустиме значення ""%1"" параметра ВидСсылок функції СвойстваВидаСсылок.'"),
			ВидСсылок);
	КонецЕсли;
