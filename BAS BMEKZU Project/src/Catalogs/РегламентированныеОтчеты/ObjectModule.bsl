...
Процедура ПередЗаписью(Отказ)
	...
	Если Не ЭтоГруппа Тогда
		Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Или ОбщегоНазначенияКлиентСервер.РежимОтладки() Тогда   	
			КешСхемЗвит1САктуален = Ложь;
			КешСхемЗвит1С.Очистить();
			Справочники.РегламентированныеОтчеты.ДополнитьКешТаблицу(Ссылка, КешСхемЗвит1С);
			КешСхемЗвит1САктуален = Истина;
		Иначе
			КешСхемЗвит1САктуален = Ложь;
			КешСхемЗвит1С.Очистить();
		КонецЕсли;
