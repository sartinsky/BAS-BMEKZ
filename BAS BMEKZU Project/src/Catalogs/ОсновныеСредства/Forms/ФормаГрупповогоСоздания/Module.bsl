...
Процедура Создать(Команда)
	...
	Если НЕ ЗначениеЗаполнено(НачальныйКод) Тогда
		ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(, , НСтр("ru='Присваивать коды с';uk='Присвоювати коди з'"));
	...
	Иначе
		...
		Если НЕ ЗначениеЗаполнено(ЧисловаяЧасть) Тогда
			ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(, "КОРРЕКТНОСТЬ", НСтр("ru='Присваивать коды с';uk='Присвоювати коди з'"),,,
				НСтр("ru='Указанный в поле код не содержит числовой части.';uk='Зазначений в поле код не містить числової частини.'"));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "НачальныйКод");
			ЕстьОшибкиЗаполнения = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Количество) Тогда
		ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(, , НСтр("ru='Количество элементов';uk='Кількість елементів'"));
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ЧисловаяЧасть) И Количество > МаксимумСоздаваемыхОбъектов(ЧисловаяЧасть) Тогда
			ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(, "КОРРЕКТНОСТЬ", НСтр("ru='Количество элементов';uk='Кількість елементів'"),,,
				НСтр("ru='Введенное количество объектов превышает допустимое исходя из разрядности начального кода.';uk=""Введена кількість об'єктів перевищує допустиму виходячи з розрядності початкового коду."""));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Количество");
			ЕстьОшибкиЗаполнения = Истина;
		КонецЕсли;
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Наименование) Тогда
		ТекстСообщения = ОбщегоНазначенияБПКлиентСервер.ПолучитьТекстСообщения(, , НСтр("ru='Наименование';uk='Найменування'"));
	...
	Если СоздатьЭлементыСправочника() Тогда
		ШаблонСообщения = НСтр("ru='Созданы элементы справочника ""Основные средства"": %1';uk='Створені елементи довідника ""Основні засоби"": %1'");
	...
	Иначе
		ТекстСообщенияОбОшибке = НСтр("ru='Групповое добавление элементов справочника не выполнено.';uk='Групове додавання елементів довідника не виконано.'");
		ПоказатьПредупреждение( , ТекстСообщенияОбОшибке, , НСтр("ru='Ошибка';uk='Помилка'"));
	КонецЕсли;
...
|%2';uk='Не вдалося записати елемент довідника з кодом %1
|%2'");
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Выполнение операции';uk='Опрацювання операції'"),
