...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если (ВидФормы = "ФормаОбъекта" И Не Параметры.Свойство("ЗначениеКопирования")
		И (Не Параметры.Свойство("Ключ") Или Не РаботаСПочтовымиСообщениямиВызовСервера.УчетнаяЗаписьНастроена(Параметры.Ключ)))
		И ПравоДоступа("Редактирование", Метаданные.Справочники.УчетныеЗаписиЭлектроннойПочты) Тогда
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Исключение
		ОтменитьТранзакцию();
		ЗаписьЖурналаРегистрации(НСтр("ru='Обновление персональных учетных записей';uk='Оновлення персональних облікових записів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ПроверитьПодключениеКСерверуИсходящейПочты(Профиль, АдресЭлектроннойПочты)
	...
	Тема = НСтр("ru='Тестовое сообщение информационной системы';uk='Тестове повідомлення інформаційної системи'");
	Тело = НСтр("ru='Это сообщение отправлено подсистемой электронной почты информационной системы';uk='Це повідомлення надіслано підсистемою електронної пошти інформаційної системи'");
	ИмяОтправителяПисем = НСтр("ru='Информационная система';uk='Інформаційна система'");
