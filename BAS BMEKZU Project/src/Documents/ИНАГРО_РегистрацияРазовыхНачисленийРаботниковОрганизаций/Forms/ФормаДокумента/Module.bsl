...
Процедура НачисленияВидРасчетаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.ДатаНачала) И ЗначениеЗаполнено(ТекущиеДанные.ДатаОкончания) Тогда 
		ПремияТекущегоПериода = ЭтоПремияЗатекущийПериод(ТекущиеДанные.ВидРасчета); 
		
		Если ПремияТекущегоПериода Тогда
			ТекущиеДанные.БазовыйПериодНачало = НачалоМесяца(ТекущиеДанные.ДатаНачала);
			ТекущиеДанные.БазовыйПериодКонец = КонецМесяца(ТекущиеДанные.ДатаНачала);
		Иначе
...
&НаСервере
Функция ЭтоПремияЗатекущийПериод(ВидРасчета)
