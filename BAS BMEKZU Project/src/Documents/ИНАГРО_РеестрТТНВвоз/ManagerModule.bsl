...
Функция ПечатьЗХС4(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	|	ИНАГРО_РеестрТТНВвозСписокТТН.Вес КАК Вес,
	|	ИНАГРО_РеестрТТНВвозСписокТТН.ТТН КАК ТТН,
	|	ИНАГРО_РеестрТТНВвозСписокТТН.ТТН.Влажность КАК Влажность,
	|	ИНАГРО_РеестрТТНВвозСписокТТН.ТТН.Сорность КАК СорнаяПримесь,
	|	ИНАГРО_РеестрТТНВвозСписокТТН.ТТН.ЗерноваяПримесь КАК ЗерноваяПримесь
	|ИЗ
	|	Документ.ИНАГРО_РеестрТТНВвоз.СписокТТН КАК ИНАГРО_РеестрТТНВвозСписокТТН
	|ГДЕ
	|	ИНАГРО_РеестрТТНВвозСписокТТН.Ссылка = &ТекущийДокумент" ; 
	...
	ЛабораторныйАнализТТН = Новый Запрос;
	ЛабораторныйАнализТТН.Текст =  
	"ВЫБРАТЬ
	|	ИНАГРО_ЛабораторныйАнализРезультаты.НомерСтроки КАК НомерСтроки,
	|	ИНАГРО_ЛабораторныйАнализРезультаты.Характеристика КАК Характеристика,
	|	ИНАГРО_ЛабораторныйАнализРезультаты.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	...
	|	Документ.ИНАГРО_РеестрТТНВвоз.СписокТТН КАК ИНАГРО_РеестрТТНВвозСписокТТН
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.ИНАГРО_ЛабораторныйАнализ.Результаты КАК ИНАГРО_ЛабораторныйАнализРезультаты
	|		ПО ИНАГРО_РеестрТТНВвозСписокТТН.ТТН.ЛабораторныйАнализ = ИНАГРО_ЛабораторныйАнализРезультаты.Ссылка
	|ГДЕ
	|	ИНАГРО_РеестрТТНВвозСписокТТН.Ссылка = &ТекущийДокумент
	|	И ИНАГРО_ЛабораторныйАнализРезультаты.Характеристика = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ИНАГРО_ХарактеристикиВидовКультур.Натура)";
	...
	Для каждого Ссылка Из МассивОбъектов Цикл
		...
		ЛабораторныйАнализТТН.УстановитьПараметр("ТекущийДокумент", Ссылка);
		ЛабораторныйАнализТТНРезультаты = ЛабораторныйАнализТТН.Выполнить().Выгрузить();
		...
		//	Шапка.Параметры.Регион = РезультатШапка.ПолеТок.Регион;
		//КонецЕсли;
		СведенияОПокупателе = БухгалтерскийУчетПереопределяемый.СведенияОЮрФизЛице(РезультатШапка.Владелец, РезультатШапка.Дата);
		Шапка.Параметры.Регион = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОПокупателе,  "ЮридическийАдрес,",  Ложь);
		...
		Для Каждого СтрСпискаТТН из СписокТТНРезультат Цикл
			...
			ОблСтрока.Параметры.ПрНат   = 0;
			ОблСтрока.Параметры.ЦПНат   = 0;
			
			Влажность_ = ?(ЗначениеЗаполнено(СтрСпискаТТН.Влажность),СтрСпискаТТН.Влажность,РезультатШапка.Влажность); 
			ОблСтрока.Параметры.ПрВолог = Влажность_;
			ОблСтрока.Параметры.ЦПВол = Окр(СтрСпискаТТН.Вес*Влажность_/100,0);
			ИтЦПВол = ИтЦПВол + ОблСтрока.Параметры.ЦПВол;
			
			СорнаяПримесь_ = ?(ЗначениеЗаполнено(СтрСпискаТТН.СорнаяПримесь),СтрСпискаТТН.СорнаяПримесь,РезультатШапка.СорнаяПримесь); 
			ОблСтрока.Параметры.ПрСм = СорнаяПримесь_;
			ОблСтрока.Параметры.ЦПСм = Окр(СтрСпискаТТН.Вес*СорнаяПримесь_/100,0);
			ИтЦПСм = ИтЦПСм + ОблСтрока.Параметры.ЦПСм;
			
			ЗерноваяПримесь_ = ?(ЗначениеЗаполнено(СтрСпискаТТН.ЗерноваяПримесь),СтрСпискаТТН.ЗерноваяПримесь,РезультатШапка.ЗерноваяПримесь); 
			ОблСтрока.Параметры.ПрЗер = ЗерноваяПримесь_;
			ОблСтрока.Параметры.ЦПЗер = Окр(СтрСпискаТТН.Вес*ЗерноваяПримесь_/100,0);
			ИтЦПЗер = ИтЦПЗер + ОблСтрока.Параметры.ЦПЗер;
			
			// Если в ТТН из СпискаТТН заполнен Лаб.анализ - берем значение характеристики из него, если нет - берем из Лаб.анализа из РеестраТТН
			Если ЗначениеЗаполнено(СтрСпискаТТН.ТТН) И ЗначениеЗаполнено(СтрСпискаТТН.ТТН.ЛабораторныйАнализ) Тогда
				Для Каждого СтрАнал из ЛабораторныйАнализТТНРезультаты Цикл
					Если СтрАнал.ТТН = СтрСпискаТТН.ТТН Тогда 
			...
			Иначе
				Для Каждого СтрАнал из ЛабораторныйАнализРезультаты Цикл
					Если СтрАнал.Характеристика = ПланыВидовХарактеристик.ИНАГРО_ХарактеристикиВидовКультур.Натура Тогда
						ОблСтрока.Параметры.ПрНат = СтрАнал.Значение;
						ОблСтрока.Параметры.ЦПНат = Окр(СтрСпискаТТН.Вес*СтрАнал.Значение/100,0);
						ИтЦПНат = ИтЦПНат + ОблСтрока.Параметры.ЦПНат;
					КонецЕсли;	
