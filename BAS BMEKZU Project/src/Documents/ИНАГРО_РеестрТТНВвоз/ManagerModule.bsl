   ...
   УстановитьПривилегированныйРежим(Истина);
    ...
    Для каждого Ссылка Из МассивОбъектов Цикл
        ...
        Для Каждого СтрСпискаТТН из СписокТТНРезультат Цикл
			...
			ОблСтрока.Параметры.ПрВолог = СтрСпискаТТН.Влажность;
			ОблСтрока.Параметры.ЦПВол = Окр(СтрСпискаТТН.Вес*СтрСпискаТТН.Влажность/100,0);
			ИтЦПВол = ИтЦПВол + ОблСтрока.Параметры.ЦПВол;
			
			ОблСтрока.Параметры.ПрСм = СтрСпискаТТН.СорнаяПримесь;
			ОблСтрока.Параметры.ЦПСм = Окр(СтрСпискаТТН.Вес*СтрСпискаТТН.СорнаяПримесь/100,0);
			ИтЦПСм = ИтЦПСм + ОблСтрока.Параметры.ЦПСм;
			
			ОблСтрока.Параметры.ПрЗер = СтрСпискаТТН.ЗерноваяПримесь;
			ОблСтрока.Параметры.ЦПЗер = Окр(СтрСпискаТТН.Вес*СтрСпискаТТН.ЗерноваяПримесь/100,0);
