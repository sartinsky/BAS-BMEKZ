...
Функция СформироватьЗапросДляПечати(Режим, Ссылка, масСотрудники = Неопределено)
	...
	Если Режим = "ПоРеквизитамДокумента" Тогда
		...
		|					ТОГДА "" "" + ФИОФизическихЛицСрезПоследних.Имя
		|				ИНАЧЕ """"
		|			КОНЕЦ + ВЫБОР
		|				КОГДА ПОДСТРОКА(ФИОФизическихЛицСрезПоследних.Отчество, 1, 1) <> """"
		|					ТОГДА "" "" + ФИОФизическихЛицСрезПоследних.Отчество
		|				ИНАЧЕ """"
		|			КОНЕЦ
...
Функция ПечатьДоговорНаВыполнениеРаботСФизЛицом(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
	...
	Для каждого Ссылка Из МассивОбъектов Цикл
		...
		Пока ВыборкаРаботники.Следующий() Цикл
			...
			ОбластьМакета.Параметры.НомерДок = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаДляШапки.НомерДок);
			
			// Данные по работнику.
			ОбластьМакета.Параметры.Заполнить(ВыборкаРаботники);
			ОбластьМакета.Параметры.ДатаДок = Формат(ВыборкаДляШапки.ДатаДок, "ДФ=dd.MM.yyyy");
			ОбластьМакета.Параметры.ФИОРуководителя = ФизическиеЛицаКлиентСервер.ИмяФамилияВФорматеДСТУ(ОбщегоНазначенияБПВызовСервера.ДанныеФизЛица(Ссылка.Организация,ОтветственныеЛица.Руководитель,Ссылка.Дата));
