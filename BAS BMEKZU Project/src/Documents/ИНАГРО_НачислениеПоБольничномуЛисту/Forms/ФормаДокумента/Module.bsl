...
Процедура НомерСлучаяНетрудоспособностиПриИзмененииНаСервере()
	ПервичныйБольничныйЛист = НайтиПервичный(Объект.НомерСлучаяНетрудоспособности, Объект.Сотрудник);
	ЗаполнитьПоДаннымПервичногоБЛ();
...
Функция НайтиПервичный(НомерСлучая, Сотрудник) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	Документ.ИНАГРО_НачислениеПоБольничномуЛисту КАК НачислениеПоБольничномуЛисту
	               |ГДЕ
				   ...
				   |	И (НачислениеПоБольничномуЛисту.Сотрудник = &Сотрудник ИЛИ &Сотрудник = ЗНАЧЕНИЕ(Справочник.Сотрудники.ПустаяСсылка)) 
	               |
	               |УПОРЯДОЧИТЬ ПО
