...
Функция СформироватьЗапросСостояния()
	...
	|	ВЫБОР
	|		КОГДА ПродолжениеИнтервала.НомерСтроки ЕСТЬ NULL
	|			ТОГДА ТЧНачисления.ПроизошедшееСобытие
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ПроизошедшееСобытие
	|ИЗ
	|	Документ.ИНАГРО_НачислениеПоБольничномуЛисту.Начисления КАК ТЧНачисления
	...
	|		ИЗ
	|			Документ.ИНАГРО_НачислениеПоБольничномуЛисту.Начисления КАК Строки
	|		ГДЕ
	|			Строки.Ссылка = &ДокументСсылка) КАК ДругиеСтроки
	|		ПО (ДругиеСтроки.Сотрудник = ТЧНачисления.Сотрудник)
	|			И (ДругиеСтроки.Назначение = ТЧНачисления.Назначение)
	|			И (ДругиеСтроки.НомерСтроки < ТЧНачисления.НомерСтроки)
	|			И (ДругиеСтроки.ДатаНачала = ТЧНачисления.ДатаНачала)
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			Строки.НомерСтроки КАК НомерСтроки,
	|			Строки.Сотрудник КАК Сотрудник,
	|			Строки.Состояние КАК Состояние,
	|			Строки.ДатаНачала КАК ДатаНачала,
	|			Строки.Назначение КАК Назначение
	|		ИЗ
	|			Документ.ИНАГРО_НачислениеПоБольничномуЛисту.Начисления КАК Строки
	|		ГДЕ
	|			Строки.Ссылка = &ДокументСсылка) КАК ПродолжениеИнтервала
	|		ПО (ПродолжениеИнтервала.Сотрудник = ТЧНачисления.Сотрудник)
	|			И ТЧНачисления.Назначение = ПродолжениеИнтервала.Назначение
...
Функция АвтозаполнениеРасчетСреднего() Экспорт
	...
	|			ИЛИ Начисления.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_Начисления.ПростойСПриостановкойТрудДог)
	|			ИЛИ Начисления.ВидРасчета = ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_Начисления.МобилизацияБезОплаты)
	|			ТОГДА 1
...
Функция РассчитатьСреднююПорядок1266( КомментироватьРасчет = Ложь ) Экспорт	
	...
	ЗапросДопустимыеНеявки.Текст = 
		...
		|	ИЛИ ИНАГРО_Начисления.Ссылка = ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_Начисления.ПростойСПриостановкойТрудДог)
		|	ИЛИ ИНАГРО_Начисления.Ссылка = ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_Начисления.МобилизацияБезОплаты)
		|";
	...
	Если Истина Тогда
		...
		|			И	Начисления.ВидРасчета <> ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_Начисления.ПростойСПриостановкойТрудДог) 
		|			И	Начисления.ВидРасчета <> ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_Начисления.МобилизацияБезОплаты)
		|
	...
	////////////////////////////////////////////
	|ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|	ИНАГРО_НачисленияДанныеГрафика.Сотрудник КАК Сотрудник,
	|	ИНАГРО_НачисленияДанныеГрафика.Назначение КАК Назначение,
	...
	|	СУММА(ВЫБОР
	|			КОГДА ИНАГРО_НачисленияДанныеГрафика.Сторно
	|				ТОГДА -1
	|			ИНАЧЕ 1
	|		КОНЕЦ * ИНАГРО_НачисленияДанныеГрафика.ПроизводственныйКалендарьКалендарныеДниСПраздникамиФактическийПериодДействия) КАК ДопустимыеНеявки
	|ИЗ
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	ИНАГРО_НачисленияДанныеГрафика.Сотрудник,
	|	ИНАГРО_НачисленияДанныеГрафика.Назначение,
	...
	|			И	Неявки.Сотрудник = Неявки.Назначение
	//////////////////////////////////////////////////
	|
...
Процедура ПолучитьПериодИПрочиеПараметрыСреднейПоРаботнику() 
	...
	Если ВыборкаПараметры.Следующий() Тогда
		...
		Если ПричинаНетрудоспособности = Справочники.ИНАГРО_ПричиныНетрудоспособности.COVID_19 Тогда
			ПроцентОплаты = 50
		Иначе	
			ПроцентОплаты	= ?(ПричинаНетрудоспособности = Справочники.ИНАГРО_ПричиныНетрудоспособности.ПоБеременностиИРодам 
			ИЛИ ПричинаНетрудоспособности = Справочники.ИНАГРО_ПричиныНетрудоспособности.ТравмаНаПроизводстве
			ИЛИ ПричинаНетрудоспособности = Справочники.ИНАГРО_ПричиныНетрудоспособности.ТравмаНаПроизводстве2023
			ИЛИ ПричинаНетрудоспособности = Справочники.ИНАГРО_ПричиныНетрудоспособности.ПрофЗаболевание2023, 100, ВыборкаПараметры.ПроцентОплаты);
		КонецЕсли;
		// - Стаж
