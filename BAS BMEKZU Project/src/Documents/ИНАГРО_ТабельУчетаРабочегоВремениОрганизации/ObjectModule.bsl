...
Функция Автозаполнение(Форма, ПостроительЗапроса = Неопределено, Сотрудники = Неопределено, ТекущаяСтрока = Неопределено) Экспорт
	...
	ЗапросТабель.УстановитьПараметр("ПоВечерним", Перечисления.ИНАГРО_ВидыУчетаВремени.ПоВечернимЧасам);
	ЗапросТабель.УстановитьПараметр("ПоНочным", Перечисления.ИНАГРО_ВидыУчетаВремени.ПоНочнымЧасам); 

	ЗапросТабель.УстановитьПараметр("КлассификаторЯ", Справочники.ИНАГРО_КлассификаторИспользованияРабочегоВремени.Работа);
	ЗапросТабель.УстановитьПараметр("КлассификаторВЧ", Справочники.ИНАГРО_КлассификаторИспользованияРабочегоВремени.Вечерние);
	ЗапросТабель.УстановитьПараметр("КлассификаторНЧ", Справочники.ИНАГРО_КлассификаторИспользованияРабочегоВремени.Ночные);
	...
	ЗапросТабель.Текст = "Выбрать * ИЗ ВТСотрудники";
	...
	ЗапросТабель.Текст = "ВЫБРАТЬ
		 ...
		 |	Основной.Период";
	ЗапросТабель.Выполнить();
    ЗапросТабель.Текст = "Выбрать * ИЗ ВТГрафикиПоПериодам";	
	...
	ЗапросТабель.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
						 ...
						 //|	ВЫБОР
						 //|		КОГДА РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(СостояниеРаботников.Период, ДЕНЬ), СостояниеРаботников.Период, СЕКУНДА) = 1
						 //|			ТОГДА НАЧАЛОПЕРИОДА(СостояниеРаботников.Период, ДЕНЬ)
	                     ...
	                     |	И СостояниеРаботников.Сотрудник В
	                     |			(ВЫБРАТЬ
	                     |				ВТСотрудники.Назначение
	                     |			ИЗ
	                     |				ВТСотрудники)
						 ...
						 //|	МАКСИМУМ(ВТСостояниеРаботников.Период) КАК Период,
						 //|	НАЧАЛОПЕРИОДА(ВТСостояниеРаботников.Период, ДЕНЬ) КАК ПериодДень
						 //|	ВТСостояниеРаботников.Период КАК ПериодДень
						 //|ПОМЕСТИТЬ ВТПериодыДень
						 //|ИЗ
						 //|	ВТСостояниеРаботниковПред КАК ВТСостояниеРаботников
						 //|
						 //|СГРУППИРОВАТЬ ПО
						 //|	НАЧАЛОПЕРИОДА(ВТСостояниеРаботников.Период, ДЕНЬ),
						 //|	ВТСостояниеРаботников.Период,
						 //|	ВТСостояниеРаботников.Сотрудник
						 //|;
						 //|
						 //|////////////////////////////////////////////////////////////////////////////////
						 //|ВЫБРАТЬ
						 //|	ВТСостояниеРаботниковПред.Сотрудник КАК Сотрудник,
						 //|	ВТСостояниеРаботниковПред.Период КАК Период,
						 //|	ВТСостояниеРаботниковПред.Состояние КАК Состояние
						 //|ПОМЕСТИТЬ ВТСостояниеРаботников
						 //|ИЗ
						 //|	ВТСостояниеРаботниковПред КАК ВТСостояниеРаботниковПред
						 //|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПериодыДень КАК ВТПериодыДень
						 //|		ПО ВТСостояниеРаботниковПред.Сотрудник = ВТПериодыДень.Сотрудник
						 //|			И ВТСостояниеРаботниковПред.Период = ВТПериодыДень.Период";
	...
	//	|	ВТГрафикиПоПериодам.Сотрудник КАК Сотрудник,
	//	|	ВТГрафикиПоПериодам.Назначение КАК Назначение,
	...
	//	|ИЗ
	//	|	ВТГрафикиПоПериодам КАК ВТГрафикиПоПериодам
	//	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИНАГРО_РаботникиОрганизаций КАК РаботникиПрием
	//	|		ПО (РаботникиПрием.Сотрудник = ВТГрафикиПоПериодам.Назначение)
	...
	//	|		ПО (РаботникиУвольнение.Сотрудник = ВТГрафикиПоПериодам.Назначение)
	...
	//	|		ПО (ГрафикиРаботыПоВидамВремени.ГрафикРаботы = ВТГрафикиПоПериодам.ГрафикРаботы)
	...
	//	|			И (ГрафикиРаботыПоВидамВремени.Дата МЕЖДУ ВТГрафикиПоПериодам.ПериодНачало И ВТГрафикиПоПериодам.ПериодКонец)
	//	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	//	|			ИНАГРО_РабочееВремяРаботниковОрганизаций.Период КАК Период,
	...
	//	|		ПО (УчтенноеВремяРаботниковОрганизацийПоДням.Сотрудник = ВТГрафикиПоПериодам.Сотрудник)
	//	|			И (УчтенноеВремяРаботниковОрганизацийПоДням.Назначение = ВТГрафикиПоПериодам.Назначение)
	...
	//	|		ПО ВТГрафикиПоПериодам.Сотрудник = ФактическийПериодДействия.Сотрудник
	//	|			И ВТГрафикиПоПериодам.Назначение = ФактическийПериодДействия.Назначение
	...
	//	|		ПО ВТГрафикиПоПериодам.Сотрудник = ВТСостояниеСотрудников.Сотрудник
	...
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.Болезнь)
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.Командировка)
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ОсновнойОтпуск)
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ОтпускНаОбучение)
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ОтпускНаОбучениеНеоплачиваемый)
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.НеоплачиваемыйОтпускПоРазрешениюРаботодателя)
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ОтпускПоБеременностиИРодам)
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.НеявкиПоНевыясненнымПричинам)
	//	|			ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.НеотработанноеВремяПоЗаконодательству))
	...
	//	|	БукваВидаВремени";
		...
		|	ДниСотрудников.Буква,
		|	ДниСотрудников.ОсновноеЗначение,
		|	ДниСотрудников.НачалоПериода,
		|	ДниСотрудников.КонецПериода,
		|	ДниСотрудников.ПодразделениеОрганизации,
		|	ДниСотрудников.ВведенныйДокументВЦеломЗаПериод,
		|	ВЫБОР
		...
		|					ТОГДА ВЫБОР
		|							КОГДА ГрафикиРаботыПоВидамВремени.ОсновноеЗначение = 0
		|									И НЕ ВТСостояниеСотрудников.ВидВремени В (ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.Болезнь), ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ОсновнойОтпуск), ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ОтпускНаОбучение), ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ОтпускПоБеременностиИРодам))
		|								ТОГДА ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ПустаяСсылка)
		|							КОГДА ГрафикиРаботыПоВидамВремени.ВидУчетаВремени В (&ПоВечерним, &ПоНочным)
		|								ТОГДА NULL
		...
		|				ЛЕВОЕ СОЕДИНЕНИЕ ВТСостояниеСотрудников КАК ВТСостояниеСотрудников
		|				ПО ВТГрафикиПоПериодам.Сотрудник = ВТСостояниеСотрудников.Сотрудник
		|					И (ДОБАВИТЬКДАТЕ(ГрафикиРаботыПоВидамВремени.Дата, СЕКУНДА, 3) МЕЖДУ ВТСостояниеСотрудников.ПериодНачало И ВТСостояниеСотрудников.ПериодКонец)
		|		ГДЕ
		|	                НЕ ВТГрафикиПоПериодам.ПодразделениеОрганизации ЕСТЬ NULL" +?(ОтборПодразделения," И ВТГрафикиПоПериодам.ПодразделениеОрганизации = &ПодразделениеОрганизации","")+"
		...
		|					ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.ОтпускПоБеременностиИРодам)
		|					ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.НеявкиПоНевыясненнымПричинам)
		|					ИЛИ ВТСостояниеСотрудников.ВидВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.НеотработанноеВремяПоЗаконодательству))) КАК ДниСотрудников
		|		ПО (НЕ ДниСотрудников.ВидВремени ЕСТЬ NULL)
		|			И (ДниСотрудников.Сотрудник = Сотрудники.Сотрудник И ДниСотрудников.Назначение = Сотрудники.Назначение)
		|ГДЕ
		|	НЕ ДниСотрудников.ВидВремени ЕСТЬ NULL
		|
		|УПОРЯДОЧИТЬ ПО
		|	Сотрудники.Сотрудник,
		|	Сотрудники.Назначение,
...
Функция ПроверкаПравомерностиЗаписиПоПодразделению(Отказ)
	...
	Если ЗначениеЗаполнено(ПодразделениеОрганизации) Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ИНАГРО_ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремяВЦеломЗаПериод.Сотрудник КАК Сотрудник,
		|	ИНАГРО_ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремяВЦеломЗаПериод.Назначение КАК Назначение
		|ПОМЕСТИТЬ ВТСотрудники
		|ИЗ
		|	Документ.ИНАГРО_ТабельУчетаРабочегоВремениОрганизации.ОтработанноеВремяВЦеломЗаПериод КАК ИНАГРО_ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремяВЦеломЗаПериод
		|ГДЕ
		|	ИНАГРО_ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремяВЦеломЗаПериод.Ссылка = &ДокументСсылка
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ИНАГРО_ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.Сотрудник,
		|	ИНАГРО_ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.Назначение
		|ИЗ
		|	Документ.ИНАГРО_ТабельУчетаРабочегоВремениОрганизации.ОтработанноеВремя КАК ИНАГРО_ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя
		|ГДЕ
		|	ИНАГРО_ТабельУчетаРабочегоВремениОрганизацииОтработанноеВремя.Ссылка = &ДокументСсылка
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	РаботникиОрганизацийНач.ПодразделениеОрганизации.Наименование КАК ПодразделениеОрганизации,
		|	РаботникиОрганизацийНач.Сотрудник КАК Сотрудник
		...
		|				И Сотрудник В
		|					(ВЫБРАТЬ
		|						ВТСотрудники.Назначение КАК Сотрудник
		|					ИЗ
		|						ВТСотрудники КАК ВТСотрудники)) КАК РаботникиОрганизацийНач
		|ГДЕ
		|	РаботникиОрганизацийНач.ПодразделениеОрганизации = &Подразделение
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ИНАГРО_РаботникиОрганизаций.Организация = &Организация
		|	И ИНАГРО_РаботникиОрганизаций.Сотрудник В
		|			(ВЫБРАТЬ
		|				ВТСотрудники.Назначение КАК Сотрудник
		|			ИЗ
		|				ВТСотрудники КАК ВТСотрудники)
		|	И ИНАГРО_РаботникиОрганизаций.Период МЕЖДУ &НачалоМесяца И &КонецМесяца
		|	И ИНАГРО_РаботникиОрганизаций.ПодразделениеОрганизации = &Подразделение
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВТСотрудники.Сотрудник КАК Сотрудник,
		|	ВТПодразделения.ПодразделениеОрганизации КАК ПодразделениеОрганизации
		|ИЗ
		|	ВТСотрудники КАК ВТСотрудники
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТПодразделения КАК ВТПодразделения
		|		ПО ВТСотрудники.Назначение = ВТПодразделения.Сотрудник
		|ГДЕ
		|	ВТПодразделения.ПодразделениеОрганизации ЕСТЬ NULL";
		...
		Если ВыборкаРезультата.Количество() > 0 Тогда
			...
			Пока ВыборкаРезультата.Следующий() Цикл
				...
				СтрокаСотрудников = СтрокаСотрудников + Символы.ПС + ВыборкаРезультата.Сотрудник;
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрокаСотрудников);
			Возврат Ложь;
		Иначе
			Возврат Истина;
		КонецЕсли;
