...
Процедура ТоварыКоличествоПриИзменении(Элемент)
	...
	РасситатьСуммуАкцизаВСтроке(Элементы.Товары.ТекущиеДанные, Объект);
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.СтатьяДекларацииПоАкцизномуНалогу.Видимость = Форма.ТипСклада <> ПредопределенноеЗначение("Перечисление.ТипыСкладов.ОптовыйСклад");
	Элементы.ТоварыСуммаАкцизногоНалога.Видимость = ЗначениеЗаполнено(Форма.Объект.СтатьяДекларацииПоАкцизномуНалогу);
...
&НаКлиентеНаСервереБезКонтекста
Процедура РасситатьСуммуАкцизаВСтроке(СтрокаТабличнойЧасти, Объект)

	ДанныеСтроки = Новый Структура("Номенклатура, Коэффициент, Количество");
...
Процедура ПересчитатьСуммыАкцизаНаСервере()
	...
	Для каждого СтрокаТоваров Из Объект.Товары Цикл
		...
		РасситатьСуммуАкцизаВСтроке(СтрокаТоваров, Объект);
...
Функция ПолучитьСуммуАкцизногоНалога(ДанныеСтроки, ДанныеОбъекта)
	...
	Если ДанныеОбъекта.СтатьяДекларацииПоАкцизномуНалогу.Пустая() ИЛИ НЕ ДанныеСтроки.Номенклатура.ПодакцизныйТовар Тогда
		Возврат 0;
	КонецЕсли;
	
	ДанныеСтроки.Вставить("ЦенаВРознице");
	ОбработкаТабличныхЧастей.ЗаполнитьЦенуВРозницеТабЧасти(ДанныеСтроки, ДанныеОбъекта);
	
	СуммаБазы = ДанныеСтроки.ЦенаВРознице * ДанныеСтроки.Коэффициент * ДанныеСтроки.Количество;
