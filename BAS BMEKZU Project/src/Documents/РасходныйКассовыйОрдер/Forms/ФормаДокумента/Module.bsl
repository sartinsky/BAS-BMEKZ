...
Процедура ИНАГРО_ВедомостьВыплатПайщикамНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Организация) И ЗначениеЗаполнено(Объект.ИНАГРО_ВедомостьВыплатПайщикам) Тогда
		СтандартнаяОбработка = Ложь;
...
Функция СуммаВыплаченногоПаяПайщику(Знач ПлатежнаяВедомость, Знач Контрагент, Знач ДоговорКонтрагента)
	...
	Если ТипЗнч(ПлатежнаяВедомость) = Тип("ДокументСсылка.ИНАГРО_НачислениеДругихВыплат") Тогда 
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	СУММА(ИНАГРО_НачислениеДругихВыплатТовары.КВыдаче) КАК СуммаКВыплате,
