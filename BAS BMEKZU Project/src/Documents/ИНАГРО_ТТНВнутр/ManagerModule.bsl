...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
    ...
    // Проверяем, нужно ли для макета формировать табличный документ.
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Пропуск_2021") Тогда		
		// Формируем табличный документ и добавляем его в коллекцию печатных форм.
		...
		ПечатьПропускФ196_2021(МассивОбъектов, ОбъектыПечати, ПараметрыВывода),, "ОбщийМакет.ИНАГРО_ПФ_MXL_UK_Пропуск_Ф196_2021", ,Истина);
...
Функция ПечатьПропускФ196_2021(МассивОбъектов, ОбъектыПечати, ПараметрыВывода)
    ...
    ТабДокумент = Новый ТабличныйДокумент;
    ТабДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ИНАГРО_ТТНВнутр_Пропуск_2021";
    ...
    |   ИНАГРО_ТТНВнутр.ВерсияДанных КАК ВерсияДанных,
    |   ИНАГРО_ТТНВнутр.ПометкаУдаления КАК ПометкаУдаления,
    |   ИНАГРО_ТТНВнутр.Номер КАК Номер,
    |   ИНАГРО_ТТНВнутр.Дата КАК Дата,
    |   ИНАГРО_ТТНВнутр.Проведен КАК Проведен,
    |   ИНАГРО_ТТНВнутр.Вес КАК Вес,
    |   ИНАГРО_ТТНВнутр.ВесБрутто КАК ВесБрутто,
    |   ИНАГРО_ТТНВнутр.ВесНетто КАК ВесНетто,
    |   ИНАГРО_ТТНВнутр.Весовщик КАК Весовщик,
    ...
    |   ИНАГРО_ТТНВнутр.ЗачетныйВес КАК ЗачетныйВес,
    |   ИНАГРО_ТТНВнутр.ЗерноваяПримесь КАК ЗерноваяПримесь,
    |   ИНАГРО_ТТНВнутр.Комментарий КАК Комментарий,
    |   ИНАГРО_ТТНВнутр.ЛабораторныйАнализ КАК ЛабораторныйАнализ,
    |   ИНАГРО_ТТНВнутр.МестоХранения КАК МестоХранения,
    |   ИНАГРО_ТТНВнутр.Номенклатура КАК Номенклатура,
    |   ИНАГРО_ТТНВнутр.НомерАнализа КАК НомерАнализа,
    |   ИНАГРО_ТТНВнутр.ОписаниеДвиженияФормы36 КАК ОписаниеДвиженияФормы36,
    |   ИНАГРО_ТТНВнутр.Организация КАК Организация,
    |   ИНАГРО_ТТНВнутр.Ответственный КАК Ответственный,
    |   ИНАГРО_ТТНВнутр.Склад КАК Склад,
    |   ИНАГРО_ТТНВнутр.СорнаяПримесь КАК СорнаяПримесь,
    |   ИНАГРО_ТТНВнутр.УдалитьЕдиницаИзмерения КАК УдалитьЕдиницаИзмерения,
    |   ИНАГРО_ТТНВнутр.Урожай КАК Урожай,
    |   ИНАГРО_ТТНВнутр.Представление КАК Представление,
    |   ИНАГРО_ТТНВнутр.МоментВремени КАК МоментВремени,
    |   ИНАГРО_ТТНВнутр.Организация.НаименованиеПолное КАК ОрганизацияНаименованиеПолное,
    |   ИНАГРО_ТТНВнутр.Организация.Наименование КАК ОрганизацияНаименование,
    |   ИНАГРО_ТТНВнутр.Владелец.Наименование КАК ВладелецНаименование,
    |   ИНАГРО_ТТНВнутр.ВесВывоз КАК ВесВывоз,
    |   ИНАГРО_ТТНВнутр.НомерТранспорта КАК НомерТранспорта,
    |   ИНАГРО_ТТНВнутр.НомерПрицепа КАК НомерПрицепа,
    |   ИНАГРО_ТТНВнутр.Прицеп КАК Прицеп,
    |   ИНАГРО_ТТНВнутр.Силос КАК Силос,
    |   ИНАГРО_ТТНВнутр.ПриказНаПеремещение КАК ПриказНаПеремещение,
    |   ИНАГРО_ТТНВнутр.ВидОперации КАК ВидОперации,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.Номенклатура КАК НоменклатураТЧ,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.НоваяНоменклатура КАК НоваяНоменклатура,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.ВидФасовки КАК ВидФасовки,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.Количество КАК Количество,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.Вес КАК ВесТЧ,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.Урожай КАК Урожай1,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.НовыйУрожай КАК НовыйУрожай,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.НовыйСчетУчетаБУ КАК НовыйСчетУчетаБУ,
    |   ИНАГРО_ТТНВнутрСписокНоменклатуры.НомерСтроки КАК НомерСтроки
    |ИЗ
    |   Документ.ИНАГРО_ТТНВнутр КАК ИНАГРО_ТТНВнутр
    |       ЛЕВОЕ СОЕДИНЕНИЕ Документ.ИНАГРО_ТТНВнутр.СписокНоменклатуры КАК ИНАГРО_ТТНВнутрСписокНоменклатуры
    |       ПО ИНАГРО_ТТНВнутр.Ссылка = ИНАГРО_ТТНВнутрСписокНоменклатуры.Ссылка
    |ГДЕ
    |   ИНАГРО_ТТНВнутр.Ссылка В(&МассивОбъектов)
    |ИТОГИ ПО
    |   Ссылка";   
    
    ЗапросШапка.УстановитьПараметр("МассивОбъектов",МассивОбъектов);
    Шапка = ЗапросШапка.Выполнить().Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
    Пока Шапка.Следующий() Цикл  
        ...
        КвоЭкземпляров = 2;
        СтруктураСохранения = ХранилищеОбщихНастроек.Загрузить("ИНАГРО_ТТНВнутр_Пропуск_2021");	
        Если СтруктураСохранения <> Неопределено Тогда
            КвоЭкземпляров = СтруктураСохранения.ВыбранноеКоличество; 
        КонецЕсли;
        ...
        Пока НомерЭкземпляраДелНаДва <= КвоЭкземпляровПоделенноеНаДва Цикл
            ...
            ОблШапка.Параметры.Организация = Шапка.Организация.НаименованиеПолное;
            ОблШапка.Параметры.ЕДРПУ = Шапка.Организация.КодПоЕДРПОУ;
            ОблШапка.Параметры.НомерДок = Шапка.Номер;
            Если ЗначениеЗаполнено(Шапка.НомерПрицепа) Тогда
                ОблШапка.Параметры.ВодительАвтомобиль = "автомобілю № " + Строка(Шапка.НомерТранспорта) + " прицепу № " + Строка(Шапка.НомерПрицепа);
            ...
            ОблШапка.Параметры.Месяц              = МесяцПрописью(Шапка.Дата,ПараметрыВывода);;
            ОблШапка.Параметры.Год                = Формат(Шапка.Дата, "ДФ=гг");
            ОблШапка.Параметры.Часов              = Формат(Шапка.Дата, "ДФ=Ч");
            ОблШапка.Параметры.Минут              = Формат(Шапка.Дата, "ДФ=мм");
            Если Шапка.ВидОперации = Перечисления.ИНАГРО_ВидыОперацийТТНВнутр.ПеремещениеФасованного Тогда
            ...
            Если Шапка.ВидОперации = Перечисления.ИНАГРО_ВидыОперацийТТНВнутр.ПеремещениеФасованного Тогда
                ...
                Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
                    КоличествоМест = КоличествоМест +  ВыборкаДетальныеЗаписи.Количество;
                    ОблСтрока.Параметры.Ном				= ВыборкаДетальныеЗаписи.НомерСтроки;
                    ОблСтрока.Параметры.Номенклатура	= ВыборкаДетальныеЗаписи.НоменклатураТЧ;	
                    ОблСтрока.Параметры.Вес				= ВыборкаДетальныеЗаписи.ВесТЧ;
                    ОблСтрока.Параметры.КвоМест		    = ВыборкаДетальныеЗаписи.Количество;
                    ОблСтрока.Параметры.ВидФасовки		= ВыборкаДетальныеЗаписи.ВидФасовки;
                    ВывестиОбластьПропуска(ТабДокумент, ОблСтрока, КвоЭкземпляровНаСтраницу,КоличествоНапечатанныхЭкземпляров,КвоЭкземпляров);
            ...
            Иначе
                ...
                ОблСтрока.Параметры.Номенклатура	= Шапка.Номенклатура;	
                ОблСтрока.Параметры.Вес				= Шапка.ВесВывоз;
                ОблСтрока.Параметры.ВидФасовки		= Шапка.ВидФасовки;
                       КонецЕсли;
            ...
            ОблДно.Параметры.ЗавСкладом = ИНАГРО_Элеватор.ОтветственныеЛицаИмяФамилия(Шапка.Организация,Шапка.Дата, Шапка.Склад);
           
            
            ОблОбратнаяСтраница = Макет.ПолучитьОбласть("ОбратнаяСтраница|Лист");
            
            ОтветственноеЛицо = "";
            ОтветственноеЛицо = РегистрыСведений.ФИОФизическихЛиц.СрезПоследних(Шапка.Дата, Новый Структура("ФизическоеЛицо", Шапка.Ответственный.ФизическоеЛицо));
            ...
            ТабДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Ландшафт;
            ТабДокумент.ИмяПараметровПечати = "ИНАГРО_ТТНВнутр_Пропуск_Ф196_2021"; 		
        КонецЦикла;
