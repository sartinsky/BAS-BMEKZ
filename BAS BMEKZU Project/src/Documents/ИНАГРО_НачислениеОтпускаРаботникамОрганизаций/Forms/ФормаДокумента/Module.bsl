...
Процедура НачисленияПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		...
		ИначеЕсли Объект.Отпуска.Количество() > 0 Тогда
			ТекущаяСтрока.ДатаНачала = Объект.Отпуска[Объект.Отпуска.Количество()-1].ДатаНачала + 86400;
		КонецЕсли; 
...
Процедура РассчитатьРасчетСреднего(Команда)
	...
	Если Объект.Проведен Тогда 
		...
		ПоказатьВопрос(Обработчик, ТекстВопроса, РежимДиалогаВопрос.ОКОтмена);
		Возврат;
	ИначеЕсли Объект.Ссылка = ПредопределенноеЗначение("Документ.ИНАГРО_НачислениеОтпускаРаботникамОрганизаций.ПустаяСсылка") Тогда
...
Процедура УправлениеФормой(Форма)
	...
	Если ЗначениеЗаполнено(Объект.ВидДокументаОтпуск) Тогда
		Если Объект.ВидДокументаОтпуск = ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыДокументаОтпуск.Отпускные") Тогда
			Элементы.ГруппаВидОтпуска.ТекущаяСтраница = Элементы.ГруппаОтпускные;
		ИначеЕсли Объект.ВидДокументаОтпуск = ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыДокументаОтпуск.КомпенсацияОтпуска") Тогда
			Элементы.ГруппаВидОтпуска.ТекущаяСтраница = Элементы.ГруппаКомпенсацияОтпуска;
		ИначеЕсли Объект.ВидДокументаОтпуск = ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыДокументаОтпуск.ОтзывИзОтпуска") Тогда
			Элементы.ГруппаВидОтпуска.ТекущаяСтраница = Элементы.ГруппаОтзывИзОтпуска;
		ИначеЕсли Объект.ВидДокументаОтпуск = ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыДокументаОтпуск.ВозвратПриУвольнении") Тогда
			Элементы.ГруппаВидОтпуска.ТекущаяСтраница = Элементы.ГруппаВозврат;
		КонецЕсли;
	Иначе
		Элементы.ГруппаВидОтпуска.ТекущаяСтраница = Элементы.ГруппаОтпускные;
	КонецЕсли;
...
Функция	ПолучитьДатуНачалаСобытия()
	...
	тбОтпуска = Объект.Отпуска.Выгрузить();
