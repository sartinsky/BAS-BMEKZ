...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ВыборкаПоШапкеДокумента.Следующий() Тогда		
		...
		Если НЕ Отказ Тогда
			...
			ПроверитьЗаполнениеТабличнойЧасти(ВыборкаПоСтраховойСтаж, Отказ, Заголовок);
		КонецЕсли;	
		Если НЕ Отказ Тогда
			ВыборкаПоСтраховойСтаж.Сбросить();
			//ВыборкаПоСтраховойСтаж = СформироватьЗапросПоСтраховойСтаж().Выбрать();
	...
	Пока ВыборкаПоТабличнойЧастиДокумента.Следующий() Цикл
		Если Не ВыборкаПоТабличнойЧастиДокумента.УчитыватьИтогиГода Тогда
			...
			Если ЧислоДнейПомесячно = 0 и (ВыборкаПоТабличнойЧастиДокумента.ВсегоМесяцев > 0 Или ВыборкаПоТабличнойЧастиДокумента.ВсегоДней > 0) Тогда
				ОбщегоНазначенияБПКлиентСервер.СообщитьОбОшибке(НСтр("ru='В строке №" + ВыборкаПоТабличнойЧастиДокумента.НомерСтроки + " год не входит в период справки полностью, заполните стаж помесячно';uk='В рядку №" + ВыборкаПоТабличнойЧастиДокумента.НомерСтроки + " рік не входить в період довідки повністю, заповніть стаж помісячно'"), Отказ, Заголовок);
			КонецЕсли;
		КонецЕсли;	
	КонецЦикла;	
