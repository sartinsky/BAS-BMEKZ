...
&НаКлиенте
Процедура СоздатьПредварительныйРасчет(Команда)
	
	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийАктРасчет310.ПредварительныйРасчет"));
	ОткрытьФорму("Документ.ИНАГРО_АктРасчет310.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);

КонецПроцедуры

&НаКлиенте
Процедура СоздатьРасчетПоФакту(Команда)
	
	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийАктРасчет310.РасчетПоФакту"));
	ОткрытьФорму("Документ.ИНАГРО_АктРасчет310.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);
