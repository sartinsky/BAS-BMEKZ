...
Процедура ИзменитьВидОперации(Команда)
	...
	ОткрытьФорму("Документ.ИНАГРО_РасчетКорректировкиЦены.Форма.ФормаДокумента", ПараметрыФормы, ЭтаФорма);
...
Процедура СоздатьВходящиеДокументы(Команда)
	...
	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийКорректировкиСтоимости.ВходящиеДокументы"));
	ОткрытьФорму("Документ.ИНАГРО_РасчетКорректировкиЦены.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);

КонецПроцедуры 

&НаКлиенте
Процедура СоздатьИсходящиеДокументы(Команда)
	
	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийКорректировкиСтоимости.ИсходящиеДокументы"));
	ОткрытьФорму("Документ.ИНАГРО_РасчетКорректировкиЦены.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);
