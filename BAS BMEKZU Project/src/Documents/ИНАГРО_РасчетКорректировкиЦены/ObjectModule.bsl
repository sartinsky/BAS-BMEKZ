...
Процедура ДвиженияПоРегистрам(РежимПроведения, СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоДанныеРаспределения, Отказ, Заголовок);
	...
	Если ВидОперации = Перечисления.ИНАГРО_ВидыОперацийКорректировкиСтоимости.ВходящиеДокументы Тогда 
		...
		Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
			...
			// Движение по рег. ИНАГРО_ЖивотныеОрганизации, ИНАГРО_РастенияОрганизации
			Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() И ДокументОснование.Метаданные().Имя ="ИНАГРО_ПоступлениеБиологическихАктивов" 
				И НЕ СтрокаТаблицы.СчетУчетаБУ.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ПриобретениеВыращиваниеДолгосрочныхБиологическихАктивов) Тогда
				Если ДокументОснование.ВидОперации = Перечисления.ИНАГРО_ВидыОперацийПоступленияБА.ПокупкаКомиссияЖивотных Тогда
					...
					Движение.КодОперации   = Перечисления.ИНАГРО_КодыОперацийУчетЖивотных.Покупка;
				Иначе
					Движение = Движения.ИНАГРО_РастенияОрганизации.Добавить();
					Движение.КодОперации    = Перечисления.ИНАГРО_КодыОперацийУчетРастений.Покупка;
				КонецЕсли;	
				...
				Движение.Склад                 = СтрокаТаблицы.Склад;
				Движение.Количество            = 0;
				Движение.Сумма		           = СтрокаТаблицы.ПроводкиСуммаБезНДСРегл;
				Движение.ДокументОприходования = ДокументОснование;
			КонецЕсли;
	...
	Иначе // ВидОперации = ИсходящиеДокументы
		...
		Для каждого СтрокаТаблицы Из ТаблицаПоТоварам Цикл
			...
			Если СтрокаТаблицы.Сумма <> 0 Тогда
				...
				//// Движение по рег. ИНАГРО_ЖивотныеОрганизации, ИНАГРО_РастенияОрганизации
				//Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() И ДокументОснование.Метаданные().Имя ="ИНАГРО_РеализацияБиологическихАктивов" 
				//	И НЕ ДокументОснование.ВидОперации = Перечисления.ИНАГРО_ВидыОперацийРеализацииБА.ВПереработку Тогда
				//	
				//	Если ДокументОснование.ВидОперации = Перечисления.ИНАГРО_ВидыОперацийРеализацииБА.ПродажаКомиссияЖивотных Тогда
				//		Движение = Движения.ИНАГРО_ЖивотныеОрганизации.Добавить();
				//		Движение.КодОперации   = Перечисления.ИНАГРО_КодыОперацийУчетЖивотных.Продажа;
				//	Иначе
				//		Движение = Движения.ИНАГРО_РастенияОрганизации.Добавить();
				//		Движение.КодОперации    = Перечисления.ИНАГРО_КодыОперацийУчетРастений.Продажа;
				//	КонецЕсли;	
				...
				//	Движение.Склад                 = СтрокаТаблицы.Склад;
				//	Движение.Количество            = 0;
				//	Движение.Сумма		           = СтрокаТаблицы.ПроводкиСуммаСНДСРегл;
				//	Движение.ДокументОприходования = ДокументОснование;
				//КонецЕсли;
...
Процедура ДвиженияПоБА(СтруктураШапкиДокумента, СтрокаТЧ, Отказ)
		...
		ИначеЕсли СтрокаТЧ.ДокументРаспределения.Метаданные().Имя = "ИНАГРО_ПеремещениеБиологическихАктивов" Тогда
				...
				//Для каждого Стр Из СтрокаТЧ.ДокументРаспределения.Животные Цикл
					...
					//Если ЗначениеЗаполнено(Стр.Номенклатура) И Стр.Номенклатура = СтрокаТЧ.Номенклатура  Тогда
						...
						Если НЕ СтрокаТЧ.СчетКт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ПриобретениеВыращиваниеДолгосрочныхБиологическихАктивов) Тогда
							...
							Если ТипЗнч(СтрокаТЧ.СубконтоКт1) = Тип("СправочникСсылка.Склады") Тогда
								Движение.Склад 		           = СтрокаТЧ.СубконтоКт1;
							ИначеЕсли ТипЗнч(СтрокаТЧ.СубконтоКт2) = Тип("СправочникСсылка.Склады") Тогда
								Движение.Склад 		           = СтрокаТЧ.СубконтоКт2;
							ИначеЕсли ТипЗнч(СтрокаТЧ.СубконтоКт3) = Тип("СправочникСсылка.Склады") Тогда
								Движение.Склад 		           = СтрокаТЧ.СубконтоКт3;
							КонецЕсли;	
							Движение.Номенклатура          = СтрокаТЧ.Номенклатура;
							// Так нельзя т.к. СтрокаТЧ.ДокументРаспределения.Животные.Количество может проходить по нескольким партиям, отличным от корректируемого документа
							//Движение.Сумма                 = СтрокаТЧ.СуммаРаспределения / СтрокаТЧ.Количество * Стр.Количество;
							Движение.Сумма                 = СтрокаТЧ.СуммаРаспределения;
							Движение.ДокументОприходования = ДокументОснование;
						КонецЕсли;
						
						Если НЕ СтрокаТЧ.СчетДт.ПринадлежитЭлементу(ПланыСчетов.Хозрасчетный.ПриобретениеВыращиваниеДолгосрочныхБиологическихАктивов) Тогда
							...
							Если ТипЗнч(СтрокаТЧ.СубконтоДт1) = Тип("СправочникСсылка.Склады") Тогда
								Движение.Склад 		           = СтрокаТЧ.СубконтоДт1;
							ИначеЕсли ТипЗнч(СтрокаТЧ.СубконтоДт2) = Тип("СправочникСсылка.Склады") Тогда
								Движение.Склад 		           = СтрокаТЧ.СубконтоДт2;
							ИначеЕсли ТипЗнч(СтрокаТЧ.СубконтоДт3) = Тип("СправочникСсылка.Склады") Тогда
								Движение.Склад 		           = СтрокаТЧ.СубконтоДт3;
							КонецЕсли;	
							//Движение.Номенклатура          = Стр.НоменклатураНовая;
							Если ТипЗнч(СтрокаТЧ.СубконтоДт1) = Тип("СправочникСсылка.БиологическиеАктивы") Тогда
								Движение.Номенклатура 		           = СтрокаТЧ.СубконтоДт1;
							ИначеЕсли ТипЗнч(СтрокаТЧ.СубконтоДт2) = Тип("СправочникСсылка.БиологическиеАктивы") Тогда
								Движение.Номенклатура 		           = СтрокаТЧ.СубконтоДт2;
							ИначеЕсли ТипЗнч(СтрокаТЧ.СубконтоДт3) = Тип("СправочникСсылка.БиологическиеАктивы") Тогда
								Движение.Номенклатура 		           = СтрокаТЧ.СубконтоДт3;
							КонецЕсли;	
							// Так нельзя т.к. СтрокаТЧ.ДокументРаспределения.Животные.Количество может проходить по нескольким партиям, отличным от корректируемого документа
							//Движение.Сумма                 = СтрокаТЧ.СуммаРаспределения / СтрокаТЧ.Количество * Стр.Количество;
							Движение.Сумма                 = СтрокаТЧ.СуммаРаспределения;
		...
		ИначеЕсли СтрокаТЧ.ДокументРаспределения.Метаданные().Имя = "ВводВЭксплуатациюОС" Тогда
			Если ЭтоЖивотное Тогда 				
				Движение = Движения.ИНАГРО_ЖивотныеОрганизации.Добавить();
				Движение.КодОперации = Перечисления.ИНАГРО_КодыОперацийУчетЖивотных.ВводВОсновноеСтадо;
			Иначе	
				Движение = Движения.ИНАГРО_РастенияОрганизации.Добавить();
				Движение.КодОперации = Перечисления.ИНАГРО_КодыОперацийУчетРастений.ПереводВДолгосрочныеРаст;
			КонецЕсли;
			Движение.Период                = СтруктураШапкиДокумента.Дата;
			Движение.ВидДвижения           = ВидДвиженияНакопления.Приход;
			Движение.Организация           = СтруктураШапкиДокумента.Организация;
			Движение.Номенклатура          = СтрокаТЧ.ДокументРаспределения.ИНАГРО_БиологическийАктивНовый;
			Движение.Склад 		  		   = СтрокаТЧ.ДокументРаспределения.ИНАГРО_СкладНовый;
			Движение.Сумма                 = СтрокаТЧ.СуммаРаспределения; 
			Движение.ДокументОприходования = ДокументОснование;
		...
		Иначе
			...
			Если СтрокаТЧ.ДокументРаспределения.Метаданные().Имя = "ИНАГРО_ВедомостьРеализацияБиологическихАктивов" Тогда 
				Движение.КодОперации  = Перечисления.ИНАГРО_КодыОперацийУчетЖивотных.Продажа;   				
				Движение.Склад 		  = СтрокаТЧ.ДокументРаспределения.Склад;
			КонецЕсли;
