...
|По звіті комісіонера можна вводити Додаток 2 до Податкової накладної на різницю в ціні передачі й реалізації товарів.'");
	...
	Если ОснованиеСодержитНоменклатуру Тогда
		...
		Если     ТипЗнч(Основание) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") ИЛИ 
			(ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() И  ТипЗнч(Основание) = Тип("ДокументСсылка.ИНАГРО_СчетНаОплатуПокупателюБиологическихАктивов")) Тогда
		...
		Если  Основание.ДоговорКонтрагента.ВедениеВзаиморасчетовНУ = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоРасчетнымДокументам 
			И (ТипЗнч(СделкаНалоговыйУчет) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") 
			ИЛИ (ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() И  ТипЗнч(СделкаНалоговыйУчет) = Тип("ДокументСсылка.ИНАГРО_СчетНаОплатуПокупателюБиологическихАктивов"))) Тогда
