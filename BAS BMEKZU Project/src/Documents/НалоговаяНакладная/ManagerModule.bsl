		...
		Если ТекРабочаяДата >= '2016-04-01' Тогда
			...
			Если НЕ ВыгрузкаПечатьНН_ПоФормеНаДатуДокумента = Истина Тогда
				...
				Если ЗначениеЗаполнено(ДатаНачалаИспользованияФормыНН15) И ТекРабочаяДата >= ДатаНачалаИспользованияФормыНН15 Тогда					
					...
					НовТабДокумент =  ПечатьДокументаНалоговыйКодекс2023(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, "НалоговаяНакладная2023Мес7", Объект);
		            ТабДокументСформирован = Истина;
					
				ИначеЕсли ТекРабочаяДата >= '2023-04-01' Тогда
					
					НовТабДокумент =  ПечатьДокументаНалоговыйКодекс2023(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, "НалоговаяНакладная2023", Объект);
		            ТабДокументСформирован = Истина;
...
Функция ПечатьДокументаНалоговыйКодекс2023(ТолькоФормироватьСтруктуруПоказателейПечатнойФормы, ИмяМакета = "НалоговаяНакладнаяАльбом", Объект)
	...
	// свернем подобные строки    
	НаборРекизитовСвертки = "КодВид, Номенклатура, КодУКТВЭД, ЕдиницаИзмерения, ЕдиницаИзмеренияКод, ЦенаБезНДСРегл, СтавкаНДС, КодЛьготы";
	ТаблицаНоменклатуры.Свернуть("НомерСтрокиНН, " + НаборРекизитовСвертки, "Количество, СуммаБезНДСРегл, СуммаНДСРегл, СуммаБезНДСПревышенияРегл, СуммаНДСПревышенияРегл"); 
	ТаблицаНоменклатуры.Сортировать("НомерСтрокиНН");
	// переименуем колонки
	ТаблицаНоменклатуры.Колонки.ЦенаБезНДСРегл.Имя 				 = "ЦенаБезНДС";
