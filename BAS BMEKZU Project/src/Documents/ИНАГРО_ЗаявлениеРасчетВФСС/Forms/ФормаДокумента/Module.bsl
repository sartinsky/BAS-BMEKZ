...
Процедура ЗаполнитьТЧДляДокументовПособий2018(Больничные, БольничныеНС, Декретные, ПособиеНаПогребение, ЗаполнитьВсе)
	...
	Запрос.УстановитьПараметр("РежимЗаполнения", Объект.РежимЗаполнения);
...
Процедура ВыполнитьВыгрузку(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ВыборкаШапка.РежимЗаполнения = 1 Тогда
		ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, , "CHECK_2", "1");
	ИначеЕсли ВыборкаШапка.РежимЗаполнения = 2 Тогда
	...
	Для каждого Выборка ИЗ ВыборкаТЧ.ТЧ11 Цикл
		...
		Если ЗначениеЗаполнено(Выборка.РезультатФСС) Тогда
			...
			ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, "TAB1", "TAB1_A7", ?(Объект.РежимЗаполнения = 1, "",Выборка.Серия));
			ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, "TAB1", "TAB1_A8", СокрЛП(Выборка.Номер));
			ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, "TAB1", "TAB1_A9", Выборка.ЭтоПродолжениеБольничного);
			ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, "TAB1", "TAB1_A10", ?(Объект.РежимЗаполнения = 1, Выборка.КодПричиныЭлектронный, Выборка.КодПричины));
	...
	Для каждого Выборка ИЗ ВыборкаТЧ.ТЧ13 Цикл
		...
		Если ЗначениеЗаполнено(Выборка.РезультатФСС) Тогда
			...
			ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, "TAB3", "TAB3_A9", ?(Объект.РежимЗаполнения = 1, "",Выборка.Серия));
			ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, "TAB3", "TAB3_A10", СокрЛП(Выборка.Номер));
			ДобавитьПоказательВОтчетЗвит1С(ГоловнойОтчет, "TAB3", "TAB3_A11", ?(Объект.РежимЗаполнения = 1, Выборка.КодПричиныЭлектронный, Выборка.КодПричины));
