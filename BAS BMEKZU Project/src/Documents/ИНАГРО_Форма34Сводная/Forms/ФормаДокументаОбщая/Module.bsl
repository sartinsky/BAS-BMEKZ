...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "СозданЛабораторныйАнализ" ИЛИ ИмяСобытия = "ИзмененЛабораторныйАнализ" Тогда
		...
		Если  ТипЗнч(Параметр) = Тип("Структура") 
			...
			Если Параметр.Свойство("Индекс") Тогда
				Индекс = Параметр.Индекс;
			КонецЕсли;
			...
			Если ИНАГРО_Элеватор.ЛабораторныйАнализПроверкаЗаполнения(Параметр.ДокументСсылка, ПолучитьПараметрыДляПроверкиЗаполнения()) Тогда
				Если Индекс <> "РасчетПоВладельцам" Тогда
					Объект["ЛабораторныйАнализ" + Индекс] = Параметр.ДокументСсылка;
					ЛабораторныйАнализПриИзменении = "ЛабораторныйАнализ" + Индекс + "ПриИзменении(Неопределено)"; 
					Выполнить (ЛабораторныйАнализПриИзменении);
					Модифицированность = Истина;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЛабораторныйАнализ1ПриИзменении(Элемент)
	...
	ДанныеОбъекта = ПолучитьДанныеЛабораторногоАнализа();
	ДанныеОбъекта.ЛабораторныйАнализ = Объект.ЛабораторныйАнализ1;
	
	ИНАГРО_Элеватор.ЛабораторныйАнализПриИзмененииНаСервере(ДанныеОбъекта);
		
	Объект.НомерАнализа1    = ДанныеОбъекта.НомерАнализа; 
	Объект.Влажность1       = ДанныеОбъекта.Влажность;
	Объект.СорнаяПримесь1   = ДанныеОбъекта.СорнаяПримесь;
	Объект.ЗерноваяПримесь1 = ДанныеОбъекта.ЗерноваяПримесь;	
...
Процедура ЛабораторныйАнализ2ПриИзменении(Элемент)
	...
	ДанныеОбъекта = ПолучитьДанныеЛабораторногоАнализа();
	ДанныеОбъекта.ЛабораторныйАнализ = Объект.ЛабораторныйАнализ2;
	
	ИНАГРО_Элеватор.ЛабораторныйАнализПриИзмененииНаСервере(ДанныеОбъекта);
	
	Объект.НомерАнализа2    = ДанныеОбъекта.НомерАнализа;
	Объект.Влажность2       = ДанныеОбъекта.Влажность;
	Объект.СорнаяПримесь2   = ДанныеОбъекта.СорнаяПримесь;
	Объект.ЗерноваяПримесь2 = ДанныеОбъекта.ЗерноваяПримесь;	
...
Процедура ЛабораторныйАнализ7ПриИзменении(Элемент)
	...
	ДанныеОбъекта = ПолучитьДанныеЛабораторногоАнализа();
	ДанныеОбъекта.ЛабораторныйАнализ = Объект.ЛабораторныйАнализ7;
	
	ИНАГРО_Элеватор.ЛабораторныйАнализПриИзмененииНаСервере(ДанныеОбъекта);

	Объект.НомерАнализа7    = ДанныеОбъекта.НомерАнализа;
	Объект.Влажность7       = ДанныеОбъекта.Влажность;
	Объект.СорнаяПримесь7   = ДанныеОбъекта.СорнаяПримесь;
	Объект.ЗерноваяПримесь7 = ДанныеОбъекта.ЗерноваяПримесь;	
...
Процедура ЛабораторныйАнализ8ПриИзменении(Элемент)
	...
	ДанныеОбъекта = ПолучитьДанныеЛабораторногоАнализа();
	ДанныеОбъекта.ЛабораторныйАнализ = Объект.ЛабораторныйАнализ8;
	
	ИНАГРО_Элеватор.ЛабораторныйАнализПриИзмененииНаСервере(ДанныеОбъекта);
	
	Объект.НомерАнализа8    = ДанныеОбъекта.НомерАнализа;
	Объект.Влажность8       = ДанныеОбъекта.Влажность;
	Объект.СорнаяПримесь8   = ДанныеОбъекта.СорнаяПримесь;
	Объект.ЗерноваяПримесь8 = ДанныеОбъекта.ЗерноваяПримесь;	
...
Процедура ЛабораторныйАнализ9ПриИзменении(Элемент)
	...
	ДанныеОбъекта = ПолучитьДанныеЛабораторногоАнализа();
	ДанныеОбъекта.ЛабораторныйАнализ = Объект.ЛабораторныйАнализ9;
	
	ИНАГРО_Элеватор.ЛабораторныйАнализПриИзмененииНаСервере(ДанныеОбъекта);

	Объект.НомерАнализа9    = ДанныеОбъекта.НомерАнализа;
	Объект.Влажность9       = ДанныеОбъекта.Влажность;
	Объект.СорнаяПримесь9   = ДанныеОбъекта.СорнаяПримесь;
	Объект.ЗерноваяПримесь9 = ДанныеОбъекта.ЗерноваяПримесь;	
...
Процедура ЛабораторныйАнализ10ПриИзменении(Элемент)
	...
	ДанныеОбъекта = ПолучитьДанныеЛабораторногоАнализа();
	ДанныеОбъекта.ЛабораторныйАнализ = Объект.ЛабораторныйАнализ10;
	
	ИНАГРО_Элеватор.ЛабораторныйАнализПриИзмененииНаСервере(ДанныеОбъекта);

	Объект.НомерАнализа10    = ДанныеОбъекта.НомерАнализа;
	Объект.Влажность10       = ДанныеОбъекта.Влажность;
	Объект.СорнаяПримесь10   = ДанныеОбъекта.СорнаяПримесь;
	Объект.ЗерноваяПримесь10 = ДанныеОбъекта.ЗерноваяПримесь;				
...
&НаКлиенте
Функция ПолучитьДанныеЛабораторногоАнализа()
...
Функция ПолучитьПараметрыДляПроверкиЗаполнения()
	...
	ИначеЕсли ТекущийЭлемент = Элементы.ЛабораторныйАнализ10 Тогда 
		ПараметрыДляПроверкиЗаполнения.Вставить("Номенклатура", Объект.Отходы2Категории);
	Иначе //ТекущийЭлемент = Элементы.ЛабораторныйАнализ1 ИЛИ ТекущийЭлемент = Элементы.ЛабораторныйАнализ2 Тогда
		ПараметрыДляПроверкиЗаполнения.Вставить("Номенклатура",  Объект.Номенклатура);
	КонецЕсли;
...
Процедура тСводныеРасчеты()
	...
	Если Объект.СпособРасчетаУбылиОтОчистки = Перечисления.ИНАГРО_СпособыРасчетаУбылиОтОчистки.СУчетомУбылиОтСушки тогда
		ФизическийВесДляБалансаДо 	 = Объект.ФизическийВес1;
		ФизическийВесДляБалансаПосле = Объект.ФизическийВес2;
	иначе
		ФизическийВесДляБалансаДо 	 = Объект.ФизическийВес3;
	...
	БалансСорнойПримеси = РассчитатьБалансСорнойПримеси(ИтогЦентнероПроцентов, ФизическийВесДляБалансаДо, Объект.СорнаяПримесь3,
													    ФизическийВесДляБалансаПосле, Объект.СорнаяПримесь4,
													    Объект.ФизическийВес8, Объект.СорнаяПримесь8,
													    Объект.ФизическийВес7, Объект.СорнаяПримесь7,
													    Объект.ФизическийВес9, Объект.СорнаяПримесь9,
...
&НаКлиенте
Процедура РасчетПоВладельцамРеестрТТНПриИзменении(Элемент)
	
	ТекущиеДанные = Элементы.РасчетПоВладельцам.ТекущиеДанные;
	
	Если НЕ ТипЗнч(ТекущиеДанные.РеестрТТН) = Тип("ДокументСсылка.ИНАГРО_РеестрТТНВвозЖД") Тогда
		
		ДанныеОбъекта = Новый Структура("РеестрТТН,ЛабораторныйАнализ");
		ЗаполнитьЗначенияСвойств(ДанныеОбъекта, ТекущиеДанные);
		ПолучитьЛабораторныйАнализНаСервере(ДанныеОбъекта);
...
&НаСервере
Процедура ПолучитьЛабораторныйАнализНаСервере(ДанныеОбъекта)
	
	ДанныеОбъекта.ЛабораторныйАнализ = ДанныеОбъекта.РеестрТТН.ЛабораторныйАнализ;
