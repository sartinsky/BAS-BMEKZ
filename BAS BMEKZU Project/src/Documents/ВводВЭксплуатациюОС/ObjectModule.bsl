...
Процедура ФормированиеДвиженийРегл(СтруктураШапкиДокумента, ТаблицаПоОС, Отказ, Заголовок)
	...
	Если ЗначениеЗаполнено(ИНАГРО_ДокументОперативногоУчета) Тогда
	...
	Если  СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ИНАГРО_БиологическиеАктивы И ПартионныйУчетБУ 
		И ИНАГРО_ДокументОперативногоУчета.Метаданные().Имя = "ИНАГРО_ПеремещениеБиологическихАктивов" Тогда // ИНАГРО
		...
		МодульИНАГРО_БиологическиеАктивы = ОбщегоНазначения.ОбщийМодуль("ИНАГРО_БиологическиеАктивы");
		МодульИНАГРО_БиологическиеАктивы.ИНАГРО_ДвижениеПартийТоваров(ТаблицаЖивотных, 
																	Истина, 
																	Ложь, 
																	Отказ, , 
																	СодержаниеПроводки,
																	МодульИНАГРО_БиологическиеАктивы.ЭтоЖивотное(ЭтотОбъект.ИНАГРО_БиологическийАктив),
	...
	Для Каждого Строка Из ТЗДанныеОС Цикл
		...
		Если СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ОбъектыСтроительства  
			ИЛИ (ЗначениеЗаполнено(ИНАГРО_ДокументОперативногоУчета) И ИНАГРО_ДокументОперативногоУчета.Метаданные().Имя = "ИНАГРО_ОприходованиеБиологическихАктивов")
			ИЛИ СтруктураШапкиДокумента.ВидОперации = Перечисления.ВидыОперацийПринятияКУчетуОсновныхСредств.ИНАГРО_БиологическиеАктивы И НЕ ПартионныйУчетБУ Тогда // ИНАГРО
			...
			Если Строка.Сумма > 0 Тогда
				...
				Если ЗначениеЗаполнено(ИНАГРО_ДокументОперативногоУчета) И ИНАГРО_ДокументОперативногоУчета.Метаданные().Имя = "ИНАГРО_ОприходованиеБиологическихАктивов" И ПартионныйУчетБУ Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипДанныхЗаполнения = Тип("Структура") И ДанныеЗаполнения.Свойство("ИНАГРО_ДокументОперативногоУчета") Тогда
		...
		Если ДанныеЗаполнения.ИНАГРО_ДокументОперативногоУчета.Метаданные().Имя = "ИНАГРО_ПеремещениеБиологическихАктивов" ИЛИ
			...
			Если НЕ ЗначениеЗаполнено(ДанныеЗаполнения.ВидОперации) Тогда
	...
	Если НЕ Отказ Тогда
		...
		Если АмортизацияБА И ЗначениеЗаполнено(ИНАГРО_ДокументОперативногоУчета) Тогда  
			...
			Если ЭтотОбъект.ОС.Количество()> 0 Тогда
				...
				ТаблицаБА.ЗаполнитьЗначения(ЭтотОбъект.СтоимостьБУ/ЭтотОбъект.ОС.Количество(), "СтоимостьБУ");
				
				ТаблицаБА.Колонки.Добавить("СтоимостьНУ");
				ТаблицаБА.ЗаполнитьЗначения(ЭтотОбъект.СтоимостьНУ/ЭтотОбъект.ОС.Количество(), "СтоимостьНУ");
			КонецЕсли;
