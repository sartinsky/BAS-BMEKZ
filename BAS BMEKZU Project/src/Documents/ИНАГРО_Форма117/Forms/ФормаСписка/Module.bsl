...
Процедура ИзменитьВидОперации(Команда)
	...
	ОткрытьФорму("Документ.ИНАГРО_Форма117.Форма.ФормаДокумента", ПараметрыФормы, ЭтаФорма);
...
&НаКлиенте
Процедура СоздатьПоВладельцу(Команда)
	
	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийФорма117.ПоВладельцу"));
	ОткрытьФорму("Документ.ИНАГРО_Форма117.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);

КонецПроцедуры

&НаКлиенте
Процедура СоздатьСводная(Команда)
	
	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийФорма117.Сводная"));
	ОткрытьФорму("Документ.ИНАГРО_Форма117.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);
