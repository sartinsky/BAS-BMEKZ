...
Процедура ОрганизацияПриИзменении(Элемент)
	...
	Если ХозрасчетныйНаборЗаписей.Количество() > 0 Тогда
		ТекстВопроса = НСтр("ru='Указанные в проводках банковские счета, договоры, подразделения, документы будут очищены. 
|Продолжить?';uk='Зазначені в проводках банківські рахунки, договори, підрозділу, документи будуть очищені. 
|Продовжити?'");
...
Процедура СторнируемыйДокументНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Организация) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выберите организацию.';uk='Виберіть організацію.'"));
...
|Продолжить?';uk='Існуючі проводки і рухи регістрів будуть очищені.
|Продовжити?'");
...
Процедура ХозрасчетныйПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда
		ЗаполнитьНадписиВПроводке(Элементы.Хозрасчетный.ТекущиеДанные, ДокументДо2015);
	КонецЕсли;
...
|Снимите пометку удаления.';uk='Операція відмічена для вилучення, тому не можна переключити активність.
|Зніміть позначку вилучення.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьИзменениеСчета(Форма, ДтКт)
	...
	ПоляОбъекта = Новый Структура("Субконто1,Субконто2,Субконто3,НалоговыйУчет,УчетПоНалоговымНазначениямНДС,УчетСуммНУ,Валютный,Количественный,Поштучный,Организация");  // ИНАГРО Добавлены головы
	ПоляОбъекта.Субконто1      = "Субконто" + ДтКт + "1";
	ПоляОбъекта.Субконто2      = "Субконто" + ДтКт + "2";
	ПоляОбъекта.Субконто3      = "Субконто" + ДтКт + "3";
	ПоляОбъекта.НалоговыйУчет  = "НалоговыйУчет" + ДтКт;
	ПоляОбъекта.УчетПоНалоговымНазначениямНДС  = "УчетПоНалоговымНазначениямНДС" + ДтКт;
	ПоляОбъекта.УчетСуммНУ  = "УчетСуммНУ" + ДтКт;
	ПоляОбъекта.Валютный       = "Валютный" + ДтКт;
	ПоляОбъекта.Количественный = "Количественный" + ДтКт;
	ПоляОбъекта.Поштучный 	   = "Поштучный" + ДтКт;   // ИНАГРО Добавлены головы
	...
	Если НЕ Форма.ДокументДо2015 Тогда
		Если НЕ СтрокаТаблицы["УчетПоНалоговымНазначениямНДС"+ДтКт+"Доступность"] Тогда
			СтрокаТаблицы["НалоговоеНазначение"+ДтКт] = Неопределено;
		КонецЕсли;
		Если НЕ СтрокаТаблицы["УчетСуммНУ"+ДтКт+"Доступность"] Тогда
			СтрокаТаблицы["СуммаНУ"+ДтКт] = 0;
		КонецЕсли;
	КонецЕсли;
	
	ЗаполнитьНадписиВПроводке(СтрокаТаблицы, Форма.ДокументДо2015);
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	ПоляОбъектаДт = Новый Структура(
		"Субконто1, Субконто2, Субконто3, НалоговыйУчет, УчетПоНалоговымНазначениямНДС, УчетСуммНУ, Валютный, Количественный, Поштучный", // ИНАГРО Добавлены головы
		"СубконтоДт1", "СубконтоДт2", "СубконтоДт3", "НалоговыйУчетДт", "УчетПоНалоговымНазначениямНДСДт", "УчетСуммНУДт", "ВалютныйДт", "КоличественныйДт", "ПоштучныйДт"); // ИНАГРО Добавлены головы
		
	ПоляОбъектаКт = Новый Структура(
		"Субконто1, Субконто2, Субконто3, НалоговыйУчет, УчетПоНалоговымНазначениямНДС, УчетСуммНУ, Валютный, Количественный, Поштучный",  // ИНАГРО Добавлены головы
		"СубконтоКт1", "СубконтоКт2", "СубконтоКт3", "НалоговыйУчетКт", "УчетПоНалоговымНазначениямНДСКт", "УчетСуммНУКт", "ВалютныйКт", "КоличественныйКт", "ПоштучныйКт"); // ИНАГРО Добавлены головы
	...
	Для каждого Проводка Из ХозрасчетныйНаборЗаписей Цикл
		...
		ЗаполнитьНадписиВПроводке(Проводка, ДокументДо2015);
	КонецЦикла;
...
Процедура ПодготовитьФормуНаСервере(ДокументДвижений)
	...
	Элементы.СторнируемыйДокумент.Видимость = Найти(Объект.СпособЗаполнения, "Сторно") > 0;
	Элементы.ГруппаТиповаяОперация.Видимость = (Объект.СпособЗаполнения = "ТиповаяОперация");
	
КонецПроцедуры

&НаСервере
Процедура УстановитьСостояниеДокумента(СтрокаУказаниеСтатуса = Неопределено) Экспорт
...
Процедура СоздатьЭлементыФормы()
	...
	Элементы.ФормаПереключитьАктивностьДвижений.Заголовок = ?(АктивностьДвижений, 
		НСтр("ru='Выключить активность движений';uk='Вимкнути активність рухів'"), НСтр("ru='Включить активность движений';uk='Включити активність рухів'"));
...
Процедура СформироватьДвиженияСторноНаСервере()
	...
	Объект.Содержание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Сторно документа ""%1""';uk='Сторно документа ""%1""'"),
...
Функция СтруктураДанныхПроводки(ТекущиеДанные)
	...
	СтруктураПроводки = Новый Структура("НомерСтроки,СчетДт,НалоговоеНазначениеДт,СубконтоДт1,СубконтоДт2,СубконтоДт3,
		|КоличествоДт,ВалютаДт,ВалютнаяСуммаДт,
		|СчетКт,НалоговоеНазначениеКт,СубконтоКт1,СубконтоКт2,СубконтоКт3,
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьНадписиВПроводке(Проводка, ДокументДо2015)
	
	Проводка.НадписьНУДт = "";
	Если (ДокументДо2015 И Проводка.НалоговыйУчетДтДоступность) 
		ИЛИ (НЕ ДокументДо2015 И (Проводка.УчетПоНалоговымНазначениямНДСДтДоступность ИЛИ Проводка.УчетСуммНУДтДоступность))  Тогда
		Проводка.НадписьНУДт = НСтр("ru='НУ:';uk='ПО:'");
	КонецЕсли;
	Проводка.НадписьНУКт = "";
	Если (ДокументДо2015 И Проводка.НалоговыйУчетКтДоступность) 
		ИЛИ (НЕ ДокументДо2015 И (Проводка.УчетПоНалоговымНазначениямНДСКтДоступность ИЛИ Проводка.УчетСуммНУКтДоступность))  Тогда
		Проводка.НадписьНУКт = НСтр("ru='НУ:';uk='ПО:'");
	КонецЕсли;
	...
	Проводка.НадписьКоличествоГоловДт = ?(Проводка.ПоштучныйДтДоступность, НСтр("ru='Гол.:';uk='Гол.:'"), "");    // ИНАГРО Добавлены головы
	Проводка.НадписьКоличествоГоловКт = ?(Проводка.ПоштучныйКтДоступность, НСтр("ru='Гол.:';uk='Гол.:'"), "");    // ИНАГРО Добавлены головы
...
Процедура УстановитьПараметрыФункциональныхОпцийФормыОбъектаОперация()
	...
	ДокументДо2015 = ПериодДок < '2015 01 01';
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПараметровТиповойОперации Цикл
		...
		Для Каждого ТипЭлемента Из ТипРеквизита.Типы() Цикл
			Если ТипыСвязанныеСОрганизацией.СодержитТип(ТипЭлемента) Тогда
				Если ТипЭлемента = Тип("СправочникСсылка.ПодразделенияОрганизаций") ИЛИ ТипЭлемента = Тип("СправочникСсылка.БанковскиеСчета") Тогда
...
Процедура СформироватьДвиженияПоТиповойОперацииСервер()
	...
	Для Каждого СтрокаТаблицыХозрасчетный ИЗ ТиповаяОперация.Хозрасчетный Цикл
		...
		Для Каждого Колонка из КолонкиХозрасчетный.Колонки Цикл
			...
			Если ЭтоПараметр(Значение) Тогда
				...
				КонецЕсли;
			КонецЕсли;
			Если Проводка.Свойство(ИмяКолонки) Тогда
				// Установим значение
	...
	Для Каждого СтрокаСписокРегистров Из СписокРегистров Цикл
		...
		Движение		 = ЭтотОбъект[СтруктураРегистра.ИмяРегистра+"НаборЗаписей"];
...
Процедура ОбновимОтображениеРегистров(СписокРегистров)
	...
	Для Каждого СтрокаРегистра Из Регистры Цикл
		...
		СтрокаРегистра.Записывать = Пометка;
