...
Процедура ПроводкиПоНДС(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоУслугам, ТаблицаПоТаре, ТаблицаПоОС, ТаблицаПоНМА, Отказ)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДвижений Цикл
		...
		Если    СтрокаТаблицы.ИзменениеСуммыНДСРегл <> 0 Тогда
			...
			Проводка.Период                     = ?(СтруктураШапкиДокумента.СуммаНДСДокумента < 0 И СтруктураШапкиДокумента.Дата >= '2015-07-01', СтруктураШапкиДокумента.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных, СтруктураШапкиДокумента.Дата);
...
Процедура ДвиженияПоРегиструОжидаемыйИПодтвержденныйНДСПродаж(СтруктураШапкиДокумента, ТаблицаПоТоварам, ТаблицаПоТаре, ТаблицаПоУслугам, ТаблицаПоОС, ТаблицаПоНМА, Отказ, ЧастичноеПроведение = Ложь)
	...
	Если НЕ Отказ И ТаблицаДвижений.Количество() > 0 Тогда
		...
		Если ЧастичноеПроведение Тогда
			НаборДвижений.мПериод = СтруктураШапкиДокумента.Дата;
		Иначе
			НаборДвижений.мПериод = ?(СтруктураШапкиДокумента.СуммаНДСДокумента < 0  И СтруктураШапкиДокумента.Дата >= '2015-07-01', СтруктураШапкиДокумента.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных, СтруктураШапкиДокумента.Дата);
		КонецЕсли; 
