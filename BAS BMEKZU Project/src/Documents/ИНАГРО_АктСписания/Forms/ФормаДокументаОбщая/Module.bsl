...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ВидОперации) Тогда
		...
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийПереработка.ДавальческоеСырье") И СобственноеПодразделение Тогда
			...
			ТекстСообщения = НСтр("ru='При давальческом сырье Владелец не может быть собственным подразделением. Выберите другого Владельца!';uk='При давальницькій сировині Власник не може бути власним підрозділом. Оберіть іншого Власника!'");
...
&НаКлиенте
Процедура ВладелецВОперативномУчетеПриИзменении(Элемент)
	
	Если ЗначениеЗаполнено(Объект.ВладелецВОперативномУчете) Тогда		
		
		ПараметрыОтбораДоговора = ПолучитьПараметрыДляДоговоров();
		
		ИНАГРО_ЭлеваторЗаполнениеДокументов.ПриИзмененииЗначенияКонтрагента(Объект.Дата, Объект.Организация, Объект.ВладелецВОперативномУчете, Объект.ДоговорКонтрагентаВОперативномУчете, ПараметрыОтбораДоговора);		
...
Процедура СырьеНоменклатураПриИзмененииНаСервере(СтрокаТабличнойЧасти, Знач ДанныеОбъекта)	
	...
	Документы.ИНАГРО_АктСписания.ЗаполнитьСчетаУчетаВСтрокеТабличнойЧасти(ДанныеОбъекта, СтрокаТабличнойЧасти, СведенияОНоменклатуре);
...
Процедура СоздатьЛабораторныйАнализ(Команда)
	...
	Если ТекущийЭлемент = Элементы.Сырье Тогда
		
		ТекущиеДанные = Элементы.Сырье.ТекущиеДанные;
		
		Если Объект.Сырье.Количество() = 0 ИЛИ ТекущиеДанные = Неопределено Тогда			
			Возврат;
		Иначе
...
Процедура ЗаполнитьПоРецептуНаСервере()
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.Рецепт.СписокНоменклатуры Цикл		
		СтрокаСырье = Объект.Сырье.Добавить();
		СтрокаСырье.Номенклатура     = СтрокаТабличнойЧасти.Номенклатура;			
		СтрокаСырье.ЕдиницаИзмерения = СтрокаТабличнойЧасти.Номенклатура.БазоваяЕдиницаИзмерения;
		СтрокаСырье.Коэффициент      = 1;			
		СтрокаСырье.ФизическийВес    = Окр(Объект.Рецепт.ВесПартии * СтрокаТабличнойЧасти.Процент /Сто * (100 + Объект.ПроцентПотерь) / 100, 3);
		СтрокаСырье.ЗачетныйВес      = ИНАГРО_Элеватор.ЗачетныйВесПриПроведении(ПолучитьПараметрыДляРасчетаЗачетногоВеса(СтрокаСырье));		
	КонецЦикла;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.СписатьНаЗатраты.Доступность            = НЕ СкладскойУчет;	
...
Процедура УстановитьПараметрыВыбораВидОперации()
	...
	МассивВидовОпераций = Документы.ИНАГРО_АктСписания.ПолучитьФиксированныйМассивВидовОпераций();
...
Процедура УстановитьЗаголовокФормы()
	...
	ТекстЗаголовка = НСтр("ru='Акт списания';uk='Акт списання'");
...
Процедура ЗаполнитьСчетаУчетаВТабличнойЧасти()
	...
	Если Объект.Сырье.Количество() > 0 Тогда
		Документы.ИНАГРО_АктСписания.ЗаполнитьСчетаУчетаВТабличнойЧасти(Объект, "Сырье", СобственноеПодразделение);
	КонецЕсли;	
...
Функция ПолучитьДанныеСтрокиТаблицыСырье(ТекущиеДанные)
	...
	ДанныеСтрокиТаблицы = Новый Структура(
		"ЛабораторныйАнализ, НомерАнализа, Номенклатура,
...
Функция ПолучитьПараметрыДляРасчетаЗачетногоВеса(СтрокаТабличнойЧасти)
	...
	ПараметрыДляРасчетаЗачетногоВеса = Новый Структура(
		...
		|ЗерноваяПримесь, ФизическийВес, ЗачетныйВес,     
		|");
