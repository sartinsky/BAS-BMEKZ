...
Процедура ИзменитьВидОперации(Команда)
	...
	ОткрытьФорму("Документ.ИНАГРО_РасходныйОрдерНаТовары.Форма.ФормаДокумента", ПараметрыФормы, ЭтаФорма);
...
&НаКлиенте
Процедура СоздатьВозвратПоПриходномуОрдеру(Команда)

	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийРасходныйОрдер.ВозвратПоПриходномуОрдеру"));
	ОткрытьФорму("Документ.ИНАГРО_РасходныйОрдерНаТовары.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьРасходПоНакладной(Команда)
	
	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийРасходныйОрдер.РасходПоНакладной"));
	ОткрытьФорму("Документ.ИНАГРО_РасходныйОрдерНаТовары.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);
...
Процедура СоздатьВозвратПоставщику(Команда)
	...
	СтруктураПараметров = ПолучитьСтруктуруПараметровФормы(
		ПредопределенноеЗначение("Перечисление.ИНАГРО_ВидыОперацийРасходныйОрдер.ВозвратПоставщику"));
	ОткрытьФорму("Документ.ИНАГРО_РасходныйОрдерНаТовары.ФормаОбъекта", СтруктураПараметров, ЭтаФорма);
