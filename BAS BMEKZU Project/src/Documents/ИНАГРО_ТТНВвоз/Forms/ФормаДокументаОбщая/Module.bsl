...
Процедура ПодборНомераПробы(Команда)
	...
	ОткрытьФорму("Документ.ИНАГРО_ТТНВвоз.Форма.ФормаЗаполненияНомераПробы", ПараметрыФормы, ЭтаФорма, УникальныйИдентификатор, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура УправлениеПоследовательностьюВводаВеса()
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если КонтролироватьПоследовательностьВводаВеса Тогда			
			...
			ДоступностьКнопкиF5     =  Объект.ВесБрутто1 = 0 И Объект.ВесТары = 0 И Объект.ВесТары1 = 0;
			ДоступностьКнопкиF6     = (Объект.ВесБрутто <> 0 И Объект.ВесТары = 0 И Объект.ВесТары1 = 0 И НЕ ЗначениеЗаполнено(Объект.НомерПрицепа))
			                      ИЛИ (Объект.ВесБрутто <> 0 И Объект.ВесБрутто1 <> 0 И Объект.ВесТары = 0 И Объект.ВесТары1 = 0);
			ДоступностьКнопкиF7     =  Объект.ВесБрутто <> 0 И Объект.ВесТары = 0  И Объект.ВесТары1 = 0 И ЗначениеЗаполнено(Объект.НомерПрицепа);
