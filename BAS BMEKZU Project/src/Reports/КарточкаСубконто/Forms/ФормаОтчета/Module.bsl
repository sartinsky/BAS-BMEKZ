...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru='Карточка субконто ';uk='Картка субконто '") + ТекстСубконто + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
...
Процедура ЗаполнитьПризнакиУчета()
	...
	|	МАКСИМУМ(ВС1.Ссылка.УчетПоНалоговымНазначениямНДС) КАК УчетПоНалоговымНазначениямНДС,
	...
	ЕстьУчетПоНалоговымНазначениямНДС = Ложь;
	ЕстьСчета                = Ложь;	
	...
	Если МассивСубконто.Количество() = 0 Тогда
		...
		ЕстьУчетПоНалоговымНазначениямНДС = Истина;
		ЕстьСчета                = Истина;
	Иначе
		...
		Пока ВыборкаСчета.Следующий() Цикл
			...
			ЕстьУчетПоНалоговымНазначениямНДС = ?(ВыборкаСчета.УчетПоНалоговымНазначениямНДС = Истина, Истина, Ложь);
	...
	Если ЗаполняемыеНастройки.Свойство("Отбор") Тогда
		Если ЗаполняемыеНастройки.Отбор Тогда
			...
			Если ЕстьНалоговыйУчет Тогда
				Поле = Отчет.КомпоновщикНастроек.Настройки.ДоступныеПоляОтбора.НайтиПоле(Новый ПолеКомпоновкиДанных("НалоговоеНазначение"));
				БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(Отчет.КомпоновщикНастроек, "НалоговоеНазначение", Поле.Тип.ПривестиЗначение(Неопределено), , Ложь); 
			КонецЕсли;
