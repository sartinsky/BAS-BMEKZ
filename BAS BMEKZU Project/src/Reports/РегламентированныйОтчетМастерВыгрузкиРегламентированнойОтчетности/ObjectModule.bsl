	...
	Если Значение <> "&НеЗаписывать" Тогда
		...
		Иначе
			...
			ИначеЕсли НЕ СтруктураАттрибутов = Неопределено Тогда
				Сообщить(НСтр("ru='Неверный список аттрибутов записываемого элемента!';uk='Невірний список атрибутів записуваного елемента'"), СтатусСообщения.Важное);
			КонецЕсли;			
			Возврат
		КонецЕсли;
			...
			Иначе	
				...
				Для каждого СтрокаТаблицы Из ТаблицаЗапроса Цикл
					...
					Если НЕ ЗначениеЗаполнено(QDATE) ИЛИ НЕ ТипЗнч(QDATE) = Тип("Дата") Тогда
						...
						СтрокаОшибки.ОписаниеОшибки 	= НСтр("ru='Не заполнена дата';uk='Чи не заповнена дата'");
					КонецЕсли;
					...
					Если Не ЗначениеЗаполнено(QNUM) и НЕ ЗначениеЗаполнено(RNUM) Тогда
						...
						СтрокаОшибки.ОписаниеОшибки 	= НСтр("ru='Не указан ни номер налоговой накладной ни номер расчета корретировки';uk='Чи не вказано ні номер податкової накладної ні номер розрахунку корретіровкі'");
					КонецЕсли;
...
Функция ПреобразоватьТипXMLОбщий(Значение,Тип,Результат,ОписаниеОшибки = "")
	...
	ИначеЕсли  (ВРег(Тип) = ВРег("DGKontrOp"))
		...
		Попытка
			Если (Результат = "0") ИЛИ  (Результат = "1") ИЛИ  (Результат = "3") Тогда
				// это допустимо
			ИначеЕсли СтрДлина(Результат) <> 3 Тогда
				ОписаниеОшибки = НСтр("ru='Значением может быть только символ ""1"" или ""2"" или ""3""';uk='Значенням може бути тільки символ ""1"" або ""2"" або ""3""'");					
			КонецЕсли;
		Исключение
			ОписаниеОшибки = НСтр("ru='Значением может быть только символ ""1"" или ""2"" или ""3""';uk='Значенням може бути тільки символ ""1"" або ""2"" або ""3""'");					
		КонецПопытки;		
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGspecNom") 
		...
		Если  Результат<> "2"
			...
			И Результат<> "5"
			И Результат<> "7"Тогда
			ОписаниеОшибки = НСтр("ru='Значение не является одним из значений: 2,3,4,5 или 7';uk='Значення не є одним із значень: 2,3,4,5 або 7'");
		КонецЕсли;
	...
	ИначеЕсли (ВРег(Тип) = ВРег("Odoh1DFColumn"))
		...
		Попытка
			...
			Если    Найти(Результат, ".") > 0
				...
				ИЛИ Контроль > 200
				ИЛИ Цел(Контроль) <> Контроль Тогда
				ОписаниеОшибки = НСтр("ru='Значение не является целым числом от 101 до 200';uk='Значення не є цілим числом від 101 до 200'");
			КонецЕсли;
		Исключение
			ОписаниеОшибки = НСтр("ru='Значение не является целым числом от 101 до 200';uk='Значення не є цілим числом від 101 до 200'");
		КонецПопытки;
