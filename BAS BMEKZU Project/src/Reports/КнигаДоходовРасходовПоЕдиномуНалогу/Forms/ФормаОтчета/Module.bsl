...
Функция СформироватьОтчетНаСервере()
	...
	Если ИБФайловая Тогда
		...
		Отчеты.КнигаДоходовРасходовПоЕдиномуНалогу.СформироватьОтчет(ПараметрыОтчета, АдресХранилища);
		РезультатВыполнения = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор, 
			"Отчеты.КнигаДоходовРасходовПоЕдиномуНалогу.СформироватьОтчет", 
			ПараметрыОтчета, 
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Отчет 			= Форма.Отчет; 
	ЗаголовокОтчета = НСтр("ru='Типовая форма учета доходов и расходов ФОП (единый налог с НДС) ';uk='Типова форма обліку доходів і витрат ФОП (єдиний податок з ПДВ) '") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
