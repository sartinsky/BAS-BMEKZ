...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2023Мес7";
	...
	// Версия формы  
	СтруктураРеквизитовФормы.мВерсияФормы = "01/08/2023";
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ФИОДиректора           = Сведения.ИмяФАМИЛИЯРук;
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если НЕ СписокСохранения.Свойство("РежимУточненияПоказателей", РежимУточненияПоказателей) Тогда
		Если СписокСохранения.Свойство("РасчетЦеныТочный") Тогда
			Если СписокСохранения.РасчетЦеныТочный Тогда
				РежимУточненияПоказателейРасчетЦеныТочный = 2;	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("РежимУточненияПоказателей", РежимУточненияПоказателей);
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, НомерПервойСтроки = 1) Экспорт
	...
	НомУдаляемойСтроки = Число(Сред(ИмяЯчейки, РегламентированнаяОтчетностьКлиентСервер.НайтиПоследнееВхождение(ИмяЯчейки, "_") + 1));
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если    ИмяРаздела = "НалоговыйДокумент" Тогда
		...
		ПозицияПодстроки = РегламентированнаяОтчетностьКлиентСервер.НайтиПоследнееВхождение(ИмяОбласти, "_");
...
&НаСервере
Процедура ЗаполнитьАвтоНаСервере(ПараметрыЗаполненияЗвит1С, ИспользоватьДанныеЗЦ = Ложь) Экспорт
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С, ИспользоватьДанныеЗЦ = Ложь)
	...
	// делаем заглушку, чтобы вписаться в стандартный механизм печати
	...
	Если (РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БП" ИЛИ РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БСП") И ЗначениеЗаполнено(мНалоговыйДокумент.ОсновнаяНалоговаяПриРезервеНомераПриПродажаНижеОбычнойЦены) Тогда
	...
	ДанныеШапки.Свойство("HKS",  СтруктураДанныхНалоговыйДокумент.HKS);
	ДанныеШапки.Свойство("HKB", СтруктураДанныхНалоговыйДокумент.HKB);
	ДанныеШапки.Свойство("R01G14",  СтруктураДанныхНалоговыйДокумент.R01G14);
	ДанныеШапки.Свойство("R03G14", СтруктураДанныхНалоговыйДокумент.R03G14);
	...
	Для каждого СтрокаТЧ Из ДанныеТаблицы Цикл
		...
		Если РежимУточненияПоказателей = 1 Тогда
			...
			Если  ЗначениеЗаполнено(СтрокаМногострочки.RG5)
				...
				Если Окр(СтрокаМногострочки.RG5 * СтрокаМногострочки.RG6, 2) <> СтрокаМногострочки.RG010 Тогда
	            	СтрокаМногострочки.RG5 = СтрокаМногострочки.RG010 / СтрокаМногострочки.RG6;
				КонецЕсли;
				
			КонецЕсли;
			
		ИначеЕсли РежимУточненияПоказателей = 2 Тогда
...
&НаКлиенте
Процедура РежимУточненияПоказателейПриИзменении(Элемент)
