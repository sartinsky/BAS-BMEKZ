...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2023КвУФ";
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("ПериодОтчета",  НомерКварталРимск + " квартал " + Формат(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ = 'гггг';Л = uk_UA"), Форма, "Уведомление");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A05", "Показник ""Обсяг реалізованих послуг (у т. ч. ПДВ)""", "+", "!КО", A01, Счет70_3,,СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	
	// Дополнительный случай, продажа в розницу через 709 счет:
	...
	Если A01_доп <> 0 Тогда
		...
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A01", "", "+", "#З урахуванням ставки ПДВ "+Строка(СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице), 0);
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A05", "", "+", "!ОБ", A01_доп, Счет70_9, Счет70_3, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A05", "", "+", "#З урахуванням ставки ПДВ "+Строка(СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице), 0);
	КонецЕсли;
    A01 = A01 + A01_доп;
	...
	// Дополнительный случай, продажа в розницу через 7091 счет
	...
	Если A02_доп <> 0 Тогда
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, "A02", "", "+", "!ОБ", A02_доп, Счет70_9, Счет70_3, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	КонецЕсли;	
	A02 = A02 + A02_доп;	
	...
	//по месяцам 
	Для Нмес = 0 По 2 Цикл
		НачМесяца = ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета,НМес);
		КонМесяца = КонецДня(КонецМесяца(НачМесяца));
		мБухИтогиМес = Обработки.БухгалтерскиеИтоги.Создать();
		мБухИтогиМес.РассчитатьИтоги("Хозрасчетный", Итоги, "Сумма", "Счет, КорСчет", НачМесяца, КонМесяца, , , , , , "Организация", СтруктураРеквизитовФормы.Организация);

		// 01. Обсяг реалізованих послуг (у т. ч. ПДВ)
		ИмяЯчейки = "B"+Формат(Месяц(НачМесяца), "ЧЦ=2; ЧВН=");
		B_мес = КО(мБухИтогиМес, Счет70_3);
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, ИмяЯчейки, Формат(НачМесяца, "ДФ = 'ММММ гггг';Л = uk_UA"), "+", "!КО", B_мес, Счет70_3,,НачМесяца, КонМесяца);

		// 05. Податок на додану вартість [з рядка 01]
		НДС_мес = ОБ(мБухИтогиМес, Счет70_3, Счет64);
		_ДобавитьСтрокуРасшифровки(ЭтаФорма, ИмяЯчейки, "Показник ""Податок на додану вартість""", "-", "!ОБ", НДС_мес, Счет70_3, Счет64, НачМесяца, КонМесяца);

		// Дополнительный случай, продажа в розницу через 709 счет:
		ОБ_709_703 = ОБ(мБухИтогиМес, Счет70_9, Счет70_3);
		
		// Для розничных продаж через 709 счет кредитовый оборот по 703 счету без НДС, дополним недостающий НДС 		
	    B_доп = Окр(ОБ_709_703 * СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице/100, 2); // Сумма без НДС -> НДС
		Если B_доп <> 0 Тогда
			_ДобавитьСтрокуРасшифровки(ЭтаФорма, ИмяЯчейки, "", "+", "!ОБ", B_доп, Счет70_9, Счет70_3, НачМесяца, КонМесяца);
			_ДобавитьСтрокуРасшифровки(ЭтаФорма, ИмяЯчейки, "", "+", "#З урахуванням ставки ПДВ "+Строка(СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице), 0);
		КонецЕсли;

		// Дополнительный случай, продажа в розницу через 7091 счет.	
		НДС_доп = Окр(ОБ_709_703 * СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице/100, 2); // Сумма без НДС -> НДС
		Если НДС_доп <> 0 Тогда
			_ДобавитьСтрокуРасшифровки(ЭтаФорма, ИмяЯчейки, "", "-", "!ОБ", НДС_доп, Счет70_9, Счет70_3, НачМесяца, КонМесяца);
			_ДобавитьСтрокуРасшифровки(ЭтаФорма, ИмяЯчейки, "", "-", "#З урахуванням ставки ПДВ "+Строка(СтруктураРеквизитовФормы.ПроцентСтавкиНДСВРознице), 0);
		КонецЕсли;
	    B_мес = B_мес + B_доп - (НДС_мес + НДС_доп);
		УстановитьЗначениеПоказателя(ИмяЯчейки, B_мес);
	КонецЦикла;	
