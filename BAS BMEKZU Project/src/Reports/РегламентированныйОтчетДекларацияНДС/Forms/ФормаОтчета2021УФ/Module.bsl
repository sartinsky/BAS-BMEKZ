...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы7     = "П7T1R";
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Приложение ДС8';uk='Додаток ДС8'");
	СтрокаУровня1.ИмяОбластиТабличногоПоля = "ОбластьПечатиПоВысоте|ОбластьПечатиПоШирине";
	СтрокаУровня1.ОриентацияСтраницы       = ОриентацияСтраницы.Портрет;
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
...
Процедура ЗаполнитьПриложение9(Форма)
	...
	// ИНАГРО
	ЗаполнитьПоказательНаСервере("R006G3D",  Форма.СтруктураРеквизитовФормы.Организация.ИНАГРО_ДатаВключенияПоДотации, Форма, "Приложение8");
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	R140GA = ПолучитьПоказательНаСервере("R022G4", Форма, "Приложение1");
	R140GB = ПолучитьПоказательНаСервере("R022G5", Форма, "Приложение1")
	...
	ТекЗначениеФлага =  ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R0111G6", Форма, "Приложение1"))
					...
					ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R0122G6", Форма, "Приложение1"))
	...
	ТекЗначениеФлага = ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R0213G6", Форма, "Приложение1"))
				   ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R0222G4", Форма, "Приложение1"));
	ЗаполнитьПоказательНаСервере("HBD", ?(ТекЗначениеФлага, "1",""), Форма, "Декларация"); 																
	ЗаполнитьПоказательНаСервере("HD8", ?(ТекЗначениеФлага, "X",""), Форма, "Декларация"); 																

	////////////////////////////////////////////////
	//Расчет для Приложения 5 (Справки к деларации)
	...
	ТекЗначениеФлага = (Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)=12)
		 И (    ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R02G8", Форма, "Приложение6"))
			ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R02G9", Форма, "Приложение6"))		
			ИЛИ ЗначениеЗаполнено(R031G7)		
			ИЛИ ЗначениеЗаполнено(R032G7)		
		    ИЛИ ЗначениеЗаполнено(R033G7));
	ЗаполнитьПоказательНаСервере("HD6", ?(ТекЗначениеФлага, "X",""), Форма, "Декларация"); 																
	...
	НовосозданноеПредприятие = ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HPVN", Форма, "Приложение8"));
 	...
 	// -- ИНАГРО. Расчет Приложения 9 происходит при заполнении удельного веса.
	...
	ТекЗначениеФлага =  ЗначениеЗаполнено(R010G3)
...
Процедура ОчиститьНаСервере() Экспорт
	...
	//СтруктураДанныхПриложение8.Очистить(); // ИНАГРО
	...
	Если Не СтруктураРеквизитовФормы.НаимТекущегоРаздела = "Приложение8" Тогда
		ОчиститьТабличноеПолеНаСервере(СтруктураРеквизитовФормы.НаимТекущегоРаздела);
	КонецЕсли;	
	//ИНАГРО --	
	...
	Для i = 0 по МассивСтатей.Количество()- 1 Цикл
		...
		Для каждого строка Из НужныеСтроки Цикл
			...
			Если  Признак = "НО" Тогда
				...
				ИначеЕсли Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС14 Тогда
					//14%
		...
		Для каждого строка Из НужныеСтроки Цикл
			...
			Если ТипЗнч(Строка.Регистратор) = Тип("ДокументСсылка.Приложение2КНалоговойНакладной") Тогда
				Если НЕ Строка.РегВЕРНН И Строка.БазаНДСОборот<=0 Тогда 
					...
					Продолжить;
				//Иначе	
				//	R511GA = R511GA + строка.БазаНДСОборот;		
				КонецЕсли;
			КонецЕсли;	
			НоваяСтрока 			   = ТаблицаКонтрагентов.Добавить();  
	...
	Для каждого строка Из Таблица Цикл
		...
		Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации()  =  "ЕРП" Тогда
			Если Строка.НалоговаяНакладная.Валюта  <> мВалютаРегламентированногоУчета Тогда
					...
					НоваяСтрока.БазаНДС = РаботаСКурсамиВалютКлиентСервер.ПересчитатьИзВалютыВВалюту(Строка.БазаНДСОборот, 
	...
	Для Каждого СтатьяДекларации из МассивСтатей Цикл
		...
		Для каждого строка Из НайденныеСтроки Цикл     
			...
			Иначе
				...
				Иначе
					Если Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС7 ИЛИ    //строка 7
						Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС14 ИЛИ   
						Строка.СтавкаНДС = Перечисления.СтавкиНДС.НДС20 Тогда  
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		ТекстЗапросаНалоговыйКредит = "ВЫБРАТЬ
									  ...
									  |			КОГДА ЕСТЬNULL(НДСНалоговыйКредитОбороты.Регистратор.ИНАГРО_Распределять, ЛОЖЬ)
									  |				И ЕСТЬNULL(НДСНалоговыйКредитОбороты.Регистратор.СпецРежимНалогообложения, 0) = 2
									  |			ТОГДА НДСНалоговыйКредитОбороты.БазаНДС * ЕСТЬNULL(&КоэффициентРаспределения, 0)
									  |		ИНАЧЕ НДСНалоговыйКредитОбороты.БазаНДС
									  |	КОНЕЦ КАК ЧИСЛО(15, 2))) КАК БазаНДСОборот,
									  |	СУММА(ВЫРАЗИТЬ(ВЫБОР
									  |			КОГДА ЕСТЬNULL(НДСНалоговыйКредитОбороты.Регистратор.ИНАГРО_Распределять, ЛОЖЬ)
									  |					И ЕСТЬNULL(НДСНалоговыйКредитОбороты.Регистратор.СпецРежимНалогообложения, 0) = 2
									  |				ТОГДА НДСНалоговыйКредитОбороты.НДС * ЕСТЬNULL(&КоэффициентРаспределения, 0)
									  |			ИНАЧЕ НДСНалоговыйКредитОбороты.НДС
									  |		КОНЕЦ КАК ЧИСЛО(15, 2))) КАК НДСОборот,
									  //-- ИНАГРО
									  ...
									  |		0,
									  |		НДСНалоговыйКредитОбороты.Организация,
									  |		НДСНалоговыйКредитОбороты.СтатьяДекларацииНДСНалоговыйКредит,
									  |		НДСНалоговыйКредитОбороты.ДоговорКонтрагента,
									  |		НДСНалоговыйКредитОбороты.СтавкаНДС,
									  |		МАКСИМУМ(НАЧАЛОПЕРИОДА(НДСНалоговыйКредитОбороты.Период, ДЕНЬ)),
									  |		НДСНалоговыйКредитОбороты.Регистратор,
									  |		НДСНалоговыйКредитОбороты.Регистратор.ДатаВходящегоДокумента,
									  |		НДСНалоговыйКредитОбороты.Регистратор.ДатаВходящегоДокумента,
									  |		НДСНалоговыйКредитОбороты.Регистратор.Дата,
									  |		СУММА(НДСНалоговыйКредитОбороты.БазаНДС - (ВЫРАЗИТЬ(НДСНалоговыйКредитОбороты.БазаНДС * ЕСТЬNULL(&КоэффициентРаспределения, 0) КАК ЧИСЛО(15, 2)))),
									  |		СУММА(НДСНалоговыйКредитОбороты.НДС - (ВЫРАЗИТЬ(НДСНалоговыйКредитОбороты.НДС * ЕСТЬNULL(&КоэффициентРаспределения, 0) КАК ЧИСЛО(15, 2)))),
									  |		МАКСИМУМ(НДСНалоговыйКредитОбороты.ДоговорКонтрагента.СхемаНалоговогоУчета.МоментОпределенияБазыНДСПоПродажам),
									  |		НДСНалоговыйКредитОбороты.ДоговорКонтрагента.Владелец,
									  |		МАКСИМУМ(ВЫРАЗИТЬ(НДСНалоговыйКредитОбороты.ДоговорКонтрагента.Владелец.НаименованиеПолное КАК СТРОКА(1000))) КАК ИмяКонтрагента,
									  |		МАКСИМУМ(ВЫБОР
									  |				КОГДА НДСНалоговыйКредитОбороты.Регистратор.ВидОперации = &ЗКПРаботыОтНерезидента
									  |					ТОГДА ""500000000000""
									  |				ИНАЧЕ НДСНалоговыйКредитОбороты.ДоговорКонтрагента.Владелец.ИНН
									  |			КОНЕЦ),
									  |		НДСНалоговыйКредитОбороты.Регистратор.ДатаВходящегоДокумента,
									  ...
									  |		И НЕ ЕСТЬNULL(НДСНалоговыйКредитОбороты.Регистратор.ВключаетсяВУточняющийРасчет, ЛОЖЬ)
									  |		И ЕСТЬNULL(НДСНалоговыйКредитОбороты.Регистратор.СпецРежимНалогообложения, 0) = 2
									  |		И ЕСТЬNULL(НДСНалоговыйКредитОбороты.Регистратор.ИНАГРО_Распределять, ЛОЖЬ)
									  |		И (НДСНалоговыйКредитОбороты.БазаНДС - (ВЫРАЗИТЬ(НДСНалоговыйКредитОбороты.БазаНДС * ЕСТЬNULL(&КоэффициентРаспределения, 0) КАК ЧИСЛО(15, 2))) <> 0
									  |				ИЛИ НДСНалоговыйКредитОбороты.НДС - (ВЫРАЗИТЬ(НДСНалоговыйКредитОбороты.НДС * ЕСТЬNULL(&КоэффициентРаспределения, 0) КАК ЧИСЛО(15, 2))) <> 0)
									  |	СГРУППИРОВАТЬ ПО
									  |     0,
									  |		НДСНалоговыйКредитОбороты.Организация,
									  |		НДСНалоговыйКредитОбороты." + ИмяКолонкиНК + ",
	...
	Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
		R03G4 = СтруктураДанныхПриложение8.R03G4;
		КоэффициентРаспределения = Окр(R03G4/100,4);
	Иначе
	...
	МассивСтатей = Новый Массив; 
	МассивСтатей.Добавить(СтрокиДекларацииНДС.НДС_НОПоСтавке20);
	МассивСтатей.Добавить(СтрокиДекларацииНДС.НДС_НОПоСтавке20ПоставкаОФ);
		
	Таблица = ТаблицаРезультатов.Скопировать();
	БазаНДСОборотСх = 0;
	НДСОборотСх = 0;
	Для i = 0 по МассивСтатей.Количество()- 1 Цикл
		...
		Отбор = Новый Структура;
		Отбор.Вставить(ИмяКолонкиНО, СтатьяДекларации);
		НужныеСтроки = Таблица.НайтиСтроки(Отбор);
		
		Для каждого строка Из НужныеСтроки Цикл
					
			Если  Строка.СпецРежим = 2 И НЕ Строка.ИННКонтрагента = "600000000000" Тогда
				БазаНДСОборотСх = БазаНДСОборотСх +  строка.БазаНДСОборот;
				НДСОборотСх = НДСОборотСх +  строка.НДСОборот;
			КонецЕсли;
	...
	КорБазаНДСОборотСх = 0;
	КорНДСОборотСх = 0;
	Для Каждого СтатьяДекларации из МассивСтатей Цикл
		
		Отбор = Новый Структура;
		Отбор.Вставить("СтатьяДекларацииНДСНалоговыеОбязательства",СтатьяДекларации);
		НайденныеСтроки = Таблица.НайтиСтроки(Отбор);
		
		Для каждого строка Из НайденныеСтроки Цикл
				
			Если Строка.СпецРежим = 2 И НЕ Строка.ИННКонтрагента = "600000000000" Тогда
				КорБазаНДСОборотСх = КорБазаНДСОборотСх + Строка.БазаНДСОборот;		
				КорНДСОборотСх = КорНДСОборотСх + Строка.НДСОборот;		
			КонецЕсли;
	...
	R0101G3 = БазаНДСОборотСх + КорБазаНДСОборотСх;
	R0102G3 = НДСОборотСх + КорНДСОборотСх;
	
	Если R0101G3 > R0102G3 Тогда
		R0103G3 =  ОкруглитьЧислоПоФормату(ЭтаФорма, R0101G3) - ОкруглитьЧислоПоФормату(ЭтаФорма, R0102G3);
	Иначе											  
		R0103G3 = 0;
	КонецЕсли;
	
	СтруктураДанныхПриложение8.R0101G3 = R0101G3;
	СтруктураДанныхПриложение8.R0102G3 = R0102G3;
	//СтруктураДанныхПриложение8.R0103G3 = R0103G3;
	...
	Запрос.УстановитьПараметр("КоэффициентРаспределения", КоэффициентРаспределения); // ИНАГРО
	ТаблицаРезультатов = Запрос.Выполнить().Выгрузить();
	...
	// ++ ИНАГРО
	ДопСтруктураПоиска = Новый Структура();
	ДопСтруктураПоиска.Вставить("СтавкаНДС", Перечисления.СтавкиНДС.СпецСт8);   
	РасчетПоСтатье(МассивСтатей,R111GA,R111GB,"R111GA","R111GB",0,  ДопСтруктураПоиска);
	// -- ИНАГРО	
	...
	ДопСтруктураПоиска = Новый Структура();
	ДопСтруктураПоиска.Вставить("СтавкаНДС", Перечисления.СтавкиНДС.НДС14);
	...
	Если ПолучитьданныеЗаПрошлыйПериодИз1С Тогда	
		...
		ЗначениеПрошлойДекларацииПоНДС = ПоказателиПредыдущейДекларации(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "РегламентированныйОтчетУточняющийРасчетДекларацияНДС", НСтр("ru='""Уточняющий расчет декларация по НДС""';uk= '""Уточнюючий розрахунок декларація по ПДВ""'"),"Декларация","ДекларацияПоНДС",НСтр("ru='строки 16.2';uk='рядка 16.2'"),"R210GB6",Истина);
		СтруктураДанныхДекларация.R162GB = ЗначениеПрошлойДекларацииПоНДС;
	Иначе
		...
		Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2021 И
			Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3  Тогда
			ИмяСхемы = ПрефиксСхемы+"21"; //за февраль 2021
		Иначе
			ИмяСхемы = ПрефиксСхемы+"23"
		КонецЕсли;
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		...
		//////////////////////////////////
		ЗапросП6 = Новый Запрос();
		ЗапросП6.УстановитьПараметр("Организация", СтруктураРеквизитовФормы.Организация);
		ЗапросП6.УстановитьПараметр("НачПериод", СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
		ЗапросП6.УстановитьПараметр("КонПериод", СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		ЗапросП6.УстановитьПараметр("СпецРежимНалогообложения", СтруктураРеквизитовФормы.СпецРежим);
		
		Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" Тогда
				ЗапросП6.Текст = "ВЫБРАТЬ
		...
		РезультатЗапросаП6 = ЗапросП6.Выполнить();
		
		ДанныеФормы 	= ЭтаФорма[СтруктураРеквизитовФормы.мИдГруппы6];
		Таблица = ДанныеФормыВЗначение(ДанныеФормы, Тип("ТаблицаЗначений"));
		Таблица.Очистить();
		
		ВыборкаП6НН = РезультатЗапросаП6.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаП6НН.Следующий() Цикл
			
			ВыборкаП6П2 = ВыборкаП6НН.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
			Пока ВыборкаП6П2.Следующий() Цикл
				
				Строка = Таблица.Добавить();
				
				ВыборкаП6Детали = ВыборкаП6П2.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				
				Пока ВыборкаП6Детали.Следующий() Цикл
					
					Строка.П6T2RXXXXG2D = ВыборкаП6Детали.ННДата;			 
					
					ТекНомерНН = "" + РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(ВыборкаП6Детали.НННомер, Истина, Истина)
								 + ?(ЗначениеЗаполнено(ВыборкаП6Детали.ННСпецРежимНалогообложения), "/" + ВыборкаП6Детали.ННСпецРежимНалогообложения, "");			
					Строка.П6T2RXXXXG3S = ТекНомерНН;
					
					Строка.П6T2RXXXXG4D = ВыборкаП6Детали.П2Дата;			 
					
					ТекНомерП2 = "" + РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(ВыборкаП6Детали.П2Номер, Истина, Истина)
								 + ?(ЗначениеЗаполнено(ВыборкаП6Детали.П2СпецРежимНалогообложения), "/" + ВыборкаП6Детали.П2СпецРежимНалогообложения, "");			
								 
					Строка.П6T2RXXXXG5S = ТекНомерП2;			 
					
					Если ВыборкаП6Детали.СтавкаНДС = Перечисления.СтавкиНДС.НДС20 
						ИЛИ ВыборкаП6Детали.СтавкаНДС = Перечисления.СтавкиНДС.СпецСт8 Тогда // ИНАГРО

						Строка.П6T2RXXXXG6 = ВыборкаП6Детали.КорректировкаСуммы;			 
						Строка.П6T2RXXXXG9 = ВыборкаП6Детали.КорректировкаНДС;			 
						
						
					ИначеЕсли ВыборкаП6Детали.СтавкаНДС = Перечисления.СтавкиНДС.НДС7 Тогда
					
						Строка.П6T2RXXXXG7 = ВыборкаП6Детали.КорректировкаСуммы;			 
						Строка.П6T2RXXXXG10 = ВыборкаП6Детали.КорректировкаНДС;			 
						
					Иначе     //14%
						
						Строка.П6T2RXXXXG8 = ВыборкаП6Детали.КорректировкаСуммы;			 
						Строка.П6T2RXXXXG11 = ВыборкаП6Детали.КорректировкаНДС;			 
		...
		Пока Таблица.Количество() < мСтруктураИсхКолвоСтрокРазделов[СтруктураРеквизитовФормы.мИдГруппы6] Цикл
			Таблица.Добавить();
		КонецЦикла;
		Таблица.Сортировать("П6T2RXXXXG2D,П6T2RXXXXG3S,П6T2RXXXXG4D,П6T2RXXXXG5S");
		...
		РезультатЗапросаП6 = ЗапросП6.Выполнить();
		ВыборкаП6 = РезультатЗапросаП6.Выбрать();
		Пока ВыборкаП6.Следующий() Цикл
			
			ЛетИспользования = ВыборкаП6.ЛетИспользования;
			Если ЛетИспользования > 3 ИЛИ ЛетИспользования < 1 Тогда
				Продолжить;
			КонецЕсли;
			
			СтруктураДанныхПриложение6["R03" + ЛетИспользования + "G3"] = ВыборкаП6.СуммаПоставки;
			СтруктураДанныхПриложение6["R03" + ЛетИспользования + "G4"] = ВыборкаП6.НДСПоставки;
			СтруктураДанныхПриложение6["R03" + ЛетИспользования + "G5"] = ВыборкаП6.НДСПропорциональноКредитДоПерерасчета;
			СтруктураДанныхПриложение6["R03" + ЛетИспользования + "G6"] = ВыборкаП6.НДСПропорциональноКредит;
	...
	КонецЕсли;	
	ТекстЗапросаПриложение7 = "ВЫБРАТЬ
	...
	Запрос.Текст = ТекстЗапросаПриложение7;
	...
	ВыборкаПриложение7 = Запрос.Выполнить().Выбрать();
	
	СоответствиеПолейТаблица = Новый Структура();
	СоответствиеПолейТаблица.Вставить("П7T1RXXXXG2S",  "ВидДокумента");
	СоответствиеПолейТаблица.Вставить("П7T1RXXXXG3S",  "ИмяКонтрагента");
	СоответствиеПолейТаблица.Вставить("П7T1RXXXXG4",   "ИННКонтрагента");
	СоответствиеПолейТаблица.Вставить("П7T1RXXXXG5D",  "ДатаНН");
	СоответствиеПолейТаблица.Вставить("П7T1RXXXXG6S",  "НомерНН");
	СоответствиеПолейТаблица.Вставить("П7T1RXXXXG7",   "Сумма");
	СоответствиеПолейТаблица.Вставить("П7T1RXXXXG8",   "НДС");
	СоответствиеПолейТаблица.Вставить("П7T1RXXXXG9S",  "Описание");
	...
	Пока ВыборкаПриложение7.Следующий() Цикл
		...
		Если ВыборкаПриложение7.НомерТаблицыПриложения = 1 ИЛИ ВыборкаПриложение7.НомерТаблицыПриложения = 2 Тогда
		Иначе
			Продолжить;
		КонецЕсли;
		...
		Для каждого ПолеТаблицы Из СоответствиеПолейТаблица Цикл
			...
			СтрокаТаблицы[ПолеТаблицы.Ключ]	= ВыборкаПриложение7[ПолеТаблицы.Значение];
		
		КонецЦикла;

		СтруктураДанныхДекларация.HD7 = "Х";
		...
		Для каждого СохраненныйОтчет Из РезультатЗапроса Цикл
			...
			Если Уточняющий Тогда
				...
				Если  СлужебныеДанные.Свойство("HZY", ТекГодУточненки)
					И СлужебныеДанные.Свойство("HZKV",ТекКварталУточненки)
					И СлужебныеДанные.Свойство("HZM", ТекМесяцУточненки)Тогда
					
					ТекКварталУточненкиЧ = ?(ЗначениеЗаполнено(ТекКварталУточненки),Число(ТекКварталУточненки),0); 
					...
					Если НЕ (  Число(ТекГодУточненки) = ГодУточненки 
					  	     И ТекКварталУточненкиЧ	  = КварталУточненки
			...
			//уточняющих может быть несколько
			Попытка
				ЗначениеДекларации = ЗначениеДекларации + ПоказателиДекларации[ИмяПоказателя];
				ЕстьПодходящийОтчет = Истина;
			Исключение
				Возврат Неопределено;
			КонецПопытки;
				
			Если НЕ Уточняющий Тогда
...
Процедура  ОпределитьСписокВыгружаемыхСхем(тбСхемыXML);
	...
	Для Каждого стр Из  тбСхемыXML Цикл
		...
		ИначеЕсли стр.НазваниеСтраницыПанели = "Справка" Тогда 
			Если ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HD6", ЭтаФорма, "Декларация")) Тогда
				стр.фВыгрузка = 1;
			иначе	
				стр.фВыгрузка = 0;
			КонецЕсли;
...
Функция СформироватьДанныеДляЗвит1С(ОтчетЗвит1С = Неопределено) Экспорт 
	...
	Если НЕ глМенеджерЗвит1С.ФлагОтладки Тогда
		ПараметрыЗаполненияЗвит1С = Неопределено;
		ОчиститьП8НаКлиенте();
		ЗаполнитьУдельныйВесНаСервере();	
		РасчетУдельногоВеса(ЭтаФорма);
...
&НаКлиенте
Процедура ЗаполнитьУдельныйВес(Команда) // ИНАГРО
...
&НаКлиенте
Процедура ОчиститьП8НаКлиенте() // ИНАГРО
	
	ТекРаздел = ЭтаФорма.СтруктураРеквизитовФормы.НаимТекущегоРаздела;
	
	ИдентификаторСтроки = 0; 
	ИдентификаторТекСтроки = 0;
	ДеревоЭлементов = ЭтаФорма.РазделыОтчета.ПолучитьЭлементы();
	
	Для каждого Элемент Из ДеревоЭлементов Цикл
		Если Элемент.КолонкаРазделыОтчетаСокрНаим = "Приложение8" Тогда
			ИдентификаторСтроки = Элемент.ПолучитьИдентификатор(); 		
		КонецЕсли;
		
		Если Элемент.КолонкаРазделыОтчетаСокрНаим = ТекРаздел Тогда
			ИдентификаторТекСтроки = Элемент.ПолучитьИдентификатор(); 		
		КонецЕсли;
			
	КонецЦикла;  
	
	ЭтаФорма.Элементы.РазделыОтчета.ТекущаяСтрока = ИдентификаторСтроки;
	
	ОчиститьТабличноеПолеНаСервере(СтруктураРеквизитовФормы.НаимТекущегоРаздела);
	
	ЭтаФорма.Элементы.РазделыОтчета.ТекущаяСтрока = ИдентификаторТекСтроки;

	РегламентированнаяОтчетностьКлиент.ОбновитьДеревоРазделовВФормеОтчета(ЭтаФорма);
	
	// Типовая процедура. Хотя по факту заполняется данные 8 формы. 
	ЗаполнитьПриложение9НаСервере();	
	
КонецПроцедуры

&НаКлиенте
Процедура РасчетУдельногоВесаНаКлиенте() Экспорт  // ИНАГРО
...
&НаСервере
Процедура ЗаполнитьУдельныйВесНаСервере() // ИНАГРО
	
	Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
		ВнестиИнфоВПриложение9();
	КонецЕсли;
	
	//РасчетУдельногоВесаНаСервере(ЭтаФорма);
...
&НаСервере
Процедура РасчетУдельногоВесаНаСервере(ЭтаФорма) Экспорт // ИНАГРО
...
&НаКлиентеНаСервереБезКонтекста
Процедура РасчетУдельногоВеса(Форма) // ИНАГРО
	
	// Перенесено из Процедуры "Расчет" - Приложения 8
	...
	Для СтрНомер = 1 По 12 Цикл
		...
		Если  НовосозданноеПредприятие
			И ПолучитьПоказательНаСервере(ИмяТекущейЯчейкиМес, Форма, "Приложение8") >= Месяц(Форма.СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета)
			И ПолучитьПоказательНаСервере(ИмяТекущейЯчейкиМес, Форма, "Приложение8") <= Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)
			И ПолучитьПоказательНаСервере(ИмяТекущейЯчейкиГод, Форма, "Приложение8") = Год(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) Тогда
	...
	Если Не СуммаПоСтолбцу6 = 0 И НЕ НовосозданноеПредприятие Тогда
		ЗаполнитьПоказательНаСервере("HPV",   Окр(100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, 2), Форма, "Приложение8"); // ИНАГРО
	    ЗаполнитьПоказательНаСервере("R03G3", Окр(100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, 2), Форма, "Приложение8");
	    ЗаполнитьПоказательНаСервере("R03G4", Окр(100 * СуммаПоСтолбцу4 / СуммаПоСтолбцу6, 2), Форма, "Приложение8");
	    ЗаполнитьПоказательНаСервере("R03G5", Окр(100 * СуммаПоСтолбцу5 / СуммаПоСтолбцу6, 2), Форма, "Приложение8");
	Иначе
		ЗаполнитьПоказательНаСервере("HPV",   0, Форма, "Приложение8"); // ИНАГРО
	...
	Если Не СуммаПоСтолбцу6НП = 0 И НовосозданноеПредприятие Тогда
		ЗаполнитьПоказательНаСервере("HPVN",  Окр(100 * СуммаПоСтолбцу3НП / СуммаПоСтолбцу6НП, 2), Форма, "Приложение8"); // ИНАГРО
	    ЗаполнитьПоказательНаСервере("R04G3", Окр(100 * СуммаПоСтолбцу3НП / СуммаПоСтолбцу6НП, 2), Форма, "Приложение8");
	    ЗаполнитьПоказательНаСервере("R04G4", Окр(100 * СуммаПоСтолбцу4НП / СуммаПоСтолбцу6НП, 2), Форма, "Приложение8");
	    ЗаполнитьПоказательНаСервере("R04G5", Окр(100 * СуммаПоСтолбцу5НП / СуммаПоСтолбцу6НП, 2), Форма, "Приложение8");
	Иначе
		ЗаполнитьПоказательНаСервере("HPVN",  0, Форма, "Приложение8"); // ИНАГРО
...
&НаСервере
Процедура ВнестиИнфоВПриложение9() // ИНАГРО
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	И НДСНалоговыеОбязательстваОбороты.Период <= &КонДата
	                |	И НЕ ЕСТЬNULL(НДСНалоговыеОбязательстваОбороты.Регистратор.ВключаетсяВУточняющийРасчет, ЛОЖЬ)
	                |	И НДСНалоговыеОбязательстваОбороты.Период < НАЧАЛОПЕРИОДА(&НачДата, МЕСЯЦ)
	                |;
	                |
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                ...
	                |				КОГДА Приложение2КНалоговойНакладнойТовары.Ссылка.СуммаДокумента < 0
	                |						И Приложение2КНалоговойНакладнойТовары.Ссылка.СуммаНДСДокумента <> 0
	                |						И Приложение2КНалоговойНакладнойТовары.Ссылка.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	                ...
	                |ВЫБРАТЬ
	                |	13 - РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(ВЫБОР
	                |				КОГДА Приложение2КНалоговойНакладнойУслуги.Ссылка.СуммаДокумента < 0
	                |						И Приложение2КНалоговойНакладнойУслуги.Ссылка.СуммаНДСДокумента <> 0
	                |						И Приложение2КНалоговойНакладнойУслуги.Ссылка.ДатаВключенияВЕдиныйРеестрНалоговыхНакладных <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	                ...
	                |////////////////////////////////////////////////////////////////////////////////
	                |ВЫБРАТЬ
	                |	НДСПоНоменклатуре.Период КАК Период,
	                |	НДСПоНоменклатуре.СтавкаНДС,
	                |	НДСПоНоменклатуре.СтатьяДекларацииНДСНалоговыеОбязательства,
	                |	НДСПоНоменклатуре.НоменклатураИНАГРО_ВидПродукции,
	                |	НДСПоНоменклатуре.СуммаНДС,
	                |	НДСПоНоменклатуре.Сумма,
	                |	НДСПоНоменклатуре.ЭтоУсловнаяПродажа,
	                |	НДСПоНоменклатуре.СпецРежимНалогообложения,
	                |	НДСПоНоменклатуре.Дата,
	                |	НДСПоНоменклатуре.Номенклатура,
	                |	ТоварДотационный.Актуальность,
	                |	НАЧАЛОПЕРИОДА(ДОБАВИТЬКДАТЕ(&НачДата, МЕСЯЦ, -12), МЕСЯЦ) КАК Поле1
	                |ИЗ
	                |	НДСПоНоменклатуре КАК НДСПоНоменклатуре
	                |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	                |			ИНАГРО_ПродукцияДотационнаяСрезПоследних.Номенклатура КАК Номенклатура,
	                |			ИНАГРО_ПродукцияДотационнаяСрезПоследних.Актуальность КАК Актуальность
	                |		ИЗ
	                |			РегистрСведений.ИНАГРО_ПродукцияДотационная.СрезПоследних(&КонДата, ) КАК ИНАГРО_ПродукцияДотационнаяСрезПоследних
	                |		ГДЕ
	                |			ИНАГРО_ПродукцияДотационнаяСрезПоследних.Актуальность) КАК ТоварДотационный
	                |		ПО НДСПоНоменклатуре.Номенклатура = ТоварДотационный.Номенклатура
	                |ИТОГИ ПО
	                |	Период";
	...
	Запрос.УстановитьПараметр ("КонДата",КонецМесяца(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета));
	Запрос.УстановитьПараметр ("Организация", СтруктураРеквизитовФормы.Организация);
	...
	Пока РезультатПериод.Следующий() Цикл
		...
		Пока РезультатВидПродукции.Следующий() Цикл
			...
			Если РезультатВидПродукции.СтатьяДекларацииНДСНалоговыеОбязательства = СтрокиДекларацииНДС.НДС_НОПоСтавке0Экспорт
				...
				ИЛИ РезультатВидПродукции.СтатьяДекларацииНДСНалоговыеОбязательства =СтрокиДекларацииНДС.НДС_НОКорректировкаТрансфЦенообразование Тогда
					Если РезультатВидПродукции.ЭтоУсловнаяПродажа = Истина ТОГДА
					Иначе
						Если РезультатВидПродукции.НоменклатураИНАГРО_ВидПродукции = Перечисления.ИНАГРО_ВидыСХПродукции.ЗерновыеТехнические 
							ИЛИ РезультатВидПродукции.НоменклатураИНАГРО_ВидПродукции = Перечисления.ИНАГРО_ВидыСХПродукции.Животноводство 
							ИЛИ РезультатВидПродукции.НоменклатураИНАГРО_ВидПродукции = Перечисления.ИНАГРО_ВидыСХПродукции.Другое ТОгда
							
							СтруктураДанныхПриложение8["R02"+НомерСтроки+"G3"] = СтруктураДанныхПриложение8["R02"+НомерСтроки+"G3"] + РезультатВидПродукции.Сумма;
							Если РезультатВидПродукции.Актуальность = Истина Тогда
								СтруктураДанныхПриложение8["R02"+НомерСтроки+"G4"] = СтруктураДанныхПриложение8["R02"+НомерСтроки+"G4"] + РезультатВидПродукции.Сумма;
							КонецЕсли;
						Иначе 
							СтруктураДанныхПриложение8["R02"+НомерСтроки+"G5"] = СтруктураДанныхПриложение8["R02"+НомерСтроки+"G5"] + РезультатВидПродукции.Сумма;
						КонецЕсли
					КонецЕСли;
				КонецЕсли;
		КонецЦикла;
