...
Процедура ПодготовитьДанныеДляПриложения1КорВРазрезеКонтрагентов(Таблица,ИмяКолонки,Статья, Признак)
	...
	Для Каждого СтатьяДекларации из МассивСтатей Цикл
		...
		Для каждого строка Из НайденныеСтроки Цикл
			...
			Если Признак = "НО" И НЕ Строка.РегВЕРНН И Строка.НДСОборот<=0 Тогда 
				//не зарегистрированные и отрицательные
				Продолжить;
			КонецЕсли;
...
Процедура РасчетУдельногоВеса(Форма) // ИНАГРО
	...
	Если Не СуммаПоСтолбцу6 = 0 И НЕ НовосозданноеПредприятие Тогда
		ЗаполнитьПоказательНаСервере("HPV",   100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, Форма, "Приложение8"); // ИНАГРО
	...
	Если Не СуммаПоСтолбцу6НП = 0 И НовосозданноеПредприятие Тогда
		ЗаполнитьПоказательНаСервере("HPVN",  100 * СуммаПоСтолбцу3НП / СуммаПоСтолбцу6НП, Форма, "Приложение8"); // ИНАГРО
