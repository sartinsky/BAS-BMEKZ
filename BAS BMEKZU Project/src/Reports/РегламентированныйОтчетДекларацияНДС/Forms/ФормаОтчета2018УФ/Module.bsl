...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьФлаги(Форма)
	...
	ТекЗначениеФлага =  ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R014G4", Форма, "Приложение1"))
					...
					ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R140GA", Форма, "Декларация"))		
					ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R140GB", Форма, "Декларация"));		
	ЗаполнитьПоказательНаСервере("HD1", ?(ТекЗначениеФлага, "X",""), Форма, "Декларация"); 																
	
	ТекЗначениеФлага =  ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R210GB", Форма, "Декларация"));
	ЗаполнитьПоказательНаСервере("HD2", ?(ТекЗначениеФлага, "X",""), Форма, "Декларация"); 																

	ТекЗначениеФлага =  ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R010G3", Форма, "Приложение3"))
					ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R202GB", Форма, "Декларация"));		
	...
	ТекЗначениеФлага =  ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R50GA", Форма, "Декларация"));
	...
	ТекЗначениеФлага = (Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)=12)
		 И (    ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R02G8", Форма, "Приложение7"))
			...
			ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R031G7", Форма, "Приложение7"))		
			ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R032G7", Форма, "Приложение7"))		
		    ИЛИ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("R033G7", Форма, "Приложение7")));
...
Процедура РасчетУдельногоВеса()
	...
	СтруктураДанныхПриложение9.R02G3 = 0;
	СтруктураДанныхПриложение9.R02G4 = 0;
	СтруктураДанныхПриложение9.R02G5 = 0;
	СтруктураДанныхПриложение9.R02G6 = 0;
	...
	Для СтрНомер = 1 По 12 Цикл
		...
		СтруктураДанныхПриложение9[ИмяТекущейЯчейкиСтолбца6] = СтруктураДанныхПриложение9[ИмяТекущейЯчейкиСтолбца3] + СтруктураДанныхПриложение9[ИмяТекущейЯчейкиСтолбца5];
		
		СуммаПоСтолбцу3 = СуммаПоСтолбцу3 + СтруктураДанныхПриложение9[ИмяТекущейЯчейкиСтолбца3];	
		СуммаПоСтолбцу4 = СуммаПоСтолбцу4 + СтруктураДанныхПриложение9[ИмяТекущейЯчейкиСтолбца4];	
		СуммаПоСтолбцу5 = СуммаПоСтолбцу5 + СтруктураДанныхПриложение9[ИмяТекущейЯчейкиСтолбца5];
		СуммаПоСтолбцу6 = СуммаПоСтолбцу6 + СтруктураДанныхПриложение9[ИмяТекущейЯчейкиСтолбца6];
	КонецЦикла;	
	
	СтруктураДанныхПриложение9.R02G3 = СуммаПоСтолбцу3;
	СтруктураДанныхПриложение9.R02G4 = СуммаПоСтолбцу4;
	СтруктураДанныхПриложение9.R02G5 = СуммаПоСтолбцу5;
	СтруктураДанныхПриложение9.R02G6 = СуммаПоСтолбцу6;
	
	Если Не СуммаПоСтолбцу6 = 0 Тогда
		Если Не СтруктураРеквизитовФормы.ОтчетЗаДваМесяца Тогда
			СтруктураДанныхПриложение9.HPV    = Формат(100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
			СтруктураДанныхПриложение9.R03G3  = Формат(100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
			СтруктураДанныхПриложение9.R03G4  = Формат(100 * СуммаПоСтолбцу4 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
			СтруктураДанныхПриложение9.R03G5  = Формат(100 * СуммаПоСтолбцу5 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
			
			СтруктураДанныхПриложение9.HPVN   = 0;
			СтруктураДанныхПриложение9.R04G3  = 0;
			СтруктураДанныхПриложение9.R04G4  = 0;
			СтруктураДанныхПриложение9.R04G5  = 0;
		Иначе
			СтруктураДанныхПриложение9.HPVN    = Формат(100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
			СтруктураДанныхПриложение9.R04G3  = Формат(100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
			СтруктураДанныхПриложение9.R04G4  = Формат(100 * СуммаПоСтолбцу4 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
			СтруктураДанныхПриложение9.R04G5  = Формат(100 * СуммаПоСтолбцу5 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
			
			СтруктураДанныхПриложение9.HPV    = 0;
			СтруктураДанныхПриложение9.R03G3  = 0;
			СтруктураДанныхПриложение9.R03G4  = 0;
			СтруктураДанныхПриложение9.R03G5  = 0;
		КонецЕсли;
	Иначе
		СтруктураДанныхПриложение9.HPV    = 0;
		СтруктураДанныхПриложение9.R03G3  = 0;
...
&НаСервере
Процедура ЗаполнитьУдВесНаСервере()
