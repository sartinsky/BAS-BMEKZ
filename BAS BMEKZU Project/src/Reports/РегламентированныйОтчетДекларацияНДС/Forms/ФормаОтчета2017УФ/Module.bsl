...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	//ИНАГРО ++
	//Пересчет приложения 5 (ячеек по дотациям) производить не будем, так как в области заложены суммы по дотационным НН по неплательщикам
	//R012G4 = 0;
	//R012G5 = 0;
	//Для каждого Строка Из ТаблицаБлока Цикл
	...
	//ИНАГРО ++
	R0101G3 = ПолучитьПоказательНаСервере("R012G5", Форма, "Приложение5") + ПолучитьПоказательНаСервере("R011G5", Форма, "Приложение1");
	R0102G3 = ПолучитьПоказательНаСервере("R025G5", Форма, "Приложение5") + ПолучитьПоказательНаСервере("R025G6", Форма, "Приложение5") + ПолучитьПоказательНаСервере("R022G5", Форма, "Приложение1");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
		...
		R021G4 = СтруктураДанныхПриложение9.R021G4;
		КоэффициентРаспределения = Окр(R021G4/100,4);
	Иначе
	...
	Запрос.УстановитьПараметр("КоэффициентРаспределения", КоэффициентРаспределения);
...
Процедура ВнестиИнфоВПриложение9()
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |	ЕСТЬNULL(ТоварДотационный.Актуальность, ЛОЖЬ) КАК Актуальность,
	...
	Запрос.УстановитьПараметр ("НачДата", СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	//Запрос.УстановитьПараметр ("СпецРежим",СпецРежим);
	УсловнаяПродажа = Новый Массив();
	УсловнаяПродажа.Добавить(Перечисления.ВидыОперацийНалоговаяНакладная.УсловнаяПродажа);
	УсловнаяПродажа.Добавить(Перечисления.ВидыОперацийНалоговаяНакладная.УсловнаяПродажаСписаниеОС);
	УсловнаяПродажа.Добавить(Перечисления.ВидыОперацийПриложение2КНалоговойНакладной.УсловнаяПродажаВозврат);
		...
		Пока РезультатВидПродукции.Следующий() Цикл
			...
			Если РезультатВидПродукции.СтатьяДекларацииНДСНалоговыеОбязательства    = СтрокиДекларацииНДС.НДС_НОПоСтавке0Экспорт
				...
				ИЛИ РезультатВидПродукции.СтатьяДекларацииНДСНалоговыеОбязательства = СтрокиДекларацииНДС.НДС_НОКорректировкаТрансфЦенообразование Тогда
					Если РезультатВидПродукции.ЭтоУсловнаяПродажа Тогда
					Иначе
						Если РезультатВидПродукции.НоменклатураИНАГРО_ВидПродукции    = Перечисления.ИНАГРО_ВидыСХПродукции.ЗерновыеТехнические 
							...
							Если РезультатВидПродукции.Актуальность Тогда
	...
	////Сразу расчитаем коэффициенты, так как они нужны нам для дальнейшего заполнения таблиц	
	//СуммаПоСтолбцу3 = 0;
	//СуммаПоСтолбцу4 = 0;
	//СуммаПоСтолбцу5 = 0;
	//СуммаПоСтолбцу6 = 0;
	...
	//
	//Если Не СуммаПоСтолбцу6 = 0 Тогда
	//	СтруктураДанныхПриложение9.R021G3  = Формат(100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
	//	СтруктураДанныхПриложение9.HPV     = Формат(100 * СуммаПоСтолбцу3 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
	//	
	//	СтруктураДанныхПриложение9.R021G4  = Формат(100 * СуммаПоСтолбцу4 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
	//	СтруктураДанныхПриложение9.R021G5  = Формат(100 * СуммаПоСтолбцу5 / СуммаПоСтолбцу6, "ЧЦ=5; ЧДЦ=2");
	//Иначе
	//	СтруктураДанныхПриложение9.R021G3  = 0;
	//	СтруктураДанныхПриложение9.R021G4  = 0;
	//	СтруктураДанныхПриложение9.R021G5  = 0;
	//КонецЕсли;
