...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("ЗаполнятьПриложение5", Ложь);
	СтруктураРеквизитовФормы.Вставить("мЗаполнятьПриложение5", Ложь);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если НЕ СписокСохранения.Свойство("ЗаполнятьПриложение5", СтруктураРеквизитовФормы.ЗаполнятьПриложение5) Тогда
		СтруктураРеквизитовФормы.ЗаполнятьПриложение5 = Ложь;
	КонецЕсли;	
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("ЗаполнятьПриложение5", СтруктураРеквизитовФормы.ЗаполнятьПриложение5);
...
Процедура ПодготовитьДанныеДляПриложения1ННВРазрезеКонтрагентов(Таблица,МассивСтатей,ИмяКолонки,Признак)
	...
	Иначе
		...
		НДС20СХ  = Перечисления.СтавкиНДС.СпецСт8;	
	КонецЕсли;
	...
	Для i = 0 по МассивСтатей.Количество()- 1 Цикл
		...
		Для каждого строка Из НужныеСтроки Цикл
			...
			Если  Признак = "НО" Тогда
				...
				ИначеЕсли Строка.СтавкаНДС = НДС20 
					ИЛИ Строка.СтавкаНДС = НДС20СХ Тогда
					//20%
			...
			ИначеЕсли Признак = "НК" Тогда	
				...
				Если  НЕ Строка.СтавкаНДС = НДС7
					...
					И НЕ Строка.СтавкаНДС = НДС20 
					И НЕ Строка.СтавкаНДС = НДС20СХ Тогда // ИНАГРО
				...
				Если ТипЗнч(Строка.Регистратор) = Тип(ТипДокументаИмпорт) Тогда
					...
					ИначеЕсли Строка.СтавкаНДС = НДС20 
						ИЛИ Строка.СтавкаНДС = НДС20СХ Тогда // ИНАГРО
					...
					Иначе
						// импорт без НДС в Декларации не отражается.
...
Процедура ПодготовитьДанныеДляПриложения1КорВРазрезеКонтрагентов(Таблица,ИмяКолонки,Статья, Признак)
	...
	Иначе
		...
		НДС7   = Перечисления.СтавкиНДС.НДС7;	
		НДС14  = Перечисления.СтавкиНДС.НДС14;
		НДС20СХ  = Перечисления.СтавкиНДС.СпецСт8; // ИНАГРО
	КонецЕсли;
			...
			Иначе
				...
				Иначе
					Если Строка.СтавкаНДС = НДС7 ИЛИ    //строка 7
						...
						Строка.СтавкаНДС = НДС20 ИЛИ 
						Строка.СтавкаНДС = НДС20СХ Тогда // ИНАГРО	
	...
	ТаблицаКонтрагентов.Свернуть("ИННКонтрагента,Дата,КонтрагентСпецКода,ОС,УслугиНерезид,Номер", "БазаНДС,НДС,НДС7,НДС14");
	ТаблицаКонтрагентов.Сортировать("ИННКонтрагента,Дата");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если    СтруктураРеквизитовФормы.мЗаполнятьПриложение5 = Истина 
		ИЛИ СтруктураРеквизитовФормы.ЗаполнятьПриложение5 Тогда
		// 5 
	...
	// ++ ИНАГРО
	ДопСтруктураПоиска = Новый Структура();
	ДопСтруктураПоиска.Вставить("СтавкаНДС", НДС20СХ);   
	РасчетПоСтатье(МассивСтатей,R111GA,R111GB,"R111GA","R111GB",0,  ДопСтруктураПоиска);
	// -- ИНАГРО	
	...
	Иначе
		...
		Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2023 И
			Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 4  Тогда
			ИмяСхемы = ПрефиксСхемы+"23"; //за март 2023
		Иначе
			ИмяСхемы = ПрефиксСхемы+"24"
		КонецЕсли;
		...
		Пока ВыборкаП6НН.Следующий() Цикл
			...
			Пока ВыборкаП6П2.Следующий() Цикл
				...
				Пока ВыборкаП6Детали.Следующий() Цикл
					...
					Если ВыборкаП6Детали.СтавкаНДС = НДС20 
						ИЛИ ВыборкаП6Детали.СтавкаНДС = НДС20СХ Тогда // ИНАГРО
					
						Строка.П6T2RXXXXG6 = ВыборкаП6Детали.КорректировкаСуммы;			 
						Строка.П6T2RXXXXG9 = ВыборкаП6Детали.КорректировкаНДС;			 
						
						
					ИначеЕсли ВыборкаП6Детали.СтавкаНДС = НДС7 Тогда
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Иначе // физ. лицо	
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0200124"; 
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Декларация о НДС';uk='Декларація про ПДВ'");
		НовСтрока.НазваниеСтраницыПанели ="Декларация";
		НовСтрока.Префикс = "Д";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0200424"; 
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Приложение 1 (Информация о суммах НДС)';uk='Додаток 1 (Відомості про суми ПДВ)'");
		НовСтрока.НазваниеСтраницыПанели ="Приложение1";
		НовСтрока.Префикс = "П1";
		
		НовСтрока= тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0215224"; 
		...
		НовСтрока= тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0200524"; 
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Приложение 3 (Расчет суммы бюджетного возмещения)';uk='Додаток 3 (Розрахунок суми бюджетного відшкодування)'");
		НовСтрока.НазваниеСтраницыПанели ="Приложение3";
		НовСтрока.Префикс = "П3";

		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0299824";
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Приложение 4 (Заявление про возврат суммы бюджетного возмещения)';uk='Додаток 4(Заява про повернення сумми бюджетного відшкодування)'");
		НовСтрока.НазваниеСтраницыПанели ="Приложение4";
		НовСтрока.Префикс ="П4";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0299324";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение 5 (Расчет суммы льгот)';uk='Додаток 5 (Розрахунок суми пільг)'");
		НовСтрока.НазваниеСтраницыПанели ="Справка";
		НовСтрока.Префикс ="С1";

		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0215724"; 
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение 6 ';uk='Додаток 6 '");
		НовСтрока.НазваниеСтраницыПанели ="Приложение7";
		НовСтрока.Префикс ="П6";
		                                    
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0215824";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение 7 ';uk='Додаток 7 '");
		НовСтрока.НазваниеСтраницыПанели ="Приложение7";
		НовСтрока.Префикс ="П7";
...
Процедура ОткрытьФормуПараметров(Команда)
	...
	мПараметрыФормы.Вставить("ЗаполнятьПриложение5", СтруктураРеквизитовФормы.ЗаполнятьПриложение5);
