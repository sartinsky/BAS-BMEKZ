...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "D1T1R";
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	ЗаполнитьПоказательНаСервере("HFILL",  Форма.ДатаПодписи, Форма, "Приложение");
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение", "Д_", 8, Формат(Форма.ДатаПодписи,"ДФ=ddMMyyyy"), Ложь);
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение", "HKBOS_",  10, ИННДиректора, Ложь);	
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение", "HKBUH_",  10, ИННГлавБуха, Ложь);	
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для каждого Строка Из ТаблицаБлока Цикл
    	...
    	Строка.D1T1RXXXXG9 = Строка.D1T1RXXXXG5 * Строка.D1T1RXXXXG7 * Строка.D1T1RXXXXG8 /12;    // ИНАГРО ++ Добавлено умножение на графу 7
		ИмяЯчейкиПрописью = "D1T1RXXXXG9_"+Формат(ТаблицаБлока.Индекс(Строка) + 1, "ЧГ=;");
		ЗаполнитьПоказательНаСервере(ИмяЯчейкиПрописью, Строка.D1T1RXXXXG9, Форма, "Приложение");
		...
		ЗаполнитьПоказательНаСервере(ИмяЯчейкиПрописью, Строка.D1T1RXXXXG10, Форма, "Приложение");
	...
	ЗаполнитьПоказательНаСервере("R002G4", R002G4, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R002G3", R002G3, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R002G7", R002G7, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R002G9", R002G9, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R002G10", R002G10, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R002G12", R002G12, Форма, "Приложение"); 
	
	ЗаполнитьПоказательНаСервере("R03G3", R002G12, Форма, "Приложение");
    //(ЗМПЗ) (ЗМПЗ = сума рядків 1.n граф  9 та/або 10), грн   
	R002G11 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G9", Форма, "Приложение"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G10", Форма, "Приложение"));
	ЗаполнитьПоказательНаСервере("R002G11", R002G11, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R01G3", R002G11, Форма, "Приложение");    
	
	R02G3 	= ПолучитьПоказательНаСервере("R021G3", Форма, "Приложение")
			+ ПолучитьПоказательНаСервере("R022G3", Форма, "Приложение")
			+ ПолучитьПоказательНаСервере("R023G3", Форма, "Приложение")
			+ ПолучитьПоказательНаСервере("R024G3", Форма, "Приложение")
			+ ПолучитьПоказательНаСервере("R025G3", Форма, "Приложение")
			+ ПолучитьПоказательНаСервере("R026G3", Форма, "Приложение")
			+ ПолучитьПоказательНаСервере("R027G3", Форма, "Приложение")
			+ ПолучитьПоказательНаСервере("R028G3", Форма, "Приложение"); 
			
	ЗаполнитьПоказательНаСервере("R02G3", R02G3, Форма, "Приложение");    
			
	//(рядок 01 - рядок 02 - рядок 03 розділу ІІ) (+/-) (позитивне значення переноситься до рядка 11 Декларації)
	R04G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R01G3", Форма, "Приложение"))
			- ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R02G3", Форма, "Приложение"))
			- ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R03G3", Форма, "Приложение"));
	ЗаполнитьПоказательНаСервере("R04G3", R04G3, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R011G3", ?(R04G3>0,R04G3,0), Форма, "ДекларацияЕН");
	Если R04G3 > 0 Тогда
	...
	R012G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R010G3", Форма, "ДекларацияЕН"))
	...
	R012G4 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R010G4", Форма, "ДекларацияЕН"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R011G4", Форма, "ДекларацияЕН"));
	ЗаполнитьПоказательНаСервере("R012G4", R012G4, Форма, "ДекларацияЕН");

	R013G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R013G3", Форма, "ДекларацияЕН"));
	R014G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R014G3", Форма, "ДекларацияЕН"));
	Если R014G3 > R013G3 Тогда
		R015G3 = R014G3 - R013G3;	
		R016G3 = 0;
	Иначе	
		R015G3 = 0;	
		R016G3 = - (R014G3 - R013G3);	
	КонецЕсли;
...
Процедура ВводПоказателяВПолеТабличногоДокументЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	Если    ДополнительныеПараметры.ИмяЯчейки = "HKBOS" 
		ИЛИ ДополнительныеПараметры.ИмяЯчейки = "HKBUH" Тогда
		ЗаполнитьПоказательНаСервере(ДополнительныеПараметры.ИмяЯчейки, Значение, ЭтаФорма, "Приложение");	
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Допустимые объемы дохода плательщиков единого налога 4-й группы определяются 
                                |в зависимости от размера минимальной зарплаты на начало отчетного года.';uk= 'Допустимі обсяги доходу платників єдиного податку 4-ї групи визначаються в залежності від суми мінімальної 
                                |зарплати на початок звітного року.'");
		Сообщение.Сообщить();

		Если РезультатЗапроса.Следующий() Тогда
			ОграничениеСуммыДоходаЕдинщика = 1167*РезультатЗапроса.Размер;
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Розмер минимальной оплаты труда на ';uk= 'Розмір мінімальної оплати праці на '") + Формат(НачалоГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ДФ=dd.MM.yyyy") + ": " + Формат(РезультатЗапроса.Размер, "ЧЦ=15; ЧДЦ=2");
			Сообщение.Сообщить();
		Иначе
			ОграничениеСуммыДоходаЕдинщика = Неопределено;
			Сообщение = Новый СообщениеПользователю;
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "ЕРП" 
		ИЛИ РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "BASУТ" Тогда
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |	СтатусыПлательщиковЕдиногоНалога.ПлательщикНДС
			               |ИЗ
			               |	Продажи КАК Продажи
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////
	// значение строки 8 предыдущей декларации
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 3 Тогда
		...
		Иначе
			...
			ИмяСхемы = "J0103508";
			Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20230101' Тогда
				ИмяСхемы = "J0103507";
			КонецЕсли;
...
Процедура ЗаполнитьМЗЗ(ПараметрыЗаполненияЗвит1С)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		//СоответствиеПолейТаблица.Вставить("T1RXXXXКонтрагент",  "Владелец");
		...
		Пока Таблица.Количество() < мСтруктураИсхКолвоСтрокРазделов[СтруктураРеквизитовФормы.мИдГруппы2] Цикл
			Таблица.Добавить();
		КонецЦикла;
		ЗначениеВРеквизитФормы(Таблица, СтруктураРеквизитовФормы.мИдГруппы2);
	...
	Если ТипЗнч(ПараметрыЗаполненияЗвит1С) = Тип("Структура") Тогда
		...
		Попытка
			...
			Иначе
				...
				Ст9_3  = Данные.GetValue("MAIN", "A8_3");
				Ст9_4  = Данные.GetValue("MAIN", "A8_4");
				
			КонецЕсли;
			
			ЗаполнитьПоказательНаСервере("R009G3", Ст9_3, ЭтаФорма, "ДекларацияЕН");
			ЗаполнитьПоказательНаСервере("R009G4", Ст9_4, ЭтаФорма, "ДекларацияЕН");
