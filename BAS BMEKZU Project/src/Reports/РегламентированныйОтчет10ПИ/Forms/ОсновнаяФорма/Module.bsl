...
Процедура ПоказатьПериод(Форма)
	...
	Если Год(Форма.мДатаКонцаПериодаОтчета) >= 2012
	    ...
	    СтрПериодОтчета = ПредставлениеПериода( НачалоГода(Форма.мДатаНачалаПериодаОтчета), КонецДня(Форма.мДатаКонцаПериодаОтчета), "ФП = Истина; Л="+РегламентированнаяОтчетностьКлиентСервер.ПолучитьКодЯзыкаИнтерфейса());
	Иначе	
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Устанавливаем границы периода построения отчета как Год
	...
	Если  мПериодичность = ПеречислениеПериодичностьГод Тогда
