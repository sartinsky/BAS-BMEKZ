...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2020УФ";
	...
	// Версия формы
	//СтруктураРеквизитовФормы.мВерсияФормы = "01/10/2020";
	СтруктураРеквизитовФормы.мВерсияФормы = "12/02/2021";
...
Процедура ЗаполнитьНачальныеНастройки()
	...
	Для Инд = 0 По МакетСоставаПоказателей.Области.Количество() - 1 Цикл
		...
		Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
			...
			// код показателя по форме отчета (имя ячейки в полях табличного документа формы)
...
// содержащую настройки отображения страниц, вывода на печать и выгрузки в ИМНС.
//
&НаСервере
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	//    - число, метка отображения страницы на основной панели формы;
	...
	СтрокаУровня1.Представление            = НСтр("ru='Отчет о трудоустройстве лиц с инвалидностью';uk= 'Звіт з працевлаштування осіб з інвалідністю'");
...
// Формирует дерево значений - структуру страниц отчета,
// содержащую настройки отображения страниц, вывода на печать и выгрузки.
//
&НаСервере
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	//ОПФГ    	   = Сведения.ОПФГ;
	//ВЭД    		   = Сведения.ВЭД;
	//ФС    		   = Сведения.ФормаСобственности;
	...
	ФИОБухгалтера     = СведенияДатаПодписи.ИмяФАМИЛИЯБух;
	...
	//ЗаполнитьПоказательНаСервере("ОПФ",	  ОПФГ, Форма, "Уведомление");
	//ЗаполнитьПоказательНаСервере("ВЭД",   ВЭД, Форма, "Уведомление");
	//ЗаполнитьПоказательНаСервере("ФС",    ФС, Форма, "Уведомление");
	...
	ЗаполнитьПоказательНаСервере("Email", Email, Форма, "Уведомление");	
	ЗаполнитьПоказательНаСервере("Телефон1", Телефон, Форма, "Уведомление");	
	ЗаполнитьПоказательНаСервере("Факс1", Факс, Форма, "Уведомление");	
	ЗаполнитьПоказательНаСервере("Email1", Email, Форма, "Уведомление");	
...
Процедура ОбновитьПользовательскиеНастройки(СписокСохранения, ВыполнялосьОбновлениеДанных)
	...
	ВерсияФормы = СписокСохранения.ВерсияФормы;
	Если ВерсияФормы = "01/10/2020" Тогда
	
		СписокСохранения.ВерсияФормы = "12/02/2021";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если  R_03 - R_02 > 0 Тогда  
		Если R_01 > 8 И R_01 <= 15 Тогда
			R_06  = 1 * R_05 / 2 ;
		ИначеЕсли R_01 > 15 И R_01 <= 25 Тогда
			R_06  = 1 * R_05 ;
		ИначеЕсли R_01 >  25 Тогда 
			R_06 = (R_03 - R_02) * R_05;
		КонецЕсли;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	Если РегламентированнаяОтчетностьПереопределяемый.ИДПодсистемыЗарплаты() = "ЕРПЗИК" Тогда
		ТаблЧисленностиССЧ = КадровыйУчет.СреднесписочнаяЧисленностьРаботающих10ПИ(СтруктураРеквизитовФормы.Организация, СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета,СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета); 
		
		R_01 = ТаблЧисленностиССЧ.ЧисленностьРаботников; 
		СтруктураДанныхУведомление.Вставить("R_01",  R_01);
		
		//R_21 = ТаблЧисленностиССЧ.ЧисленностьМужчин; 
		//СтруктураДанныхУведомление.Вставить("R_21",  R_21);
		//
		//R_31 = ТаблЧисленностиССЧ.ЧисленностьЖенщин; 
		//СтруктураДанныхУведомление.Вставить("R_31",  R_31);
		//
		//R_41 = ТаблЧисленностиССЧ.ЧисленностьМужчинПроживающихВГородах; 
		//СтруктураДанныхУведомление.Вставить("R_41",  R_41);
		//
		//R_51 = ТаблЧисленностиССЧ.ЧисленностьЖенщинПроживающихВГородах; 
		//СтруктураДанныхУведомление.Вставить("R_51",  R_51);
		//
		//R_61 = ТаблЧисленностиССЧ.ЧисленностьМужчинПроживающихВСелах; 
		//СтруктураДанныхУведомление.Вставить("R_61",  R_61);
		//
		//R_71 = ТаблЧисленностиССЧ.ЧисленностьЖенщинПроживающихВСелах; 
		//СтруктураДанныхУведомление.Вставить("R_71",  R_71);
		//
		//R_81 = ТаблЧисленностиССЧ.ЧисленностьМужчинОт18До35; 
		//СтруктураДанныхУведомление.Вставить("R_81",  R_81);

		//R_91 = ТаблЧисленностиССЧ.ЧисленностьЖенщинОт18До35; 
		//СтруктураДанныхУведомление.Вставить("R_91",  R_91);

		//R_101 = ТаблЧисленностиССЧ.ЧисленностьМужчинОт36До60; 
		//СтруктураДанныхУведомление.Вставить("R_101",  R_101);

		//R_111 = ТаблЧисленностиССЧ.ЧисленностьЖенщинОт36До60; 
		//СтруктураДанныхУведомление.Вставить("R_111",  R_111);
		//
		//R_121 = ТаблЧисленностиССЧ.ЧисленностьМужчинОт60; 
		//СтруктураДанныхУведомление.Вставить("R_121",  R_121);

		//R_131 = ТаблЧисленностиССЧ.ЧисленностьЖенщинОт60; 
		//СтруктураДанныхУведомление.Вставить("R_131",  R_131);
		
		R_02 = ТаблЧисленностиССЧ.ЧисленностьИнвалидов; 
		СтруктураДанныхУведомление.Вставить("R_02",  R_02);
		
		R_22 = ТаблЧисленностиССЧ.ЧисленностьМужчинИнвалидов; 
		СтруктураДанныхУведомление.Вставить("R_22",  R_22);
		
		R_32 = ТаблЧисленностиССЧ.ЧисленностьЖенщинИнвалидов; 
		СтруктураДанныхУведомление.Вставить("R_32",  R_32);
		
		R_42 = ТаблЧисленностиССЧ.ЧисленностьМужчинИнвалидовПроживающихВГородах; 
		СтруктураДанныхУведомление.Вставить("R_42",  R_42);
		
		R_52 = ТаблЧисленностиССЧ.ЧисленностьЖенщинИнвалидовПроживающихВГородах; 
		СтруктураДанныхУведомление.Вставить("R_52",  R_52);
		
		R_62 = ТаблЧисленностиССЧ.ЧисленностьМужчинИнвалидовПроживающихВСелах; 
		СтруктураДанныхУведомление.Вставить("R_62",  R_62);
		
		R_72 = ТаблЧисленностиССЧ.ЧисленностьЖенщинИнвалидовПроживающихВСелах; 
		СтруктураДанныхУведомление.Вставить("R_72",  R_72);
		
		R_82 = ТаблЧисленностиССЧ.ЧисленностьМужчинИнвалидовОт18До35; 
		СтруктураДанныхУведомление.Вставить("R_82",  R_82);

		R_92 = ТаблЧисленностиССЧ.ЧисленностьЖенщинИнвалидовОт18До35; 
		СтруктураДанныхУведомление.Вставить("R_92",  R_92);

		R_102 = ТаблЧисленностиССЧ.ЧисленностьМужчинИнвалидовОт36До60; 
		СтруктураДанныхУведомление.Вставить("R_102",  R_102);

		R_112 = ТаблЧисленностиССЧ.ЧисленностьЖенщинИнвалидовОт36До60; 
		СтруктураДанныхУведомление.Вставить("R_112",  R_112);
		
		R_122 = ТаблЧисленностиССЧ.ЧисленностьМужчинИнвалидовОт60; 
		СтруктураДанныхУведомление.Вставить("R_122",  R_122);

		R_132 = ТаблЧисленностиССЧ.ЧисленностьЖенщинИнвалидовОт60; 
		СтруктураДанныхУведомление.Вставить("R_132",  R_132);
		...
		Запрос.УстановитьПараметр("СчетУчета", ПС.РасчетыПоЗаработнойПлате);
		...
		|
		|ГДЕ
		|	ЕСТЬNULL(СотрудникиОрганизации.ВидЗанятости, ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ПустаяСсылка)
		...
		|				И НачислениеУдержание.ГруппаУчета.СчетУчета = &СчетУчета) КАК Начисления
	...
	Иначе	
		...
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ШТАТ"); 
		// ИНАГРО --
		...
		// ИНАГРО ++
		//ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ИНВШТАТ"); 
		// ИНАГРО --
		...
		// ИНАГРО ++
		//ТаблЧисленностиССЧ = КадровыйУчет.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.Организация, ПараметрыСредняяЧисленность); 
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖШТАТ"); 
		// ИНАГРО --
		ЧисленностьЖенщин = ТаблЧисленностиССЧ[0].Средняя; 
		ЧисленностьМужчин = R_01 - ЧисленностьЖенщин;
		R_21 = ЧисленностьМужчин; 
		СтруктураДанныхУведомление.Вставить("R_21",  R_21);
		R_31 = ЧисленностьЖенщин; 
		СтруктураДанныхУведомление.Вставить("R_31",  R_31);
		
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "СЕЛШТАТ"); 
		ЧисленностьРаботниковПроживающихВСелах = ТаблЧисленностиССЧ[0].Средняя;
	
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖСЕЛШТАТ"); 
		ЧисленностьЖенщинПроживающихВСелах = ТаблЧисленностиССЧ[0].Средняя;
		ЧисленностьМужчинПроживающихВСелах = ЧисленностьРаботниковПроживающихВСелах - ЧисленностьЖенщинПроживающихВСелах;
		
		R_71 = ЧисленностьЖенщинПроживающихВСелах;
		СтруктураДанныхУведомление.Вставить("R_71",  R_71);
		
		R_61 = ЧисленностьМужчинПроживающихВСелах; 
		СтруктураДанныхУведомление.Вставить("R_61",  R_61);
		
		ЧисленностьРаботниковПроживающихВГородах = R_01 - ЧисленностьРаботниковПроживающихВСелах;
		
		ЧисленностьЖенщинПроживающихВГородах = ЧисленностьЖенщин - ЧисленностьЖенщинПроживающихВСелах;
		ЧисленностьМужчинПроживающихВГородах = ЧисленностьРаботниковПроживающихВГородах - ЧисленностьЖенщинПроживающихВГородах;
		
		R_41 = ЧисленностьМужчинПроживающихВГородах;
		СтруктураДанныхУведомление.Вставить("R_41",  R_41);
		
		R_51 = ЧисленностьЖенщинПроживающихВГородах;
		СтруктураДанныхУведомление.Вставить("R_51",  R_51);
		
		// Работники от 18 до 35 лет
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "От18До35");
		ЧисленностьРаботниковОт18До35 = ТаблЧисленностиССЧ[0].Средняя;
	
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖОт18До35");
		ЧисленностьЖенщинОт18До35 = ТаблЧисленностиССЧ[0].Средняя;
		ЧисленностьМужчинОт18До35 = ЧисленностьРаботниковОт18До35 - ЧисленностьЖенщинОт18До35;

		R_81 = ЧисленностьМужчинОт18До35;
		СтруктураДанныхУведомление.Вставить("R_81",  R_81);
		
		R_91 = ЧисленностьЖенщинОт18До35;
		СтруктураДанныхУведомление.Вставить("R_91",  R_91);
		
		// Работники от 36 до 60 лет
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "От36До60"); 
		ЧисленностьРаботниковОт36До60 = ТаблЧисленностиССЧ[0].Средняя;
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖОт36До60"); 
		ЧисленностьЖенщинОт36До60 = ТаблЧисленностиССЧ[0].Средняя;
		ЧисленностьМужчинОт36До60 = ЧисленностьРаботниковОт36До60 - ЧисленностьЖенщинОт36До60;
		
		R_101 = ЧисленностьМужчинОт36До60;
		СтруктураДанныхУведомление.Вставить("R_101",  R_101);
		
		R_111 = ЧисленностьЖенщинОт36До60;
		СтруктураДанныхУведомление.Вставить("R_111",  R_111);
		
		
		// Работники от 60 лет
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "От60"); 
		ЧисленностьРаботниковОт60 = ТаблЧисленностиССЧ[0].Средняя;
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖОт60"); 
		ЧисленностьЖенщинОт60 = ТаблЧисленностиССЧ[0].Средняя;
		ЧисленностьМужчинОт60 = ЧисленностьРаботниковОт60 - ЧисленностьЖенщинОт60;
		
		R_121 = ЧисленностьМужчинОт60;
		СтруктураДанныхУведомление.Вставить("R_121",  R_121);
		
		R_131 = ЧисленностьЖенщинОт60;
		СтруктураДанныхУведомление.Вставить("R_131",  R_131);
		
		// Лица с инвалидностью
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖИНВШТАТ"); 
		ЧисленностьЖенщинИнвалидов = ТаблЧисленностиССЧ[0].Средняя; 
		ЧисленностьМужчинИнвалидов = R_02 - ЧисленностьЖенщинИнвалидов;
		
		R_22 = ЧисленностьМужчинИнвалидов;
		СтруктураДанныхУведомление.Вставить("R_22",  R_22);
		
		R_32 = ЧисленностьЖенщинИнвалидов;
		СтруктураДанныхУведомление.Вставить("R_32",  R_32);
		
		// Численность лиц с инвалидностью, проживающих в сельских населенных пунктах и ПГТ
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ИНВСЕЛШТАТ"); 
		ЧисленностьИнвалидовПроживающихВСелах = ТаблЧисленностиССЧ[0].Средняя;
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖИНВСЕЛШТАТ"); 
		ЧисленностьЖенщинИнвалидовПроживающихВСелах = ТаблЧисленностиССЧ[0].Средняя;
		ЧисленностьМужчинИнвалидовПроживающихВСелах = ЧисленностьИнвалидовПроживающихВСелах - ЧисленностьЖенщинИнвалидовПроживающихВСелах; 
		
		R_62 = ЧисленностьМужчинИнвалидовПроживающихВСелах;
		СтруктураДанныхУведомление.Вставить("R_62",  R_62);

		R_72 = ЧисленностьЖенщинИнвалидовПроживающихВСелах;
		СтруктураДанныхУведомление.Вставить("R_72",  R_72);
				
		ЧисленностьИнвалидовПроживающихВГородах = R_02 - ЧисленностьИнвалидовПроживающихВСелах;
		
		ЧисленностьЖенщинИнвалидовПроживающихВГородах = ЧисленностьЖенщинИнвалидов - ЧисленностьЖенщинИнвалидовПроживающихВСелах;
		ЧисленностьМужчинИнвалидовПроживающихВГородах = ЧисленностьМужчинИнвалидов - ЧисленностьМужчинИнвалидовПроживающихВСелах;
		
		R_42 = ЧисленностьМужчинИнвалидовПроживающихВГородах;
		СтруктураДанныхУведомление.Вставить("R_42",  R_42);
		
		R_52 = ЧисленностьЖенщинИнвалидовПроживающихВГородах;
		СтруктураДанныхУведомление.Вставить("R_52",  R_52);
	
		// Лица с инвалидностью от 18 до 35 лет
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ИНВШТАТОТ18ДО35"); 
		ЧисленностьИнвалидовОт18До35 = ТаблЧисленностиССЧ[0].Средняя;
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖИНВШТАТОТ18ДО35"); 
		ЧисленностьЖенщинИнвалидовОт18До35 = ТаблЧисленностиССЧ[0].Средняя;
		ЧисленностьМужчинИнвалидовОт18До35 = ЧисленностьИнвалидовОт18До35 - ЧисленностьЖенщинИнвалидовОт18До35;
		
		R_82 = ЧисленностьМужчинИнвалидовОт18До35;
		СтруктураДанныхУведомление.Вставить("R_82",  R_82);
		
		R_92 = ЧисленностьЖенщинИнвалидовОт18До35;
		СтруктураДанныхУведомление.Вставить("R_92",  R_92);

		// Лица с инвалидностью от 36 до 60 лет
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ИНВШТАТОТ36ДО60"); 
		ЧисленностьИнвалидовОт36До60 = ТаблЧисленностиССЧ[0].Средняя;
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖИНВШТАТОТ36ДО60"); 
		ЧисленностьЖенщинИнвалидовОт36До60 = ТаблЧисленностиССЧ[0].Средняя;
		ЧисленностьМужчинИнвалидовОт36До60 = ЧисленностьИнвалидовОт36До60 - ЧисленностьЖенщинИнвалидовОт36До60;
		
		R_102 = ЧисленностьМужчинИнвалидовОт36До60;
		СтруктураДанныхУведомление.Вставить("R_102",  R_102);
		
		R_112 = ЧисленностьЖенщинИнвалидовОт36До60;
		СтруктураДанныхУведомление.Вставить("R_112",  R_112);
		
		// Лица с нивалидностью от 60 лет
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ИНВШТАТОТ60"); 
		ЧисленностьИнвалидовОт60 = ТаблЧисленностиССЧ[0].Средняя;
		
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ЖИНВШТАТОТ60"); 
		ЧисленностьЖенщинИнвалидовОт60 = ТаблЧисленностиССЧ[0].Средняя;
		ЧисленностьМужчинИнвалидовОт60 = ЧисленностьИнвалидовОт60 - ЧисленностьЖенщинИнвалидовОт60;
		
		R_122 = ЧисленностьМужчинИнвалидовОт60;
		СтруктураДанныхУведомление.Вставить("R_122",  R_122);
		
		R_132 = ЧисленностьЖенщинИнвалидовОт60;
		СтруктураДанныхУведомление.Вставить("R_132",  R_132);
		...
		|ВЫБРАТЬ
		|  СУММА(ВЫБОР КОГДА Сотрудник.ИНАГРО_ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы) ТОГДА СуммаВзаиморасчетовПриход ИНАЧЕ 0 КОНЕЦ) КАК РезультатШтат
		|ИЗ
