	...
	Для каждого ОписаниеТипаОбъектов Из НастройкиПрав.Строки Цикл
		...
		ЗаголовокДляПодпапок =
			НСтр("ru='Для
                       ...
                       |;uk='Для
                       |підпапок'");
		ПодсказкаДляПодпапок = НСтр("ru='Права не только для текущей папки, но и для ее нижестоящих папок';uk='Права не тільки для поточної папки, але і для її підпорядкованих папок'");
...
Функция ПредставлениеРежимаЗапуска(РежимЗапуска)
	...
	ИначеЕсли РежимЗапуска = "ОбычноеПриложение" Тогда
		ПредставлениеРежимаЗапуска = НСтр("ru='Обычное приложение';uk='Звичайна прикладна програма'");
