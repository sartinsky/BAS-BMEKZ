...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ТаблЧисленностиССЧ  = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), КонецКвартала(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ШТАТ"); 
  	//ИНАГРО -- 
