...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища, ТабДокумент = Неопределено) Экспорт
	...
	|	ЕСТЬNULL(КнигаДоходовРасходовИтог.Статья10, 0) КАК Статья10,
	...
	|			КОНЕЦ) КАК Статья6,
	|		СУММА(ВЫБОР
	|				КОГДА ОборотСНачалаГода.Статья = &СтатьяРеализацияСХПродукции
	|					ТОГДА ОборотСНачалаГода.СуммаОборот
	|				ИНАЧЕ 0
	|			КОНЕЦ) КАК Статья7,
	|		СУММА(ВЫБОР
	...
	|			КОНЕЦ),
	|		СУММА(ВЫБОР
	|				КОГДА ОборотСНачалаКвартала.Статья = &СтатьяРеализацияСХПродукции
	|					ТОГДА ОборотСНачалаКвартала.СуммаОборот
	|				ИНАЧЕ 0
	|			КОНЕЦ),
	|		СУММА(ВЫБОР
	...
	Если ВыборкаМесяц.Следующий() Тогда
		...
		Если ВыборкаДень.Следующий() Тогда
			...
			Если ВыборкаПериоды.Следующий() Тогда
				...
				ПараметрыГод.Статья10 = ВыборкаПериоды.Статья10;
			...
			Если ВыборкаПериоды.Следующий() Тогда
				...
				ПараметрыКвартал.Статья10 = ВыборкаПериоды.Статья10;
	...
	Пока ВыборкаМесяц.Следующий() Цикл
		...
		Если СменаКвартала Тогда
			...
			ПараметрыКвартал.Статья10 = ВыборкаМесяц.Статья10;
		Иначе
		...
		Если СменаГода Тогда
			...
			ПараметрыГод.Статья10 = ВыборкаМесяц.Статья10;
