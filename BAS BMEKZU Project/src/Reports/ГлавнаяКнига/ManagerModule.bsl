...
Функция СформироватьЗаголовок(ПараметрыОтчета)
	...
	Если Не ПустаяСтрока(ТекстПроИтоги) Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = НСтр("ru='Детализация по ';uk='Деталізація по '") + ТекстПроИтоги;
	КонецЕсли;
	
	Если ПараметрыОтчета.ВыводитьЕдиницуИзмерения Тогда
		ЗаголовокОтчета.Параметры.ТекстПроИтоги = СокрЛП("" + ЗаголовокОтчета.Параметры.ТекстПроИтоги + Символы.ПС + НСтр("ru='Единица измерения: Гривна (код 980)';uk='Одиниця виміру: Гривня (код 980)'"));
	КонецЕсли;
...
Функция ПолучитьТекстОбщегоЗапроса(ПараметрыОтчета)
	...
	ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграниченийПоРеквизитам, "Организация", ПараметрыОтчета.Организация);
...
Процедура ВывестиСчет(ПараметрыОтчета, ОбщийИтог, СтруктураПараметров, ПоказыватьЗаголовок)
	...
	Если ПараметрыОтчета.ПоСубсчетам Тогда
		...
		КонецЕсли;
	Иначе
		Если ПропуститьСчет(ОбщийИтог.СчетРодитель, ПараметрыОтчета.ИспользоватьКлассыСчетовВКачествеГрупп) Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ВывестиШапкуТаблицы(ПараметрыОтчета, СтруктураПараметров, Выборка, ШиринаВыводимогоРаздела)
	...
	НачалоЗаголовка.Параметры.ОписаниеСчета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Главная книга. Счет %1 ""%2""';uk='Головна книга. Рахунок %1 ""%2""'"), Выборка.СчетПредставление, Выборка.СчетНаименование);
	
	Если ЗначениеЗаполнено(ПараметрыОтчета.Периодичность) Тогда
		НачалоЗаголовка.Параметры.Заголовок = НСтр("ru='Период';uk='Період'");
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Не ПараметрыОтчета.ПоСубсчетамКорСчетов Тогда
			...
			Если ВерхнийУровень < Выборка.Уровень() Тогда
				Если ПараметрыОтчета.ИспользоватьКлассыСчетовВКачествеГрупп Тогда
...
Функция ПолучитьСтрокуФорматаПериода(Периодичность)
	...
	Если ЗначениеЗаполнено(Периодичность) Тогда
		Если Периодичность = 12 Тогда
			ФорматПериода = НСтр("ru='ДФ = ""гггг """"г.""""""';uk= 'ДФ = ""гггг """"р.""""""'");
		ИначеЕсли Периодичность = 11 Тогда
			ФорматПериода = НСтр("ru='ДФ = """"""Полугодие с"""" дд.ММ.гггг """"""';uk= 'ДФ = """"""Півріччя з"""" дд.ММ.гггг """"""'");
		ИначеЕсли Периодичность = 10 Тогда
			ФорматПериода = НСтр("ru='ДФ = ""к"""" квартал"""" гггг """"г.""""""';uk= 'ДФ = ""к"""" квартал"""" гггг """"р.""""""'");
		ИначеЕсли Периодичность = 9 Тогда
			ФорматПериода = НСтр("ru='ДФ = ""ММММ гггг """"г.""""""';uk= 'ДФ = ""ММММ гггг """"р.""""""'");
		КонецЕсли;
	КонецЕсли;
...
Функция ПропуститьСчет(Сч, ИспользоватьКлассыСчетовВКачествеГрупп)
	...
	Если Сч = NULL Тогда
		Результат = Истина;
	Иначе
		Если (Не Сч.Пустая()) И ((Сч.Уровень() > 0) ИЛИ (Не ИспользоватьКлассыСчетовВКачествеГрупп)) Тогда
			Результат = Истина;
		КонецЕсли;
	КонецЕсли;
