...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Иначе
		...
		Если    ОкруглитьЧислоПоФормату(Форма, R70GA6) <> ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R010G4", Форма, "Приложение1")) 
			...
			+ НСтр("ru='значения строки 7 не совпадает с итогом по Таблице 1 Приложения Д1 к декларации';uk='значення рядка 7 декларації не співпадає з підсумком по Таблиці 1 Додатку Д1 до декларації'");
		КонецЕсли;		
	...
	Иначе
		...
		Если    ОкруглитьЧислоПоФормату(Форма, R140GA6) <> ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R020G4", Форма, "Приложение1")) 
			...
			ТекстСообщенияОбОшибкахРасчета = ТекстСообщенияОбОшибкахРасчета + Символы.ПС + Символы.Таб 
			+ НСтр("ru='значения строки 14 не совпадает с итогом по Таблице 2 Приложения Д1 к декларации';uk='значення рядка 14 декларації не співпадає з підсумком по Таблиці 2 Додатку Д1 до декларації'");
		КонецЕсли;		
	КонецЕсли;		
	...
	Иначе
		...
		R101GA6 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R101GA", Форма, "Декларация")) - ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R101GA4", Форма, "Декларация"));
		ЗаполнитьПоказательНаСервере("R101GA6", R101GA6, Форма, "Декларация");
		R102GA6 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R102GA", Форма, "Декларация")) - ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R102GA4", Форма, "Декларация"));
		ЗаполнитьПоказательНаСервере("R102GA6", R102GA6, Форма, "Декларация");
		R101GB6 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R101GB", Форма, "Декларация")) - ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R101GB4", Форма, "Декларация"));
		ЗаполнитьПоказательНаСервере("R101GB6", R101GB6, Форма, "Декларация");
		R102GB6 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R102GB", Форма, "Декларация")) - ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R102GB4", Форма, "Декларация"));
	...
	////////////////////////////////////////////////
	//Расчет для Приложения 6 (Справки к декларации)
	...
	Иначе
		...
		Если  ОкруглитьЧислоПоФормату(Форма, R150GB6) <> ОкруглитьЧислоПоФормату(Форма, R150GB) Тогда	
			ТекстСообщенияОбОшибкахРасчета = ТекстСообщенияОбОшибкахРасчета + Символы.ПС + Символы.Таб 
			+ НСтр("ru='значения строки 15 не совпадает с итогом по Таблице 3 Приложения Д7 к декларации';uk='значення рядка 15 декларації не співпадає з підсумком по Таблиці 3 Додатку Д7 до декларації'");
		КонецЕсли;		
	КонецЕсли;		
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Запрос.УстановитьПараметр ("КонДата",СтруктураРеквизитовФормы.ДатаКон);
	...
	ЗапросСправка.УстановитьПараметр ("КонДата",СтруктураРеквизитовФормы.ДатаКон);
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации()  =  "ЕРП" Тогда
		ТекстЗапросСправка = 
			...
			|   И    &УточняемыйПериод = КОНЕЦПЕРИОДА(ЕстьNULL(НалоговаяНакладнаяТоварыПольз.Ссылка.ДатаОтгрузкиОплаты, ДАТАВРЕМЯ(1,1,1)), "+ТекПериодичность+")
			...
			|   И    &УточняемыйПериод = КОНЕЦПЕРИОДА(ЕстьNULL(НалоговаяНакладнаяТовары.Ссылка.ДатаОтгрузкиОплаты, ДАТАВРЕМЯ(1,1,1)), "+ТекПериодичность+")
	...
	Иначе
		ТекстЗапросСправка = 
			...
			|	И НалоговаяНакладнаяТовары.Ссылка.Организация = &Организация
			|	И НалоговаяНакладнаяТовары.Ссылка.ВключаетсяВУточняющийРасчет = ИСТИНА
			|   И    &УточняемыйПериод = КОНЕЦПЕРИОДА(ЕстьNULL(НалоговаяНакладнаяТовары.Ссылка.УточняемыйПериод, ДАТАВРЕМЯ(1,1,1)), "+ТекПериодичность+")
			|	И НалоговаяНакладнаяТовары.Ссылка.Проведен = ИСТИНА
			|
			|СГРУППИРОВАТЬ ПО
			|	НалоговаяНакладнаяТовары.Номенклатура.Ссылка,
			...
			|   И    &УточняемыйПериод = КОНЕЦПЕРИОДА(ЕстьNULL(НалоговаяНакладнаяУслуги.Ссылка.УточняемыйПериод, ДАТАВРЕМЯ(1,1,1)), "+ТекПериодичность+")
			...
			|   И    &УточняемыйПериод = КОНЕЦПЕРИОДА(ЕстьNULL(НалоговаяНакладнаяНМА.Ссылка.УточняемыйПериод, ДАТАВРЕМЯ(1,1,1)), "+ТекПериодичность+")
			...
			|   И    &УточняемыйПериод = КОНЕЦПЕРИОДА(ЕстьNULL(НалоговаяНакладнаяОС.Ссылка.УточняемыйПериод, ДАТАВРЕМЯ(1,1,1)), "+ТекПериодичность+")
	...
	// сводные условные продажи (код 60000000000) вместе с коррективами
...
Процедура УстановитьФлаги()
	...
	ТекЗначениеФлага =  ЗначениеЗаполнено(СтруктураДанныхПриложение1.R014G4)
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R014G5)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R0141G4)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R0141G6)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R013G4)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R013G5)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхДекларация.R70GA)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхДекларация.R70GB)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R023G4)		
					ИЛИ ЗначениеЗаполнено(СтруктураДанныхПриложение1.R023G5)		
