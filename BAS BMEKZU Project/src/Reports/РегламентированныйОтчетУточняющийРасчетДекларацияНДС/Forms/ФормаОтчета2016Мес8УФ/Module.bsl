...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если НЕ ЭтоОбычнаяДекларация Тогда											
		...
		//ИНАГРО ++
		Если ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.УточняемыйОтчет) Тогда
			R180GB4 = ПолучитьПоказательНаСервере("R180GB", Форма, "Декларация"); 
			R181GB4 = ПолучитьПоказательНаСервере("R181GB", Форма, "Декларация");
			R182GB4 = ПолучитьПоказательНаСервере("R182GB", Форма, "Декларация");
			R190GB4 = ПолучитьПоказательНаСервере("R190GB", Форма, "Декларация");
			R191GB4 = 0;
			...
			ЗаполнитьПоказательНаСервере("R182GB4", R182GB4, Форма, "Декларация");
			...
			R180GB = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R180GB", Форма, "Декларация")) 
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0113GB", Форма, "Приложение10"))
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0213GB", Форма, "Приложение10"))
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0313GB", Форма, "Приложение10"));
			ЗаполнитьПоказательНаСервере("R180GB", R180GB, Форма, "Декларация");
			
			R181GB = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R181GB", Форма, "Декларация")) 
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R01131GB", Форма, "Приложение10"))
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R02131GB", Форма, "Приложение10"))
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R03131GB", Форма, "Приложение10"));
			ЗаполнитьПоказательНаСервере("R181GB", R181GB, Форма, "Декларация");
			
			R182GB = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R182GB", Форма, "Декларация")) 
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R01132GB", Форма, "Приложение10"))
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R02132GB", Форма, "Приложение10"))
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R03132GB", Форма, "Приложение10"));
			ЗаполнитьПоказательНаСервере("R182GB", R182GB, Форма, "Декларация");

			R190GB = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R190GB", Форма, "Декларация")) 
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0114GB", Форма, "Приложение10"))
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0214GB", Форма, "Приложение10"))
						+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0314GB", Форма, "Приложение10"));
	...
	//Если ЭтоОбычнаяДекларация Тогда //ИНАГРО
	...
	КонецЕсли; //ИНАГРО	
		...
		//ИНАГРО ++
		R200GB = ПолучитьПоказательНаСервере("R200GB", Форма, "Декларация");
		R201GB = ПолучитьПоказательНаСервере("R201GB", Форма, "Декларация");
		//ИНАГРО --
		...
		//ИНАГРО ++
		R191GB = ПолучитьПоказательНаСервере("R191GB", Форма, "Декларация");
		//ИНАГРО --
		...
		//ИНАГРО ++
		R200GB4 = ПолучитьПоказательНаСервере("R200GB4", Форма, "Декларация");
		R201GB4 = ПолучитьПоказательНаСервере("R201GB4", Форма, "Декларация");
		//ИНАГРО --
		...
		//ИНАГРО ++
		R191GB4 = ПолучитьПоказательНаСервере("R191GB4", Форма, "Декларация");
		//ИНАГРО --
		...
		Для каждого строка Из НужныеСтроки Цикл
			...
			ИначеЕсли ИмяГруппы = СтруктураРеквизитовФормы.мИдГруппы52 Тогда	
				...
				Если ТипЗнч(Строка.Регистратор) = Тип(ТипДокументаИмпорт)
					ИЛИ (НЕ Строка.Регистратор = Неопределено И Строка.Регистратор.Метаданные().Имя = "ИНАГРО_ГТДИмпортЖивотных") Тогда //ИНАГРО
...
Процедура ВнестиИнфоВПриложение9()
	...
	Запрос.Текст =  "ВЫБРАТЬ
	                ...
	                |
	                |ВЫБРАТЬ
	                |	13 - РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Приложение2КНалоговойНакладнойТовары.Ссылка.Дата, МЕСЯЦ), НАЧАЛОПЕРИОДА(&КонДата, МЕСЯЦ), МЕСЯЦ),
	                |	ВЫБОР
	                ...
	                |
	                |ВЫБРАТЬ
	                |	13 - РАЗНОСТЬДАТ(НАЧАЛОПЕРИОДА(Приложение2КНалоговойНакладнойУслуги.Ссылка.Дата, МЕСЯЦ), НАЧАЛОПЕРИОДА(&КонДата, МЕСЯЦ), МЕСЯЦ),
	                |	ВЫБОР
	...
	УсловнаяПродажа.Добавить(Перечисления.ВидыОперацийПриложение2КНалоговойНакладной.УсловнаяПродажаВозврат);
	Запрос.УстановитьПараметр ("УсловнаяПродажа", УсловнаяПродажа);
	//Если ОтчетЗаДваМесяца Тогда
	//	Запрос.УстановитьПараметр ("НачДата",ДобавитьМесяц(мДатаНачалаПериодаОтчета, -1));	
	//КонецЕсли;
