...
Процедура ЗаполнитьАвто(Команда)
	...
	Если ИспКлассыСчетовПрошлГода = Ложь или НЕ ЗначениеЗаполнено(ИспКлассыСчетовПрошлГода) Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не установлены ""Используемые классы счетов расходов"" за %1 год! Автоматическое заполнения отчета невозможно!';uk='Не встановлени ""Використовувані класи рахунків витрат"" за %1 рік! Автоматичне заповнення звіту неможливо!'"), Год(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, -12))); 
		ПоказатьПредупреждение(, ТекстСообщения);				
		//Сообщить(ТекстСообщения);
		Возврат
	КонецЕсли;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
   ...
   КонецПопытки;	
	...
	ИначеЕсли РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() = "БСП"  Тогда
		ТаблЧисленностиССЧ = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ССЧ", "КВАРТАЛ", СтруктураРеквизитовФормы.Организация); 
		СреднеспКво = ТаблЧисленностиССЧ[0].Средняя;
