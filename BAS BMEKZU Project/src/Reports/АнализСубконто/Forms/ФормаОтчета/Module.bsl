...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru='Анализ субконто ';uk='Аналіз субконто '") + ТекстСубконто + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка") Тогда
		Если ЗаполняемыеНастройки.Группировка Тогда
			...
			Если ЕстьНалоговыйУчет Тогда
				...
				НоваяСтрока.Использование  = ЕстьУчетПоНалоговымНазначениямНДС;
...
Процедура ОбновитьИсходныеЗначения(Форма)
	...
	Форма.ВидСубконтоИсходноеЗначение = ?(Отчет.СписокВидовСубконто.Количество() = 0,
		Неопределено,
		Отчет.СписокВидовСубконто[0].Значение);
	Если  ПустаяСтрока(Форма.ВидСубконтоИсходноеЗначение) ИЛИ Форма.ВидСубконтоИсходноеЗначение = Неопределено Тогда
		Отчет.Группировка.Очистить();
		Для Каждого ЭлементОтбора Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
			Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы.Удалить(ЭлементОтбора);
		КонецЦикла;
...
Процедура УправлениеФормой(Форма)
	...
	Элементы.ПоказательРазница.Доступность       = Форма.ЕстьНалоговыйУчет;	
