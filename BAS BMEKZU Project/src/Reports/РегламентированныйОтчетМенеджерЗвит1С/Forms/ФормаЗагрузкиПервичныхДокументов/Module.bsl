...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИдBASERP = РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации(Истина)= "BASERP";
...
Процедура ВнестиДанныеЗвит1С(ТаблицаДокЗвит1С, КодОрганизации, ЕДРПОУКонтрагента, КонтрагентКодФилиала, ТекТипДокумента)
	...
	// ИНАГРО ++
	// Добавим возможность загрузки табличных частей по шаблонам ФлайДок
	ИначеЕсли Лев(ИмяСхемыЗвит1С, 7) = "1С82РАХ" Тогда
		ТекТипДокумента = "Рахунок";
	ИначеЕсли Лев(ИмяСхемыЗвит1С, 7) = "1С82АВР" Тогда   
		ТекТипДокумента = "Акт";
	ИначеЕсли Лев(ИмяСхемыЗвит1С, 6) = "1С82ВН" Тогда
		ТекТипДокумента = "Накладна";
	// ИНАГРО --
	...
	Если РезультатОткрытия = 1 Тогда
		...
		Если    ВРЕГ(ЛЕВ(ИмяСхемыЗвит1С,5)) = "1CPAX"
			...
			ИЛИ ВРЕГ(ЛЕВ(ИмяСхемыЗвит1С,7)) = "1С82РАХ"    // ИНАГРО
			ИЛИ ВРЕГ(ЛЕВ(ИмяСхемыЗвит1С,7)) = "1С82АВР"    // ИНАГРО
			ИЛИ ВРЕГ(ЛЕВ(ИмяСхемыЗвит1С,6)) = "1С82ВН"     // ИНАГРО
			ИЛИ ВРЕГ(ЛЕВ(ИмяСхемыЗвит1С,7)) = "1С82АВП"    // ИНАГРО
...
Процедура Загрузить(Команда)
	...
	Для каждого Строка Из ДокументыЗвит1С Цикл
		...
		Если  ВРЕГ(ЛЕВ(Строка.charCode,5)) <> "1CPAX"
			...
			И ВРЕГ(ЛЕВ(Строка.charCode,4)) <> "1CBH"  
			// ИНАГРО ++
			И ВРЕГ(ЛЕВ(Строка.charCode,7)) <> "1С82РАХ"    
			И ВРЕГ(ЛЕВ(Строка.charCode,7)) <> "1С82АВР" 
			И ВРЕГ(ЛЕВ(Строка.charCode,7)) <> "1С82АВП"    
			И ВРЕГ(ЛЕВ(Строка.charCode,6)) <> "1С82ВН" Тогда
			// ИНАГРО --
			Продолжить;
		КонецЕсли;
		...
		Если НЕ ВРЕГ(ЛЕВ(Строка.charCode,6)) = "AKTBFR" 
			И НЕ ВРЕГ(ЛЕВ(Строка.charCode,4)) = "ZNFR" 
			// ИНАГРО ++
			И НЕ ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82РАХ"    
			И НЕ ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВР" 
			И НЕ ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВП"    
			И НЕ ВРЕГ(ЛЕВ(Строка.charCode,6)) = "1С82ВН" Тогда
			// ИНАГРО --
			ДанныеЭДОШапка.Добавить(Результат.GetValue("MAIN", "FIELD"), "ТипКода");
		КонецЕсли;
		
		Если ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82РАХ"    
			ИЛИ ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВР" 
			ИЛИ ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВП"    
			ИЛИ ВРЕГ(ЛЕВ(Строка.charCode,6)) = "1С82ВН" Тогда
			ДанныеЭДОШапка.Добавить("Артикул", "ТипКода");
		КонецЕсли;
		...
		Пока ЕстьСтрока Цикл
			...
			Иначе
				ДанныеСтроки.Добавить(Результат.GetValue( "TAB1", "TAB1_NOMENKLATURA_NAME"), "НоменклатураПоставщика");
			КонецЕсли;
			//ИНАГРО --
			...
			ИначеЕсли ВРЕГ(ЛЕВ(Строка.charCode,6)) = "1С82ВН" Тогда
				ДанныеСтроки.Добавить(Результат.GetValue( "TAB1", "TAB1_NOMENKLATURA_NUM"),   "КодАртикул");
				// ИНАГРО --
			КонецЕсли;
			...
			Если НЕ ВРЕГ(ЛЕВ(Строка.charCode,5)) = "AKT1C" 
				...
				// ИНАГРО ++
				И НЕ ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВР" 
				И НЕ ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВП" Тогда
				// ИНАГРО --
			...
			Если  НЕ ВРЕГ(ЛЕВ(Строка.charCode,4)) = "ZNFR" 
				И НЕ ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВП" Тогда // ИНАГРО
			...
			ИначеЕсли ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82РАХ"
				ИЛИ ВРЕГ(ЛЕВ(Строка.charCode,6)) = "1С82ВН" Тогда
				ДанныеСтроки.Добавить(Результат.GetValue( "TAB1", "TAB1_A4"), 		"СуммаСкидки");
			ИначеЕсли ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВП" Тогда
				ДанныеСтроки.Добавить(Результат.GetValue( "TAB1", "TAB1_A7"), 		"СуммаБезСкидки");
				ДанныеСтроки.Добавить(Результат.GetValue( "TAB1", "TAB1_A4"), 		"СуммаСкидки");
				// ИНАГРО --
			КонецЕсли;
	...
	Для каждого Строка Из ДокументыЗвит1С Цикл
		...
		Если Строка.ДанныеЭДОРезультатОткрытия = 1 Тогда
			...
			ИначеЕсли Строка.ТипДокумента = "Накладна" Тогда
				...
				// загрузку ТЧ мы обеспечиваем лишь для определенных типов шаблонов Звита
				Если ВРЕГ(ЛЕВ(Строка.charCode,4)) = "1CBH" 
					ИЛИ ВРЕГ(ЛЕВ(Строка.charCode,6)) = "1С82ВН" Тогда  // ИНАГРО
					// шаблон обмена FREDO ДокМен
					
					УчитыватьНДС     		 = (Строка.charCode <> "1CBHB" И Строка.charCode <> "1С82ВНБ");
					ПервДок.ЦенаВключаетНДС  = (Строка.charCode = "1CBH" ИЛИ Строка.charCode = "1С82ВН");
	...
	Для каждого Строка Из ДокументыЗвит1С Цикл
		...
		Если Строка.ДанныеЭДОРезультатОткрытия = 1 Тогда
			...
			Если Строка.ТипДокумента = "Рахунок" Тогда
				...
				Если ВРЕГ(ЛЕВ(Строка.charCode,5)) = "1CPAX" Тогда
					...
					Для каждого ДанныеСтрокиСписок Из ДанныеЭДОТаблица Цикл
						...
						Если    НЕ ЗначениеЗаполнено(ЕдиницаИзмерения1С) 
							ИЛИ НЕ ИдКонфигурации = "БП" ИЛИ ИдКонфигурации = "БСП" Тогда
				...
				// ИНАГРО ++ Загрузку ТЧ мы обеспечиваем лишь для определенных типов шаблонов ФлайДок.
				ИначеЕсли ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82РАХ" Тогда
					// шаблон обмена FREDO ДокМен
					
					УчитыватьНДС     		 = (Строка.charCode <> "1С82РАХБ");    
					ПервДок.СуммаВключаетНДС  = (Строка.charCode = "1С82РАХ");      
					...
					Для каждого ДанныеСтрокиСписок Из ДанныеЭДОТаблица Цикл
						...
						Сумма = 				ДанныеСтроки.Сумма;
						СуммаСкидки = 			ДанныеСтроки.СуммаСкидки;
						СуммаБезСкидки = 		Сумма - ?(СуммаСкидки = Неопределено, 0, СуммаСкидки);						
						...
						Если УчитыватьНДС Тогда
							...
							СтавкаНДСЧисло = Строка.СуммаНДС / (Строка.СуммаДокумента - Строка.СуммаНДС);
														
							Если Окр(СтавкаНДСЧисло,1) = 0.2 Тогда
								СтрокаПервДок.СтавкаНДС = Перечисления.СтавкиНДС.НДС20;	
								Если ПервДок.СуммаВключаетНДС Тогда
									СтрокаПервДок.СуммаНДС = СтрокаПервДок.Сумма / 1.2 * 0.2;
								Иначе
									СтрокаПервДок.СуммаНДС = СтрокаПервДок.Сумма * 0.2;
								КонецЕсли;
							ИначеЕсли Окр(СтавкаНДСЧисло,2) = 0.07 Тогда
								СтрокаПервДок.СтавкаНДС = Перечисления.СтавкиНДС.НДС7;	
								Если ПервДок.СуммаВключаетНДС Тогда
									СтрокаПервДок.СуммаНДС = СтрокаПервДок.Сумма / 1.07 * 0.07;
								Иначе
									СтрокаПервДок.СуммаНДС = СтрокаПервДок.Сумма * 0.07;
								КонецЕсли;
							ИначеЕсли Окр(СтавкаНДСЧисло,2) = 0.14 Тогда
								СтрокаПервДок.СтавкаНДС = Перечисления.СтавкиНДС.НДС14;	
								Если ПервДок.СуммаВключаетНДС Тогда
									СтрокаПервДок.СуммаНДС = СтрокаПервДок.Сумма / 1.14 * 0.14;
								Иначе
									СтрокаПервДок.СуммаНДС = СтрокаПервДок.Сумма * 0.14;
								КонецЕсли;
							ИначеЕсли Окр(СтавкаНДСЧисло,0) = 0 Тогда
								СтрокаПервДок.СтавкаНДС = Перечисления.СтавкиНДС.НДС0;		
							//ИначеЕсли СтавкаНДС  = "Б" Тогда
							//	СтрокаПервДок.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;		
							//ИначеЕсли СтавкаНДС  = "Н" Тогда
			...
			ИначеЕсли Строка.ТипДокумента = "Акт" Тогда
				...
				// ИНАГРО ++ Загрузку ТЧ мы обеспечиваем лишь для определенных типов шаблонов ФлайДок
				ИначеЕсли ВРЕГ(ЛЕВ(Строка.charCode,7)) = "1С82АВР" Тогда
					// шаблон обмена FREDO ДокМен
					
					УчитыватьНДС     		 = (Строка.charCode <> "1С82АВРБ");
					ПервДок.СуммаВключаетНДС = (Строка.charCode = "1С82АВР");
					...
					Для каждого ДанныеСтрокиСписок Из ДанныеЭДОТаблица Цикл
						...
						Цена = 					ДанныеСтроки.Цена;
						Сумма = 				ДанныеСтроки.Сумма;
						СуммаСкидки = 			0;      // В шаблонах Флайдок пока нет информации о сумме скидки
						СуммаБезСкидки = 		Сумма - ?(СуммаСкидки = Неопределено, 0, СуммаСкидки);						
			...
			ИначеЕсли Строка.ТипДокумента = "Накладна" Тогда
				...
				// ИНАГРО ++	шаблон обмена ФлайДок
				ИначеЕсли ВРЕГ(ЛЕВ(Строка.charCode,6)) = "1С82ВН" Тогда
					
					УчитыватьНДС     		 = (Строка.charCode <> "1С82ВНБ");
					ПервДок.СуммаВключаетНДС = (Строка.charCode = "1С82ВН");
					...
					Для каждого ДанныеСтрокиСписок Из ДанныеЭДОТаблица Цикл
						...
						ИмяТЧ = "Товары";
						// все заносим в ТЧ Товары
						Если НЕ ВРЕГ(Лев(ТипНоменклатуры,1)) = "П" Тогда
							ИмяТЧ = "Товары";
							СтрокаПервДок = ПервДок[ИмяТЧ].Добавить();
							СтрокаПервДок.НаименованиеПоставщика = НоменклатураПоставщика;
							Если ИдКонфигурации = "БП" ИЛИ ИдКонфигурации = "БСП" Тогда
