...
Процедура ЗаписатьВОтчетЗвит1С(Отчет, ИмяТаблицыЗвит1С, ИмяЗвит1С, Значение) Экспорт
	...
	Исключение
		...
		ИначеЕсли ФлагОтладкиПакетноеТестирование Тогда
			...
			Сообщить("FREDO Звіт. Помилка установки показника ( схема """+Отчет.CharCode+?(ПустаяСтрока(ИмяТаблицыЗвит1С),"",""", назва таблиці """+ИмяТаблицыЗвит1С)+""", назва во ""FREDO Звіт"" """+ИмяЗвит1С+""", тип значення """+ТипЗнч(Значение)+""", значення """+ Значение+""")", СтатусСообщения.Внимание);
	...
	ИначеЕсли Команда = "REPORT" Тогда
		...
		Если СоответствиеОтчета = Неопределено Тогда 
			...
			Если СоответствиеОтчета = Неопределено Тогда 
				...
				ТекстОшибки = "Заповнення звіта """+ИмяСхемы+"""
							  |даними з облікової системи не передбачене!";
				глКомпонентаЗвит1С.CancelReport(ОтчетЗвит1С, ТекстОшибки);
		...
		Если СоответствиеОтчета.ИмяОтчета = "РегламентированныйОтчетНалоговаяНакладная"
		 ИЛИ СоответствиеОтчета.ИмяОтчета = "РегламентированныйОтчетПриложение2КНалоговойНакладной" Тогда
			...
			ТекстОшибки = "Заповнення звіта """+ИмяСхемы+"""
						  |Виконується з форми відповідного документа облікової системи!";
			глКомпонентаЗвит1С.CancelReport(ОтчетЗвит1С, ТекстОшибки);
		...
		Если ОрганизацииЗвит1С.Количество() = 0 Тогда
			ТекстОшибки = "В обліковій системі не знайдено фірму з ЄДРПОУ """+ОтчетЗвит1С.FIRM_EDRPOU+""""+ " на "+Формат(ДатаНачалаПериодаОтчета, "Л=uk_UA; ДФ=dd.MM.yyyy")+"
		...
		ИначеЕсли ОрганизацииЗвит1С.Количество() > 1 Тогда
			ТекстОшибки = "В обліковій системі знайдено декілька фірм з однаковим ЄДРПОУ """+ОтчетЗвит1С.FIRM_EDRPOU+""""+" на "+Формат(ДатаНачалаПериодаОтчета, "Л=uk_UA; ДФ=dd.MM.yyyy")+Символы.ПС;
		...
		Если ФлагОтладки Тогда
			Сообщить("FREDO Звіт. Запит заповнення звіта із FREDO Звіт (схема """+ИмяСхемы+""", тип періода """+ТипПериода+""", періодичність """+Периодичность+""")");
		КонецЕсли;
...
Функция ВыгрузитьПервичныйДокумент(Документ, ФормаДокумента = Неопределено) Экспорт
	...
	Для каждого ДокументСсылка Из ДокументыМассив Цикл
		...
		Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
			ДопустимыеТипы.Добавить(Тип("ДокументСсылка.ИНАГРО_РеализацияБиологическихАктивов")); // ИНАГРО
		КонецЕсли;
		ДопустимыеТипы.Добавить(Тип("ДокументСсылка.ИНАГРО_ЗаявлениеРасчетВФСС")); // ИНАГРО
		ДопустимыеТипы.Добавить(Тип("ДокументСсылка.ИНАГРО_ПодтверждениеВыплатФСС")); // ИНАГРО
		...
		Иначе	
			...
			Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
				ДопустимыеТипы.Добавить(Тип("ДокументСсылка.ИНАГРО_СчетНаОплатуПокупателюБиологическихАктивов")); // ИНАГРО
			КонецЕсли;
...
Функция ОпределитьТипДокументаПоСсылкеНаСервере(Документ) Экспорт
	...
	Иначе	
		...
		Если ТипЗнч(Документ) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") 
			ИЛИ Документ.Метаданные().Имя = "ИНАГРО_СчетНаОплатуПокупателюБиологическихАктивов" Тогда // ИНАГРО
			
			Результат = "Рахунок";
			
		ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.СчетНаОплатуПоставщика")
			ИЛИ Документ.Метаданные().Имя = "ИНАГРО_СчетНаОплатуПоставщикаБиологическихАктивов" Тогда // ИНАГРО
		...
		ИначеЕсли Документ.Метаданные().Имя = "ИНАГРО_РеализацияБиологическихАктивов" Тогда	// ИНАГРО
			...
			Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
			                      |	""Товары"" КАК ВидНоменклатуры
			                      |ИЗ
			                      |	Документ.ИНАГРО_РеализацияБиологическихАктивов.Услуги КАК Док
			                      |ГДЕ
			                      |	Док.Ссылка = &Документ
			                      |
			                      |ОБЪЕДИНИТЬ
		...
		ИначеЕсли Документ.Метаданные().Имя = "ИНАГРО_ПоступлениеБиологическихАктивов" Тогда // ИНАГРО
			...
			|	""Товары"" КАК ВидНоменклатуры
			|ИЗ
			|	Документ.ИНАГРО_ПоступлениеБиологическихАктивов.Товары КАК Док
			|ГДЕ
			|	Док.Ссылка = &Документ
			...
			|	""Услуги""
			|ИЗ
			|	Документ.ИНАГРО_ПоступлениеБиологическихАктивов.Услуги КАК Док
			|ГДЕ
			|	Док.Ссылка = &Документ");
