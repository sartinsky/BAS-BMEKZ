...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	|ИЗ  РегистрСведений.ИНАГРО_РаботникиОрганизаций.СрезПоследних(&Дата, Организация = &Организация И Сотрудник.ИНАГРО_ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)) КАК РаботникиОрганизаций
	|ГДЕ
	...
	Если КоличествоСотрудников > 20 Тогда
		...
		ТаблСреднеспЧисленности = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачалоГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, -12)), КонецГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, -12)), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ШТАТ"); 
		СтруктураДанныхУведомление.П011 = ТаблСреднеспЧисленности[0].Средняя;
		
		ТаблСреднеспЧисленности = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачалоГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, -12)), КонецГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, -12)), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ДОПГАРАНТИИНЕИНВШТАТ");
		Если ТаблСреднеспЧисленности.Колонки.Найти("РасчетВыполнен") = Неопределено
			...
			ТаблСреднеспЧисленности = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачалоГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, -12)), КонецГода(ДобавитьМесяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, -12)), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ДОПГАРАНТИИШТАТ");
			СтруктураДанныхУведомление.П021 = ТаблСреднеспЧисленности[0].Средняя;
		Иначе	
			СтруктураДанныхУведомление.П021 = ТаблСреднеспЧисленности[0].Средняя;
		КонецЕсли;
	Иначе	
		ТаблСреднеспЧисленности = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ШТАТ");
		СтруктураДанныхУведомление.П041 = ТаблСреднеспЧисленности[0].Средняя;

		ТаблСреднеспЧисленности = ИНАГРО_ПроведениеРасчетов.ПолучитьСреднююЧисленность(НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ССЧ", "КВАРТАЛСНАЧАЛАГОДА", СтруктураРеквизитовФормы.Организация, "ДОПГАРАНТИИ10ЛЕТШТАТ");
		СтруктураДанныхУведомление.П051 = ТаблСреднеспЧисленности[0].Средняя;
	КонецЕсли;
