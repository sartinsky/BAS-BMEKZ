...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2022";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/07/2022";
...
Процедура ВывестиМакетСтраницыВТабличныйДокументНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
   ...
   ТабличныйДокумент.Вывести(ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет(Сред(ЭтаФорма.ИмяФормы, Найти(ЭтаФорма.ИмяФормы, "ФормаОтчета")) + "_" + ИмяТекРаздела));
	...
	Если ИмяТекРаздела = "ОтчетОДебиторскойИКредиторскойЗадолженности" 
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("FIRM_NAME", ОргНазв, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");

	
	Если Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		ПериодОтчета = "станом на 31 грудня " +Формат(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета, "ДФ=yyyy")+ "року";
	Иначе	
    ...
    ЗаполнитьПоказательНаСервере("FIRM_ADR",  ОргЮрАдрес, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
    ЗаполнитьПоказательНаСервере("FIRM_ADR_FIZ", ОргАдрес, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	ЗаполнитьПоказательНаСервере("TEL", Телефон, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	ЗаполнитьПоказательНаСервере("VIK_EMAIL", EMAIL, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	
	КолСимволов = Мин(СтрДлина(Строка(ЕДРПОУ)),8);
	
	ЗаполнитьПоказательНаСервере("FIRM_EDRPOU", ЕДРПОУ, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности", "ЕДРПОУ_",  8, ЕДРПОУ, Ложь);		
	
	ЗаполнитьПоказательНаСервере("KATOTTG", КодКАТОТТГ, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	ЗаполнитьПоказательНаСервере("KATOTTG_FACT", Сред(КодКАТОТТГ,3), Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности", "КАТОТТГ"+"_",  19, КодКАТОТТГ, Ложь);		
	ЗаполнитьПоказательНаСервере("RUK", ФИОДиректора, Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");									
	...
	КонецЕсли;
	ЗаполнитьПоказательНаСервере("TIN",  ЕДРПОУ, Форма, "СлужебныеДанные");
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Знач1 = ПолучитьПоказательНаСервере("A010_1", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности")
			+ ПолучитьПоказательНаСервере("A010_2", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности")
			+ ПолучитьПоказательНаСервере("A010_3", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности")
			+ ПолучитьПоказательНаСервере("A010_4", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности")
			+ ПолучитьПоказательНаСервере("A010_5", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности")
			+ ПолучитьПоказательНаСервере("A010_6", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
			
	Если Знач1 <> 0 Тогда
		ЗаполнитьПоказательНаСервере("H001", "", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
		ЗаполнитьПоказательНаСервере("HK1", "", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
		ЗаполнитьПоказательНаСервере("HK2", "", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
		ЗаполнитьПоказательНаСервере("HK3", "", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
		ЗаполнитьПоказательНаСервере("HK4", "", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
		ЗаполнитьПоказательНаСервере("HK5", "", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
		ЗаполнитьПоказательНаСервере("HK6", "", Форма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если ВРЕГ(Лев(ИмяЯчейки, 6))  = "ЕДРПОУ" Тогда
		...
		ИмяОбластиЗначения = "FIRM_EDRPOU";  
	...
	ИначеЕсли ВРЕГ(Лев(ИмяЯчейки, 7))  = "КАТОТТГ" Тогда
		...
		ИмяОбластиЗначения = "KATOTTG";  
		  
		Значение = ТабличныйДокумент.Области["KATOTTG"].Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
																				 "KATOTTG",  НаименованиеТекЛиста,
																				 "КАТОТТГ_", НаименованиеТекЛиста,
	...
	ИначеЕсли ВРЕГ(ИмяЯчейки) = "H001" Тогда
		...
		Если ЗначениеЗаполнено(ТабличныйДокумент.Области[ИмяЯчейки].Значение) Тогда
			ТабличныйДокумент.Области["A010_1"].Значение = 0;
			ТабличныйДокумент.Области["A010_2"].Значение = 0;
			ТабличныйДокумент.Области["A010_3"].Значение = 0;
			ТабличныйДокумент.Области["A010_4"].Значение = 0;
			ТабличныйДокумент.Области["A010_5"].Значение = 0;
			ТабличныйДокумент.Области["A010_6"].Значение = 0;
			ТабличныйДокумент.Области["N1"].Значение = "";
		КонецЕсли;
	КонецЕсли;		
...
Процедура ВводПоказателяВПолеТабличногоДокументЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	Если ВРЕГ(Лев(ИмяЯчейки, 7))  = "KATOTTG" Тогда
		ЗаполнитьПоказательНаСервере("KATOTTG_FACT", Сред(Значение,3), ЭтаФорма, "ОтчетОДебиторскойИКредиторскойЗадолженности");
	КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	НовСтрока.ИмяСхемы = "S0210115"; 
	НовСтрока.ПредставлениеСхемы =НСтр("ru='Форма № 1-Б';uk= 'Форма № 1-Б'");
	НовСтрока.НазваниеСтраницыПанели ="ОтчетОДебиторскойИКредиторскойЗадолженности";
...
Процедура СкорректироватьИмена(ИмяПанели, СтруктураВход)
	...
	Если ИмяПанели  = "ОтчетОДебиторскойИКредиторскойЗадолженности" Тогда
		
		Структура.Вставить("MY_DATE", 	СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		Структура.Вставить("VIK_TEL", 	ОставитьТолькоЦифры(СтруктураВход.TEL));
		Структура.Вставить("OBL", 	 Сред(СтруктураВход.KATOTTG,3,2));
		Структура.Вставить("RAY", 	 "0"+Сред(СтруктураВход.KATOTTG,5,2));
		
		Если ЗначениеЗаполнено(СтруктураВход.H001) Тогда
			Если ЗначениеЗаполнено(СтруктураВход.HK1) Тогда
				НомерПричины = 33;
				Причина = "Не здійснюється вид економічної діяльності, який спостерігається";
			ИначеЕсли ЗначениеЗаполнено(СтруктураВход.HK2) Тогда	
				НомерПричины = 34;
				Причина = "Одиниця припинена або перебуває в стадії припинення";
			ИначеЕсли ЗначениеЗаполнено(СтруктураВход.HK3) Тогда	
				НомерПричины = 37;
				Причина = "Здійснюється сезонна діяльність або економічна діяльність, пов'язана з тривалим циклом виробництва";
			ИначеЕсли ЗначениеЗаполнено(СтруктураВход.HK4) Тогда	
				НомерПричины = 39;
				Причина = "Тимчасово призупинено економічну діяльність через економічні чинники/карантинні обмеження";
			ИначеЕсли ЗначениеЗаполнено(СтруктураВход.HK5) Тогда	
				НомерПричины = 40;
				Причина = "Проведено чи проводиться реорганізація або передано виробничі фактори іншій одиниці";
			Иначе	
				НомерПричины = 42;
				Причина = "Відсутнє явище, яке спостерігається";
			КонецЕсли;
			Структура.Вставить("REASON_KOD", 	НомерПричины);
			Структура.Вставить("REASON", 		Причина);
			Структура.Вставить("ZERO_ZVIT", 	1);
		Иначе	
			Структура.Вставить("ZERO_ZVIT", 	Неопределено);
		КонецЕсли;
	...
	ИначеЕсли ИмяПанели  = "СлужебныеДанные" Тогда
		...
		Структура.Вставить("HTIN", 	 СтруктураВход.TIN);
...
Функция ПолучитьДанныеФормыНаСервере();
	...
	СтруктураРеквизитовФормы.ДопПоказателиОтчетаЗвит1С.Вставить("TEL", ОставитьТолькоЦифры(ПолучитьПоказательНаСервере("TEL", ЭтаФорма, "ОтчетОДебиторскойИКредиторскойЗадолженности")));
