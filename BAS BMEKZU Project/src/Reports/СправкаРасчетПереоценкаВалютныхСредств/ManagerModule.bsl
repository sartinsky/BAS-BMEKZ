...
Функция ПолучитьТекстЗаголовка(ПараметрыОтчета, ОрганизацияВНачале = Истина) Экспорт 
	...
	Возврат НСтр("ru='Справка-расчет переоценки валютных средств ';uk='Довідка-розрахунок переоцінки валютних коштів '") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(?(ПараметрыОтчета.СНачалаГода,НачалоГода(ПараметрыОтчета.НачалоПериода),ПараметрыОтчета.НачалоПериода),
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	МассивСумм.Добавить("СуммаВГривнах");
	МассивСумм.Добавить("СуммаВГривнахПослеПереоценки");
