...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мИдГруппы2 = "R2";
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРаздела(ИдГруппы, ИдГрупп) Экспорт
	...
	Если ИдГруппы = ИдГрупп.мИдГруппы1  ИЛИ ИдГруппы = ИдГрупп.мИдГруппы2 Тогда
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	ДанныеТаблицы2 	=	СтруктураПараметров.ПервичныйДокумент.R2;
	...
	мСтруктураИсхКолвоСтрокРазделов.Свойство(СтруктураРеквизитовФормы.мИдГруппы2, ИсходноеКоличествоСтрокГруппы);
	...
	ДанныеМногострочки2 = ДанныеФормыВЗначение(ДанныеФормы, Тип("ТаблицаЗначений"));
	// для второй таблицы
	НомерСтроки = 1;
	ДобавлятьСтроку = Ложь;
	Для каждого СтрокаТЧ2 Из ДанныеТаблицы2 Цикл
		
		Если ДобавлятьСтроку Тогда
			СтрокаМногострочки2 = ДанныеМногострочки2.Добавить();
		Иначе		
			СтрокаМногострочки2 = ДанныеМногострочки2[0];
			ДобавлятьСтроку = Истина;
		КонецЕсли;
		
		Для каждого Колонка Из ДанныеТаблицы2.Колонки Цикл
			
			ИмяКолонки = Колонка.Имя;
			
			ИмяXSD = ИмяКолонки;
			
			Если ДанныеМногострочки2.Колонки.Найти("R2" + ИмяXSD) = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			СтрокаМногострочки2["R2" + ИмяXSD] = СтрокаТЧ2[ИмяКолонки];
	...
	ЗначениеВРеквизитФормы(ДанныеМногострочки2, СтруктураРеквизитовФормы.мИдГруппы2);
