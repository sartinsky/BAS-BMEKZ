...
Функция ПодготовитьПараметрыОтчета()
	...
	ИначеЕсли Отчет.НаборПоказателейОтчета = 2 Тогда
		...
		ПараметрыОтчета.Вставить("НазваниеНабораПоказателейОтчета"  , НСтр("ru='налоговый учет';uk='податковий облік'"));
	КонецЕсли;	
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru='Справка-расчет распределения косвенных расходов';uk='Довідка-розрахунок розподілу непрямих витрат'") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(?(Отчет.СНачалаГода,НачалоГода(Отчет.НачалоПериода),Отчет.НачалоПериода), Отчет.КонецПериода);
