...
Процедура СформироватьОбложкуИнвентарнойКнигиОС(ПараметрыОтчета)
	...
	Макет.Параметры.НаименованиеОрганизации          = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(Сведения, "ПолноеНаименование,");
	Макет.Параметры.ОКПО                             = БухгалтерскийУчетПереопределяемый.ПолучитьКодОрганизации(Сведения);
	
	Если ЗначениеЗаполнено(НачалоПериода) Тогда
		СтрокаС = Формат(НачалоПериода, "ДЛФ=DD"+";Л="+Локализация.ОпределитьКодЯзыкаДляФормат(Локализация.КодЯзыкаИнтерфейса()));
	Иначе
		СтрокаС = НСтр("ru='""____"" ___________ ______ г.';uk='""____"" ___________ ______ р.'");
	КонецЕсли;
	
	Если ЗначениеЗаполнено(КонецПериода) Тогда
		СтрокаПо = Формат(КонецПериода, "ДЛФ=DD"+";Л="+Локализация.ОпределитьКодЯзыкаДляФормат(Локализация.КодЯзыкаИнтерфейса()));
	Иначе
		СтрокаПо = НСтр("ru='""____"" ___________ ______ г.';uk='""____"" ___________ ______ р.'");
	КонецЕсли;
	ШаблонСтрокаПериода = НСтр("ru='с %1 по %2';uk='з %1 по %2'");
	...
	СписокСформированныхЛистов.Добавить(ДокументРезультат, НСтр("ru='Обложка';uk='Обкладинка'"));
...
Процедура СформироватьСтраницыИнвентарнойКнигиОС(ПараметрыОтчета)
	...
	|
	|" + ?(НЕ Подразделение.Пустая() ИЛИ НЕ МОЛ.Пустая(), "ГДЕ " ,"") + ?(Подразделение.Пустая(), "", "(СрезПоследнихПоРегистрамСведенийИБухгалтерии.ПоследнееМестонахождение = &Подразделение ИЛИ СрезПоследнихПоРегистрамСведенийИБухгалтерии.ПредпоследнееМестонахождение = &Подразделение)") + 
	?(МОЛ.Пустая(), "", ?(Подразделение.Пустая(), "", " И ") + "(СрезПоследнихПоРегистрамСведенийИБухгалтерии.ПоследнееМОЛ = &МОЛ ИЛИ СрезПоследнихПоРегистрамСведенийИБухгалтерии.ПредпоследнееМОЛ = &МОЛ)") + "
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаДокумента
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипДокумента = "ВводВЭксплуатациюОС" Тогда
			...
			СтрокаИнвентарнойКниги.ПоступилоМОЛ                                = ОбщегоНазначенияБПВызовСервера.ФамилияИнициалыФизЛица(Выборка.ПоследнееМОЛПредставление);
		...
		ИначеЕсли Выборка.ТипДокумента = "ПеремещениеОС" И Выборка.ПоследнееМестонахождение = Подразделение Тогда
			...
			СтрокаИнвентарнойКниги.ПеремещениеИлиВыбытиеМОЛ                    = ОбщегоНазначенияБПВызовСервера.ФамилияИнициалыФизЛица(Выборка.ПредпоследнееМОЛПредставление);
			СтрокаИнвентарнойКниги.ПеремещениеИлиВыбытиеПричинаВыбытия         = "";
		...
		Иначе                                                         
			...
			СтрокаИнвентарнойКниги.ПеремещениеИлиВыбытиеМОЛ                    = ОбщегоНазначенияБПВызовСервера.ФамилияИнициалыФизЛица(Выборка.ПоследнееМОЛПредставление);
	...
	СписокСформированныхЛистов.Добавить(ДокументРезультат, НСтр("ru='Страницы';uk='Сторінки'"));
