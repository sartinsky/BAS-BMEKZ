...
&НаКлиенте
Перем ОбработчикПослеФормированияНаКлиенте Экспорт; //Обработчик после Формирования
&НаКлиенте
...
Процедура ПриЗагрузкеВариантаНаСервере(НовыеНастройкиКД)
	...
	Если РежимРасшифровки Тогда
		ОтчетНаименованиеТекущегоВарианта = ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(
			НовыеНастройкиКД.ДополнительныеСвойства, "ВариантНаименование", ПредставлениеТекущегоВарианта);
...
//++ InAgro Степаненко 
&НаКлиенте
Процедура ВлжностьОтПриИзменении(Элемент)
    УстановитьЗначениеПоказателя(ЭтаФорма.ВлажностьОт,"Влажность",ВидСравненияКомпоновкиДанных.БольшеИлиРавно)
КонецПроцедуры

&НаКлиенте
Процедура ВлажностьДоПриИзменении(Элемент)
    УстановитьЗначениеПоказателя(ЭтаФорма.ВлажностьДо,"Влажность",ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);
КонецПроцедуры

&НаКлиенте
Процедура СорностьОтПриИзменении(Элемент)
     УстановитьЗначениеПоказателя(ЭтаФорма.СорностьОт,"СорнаяПримесь",ВидСравненияКомпоновкиДанных.БольшеИлиРавно);    
КонецПроцедуры

&НаКлиенте
Процедура СорностьДоПриИзменении(Элемент)
    УстановитьЗначениеПоказателя(ЭтаФорма.СорностьДо,"СорнаяПримесь",ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);  
КонецПроцедуры
// -- InAgro Степаненко 
////////////////////////////////////////////////////////////////////////////////
// Табличный документ
...
Процедура ПередФормированиемНаКлиенте()
	...
	ВыполнятьЗамеры = НастройкиОтчета.ВыполнятьЗамеры И ЗначениеЗаполнено(НастройкиОтчета.КлючЗамеров);
	Если ВыполнятьЗамеры Тогда
		Комментарий = НастройкиОтчета.ПрефиксЗамеров + "; " + НСтр("ru='Непосредственно:';uk='Безпосередньо:'") + " " + Строка(Непосредственно);
		МодульОценкаПроизводительностиКлиент = ОбщегоНазначенияКлиент.ОбщийМодуль("ОценкаПроизводительностиКлиент");
		ИдентификаторЗамера = МодульОценкаПроизводительностиКлиент.НачатьЗамерВремени(
			Ложь,
...
//++ InAgro Степаненко 
&НаКлиенте 
Процедура УстановитьЗначениеПоказателя(Значение,Показатель,ВидСравнения)
    
    Если НЕ ТипЗнч(Отчет.КомпоновщикНастроек.Настройки.Структура[0]) = Тип("ГруппировкаКомпоновкиДанных") Тогда
        НастрокиОтборы = Отчет.КомпоновщикНастроек.Настройки.Структура[0].Настройки.Отбор.Элементы;
    Иначе	
        НастрокиОтборы = Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы;
    КонецЕсли;
                     
    Для каждого ЭлементОтбора Из НастрокиОтборы Цикл
        
        Если ЭлементОтбора.ЛевоеЗначение = новый ПолеКомпоновкиДанных(Показатель)  Тогда
            Если ЭлементОтбора.ВидСравнения = ВидСравнения Тогда
...
Функция ФоновоеЗаданиеЗапустить(Знач ФормированиеПриОткрытии, Непосредственно)
	...
	ПараметрыЗапуска.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Выполнение отчета: %1';uk='Виконання звіту: %1'"),
		ИмяОтчета);
...
Процедура ЗарегистрироватьОтключаемыеСвязи(Информация)
	...
	Для Каждого ОписаниеСвязи Из Информация.ОтключаемыеСвязи Цикл
		...
		Если ОписаниеСвязи.Свойство("ВедущийТип")
			И ТипЗнч(ОписаниеСвязи.ВедущийТип) = Тип("Тип") Тогда 
			Связь.ВедущийТип = Новый ОписаниеТипов(
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеСвязи.ВедущийТип));
		КонецЕсли;
		
		Если ОписаниеСвязи.Свойство("ПодчиненныйТип")
			И ТипЗнч(ОписаниеСвязи.ПодчиненныйТип) = Тип("Тип") Тогда 
			Связь.ПодчиненныйТип = Новый ОписаниеТипов(
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ОписаниеСвязи.ПодчиненныйТип));
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
