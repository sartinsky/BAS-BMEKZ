...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КадровыйПереводДвумяСтроками = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("РегламентированнаяОтчетность_ЕСВ_КадровыйПереводДвумяСтроками", "КадровыйПереводДвумяСтроками", Истина);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ОбщегоНазначенияВызовСервера.ХранилищеОбщихНастроекСохранить("РегламентированнаяОтчетность", "РегламентированнаяОтчетность_ЕСВ_КадровыйПереводДвумяСтроками",  ЭтаФорма.КадровыйПереводДвумяСтроками);	
...
Функция ОбработатьДРФО(Знач ДРФО)
	...
	ИначеЕсли СтрДлина(ДРФО) = 9 Тогда
		...
		//Новый паспорт в виде пластиковой карты 
		Если СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ДРФО) Тогда
			// к номеру паспорта добавляем "П"
			Возврат "П" + ДРФО;
		Иначе
			// не соответствует формату паспорта
			Возврат ДРФО;
...
Функция  СформироватьСписокТрудовыхОтношенийТаблица5()
	...
	Для Каждого стр ИЗ Работавшие Цикл
		...
		Если стр.ПричинаИзмененияСостояния = Перечисления.ПричиныИзмененияСостояния.Перемещение И (НачалоДня(стр.ДатаПриема)-1)>= СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета И КадровыйПереводДвумяСтроками Тогда
			...
			КонецЕсли;	
			ТекСтрока.T5T1RXXXXG16S  = ОбработатьПредставлениеОснования(стр.Основание, стр.ОснованиеДата);
		
			//Должность показываем для всех кадровых событий
			ТекСтрока.T5T1RXXXXG12S  	= СокрЛП(стр.ДолжностьНаименованиеПоКППредыдущий);
			ТекСтрока.T5T1RXXXXG13  	= СокрЛП(стр.КодЗКППТРПредыдущий);
			ТекСтрока.T5T1RXXXXG14S  	= СокрЛП(стр.ДолжностьКодКППредыдущий);
		...
		//Должность показываем для всех кадровых событий
		ТекСтрока.T5T1RXXXXG12S  	= СокрЛП(стр.ДолжностьНаименованиеПоКП);
		ТекСтрока.T5T1RXXXXG13  	= СокрЛП(стр.КодЗКППТР);
		ТекСтрока.T5T1RXXXXG14S  	= СокрЛП(стр.ДолжностьКодКП);
		...
		Если стр.ПричинаИзмененияСостояния = Перечисления.ПричиныИзмененияСостояния.Увольнение Тогда
			ТекСтрока.T5T1RXXXXG17S   = стр.СтатьяКЗОТ;
		Иначе
			
			Если ЗначениеЗаполнено(стр.ДатаСозданияРабочегоМеста) Тогда
				ТекСтрока.T5T1RXXXXG18D  = стр.ДатаСозданияРабочегоМеста;   
			КонецЕсли;
