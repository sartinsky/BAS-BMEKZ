...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru='Ожидаемый и подтвержденный НДС продаж ';uk='Очікуваний та підтверджений ПДВ продажів '") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(?(Отчет.СНачалаГода,НачалоГода(Отчет.НачалоПериода),Отчет.НачалоПериода), Отчет.КонецПериода);
...
Процедура ЗаполнитьГруппировкиПоУмолчанию()
	...
	НоваяСтрока = Отчет.Группировка.Добавить();
	НоваяСтрока.Поле = "ВозвратнаяТара";
	НоваяСтрока.Представление = "Возвратная тара";
	НоваяСтрока.Использование = Истина;
	НоваяСтрока.ТипГруппировки = 0;
...
&НаСервере
Процедура УстановитьИспользованиеГруппировки(ИмяГруппировки, Значение)
	
	НайденныеСтроки = Отчет.Группировка.НайтиСтроки(Новый Структура("Поле", ИмяГруппировки));
	Если НайденныеСтроки.Количество() = 1 Тогда
		Если НайденныеСтроки[0].Использование <> Значение Тогда
			НайденныеСтроки[0].Использование = Значение;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура УстановитьФлагиГруппировокПоВидуОтчета()
	
	Если Отчет.ВидОтчета = "РасчетыВозврат" Тогда
		
		УстановитьИспользованиеГруппировки("Валюта", Истина);
		УстановитьИспользованиеГруппировки("Контрагент", Истина);
		УстановитьИспользованиеГруппировки("ДоговорКонтрагента", Истина);
		
	ИначеЕсли Отчет.ВидОтчета = "Розница" Тогда
		
		УстановитьИспользованиеГруппировки("Валюта", Ложь);
		УстановитьИспользованиеГруппировки("Контрагент", Ложь);
		УстановитьИспользованиеГруппировки("ДоговорКонтрагента", Ложь);		
		
	ИначеЕсли Отчет.ВидОтчета = "УсловнаяПродажа" Тогда
...
Процедура ВидОтчетаПриИзменении(Элемент)
	...
	УстановитьФлагиГруппировокПоВидуОтчета();
