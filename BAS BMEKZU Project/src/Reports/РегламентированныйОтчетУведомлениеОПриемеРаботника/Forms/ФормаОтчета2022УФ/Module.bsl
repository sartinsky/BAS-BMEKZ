...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.Вставить("ДатаНач", 	ТекущаяДатаСеанса());
    СтруктураРеквизитовФормы.Вставить("ДатаКон", 	ТекущаяДатаСеанса());
    СтруктураРеквизитовФормы.Вставить("ВключатьГПХ", 	Ложь);
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("ДатаНач");
	СтруктураРеквизитовФормы.Вставить("ДатаКон");
...
Процедура ПоказатьПериод()
	...
	СтруктураРеквизитовФормы.СтрПериодОтчета = ПредставлениеПериода(НачалоДня(СтруктураРеквизитовФормы.ДатаНач), КонецДня(СтруктураРеквизитовФормы.ДатаКон), "ФП = Истина");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	PERIOD_MONTH = Месяц(мДатаНачалаПериодаОтчета);
	PERIOD_YEAR	 = Год(мДатаНачалаПериодаОтчета);
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если НЕ СписокСохранения.Свойство("ВключатьГПХ", СтруктураРеквизитовФормы.ВключатьГПХ) Тогда
		СтруктураРеквизитовФормы.ВключатьГПХ = Ложь;
	КонецЕсли;	
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("ВключатьГПХ", СтруктураРеквизитовФормы.ВключатьГПХ);
...
Процедура ДатаПодписиПриИзменении(Элемент)
	...
	Если СтруктураРеквизитовФормы.ДатаНач > СтруктураРеквизитовФормы.ДатаКон Тогда
		СтруктураРеквизитовФормы.ДатаКон	= СтруктураРеквизитовФормы.ДатаНач; 
	КонецЕсли;
	СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета  = СтруктураРеквизитовФормы.ДатаКон;
	СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета = НачалоМесяца(ДатаПодписи);
	
	PERIOD_MONTH = Месяц(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	PERIOD_YEAR	 = Год(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РаботникиОрганизаций.Сотрудник.ФизическоеЛицо.КодПоДРФО КАК ИНН,
	               |	ВЫБОР
	               ...
	               |	РаботникиОрганизаций.Сотрудник.ФизическоеЛицо.УНЗР КАК УНЗР
	               |ИЗ
	               |	РегистрСведений.ИНАГРО_РаботникиОрганизаций.СрезПервых(, Организация = &Организация) КАК РаботникиОрганизаций
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаОкончания, ) КАК ФИОФизЛиц
	               |		ПО (ФИОФизЛиц.ФизическоеЛицо = РаботникиОрганизаций.Сотрудник.ФизическоеЛицо)
	               |ГДЕ
	...
	//ПараметрыПолученияСотрудников.НачалоПериода = СтруктураРеквизитовФормы.ДатаНач;
	//ПараметрыПолученияСотрудников.ОкончаниеПериода = СтруктураРеквизитовФормы.ДатаКон;
	//
	//ТаблицаСотрудниковОрганизации = КадровыйУчет.СотрудникиОрганизации(Истина, ПараметрыПолученияСотрудников);
	//
	//Если Метаданные.Справочники.ФизическиеЛица.Реквизиты.Найти("УНЗР") = Неопределено Тогда
	//
	...
	//		КонецДня(СтруктураРеквизитовФормы.ДатаКон));
	//		
	//	ТаблицаКадровыеДанныеСотрудников.Колонки.Добавить("УНЗР", Новый ОписаниеТипов("Строка",,Новый КвалификаторыСтроки(14)));	
	//Иначе
	//	ТаблицаКадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(
	//		Истина, 
	//		ТаблицаСотрудниковОрганизации.ВыгрузитьКолонку("Сотрудник"),                                                     
	//		"ДатаПриема, ФизическоеЛицо, КодПоДРФО, УНЗР, ВидЗанятости, ПриказОПриеме, ПриказОПриемеНомер, ПриказОПриемеДата",
	//		КонецДня(СтруктураРеквизитовФормы.ДатаКон));
	//КонецЕсли;		
	//
	//	
	...
	//Запрос.УстановитьПараметр("ДатаНач", СтруктураРеквизитовФормы.ДатаНач);
	//Запрос.УстановитьПараметр("ДатаКон", СтруктураРеквизитовФормы.ДатаКон);
	//Запрос.УстановитьПараметр("Организация", СтруктураРеквизитовФормы.Организация);
	//
	//Если ЗарплатаКадрыРасширенная И СтруктураРеквизитовФормы.ВключатьГПХ Тогда
	...
	//		|	ПериодыДействияДоговоровГражданскоПравовогоХарактера.ФизическоеЛицо.КодПоДРФО КАК КодПоДРФО,
	//		|	ПериодыДействияДоговоровГражданскоПравовогоХарактера.ФизическоеЛицо.УНЗР КАК УНЗР,
	//		|	ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ПустаяСсылка) КАК ВидЗанятости,
	//		|	ПериодыДействияДоговоровГражданскоПравовогоХарактера.ДатаНачала КАК ДатаПриема,
	//		|	ПериодыДействияДоговоровГражданскоПравовогоХарактера.ДокументОснование.Дата КАК ПриказОПриемеДата,
	//		|	ПериодыДействияДоговоровГражданскоПравовогоХарактера.ДокументОснование.Номер КАК ПриказОПриемеНомер
	//		|ИЗ
	...
	//	
	//	Если Метаданные.Справочники.ФизическиеЛица.Реквизиты.Найти("УНЗР") = Неопределено Тогда
	//		Запрос.Текст = СтрЗаменить(Запрос.Текст, "ПериодыДействияДоговоровГражданскоПравовогоХарактера.ФизическоеЛицо.УНЗР", """""");	
	//	КонецЕсли;
	//	Выборка = Запрос.Выполнить().Выбрать();
	//	Пока Выборка.Следующий() Цикл
	//		НС = ТаблицаКадровыеДанныеСотрудников.Добавить();
	...
	//	Истина, 
	//	ТаблицаКадровыеДанныеСотрудников.ВыгрузитьКолонку("ФизическоеЛицо"), 
	...
	//|	СотрудникиОрганизации.УНЗР КАК УНЗР,
	...
	//|			ТОГДА 1
	//|		КОГДА СотрудникиОрганизации.ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ПустаяСсылка)
	//|			ТОГДА 4
	//|		ИНАЧЕ 2
	//|	КОНЕЦ КАК Категория,
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			Строка["T1RXXXXG5"] = Результат.Категория;      		   
			Строка["T1RXXXXG6S"] = Результат.ИНН;      		   
			Строка["T1RXXXXG7S"] = Результат.УНЗР;
			Строка["T1RXXXXG81S"] = Результат.Фамилия;      		   
			Строка["T1RXXXXG82S"] = Результат.Имя;
			Строка["T1RXXXXG83S"] = Результат.отчество;
			Строка["T1RXXXXG9S"] = РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(Результат.НомерПриказа, Истина, Истина);
			Строка["T1RXXXXG10D"] = Результат.ДатаПриказа;
			Строка["T1RXXXXG11D"] = Результат.ДатаНачала;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Для Сч = 1 По КоличествоСтраниц Цикл
		...
		Если СтруктураРеквизитовФормы.Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
			НовСтрока.ИмяСхемы = "J3001002"; 
		Иначе	
			НовСтрока.ИмяСхемы = "F3001002"; 
		КонецЕсли;
...
Процедура ОткрытьФормуПараметров(Команда)
	...
	мПараметрыФормы.Вставить("ВключатьГПХ", СтруктураРеквизитовФормы.ВключатьГПХ);
