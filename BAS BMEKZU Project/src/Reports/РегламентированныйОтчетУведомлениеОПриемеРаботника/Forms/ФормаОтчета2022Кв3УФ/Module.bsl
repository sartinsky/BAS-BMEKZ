...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2022Кв3УФ";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "27/07/2022";
    ...
    СтруктураРеквизитовФормы.Вставить("ВключатьГПХ", 	Истина);
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	//номер листа
	ЗаполнитьПоказательНаСервере("NUMList", 1, 		Форма, "Уведомление");
...
Процедура ВосстановитьСохраненныеДанные()
	...
	Если НЕ СписокСохранения.Свойство("ВключатьГПХ", СтруктураРеквизитовФормы.ВключатьГПХ) Тогда
		СтруктураРеквизитовФормы.ВключатьГПХ = Истина;
	КонецЕсли;	
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ 
				   ...
				   |	ДоговорНаВыполнениеРаботСФизЛицом.Сотрудник.ФизическоеЛицо.КодПоДРФО КАК ИНН,
				   |	ДоговорНаВыполнениеРаботСФизЛицом.Сотрудник.ФизическоеЛицо.УНЗР КАК УНЗР,
				   |	ДоговорНаВыполнениеРаботСФизЛицом.Ссылка КАК Приказ,
	               |	ДоговорНаВыполнениеРаботСФизЛицом.Дата КАК ДатаПриказа,
	               |	4 КАК Категория,
	               |	ДоговорНаВыполнениеРаботСФизЛицом.ДатаНачала КАК ДатаНачала
	               |ИЗ
	               |	Документ.ИНАГРО_ДоговорНаВыполнениеРаботСФизЛицом КАК ДоговорНаВыполнениеРаботСФизЛицом
	               |ГДЕ
				   |	&ВключатьГПХ
				   |	И ДоговорНаВыполнениеРаботСФизЛицом.ДатаНачала МЕЖДУ &ДатаНачала И &ДатаОкончания
				   |	И ДоговорНаВыполнениеРаботСФизЛицом.Проведен
				   |	И НЕ ДоговорНаВыполнениеРаботСФизЛицом.ПометкаУдаления
				   |;
				   |
				   |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	РаботникиОрганизаций.ИНН КАК ИНН,
				   |	РаботникиОрганизаций.УНЗР КАК УНЗР,
	               |	РаботникиОрганизаций.Приказ КАК Приказ,
	               |	РаботникиОрганизаций.ДатаПриказа КАК ДатаПриказа,
	               |	РаботникиОрганизаций.Категория КАК Категория,
	               |	РаботникиОрганизаций.ДатаНачала КАК ДатаНачала
	               |ИЗ
	               |	ВТРаботникиОрганизаций КАК РаботникиОрганизаций
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ФИОФизическихЛиц.СрезПоследних(&ДатаОкончания, ) КАК ФИОФизическихЛиц
	               |		ПО (ФИОФизическихЛиц.ФизическоеЛицо = РаботникиОрганизаций.ФизическоеЛицо)
	               |УПОРЯДОЧИТЬ ПО
	...
	Запрос.УстановитьПараметр("ВключатьГПХ", 	СтруктураРеквизитовФормы.ВключатьГПХ);  
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Результат.Следующий() Цикл
			...
			Строка["T1RXXXXG9S"] = РегламентированнаяОтчетностьКлиентСервер.ПолучитьНомерНаПечать(Результат.Приказ.Номер, Истина, Истина);
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Для Сч = 1 По КоличествоСтраниц Цикл
		...
		Если СтруктураРеквизитовФормы.Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
			НовСтрока.ИмяСхемы = "J3001003"; 
		Иначе	
			НовСтрока.ИмяСхемы = "F3001003"; 
		КонецЕсли;
