...
&НаСервере
Процедура УстановитьШапкаНомерРаздела()
	ШапкаНомерРаздела= "(пункт 4 розділу I)";
	Попытка
		УстановитьЗначениеПоказателя("Штампик",  ШапкаНомерРаздела);
	Исключение
	КонецПопытки;
...
Процедура ОбновитьПользовательскиеНастройки(СписокСохранения, ВыполнялосьОбновлениеДанных)
	...
	Если ВерсияФормы = "01/07/2013" Тогда
		...
		Если СписокСохранения.Свойство( "ПоказателиОтчета", ПоказателиОтчета) Тогда
			ПоказателиОтчета.ПолеТабличногоДокументаОтчетМП.Вставить( "Штампик", "(пункт 5 розділу I)");
