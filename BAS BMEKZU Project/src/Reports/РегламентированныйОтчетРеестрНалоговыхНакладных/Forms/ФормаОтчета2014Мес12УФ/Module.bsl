...
Функция ПодготовитьЗапросРаздел1_2014Мес12(СортировкаТолькоПоНомеру = Ложь) 
	...
	ТекстЗапроса = "ВЫБРАТЬ
				   ...
				   |	ВЫБОР КОГДА ВложенныйЗапрос.СтавкаНДС = &СтавкаНДС20 ИЛИ ВложенныйЗапрос.СтавкаНДС = &СтавкаНДССпец ТОГДА
				   //ИНАГРО --
	   ...
	   СтруктураПараметров.Вставить("СтавкаНДС20",  Перечисления.СтавкиНДС.НДС20);
   	   СтруктураПараметров.Вставить("СтавкаНДССпец",Перечисления.СтавкиНДС.СпецСт8); //ИНАГРО
...
Функция ПодготовитьЗапросРаздел2_2014Мес12()  
	...
	|		КОГДА НДСНалоговыйКредит.Регистратор ССЫЛКА Документ.ИНАГРО_ГТДИмпортБиологическихАктивов
	|			ТОГДА НДСНалоговыйКредит.Регистратор.НомерГТД")+"
	//ИНАГРО --
	|	КОНЕЦ КАК G4S,
	|	ВЫБОР
	...
	|	ВЫБОР КОГДА СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НДС20) ИЛИ СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.СпецСт8) ИЛИ СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС) ИЛИ СтавкаНДС = ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.НеНДС) ТОГДА ВЫБОР
    //ИНАГРО --
	|		КОГДА       НДСНалоговыйКредит.СтатьяДекларацииНДСНалоговыйКредит = &СтДекл10_1
	|				ИЛИ НДСНалоговыйКредит.СтатьяДекларацииНДСНалоговыйКредит = &СтДекл10_1_ОС
	|				ИЛИ НДСНалоговыйКредит.СтатьяДекларацииНДСНалоговыйКредит = &СтДекл11_1
	...
	//|	ВЫБОР
	//|		КОГДА НДСНалоговыйКредит.Регистратор ССЫЛКА Документ.РегистрацияВходящегоНалоговогоДокумента
	//|			ТОГДА НДСНалоговыйКредит.Регистратор.ИНАГРО_СпецРежимВход
	//|	КОНЕЦ КАК СпецРеж,
	//|	ВЫБОР
	//|		КОГДА НДСНалоговыйКредит.Регистратор ССЫЛКА Документ.РегистрацияВходящегоНалоговогоДокумента
	//|			ТОГДА НДСНалоговыйКредит.Регистратор.ИНАГРО_Филиал
	//|	КОНЕЦ КАК Филиал
    //ИНАГРО --
	|ИЗ
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() <> "БП" И РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации() <> "БСП" Тогда
		Возврат;
	КонецЕсли;	
...
Процедура ВывестиСтрокуВТаблицу(Выборка, Таблица,  Раздел, ИндексСтроки)
	...
	Если Раздел = СтруктураРеквизитовФормы.мИдГруппы2 Тогда
		...
		Если ТипЗнч(Выборка.Документ) = Тип("ДокументСсылка.ГТДИмпорт") ИЛИ (НЕ Выборка.Документ = Неопределено И Выборка.Документ.Метаданные().Имя = "ИНАГРО_ГТДИмпортБиологическихАктивов") Тогда
	    //ИНАГРО --
	...
	Если Раздел = СтруктураРеквизитовФормы.мИдГруппы2 Тогда
		...
		//СтрокаТаблицы[Раздел + "XXXX" + "G4S"]  = Выборка.G4S + ?(ЗначениеЗаполнено(Выборка.СпецРеж), "/"+Выборка.СпецРеж,"") + ?(ЗначениеЗаполнено(Выборка.Филиал), "/"+Выборка.Филиал,"");
		//ИНАГРО --
	Иначе
		//выданные НН
	...
	Если Раздел = СтруктураРеквизитовФормы.мИдГруппы2 Тогда
		...
		ИначеЕсли ТипЗнч(Выборка.Документ) = Тип("ДокументСсылка.ГТДИмпорт") ИЛИ (НЕ Выборка.Документ = Неопределено И Выборка.Документ.Метаданные().Имя = "ИНАГРО_ГТДИмпортБиологическихАктивов") Тогда
		//ИНАГРО --
			ВидДокумента = "МД" + Выборка.ВидНосителяДокумента;
	...
	Если Раздел = СтруктураРеквизитовФормы.мИдГруппы2 Тогда
		...
		Если    Выборка.ВидОперации = Перечисления.ВидыОперацийРегистрацияВходящегоНалоговогоДокумента.РаботыОтНерезидентаПрошлогоПериода 
			ИЛИ ТипЗнч(Выборка.Документ) = Тип("ДокументСсылка.ГТДИмпорт")
			ИЛИ (НЕ Выборка.Документ = Неопределено И Выборка.Документ.Метаданные().Имя = "ИНАГРО_ГТДИмпортБиологическихАктивов") //ИНАГРО
