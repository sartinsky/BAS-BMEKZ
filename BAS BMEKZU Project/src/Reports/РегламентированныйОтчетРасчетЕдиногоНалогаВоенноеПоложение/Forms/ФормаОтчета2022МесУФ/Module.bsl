...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2022МесУФ";
	...
	СтруктураРеквизитовФормы.мИдГруппы1 = "D1T1R";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "05/05/2022";
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Декларация плательщика единого налога (месячная форма)';uk='Декларація платника єдиного податку (місячна форма)'");
...
Процедура СохранитьДанныеРаздела(НаимРаздела, ОчищатьРазделЕслиЕстьМногострочность)
	...
	//
	ИначеЕсли НаимРаздела = "ДекларацияЕН" Тогда
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	// Многострочные и обычные разделы.
	//
	ИначеЕсли ИмяТекРаздела = "ДекларацияЕН"  Тогда
...
Процедура ПоказатьПериод()
	...
	СтруктураРеквизитовФормы.СтрПериодОтчета = ПредставлениеПериода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ФП = Истина");
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("HFILL",  ТекущаяДатаСеанса(), Форма, "ДекларацияЕН");
	...
	//ЗаполнитьПоказательНаСервере("HNAME", Организация.ИндивидуальныйПредприниматель, Форма, "Приложение2");
	//
	...
	//ПроставитьВКвадратыЗначенияНаВсехЗакладках(Форма, "HTIN_",  8, КодЕДРПОУ, Ложь,,"Приложение2");	
	//
	//Паспорт = ОбработатьДРФО(КодЕДРПОУ);
	//ЗаполнитьПоказательНаСервере("HTINPF", Паспорт, Форма, "Приложение2");
	//РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение2", "HTINPF_",  10, Паспорт, Ложь);	

	//ЗаполнитьПоказательНаСервере("HKVED", КодКВЕД, Форма, "Приложение2");
	//РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение2", "KVED_",  5, КодКВЕД, Ложь);		
	//
	...
	//
	////Код особы всегда = 6
	//ЗаполнитьПоказательНаСервере("R081G1", 6, Форма, "Приложение2");
	//
	...
	//
	//ЗаполнитьПоказательНаВсехЗакладках("HFILL",  Форма.ДатаПодписи, Форма, "Приложение2");
	//ПроставитьВКвадратыЗначенияНаВсехЗакладках(Форма, "Д_",  8, Формат(Форма.ДатаПодписи, "ДФ=ddMMyyyy"), Ложь,,"Приложение2");	

	ЗаполнитьПоказательНаВсехЗакладках("HZ",  "X",  Форма, "");
		
	ЗаполнитьПоказательНаВсехЗакладках("HZM",  Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), Форма, "");
	...
	ЗаполнитьПоказательНаВсехЗакладках("HZY",  		 Год, Форма, "");
	ПроставитьВКвадратыЗначенияНаВсехЗакладках(Форма, "HZY_", 4, Формат(Год, "ЧГ="), Ложь,,"");	
...
Процедура ЗаполнитьПоказательНаВсехЗакладках(ИмяПоказателя, Значение, Форма, СписокИсключений = "")
	...
	Если Найти(СписокИсключений, "|ДекларацияЕН|") = 0 Тогда
		ЗаполнитьПоказательНаСервере(ИмяПоказателя, Значение, Форма, "ДекларацияЕН");
	КонецЕсли;
...
Процедура ПроставитьВКвадратыЗначенияНаВсехЗакладках(Форма, ИмяПоказателя, Длина, Значение, ДополнитьЗначение = Ложь, НольКакНоль = Ложь, СписокИсключений = "")
	...
	Если Найти(СписокИсключений, "|ДекларацияЕН|") = 0 Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", ИмяПоказателя,  Длина, Значение, ДополнитьЗначение, НольКакНоль);		
	КонецЕсли;
...
// Обновляет структуру многострочного раздела, заданного
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	КонецЕсли;
    //декларация
	R002G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R001G3", Форма, "ДекларацияЕН")) * 0.02;
	ЗаполнитьПоказательНаСервере("R002G3", R002G3, Форма, "ДекларацияЕН");
	
	R004G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G3", Форма, "ДекларацияЕН"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R003G3", Форма, "ДекларацияЕН"));
	ЗаполнитьПоказательНаСервере("R004G3", R004G3, Форма, "ДекларацияЕН");
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	//Если ИмяРаздела = "ДекларацияЕН" И Область.Имя = "HZMP" Тогда
	//	СтруктураДанныхПриложение2.HZMP = ТабличныйДокумент.Области.HZMP.Значение;
	//КонецЕсли;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Попытка
		Запрос.УстановитьПараметр("Группа", Перечисления.ГруппыПлательщиковЕдиногоНалога.ТретьяГруппаОсобая);
	Исключение
	...
	ИначеЕсли Метаданные.РегистрыСведений.Найти("СтатусыПлательщиковЕдиногоНалога") <> Неопределено Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               |	НАЧАЛОПЕРИОДА(КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Период, ДЕНЬ) КАК Период,
		               |	КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Статья,
		               |	СУММА(КнигаДоходовРасходовПоЕдиномуНалогуОбороты.СуммаОборот) КАК Сумма
	...
	Пока Выборка.Следующий() Цикл
		...
		Если    Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыРеализация
			...
			ИЛИ Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыДоходыДругойСпособРасчетов
			ИЛИ Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыПросроченнаяЗадолженность 
			ИЛИ Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыДоходыПоВидуДеятельностиНеЕдиныйНалог
			ИЛИ Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыРеализацияСХПродукции
			ИЛИ Выборка.Статья = СтрокиДекларации.ЕННК_ДоходыДоходыПоВидуДеятельностиНеУказанномуВСвидетельстве Тогда	

			Доход = Доход + Выборка.Сумма;	
			  
		КонецЕсли;	
		
	КонецЦикла;
	
	СтруктураДанныхДекларацияЕН.R001G3 = Доход;
	
	///////////////////////////////////////////////////////////////////////////////////////////////////
	//// значение строки 2 и 4 предыдущей декларации
	//// Прошлый месяц относительно текущего месяца 
	//ДатПрошлогоПериода2 = НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) - 1;
	//ДатПрошлогоПериода1 = ДатПрошлогоПериода2;
	//
	...
	//																	"ДекларацияЕН",	,
	//																	НСтр("ru='строки 05';uk='рядка 05'"));
	//		
	//		Если НЕ ПоказателиПрошлойДекларации = Неопределено Тогда
	//			Попытка
	//				Ст05  = ПоказателиПрошлойДекларации.R004G3;
	//				СтруктураДанныхДекларацияЕН.R005G3 = Ст05;
...
Процедура ЗаполнитьМЗЗ(ПараметрыЗаполненияЗвит1С)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Таблица.Количество() < мСтруктураИсхКолвоСтрокРазделов[СтруктураРеквизитовФормы.мИдГруппы1] Цикл
			Таблица.Добавить();
		КонецЦикла;
		ЗначениеВРеквизитФормы(Таблица, СтруктураРеквизитовФормы.мИдГруппы1);
...
Процедура  ОпределитьСписокВыгружаемыхСхем(тбСхемыXML);
	...
	Для Каждого стр Из  тбСхемыXML Цикл
		...
		Если Лев(Стр.Префикс,1) = "1"  Тогда
			Если НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HD1", ЭтаФорма, "ДекларацияЕН")) Тогда
				Сообщение.Текст = НСтр("ru='Приложение 1 не выгружается, т.к. не установлена соответствующая пометка в Декларации';uk='Додаток 1 не вивантажується, т.щ. не встановлена відповідна позначка в Декларації'");
				Сообщение.Сообщить();
				стр.фВыгрузка = 0;
			КонецЕсли;
		КонецЕсли;

		Если Лев(Стр.Префикс,1) = "2"  Тогда
			Если НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HD2", ЭтаФорма, "ДекларацияЕН")) Тогда
				Сообщение.Текст = НСтр("ru='Приложение 2 не выгружается, т.к. не установлена соответствующая пометка в Декларации';uk='Додаток 2 не вивантажується, т.щ. не встановлена відповідна позначка в Декларації'");
				Сообщение.Сообщить();
				стр.фВыгрузка = 0;
			КонецЕсли;
		КонецЕсли;
