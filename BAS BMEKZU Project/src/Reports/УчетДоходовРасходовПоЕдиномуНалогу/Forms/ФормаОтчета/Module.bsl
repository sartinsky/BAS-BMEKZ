...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Отчет 			= Форма.Отчет;
	ЗаголовокОтчета = НСтр("ru='Типовая форма учета доходов и расходов ФОП ';uk='Типова форма обліку доходів та витрат ФОП'") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
