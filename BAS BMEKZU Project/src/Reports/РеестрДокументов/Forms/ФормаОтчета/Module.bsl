...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ (Параметры.Свойство("Ключ") И ТипЗнч(Параметры.Ключ) = Тип("Структура")) Тогда
		ТекстСообщения = НСтр("ru='Реестр документов может быть сформирован только из формы списка документов';uk='Реєстр документів може бути сформований тільки з форми списку документів'");
	...
	Иначе
		Заголовок = НСтр("ru='Реестр документов';uk='Реєстр документів'");
	КонецЕсли;
	...
	Для каждого КлючИЗначение Из ДополнительныеРеквизиты Цикл
		...
		Если КолонкаДопРеквизита = Неопределено Тогда
			...
			Если КлючИЗначение.Ключ = "ДатаВходящегоДокумента" ИЛИ КлючИЗначение.Ключ = "НомерВходящегоДокумента" Тогда
				...
				КонецЕсли;
			ИначеЕсли КлючИЗначение.Ключ = "ЕстьТовары" ИЛИ КлючИЗначение.Ключ = "ЕстьОборудование" ИЛИ КлючИЗначение.Ключ = "ЕстьУслуги" Тогда	
...
Процедура ИнициализироватьОтчет(ОсновнаяТаблица, ТекстЗапроса)
	...
	Если ЕстьОтборПоПервичнымДокументам Тогда
		...
		Если СтрНайти(ИмяТаблицы, "ПоступлениеТоваровУслуг") > 0 Тогда
			Элементы.ТоварыУслуги.СписокВыбора[1].Представление = НСтр("ru='Только поступления товаров';uk='Тільки надходження товарів'");
			Элементы.ТоварыУслуги.СписокВыбора[2].Представление = НСтр("ru='Только полученные услуги';uk='Тільки отримані послуги'");
		КонецЕсли;
	КонецЕсли;
...
Функция СформироватьНаСервере()
	...
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		НСтр("ru='Формирование реестра документов';uk='Формування реєстру документів'"));
