...
Функция СформироватьОтчетНаСервере()
	...
	Если ИБФайловая Тогда
		...
		Отчеты.ТоварныйОтчет.СформироватьОтчет(ПараметрыОтчета, АдресХранилища);
		РезультатВыполнения = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
			УникальныйИдентификатор, 
			"Отчеты.ТоварныйОтчет.СформироватьОтчет", 
			ПараметрыОтчета, 
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru='Товарный отчет';uk='Товарний звіт'") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
...
Процедура УстановитьНачальныеНастройки()
	...
	Если ЗначениеЗаполнено(ОсновнойСклад) И 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОсновнойСклад, "ТипСклада") <> Перечисления.ТипыСкладов.ОптовыйСклад Тогда
