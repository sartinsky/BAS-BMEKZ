...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	КонецЦикла;
	ЗаполнитьПоказательНаСервере("R09G1", ?(Форма.D6.Итог("D6R01G5")>0, "Х", ""), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("R09G2", ?(Форма.D6.Итог("D6R01G5")>0, Форма.D6.Количество(), ""), Форма, "ДекларацияАкциз");
	ЗаполнитьПоказательНаСервере("R09G3", ?(Форма.D6.Итог("D6R01G5")>0, Форма.D6.Количество(), ""), Форма, "ДекларацияАкциз");
