...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/09/2021";
	...
	Если ВерсияФормы = "01/03/2021" Тогда
		...
		Если СписокСохранения.Свойство("ДанныеМногостраничныхРазделов", ДанныеМногостраничныхРазделов) Тогда
			...
			Для Каждого ЭлементСтруктуры Из ДанныеМногостраничныхРазделов Цикл
				...
				Если ЭлементСтруктуры.Ключ = "ДекларацияАкциз" Тогда
					Для каждого СтраницаОтчета Из ЭлементСтруктуры.Значение Цикл
						ДанныеСтраницы = СтраницаОтчета.Данные;
						Для каждого Реквизит Из ДанныеСтраницы Цикл
							ЗначениеРеквизита = Реквизит.Значение;
						    ИмяРеквизита = Реквизит.Ключ;
							Если Лев(ВРЕГ(ИмяРеквизита), 3)  = ВРЕГ("R09") Тогда
								ДанныеСтраницы.Вставить(ИмяРеквизита, "");
								ДанныеСтраницы.Вставить("R010"+Прав(ВРЕГ(ИмяРеквизита),2), ЗначениеРеквизита);
							КонецЕсли;
							Если Лев(ВРЕГ(ИмяРеквизита), 4)  = ВРЕГ("R011") Тогда
								ДанныеСтраницы.Вставить(ИмяРеквизита, "");
								ДанныеСтраницы.Вставить("R012"+Прав(ВРЕГ(ИмяРеквизита),2), ЗначениеРеквизита);
							КонецЕсли;
							Если Лев(ВРЕГ(ИмяРеквизита), 4)  = ВРЕГ("R013") Тогда
								ДанныеСтраницы.Вставить(ИмяРеквизита, "");
								ДанныеСтраницы.Вставить("R014"+Прав(ВРЕГ(ИмяРеквизита),2), ЗначениеРеквизита);
							КонецЕсли;
									 				
						КонецЦикла;
						ИндексСтраницы = Число(СтрЗаменить(НайденноеЗначение,"ТаблицаСтраницыРаздела",""))-1;
						СписокСохранения.Вставить(ДанныеМногостраничныхРазделов.ДекларацияАкциз[ИндексСтраницы].Данные, ДанныеСтраницы);
					КонецЦикла;
				КонецЕсли;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ЭтоУточненка ИЛИ  HZMP8 Тогда
		ЗаполнитьПоказательНаСервере("R012G1", "Х", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R012G2", Форма.D8.Количество(), Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R012G3", Форма.D8.Количество(), Форма, "ДекларацияАкциз");
	Иначе
		ЗаполнитьПоказательНаСервере("R012G1", "", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R012G2",  0, Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R012G3",  0, Форма, "ДекларацияАкциз");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ПолучитьПоказательНаСервере("D10T1RXXXXG2S_1", Форма, "ДекларацияАкциз")) Тогда
		ЗаполнитьПоказательНаСервере("R014G1","Х", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R014G2", 1,  Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R014G3", 1,  Форма, "ДекларацияАкциз");
	Иначе
		ЗаполнитьПоказательНаСервере("R014G1", "", Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R014G2", 0,  Форма, "ДекларацияАкциз");
		ЗаполнитьПоказательНаСервере("R014G3", 0,  Форма, "ДекларацияАкциз");
	КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Для каждого Страница Из ТаблицаСтраницыРаздела1 Цикл
		...
		НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0209508", "F0209508");  
		...
		Для  Инд = 1 ПО Страница.ДанныеДопСтрок[0].Значение.D6.Количество() Цикл
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295608", "F0295608");  
		...
		Для  Инд = 1 ПО Страница.ДанныеДопСтрок[0].Значение.D8.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295808", "F0295808");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ЮрЛицо, "J0295908", "F0295908");  
