...
Процедура ЗаполнитьАвто(Команда)
    ...
    //Если СтруктураРеквизитовФормы.НаДату = Неопределено Тогда
    //	ТекстСообщения = НСтр("ru='В настройках отчета не заполнено поле ""На дату"" ';uk='В налаштуваннях звіту не заповнене поле ""На дату"" '");
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("Район");
	СтруктураРеквизитовФормы.Вставить("ГруппаКонтрагентов");
	СтруктураРеквизитовФормы.Вставить("НаДату");
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент) Экспорт
	...
	Если    Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Отчет" Тогда
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Отчет", "ЕДРПОУ", 	7, 	КодЕДРПОУ, Ложь,Истина);
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	// === Пшениця ===
     сч = 1;                   // 806
    Пока сч < 9 Цикл 		
        Значение = Форма.СтруктураДанныхОтчет["A807" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A808" + "_" + Сч]
        + Форма.СтруктураДанныхОтчет["A809" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A810" + "_" + Сч]
        + Форма.СтруктураДанныхОтчет["A811" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A812" + "_" + Сч];
        Форма.СтруктураДанныхОтчет["A806" + "_" + Сч] = Форма.СтруктураДанныхОтчет["A806" + "_" + Сч] + Значение;
        сч = сч + 1; 		
    КонецЦикла; 
    ...
    Пока сч < 9 Цикл 		
        Значение = Форма.СтруктураДанныхОтчет["A800" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A806" + "_" + Сч];
        Форма.СтруктураДанныхОтчет["A004" + "_" + Сч] = Форма.СтруктураДанныхОтчет["A004" + "_" + Сч] + Значение;
        сч = сч + 1;		
    КонецЦикла; 
    ...
    сч = 1;
    Пока сч < 8 Цикл
        Значение = Форма.СтруктураДанныхОтчет["A813" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A814" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A815" + "_" + Сч];
        Форма.СтруктураДанныхОтчет["A011" + "_" + Сч] = Форма.СтруктураДанныхОтчет["A011" + "_" + Сч] + Значение;
        сч = сч + 1;
    КонецЦикла;
    ...
    сч = 1;
    Пока сч < 9 Цикл       // 816
        Значение = Форма.СтруктураДанныхОтчет["A817" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A818" + "_" + Сч];
        Форма.СтруктураДанныхОтчет["A816" + "_" + Сч] =  Форма.СтруктураДанныхОтчет["A816" + "_" + Сч] +Значение;
        сч = сч + 1;
    КонецЦикла;
    
    сч = 1;
    Пока сч < 8 Цикл       // 819
        Значение = Форма.СтруктураДанныхОтчет["A820" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A821" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A822" + "_" + Сч];
        Форма.СтруктураДанныхОтчет["A819" + "_" + Сч] = Форма.СтруктураДанныхОтчет["A819" + "_" + Сч] +Значение;
        сч = сч + 1;
    КонецЦикла;
    
    сч = 1;
    Пока сч < 9 Цикл       // 012		
        Значение = Форма.СтруктураДанныхОтчет["A816" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A819" + "_" + Сч];
        Форма.СтруктураДанныхОтчет["A012" + "_" + Сч] = Форма.СтруктураДанныхОтчет["A012" + "_" + Сч] +Значение;
        сч = сч + 1;
    КонецЦикла;
    ...
    сч = 1;
    Пока сч < 9 Цикл       // 015
        Значение = Форма.СтруктураДанныхОтчет["A823" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A824" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A825" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A826" + "_" + Сч];
        Форма.СтруктураДанныхОтчет["A015" + "_" + Сч] = Форма.СтруктураДанныхОтчет["A015" + "_" + Сч] + Значение;
        сч = сч + 1;
    КонецЦикла; 
    ...
    сч = 1;
    Пока сч < 9 Цикл
        Значение = Форма.СтруктураДанныхОтчет["A032" + "_" + Сч];
        Форма.СтруктураДанныхОтчет["A027" + "_" + Сч] =  Форма.СтруктураДанныхОтчет["A027" + "_" + Сч] + Значение;
        сч = сч + 1;
    КонецЦикла;    
    
    // Культури зернові та зернобобові  003
    сч = 1;
    Пока сч < 9 Цикл
        Значение = Форма.СтруктураДанныхОтчет["A004" + "_" + Сч]+  Форма.СтруктураДанныхОтчет["A011" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A012" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A015" + "_" + Сч]+
        Форма.СтруктураДанныхОтчет["A021" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A022" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A024" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A025" + "_" + Сч]+ Форма.СтруктураДанныхОтчет["A027" + "_" + Сч];   
        Форма.СтруктураДанныхОтчет["A003" + "_" + Сч] =  Форма.СтруктураДанныхОтчет["A003" + "_" + Сч] +Значение;
        сч = сч + 1;	
    КонецЦикла;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Для каждого КодСтроки Из КодыСтроки  Цикл
		...
		//Области["A" + КодСтроки + "_1"].Значение = НужнаяСтрока[0].ЗачетныйВесКонечныйОстаток/1000;
	    ЭтаФорма.СтруктураДанныхОтчет["A" + КодСтроки + "_2"] = НужнаяСтрока[0].ЗачетныйВесПриход/1000; 		
		//Области["A" + КодСтроки + "_3"].Значение = НужнаяСтрока[0].КоличествоЗакупленнойПродукции/1000; 
		//Области["A" + КодСтроки + "_4"].Значение = НужнаяСтрока[0].СтоимостьКупленнойПродукции;
		 ЭтаФорма.СтруктураДанныхОтчет["A" + КодСтроки + "_5"] = НужнаяСтрока[0].ПереработкаЗачетныйВес/1000; 
	...
	Для каждого КодСтроки Из КодыСтроки  Цикл
		...
		ЭтаФорма.СтруктураДанныхОтчет["A" + КодСтроки + "_3"] = НужнаяСтрока[0].КоличествоЗакупленнойПродукции/1000;			 
...
Процедура НастройкаПоказателей(Команда)
    ...
    ОткрытьФорму("Отчет.ИНАГРО_РегламентированныйОтчет1Зерно.Форма.ФормаНастройкиСоответсвий2019",,ЭтаФорма,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
