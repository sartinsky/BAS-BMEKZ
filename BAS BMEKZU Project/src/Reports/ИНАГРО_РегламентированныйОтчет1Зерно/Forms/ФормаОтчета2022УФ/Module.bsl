...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбновитьДанныеПоказателейНаСервере();
	СтруктураРеквизитовФормы.СписокПечатаемыхЛистов = Новый СписокЗначений;
		
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2022УФ";
	...
	СтруктураДанныхОтчет		= Новый Структура;
...
Процедура ПриОткрытии(Отказ)
	...
	Заголовок = Заголовок + ?(Найти(ЭтаФорма.ИмяФормы, "Внешний") > 0, НСтр("ru=' - внешний отчет';uk=' - зовнішній звіт'"), "");
...
Процедура ОткрытьФормуПараметров(Команда)
	...
	мПараметрыФормы.Вставить("Район", 					СтруктураРеквизитовФормы.Район);
	мПараметрыФормы.Вставить("ГруппаКонтрагентов", 		СтруктураРеквизитовФормы.ГруппаКонтрагентов);
	мПараметрыФормы.Вставить("НаДату", 					СтруктураРеквизитовФормы.НаДату);	
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.ИмяСтраницы              = "Отчет";
...
Функция ОпределитьТабличноеПолеПоИдМногострочногоРаздела(ИдГруппы, ИдГрупп) Экспорт
	...
	Если  ИдГруппы = ИдГрупп.мИдГруппы1
		...
		ИЛИ ИдГруппы = ИдГрупп.мИдГруппы3 Тогда
		
		ТекТабличноеПоле = "Отчет";
...
Функция ОпределитьИдМногострочногоРазделаПоАктивнойСтранице(ИмяЛиста = Неопределено, ТекущаяСтраница, ИдГрупп)
	...
	Если ИмяТекущейСтраницыПанели = "Отчет" Тогда
...
Процедура УстановитьФорматВыводаВПоляхВвода(Форма, НаимРаздела) Экспорт
	...
	Для Каждого СтрПоказатели Из Форма.мТаблицаСоставПоказателей Цикл
		...
		Если ТипДанныхПоказателя = "Е" И ПустаяСтрока(КодПоказателяПоСтруктуре) Тогда
			...
			Иначе
				// Определяем кол-во строк многострочного раздела и применяем процедуру форматирования к данной графе.
				НСтр = 1;
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	Если Форма.СтруктураРеквизитовФормы.мПериодичность = Форма.ПеречислениеПериодичностьГод Тогда
	...
	ЗаполнитьПоказательНаСервере("REP_PERNM",НРег(Формат(мДатаКонцаПериодаОтчета,"Л=uk_UA; ДФ='MMMM yyyy'")), Форма, "Отчет");
	
	ЗаполнитьПоказательНаСервере("HZM", МесяцОтчета, Форма, "СлужебныеДанные");
		
	ЗаполнитьПоказательНаСервере("FIRM_NAME", Организация.НаименованиеПолное, Форма, "Отчет");
	...
	// По типовому Варианту
 	Сведения = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОбОрганизацииБП(Организация, мДатаКонцаПериодаОтчета, СписокПоказателей);

	КодЕДРПОУ         	= Сведения.КодПоЕДРПОУ;
	КодКВЕД             = Сведения.КодКВЕД;
	КодКВЕД_ВКвадраты	= Сведения.КодКВЕД;
	...
	FIRM_ADR_FIZ        = Сведения.ФактическийАдрес;
	...
	// По типовому Варианту
 	СведенияДатаПодписи = РегламентированнаяОтчетностьПереопределяемый.ПолучитьСведенияОбОрганизацииБП(Организация, Форма.ДатаПодписи, СписокПоказателей);
	
 	ФИОДиректора    = СведенияДатаПодписи.ИмяФАМИЛИЯРук;
	ИННДиректора    = СведенияДатаПодписи.ИННРук;
			
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Отчет", "ЕДРПОУ", 		8, 	КодЕДРПОУ, 	Ложь);
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Отчет", "КОАТУУ", 		10, КодКОАТУУ, 	Ложь);
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Отчет", "КАТОТТГ", 	19, КодКАТОТТГ, Ложь);
    
    ЗаполнитьПоказательНаСервере("FIRM_ADR",  	    HLOC, 		Форма, "Отчет");
    ЗаполнитьПоказательНаСервере("FIRM_ADR_FIZ",  	FIRM_ADR_FIZ, 		Форма, "Отчет");
	ЗаполнитьПоказательНаСервере("VIK_EMAIL", 	    Email, 	 Форма, "Отчет");
    ЗаполнитьПоказательНаСервере("VIK_TEL",         Телефон, Форма, "Отчет");	
    ЗаполнитьПоказательНаСервере("RUK",        		ФИОДиректора, Форма, "Отчет");
	...
	ЗаполнитьПоказательНаСервере("SPATO", 	КодКОАТУУ, 	Форма, "СлужебныеДанные");
	ЗаполнитьПоказательНаСервере("KATOTTG",  КодКАТОТТГ, Форма, "СлужебныеДанные");
	ЗаполнитьПоказательНаСервере("KATOTTG_FACT",  Прав(КодКАТОТТГ,17), Форма, "СлужебныеДанные");	
	...
	Если Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда
		...
		HTIN = КодЕДРПОУ;
	Иначе	
		ЗаполнитьПоказательНаСервере("HTINF",  КодЕДРПОУ, Форма, "СлужебныеДанные");	
		HTINF = КодЕДРПОУ;
	КонецЕсли;
...
Процедура ЗаполнитьПоказательНаВсехЗакладках(ИмяПоказателя, Значение, Форма, СписокИсключений = "")
	...
	Если Найти(СписокИсключений, "|Декларация|") = 0 Тогда
		ЗаполнитьПоказательНаСервере(ИмяПоказателя, Значение, Форма, "Отчет");
	КонецЕсли;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Перем ТаблицаБлокаT1, ТаблицаБлокаT2;
	...
	Если НЕ(Лев(ОбластьИмя, 4) = "A003" ИЛИ Лев(ОбластьИмя, 4) = "A004" ИЛИ Лев(ОбластьИмя, 4) = "A806" ИЛИ Лев(ОбластьИмя, 4) = "A011"
		ИЛИ Лев(ОбластьИмя, 4) = "A012" ИЛИ Лев(ОбластьИмя, 4) = "A816" ИЛИ Лев(ОбластьИмя, 4) = "A819" ИЛИ Лев(ОбластьИмя, 4) = "A015"
		ИЛИ Лев(ОбластьИмя, 4) = "A027") Тогда
	
		 //Рассчитываем итоги отчета       	
		// === Пшениця ===
	    сч = 1;                   // 806
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A807_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A808_" + Сч, Форма, "Отчет")
	        + ПолучитьПоказательНаСервере("A809_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A810_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A806_" + Сч, Значение, Форма, "Отчет");
	        сч = сч + 1; 
	    КонецЦикла; 
		
	    сч = 1;                   // 004
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A800_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A806_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A004_" + Сч, Значение, Форма, "Отчет");
	        сч = сч + 1; 
	    КонецЦикла; 
	    ...
	    сч = 1;                  
		Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A813_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A814_" + Сч, Форма, "Отчет") +  ПолучитьПоказательНаСервере("A815_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A011_" + Сч, Значение, Форма, "Отчет");
	        сч = сч + 1; 
	    КонецЦикла; 
	    ...
	    сч = 1;                   // 816
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A817_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A818_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A816_" + Сч, Значение, Форма, "Отчет");
	        сч = сч + 1; 
	    КонецЦикла; 
		
	    сч = 1;                   // 819
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A820_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A821_" + Сч, Форма, "Отчет") + ПолучитьПоказательНаСервере("A822_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A819_" + Сч, Значение, Форма, "Отчет");
	        сч = сч + 1; 
		КонецЦикла; 
		
	    сч = 1;                   // 012
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A816_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A819_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A012_" + Сч, Значение, Форма, "Отчет");
	        сч = сч + 1; 
	    КонецЦикла; 
	    ...
	    сч = 1;                   // 015
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A823_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A824_" + Сч, Форма, "Отчет") + ПолучитьПоказательНаСервере("A825_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A826_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A015_" + Сч, Значение, Форма, "Отчет");
	        сч = сч + 1; 
	    КонецЦикла; 
	    
	    // === Зернобобові сушені 027
		
	    сч = 1;                   // 015
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A032_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A027_" + Сч, Значение, Форма, "Отчет");
	        сч = сч + 1; 
	    КонецЦикла; 
		...
		A003 = 0;
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A004_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A011_" + Сч, Форма, "Отчет")
	        + ПолучитьПоказательНаСервере("A012_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A015_" + Сч, Форма, "Отчет")
	        + ПолучитьПоказательНаСервере("A021_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A022_" + Сч, Форма, "Отчет")
	        + ПолучитьПоказательНаСервере("A024_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A025_" + Сч, Форма, "Отчет")
	        + ПолучитьПоказательНаСервере("A027_" + Сч, Форма, "Отчет");
	        ЗаполнитьПоказательНаСервере("A003_" + Сч, Значение, Форма, "Отчет");
			A003 = A003 + Значение;
	        сч = сч + 1; 
		КонецЦикла; 
	...
	Иначе
		...
		A003 = 0;
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A003_" + Сч, Форма, "Отчет");
			A003 = A003 + Значение;
	        сч = сч + 1; 
		КонецЦикла; 
	...
	A041 = 0;
    Пока сч < 5 Цикл 		
        Значение = ПолучитьПоказательНаСервере("A041_" + Сч, Форма, "Отчет");
		A041 = A041 + Значение;
        сч = сч + 1; 
	КонецЦикла; 
	...
	A048 = 0;
    Пока сч < 5 Цикл 		
        Значение = ПолучитьПоказательНаСервере("A048_" + Сч, Форма, "Отчет");
		A048 = A048 + Значение;
        сч = сч + 1; 
	КонецЦикла; 
	...
	A052 = 0;
    Пока сч < 5 Цикл 		
        Значение = ПолучитьПоказательНаСервере("A052_" + Сч, Форма, "Отчет");
		A052 = A052 + Значение;
        сч = сч + 1; 
	КонецЦикла; 
	
	// === Общий итог ===     // 806
    Значение = A003 + A041 + A048 + A052;
    ЗаполнитьПоказательНаСервере("A998", Значение, Форма, "Отчет");
...
Процедура УдалитьДополнительнуюСтроку(Форма, СпециальноеДобавление = Ложь, ИмяКолонкиНомера = Неопределено, НомерПервойСтроки = 1) Экспорт
	...
	// Назначим текущей областью таблицы первую редактируемую ячейку удаленной строки,
	// если ее порядковый номер меньше кол-ва строк многострочного раздела,
	// или последней строки раздела в противном случае
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если Врег(ЛЕВ(ИмяЯчейки, 6)) = "ИННРУК" Тогда
		
		Если НЕ НаименованиеТекЛиста = "Отчет" Тогда
			Возврат;
		КонецЕсли;
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 ...
																				 "ИННРук", НаименованиеТекЛиста,
																				 10, Ложь, Неопределено));
		
		ПоказатьВводСтроки(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"), 10);
						
	ИначеЕсли ИмяЯчейки = "ZERO" Тогда
		Если ТабличныйДокумент.Области["ZERO_ZVIT"].Значение = 1 Тогда
			ТабличныйДокумент.Области["ZERO"].Значение = "";
			ТабличныйДокумент.Области["ZERO_ZVIT"].Значение = 0;
		Иначе
			ТабличныйДокумент.Области["ZERO"].Значение = "V";
			ТабличныйДокумент.Области["ZERO_ZVIT"].Значение = 1;
		КонецЕсли;
		
	ИначеЕсли Врег(ЛЕВ(ИмяЯчейки, 6)) = "ИННБУХ" Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 ...
																				 "ИННБух", НаименованиеТекЛиста,
																				 10, Ложь, Неопределено));
		
		ПоказатьВводСтроки(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"), 10);
		
	ИначеЕсли  ИмяЯчейки = "HPZ" Тогда
		
		ТабличныйДокумент.Области["HPZ"].Значение = "X";
		ТабличныйДокумент.Области["HPO"].Значение = "";
				
	ИначеЕсли  ИмяЯчейки = "HPO" Тогда
		
		ТабличныйДокумент.Области["HPZ"].Значение = "";
		ТабличныйДокумент.Области["HPO"].Значение = "X";
	...
	ИначеЕсли ИмяЯчейки = "HZU" Тогда
		...
		СтруктураДанныхСлужебныеДанные["C_DOC_STAN"] = 3;
...
Процедура ВводПоказателяВПолеТабличногоДокументЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	Если    Найти(ДополнительныеПараметры.ИмяЯчейкиКвадратов, "ИННРук") > 0 
		ИЛИ Найти(ДополнительныеПараметры.ИмяЯчейкиКвадратов, "ИННБух") > 0 Тогда
		РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(ЭтаФорма, "Отчет",  ДополнительныеПараметры.ИмяЯчейкиКвадратов, 10, Значение, ДополнительныеПараметры.ДополнитьЗначение, ДополнительныеПараметры.НольКакНоль);
	КонецЕсли;	
...
Процедура ОчиститьНаСервере() Экспорт
    ...
    //HPZ  = ПолучитьПоказательНаСервере("HPZ", ЭтаФорма,"Отчет");
    //HPO  = ПолучитьПоказательНаСервере("HPO", ЭтаФорма,"Отчет");
    //HZ   = ПолучитьПоказательНаСервере("HZ",  ЭтаФорма,"Отчет");
    //HZN  = ПолучитьПоказательНаСервере("HZN", ЭтаФорма,"Отчет");
    //HZU  = ПолучитьПоказательНаСервере("HZU" ,ЭтаФорма,"Отчет");

	СтруктураДанныхОтчет.Очистить();
    ...
    //СписокУстанавливаемыхЗначений.Добавить(HPZ,  "HPZ");
    //СписокУстанавливаемыхЗначений.Добавить(HPO,  "HPO");
    //СписокУстанавливаемыхЗначений.Добавить(HZ, 	 "HZ");
    //СписокУстанавливаемыхЗначений.Добавить(HZN,  "HZN");
    //СписокУстанавливаемыхЗначений.Добавить(HZU,  "HZU");
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Запрос = ПодготовитьЗапрос();
	Запрос2 = ПодготовитьЗапрос2();
	...
	Запрос.УстановитьПараметр("КодОперации", Перечисления.ИНАГРО_КодыОперацийХПП.Переработка);
	Запрос.УстановитьПараметр("ТаблицаПоказателей", ТаблицаПоказателей.Выгрузить());
    Запрос.УстановитьПараметр("ТаблицаКультур", ТаблицаКультур.Выгрузить());

	ЗапросПоСобственному =новый запрос;
	ЗапросПоСобственному.Текст= "ВЫБРАТЬ
	|	ИНАГРО_ДополнительныеРеквизитыКонтрагентов.Контрагент
	|ИЗ
	|	РегистрСведений.ИНАГРО_ДополнительныеРеквизитыКонтрагентов КАК ИНАГРО_ДополнительныеРеквизитыКонтрагентов" ;
	Выборка = ЗапросПоСобственному.Выполнить().Выбрать();
	Выборка.Следующий();
	СобственныйКонтрагент =выборка.контрагент;

	
	Запрос2.УстановитьПараметр("Дата", СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	Запрос2.УстановитьПараметр("ДатаНачала",СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
	Запрос2.УстановитьПараметр("Организация", СтруктураРеквизитовФормы.Организация);
	Запрос2.УстановитьПараметр("Владелец", СобственныйКонтрагент);
	Запрос2.УстановитьПараметр("КодОперации", Перечисления.ИНАГРО_КодыОперацийХПП.Переработка);
    Запрос2.УстановитьПараметр("ТаблицаПоказателей", ТаблицаПоказателей.Выгрузить());
    Запрос2.УстановитьПараметр("ТаблицаКультур", ТаблицаКультур.Выгрузить());


	
	РезультатЗапрос = Запрос.Выполнить().Выгрузить();
	
	ПокупкаЗерна = Запрос2.Выполнить().Выгрузить();
	
	КодыСтроки = РезультатЗапрос.ВыгрузитьКолонку("КодСтроки");
    ...
    Для каждого КодСтроки Из КодыСтроки  Цикл
		...
		Отбор.Вставить("КодСтроки",КодСтроки);
		
		НужнаяСтрока = РезультатЗапрос.НайтиСтроки(Отбор);
		
		ЭтаФорма.СтруктураДанныхОтчет["A" + КодСтроки + "_4"] = НужнаяСтрока[0].ПереработкаЗачетныйВес/1000; 
	...
	Для каждого КодСтроки Из КодыСтроки  Цикл
		...
		НужнаяСтрока = ПокупкаЗерна.НайтиСтроки(Отбор);
		
		ЭтаФорма.СтруктураДанныхОтчет["A" + КодСтроки + "_1"] = НужнаяСтрока[0].ВесОстатокОбщееХранение/1000;		
		ЭтаФорма.СтруктураДанныхОтчет["A" + КодСтроки + "_2"] = НужнаяСтрока[0].КоличествоЗакупленнойПродукции/1000;			 
	...
	Если ТипЗнч(ПараметрыЗаполненияЗвит1С) = Тип("Структура") Тогда
		...
		Попытка
			...
			Если Данные = Неопределено Тогда
				...
				Если глМенеджерЗвит1С.ИмяСхемыЗвит1С = ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ИмяСхемы1 Тогда
					ТекстСообщения = "Збереженого звіту ""Податкова декларація плати за землю"" ("+ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ИмяСхемы1+") за "+ПредставлениеПериода(ПараметрыЗаполненияЗвит1С.ДатПрошлогоПериода1, ПараметрыЗаполненияЗвит1С.ДатПрошлогоПериода2, "Л=uk_UA")+" не виявлено!
					                 |Автоматичне заповнення показника рядка 16!";
					глМенеджерЗвит1С.ПредупреждениеЗвит1С(ТекстСообщения, НСтр("ru='Загрузка данных из FREDO Звіт';uk='Завантаження даних з FREDO Звіт'"));
				КонецЕсли;
		...
		Исключение
			...
			Если глМенеджерЗвит1С.ФлагОтладки Тогда
				ТекстСообщения = НСтр("ru='FREDO Звіт. Исключение при получении данных из FREDO Звіт (%1, %2, %3, %4, %5)';
									  |uk='FREDO Звіт. Виняток при отриманні даних з FREDO Звіт ('%1, %2, %3, %4, %5)'");
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ИмяСхемы1,ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ТипПериода,ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.НомерПериода,ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ГодПериода,ОписаниеОшибки()));
			КонецЕсли;
		...
		Попытка
			...
			Если Данные = Неопределено Тогда
				...
				Если глМенеджерЗвит1С.ИмяСхемыЗвит1С = ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ИмяСхемы2 Тогда
					ТекстСообщения = "Податкова декларація плати за землю"" ("+ПараметрыЗаполненияЗвит1С.ПараметрыЗапроса.ИмяСхемы2+") за "+ПредставлениеПериода(ПараметрыЗаполненияЗвит1С.ДатПрошлогоПериода1, ПараметрыЗаполненияЗвит1С.ДатПрошлогоПериода2, "Л=uk_UA")+" не виявлено!
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Если СтруктураРеквизитовФормы.Организация.ЮридическоеФизическоеЛицо = Перечисления.ЮридическоеФизическоеЛицо.ЮридическоеЛицо Тогда	
		...
		НовСтрока.ИмяСхемы = "S2700510"; 
		НовСтрока.ПредставлениеСхемы =НСтр("ru = 'Отчет о поступлении зерновых и масляничных культур на переработку и хранение'; uk = 'Звіт про надходження зернових та олійних культур на перероблення та зберігання'");
		НовСтрока.НазваниеСтраницыПанели ="Отчет";
...
Функция ПодготовитьЗапрос() Экспорт 
	Запрос = Новый Запрос;
	Запрос.Текст =
        "ВЫБРАТЬ
        |	ТаблицаПоказателей.КодСтроки КАК КодСтроки,
        |	ТаблицаПоказателей.КлючСвязи КАК КлючСвязи,
        |	ТаблицаПоказателей.НаименованиеСтроки КАК НаименованиеСтроки
        ...
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ
        |	ТаблицаКультур.Культура КАК Культура,
        |	ТаблицаКультур.КлючСвязи КАК КлючСвязи
        |ПОМЕСТИТЬ ТаблицаКультур
        |ИЗ
        |	&ТаблицаКультур КАК ТаблицаКультур
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ
        |	ТаблицаКультур.Культура КАК Номенклатура,
        |	ТаблицаПоказателей.КодСтроки КАК КодСтроки,
        |	ТаблицаПоказателей.НаименованиеСтроки КАК НаименованиеСтроки
        |ПОМЕСТИТЬ ИНАГРО_СтруктураСтатистическихОтчетовСоответствия
        |ИЗ
        |	ТаблицаПоказателей КАК ТаблицаПоказателей
        |		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаКультур КАК ТаблицаКультур
        |		ПО ТаблицаПоказателей.КлючСвязи = ТаблицаКультур.КлючСвязи
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗЛИЧНЫЕ
        |	ИНАГРО_СтруктураСтатистическихОтчетовСоответствия.НаименованиеСтроки КАК НаименованиеСтроки,
        |	ВЫБОР
        |		КОГДА ИНАГРО_СтруктураСтатистическихОтчетовСоответствия.Номенклатура ССЫЛКА Справочник.Номенклатура
        |			ТОГДА ИНАГРО_СтруктураСтатистическихОтчетовСоответствия.Номенклатура
        |		ИНАЧЕ ВЫБОР
        |				КОГДА ИНАГРО_СтруктураСтатистическихОтчетовСоответствия.Номенклатура ССЫЛКА Справочник.ИНАГРО_ВидыКультур
        |					ТОГДА ИНАГРО_ДополнительныеРеквизитыНоменклатуры.Номенклатура
        |				ИНАЧЕ 0
        |			КОНЕЦ
        |	КОНЕЦ КАК Номенклатура,
        |	ИНАГРО_СтруктураСтатистическихОтчетовСоответствия.КодСтроки КАК КодСтроки
        |ПОМЕСТИТЬ СписокНоменклатуры
        |ИЗ
        |	ИНАГРО_СтруктураСтатистическихОтчетовСоответствия КАК ИНАГРО_СтруктураСтатистическихОтчетовСоответствия
        ...
        |		ПО ИНАГРО_СтруктураСтатистическихОтчетовСоответствия.Номенклатура = ИНАГРО_ДополнительныеРеквизитыНоменклатуры.ВидКультуры
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	СписокНоменклатуры.НаименованиеСтроки КАК НаименованиеСтроки,
        |	СписокНоменклатуры.Номенклатура КАК Номенклатура,
        |	СписокНоменклатуры.КодСтроки КАК КодСтроки,
        |	СУММА(Переработка.Количество) КАК ПереработкаКоличество,
        |	СУММА(Переработка.ЗачетныйВес) КАК ПереработкаЗачетныйВес,
        |	Переработка.КодОперации КАК КодОперации
        |ПОМЕСТИТЬ ВТ_Переработка
        |ИЗ
        |	СписокНоменклатуры КАК СписокНоменклатуры
        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИНАГРО_Остатки КАК Переработка
        |		ПО СписокНоменклатуры.Номенклатура = Переработка.Номенклатура
        |ГДЕ
        |	Переработка.Период >= &ДатаНачала
        |	И Переработка.Период <= &Дата
        |	И Переработка.Организация = &Организация
        |	И Переработка.Владелец <> &Владелец
        |	И Переработка.КодОперации = &КодОперации
        |	И ВЫБОР
        |			КОГДА Переработка.Регистратор ССЫЛКА Документ.ИНАГРО_Переработка
        |					ИЛИ Переработка.Регистратор ССЫЛКА Документ.ИНАГРО_АктФасовки
        |				ТОГДА Переработка.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
        |			ИНАЧЕ ИСТИНА
        |		КОНЕЦ
        |
        |СГРУППИРОВАТЬ ПО
        |	СписокНоменклатуры.НаименованиеСтроки,
        |	СписокНоменклатуры.КодСтроки,
        |	Переработка.КодОперации,
        ...
        |	СписокНоменклатуры.НаименованиеСтроки КАК НаименованиеСтроки,
        |	СписокНоменклатуры.КодСтроки КАК КодСтроки,
        |	СУММА(ЕСТЬNULL(ИНАГРО_ОстаткиОстаткиИОбороты.ЗачетныйВесПриход, 0)) КАК ЗачетныйВесПриход,
        |	СУММА(ЕСТЬNULL(ИНАГРО_ОстаткиОстаткиИОбороты.ЗачетныйВесРасход, 0)) КАК ЗачетныйВесРасход,
        |	СУММА(0) КАК СтоимостьКупленнойПродукции,
        |	СУММА(0) КАК КоличествоЗакупленнойПродукции,
        |	СУММА(ЕСТЬNULL(ИНАГРО_ОстаткиОстаткиИОбороты.ВесПриход, 0)) КАК ВесПриход,
        |	ИНАГРО_ОстаткиОстаткиИОбороты.Номенклатура КАК Номенклатура,
        |	СУММА(ИНАГРО_ОстаткиОстаткиИОбороты.ЗачетныйВесНачальныйОстаток) КАК ЗачетныйВесНачальныйОстаток,
        |	СУММА(0) КАК ЗачетныйВесКонечныйОстаток
        |ПОМЕСТИТЬ ВТ_Обороты_Закупка
        |ИЗ
        |	СписокНоменклатуры КАК СписокНоменклатуры
        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИНАГРО_Остатки.ОстаткиИОбороты(&ДатаНачала, &Дата, Авто, , Организация = &Организация) КАК ИНАГРО_ОстаткиОстаткиИОбороты
        |		ПО СписокНоменклатуры.Номенклатура = ИНАГРО_ОстаткиОстаткиИОбороты.Номенклатура
        |ГДЕ
        |	ИНАГРО_ОстаткиОстаткиИОбороты.Владелец <> &Владелец
        ...
        |	СУММА(0),
        |	СУММА(0),
        |	СУММА(ЕСТЬNULL(ИНАГРО_ЗакупкиОрганизацийОбороты.СтоимостьОборот, 0) - ЕСТЬNULL(ИНАГРО_ЗакупкиОрганизацийОбороты.СуммаНДСОборот, 0)),
        |	СУММА(0),
        |	СУММА(0),
        |	СписокНоменклатуры.Номенклатура,
        |	СУММА(0),
        |	СУММА(ЕСТЬNULL(ИНАГРО_ОстаткиОстаткиИОбороты.ЗачетныйВесКонечныйОстаток, 0))
        |ИЗ
        |	СписокНоменклатуры КАК СписокНоменклатуры
        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИНАГРО_ЗакупкиОрганизаций.Обороты(&ДатаНачала, &Дата, Регистратор, Организация = &Организация) КАК ИНАГРО_ЗакупкиОрганизацийОбороты
        |		ПО СписокНоменклатуры.Номенклатура = ИНАГРО_ЗакупкиОрганизацийОбороты.Номенклатура
        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИНАГРО_Остатки.ОстаткиИОбороты(&ДатаНачала, &Дата, Авто, , Организация = &Организация) КАК ИНАГРО_ОстаткиОстаткиИОбороты
        |		ПО СписокНоменклатуры.Номенклатура = ИНАГРО_ОстаткиОстаткиИОбороты.Номенклатура
        |ГДЕ
        |	ИНАГРО_ОстаткиОстаткиИОбороты.Владелец <> &Владелец
        ...
        |	СУММА(0),
        |	СУММА(0),
        |	СУММА(ЕСТЬNULL(ИНАГРО_КонтрольВвозаСобственногоЗернаОбороты.ЗачетныйВесОборот, 0)),
        |	СУММА(0),
        |	ИНАГРО_КонтрольВвозаСобственногоЗернаОбороты.Номенклатура,
        |	СУММА(0),
        |	СУММА(0)
        |ИЗ
        |	СписокНоменклатуры КАК СписокНоменклатуры
        |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИНАГРО_КонтрольВвозаСобственногоЗерна.Обороты(&ДатаНачала, &Дата, Регистратор, Организация = &Организация) КАК ИНАГРО_КонтрольВвозаСобственногоЗернаОбороты
        |		ПО СписокНоменклатуры.Номенклатура = ИНАГРО_КонтрольВвозаСобственногоЗернаОбороты.Номенклатура
        |ГДЕ
        |	ИНАГРО_КонтрольВвозаСобственногоЗернаОбороты.Владелец <> &Владелец
        |
        |СГРУППИРОВАТЬ ПО
        |	СписокНоменклатуры.НаименованиеСтроки,
        |	СписокНоменклатуры.КодСтроки,
        ...
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ
        |	ВТ_Обороты_Закупка.НаименованиеСтроки КАК НаименованиеСтроки,
        |	ВТ_Обороты_Закупка.КодСтроки КАК КодСтроки,
        |	СУММА(ВТ_Обороты_Закупка.ЗачетныйВесПриход) КАК ЗачетныйВесПриход,
        |	СУММА(ВТ_Обороты_Закупка.ЗачетныйВесРасход) КАК ЗачетныйВесРасход,
        |	СУММА(ВТ_Обороты_Закупка.СтоимостьКупленнойПродукции) КАК СтоимостьКупленнойПродукции,
        |	СУММА(ВТ_Обороты_Закупка.КоличествоЗакупленнойПродукции) КАК КоличествоЗакупленнойПродукции,
        |	СУММА(ВТ_Обороты_Закупка.ВесПриход) КАК ВесПриход,
        |	ВТ_Обороты_Закупка.Номенклатура КАК Номенклатура,
        |	СУММА(ВТ_Обороты_Закупка.ЗачетныйВесНачальныйОстаток) КАК ЗачетныйВесНачальныйОстаток,
        |	СУММА(ВТ_Обороты_Закупка.ЗачетныйВесКонечныйОстаток) КАК ЗачетныйВесКонечныйОстаток
        |ПОМЕСТИТЬ ВТ_ЗакупкиСгрупированая
        |ИЗ
        |	ВТ_Обороты_Закупка КАК ВТ_Обороты_Закупка
        |
        |СГРУППИРОВАТЬ ПО
        |	ВТ_Обороты_Закупка.НаименованиеСтроки,
        |	ВТ_Обороты_Закупка.КодСтроки,
        |	ВТ_Обороты_Закупка.Номенклатура
        |;
        |
        |////////////////////////////////////////////////////////////////////////////////
        |ВЫБРАТЬ РАЗРЕШЕННЫЕ
        |	СписокНоменклатуры.НаименованиеСтроки КАК НаименованиеСтроки,
        |	СписокНоменклатуры.КодСтроки КАК КодСтроки,
        |	ЕСТЬNULL(ВТ_Переработка.ПереработкаЗачетныйВес, 0) КАК ПереработкаЗачетныйВес,
        |	ЕСТЬNULL(ВТ_Переработка.ПереработкаКоличество, 0) КАК ПереработкаКоличество,
        |	ЕСТЬNULL(ВТ_ЗакупкиСгрупированая.ЗачетныйВесНачальныйОстаток, 0) КАК ЗачетныйВесНачальныйОстаток,
        |	ЕСТЬNULL(ВТ_ЗакупкиСгрупированая.ЗачетныйВесКонечныйОстаток, 0) КАК ЗачетныйВесКонечныйОстаток,
        |	ЕСТЬNULL(ВТ_ЗакупкиСгрупированая.ЗачетныйВесПриход, 0) КАК ЗачетныйВесПриход,
        |	ЕСТЬNULL(ВТ_ЗакупкиСгрупированая.ЗачетныйВесРасход, 0) КАК ЗачетныйВесРасход,
        |	ЕСТЬNULL(ВТ_ЗакупкиСгрупированая.СтоимостьКупленнойПродукции, 0) КАК СтоимостьКупленнойПродукции,
        |	ЕСТЬNULL(ВТ_ЗакупкиСгрупированая.КоличествоЗакупленнойПродукции, 0) КАК КоличествоЗакупленнойПродукции
        |ИЗ
        |	СписокНоменклатуры КАК СписокНоменклатуры
        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_Переработка КАК ВТ_Переработка
        |		ПО СписокНоменклатуры.Номенклатура = ВТ_Переработка.Номенклатура
        |			И (ВТ_Переработка.КодСтроки = СписокНоменклатуры.КодСтроки)
        |		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ЗакупкиСгрупированая КАК ВТ_ЗакупкиСгрупированая
        |		ПО СписокНоменклатуры.Номенклатура = ВТ_ЗакупкиСгрупированая.Номенклатура
        |			И (ВТ_ЗакупкиСгрупированая.КодСтроки = СписокНоменклатуры.КодСтроки)";
...
Функция ПодготовитьЗапрос2()
	...
	Запрос.Текст = "ВЫБРАТЬ
                   ...
                   |    СписокНоменклатуры.Номенклатура КАК Номенклатура,
                   |    СписокНоменклатуры.КодСтроки КАК КодСтроки,
                   |    СУММА(ИНАГРО_ОстаткиОстаткиИОбороты.ЗачетныйВесПриход) КАК ВесПриход,
                   |    ИНАГРО_ОстаткиОстаткиИОбороты.ЗачетныйВесКонечныйОстаток КАК ВесКонечныйОстаток,
                   |    ИНАГРО_ОстаткиОстаткиИОбороты.ВидХранения КАК ВидХранения,
                   |    ИНАГРО_ОстаткиОстаткиИОбороты.Владелец КАК Владелец
                   |ПОМЕСТИТЬ ОстаткиИОбороты
                   |ИЗ
                   |    СписокНоменклатуры КАК СписокНоменклатуры
                   |        ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИНАГРО_Остатки.ОстаткиИОбороты(&ДатаНачала, &Дата, , , Организация = &Организация) КАК ИНАГРО_ОстаткиОстаткиИОбороты
                   |        ПО СписокНоменклатуры.Номенклатура = ИНАГРО_ОстаткиОстаткиИОбороты.Номенклатура
                   |
                   |СГРУППИРОВАТЬ ПО
                   |    СписокНоменклатуры.Номенклатура,
                   |    СписокНоменклатуры.НаименованиеСтроки,
                   |    СписокНоменклатуры.КодСтроки,
                   |    ИНАГРО_ОстаткиОстаткиИОбороты.ЗачетныйВесКонечныйОстаток,
                   ...
                   |////////////////////////////////////////////////////////////////////////////////
                   |ВЫБРАТЬ
                   |    ВложенныйЗапрос.НаименованиеСтроки КАК НаименованиеСтроки,
                   |    ВложенныйЗапрос.Номенклатура КАК Номенклатура,
                   |    ВложенныйЗапрос.КодСтроки КАК КодСтроки,
                   |    ВложенныйЗапрос.ВесОстатокВсего КАК ВесОстатокВсего,
                   |    ВложенныйЗапрос.Владелец КАК Владелец,
                   |    ВложенныйЗапрос.ВидХранения КАК ВидХранения,
                   |    ВложенныйЗапрос.ВесОстатокЧужое КАК ВесОстатокЧужое,
                   |    ВложенныйЗапрос.Регистратор КАК Регистратор
                   |ПОМЕСТИТЬ ОБоротБезПереоформления
                   |ИЗ
                   |    (ВЫБРАТЬ
                   |        СписокНоменклатуры.НаименованиеСтроки КАК НаименованиеСтроки,
                   |        СписокНоменклатуры.Номенклатура КАК Номенклатура,
                   |        СписокНоменклатуры.КодСтроки КАК КодСтроки,
                   |        СУММА(ИНАГРО_ОстаткиОбороты.ЗачетныйВесПриход) КАК ВесОстатокВсего,
                   |        0 КАК ВесОстатокЧужое,
                   |        ИНАГРО_ОстаткиОбороты.Владелец КАК Владелец,
                   |        ИНАГРО_ОстаткиОбороты.ВидХранения КАК ВидХранения,
                   |        ИНАГРО_ОстаткиОбороты.Регистратор КАК Регистратор
                   |    ИЗ
                   |        СписокНоменклатуры КАК СписокНоменклатуры
                   |            ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ИНАГРО_Остатки.Обороты(&ДатаНачала, &Дата, Регистратор, Организация = &Организация) КАК ИНАГРО_ОстаткиОбороты
                   |            ПО СписокНоменклатуры.Номенклатура = ИНАГРО_ОстаткиОбороты.Номенклатура
                   |    ГДЕ
                   |        НЕ ИНАГРО_ОстаткиОбороты.Регистратор ССЫЛКА Документ.ИНАГРО_Переоформление
                   |    
                   |    СГРУППИРОВАТЬ ПО
                   |        СписокНоменклатуры.НаименованиеСтроки,
                   |        СписокНоменклатуры.КодСтроки,
                   |        СписокНоменклатуры.Номенклатура,
                   |        ИНАГРО_ОстаткиОбороты.Владелец,
                   |        ИНАГРО_ОстаткиОбороты.ВидХранения,
                   |        ИНАГРО_ОстаткиОбороты.Регистратор
                   |    
                   |    ОБЪЕДИНИТЬ ВСЕ
                   |    
                   |    ВЫБРАТЬ
                   ...
                   |        СписокНоменклатуры.КодСтроки,
                   |        0,
                   |        СУММА(ИНАГРО_ОстаткиОбороты.ЗачетныйВесПриход),
                   |        ИНАГРО_ОстаткиОбороты.Владелец,
                   |        ИНАГРО_ОстаткиОбороты.ВидХранения,
                   |        ИНАГРО_ОстаткиОбороты.Регистратор
                   |    ИЗ
                   ...
                   |    ГДЕ
                   |        НЕ ИНАГРО_ОстаткиОбороты.Регистратор ССЫЛКА Документ.ИНАГРО_Переоформление
                   |        И ИНАГРО_ОстаткиОбороты.Владелец <> &владелец
                   |    
                   |    СГРУППИРОВАТЬ ПО
                   |        СписокНоменклатуры.НаименованиеСтроки,
                   |        СписокНоменклатуры.КодСтроки,
                   ...
                   |        ИНАГРО_ОстаткиОбороты.Владелец,
                   |        ИНАГРО_ОстаткиОбороты.ВидХранения,
                   |        ИНАГРО_ОстаткиОбороты.Регистратор) КАК ВложенныйЗапрос
                   |;
                   |
                   |////////////////////////////////////////////////////////////////////////////////
                   |ВЫБРАТЬ
                   |    ВложенныйЗапрос.НаименованиеСтроки КАК НаименованиеСтроки,
                   |    ВложенныйЗапрос.КодСтроки КАК КодСтроки,
                   |    СУММА(ВложенныйЗапрос.ВесОстатокОбщееХранение) КАК ВесОстатокОбщееХранение,
                   |    СУММА(ВложенныйЗапрос.ВесОстатокВсего) КАК ВесОстатокВсего
                   |ПОМЕСТИТЬ ОстаткиНаНачало
                   |ИЗ
                   |    (ВЫБРАТЬ
                   |        СписокНоменклатуры.НаименованиеСтроки КАК НаименованиеСтроки,
                   |        СписокНоменклатуры.КодСтроки КАК КодСтроки,
                   |        СУММА(ОстаткиИОбороты.ВесКонечныйОстаток) КАК ВесОстатокОбщееХранение,
                   |        СУММА(0) КАК ВесОстатокВсего
                   |    ИЗ
                   |        СписокНоменклатуры КАК СписокНоменклатуры
                   |            ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиИОбороты КАК ОстаткиИОбороты
                   |            ПО СписокНоменклатуры.НаименованиеСтроки = ОстаткиИОбороты.НаименованиеСтроки
                   |                И СписокНоменклатуры.Номенклатура = ОстаткиИОбороты.Номенклатура
                   |                И СписокНоменклатуры.КодСтроки = ОстаткиИОбороты.КодСтроки
                   |    
                   |    СГРУППИРОВАТЬ ПО
                   |        СписокНоменклатуры.КодСтроки,
                   |        СписокНоменклатуры.НаименованиеСтроки
                   ...
                   |        СписокНоменклатуры.КодСтроки,
                   |        СУММА(0),
                   |        ОстаткиИОбороты.ВесКонечныйОстаток
                   |    ИЗ
                   |        СписокНоменклатуры КАК СписокНоменклатуры
                   |            ЛЕВОЕ СОЕДИНЕНИЕ ОстаткиИОбороты КАК ОстаткиИОбороты
                   |            ПО СписокНоменклатуры.НаименованиеСтроки = ОстаткиИОбороты.НаименованиеСтроки
                   ...
                   |        СписокНоменклатуры.НаименованиеСтроки,
                   |        СписокНоменклатуры.КодСтроки,
                   |        ОстаткиИОбороты.ВесКонечныйОстаток) КАК ВложенныйЗапрос
                   |
                   |СГРУППИРОВАТЬ ПО
                   |    ВложенныйЗапрос.НаименованиеСтроки,
                   ...
                   |    ВложенныйЗапрос.НаименованиеСтроки КАК НаименованиеСтроки,
                   |    ВложенныйЗапрос.КодСтроки КАК КодСтроки,
                   |    СУММА(ВложенныйЗапрос.ВесПриходВсего) КАК ВесПриходВсего,
                   |    СУММА(ВложенныйЗапрос.ВесПриходЧужое) КАК ВесПриходЧужое,
                   |    СУММА(ВложенныйЗапрос.ВесПриходСвое) КАК ВесПриходСвое,
                   ...
                   |        СписокНоменклатуры.НаименованиеСтроки КАК НаименованиеСтроки,
                   |        СписокНоменклатуры.КодСтроки КАК КодСтроки,
                   |        ОБоротБезПереоформления.ВесОстатокВсего КАК ВесПриходВсего,
                   |        СУММА(0) КАК ВесПриходЧужое,
                   |        0 КАК ВесПриходСвое,
                   |        0 КАК СтоимостьПриход,
                   |        0 КАК КоличествоПриход
                   |    ИЗ
                   |        СписокНоменклатуры КАК СписокНоменклатуры
                   |            ЛЕВОЕ СОЕДИНЕНИЕ ОБоротБезПереоформления КАК ОБоротБезПереоформления
                   |            ПО СписокНоменклатуры.Номенклатура = ОБоротБезПереоформления.Номенклатура
                   |                И СписокНоменклатуры.НаименованиеСтроки = ОБоротБезПереоформления.НаименованиеСтроки
                   |                И СписокНоменклатуры.КодСтроки = ОБоротБезПереоформления.КодСтроки
                   |    
                   |    СГРУППИРОВАТЬ ПО
                   |        СписокНоменклатуры.КодСтроки,
                   |        СписокНоменклатуры.НаименованиеСтроки,
                   |        ОБоротБезПереоформления.ВесОстатокВсего
                   |    
                   |    ОБЪЕДИНИТЬ ВСЕ
                   |    
                   |    ВЫБРАТЬ
                   ...
                   |        СписокНоменклатуры.КодСтроки,
                   |        СУММА(0),
                   |        ОБоротБезПереоформления.ВесОстатокЧужое,
                   |        0,
                   |        0,
                   |        0
                   |    ИЗ
                   ...
                   |                И СписокНоменклатуры.КодСтроки = ОБоротБезПереоформления.КодСтроки
                   |    ГДЕ
                   |        ОБоротБезПереоформления.Владелец <> &Владелец
                   |    
                   |    СГРУППИРОВАТЬ ПО
                   |        СписокНоменклатуры.НаименованиеСтроки,
                   |        СписокНоменклатуры.КодСтроки,
                   ...
                   |    ГДЕ
                   |        ОБоротБезПереоформления.Владелец = &Владелец
                   |        И НЕ(ОБоротБезПереоформления.Регистратор ССЫЛКА Документ.ИНАГРО_ТТНВнутр
                   |                    ИЛИ ОБоротБезПереоформления.Регистратор ССЫЛКА Документ.ИНАГРО_РеестрТТНВнутр
                   |                    ИЛИ ОБоротБезПереоформления.Регистратор ССЫЛКА Документ.ИНАГРО_Перемещение)
                   |    
                   |    СГРУППИРОВАТЬ ПО
                   |        СписокНоменклатуры.КодСтроки,
                   |        СписокНоменклатуры.НаименованиеСтроки,
                   |        ОБоротБезПереоформления.ВесОстатокВсего) КАК ВложенныйЗапрос
                   |
                   |СГРУППИРОВАТЬ ПО
                   |    ВложенныйЗапрос.НаименованиеСтроки,
                   |    ВложенныйЗапрос.КодСтроки,
                   ...
                   |ИЗ
                   |    (ВЫБРАТЬ
                   |        ОстаткиНаНачало.НаименованиеСтроки КАК НаименованиеСтроки,
                   |        ОстаткиНаНачало.КодСтроки КАК КодСтроки,
                   |        ОстаткиНаНачало.ВесОстатокОбщееХранение КАК ВесОстатокОбщееХранение,
                   |        ОстаткиНаНачало.ВесОстатокВсего КАК ВесОстатокВсего,
                   |        0 КАК ВесПриходВсего,
                   |        0 КАК ВесПриходЧужое,
                   |        0 КАК ВесПриходСвое,
                   |        0 КАК СтоимостьПриход
                   |    ИЗ
                   ...
                   |    
                   |    ВЫБРАТЬ
                   |        Приходы.НаименованиеСтроки,
                   |        Приходы.КодСтроки,
                   |        0,
                   |        0,
                   |        Приходы.ВесПриходВсего,
                   |        Приходы.ВесПриходЧужое,
                   |        Приходы.ВесПриходСвое,
                   |        Приходы.СтоимостьПриход
                   |    ИЗ
                   |        Приходы КАК Приходы) КАК ВложенныйЗапрос
                   |
                   |СГРУППИРОВАТЬ ПО
                   |    ВложенныйЗапрос.НаименованиеСтроки,
                   |    ВложенныйЗапрос.КодСтроки
                   ...
                   |////////////////////////////////////////////////////////////////////////////////
                   |ВЫБРАТЬ
                   |    ВЫБОРКА.НаименованиеСтроки КАК НаименованиеСтроки,
                   |    ВЫБОРКА.КодСтроки КАК КодСтроки,
                   |    СУММА(ВЫБОРКА.ВесОстатокОбщееХранение) КАК ВесОстатокОбщееХранение,
                   |    СУММА(ВЫБОРКА.ВесОстатокВсего) КАК ЗачетныйВесКонечныйОстаток,
                   |    СУММА(ВЫБОРКА.ВесПриходВсего) КАК ВесПриходВсего,
                   |    СУММА(ВЫБОРКА.ВесПриходЧужое) КАК ВесПриходЧужое,
                   |    СУММА(ВЫБОРКА.ВесПриходСвое) КАК КоличествоЗакупленнойПродукции
                   |ИЗ
                   |    Результат КАК ВЫБОРКА
                   |
                   |СГРУППИРОВАТЬ ПО
                   |    ВЫБОРКА.КодСтроки,
                   |    ВЫБОРКА.НаименованиеСтроки";
    Возврат Запрос;	
...
&НаКлиенте
Процедура НастройкаПоказателей(Команда)
    Оповещение = Новый ОписаниеОповещения("НастройкаПоказателейЗакрытие",ЭтаФорма);
    ОткрытьФорму("Отчет.ИНАГРО_РегламентированныйОтчет1Зерно.Форма.ФормаНастройкиСоответсвий2022",,ЭтаФорма,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
    
КонецПроцедуры

&НаКлиенте
Процедура НастройкаПоказателейЗакрытие(Результат,ДопПараметры)Экспорт
...
&НаСервере
Процедура ОбновитьДанныеПоказателейНаСервере()
    УстановитьПривилегированныйРежим(Истина);
    НастройкиПоказателей = ХранилищеОбщихНастроек.Загрузить("ИНАГРО_НастройкиПоказателей1Зерно",,,"Пользователь1Зерно");
    УстановитьПривилегированныйРежим(Ложь);
    Если НЕ НастройкиПоказателей = Неопределено Тогда
        ТаблицаКультур.Загрузить(НастройкиПоказателей.ТаблицаКультур);
        ТаблицаПоказателей.Загрузить(НастройкиПоказателей.ТаблицаПоказателей);
    КонецЕсли;	
