...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	// В обход типового варианта
	ТелефонОрг  = Сведения.Телефон;
	ФаксОрг 	= Сведения.Факс;
	
    //КодГорода = ТелефонОрг.КодГорода;
    ...
    ЗаполнитьПоказательНаСервере("EMAIL_V",         EMAIL_V, 	Форма, "Отчет");
    ЗаполнитьПоказательНаСервере("TEL_V",           ТелефонОрг, Форма, "Отчет");	
    ЗаполнитьПоказательНаСервере("FAX_V",           ФаксОрг, Форма, "Отчет");
    ЗаполнитьПоказательНаСервере("FIRM_RUK",        ФИОДиректора, Форма, "Отчет");
    ЗаполнитьПоказательНаСервере("VIK",             ФИОГлавБуха, 	Форма, "Отчет");
	...
	ЗаполнитьПоказательНаСервере("FIRM_SPATO", 	КодКОАТУУ, 	Форма, "СлужебныеДанные");
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если НЕ(Лев(ОбластьИмя, 4) = "A003" ИЛИ Лев(ОбластьИмя, 4) = "A004" ИЛИ Лев(ОбластьИмя, 4) = "A806" ИЛИ Лев(ОбластьИмя, 4) = "A011"
	    ...
	    сч = 1;                   // 015
	    Пока сч < 8 Цикл 		
	        Значение = ПолучитьПоказательНаСервере("A823_" + Сч, Форма, "Отчет")+ ПолучитьПоказательНаСервере("A824_" + Сч, Форма, "Отчет");
...
Процедура НастройкаПоказателей(Команда)
    ...
    ОткрытьФорму("Отчет.ИНАГРО_РегламентированныйОтчет1Зерно.Форма.ФормаНастройкиСоответсвий2021",,ЭтаФорма,,,,Оповещение,РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
