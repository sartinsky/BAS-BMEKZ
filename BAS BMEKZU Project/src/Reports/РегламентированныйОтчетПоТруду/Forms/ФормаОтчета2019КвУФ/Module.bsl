...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2019Кв";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "01/04/2019";
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	РасчетКоличестваИФОПКатегорийРаботников(МенеджерВременныхТаблиц); // ИНАГРО++ 
...
Процедура ВосстановитьГраницыРаспределенияРаздел3()
	...
	ЗаполнитьПоказательНаСервере("Пр3", 4500.01, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр4", 5000.01, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр5", 6000.01, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр6", 7000.01, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр7", 8000.01, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр8", 10000.01, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр9", 12000.01, ЭтаФорма, "ОтчетПоТруду");
	...
	ЗаполнитьПоказательНаСервере("Пр200", 4500.0, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр300", 5000.0, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр400", 6000.0, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр500", 7000.0, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр600", 8000.0, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр700", 10000.0, ЭтаФорма, "ОтчетПоТруду");
	ЗаполнитьПоказательНаСервере("Пр800", 12000.0, ЭтаФорма, "ОтчетПоТруду");
...
Процедура РасчетНачисленийШтатныхРаботников(МенеджерВременныхТаблиц)
	...
	Иначе	
		...
		ИмяРегистраРасчета = "ИНАГРО_Начисления";
		//ИНАГРО --
		...
		ЗапросОбшийФОТ.Текст = СтрЗаменить(ЗапросОбшийФОТ.Текст, "//ЕстьПериодДействия","");	
...
Процедура РаспределениеРаботниковПоРазмеруЗП(МенеджерВременныхТаблиц)
	...
	Иначе	
		...
		|	СУММА(Начисления.Результат) КАК Результат
		|ИЗ
		|	РегистрСведений.ИНАГРО_РаботникиОрганизаций.СрезПоследних(&Месяц,Организация = &Организация И Сотрудник.ИНАГРО_ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)) КАК РаботникиОрганизаций
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрРасчета."+ИмяРегистра+" КАК Начисления
		|	По Начисления.Сотрудник = РаботникиОрганизаций.Сотрудник
		...
		|			 СУММА(ОтработаноЧасов) КАК Отработано, 
		|			 СУММА(НормаЧасов) КАК Норма 
		|	 ИЗ РегистрРасчета."+ ИмяРегистра +" КАК Начисления
		|	 ГДЕ Организация = &Организация
		|//НетПериодДействия		И Начисления.ДатаНачалаСобытия = &НачДата 
		|//ЕстьПериодДействия		И Начисления.ПериодДействия    = &НачДата 
		|	 СГРУППИРОВАТЬ ПО Сотрудник) КАК Отработка
		|  ПО	Отработка.Сотрудник = РаботникиОрганизаций.Сотрудник
		|
		|ГДЕ
		|	  Начисления.ПериодРегистрации = &Месяц
		|	И Начисления.Организация = &Организация
		...
		//ИНАГРО ++
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//ЕстьПериодДействия","");	
		Запрос.УстановитьПараметр("НачДата", НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));
		//ИНАГРО --
	КонецЕсли;
	...
	Иначе
		...
		УсловиеЭтоОклад = "(ВидНачисления = ЗНАЧЕНИЕ(Перечисление.ИНАГРО_ВидыНачисленийРаботникаОрганизации.Основное))";
		//Размер = "Размер";
		Размер  = "Показатель1";
		//ВидРасчета = "Начисление";
		ВидРасчета  = "ВидРасчета";
		//УсловиеАктуальность = "Размер > 0";
		УсловиеАктуальность  = "Действует И ";
		//ТекстГрафикРаботы = "NULL"; 
		//ТекстЗапросаГрафик = "";
		УсловиеСоединения = "(ГрафикиРаботы.Сотрудник = Начисления.Сотрудник)";
		ТекстГрафикРаботы = "РаботникиОрганизаций.ГрафикРаботы"; 
		...
		|ГДЕ
		|	ГрафикиРаботы.ГрафикРаботы В (ВЫБРАТЬ РаботникиОрганизаций.ГрафикРаботы ИЗ ВТРаботникиОрганизаций КАК РаботникиОрганизаций)
		|	И КОНЕЦПЕРИОДА(ГрафикиРаботы.Дата, МЕСЯЦ) = &ДатаНач
	...
	Запрос.УстановитьПараметр("ПустоеЗначениеВР",ПланыВидовРасчета.ИНАГРО_Начисления.ПустаяСсылка());
	...
	|				И " + УсловиеАктуальность + "
	|				"+ УсловиеЭтоОклад+ "
	|				И Сотрудник.ИНАГРО_ВидЗанятости = ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)) КАК Начисления
	|               " + РазмерыТарифныхСтавок + "
	|ИНДЕКСИРОВАТЬ ПО
	|	Сотрудник
	|;
	...
	|		ПО РаботникиОрганизаций.ГрафикРаботы = ГрафикиРаботы.ГрафикРаботы
	|ГДЕ
	|	
	|	(ВЫБОР
	|		КОГДА Начисления.СпособУчетаВремени = ""ЗА МЕСЯЦ""
	|			ТОГДА Начисления.ОкладТариф
	|		КОГДА Начисления.СпособУчетаВремени = ""ЗА ДЕНЬ""
	|				И ГрафикиРаботы.НормаДнейЗаМесяц > 0
	|			ТОГДА Начисления.ОкладТариф * ГрафикиРаботы.НормаДнейЗаМесяц
	|		ИНАЧЕ Начисления.ОкладТариф * ГрафикиРаботы.НормаЧасовЗаМесяц
	|	КОНЕЦ) <= &ПрожиточныйМинимум	               
	|СГРУППИРОВАТЬ ПО
	|	Начисления.Сотрудник"; 
...
Процедура РасчетКоличестваИФОПКатегорийРаботников(МенеджерВременныхТаблиц)	
	...
	СформироватьМассивыРазличныхКатегорийРаботников(МенеджерВременныхТаблиц); // ИНАГРО
