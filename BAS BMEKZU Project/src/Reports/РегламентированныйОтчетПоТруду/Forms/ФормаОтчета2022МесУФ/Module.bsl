...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2022Мес";
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли ВРЕГ(Лев(ИмяЯчейки, 7))  = "КАТОТТГ" Тогда
		...
		ЗаполнитьПоказательНаСервере("КАТОТТГ1", Сред(Значение, 3), ЭтаФорма, "ОтчетПоТрудуМес");
	...
	ИначеЕсли ВРЕГ(ИмяЯчейки) = "H001" Тогда
		...
		Иначе
		    ...
		    //очищаем таблицу
			ТабличныйДокумент.Области["П1020_1"].Значение = 0;
			ТабличныйДокумент.Области["П1030_1"].Значение = 0;
			ТабличныйДокумент.Области["П1040_1"].Значение = 0;
			ТабличныйДокумент.Области["П1060_1"].Значение = 0;
			ТабличныйДокумент.Области["П1070_1"].Значение = 0;
			ТабличныйДокумент.Области["ССЧВСравнении"].Значение = "";
			ТабличныйДокумент.Области["СредняяЗПВСравнении"].Значение = "";
		КонецЕсли;		
	КонецЕсли;		
	Модифицированность = Истина;
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма, ИзмененаДатаПодписи = Ложь)
	...
	ЗаполнитьПоказательНаСервере("EMAIL", EMAIL, Форма, "ОтчетПоТрудуМес");

	ЗаполнитьПоказательНаСервере("Телефон", Телефон, Форма, "ОтчетПоТрудуМес");
	
	Если ЗначениеЗаполнено(КодКАТОТТГ) Тогда
		ЗаполнитьПоказательНаСервере("КАТОТТГ", КодКАТОТТГ, Форма, "ОтчетПоТрудуМес");
		ЗаполнитьПоказательНаСервере("КАТОТТГ1", Сред(КодКАТОТТГ, 3), Форма, "ОтчетПоТрудуМес");
		ЗаполнитьПоказательНаСервере("КОАТУУ", "", Форма, "ОтчетПоТрудуМес");
		КодКОАТУУ = "";
	Иначе	
		ЗаполнитьПоказательНаСервере("КОАТУУ", КодКОАТУУ, Форма, "ОтчетПоТрудуМес");
		ЗаполнитьПоказательНаСервере("КАТОТТГ", "", Форма, "ОтчетПоТрудуМес");
		ЗаполнитьПоказательНаСервере("КАТОТТГ1", "", Форма, "ОтчетПоТрудуМес");
	КонецЕсли;
	
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетПоТрудуМес", "КАТОТТГ"+"_",  19, КодКАТОТТГ, Ложь);		
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетПоТрудуМес", "КОАТУУ"+"_",  10, КодКОАТУУ, Ложь);		
	
	ЗаполнитьПоказательНаСервере("KVED", КодКВЕД, Форма, "ОтчетПоТрудуМес");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ОтчетПоТрудуМес", "KVED",  5, КодКВЕД, Ложь);		
	...
	//Период расчета задолженности равен кварталу
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	//ИНАГРО --
		...
		Запрос.УстановитьПараметр("НачалоПериода", СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета);
		Запрос.УстановитьПараметр("КонецПериода" , СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
		//Запрос.УстановитьПараметр("Месяц",НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));   // ИНАГРО
		Запрос.УстановитьПараметр("парамОрганизация" , СтруктураРеквизитовФормы.Организация);
		Запрос.УстановитьПараметр("НеФОТ",  	   Справочники.СтатьиНалоговыхДеклараций.ОтчетПоТруду_НеФОТ);
		//Запрос.МенеджерВременныхТаблиц = МенеджерВременныхТаблиц;
		//Запрос.Выполнить();
	...
	//ЗапросРабочееВремя.УстановитьПараметр("Месяц",НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета)); // ИНАГРО
