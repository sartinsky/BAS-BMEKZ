...
Процедура ВывестиОрганизациюП6(СтрокаОрганизации, ДокументРезультат, Макет, СоответствиеПользовательскихПолей)
	...
	Если ЗначениеЗаполнено(Руководители.ГлавныйБухгалтерФИО) Тогда
		ОбластьОтчета.Параметры.ФИОГлавногоБухгалтера = ФизическиеЛицаКлиентСервер.ИмяФамилияВФорматеДСТУ(Руководители.ГлавныйБухгалтерФИО);
	КонецЕсли;
...
Процедура ВывестиОрганизациюП7(СтрокаОрганизации, ДокументРезультат, Макет, СоответствиеПользовательскихПолей)
	...
	Руководители = ОтветственныеЛицаБП.ОтветственныеЛица(СтрокаОрганизации.Организация, ТекущаяДата());
	Если ЗначениеЗаполнено(Руководители.ГлавныйБухгалтерФИО) Тогда
		ОбластьТаблицы.Параметры.ФИОГлавногоБухгалтера = ФизическиеЛицаКлиентСервер.ИмяФамилияВФорматеДСТУ(Руководители.ГлавныйБухгалтерФИО);
	КонецЕсли;
...
Процедура ВывестиОрганизациюРасчетнаяВедомость(СтрокаОрганизации, ДокументРезультат, Макеты, ЕстьГруппировкаПоПодразделению, СоответствиеПользовательскихПолей, ДополнитьВедомостьПустымиСтроками)
	...
	Иначе
		...
		Если Документ <> Неопределено Тогда
			...
			Макеты.ШапкаДокумента.Параметры.ФИОРуководителя = ФизическиеЛицаКлиентСервер.ИмяФамилияВФорматеДСТУ(СтруктураФИОРуководителя);
			Макеты.ШапкаДокумента.Параметры.ДолжностьРуководителя = ДанныеДокумента.ДолжностьРуководителя;
			Макеты.ШапкаДокумента.Параметры.ФИОГлавногоБухгалтера = ФизическиеЛицаКлиентСервер.ИмяФамилияВФорматеДСТУ(СтруктураФИОГлавногоБухгалтера);
		Иначе
			Руководители = ОтветственныеЛицаБП.ОтветственныеЛица(СтрокаОрганизации.Организация, ТекущаяДата());
			Если ЗначениеЗаполнено(Руководители.Руководитель) Тогда
				...
				Макеты.ШапкаДокумента.Параметры.ФИОРуководителя = ФизическиеЛицаКлиентСервер.ИмяФамилияВФорматеДСТУ(Руководители.РуководительФИО);
			КонецЕсли;
			Если ЗначениеЗаполнено(Руководители.ГлавныйБухгалтерФИО) Тогда
				Макеты.ШапкаДокумента.Параметры.ФИОГлавногоБухгалтера = ФизическиеЛицаКлиентСервер.ИмяФамилияВФорматеДСТУ(Руководители.ГлавныйБухгалтерФИО);
			КонецЕсли;
		КонецЕсли;
