...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = НСтр("ru='Ожидаемый и подтвержденный НДС приобретений ';uk='Очікуваний та підтверджений ПДВ придбань '") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(?(Отчет.СНачалаГода,НачалоГода(Отчет.НачалоПериода),Отчет.НачалоПериода), Отчет.КонецПериода);
...
Процедура ЗаполнитьГруппировкиПоУмолчанию()
	...
	НоваяСтрока = Отчет.Группировка.Добавить();
	НоваяСтрока.Поле = "ДляОперацийОблагаемыхНДС";
	НоваяСтрока.Представление = "Для операций облагаемых НДС";
	НоваяСтрока.Использование = Истина;
	НоваяСтрока.ТипГруппировки = 0;
