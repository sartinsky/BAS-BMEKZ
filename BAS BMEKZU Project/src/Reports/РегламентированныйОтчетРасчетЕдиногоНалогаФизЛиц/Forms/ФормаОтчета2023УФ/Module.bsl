...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2023УФ";
	...
	СтруктураРеквизитовФормы.мИдГруппы21 = "D1T1R";
	...
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "15/12/2022";
...
Процедура СформироватьСтруктуруРеквизитовФормы()
	...
	СтруктураРеквизитовФормы.Вставить("КорректироватьПлощади", Ложь);  // ИНАГРО ++
	СтруктураРеквизитовФормы.Вставить("КоеффициентМПЗ",	0.04); // ИНАГРО ++    2022-2023 0,04, потом 0,05   
...
Процедура ФормироватьСтруктуруСтраницОтчета() 
	...
	СтрокаУровня1.Представление            = НСтр("ru='Приложение 1';uk='Додаток 1'");
	СтрокаУровня1.ИмяОбластиТабличногоПоля = "ОбластьПечатиПоВысоте|ОбластьПечатиПоШирине";
	СтрокаУровня1.ОриентацияСтраницы       = ОриентацияСтраницы.Портрет;
	СтрокаУровня1.ВыводНаПечать            = 1;
	СтрокаУровня1.ПоказатьСтраницу         = 1;
	...
	СтрокаУровня1.Представление            = НСтр("ru='Приложение 2';uk='Додаток 2'");
	СтрокаУровня1.ИмяОбластиТабличногоПоля = "ОбластьПечатиПоВысоте|ОбластьПечатиПоШирине";
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент) Экспорт
	...
	Если    Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "ДекларацияЕН" 
...
Процедура ВывестиМакетВТаблДокНаСервере(ИмяТекРаздела, НомерСтраницыРазделаОтчета)
	...
	ИначеЕсли ИмяТекРаздела = "ДекларацияЕН"
		  ИЛИ ИмяТекРаздела = "Приложение" 
		  ИЛИ ИмяТекРаздела = "Приложение2" 
...
Процедура ЗаполнитьДатуПодписи(Форма)
	...
	ЗаполнитьПоказательНаСервере("HFILL",  Форма.ДатаПодписи, Форма, "Приложение2");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение2", "Д_", 8, Формат(Форма.ДатаПодписи,"ДФ=ddMMyyyy"), Ложь);
...
Процедура ПоказатьПериод()
	...
	СтруктураРеквизитовФормы.СтрПериодОтчета = ПредставлениеПериода(НачалоГода(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета), КонецДня(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), "ФП = Истина; Л="+РегламентированнаяОтчетностьКлиентСервер.ПолучитьКодЯзыкаИнтерфейса());
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	ЗаполнитьПоказательНаСервере("HNAME", Организация.ИндивидуальныйПредприниматель, Форма, "Приложение");
	...
	ЗаполнитьПоказательНаСервере("HBOS",  ФИОДиректора, Форма, "Приложение2");
	...
	ЗаполнитьПоказательНаСервере("HTIN", КодЕДРПОУ, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("HTIN", КодЕДРПОУ, Форма, "Приложение2");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HTIN_",  10, КодЕДРПОУ, Ложь);	
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение", "HTIN_",  10, КодЕДРПОУ, Ложь);	
	
	Паспорт = ОбработатьДРФО(КодЕДРПОУ);
	ЗаполнитьПоказательНаСервере("HTINPF", Паспорт, Форма, "Приложение");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение", "HTINPF_",  10, Паспорт, Ложь);	
	...
	Если НЕ Форма.СтруктураРеквизитовФормы.ПолучениеДанныхДляЗвит1С = Истина Тогда
		...
		Если НЕ ЗаполнитьТаблицуКВЭДДаннымиПредыдущейДекларации(Форма) Тогда
			...
			КонецЕсли;			
		КонецЕсли;
		ЗаполнитьПоказательНаСервере("HKVED", КодКВЕД, Форма, "Приложение");
		РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение", "KVED_",  5, КодКВЕД, Ложь);		
	КонецЕсли;
	
	//Код особы всегда = 6
	ЗаполнитьПоказательНаСервере("R081G1", 6, Форма, "Приложение");
	...
	ЗаполнитьПоказательНаСервере("C_DOC_STAN", 1, Форма, "Приложение2");	
	...
	ЗаполнитьПоказательНаСервере("HZ",  "X",  Форма, "Приложение");
	Если (Форма.СтруктураРеквизитовФормы.мПериодичность = Форма.ПеречислениеПериодичностьМесяц)  Тогда    
		...
		ЗаполнитьПоказательНаСервере("HMONTH",  Месяц(Форма.СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета), Форма, "Приложение");
		...
		ЗаполнитьПоказательНаСервере("HD",  "X",  Форма, "Приложение");
	...
	ЗаполнитьПоказательНаСервере("HZY",  		 Год, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("HZY",  		 Год, Форма, "Приложение2");
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ДекларацияЕН", "HZY_", 4, Формат(Год, "ЧГ="), Ложь);	
	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "Приложение", "HZY_", 4, Формат(Год, "ЧГ="), Ложь);	
...
Процедура ВосстановитьСохраненныеДанные()
	...
	СписокСохранения.Свойство("КорректироватьПлощади", 	СтруктураРеквизитовФормы.КорректироватьПлощади);
...
Функция Сохранить(Автосохранение = Ложь, НаимТекРаздела) Экспорт	
	...
	СписокСохранения.Вставить("КорректироватьПлощади", 	СтруктураРеквизитовФормы.КорректироватьПлощади);	
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	R0141G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R012G3", Форма, "ДекларацияЕН"))
			- ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R013G3", Форма, "ДекларацияЕН"));
	ЗаполнитьПоказательНаСервере("R0141G3", R0141G3, Форма, "ДекларацияЕН");
	...
	Для каждого Строка Из ТаблицаБлока Цикл
		//МПЗ = НГОд  х К х М / 12 (графа 5 х графа 7 х графа 8 / 12), грн
    	Строка.D1T1RXXXXG9 = Строка.D1T1RXXXXG5 * Строка.D1T1RXXXXG7 * Строка.D1T1RXXXXG8 /12;
		ИмяЯчейкиПрописью = "D1T1RXXXXG9_"+Формат(ТаблицаБлока.Индекс(Строка) + 1, "ЧГ=;");
		ЗаполнитьПоказательНаСервере(ИмяЯчейкиПрописью, Строка.D1T1RXXXXG9, Форма, "Приложение2");
		
		//МПЗ = НГО х S х К х М / 12 графа 6  х (графа 3 + графа 4) х графа 7 х графа 8 / 12), грн
    	Строка.D1T1RXXXXG10 = Строка.D1T1RXXXXG6 * (Строка.D1T1RXXXXG3 + Строка.D1T1RXXXXG4) * Строка.D1T1RXXXXG7 * Строка.D1T1RXXXXG8 /12;
		ИмяЯчейкиПрописью = "D1T1RXXXXG10_"+Формат(ТаблицаБлока.Индекс(Строка) + 1, "ЧГ=;");
		ЗаполнитьПоказательНаСервере(ИмяЯчейкиПрописью, Строка.D1T1RXXXXG10, Форма, "Приложение2");
		R002G7 = Строка.D1T1RXXXXG7;
		
	КонецЦикла;
	R002G3 = ТаблицаБлока.Итог("D1T1RXXXXG3");
	R002G4 = ТаблицаБлока.Итог("D1T1RXXXXG4");
	R002G9 = ТаблицаБлока.Итог("D1T1RXXXXG9");
	R002G10 = ТаблицаБлока.Итог("D1T1RXXXXG10");
	R002G12 = ТаблицаБлока.Итог("D1T1RXXXXG12");
	ЗаполнитьПоказательНаСервере("R002G4", R002G4, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R002G3", R002G3, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R002G7", R002G7, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R002G9", R002G9, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R002G10", R002G10, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R002G12", R002G12, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R03G3", R002G12, Форма, "Приложение2");
    //(ЗМПЗ) (ЗМПЗ = сума рядків 1.n граф  9 та/або 10), грн   
	R002G11 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G9", Форма, "Приложение2"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R002G10", Форма, "Приложение2"));
	ЗаполнитьПоказательНаСервере("R002G11", R002G11, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R01G3", R002G11, Форма, "Приложение2");  
	
	R02G3 	= ПолучитьПоказательНаСервере("R021G3", Форма, "Приложение2")
			+ ПолучитьПоказательНаСервере("R022G3", Форма, "Приложение2")
			+ ПолучитьПоказательНаСервере("R023G3", Форма, "Приложение2")
			+ ПолучитьПоказательНаСервере("R024G3", Форма, "Приложение2")
			+ ПолучитьПоказательНаСервере("R025G3", Форма, "Приложение2")
			+ ПолучитьПоказательНаСервере("R026G3", Форма, "Приложение2")
			+ ПолучитьПоказательНаСервере("R027G3", Форма, "Приложение2")
			+ ПолучитьПоказательНаСервере("R028G3", Форма, "Приложение2"); 
			
	ЗаполнитьПоказательНаСервере("R02G3", R02G3, Форма, "Приложение2");    
	//(рядок 01 - рядок 02 - рядок 03 розділу ІІ) (+/-) (позитивне значення переноситься до рядка 14.2 Декларації)
	R04G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R01G3", Форма, "Приложение2"))
			- ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R02G3", Форма, "Приложение2"))
			- ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R03G3", Форма, "Приложение2"));
	ЗаполнитьПоказательНаСервере("R04G3", R04G3, Форма, "Приложение2");
	ЗаполнитьПоказательНаСервере("R0142G3", ?(R04G3>0,R04G3,0), Форма, "ДекларацияЕН");
	
	R014G3 =  ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0141G3", Форма, "ДекларацияЕН"))
			+ ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0142G3", Форма, "ДекларацияЕН"));
	ЗаполнитьПоказательНаСервере("R014G3", R014G3, Форма, "ДекларацияЕН");
	Если R014G3 > 0 Тогда
		ЗаполнитьПоказательНаСервере("HD2", "Х", Форма, "ДекларацияЕН");
	КонецЕсли;
	...
	Для КодСтроки = 1 По 12 Цикл
			
		R09G2_Значение = ПолучитьПоказательНаСервере("R09" +КодСтроки+"G2", Форма, "Приложение");
		R09G3_Значение = ПолучитьПоказательНаСервере("R09" +КодСтроки+"G3", Форма, "Приложение");
		R09G4_Значение = R09G2_Значение * R09G3_Значение/100;
		ЗаполнитьПоказательНаСервере("R09" +КодСтроки+"G4", R09G4_Значение, Форма, "Приложение");
		R09G2 = R09G2 + R09G2_Значение;
		R09G4 = R09G4 + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R09" +КодСтроки+"G4", Форма, "Приложение"));
		
	КонецЦикла;
	ЗаполнитьПоказательНаСервере("R09G2", R09G2, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R09G4", R09G4, Форма, "Приложение");
	ЗаполнитьПоказательНаСервере("R021G3", R09G4, Форма, "ДекларацияЕН");
	
	Если R09G4 <> 0 Тогда
		ЗаполнитьПоказательНаСервере("HD1", "Х", Форма, "ДекларацияЕН");
	КонецЕсли;
	
	Если ЭтоУточняющаяДекларация Тогда
		ЗаполнитьПоказательНаСервере("R0101G3", R09G4, Форма, "Приложение");
		
		R0103G3 = ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R0102G3", Форма, "Приложение"))-R09G4;
		Если R0103G3 > 0 Тогда
			ЗаполнитьПоказательНаСервере("R0103G3", R0103G3, Форма, "Приложение");
			ЗаполнитьПоказательНаСервере("R0104G3", 0, Форма, "Приложение");
		Иначе
			ЗаполнитьПоказательНаСервере("R0103G3", 0, Форма, "Приложение");
			ЗаполнитьПоказательНаСервере("R0104G3", - R0103G3, Форма, "Приложение");
		КонецЕсли;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	Если  ИмяЯчейки = "HZ" Тогда
		...
		Если НЕ НаименованиеТекЛиста = "ДекларацияЕН" Тогда
		...
		//Приложение
		ЗаполнитьПоказательНаСервере("HZ", "X",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("HZN", "",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("HZU", "",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("C_DOC_STAN", 1,  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("C_DOC_STAN", 1,  ЭтаФорма, "Приложение2");
		Расчет(ЭтаФорма);
	...
	ИначеЕсли ИмяЯчейки = "HZN" Тогда
		...
		//Приложение
		ЗаполнитьПоказательНаСервере("HZ", "",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("HZN", "X",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("HZU", "",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("C_DOC_STAN", 2,  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("C_DOC_STAN", 2,  ЭтаФорма, "Приложение2");
		Расчет(ЭтаФорма);
	...
	ИначеЕсли ИмяЯчейки = "HZU" 
		...
		//для уточняющей не может быть Довідкова
		ТабличныйДокумент.Области["HD"].Значение = "";
		
		ЗаполнитьПоказательНаСервере("C_DOC_STAN", 3,  ЭтаФорма, "ДекларацияЕН");
		//Приложение
		ЗаполнитьПоказательНаСервере("HZ", "",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("HZN", "",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("HZU", "X",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("HD", "",  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("C_DOC_STAN", 3,  ЭтаФорма, "Приложение");
		ЗаполнитьПоказательНаСервере("C_DOC_STAN", 3,  ЭтаФорма, "Приложение2");
		
		Расчет(ЭтаФорма);
		
	ИначеЕсли ИмяЯчейки = "HD" 
		И СтруктураРеквизитовФормы.мПериодичность = ПеречислениеПериодичностьМесяц Тогда
		...
		Если ЗначениеЗаполнено(ТабличныйДокумент.Области["HD"].Значение) Тогда
			ТабличныйДокумент.Области["HD"].Значение = "";
			ЗаполнитьПоказательНаСервере("HD", "",  ЭтаФорма, "Приложение");
		Иначе	
			ТабличныйДокумент.Области["HD"].Значение = "X";
			ЗаполнитьПоказательНаСервере("HD", "X",  ЭтаФорма, "Приложение");
		КонецЕсли;
	...
	ИначеЕсли ИмяЯчейки = "H1KVP"
		...
		ЗаполнитьПоказательНаСервере("H1KVP",	ТабличныйДокумент.Области["H1KVP"].Значение,	ЭтаФорма,"Приложение");
		ЗаполнитьПоказательНаСервере("HHYP",	ТабличныйДокумент.Области["HHYP"].Значение,		ЭтаФорма,"Приложение");
		ЗаполнитьПоказательНаСервере("H3KVP",	ТабличныйДокумент.Области["H3KVP"].Значение,	ЭтаФорма,"Приложение");
		ЗаполнитьПоказательНаСервере("HYP",		ТабличныйДокумент.Области["HYP"].Значение,		ЭтаФорма,"Приложение");
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 6)) = "HTINPF" Тогда
		
		Значение = ТабличныйДокумент.Области.HTINPF.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
	...
	ИначеЕсли  (ИмяЯчейки = "H02" ИЛИ ИмяЯчейки = "H04") 
		И СтруктураРеквизитовФормы.мПериодичность = ПеречислениеПериодичностьМесяц Тогда
		//не может быть уточняющим, довідкова
	...
	ИначеЕсли  ИмяЯчейки = "H01" ИЛИ ИмяЯчейки = "H03" Тогда
	...
	ИначеЕсли  ИмяЯчейки = "M082" ИЛИ ИмяЯчейки = "M081" ИЛИ ИмяЯчейки = "HD1" ИЛИ ИмяЯчейки = "HD2" Тогда
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 3)) = "ДП_" Тогда
		
		Значение = ТабличныйДокумент.Области.PripD.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
		...
		ПоказатьВводДаты(ОписаниеОповещения,Значение, НСтр("ru='Введите значение';uk='Введіть значення'"));
		
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 2)) = "ДС" Тогда
		
		Значение = ТабличныйДокумент.Области.R08G1D.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
	...
	ИначеЕсли  ВРЕГ(Лев(ИмяЯчейки, 3)) = "ДПО" Тогда
		
		Значение = ТабличныйДокумент.Области.R08G2D.Значение;
		ОписаниеОповещения = Новый ОписаниеОповещения("ВводПоказателяВПолеТабличногоДокументЗавершение",
													   ЭтаФорма, Новый Структура("ИмяЯчейки, ИмяСтраницы,
																				 |ИмяЯчейкиКвадратов, ИмяСтраницыКвадратов, 
																				 |Размерность, ДополнитьЗначение, НольКакНоль", 
...
Процедура ВводПоказателяВПолеТабличногоДокументЗавершение(Значение, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ИмяЯчейки = "HTIN" Тогда
		ЗаполнитьПоказательНаСервере("HTIN", Значение, ЭтаФорма, "Приложение");	
		РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(ЭтаФорма, "Приложение", ДополнительныеПараметры.ИмяЯчейкиКвадратов, ДополнительныеПараметры.Размерность, Значение, ДополнительныеПараметры.ДополнитьЗначение, ДополнительныеПараметры.НольКакНоль);
		РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(ЭтаФорма, "Приложение2", ДополнительныеПараметры.ИмяЯчейкиКвадратов, ДополнительныеПараметры.Размерность, Значение, ДополнительныеПараметры.ДополнитьЗначение, ДополнительныеПараметры.НольКакНоль);
		ЗаполнитьПоказательНаСервере("TIN",  Значение, ЭтаФорма, "СлужебныеДанные");
	КонецЕсли;
	
	Если ДополнительныеПараметры.ИмяЯчейки = "HZYP" Тогда
		ЗаполнитьПоказательНаСервере("HZYP", Значение, ЭтаФорма, "Приложение");	
		РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(ЭтаФорма, "Приложение", ДополнительныеПараметры.ИмяЯчейкиКвадратов, ДополнительныеПараметры.Размерность, Значение, ДополнительныеПараметры.ДополнитьЗначение, ДополнительныеПараметры.НольКакНоль);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ДополнительныеПараметры.ИмяСтраницыКвадратов) Тогда
		Если  ВРЕГ(Лев(ДополнительныеПараметры.ИмяЯчейкиКвадратов,3)) = "ДП_" Тогда
			ЗаполнитьПоказательНаСервере("R06G1", День(Значение),  ЭтаФорма, "Приложение");
			ЗаполнитьПоказательНаСервере("R06G2", Месяц(Значение), ЭтаФорма, "Приложение");
			ЗаполнитьПоказательНаСервере("R06G3", Год(Значение),   ЭтаФорма, "Приложение");
		КонецЕсли;	
		// нормализуем дату
		Если  ВРЕГ(Лев(ДополнительныеПараметры.ИмяЯчейкиКвадратов,1)) = "Д" Тогда
			
			Значение = Формат(Значение,"ДФ=ddMMyyyy");
		КонецЕсли;	
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	Если ИмяРаздела = "ДекларацияЕН" Тогда
		Если    ИмяОбласти = "HTIN" 
			ИЛИ ИмяОбласти = "HBOS" 
			Тогда
			СтруктураДанныхПриложение[ИмяОбласти] = ТабличныйДокумент.Области[ИмяОбласти].Значение;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
		...
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru='Допустимые объемы дохода плательщиков единого налога 1-3-й групп определяются 
                                |в зависимости от размера минимальной зарплаты на начало отчетного года.';uk= 'Допустимі обсяги доходу платників єдиного податку 1-3-й груп визначаються в залежності від суми мінімальної 
                                |зарплати на початок звітного року.'");
		Сообщение.Сообщить();
		
		Если РезультатЗапроса.Следующий() Тогда
			МинЗП = РезультатЗапроса.Размер;
			ОграничениеСуммыДоходаЕдинщика1гр = 167*МинЗП;
			ОграничениеСуммыДоходаЕдинщика2гр = 834*МинЗП;
			ОграничениеСуммыДоходаЕдинщика3гр = 1167*МинЗП;
			Сообщение = Новый СообщениеПользователю;
...
Процедура ЗаполнитьМЗЗ(ПараметрыЗаполненияЗвит1С)
	...
	Запрос = ПодготовитьЗапросПриложение();
	
	Запрос.УстановитьПараметр("ДатаКон", КонецГода(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета));  
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		СоответствиеПолейТаблица.Вставить("D1T1RXXXXG2S",  	"КадастровыйНомер"); 
		СоответствиеПолейТаблица.Вставить("D1T1RXXXXG8",  	"КоличествоМесяцев");  
						
		ДанныеФормы 	= ЭтаФорма[СтруктураРеквизитовФормы.мИдГруппы21];
		...
		Пока Результат.Следующий() Цикл
			...
			КонецЦикла;  
			СтрокаТаблицы.D1T1RXXXXНомерСтроки = НомерСтроки;
			Если ЗначениеЗаполнено(Результат.ВидПрава) И НЕ Результат.ВидПрава = "3" Тогда
			
				СтрокаТаблицы.D1T1RXXXXG3  = Результат.ПлощадьФизическая;
				СтрокаТаблицы.D1T1RXXXXG4  = 0;
			Иначе
				СтрокаТаблицы.D1T1RXXXXG3  = 0;
				СтрокаТаблицы.D1T1RXXXXG4  = Результат.ПлощадьФизическая;
			КонецЕсли; 
			
			Если Результат.ПроведенаНормативнаяОценка Тогда
				СтрокаТаблицы.D1T1RXXXXG5  = Результат.НормативнаяОценкаЗемли;
				СтрокаТаблицы.D1T1RXXXXG6  = 0;
			Иначе
				СтрокаТаблицы.D1T1RXXXXG5  = 0;
				СтрокаТаблицы.D1T1RXXXXG6  = Результат.НормативнаяОценкаЗемлиЕдПлощади;
			КонецЕсли;
			
			СтрокаТаблицы.D1T1RXXXXG7 = СтруктураРеквизитовФормы.КоеффициентМПЗ;
			
			СтрокаТаблицы.D1T1RXXXXG12 = ?(СтрокаТаблицы.D1T1RXXXXG4 = 0 ИЛИ НЕ Результат.ВидПрава = "3", 0, Окр((СтрокаТаблицы.D1T1RXXXXG5 + (СтрокаТаблицы.D1T1RXXXXG4 * СтрокаТаблицы.D1T1RXXXXG6))
										* Результат.ПроцентВыплаты * 0.2 / 100 , 2));
		...
		Пока Таблица.Количество() < мСтруктураИсхКолвоСтрокРазделов[СтруктураРеквизитовФормы.мИдГруппы21] Цикл
			Таблица.Добавить();
		КонецЦикла;
		ЗначениеВРеквизитФормы(Таблица, СтруктураРеквизитовФормы.мИдГруппы21);
...
Функция ПодготовитьЗапросПриложение() Экспорт 
	...
	Если СтруктураРеквизитовФормы.КорректироватьПлощади Тогда
		...
		|	НормативнаяОценка.ПроцентВыплаты КАК ПроцентВыплаты,
		...
		|					И Участок.Поле.ИНАГРО_ВидПоля <> ЗНАЧЕНИЕ(Перечисление.ИНАГРО_ВидыПолей.ПустаяСсылка)) КАК ИНАГРО_НормативнаяОценкаЗемлиСрезПоследних
		|	ГДЕ
		|		ИНАГРО_НормативнаяОценкаЗемлиСрезПоследних.Актуальность) КАК НормативнаяОценка
	...
	Иначе  
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	Вт_ДоговорыПериоды.ДоляЗемельногоПая КАК ДоляЗемельногоПая,
		...
		|	Вт_ДоговорыПериоды.ПроцентВыплаты,
		|	Вт_ДоговорыПериоды.ГодоваяАктуальность,
		|	Вт_ДоговорыПериоды.ДоляЗемельногоПая
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура ЗаполнитьАвансовыеПлатежиБП()
	...
	Запрос.Текст =  
	"ВЫБРАТЬ
	|	НАЧАЛОПЕРИОДА(ПлатежныйДокумент.Дата, День) КАК Период,
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	НАЧАЛОПЕРИОДА(ПлатежныйДокумент.Дата, День)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Перечисления.Период КАК ПериодПеречисления,
	|	МАКСИМУМ(СтатусыПлательщиковЕдиногоНалога.Период) КАК ПериодУчетнойПолитики
	|ПОМЕСТИТЬ ПериодыУчетнойПолитики
	|ИЗ
	|	Перечисления КАК Перечисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыПлательщиковЕдиногоНалога КАК СтатусыПлательщиковЕдиногоНалога
	|		ПО (СтатусыПлательщиковЕдиногоНалога.Период <= Перечисления.Период)
	|			И (СтатусыПлательщиковЕдиногоНалога.Организация = &Организация)
	|
	|СГРУППИРОВАТЬ ПО
	|	Перечисления.Период
	|;
	...
	|	СтатусыПлательщиковЕдиногоНалога.ГруппаПлательщикаЕдиногоНалога КАК Группа
	|ИЗ
	|	Перечисления КАК Перечисления
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПериодыУчетнойПолитики КАК ПериодыУчетнойПолитики
	|		ПО Перечисления.Период = ПериодыУчетнойПолитики.ПериодПеречисления
	...
	|			И (СтатусыПлательщиковЕдиногоНалога.Период = ПериодыУчетнойПолитики.ПериодУчетнойПолитики)
	|ГДЕ
	|	(СтатусыПлательщиковЕдиногоНалога.ГруппаПлательщикаЕдиногоНалога = ЗНАЧЕНИЕ(Перечисление.ГруппыПлательщиковЕдиногоНалога.ПерваяГруппа)
	|			ИЛИ СтатусыПлательщиковЕдиногоНалога.ГруппаПлательщикаЕдиногоНалога = ЗНАЧЕНИЕ(Перечисление.ГруппыПлательщиковЕдиногоНалога.ВтораяГруппа))
	|";
...
Процедура ЗаполнитьРазделПоГруппеБП(Группа)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	КнигаДоходовРасходовПоЕдиномуНалогуОбороты.Статья КАК Статья,
	               ...
	               |	Продажи КАК Продажи
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыПлательщиковЕдиногоНалога КАК СтатусыПлательщиковЕдиногоНалога
	               |		ПО (СтатусыПлательщиковЕдиногоНалога.Период <= Продажи.Период)
	               |			И (СтатусыПлательщиковЕдиногоНалога.Организация = &Организация)
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	Продажи.Период
	               ...
	               |	Продажи.Статья КАК Статья,
	               |	Продажи.Сумма КАК Сумма,
	               |	СтатусыПлательщиковЕдиногоНалога.ПлательщикНДС КАК ПлательщикНДС
	               |ИЗ
	               |	Продажи КАК Продажи
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ПериодыУчетнойПолитики КАК ПериодыУчетнойПолитики
	               |		ПО Продажи.Период = ПериодыУчетнойПолитики.ПериодПродаж
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СтатусыПлательщиковЕдиногоНалога КАК СтатусыПлательщиковЕдиногоНалога
	               |		ПО (СтатусыПлательщиковЕдиногоНалога.Организация = &Организация)
	               |			И (СтатусыПлательщиковЕдиногоНалога.Период = ПериодыУчетнойПолитики.ПериодУчетнойПолитики)
	               |ГДЕ
	               |	СтатусыПлательщиковЕдиногоНалога.ГруппаПлательщикаЕдиногоНалога = &Группа
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	Период";
...
Процедура ЗаполнитьРазделПоГруппеЕРП(Группа)
	...
	Запрос.УстановитьПараметр("Группа", Группа);
		...
		Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			И (СтатусыПлательщиковЕдиногоНалога.Период = ПериодыУчетнойПолитики.ПериодУчетнойПолитики)
	               |
				   |ГДЕ СтатусыПлательщиковЕдиногоНалога.ГруппаПлательщикаЕдиногоНалога = &Группа
				   |
				   |УПОРЯДОЧИТЬ ПО
	               |	Период";
...
Процедура ЗаполнитьНалогИзПредыдущейДекларации(ПараметрыЗаполненияЗвит1С)
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 3 Тогда
		...
		Иначе
			...
			// данные из звита потом заполним на клиенте, здесь только получим требуемые параметры
...
Процедура  ОпределитьСписокВыгружаемыхСхем(тбСхемыXML);
	...
	Для Каждого стр Из  тбСхемыXML Цикл
		...
		Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
			...
			Если Лев(Стр.ИмяСхемы, 6) = "F01033" ИЛИ Лев(Стр.ИмяСхемы, 5) = "F0133" Тогда
				// общая декларация
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	НовСтрока = тбСхемыXML.Добавить();
	НовСтрока.ИмяСхемы = "F0133108"; 	
	НовСтрока.ПредставлениеСхемы =НСтр("ru='Приложение 1';uk='Додаток 1'");
	НовСтрока.НазваниеСтраницыПанели ="Приложение";
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0134106"; 	
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Приложение 1 (годовая Декларация по ЕН для I и II группы).';uk= 'Додаток 1 (річна Декларація по ЄП для I та II групи)'");
		НовСтрока.НазваниеСтраницыПанели ="Приложение";
	КонецЕсли; 
	
	НовСтрока = тбСхемыXML.Добавить();
	НовСтрока.ИмяСхемы = "F0133208"; 	
	НовСтрока.ПредставлениеСхемы =НСтр("ru='Приложение 2';uk='Додаток 2'");
	НовСтрока.НазваниеСтраницыПанели ="Приложение2";
	НовСтрока.Префикс = "D1";
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0134206"; 	
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Приложение 2 (годовая Декларация по ЕН для I и II группы).';uk= 'Додаток 2 (річна Декларація по ЄП для I та II групи)'");
		НовСтрока.НазваниеСтраницыПанели ="Приложение2";
		НовСтрока.Префикс = "D1";
	КонецЕсли; 
