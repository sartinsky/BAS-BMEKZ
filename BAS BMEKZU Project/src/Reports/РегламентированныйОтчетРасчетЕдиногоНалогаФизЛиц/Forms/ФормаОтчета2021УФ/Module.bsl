...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для КодСтроки = 1 По 12 Цикл
		...
		R09G4_Значение = R09G2_Значение * R09G3_Значение;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если РегламентированнаяОтчетностьПереопределяемый.ИДКонфигурации(Истина) = "BASERP" Тогда
		ЗаполнитьАвансовыеПлатежиBASERP();
		
		ЗаполнитьРазделПоГруппеBASERP(Перечисления.ГруппыПлательщиковЕдиногоНалога.ПерваяГруппа);
		ЗаполнитьРазделПоГруппеBASERP(Перечисления.ГруппыПлательщиковЕдиногоНалога.ВтораяГруппа);
		ЗаполнитьРазделПоГруппеBASERP(Перечисления.ГруппыПлательщиковЕдиногоНалога.ТретьяГруппа);
...
&НаСервере
Процедура ЗаполнитьАвансовыеПлатежиBASERP()
...
&НаСервере
Процедура ЗаполнитьРазделПоГруппеBASERP(Группа)
