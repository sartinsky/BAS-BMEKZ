...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	//ЗаполнитьПоказательНаСервере("R09G3", R09G3, Форма, "Приложение");
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли  ИмяЯчейки = "M082" ИЛИ ИмяЯчейки = "M081" ИЛИ ИмяЯчейки = "HD1" Тогда
...
Процедура ОбработкаИзмененияСодержимогоВПолеТабличногоДокумента(Элемент, Область)
	...
	//Если Область.Имя = "HNAME" Тогда
	//	ЗаполнитьПоказательНаСервере("HNAME", ПолучитьПоказательНаСервере("HNAME", ЭтаФорма, "ДекларацияЕН"), ЭтаФорма, "Приложение");
	//КонецЕсли;
...
Процедура ЗаполнитьАвансовыеПлатежиБП()
	...
	Если Метаданные.РегистрыСведений.Найти("СтатусыПлательщиковЕдиногоНалога") <> Неопределено Тогда
		Запрос = Новый Запрос;
		Запрос.Текст =  
		"ВЫБРАТЬ
...
Процедура ЗаполнитьРазделПоГруппеБП(Группа)
	...
	Если Метаданные.РегистрыСведений.Найти("СтатусыПлательщиковЕдиногоНалога") <> Неопределено Тогда
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |		ПО (УчетнаяПолитикаОрганизаций.Организация = &Организация)
		               |			И (УчетнаяПолитикаОрганизаций.Период = ПериодыУчетнойПолитики.ПериодУчетнойПолитики)
		               |ГДЕ
...
Процедура ЗаполнитьАвансовыеПлатежиЕРП()
	...
	Если Метаданные.РегистрыСведений.Найти("СтатусыПлательщиковЕдиногоНалога") <> Неопределено Тогда
		...
		|	Перечисления.Сумма КАК Сумма,
		|	СтатусыПлательщиковЕдиногоНалога.ГруппаПлательщикаЕдиногоНалога КАК Группа
		//|	ПериодыУчетнойПолитики.ПериодПеречисления,
...
Процедура ЗаполнитьНалогИзПредыдущейДекларации(ПараметрыЗаполненияЗвит1С)
	...
	Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) > 3 Тогда
		...
		Иначе
			...
			Если Год(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 2021 И Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 3 Тогда
				ИмяСхемы = "F0103306";
			иначе
				ИмяСхемы = "F0103307";
			КонецЕсли;
...
Процедура  ОпределитьСписокВыгружаемыхСхем(тбСхемыXML);
	...
	Для Каждого стр Из  тбСхемыXML Цикл
		...
		Если Месяц(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета) = 12 Тогда
			...
			Если Лев(Стр.ИмяСхемы, 6) = "F01033" ИЛИ Лев(Стр.ИмяСхемы, 6) = "F01331" Тогда
				// общая декларация
