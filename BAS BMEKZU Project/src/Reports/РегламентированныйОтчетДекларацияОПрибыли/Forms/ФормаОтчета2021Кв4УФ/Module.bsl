...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2021Кв4УФ";
...
Процедура ПриОткрытии(Отказ)
	...
	Если Найти(Заголовок, СтруктураРеквизитовФормы.СтрПериодОтчета) = 0 Тогда
		Заголовок = Заголовок + " за " + СтруктураРеквизитовФормы.СтрПериодОтчета + " (форма 4кв.2021) ";
	КонецЕсли;
...
Процедура ЗаполнитьПриложениеРИ_АМ_ТЦ()
   ...
   // Списание    = Кт оборот 10,11,12,286 - не в корр с 10,11,12,13,286 при условии, что на конец приода нет остатка по этим счетам учета (по Дт)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			Попытка		
				Если Выборка.Статья = СтНД.НП15_РИ_Фин_ДивидендыКИК					Тогда ЗаполнитьКорректировкуРИ("R03232G3", "-",  Выборка.Сумма, Выборка.Статья)	
	...
	Иначе
		...
		Если ЮрЛицо Тогда
			Если ГодПериода > 2021 Тогда
				ИмяСхемы = "J0100120";  // Декларация о прибыли 
		...
		Иначе
			Если ГодПериода > 2021 Тогда
				ИмяСхемы = "F0100720";  // Декларация о прибыли 
			Иначе	//год 2021
				ИмяСхемы = "F0100719";  // Декларация о прибыли 
			КонецЕсли;
		КонецЕсли;	
		...
		Если ЮрЛицо Тогда
			Если ГодПериода > 2021 Тогда
				ИмяСхемы = "J0111420";  // Декларация о прибыли 
				Если ЗначениеЗаполнено(СтруктураДанныхДекларация.H091) Тогда
					// сельхозники
					ИмяСхемы = "J0151420"; 
				КонецЕсли;
			Иначе	//год 2021
				ИмяСхемы = "J0111419";  // Декларация о прибыли 
				Если ЗначениеЗаполнено(СтруктураДанныхДекларация.H091) Тогда
					// сельхозники
					ИмяСхемы = "J0151419"; 
				КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Если ЮрЛицо Тогда
		...
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0100520", "J0100120");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151820", "J0111820");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150320", "J0110320");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152820", "J0112820");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150620", "J0110620");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151320", "J0111320");  
		...
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151420", "J0111420");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела1.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151520", "J0111520");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151620", "J0111620");  
		...
		Если НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZU", ЭтаФорма, "Декларация")) Тогда
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151720", "J0111720");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152020", "J0112020");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела2.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154420", "J0114420");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела3.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154520", "J0114520");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела4.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154620", "J0114620");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела5.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154720", "J0114720");  
