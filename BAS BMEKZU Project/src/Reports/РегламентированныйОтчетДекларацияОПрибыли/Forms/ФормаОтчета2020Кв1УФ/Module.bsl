...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2020Кв1УФ";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Если ИмяСтраницыПанели = "ПриложениеПН" Тогда
		...
		ЗаполнитьПоказательНаСервере("R0115G3", R0216G5, Форма, "ПриложениеПН");
		ЗаполнитьПоказательНаСервере("R0115G6", R0216G6, Форма, "ПриложениеПН");
		
		R0127G3 = 0;
		R0127G6 = 0;
		
		Для КодСтроки = 1 По 26 Цикл
			
			ТекПроцент = 15;
			Если КодСтроки = 4 Тогда
				ТекПроцент = 6;	
			ИначеЕсли КодСтроки = 15 Тогда
				ТекПроцент = 18;	
			ИначеЕсли КодСтроки = 16
				  ИЛИ КодСтроки = 18 Тогда
				ТекПроцент = 0;	
			ИначеЕсли КодСтроки = 17 Тогда
				ТекПроцент = 4;	
			ИначеЕсли КодСтроки = 19 Тогда
				ТекПроцент = 12;	
			...
			Если КодСтроки < 21 ИЛИ КодСтроки = 23 Тогда	
				...
				R0127G3 = R0127G3 + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R01" +КодСтрокиСтр+"G3", Форма, "ПриложениеПН"));
				R0127G6 = R0127G6 + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R01" +КодСтрокиСтр+"G6", Форма, "ПриложениеПН"));
																	  
			Иначе										
				R0127G3 = R0127G3 + ОкруглитьЧислоПоФормату(Форма, ПолучитьПоказательНаСервере("R01" +КодСтрокиСтр+"G3", Форма, "ПриложениеПН"));
			КонецЕсли;	
		КонецЦикла;
		ЗаполнитьПоказательНаСервере("R0127G3", R0127G3, Форма, "ПриложениеПН");
		ЗаполнитьПоказательНаСервере("R0127G6", R0127G6, Форма, "ПриложениеПН");
	КонецЕсли;
	//////////////////////////////////////////////////////////////////////////////////////////
	// АВ
	...
	R023G3 = 0;
	Если ИмяСтраницыПанели = "ПриложениеПН" Тогда
		R023G3 = R023G3 + ПолучитьПоказательНаСервере("R0127G6", Форма, "ПриложениеПН");
	КонецЕсли;
	Для каждого ДанныеПН Из Форма.ТаблицаСтраницыРаздела1 Цикл
		...
		Если ДанныеПН.АктивнаяСтраница = Ложь Тогда
			...
			R0127G6 = 0;                                         
			Если ДанныеПН.Данные[0].Значение.Свойство("R0127G6", R0127G6) Тогда
	 			R023G3 = R023G3 + R0127G6;
			КонецЕсли;
		КонецЕсли; 
	КонецЦикла;
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
   ...
   ИначеЕсли ИмяЯчейки = "H1KVP"
		...
		ЗаполнитьПоказательНаСервере("H1KVP",	ТабличныйДокумент.Области["H1KVP"].Значение,	ЭтаФорма,ИмяЛистаДубля2);
		ЗаполнитьПоказательНаСервере("HHYP",	ТабличныйДокумент.Области["HHYP"].Значение,		ЭтаФорма,ИмяЛистаДубля2);
		ЗаполнитьПоказательНаСервере("H3KVP",	ТабличныйДокумент.Области["H3KVP"].Значение,	ЭтаФорма,ИмяЛистаДубля2);
		ЗаполнитьПоказательНаСервере("HYP",		ТабличныйДокумент.Области["HYP"].Значение,		ЭтаФорма,ИмяЛистаДубля2);
...
Процедура ЗаполнитьДанныеИзФинОтчетности(ПараметрыЗаполненияЗвит1С, КварталПрошлГод = 0)
	...
	Иначе	
		...
		ИначеЕсли СтруктураРеквизитовФормы.ВидФинОтчетности = 2 Тогда
			...
			ИмяСхемы      = "S0111005";  
			Если Год(ДатаПериодаОтчета) <= 2020 и Месяц(ДатаПериодаОтчета) < 12 Тогда
				ИмяСхемы = "S0111004";
			ИначеЕсли Год(ДатаПериодаОтчета) <= 2019 и Месяц(ДатаПериодаОтчета) < 9 Тогда
				ИмяСхемы = "S0111003";
			КонецЕсли;
...
Процедура ЗаполнитьУбытокПрошлогоГода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Иначе  //c 2019
			...
			ИмяСхемы2 = ИмяСхемы1 + "17"   //за 2019 могли сдавать и по старой схеме
		КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Иначе
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0100703";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Декларация о прибыли';uk='Декларація про прибуток'");
		НовСтрока.НазваниеСтраницыПанели ="Декларация";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0171803";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение PІ';uk='Додаток РI'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеРИ";
		НовСтрока.Префикс = "RI";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0170303";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение ЦП';uk='Додаток ЦП'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеЦП";
		НовСтрока.Префикс = "CP";
		
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы = "F0172803";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение ТЦ';uk='Додаток ТЦ'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеТЦ";
		НовСтрока.Префикс = "TC";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0170603";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение АМ';uk='Додаток АМ'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеАМ";
		НовСтрока.Префикс = "AM";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F0171303";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение ПЗ';uk='Додаток ПЗ'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеПЗ";
		НовСтрока.Префикс = "PZ";
		
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы ="F0171403";
		НовСтрока.ПредставлениеСхемы = НСтр("ru='Приложение ЗП';uk='Додаток ЗП'");
		НовСтрока.НазваниеСтраницыПанели ="ПриложениеЗП";
		НовСтрока.Префикс = "ZP";
...
Функция СформироватьДанныеДляЗвит1С(ОтчетЗвит1С = Неопределено) Экспорт 
	...
	Если СтруктураРеквизитовФормы.мПериодичность = ПредопределенноеЗначение("Перечисление.Периодичность.Год") и 
		НЕ СтруктураРеквизитовФормы.ДляФредоТипПериода Тогда
