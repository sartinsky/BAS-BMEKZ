...
Процедура ЗаполнитьПрибыльПрошлогоГода(Команда)
	...
	Иначе
		...
		МесяцПериода   = Месяц(мДатаКонцаПериодаОтчета);
		
		Если ГодПериода = 2022 Тогда
			//может быть с/х
			ИмяСхемы1 = ?(ДляФредоПроизводительСХ, "J0100522","J0100122");
			ИмяСхемы2 = ?(ДляФредоПроизводительСХ, "J0100523","J0100123");
		Иначе  //с 2023
		...
		Если ГодПериода = 2022 и ОблагаемаяПрибыльПрошлогоГода = 0 Тогда 
			ИмяСхемы1 = "J0100123";
			//может быть с/х
			ИмяСхемы2 = "J0100523"; 
			Попытка
