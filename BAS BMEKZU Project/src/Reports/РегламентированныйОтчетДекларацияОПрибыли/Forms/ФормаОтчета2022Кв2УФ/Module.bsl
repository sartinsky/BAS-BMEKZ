...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мСоставПоказателей = "СоставПоказателей2022Кв2УФ";
...
Процедура РазделыОтчетаПриАктивизацииСтроки(Элемент) Экспорт
	...
	Если    Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "Декларация"
		...
		ИЛИ Элемент.ТекущиеДанные.КолонкаРазделыОтчетаСокрНаим = "ПриложениеМЗЗ" 
...
Процедура ЗаполнитьШапкуПодписиПриложенияПН(Форма, НаимРаздела)
	...
	Если НЕ НаимРаздела = "ПриложениеКТЦ" И НЕ НаимРаздела = "ПриложениеМПЗ" И  НЕ НаимРаздела = "ПриложениеМЗЗ" Тогда 
...
Процедура ЗаполнитьСведенияОбОрганизации(Форма)
	...
	Если МесяцОтчета = 3 Тогда
		ЗаполнитьПоказательНаВсехЗакладках("H1KV", "X", Форма, "|ПриложениеТЦ|ПриложениеКТЦ|ПриложениеМПЗ|ПриложениеМЗЗ|");
	ИначеЕсли МесяцОтчета = 6 Тогда
		ЗаполнитьПоказательНаВсехЗакладках("HHY",  "X", Форма, "|ПриложениеТЦ|ПриложениеКТЦ|ПриложениеМПЗ|ПриложениеМЗЗ|");
	ИначеЕсли МесяцОтчета = 9 Тогда
		ЗаполнитьПоказательНаВсехЗакладках("H3KV", "X", Форма, "|ПриложениеТЦ|ПриложениеКТЦ|ПриложениеМПЗ|ПриложениеМЗЗ|");
	ИначеЕсли МесяцОтчета = 12 Тогда
		ЗаполнитьПоказательНаВсехЗакладках("HY",   "X", Форма, "|ПриложениеТЦ|ПриложениеКТЦ|ПриложениеМПЗ|ПриложениеМЗЗ|");
	КонецЕсли;
...
Процедура ПроставитьВКвадратыЗначенияНаВсехЗакладках(Форма, ИмяПоказателя, Длина, Значение, ДополнитьЗначение = Ложь, НольКакНоль = Ложь, СписокИсключений = "")
	...
	//КонецЕсли;
	//Если Найти(СписокИсключений, "|ПриложениеМПЗ|") = 0 Тогда
	//	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ПриложениеМПЗ", ИмяПоказателя,  Длина, Значение, ДополнитьЗначение, НольКакНоль);		
	//КонецЕсли;
	//Если Найти(СписокИсключений, "|ПриложениеМЗЗ|") = 0 Тогда
	//	РегламентированнаяОтчетностьКлиентСервер.ПроставитьВКвадратыЗначения(Форма, "ПриложениеМЗЗ", ИмяПоказателя,  Длина, Значение, ДополнитьЗначение, НольКакНоль);		
	//КонецЕсли;
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "", ОбластьИмя = "")
	...
	Для каждого СтрокаМЗ1 Из СтруктураМЗ1 Цикл
		...
		СтрокаМЗ1.MZT1RXXXXG6 =   СтрокаМЗ1.MZT1RXXXXG3
	...
	Для каждого СтрокаМЗ2 Из СтруктураМЗ2 Цикл
		...
		СтрокаМЗ2.MZT2RXXXXG6 =   СтрокаМЗ2.MZT2RXXXXG3
...
Процедура ОбработкаВыбораВПолеТабличногоДокумента(Элемент, Область, СтандартнаяОбработка)
   ...
   ИначеЕсли ИмяЯчейки = "HHY1"     //Приложение АМ
	...
	ИначеЕсли ИмяЯчейки >= "HD1" И ИмяЯчейки <= "HD9"  
		  ...
		  ИЛИ ИмяЯчейки >= "HD10" И ИмяЯчейки <= "HD12" 
		  ИЛИ ИмяЯчейки = "HD14" Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если Найти(Заголовок, СтруктураРеквизитовФормы.СтрПериодОтчета) = 0 Тогда
		Заголовок = Заголовок + " за " + СтруктураРеквизитовФормы.СтрПериодОтчета + " (форма 2кв.2022) ";
	КонецЕсли;
...
Процедура ЗаполнитьУбытокПрошлогоГода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Если ГодПериода = 2021 ИЛИ (ГодПериода = 2022 И Месяц(ДатаПрошлогоГода)=3) Тогда 
			ИмяСхемы1 = ИмяСхемы1 + "20";
			ИмяСхемы2 = "J0100520"; 
		Иначе  //c 1-го полугодия 2022
...
Процедура ЗаполнитьПоказателиПрошогоПериода(ПараметрыЗаполненияЗвит1С)
	...
	Иначе
		...
		Если ЮрЛицо Тогда  //с 1-го полугодия 2022
			ИмяСхемы = ?(Месяц(ДатаПрошлогоКвартала)>3, "J0100122","J0100120");  // Декларация о прибыли 
			Если ЗначениеЗаполнено(СтруктураДанныхДекларация.H091) Тогда
				// сельхозники
				ИмяСхемы = ?(Месяц(ДатаПрошлогоКвартала)>3, "J0100522","J0100520"); 
			КонецЕсли;
		Иначе
			ИмяСхемы = ?(Месяц(ДатаПрошлогоКвартала)>3, "F0100722","F0100720");   
		КонецЕсли;	
		...
		Если ЮрЛицо Тогда
			ИмяСхемы =  ?(Месяц(ДатаПрошлогоКвартала)>3, "J0111422","J0111420");  // Декларация о прибыли 
			Если ЗначениеЗаполнено(СтруктураДанныхДекларация.H091) Тогда
				// сельхозники
				ИмяСхемы = ?(Месяц(ДатаПрошлогоКвартала)>3, "J0151422","J0151420"); 
			КонецЕсли;
...
Функция ЗаполнитьТаблицуСоСхемамиВыгрузки()
	...
	Если ЮрЛицо Тогда
		...
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0100522", "J0100122");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151822", "J0111822");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150322", "J0110322");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152822", "J0112822");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0150622", "J0110622");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151322", "J0111322");  
		...
		НовСтрока = тбСхемыXML.Добавить(); 
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151422", "J0111422");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела1.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151522", "J0111522");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151622", "J0111622");  
		...
		Если НЕ ЗначениеЗаполнено(ПолучитьПоказательНаСервере("HZU", ЭтаФорма, "Декларация")) Тогда
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0151722", "J0111722");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0152022", "J0112022");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела2.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154422", "J0114422");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела3.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154522", "J0114522");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела4.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154622", "J0114622");  
		...
		Для  Инд = 1 ПО ТаблицаСтраницыРаздела5.Количество() Цикл
			...
			НовСтрока = тбСхемыXML.Добавить();
			НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0154722", "J0114722");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0114822", "J0154822");  
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = ?(ПроизводительСХ, "J0114922", "J0154922");  
