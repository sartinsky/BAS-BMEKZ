...
Процедура ПриОткрытии(Отказ)
	...
	Если Найти(Заголовок, СтруктураРеквизитовФормы.СтрПериодОтчета) = 0 Тогда
		Заголовок = Заголовок + " за " + СтруктураРеквизитовФормы.СтрПериодОтчета + " (форма 1кв.2022) ";
	КонецЕсли;
...
Процедура Заполнить(ПараметрыЗаполненияЗвит1С)
	...
	Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ДатаНачалаНаЕдиномНалоге) 
		...
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ДатаНачалаНаЕдиномНалоге) Тогда
			//был переход на ЕН 2%
			ДатаНачалаЕН = Макс(СтруктураРеквизитовФормы.ДатаНачалаНаЕдиномНалоге, ДатаНачалаЕН);
		КонецЕсли;	
		Если ЗначениеЗаполнено(СтруктураРеквизитовФормы.ДатаОкончанияНаЕдиномНалоге) Тогда
			//был переход с ЕН 2%
			ДатаОкончанияЕН = Мин(СтруктураРеквизитовФормы.ДатаОкончанияНаЕдиномНалоге,ДатаОкончанияЕН);
		КонецЕсли;
...
Процедура ЗаполнитьДанныеФинОтчетностиВП(НомерПериода)
	...
	Если СтруктураРеквизитовФормы.ВидФинОтчетности = 0 Тогда
		...
		Вал3 = ОкруглитьЧислоПоФорматуФИН(R001G3) - ОкруглитьЧислоПоФорматуФИН(ДО(Счет90_1) + ДО(Счет90_2) + ДО(Счет90_3) + ДО(Счет90_4));
		Вал3 = ?(Вал3 > 0, Вал3, -Вал3);  
		
		ДоходОперДеят3 = Вал3  + ОкруглитьЧислоПоФорматуФИН(КО(Счет71) - ОБ(Счет71,Счет64)) 
						- ОкруглитьЧислоПоФорматуФИН(ДО(Счет92) + ДО(Счет93) + ДО(Счет94));
		ДоходОперДеят3 = ?(ДоходОперДеят3 > 0, ДоходОперДеят3, -ДоходОперДеят3);
		
		Обыч3 = ДоходОперДеят3 + ОкруглитьЧислоПоФорматуФИН(КО(Счет72) + КО(Счет73) + КО(Счет74) - ОБ(Счет74,Счет64)) 
				- ОкруглитьЧислоПоФорматуФИН(ДО(Счет95) + ДО(Счет96) + ДО(Счет97));
		
		R002G3 = ?(Обыч3 > 0, Обыч3, -Обыч3);	 
		...
		_ДобавитьСтрокуРасшифровки( ЭтаФорма,"R002G3", "",	"+", "!КО",КО(Счет71), Счет71,,ДатаНачала, ДатаКонца, "Декларация");
		_ДобавитьСтрокуРасшифровки( ЭтаФорма,"R002G3", "",	"-", "!ОБ",ОБ(Счет71,  Счет64),Счет71,Счет64,ДатаНачала, ДатаКонца, "Декларация");	
		...
		_ДобавитьСтрокуРасшифровки( ЭтаФорма,"R002G3", "",	"+", "!КО",КО(Счет72), Счет72,,ДатаНачала, ДатаКонца, "Декларация");
		_ДобавитьСтрокуРасшифровки( ЭтаФорма,"R002G3", "",	"+", "!КО",КО(Счет73), Счет73,,ДатаНачала, ДатаКонца, "Декларация");
		_ДобавитьСтрокуРасшифровки( ЭтаФорма,"R002G3", "",	"+", "!КО",КО(Счет74), Счет74,,ДатаНачала, ДатаКонца, "Декларация");
		_ДобавитьСтрокуРасшифровки( ЭтаФорма,"R002G3", "",	"-", "!ОБ",ОБ(Счет74,Счет64),Счет74,Счет64,ДатаНачала, ДатаКонца, "Декларация");	
	...
	Иначе	//МПС и МП
		...
		R002G3 = ОкруглитьЧислоПоФорматуФИН(R001G3) - ОкруглитьЧислоПоФорматуФИН((ДО(Счет92) + ДО(Счет93) + ДО(Счет94) 
				+ ДО(Счет95) + ДО(Счет96) + ДО(Счет97)) + ДО(Счет90));
...
Процедура  ОпределитьСписокВыгружаемыхСхем(тбСхемыXML);
	...
	Для Каждого стр Из  тбСхемыXML Цикл
		...
		Если Лев(Стр.Префикс,2) = "MPZ"  Тогда
		...
		Если Лев(Стр.Префикс,2) = "MZZ"  Тогда
