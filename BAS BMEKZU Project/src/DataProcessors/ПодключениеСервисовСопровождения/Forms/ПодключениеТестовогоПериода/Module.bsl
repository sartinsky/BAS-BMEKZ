...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтотОбъект.Параметры.Свойство("АвтоТест") Тогда // Возврат при получении формы для анализа.
		Возврат;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Параметры.Идентификатор) Тогда
		ТекстИсключения = НСтр("ru='Не заполнен идентификатор сервиса сопровождения.';uk='Не заповнений ідентифікатор сервісу супроводу.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗавершениеРаботы Тогда
		Если ПоказатьВопросПриЗавершенииРаботыПрограммы() Тогда
			Отказ = Истина;
			ТекстПредупреждения = НСтр("ru='Подключение тестового периода сервиса не завершено.';uk='Підключення тестового періоду сервісу не завершено.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
...
Функция ПодключитьТестовыйПериодНаСервере(
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Подключение тестового периода (activate).';uk='Підключення тестового періоду (activate).'");
...
Функция ПроверитьПодключениеТестовогоПериодаНаСервере(
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Проверка состояния подключения тестового периода.';uk='Перевірка стану підключення тестового періоду.'");
...
Процедура ОтобразитьРезультатПодключения(ПараметрыОтображения)
	...
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключен") Тогда
		Элементы.ДекорацияНадписьПодключен.Заголовок = НСтр("ru='Сервис успешно подключен.';uk='Сервіс успішно підключений.'");
...
Процедура НастроитьОтображениеФормы(Форма)
	...
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.ПодключениеНедоступно") Тогда
		Элементы.ДекорацияИнформацияОбОшибке.Заголовок = НСтр("ru='Ошибка при получении доступных тестовых периодов:
            ...
            |Не виявлені доступні тестові періоди.'")
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступность(Форма, Знач СостояниеПодключения, Знач Подключение = Ложь)
	...
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.ОшибкаПодключения") Тогда
		...
		Элементы.ФормаПодключить.Заголовок           = НСтр("ru='Повторить попытку';uk='Повторити спробу'");
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключение") Тогда
		Элементы.СтраницыПодключения.ТекущаяСтраница = Элементы.ГруппаОшибкаТаймаут;
		Элементы.ФормаОтмена.Видимость               = Истина;
		Элементы.ФормаОтмена.Заголовок               = НСтр("ru='Закрыть';uk='Закрити'");
		Элементы.ФормаПодключить.Видимость           = Истина;
		Элементы.ФормаПодключить.Заголовок           = НСтр("ru='Проверить подключение';uk='Перевірити підключення'");
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.Подключен") Тогда
		Элементы.СтраницыПодключения.ТекущаяСтраница = Элементы.ГруппаПодключенТестовыйПериод;
		Элементы.ФормаПодключить.Видимость           = Истина;
		Элементы.ФормаОтмена.Видимость               = Ложь;
		Элементы.ФормаПодключить.Заголовок           = НСтр("ru='OK';uk='OK'");
	ИначеЕсли СостояниеПодключения = ПредопределенноеЗначение("Перечисление.СостоянияПодключенияСервисов.ПодключениеНедоступно") Тогда
		Элементы.ФормаПодключить.Видимость           = Ложь;
	ИначеЕсли ПодключениеДоступно(СостояниеПодключения) Тогда
		...
		Элементы.ФормаПодключить.Видимость           = Истина;
		Элементы.ФормаОтмена.Видимость               = Истина;
		Элементы.ФормаОтмена.Заголовок               = НСтр("ru='Отменить';uk='Скасувати'");
		Элементы.ФормаПодключить.Заголовок           = НСтр("ru='Подключить';uk='Підключити'");
	КонецЕсли;
...
//
// Параметры:
//  Форма                 - УправляемаяФорма - экземпляр формы
//                          ПодключениеСервисовСопровождения.ПодключениеТестовогоПериода;
...
Процедура ЗаполнитьДанныеШапкиФормы(ОписательСервиса)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Подключение сервиса %1';uk='Підключення сервісу %1'"),
	...
	ЧастиОписания.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Возможности сервиса:';uk='Можливості сервісу:'"), Новый Шрифт(,,Истина)));
	ЧастиОписания.Добавить(Новый ФорматированнаяСтрока(Символы.ПС));
	ЧастиОписания.Добавить(ОписательСервиса.Описание + " ");
	ЧастиОписания.Добавить(Новый ФорматированнаяСтрока("Подробнее", , , , ОписательСервиса.URLОписание));
	ЧастиОписания.Добавить(".");
	Элементы.ОписаниеСервиса.Заголовок = Новый ФорматированнаяСтрока(ЧастиОписания);
...
Процедура ДобавитьЭлементыТестовыхПериодов(ОписательПодключения, ОписательСервиса)
	...
	// Оформление тестовых периодов.
	ЧастиОписания = Новый Массив;
	ЧастиОписания.Добавить(Новый ФорматированнаяСтрока(НСтр("ru='Подключите ';uk='Підключіть '"), ЖирныйШрифт));
	ЧастиОписания.Добавить(Новый ФорматированнаяСтрока("бесплатно",ЖирныйШрифт,,, ОписательСервиса.URLУсловияПолучения));
	ЧастиОписания.Добавить(НСтр("ru=':';uk=':'"));
	
	// Создание новых элементов форм.
	ГруппаФормы  = Элементы.ГруппаДоступныеТестовыеПериоды;
	НовыйЭлемент = Элементы.Добавить(НСтр("ru='ЗаголовокТестовыеПериоды';uk='ЗаголовокТестовыеПериоды'"), Тип("ДекорацияФормы"), ГруппаФормы);
	НовыйЭлемент.Вид        = ВидДекорацииФормы.Надпись;
	НовыйЭлемент.Заголовок  = Новый ФорматированнаяСтрока(ЧастиОписания);
	...
	Если ОписательПодключения.ТестовыеПериоды.Количество() = 1 Тогда
		...
		ИмяПоляНаименования      = НСтр("ru='ТестовыйПериод1';uk='ТестовыйПериод1'");
		ИмяГруппыНаименования    = НСтр("ru='ГруппаНаименование1';uk='ГруппаНаименование1'");
		ИмяГруппыОписания        = НСтр("ru='ГруппаОписание1';uk='ГруппаОписание1'");
		ИмяПоляОписания          = НСтр("ru='Описание1';uk='Описание1'");
