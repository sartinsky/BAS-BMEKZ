...
Процедура УправлениеФормой(Форма)
	...
	Если НЕ ВозможностьВыбораФайлов Тогда
		...
		ТекстПояснения = НСтр("ru='По кнопке ""Загрузить данные"" укажите путь к файлу, 
                               |в который были выгружены данные из Бухгалтерии 7.7
                               |
                               |Файл должен быть предварительно сформирован в 
                               |Бухгалтерии 7.7 с помощью Помощника перехода';uk= 'По кнопці ""Завантажити дані"" вкажіть шлях до файлу,
                               |в який було проведене вивантаження даних із Бухгалтерії 7.7 
                               |
                               |Файл повинен бути попередньо сформований в 
                               |Бухгалтерії 7.7 за допомогою Помічника переходу'");
	Иначе
		ТекстПояснения = НСтр("ru='Укажите путь к файлу, в который были выгружены данные из 
                               |Бухгалтерии 7.7 и нажмите кнопку ""Загрузить данные""
                               |
                               |Файл должен быть предварительно сформирован в 
                               |Бухгалтерии 7.7 с помощью Помощника перехода';uk= 'Вкажіть шлях до файлу, в який було проведене вивантаження 
                               |даних із Бухгалтерії 7.7 і натисніть кнопку ""Завантажити дані""
                               |
                               |Файл повинен бути попередньо сформований в 
                               |Бухгалтерії 7.7 за допомогою Помічника переходу'");
	КонецЕсли;
...
Функция ВыполнитьЗагрузкуДанныхНаСервере(АдресФайла)
	...
	Иначе
		НаименованиеЗадания = НСтр("ru='Загрузка данных из Бухгалтерии 7.7';uk='Завантаження даних з Бухгалтерії 7.7'");
