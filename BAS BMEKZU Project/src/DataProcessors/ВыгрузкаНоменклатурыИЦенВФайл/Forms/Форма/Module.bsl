...
Процедура УстановитьПредставлениеСпискаКолонок(Форма)
	...
	Представление = НСтр("ru='Наименование';uk='Найменування'");
...
Процедура УстановитьПредставлениеОтбора(Форма)
	...
	Форма.ТекстОтборы = ?(Представление = "", НСтр("ru='Без отбора';uk= 'Без відбору'"), Представление);
...
Процедура ЗаписатьДанныеВФайлИОткрыть()
	...
	ТекстСообщения = НСтр("ru='Для сохранения файлов рекомендуется установить расширение для веб-клиента BAS Бухгалтерия.';uk='Для збереження файлів рекомендується встановити розширення для веб-клієнта BAS Бухгалтерія.'");
...
Процедура ВыгрузитьВФайлЗавершение(Подключено, ОписаниеФайла) Экспорт
	...
	ДиалогСохраненияФайла.Фильтр = НСтр("ru='Лист Excel|*.xls';uk= 'Лист Excel|*.xls'");
...
Процедура ВыгрузитьФайлВыборФайлаЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПолученныеФайлы = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru='При сохранении возникла ошибка';uk='При збереженні виникла помилка'"));
	Иначе
		// Открыть Файл
...
Процедура УдалитьВременныйФайлНаСервер(ИмяФайла)
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось удалить временный файл по причине: %1';uk='Не вдалося вилучити тимчасовий файл з причини: %1'"),
			ПодробноеПредставлениеОшибки(СведенияОбОшибке));
		
		ЗаписьЖурналаРегистрации(НСтр("ru='Шаблоны договоров. Сохранение текста договора.';uk='Шаблони договорів. Збереження тексту договору.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Функция ПоместитьФайлВоВременноеХранилищеНаСервере()
	...
	ИмяФайла = НСтр("ru='Номенклатура и цены.xls';uk= 'Номенклатура і ціни.xls'") ;
