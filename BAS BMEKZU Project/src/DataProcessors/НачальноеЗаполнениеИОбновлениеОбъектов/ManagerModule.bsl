...
Функция СформированиеЗначениеСсылочногоТипа(ТипРеквизита, ЗначениеРеквизита, СоздаватьЭлементыСправочников = Ложь)
	...
	Если НЕ ПустаяСтрока(ЗначениеРеквизита) Тогда
		...
		Если Значение = Неопределено И СоздаватьЭлементыСправочников = Истина Тогда
			Если (Менеджер = Справочники.Номенклатура ИЛИ Менеджер = Справочники.БиологическиеАктивы) // ИНАГРО 
...
Процедура ЗаполнитьПланВидовРасчета(ИмяОбъекта,ЗаполнениеСинонимовОбъектов = Ложь, ОтборИмя = "") Экспорт
	...
	//ЕстьМакетБВВ = МетаданныеОбъекта.Макеты.Найти(ИмяМакета);   // ИНАГРО -- Похоже, тут ошибка в типовом механизме
	ЕстьМакетБВВ = МетаданныеОбъекта.Макеты.Найти(ИмяМакетаБВВ);  // ИНАГРО ++
...
|Об''єкт не заповнений!'"), ИмяОбъекта, ИмяРеквизита));
	...
	Для Каждого СтрокаДанныхНачальногоЗаполнения Из ТаблицаДанныхНачальногоЗаполнения Цикл
		...
		// ИНАГРО++ 
		// Заполняем только те предопределенные элементы, которые нужно заполнять
		Если ОтборИмя <> "" Тогда
			Если ИмяПредопределенного <> ОтборИмя Тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;	
		// ИНАГРО --
