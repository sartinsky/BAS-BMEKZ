...
Функция ПолучитьИнформациюОбОбновленияхТекущегоРелиза() Экспорт
	...
	Результат = ОнлайнСервисыРегламентированнойОтчетности.ПолучитьДанныеМеханизмаОнлайнСервисовРО(Перечисления.ТипыРесурсовМеханизмаОнлайнСервисовРО.ИнформацияОРелизе);
	...
	ДеревоИнформацииТекущегоРелиза = ОнлайнСервисыРегламентированнойОтчетности.ЗагрузитьСтрокуXMLВДеревоЗначений(ДанныеТекущегоРелиза);
	Если НЕ ЗначениеЗаполнено(ДеревоИнформацииТекущегоРелиза) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	// проверяем, "понятная" ли редакция у справочной информации
	Если НЕ ОнлайнСервисыРегламентированнойОтчетности.РедакцияФорматаСправочнойИнформацииПрименима(ДеревоИнформацииТекущегоРелиза) Тогда
		Возврат Неопределено;
	КонецЕсли;
	
	// проверяем, точно ли для текущего релиза информация
	Если НЕ ОнлайнСервисыРегламентированнойОтчетности.ИнформацияОРелизеСоответствуетТекущейПрограмме(ДеревоИнформацииТекущегоРелиза) Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если УзелФайл <> Неопределено Тогда
		...
		Если УзлыРелиз.Количество() > 0 Тогда
			...
			Если ОнлайнСервисыРегламентированнойОтчетности.ФайлИнформацииОРелизеСоответствуетТекущейПрограмме(УзелРелиз) Тогда
				
				// заполняем свойства текущего релиза
				АтрибутыУзла = ОнлайнСервисыРегламентированнойОтчетности.ПолучитьАтрибутыУзла(УзелРелиз);
				...
				Если УзлыДочерниеВыпуски.Количество() > 0 Тогда
					...
					Для Каждого УзелВыпуск Из УзлыВыпуск Цикл
						АтрибутыУзла = ОнлайнСервисыРегламентированнойОтчетности.ПолучитьАтрибутыУзла(УзелВыпуск);
						...
						КонецЕсли;
						СтрВыпуск.Порядок = ОнлайнСервисыРегламентированнойОтчетности.ПорядокПоНомеруВерсии(СтрВыпуск.Версия);
						...
						Если УзлыОбъекты.Количество() > 0 Тогда
							...
							Для Каждого УзелОбъект Из УзлыОбъект Цикл
								АтрибутыУзлаОбъект = ОнлайнСервисыРегламентированнойОтчетности.ПолучитьАтрибутыУзла(УзелОбъект);
				...
				Если УзлыРелизыОбновления.Количество() > 0 Тогда
					...
					Для Каждого УзелРелизОбновление Из УзлыРелизОбновление Цикл
						АтрибутыУзла = ОнлайнСервисыРегламентированнойОтчетности.ПолучитьАтрибутыУзла(УзелРелизОбновление);
