...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоПользовательСПолнымиПравами   = Пользователи.ЭтоПолноправныйПользователь(, Ложь);
	...
	Если НЕ (Пользователи.РолиДоступны("НСИСНастройка") ИЛИ ЭтоПользовательСПолнымиПравами) Тогда
	...
	Если НЕ (ЭтоПользовательСПолнымиПравами) Тогда
		Элементы.ОпендатаботГруппаНастройки.Видимость = Ложь;
	Иначе
		ОпендатаботКлючАПИ = ОпендатаботСервер.ПолучитьКлючАПИ();
	КонецЕсли;	
...
&НаСервере
Процедура ОпендатаботКлючАПИПриИзмененииСервер()
	
	ОпендатаботСервер.УстановитьКлючАПИ(ОпендатаботКлючАПИ);
	
КонецПроцедуры

&НаКлиенте
Процедура ОпендатаботКлючАПИПриИзменении(Элемент)
	
	ОпендатаботКлючАПИПриИзмененииСервер();
	
КонецПроцедуры

&НаСервере
Функция ОпендатаботПроверитьКлючАПИНаСервере(ОпендатаботКлючАПИ)
	
	ИнформацияОКлюче = "";
	Если Не ПустаяСтрока(ОпендатаботКлючАПИ) Тогда
		СтруктураИнформацииОКлюче = ОпендатаботСервер.ПолучитьРезультатПроверкиКлюча(ОпендатаботКлючАПИ);
		ИнформацияОКлюче = СтруктураИнформацииОКлюче.ИнформацияОКлюче;
	КонецЕсли;
	
	Возврат ИнформацияОКлюче;
	
КонецФункции

&НаКлиенте
Процедура ОпендатаботПроверитьКлючАПИ(Команда)
	
	ИнформацияОКлюче = ОпендатаботПроверитьКлючАПИНаСервере(ОпендатаботКлючАПИ);
	Элементы.ОпендатаботКлючАПИРасширеннаяПодсказка.Заголовок = ИнформацияОКлюче; 
