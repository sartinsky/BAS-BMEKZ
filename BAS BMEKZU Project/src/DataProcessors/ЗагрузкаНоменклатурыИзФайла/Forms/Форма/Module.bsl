...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(АдресФайла) Или Не ЭтоАдресВременногоХранилища(АдресФайла) Тогда
		ТекстИсключения = НСтр("ru='Не выбран файл для загрузки.
            ...
            |;uk='Не вибрано файл для завантаження.
            |Відкрийте обробку з форми списку номенклатури за допомогою команди ""Завантажити"".'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	
	// Установка значений реквизитов для новой номенклатуры по умолчанию
	ЕдиницаИзмерения     = ?(ЗначениеЗаполнено(ЕдиницаИзмерения), ЕдиницаИзмерения, Справочники.КлассификаторЕдиницИзмерения.ПолучитьЕдиницуИзмеренияПоУмолчанию());
	СтавкаНДС            = ?(ЗначениеЗаполнено(СтавкаНДС), СтавкаНДС, Перечисления.СтавкиНДС.НДС20);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого СтрокаТаблицы Из Объект.Товары Цикл
		...
		Если СтрокаТаблицы.Статус = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусДубль() Тогда // обнаружены дубли
			...
			ТекстСообщения = НСтр("ru='Найдено несколько позиций номенклатуры с совпадающими реквизитами. Двойным кликом выберите одну из списка.';uk='Знайдено кілька позицій номенклатури з однаковими реквізитами. Подвійним натисненням виберіть одну зі списку.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,
	...
	Пока Выборка.Следующий() Цикл
		...
		ШаблонСообщения = НСтр("ru='Строка с номенклатурой ""%1"" уже существует в строке %2.';uk='Рядок з номенклатурою ""%1 вже існує в рядку %2.'");
...
Процедура НадписьСсылкиУстановкиРеквизитовНажатие(Элемент, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("ЕдиницаИзмерения", ЭтотОбъект.Объект.Товары[0].ЕдиницаИзмерения);
	ПараметрыФормы.Вставить("СтавкаНДС", ЭтотОбъект.Объект.Товары[0].СтавкаНДС);
...
Процедура ДанныеФайлаВыборЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Для НомерКолонки = 1 По ДанныеФайла.ШиринаТаблицы Цикл
		...
		Если Область.ПараметрРасшифровки = РезультатЗакрытия.Идентификатор
			Или (ВыбранаКолонкаСЦеной И СтрНайти(Область.Текст, НСтр("ru='Цена';uk='Ціна'")) > 0) Тогда
...
Процедура ТоварыНоменклатураАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ТекДанные.Статус = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.СтатусНовый() Тогда // будет создана новая номенклатура
		...
		ТекстПодбора = НСтр("ru='Создать: ';uk='Створити: '") + ТекДанные.Наименование 
...
Процедура ТоварыНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Строка") И СтрНайти(ВыбранноеЗначение, НСтр("ru='Создать:';uk='Створити:'")) > 0 Тогда
...
Процедура УдалитьЛишнее(Команда)
	...
	ТекстВопроса = НСтр("ru='Найти и удалить строки и колонки, не содержащие информацию для загрузки?';uk='Знайти і вилучити рядки та колонки, які не містять інформацію для завантаження?'");
...
Процедура Назад(Команда)
	...
	Если ТабличнаяЧастьИзменена Тогда
		...
		ТекстВопроса = НСтр("ru='Все изменения, произведенные вами в таблице, будут потеряны.
                  ...
                  |;uk='Всі зміни, внесені вами в таблиці, будуть втрачені.
                  |
...
Процедура ИзменитьТекстПереключателяОтбора()
	...
	СписокВыбора[0].Представление = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.ТекстСКоличеством(НСтр("ru='Все';uk='Всі'"), Статистика.Всего);
	СписокВыбора[1].Представление = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.ТекстСКоличеством(НСтр("ru='Найденные';uk='Знайдені'"), Статистика.Найденные);
	СписокВыбора[2].Представление = ЗагрузкаДанныхИзВнешнихФайловКлиентСервер.ТекстСКоличеством(НСтр("ru='Новые';uk='Нові'"), Статистика.Новые);
...
Процедура ЗаголовокСсылкиУстановкиРеквизитов()
	...
	СписокРеквизитов = "Родитель,НоменклатурнаяГруппа"
	...
	КонецЦикла;
	ЗаголовокСсылки = НСтр("ru='Реквизиты новой номенклатуры: ';uk='Реквізити нової номенклатури: '");
...
Функция ПодготовитьТабличныйДокументСервер(Знач АдресФайла, Знач РасширениеФайла, Знач ИдентификаторФормы, Знач ОписаниеКолонок)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Подготовка табличного документа к загрузке номенклатуры.';uk='Підготовка табличного документа до завантаження номенклатури.'");
...
Функция УдалитьЛишнееСервер()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Удаление незначимых строк табличного документа.';uk='Вилучення неважливих рядків табличного документа.'");
...
Процедура УдалитьЛишнееЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		...
		Если НЕ НайденыНенужныеСтроки Тогда
			...
			ТекстСообщения = НСтр("ru='Строк и колонок, не содержащих информацию для загрузки, не обнаружено.';uk='Рядків і колонок, що не містять інформацію для завантаження, не виявлено.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			
		Иначе
			
			ТекстСообщения = НСтр("ru='Удаление завершено.';uk='Вилучення завершене.'");
			ТекстПояснения = НСтр("ru='Удаление незначимых данных завершено.';uk='Вилучення неважливих даних завершено.'");
...
Функция ПодготовитьТаблицуТоваровСервер()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Подготовка таблицы товаров при загрузке номенклатуры.';uk='Підготовка таблиці товарів при завантаженні номенклатури.'");
...
Функция ЗагрузитьНаСервере()
	...
	ПараметрыЗадания.Вставить("Услуга", Ложь);
	ПараметрыЗадания.Вставить("ТипЦен", ТипЦен);
	ПараметрыЗадания.Вставить("ДатаУстановкиЦен", ДатаУстановкиЦен);
	
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Загрузка данных файла.';uk='Завантаження даних файлу.'");
...
Процедура ЗагрузкаДанныхИзФайлаЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		...
		Если Ошибки <> Неопределено Тогда
			...
			ТекстПредупреждения = НСтр("ru='При загрузке данных произошли ошибки.';uk='При завантаженні даних виникли помилки.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю(Ошибки);
			
		Иначе
			
			ПоказатьОповещениеПользователя(НСтр("ru='Загрузка номенклатуры и цен произведена.';uk='Завантаження номенклатури і цін проведено.'"));
...
Функция ПолучитьДанныеНоменклатуры(Знач Номенклатура)
	...
	ДанныеНоменклатуры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "БазоваяЕдиницаИзмерения, СтавкаНДС");
