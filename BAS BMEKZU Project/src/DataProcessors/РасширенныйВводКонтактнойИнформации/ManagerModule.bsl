...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если Параметры <> Неопределено И Параметры.Свойство("ОткрытаПоСценарию") Тогда
		...
		Если ВыбраннаяФорма = Неопределено Тогда
			ВызватьИсключение НСтр("ru='Не обрабатываемый тип адреса:';uk='Не оброблюваний тип адреси:'")+" """ + ВидИнформации + """";
		КонецЕсли;
...
Функция СписокРеквизитовНаселенныйПункт(СведенияОбАдресномОбъекте = Неопределено) Экспорт
	...
	ЗаполнитьЗначенияСвойств(Адрес, АдресныеСведения.Данные, "addressType,id,ZIPCode,value");
...
Функция СписокАвтоподбораНаселенногоПункта(Текст, ДополнительныеПараметры) Экспорт
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.АдресныйКлассификатор") Тогда
		...
		Результат.КраткоеПредставлениеОшибки = НСтр("ru='Адресные сведения отсутствуют.';uk='Адресні відомості відсутні.'");
		Возврат Результат;
	КонецЕсли;
...
// Возвращает извлеченный XDTO национального адреса или Неопределено для адреса иностранного.
//
//  Параметры:
//      ОбъектИнформации - ОбъектXDTO - Контактная информация или XDTO адреса.
//
//  Возвращаемое значение:
//      ОбъектXDTO - национальный адрес.
//      Неопределено - нет национального адреса.
//
...
Функция ОшибкиЗаполненияАдресаXDTO(Знач Адрес, ВидИнформации, РезультатГруппами = Ложь) Экспорт
	...
	Если НЕ УправлениеКонтактнойИнформациейСлужебный.ЭтоНациональныйАдрес(Адрес) Тогда
		...
		// Адрес за пределами Украины
		Если ФлагиПроверки.ТолькоНациональныйАдрес Тогда
			ТекстОшибки = НСтр("ru='Адрес должен быть только украинским.';uk='Адреса повинна бути тільки українська.'");
...
Процедура ТолькоЛатиницаВСтроке(Значение, Знач Результат, Знач РезультатГруппами)
	...
	Если Не СтроковыеФункцииКлиентСервер.ТолькоЛатиницаВСтроке(Значение, Ложь, "1234567890,") Тогда
		ТекстОшибки = НСтр("ru='Адрес в международном формате должен быть набран латиницей.';uk='Адреса в міжнародному форматі повинна бути набрана латиницею.'");
		Если РезультатГруппами Тогда
			Результат.Добавить(Новый Структура("Поля, ТипОшибки, Сообщение", Новый Массив,
...
Функция ОбщиеГруппыОшибокЗаполненияАдреса(Знач ДанныеАдреса, Знач ВидИнформации)
	...
	Если ПустаяСтрока(Регион) Тогда
		ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Регион",
			НСтр("ru='Не указана область.';uk='Не зазначена область.'"), "Регион");	
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Регион) Тогда
		...
		Если ПустаяСтрока(ДанныеАдреса.AreaType) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеУказаныСокращенияПолей, "Регион",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не указано сокращение в названии области ""%1"".';uk='Не вказано скорочення в назві області ""%1"".'"), Регион), НСтр("ru='Область';uk='Область'"));
		КонецЕсли;
		Если СтрДлина(Регион) > 50 Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Название области ""%1"" должно быть короче 50 символов.';uk='Назва області ""%1"" повинно бути коротше 50 символів.'"),Регион), НСтр("ru='Область';uk='Область'"));
		КонецЕсли;
		Если Не СтроковыеФункцииКлиентСервер.ТолькоКириллицаВСтроке(Регион, Ложь, ДопустимоКромеКириллицы) Тогда
			ДобавитьОшибкуЗаполненияАдреса(НедопустимыеСимволыПолей, Поле,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В названии области ""%1"" есть не кириллические символы.';uk='У назві області ""%1"" є не кириличні символи.'"), Регион), НСтр("ru='Область';uk='Область'"));
		КонецЕсли
	КонецЕсли;
	...
	Если Не ПустаяСтрока(ДанныеАдреса.ZipCode) Тогда
		Поле = "Индекс";
		Если СтрДлина(ДанныеАдреса.ZipCode) <> 5 Или Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ДанныеАдреса.ZipCode) Тогда	
			ДобавитьОшибкуЗаполненияАдреса(НеСоответствуетДлинаПолей, Поле,
				НСтр("ru='Почтовый индекс должен состоять из 5 цифр.';uk='Поштовий індекс повинен складатися з 5 цифр.'"),
	...
	Если ПустаяСтрока(Город) И ПустаяСтрока(НаселенныйПункт) И ПустаяСтрока(Район) Тогда
		Если НазванияГородовФедеральногоЗначения().Найти(ВРег(Регион)) = Неопределено Тогда
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "Город",
				НСтр("ru='Город может быть не указан только в регионе - городе федерального значения.';uk='Місто може бути не зазначене тільки в регіоні - місті федерального значення.'"),
				НСтр("ru='Город';uk='Місто'"));
			ДобавитьОшибкуЗаполненияАдреса(НеЗаполненыОбязательныеПоля, "НаселПункт",
				НСтр("ru='Населенный пункт может быть не указан только в регионе - городе федерального значения.';uk='Населений пункт може бути не зазначений тільки в регіоні - місті федерального значення.'"),
				НСтр("ru='Населенный пункт';uk='Населений пункт'"));
		КонецЕсли;
	КонецЕсли;
...
//        ** Регион           - Строка - текстовое представление региона (только для национальных адресов).
...
//        ** Район            - Строка - текстовое представление района (только для национальных адресов).
//        ** РайонСокращение  - Строка - сокращение района (если СтарыйСоставПолей = Ложь).
//        ** Город            - Строка - текстовое представление города (только для национальных адресов).
//        ** ГородСокращение  - Строка - сокращение города (только для национальных адресов).
//        ** НаселенныйПункт  - Строка - текстовое представление населенного пункта (только для национальных адресов).
//        ** НаселенныйПунктСокращение - сокращение населенного пункта (если СтарыйСоставПолей = Ложь).
//        ** ТипДома          - Строка - см. РаботаСАдресамиКлиентСервер.ТипыОбъектовАдресацииАдреса().
//        ** Дом              - Строка - текстовое представление дома (только для национальных адресов).
//        ** ТипКорпуса       - Строка - см. РаботаСАдресамиКлиентСервер.ТипыОбъектовАдресацииАдреса().
//        ** Корпус           - Строка - текстовое представление корпуса (только для национальных адресов).
//        ** ТипКвартиры      - Строка - см. РаботаСАдресамиКлиентСервер.ТипыОбъектовАдресацииАдреса().
//        ** Квартира         - Строка - текстовое представление квартиры (только для национальных адресов).
...
Функция АдресВСтарыйСписокПолей(XDTOАдрес, РасширенныйСоставПолей = Истина)
	...
	Иначе
		...
		ЗначениеКорпуса = "";
		Разделитель = "";
		Сокращения = РаботаСАдресамиКлиентСервер.СокращенияОбъектовАдресацииАдреса();
		Для Каждого СтрокаОбъекта Из ПараметрыОбъекта Цикл
