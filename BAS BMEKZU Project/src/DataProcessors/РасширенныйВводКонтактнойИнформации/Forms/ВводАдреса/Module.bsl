...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	АвтоподборДоступен    = КлассификаторДоступен Или ЕстьКлассификатор;
	...
	Если ТолькоНациональныйАдрес Тогда
		...
		Если Страна <> ОсновнаяСтрана Тогда
            ...
            |;uk='Адресу введено некоректно: допускається введення тільки національних адрес.
            |Значення поля ""Країна"" було змінено на %1, необхідно перевірити інші поля.'");
	...
	Иначе
		Элементы.ГруппаИнформация.Видимость = Не АдресныеСведенияКлассификатораЗагружены(НаименованиеРегиона(НаселенныйПунктДетально)).РегионЗагружен;
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда // Временное решение для работы в мобильном клиенте, будет удалено в следующих версиях
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "НаселенныйПункт", "Заголовок", НСтр("ru='Город';uk='Місто'"));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "АдресныйКлассификаторАктуализирован" ИЛИ ИмяСобытия = "ЗагруженАдресныйКлассификатор" Тогда
		...
		Если КлассификаторДоступен Или АдресныеСведенияЗагружены.РегионЗагружен Тогда
			Элементы.ГруппаИнформация.Видимость            = Ложь;
...
Процедура ИндексИзменениеТекстаРедактирования(Элемент, Текст, СтандартнаяОбработка)
	...
	Если СтрДлина(СокрЛП(Текст)) = 5 И СтрСравнить(Индекс, Текст) <> 0 И Страна = ОсновнаяСтрана Тогда	
		СтандартнаяОбработка = Ложь;
...
Процедура ПослеВыбораУлицы(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ОбработкаИзмененияАдреса();
	Иначе
		ТекстПредупреждения = НСтр("ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о улицах для введенного адреса.';uk='Вибір зі списку недоступний, т.я.  в адресному класифікаторі відсутня інформація про вулицях для введеного адреси.'");
	КонецЕсли;
...
Процедура АдресНаДатуАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если СтрСравнить(Текст, НСтр("ru='начало учета';uk='початок обліку'")) = 0 Или ПустаяСтрока(Текст) Тогда
...
Процедура АдресНаДатуПриИзменении(Элемент)
	...
	Если Не ВводНовогоАдреса Тогда
		...
		Если ЗначениеЗаполнено(Результат.ДействуетПо) Тогда
			ТекстИсторическийАдрес = " " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='действует по %1';uk='діє по %1'"), Формат(Результат.ДействуетПо - 10, "ДЛФ=DD"));
		Иначе
			ТекстИсторическийАдрес = НСтр("ru='действует по настоящее время.';uk='діє по теперішній час.'");
		КонецЕсли;
	...
	ТекстНачалаУчета = НСтр("ru='начало учета';uk='початок обліку'");
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") И ВыбранноеЗначение.Отказ Тогда
		
		Если ЗначениеЗаполнено(ВыбранноеЗначение.КраткоеПредставлениеОшибки) Тогда
			КраткоеПредставлениеОшибки = ВыбранноеЗначение.КраткоеПредставлениеОшибки;
		Иначе
			КраткоеПредставлениеОшибки = НСтр("ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о адресных сведениях.';uk='Вибір зі списку недоступний, т.я. в адресному класифікаторі відсутня інформація про адресні дані.'");
		КонецЕсли;
...
Процедура ПереопределитьТипаАдресаПоСтране(Страна, НаселенныйПунктДетально)
	...
	Если РаботаСАдресамиКлиентСервер.ЭтоОсновнаяСтрана(Страна) Тогда
		Если СтрСравнить(НаселенныйПунктДетально.AddressType, УправлениеКонтактнойИнформациейКлиентСервер.ИностранныйАдрес()) = 0 Тогда
			НаселенныйПунктДетально.AddressType = РаботаСАдресамиКлиентСервер.АдминистративноТерриториальныйАдрес();
		КонецЕсли;
	Иначе
		НаселенныйПунктДетально.AddressType = УправлениеКонтактнойИнформациейКлиентСервер.ИностранныйАдрес();
...
Процедура ПоказатьСписокДомов(ВключитьОтбор)
	...
	Иначе
		Если ЗначениеЗаполнено(ПредставлениеАдреса) Тогда
			ТекстПредупреждения = НСтр("ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о нумерации домов для введенного адреса.';uk='Вибір зі списку недоступний, т.я. в адресному класифікаторі відсутня інформація про нумерацію будинків для введеної адреси.'");
		Иначе
			ТекстПредупреждения = НСтр("ru='Для просмотра списка домов заполните поля адреса.';uk='Для перегляду списку будинків заповніть поля адреси.'");
		КонецЕсли;
...
Процедура ВопросОбОбновлениеАдресногоКлассификатора(Знач СообщениеОПроверкеАдреса)
	...
	Кнопки.Добавить("ОбновитьКлассификатор", НСтр("ru='Обновить классификатор';uk='Оновити класифікатор'"));
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Если ЭтоНациональныйАдрес Тогда
		...
		Элементы.Улица.Заголовок = НСтр("ru='Улица';uk='Вулиця'");
		ВебСервисИспользуется                        = КлассификаторДоступен;
		АвтоподборДоступен                           = КлассификаторДоступен Или ЕстьКлассификатор;
	...
	Иначе
		...
		Элементы.Улица.Заголовок     = НСтр("ru='Адрес';uk='Адреса'");
	...
	// Проверять, вводить в свободной форме и искать по индексу можем только национальные адреса.
	Элементы.Улица.МногострочныйРежим                          = НЕ ЭтоНациональныйАдрес;
	Элементы.ГруппаСтроенияИИнформация.Видимость               = ВидимостьСтроений;
	Элементы.Улица.КнопкаВыбора                                = ЭтоНациональныйАдрес И ЕстьКлассификатор;
	Элементы.Дом.КнопкаВыбора                                  = ЭтоНациональныйАдрес И ЕстьКлассификатор;
	Элементы.ПроверитьЗаполнениеАдреса.Видимость               = ЭтоНациональныйАдрес И АвтоподборДоступен;
	Элементы.ЗаполнитьПоПочтовомуИндексу.Доступность           = ЭтоНациональныйАдрес И ЕстьКлассификатор;
...
Процедура ОпределитьОтображениеЭлементовНаФорме()
	...
	Если Не ЕстьКлассификатор Тогда
		...
		Элементы.Строение.КнопкаВыбора                            = Ложь;
		Элементы.НаселенныйПункт.ПодсказкаВвода                   = НСтр("ru='Введите название населенного пункта';uk='Введіть назву населеного пункту'");
		Элементы.Улица.ПодсказкаВвода                             = НСтр("ru='Введите название улицы';uk='Введіть назву вулиці'");
	КонецЕсли;
	
	Если КлассификаторДоступен Или МеждународныйФорматАдреса Тогда
...
Процедура УстановитьСписокВыбораЭлемента(ЭлементВид, ЭлементЗначение, Данные)
	...
	Если ЭлементВид.КнопкаВыпадающегоСписка Тогда
		Представления = РаботаСАдресамиКлиентСервер.ПредставленияОбъектовАдресации();
		ЭлементВид.СписокВыбора.Очистить();
		Для каждого Элемент Из СписокТипов Цикл
			ЭлементВид.СписокВыбора.Добавить(Элемент, Представления.Получить(Элемент))
		КонецЦикла;
	КонецЕсли;
...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС)
	...
	Если ВводНовогоАдреса Тогда
		ТекстИсторическийАдрес = НСтр("ru='';uk=''");
	...
	Элементы.ТекстПроДатуДействия.Заголовок = ТекстИсторическийАдрес;
	Элементы.АдресНаДату.ФорматРедактирования = ?(ЗначениеЗаполнено(АдресНаДату), "", "ДФ='""" + НСтр("ru='начало учета';uk='початок обліку'") + """'");
...
Процедура ОтобразитьКоличествоЗаписейВИсторииИзменений()
	...
	Если НайденныеСтроки.Количество() > 1 Тогда
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='История изменений (%1)';uk='Історія змін (%1)'"), НайденныеСтроки.Количество());
	...
	Иначе
		Элементы.ИсторияИзмененийГиперссылка.Заголовок = НСтр("ru='История изменений';uk='Історія змін'");
		Элементы.ИсторияИзмененийГиперссылка.Видимость = Истина;
	КонецЕсли;
...
Процедура СообщитьОбОшибкахЗаполнения(СписокОшибок, ПредупреждатьОбОтсутствии, ТребуетсяОбновление = Ложь)
	...
	Если КоличествоОшибок = 0 И ПредупреждатьОбОтсутствии Тогда
		...
		Если НЕ ВебСервисИспользуется И ТребуетсяОбновление Тогда
			Если ЕстьПравоЗагружатьКлассификатор Тогда
                ...
                |;uk='Адреса введена коректно, але адресні відомості, завантажені у програму, давно не оновлювалися.
                |Рекомендується виконати оновлення і повторити перевірку адреси заново.'");
			...
			Иначе
                ...
                |;uk='Адресу введено коректно, але адресні відомості, завантажені у програму,
                |давно не оновлювались. Рекомендується виконати оновлення і повторити перевірку адреси заново.
                |Для оновлення адресного класифікатора необхідно звернутися до адміністратора.'");
	...
	Если КоличествоОшибок = 1 Тогда
		...
		Если СписокОшибок[0].Пометка = Ложь И НЕ ВебСервисИспользуется Тогда
			ТекстВопроса = НСтр("ru='Адрес не может быть проверен.';uk='Адреса не може бути перевірена.'") + Символы.ПС 
				+ СписокОшибок[0].Представление + Символы.ПС + НСтр("ru='Загрузить классификатор для проверки адреса?';uk='Завантажити класифікатор для перевірки адреси?'");
...
Процедура НаселенныйПунктУстановитьПоляАдреса(НаселенныйПунктДетально, Знач ВыбранноеЗначение)
	...
	ИменаУровнейНаселенногоПункта = РаботаСАдресамиКлиентСервер.ИменаУровнейАдреса("Все", ЗначениеЗаполнено(ПолученныйАдрес.street));
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если ДоступностьКлассификатора <> Неопределено Тогда
			...
			Если ДоступностьКлассификатора.Отказ Тогда
				ТекстСообщенияСервиса = НСтр("ru='Автоподбор и проверка адреса недоступны:';uk='Автоматичний вибір та перевірка адреси недоступні:'") + Символы.ПС 
...
Функция ПроверитьДоступностьКлассификатораВФоне(Знач УникальныйИдентификатор)
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru='Проверка доступности сервиса адресного классификатора';uk='Перевірка доступності сервісу адресного класифікатора'");
...
//
&НаКлиенте
Процедура ПредложениеЗагрузкиКлассификатора(Знач Текст = "", Знач ВыбранноеЗначение = Неопределено)
...
Функция НаименованиеПриДобавлениеПроизвольногоПомещения()
	Возврат НСтр("ru='Другое';uk='Інше'");
КонецФункции
...
Процедура СформироватьМенюДобавленияСтроенийИПомещений()
	...
	Для каждого СтроениеИлиПомещение Из ДополнительныеСтроенияИПомещения Цикл
		...
		Если ЗначениеЗаполнено(СтроениеИлиПомещение.Ключ) Тогда
			...
			Команда.Подсказка                 = НСтр("ru='Добавить';uk='Додати'") + " " + НРег(СтроениеИлиПомещение.Значение);
