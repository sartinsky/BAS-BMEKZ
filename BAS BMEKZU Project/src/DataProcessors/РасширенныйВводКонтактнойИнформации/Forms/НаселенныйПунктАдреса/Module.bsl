...
&НаКлиенте
Процедура РегионАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)	
...
Процедура ОтобразитьПоляПоТипуАдреса()
	...
	Элементы.Регион.Заголовок = НСтр("ru='Область';uk='Область'");
...
Функция АвтоПодборВариантов(Текст, ИмяУровня, ТипАдреса, НаселенныйПунктДетально)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("АдресныйКлассификаторСлужебный") Тогда
...
Процедура ОбработкаВыбора(Элемент, Знач ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(ВыбранноеЗначение) Тогда
		ТекстПредупреждения = НСтр("ru='Выбор из списка недоступен, т.к в адресном классификаторе отсутствует информация о адресных сведениях.';uk='Вибір зі списку недоступний, т.я. в адресному класифікаторі відсутня інформація про адресні дані.'");
	...
	СведенияОбАдресе.Вставить("Идентификатор", ВыбранноеЗначение.Идентификатор);
	...
	Если ЕстьПравоЗагружатьКлассификатор И Не КлассификаторДоступен И ВыбранноеЗначение.ПредлагатьЗагрузкуКлассификатора Тогда
		...
		ПредложениеЗагрузкиКлассификатора(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Данные для ""%1"" не загружены.';uk='Дані для ""%1"" не завантажені.'"), ВыбранноеЗначение.Представление),
