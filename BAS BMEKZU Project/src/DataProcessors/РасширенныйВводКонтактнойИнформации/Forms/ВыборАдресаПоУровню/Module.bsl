...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Родитель) И Уровень > 1 Тогда
		...
		КраткоеПредставлениеОшибки = НСтр("ru='Поле не содержит адресных сведений для выбора.';uk='Поле не містить адресних даних для вибору.'");
		Возврат;
	КонецЕсли;
...
Процедура ОповеститьВладельца(Знач Данные, Отказ = Ложь)
	...
	Иначе
		...
		Если Уровень = 7 Тогда
			Результат.КраткоеПредставлениеОшибки = НСтр("ru='Данные о улицах для введенного адреса отсутствуют';uk='Дані про вулиці для введеної адреси відсутні'");
		Иначе
			Результат.КраткоеПредставлениеОшибки = НСтр("ru='Поле не содержит адресных сведений для выбора.';uk='Поле не містить адресних даних для вибору.'");
		КонецЕсли;
	КонецЕсли;
