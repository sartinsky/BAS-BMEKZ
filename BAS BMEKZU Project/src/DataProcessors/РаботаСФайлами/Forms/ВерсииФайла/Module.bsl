...
Процедура СделатьАктивнойВыполнить()
	...
	ИначеЕсли ДанныеФайла.ПодписанЭП Тогда
		ПоказатьПредупреждение(, НСтр("ru='Смена активной версии разрешена только для неподписанных файлов.';uk='Зміна активної версії дозволена тільки для непідписаних файлів.'"));
	Иначе
...
Процедура Сравнить(Команда)
	...
	Если ЧислоВыделенныхСтрок <> 2 И ЧислоВыделенныхСтрок <> 1 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для просмотра отличий необходимо выбрать две версии файла.';uk='Для перегляду відмінностей необхідно вибрати дві версії файлу.'"));
		Возврат;
	КонецЕсли;
...
Процедура СменитьАктивнуюВерсиюФайла(Версия)
	...
	Попытка
		...
		Если ФайлОбъект.ПодписанЭП Тогда
			ВызватьИсключение НСтр("ru='У подписанного файла нельзя изменять активную версию.';uk='У підписаного файлу не можна змінювати активну версію.'");
		КонецЕсли;
