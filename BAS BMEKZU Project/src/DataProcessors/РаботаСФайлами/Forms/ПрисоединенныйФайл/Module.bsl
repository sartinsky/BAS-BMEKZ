...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда // Временное решение для работы в мобильном клиенте, будет удалено в следующих версиях
		...
		Для каждого ИмяЭлемента Из ИменаЭлементаОписание Цикл
			...
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, ИмяЭлемента, "ПодсказкаВвода", НСтр("ru='Краткое описание';uk='Короткий опис'"));
...
Процедура Печать(Команда)
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Объект.Ссылка) Тогда
		ПечатьФайла();
	Иначе
		ПоказатьПредупреждение(, НСтр("ru='Для печати файла необходимо записать данные. Записать?';uk='Для друку файлу необхідно записати дані. Записати?'"));
	КонецЕсли;
...
Процедура УстановитьДоступностьКнопок(Форма, Элементы)
	...
	Если Не ДоступнаПечатьСоШтампом Тогда
		...
		Элементы.Печать.Заголовок = НСтр("ru='Печать';uk='Друк'");
	Иначе
		Элементы.ПодменюПечать.Вид = ВидГруппыФормы.Подменю;
		Элементы.Печать.Заголовок = НСтр("ru='Сразу на принтер';uk='Відразу на принтер'");
	КонецЕсли;
...
Процедура ОбновитьПояснениеОблачногоСервиса(ПрисоединенныйФайл)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСинхронизациюФайлов") Тогда
		...
		Если СведенияОСинхронизации.Количество() > 0 Тогда
			...
			ЧастиСтроки.Добавить(НСтр("ru='Файл доступен только для просмотра, работа с ним ведется в облачном сервисе';uk='Цей Файл доступний тільки для перегляду, робота з ним ведеться в хмарному сервісі'"));
			...
			ЧастиСтроки.Добавить(НСтр("ru='Синхронизирован';uk='Синхронізований'"));
			ЧастиСтроки.Добавить(": ");
