...
Процедура СохранитьПапку()
	...
	Если ПустаяСтрока(ПапкаДляЭкспорта) Или ПапкаДляЭкспорта = ПолучитьРазделительПути() Тогда
		ПоказатьПредупреждение(, НСтр("ru='Необходимо указать папку.';uk='Необхідно вказати папку.'"));
		Возврат;
	КонецЕсли;
	...
	Если Не КаталогВыгрузки.Существует() Тогда
		...
		Попытка
			...
			Если Не КаталогВыгрузки.Существует() Тогда
				ВызватьИсключение НСтр("ru='После успешного создания подпапка не найдена.';uk='Після успішного створення підпапка не знайдена.'");
			КонецЕсли;
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось создать подпапку ""%1"" в папке ""%2"" по причине:
                           |%3'
                           |;uk='Не вдалося створити підпапку ""%1"" в папці ""%2"" з причини:
                           |%3'"),
				Строка(ЧтоСохраняем),
				ПапкаДляЭкспорта,
...
Процедура СохранитьПапкуЗавершение(Результат, ПараметрыВыполнения) Экспорт
	Если Результат.Успех = Истина Тогда
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Экспорт папки';uk='Експорт папки'"),,
		             СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		               НСтр("ru='Успешно завершен экспорт папки ""%1""
                                  |в папку на диске ""%2"".'
                                  |;uk='Успішно завершено експорт папки ""%1""
                                  |в папку на диску ""%2"".'"),
