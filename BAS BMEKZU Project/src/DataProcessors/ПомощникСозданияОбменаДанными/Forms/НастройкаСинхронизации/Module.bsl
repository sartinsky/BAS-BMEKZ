...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(УзелОбмена)
        ...
        |;uk='Настройка синхронізації даних ще не завершена.
        |Завершити роботу з помічником? Настройкe можна буде продовжити пізніше.'");
	...
	Если Не НастройкаНовойСинхронизации Тогда
		Если МодельСервиса Тогда
			СтрокаПредупреждение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Настройка підключення до ""%1"" вже виконана.
                |Редагування параметрів підключення не передбачено.'"), УзелОбмена);
	...
	Если ПустаяСтрока(ЗаголовокПомощникаСозданияОбмена) Тогда
		...
		Иначе
			Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Настройка синхронизации данных с ""%1""';uk='Настройка синхронізації даних з ""%1""'"),
