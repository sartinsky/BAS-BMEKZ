...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ru='Отказаться от удаления настройки синхронизации данных?';uk='Відмовитися від видалення настройки синхронізації даних?'");
...
Процедура ПриЗавершенииУдаленияНастройкиСинхронизации()
	...
	Если НастройкаУдалена Тогда
		...
		Если УдалитьНастройкуВКорреспонденте
            ...
            |;uk='Настройки синхронізації даних в цій програмі
            |і програмою-кореспонденті успішно видалені.'");
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не удалось удалить настройку синхронизации данных.';uk='Не вдалося видалити настройку синхронізації даних.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЗавершенииПолученияСпискаПриложенийНаСервере(ПерейтиДалее)
	...
	Если Не СтатусЗавершения.Отказ Тогда
		...
		КонецЕсли;
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтатусЗавершения.СообщениеОбОшибке);
		ПерейтиДалее = Ложь;
	КонецЕсли;
