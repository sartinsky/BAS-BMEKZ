...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстПредупреждения = НСтр("ru='Прервать настройку параметров подключения для синхронизации данных?';uk='Перервати настройку параметрів підключення для синхронізації даних?'");
...
Процедура ИмяФайлаНастроекПодключенияДляЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("ru='Выберите файл с настройками подключения';uk='Виберіть файл з настройками підключення'"));
	НастройкиДиалога.Вставить("Фильтр",    НСтр("ru='Файл настроек подключения (*.xml)';uk='Файл з настройками підключення (*.xml)'") + "|*.xml");
...
Процедура ИмяФайлаНастроекXDTOКорреспондентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога = Новый Структура;
	НастройкиДиалога.Вставить("Заголовок", НСтр("ru='Выберите файл с настройками корреспондента';uk='Виберіть файл з настройками кореспондента'"));
	НастройкиДиалога.Вставить("Фильтр",    НСтр("ru='Файл настроек корреспондента (*.xml)';uk='Файл з настройками кореспондента (*.xml)'") + "|*.xml");
...
Процедура ИмяФайлаНастроекПодключенияДляВыгрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НастройкиДиалога.Вставить("Заголовок", НСтр("ru='Укажите файл для сохранения настроек подключения';uk='Вкажіть файл для збереження налаштувань підключення'"));
	НастройкиДиалога.Вставить("Фильтр",    НСтр("ru='Файл настроек подключения (*.xml)';uk='Файл з настройками підключення (*.xml)'") + "|*.xml");
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не удалось подключиться к программе. Проверьте настройки подключения.';uk='Не вдалося підключитися до програми. Перевірте настройки підключення.'"));
		КонецЕсли;
	КонецЕсли;
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось подключиться к программе. Проверьте настройки подключения %1.';uk='Не вдалося підключитися до програми. Перевіртенастройки підключення %1.'"), ПроверяемыйВидТранспорта));
		КонецЕсли;
...
Процедура ПолучениеФайлаНастроекПодключенияЗавершение(ПолученныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ПолученныеФайлы = Неопределено Тогда
		...
		Результат.СообщениеОбОшибке = НСтр("ru='Не удалось сохранить настройки подключения в файл.';uk='Не вдалося зберегти параметри підключення в файл.'");
	КонецЕсли;
...
Процедура СохранениеНастроекПодключенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Если ПустаяСтрока(ТекстСообщения) Тогда
			ТекстСообщения = НСтр("ru='Не удалось сохранить настройки подключения.';uk='Не вдалося зберегти настройки підключення.'");
		КонецЕсли;
...
Процедура ЗаполнитьДоступныеВидыТранспорта()
	...
	Для Каждого ТекущийВидТранспорта Из ИспользуемыеТранспортыСообщенийОбмена Цикл
		// В модели сервиса поддерживается обмен только через "Интернет" и "Интернет-пассивное соединение"
		// для планов обмена XDTO.	
		Если МодельСервиса Тогда
...
Процедура ПриЗавершенииПомещенияФайлаНастроекПодключенияДляЗагрузки(РезультатВыбора, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Отказ Тогда
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("ru='Не удалось загрузить файл с настройками подключения.';uk='Не вдалося завантажити файл з настройками підключення.'");
		КонецЕсли;
	Иначе
...
Процедура ПриЗавершенииПомещенияФайлаНастроекXDTOКорреспондента(РезультатВыбора, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Отказ Тогда
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("ru='Не удалось загрузить файл с настройками корреспондента.';uk='Не вдалося завантажити файл з настройками кореспондента.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьВозможностьИспользованияФормы(Отказ = Ложь)
	...
	Если Не Параметры.Свойство("ИмяПланаОбмена")
		...
		ТекстСообщения = НСтр("ru='Форма не предназначена для непосредственного использования.';uk='Форма не призначена для безпосереднього використання.'");
...
Процедура ИнициализироватьСвойстваФормы()
	...
	Если НастройкаРИБ Тогда
		Заголовок = НСтр("ru='Настройка распределенной информационной базы';uk='Настройка розподіленої інформаційної бази'");
	Иначе
		Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Настройка подключения к ""%1""';uk='Настройка підключення до ""%1""'"),
...
Функция Подключаемый_СтраницаСпособНастройкиПодключения_ПриПереходеДалее(Отказ)
	...
	Если СпособНастройкиПодключения = "ПриложениеИзСписка" Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Выберите приложение из списка для продолжения настройки подключения.';uk='Виберіть програму зі списку для продовження настройки з''єднання.'"),
		...
		ИначеЕсли ТекущиеДанные.НастройкаСинхронизацииВМенеджереСервиса Тогда
			ПоказатьПредупреждение(, НСтр("ru='Для настройки синхронизации данных с выбранным приложением перейдите в менеджер сервиса.
                ...
                |;uk='Для настройки синхронізації даних з вибраним додатком відкрийте менеджер сервісу.
	...
	ИначеЕсли СпособНастройкиПодключения = "НастроитьВручную" Тогда
		...
		Если ВидПодключения = "ОбычныеКаналыСвязи"
			...
			Если ПустаяСтрока(ИмяФайлаНастроекПодключенияДляЗагрузки) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Выберите файл с настройками подключения.';uk='Виберіть файл з настройками підключення.'"),
		...
		ИначеЕсли ВидПодключения = "ПассивныйРежим" Тогда
			Если ПустаяСтрока(ИмяФайлаНастроекXDTOКорреспондента) Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
					НСтр("ru='Выберите файл с настройками программы-корреспондента.';uk='Виберіть файл з настройками програми-кореспондента.'"),
...
Функция Подключаемый_СтраницаПараметрыПодключенияИнтернет_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(ИнтернетВебАдрес) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите адрес программы в Интернет.';uk='Зазначте адресу програми в Інтернет.'"),
...
Функция Подключаемый_СтраницаПараметрыПодключенияВнешнееСоединение_ПриПереходеДалее(Отказ)
	...
	Если ВнешнееСоединениеВариантРаботыИнформационнойБазы = "Файловый" Тогда
		...
		Если ПустаяСтрока(ВнешнееСоединениеКаталогИнформационнойБазы) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Выберите каталог расположения информационной базы.';uk='Виберіть каталог розташування інформаційної бази.'"),
	...
	ИначеЕсли ВнешнееСоединениеВариантРаботыИнформационнойБазы = "КлиентСерверный" Тогда
		...
		Если ПустаяСтрока(ВнешнееСоединениеКластерСерверов) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите имя кластера серверов платформы.';uk='Зазначте ім''я кластера серверів платформи.'"),
		...
		Если ПустаяСтрока(ВнешнееСоединениеИмяИнформационнойБазы) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите имя информационной базы в кластере серверов платформы.';uk='Зазначте ім''я інформаційної бази у кластері серверів платформи.'"),
...
Функция Подключаемый_СтраницаПараметрыПодключенияОбычныеКаналыСвязи_ПриПереходеДалее(Отказ)
	...
	Если Не ОбычныеКаналыСвязиFILEИспользование
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Выберите хотя бы один способ передачи файлов с данными.';uk='Виберіть хоча б один спосіб передачі файлів з даними.'"),
	...
	Если ОбычныеКаналыСвязиАрхивироватьФайлы
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите пароль для архивации файлов.';uk='Зазначте пароль для архівації файлів.'"),
	...
	Если ОбычныеКаналыСвязиFILEИспользование Тогда
		...
		Если ПустаяСтрока(ОбычныеКаналыСвязиFILEКаталог) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Выберите каталог для передачи файлов с данными.';uk='Виберіть каталог для передачі файлів з даними.'"),
	...
	Если ОбычныеКаналыСвязиFTPИспользование Тогда
		...
		Если ПустаяСтрока(ОбычныеКаналыСвязиFTPПуть) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите адрес каталога для передачи файлов с данными.';uk='Зазначте адресу каталогу для передачі файлів з даними.'"),
		...
		Если ОбычныеКаналыСвязиFTPИспользоватьОграничениеРазмераФайла
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите максимальный допустимый размер файлов с данными.';uk='Зазначте максимальний припустимий розмір файлів з даними.'"),
	...
	Если ОбычныеКаналыСвязиEMAILИспользование Тогда
		...
		Если Не ЗначениеЗаполнено(ОбычныеКаналыСвязиEMAILУчетнаяЗапись) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Выберите учетную запись электронной почты для отправки сообщений с данными.';uk='Виберіть обліковий запис електронної пошти для відправки повідомлень з даними.'"),
		...
		Если ОбычныеКаналыСвязиEMAILИспользоватьОграничениеРазмераВложения
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите максимальный допустимый размер почтового вложения.';uk='Зазначте максимальний припустимий розмір поштового вкладення.'"),
...
Функция Подключаемый_СтраницаОбщиеНастройкиСинхронизации_ПриПереходеДалее(Отказ)
	...
	Если Не ВидПодключения = "ПассивныйРежим" Тогда
		...
		Если ДоступноИзменениеНаименования
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите наименование этой программы.';uk='Укажіть найменування цієї програми.'"),
				, "Наименование", , Отказ);
		КонецЕсли;
		...
		Если ДоступноИзменениеПрефикса
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите префикс этой программы.';uk='Вкажіть префікс цієї програми.'"),
				, "Префикс", , Отказ);
		КонецЕсли;
	...
	Если ДоступноИзменениеНаименованияКорреспондента
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите наименование программы-корреспондента.';uk='Зазначте найменування програми-кореспондента.'"),
	...
	Если Не ВидПодключения = "ПассивныйРежим" Тогда
		...
		Если ДоступноИзменениеПрефиксаКорреспондента
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Укажите префикс программы-корреспондента.';uk='Зазначте префікс програми-кореспондента.'"),
	...
	Если СохранитьПараметрыПодключенияВФайл
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите путь к файлу для сохранения настроек подключения.';uk='Зазначте шлях до файлу для збереження настройок підключення.'"),
