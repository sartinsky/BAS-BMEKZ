...
Процедура ПриПолученииДоступныхНастроекСинхронизацииДанных(ТаблицаНастройки,
	...
	// Удаляем устаревшие варианты настройки обмена.
	Если МодельСервиса Тогда
...
Процедура ПриНачалеПроверкиПодключения(НастройкиПодключения, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(НастройкиПодключения.ИмяПланаОбмена,			
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Проверка подключения %1';uk='Перевірка підключення %1'"), НастройкиПодключения.ВидТранспортаСообщенийОбмена));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Проверка подключения %1 уже выполняется.';uk='Перевірка підключення %1 вже виконується.'"), НастройкиПодключения.ВидТранспортаСообщенийОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Проверка подключения к корреспонденту: %1.';uk='Перевірка підключення до кореспонденту: %1.'"), НастройкиПодключения.ВидТранспортаСообщенийОбмена);
...
Процедура ПриНачалеСохраненияНастроекПодключения(НастройкиПодключения, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(НастройкиПодключения.ИмяПланаОбмена,
		НСтр("ru='Сохранение настроек подключения';uk='Збереження настройок підключення'"));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сохранение настроек подключения для ""%1"" уже выполняется.';uk='Збереження настройок підключення для ""%1"" вже виконується.'"), НастройкиПодключения.ИмяПланаОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Сохранение настроек подключения: %1.';uk='Збереження настройок підключення: %1.'"), НастройкиПодключения.ИмяПланаОбмена);
	ПараметрыВыполнения.КлючФоновогоЗадания = КлючФоновогоЗадания;
	ПараметрыВыполнения.ЗапуститьНеВФоне    = Ложь;
	ПараметрыВыполнения.ЗапуститьВФоне      = Истина;
...
Процедура ПриНачалеСохраненияНастроекСинхронизации(НастройкиСинхронизации, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(ИмяПланаОбмена,
		НСтр("ru='Сохранение настроек синхронизации данных';uk='Збереження настройок синхронізації даних'"));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сохранение настроек синхронизации данных для ""%1"" уже выполняется.';uk='Збереження настройок синхронізації даних для ""%1"" вже виконується.'"), ИмяПланаОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Сохранение настроек синхронизации данных: %1.';uk='Збереження настройок синхронізації даних: %1.'"), ИмяПланаОбмена);
	ПараметрыВыполнения.КлючФоновогоЗадания = КлючФоновогоЗадания;
	ПараметрыВыполнения.ЗапуститьНеВФоне    = Ложь;
	ПараметрыВыполнения.ЗапуститьВФоне      = Истина;
...
Процедура ПриНачалеУдаленияНастройкиСинхронизации(НастройкиУдаления, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(ИмяПланаОбмена,
		НСтр("ru='Удаление настройки синхронизации данных';uk='Вилучення настройки синхронізації даних'"));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Удаление настройки синхронизации данных для ""%1"" уже выполняется.';uk='Вилучення настройки синхронізації даних для ""%1"" вже виконується.'"), ИмяПланаОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удаление настройки синхронизации данных: %1.';uk='Вилучення настройки синхронізації даних: %1.'"), ИмяПланаОбмена);
	ПараметрыВыполнения.КлючФоновогоЗадания = КлючФоновогоЗадания;
	ПараметрыВыполнения.ЗапуститьНеВФоне    = Ложь;
	ПараметрыВыполнения.ЗапуститьВФоне      = Истина;
...
Процедура ПриНачалеРегистрацииДанныхДляНачальнойВыгрузки(НастройкиРегистрации, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Регистрация данных для начальной выгрузки (%1)';uk='Реєстрація даних для початкового вивантаження (%1)'"),
		НастройкиРегистрации.УзелОбмена);

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Регистрация данных для начальной выгрузки для ""%1"" уже выполняется.';uk='Реєстрація даних для початкового вивантаження для ""%1"" вже виконується.'"),
			НастройкиРегистрации.УзелОбмена);
	КонецЕсли;
...
Процедура ПриНачалеЗагрузкиНастроекXDTO(НастройкиЗагрузки, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Загрузка настроек XDTO (%1)';uk='Завантаження настройок XDTO (%1)'"),
		НастройкиЗагрузки.УзелОбмена);

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загрузка настроек XDTO для ""%1"" уже выполняется.';uk='Завантаження настройок XDTO для ""%1"" вже виконується.'"),
			НастройкиЗагрузки.УзелОбмена);
	КонецЕсли;
...
Функция КлючФоновогоЗадания(ИмяПланаОбмена, Действие)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='ПланОбмена:%1 Действие:%2';uk='ПланОбмена:%1 Дія:%2'"), ИмяПланаОбмена, Действие);
...
Процедура ПроверитьПодключениеККорреспонденту(Параметры, АдресРезультата) Экспорт
	...
	Если НастройкиПодключения.ВидТранспортаСообщенийОбмена = Перечисления.ВидыТранспортаСообщенийОбмена.COM Тогда
		...
		Если РезультатПроверки.ВерсииИнтерфейса.Найти("3.0.1.1") <> Неопределено Тогда
			...
			Если Не ПараметрыКорреспондента.ПланОбменаСуществует Тогда
				РезультатПроверки.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |;uk='В кореспонденті не знайдений план обміну ""%1"".
                    |Переконайтеся, що
                    | - вибрано правильний вид програми для налаштування обміну;
                    | - коректно вказано розташування програми, до якої здійснюється підключення.'"),
		...
		Иначе
            ...
            |;uk='Кореспондент не підтримує версію 3.0.1.1 інтерфейсу ""ОбменДанными"".
            |Для налаштування підключення необхідно оновити конфігурацію кореспондента, чи починати з нього.'");
		...
		Если РезультатПроверки.ВерсииИнтерфейса.Найти("3.0.1.1") <> Неопределено Тогда
			...
			Если Не ПараметрыКорреспондента.ПланОбменаСуществует Тогда
				РезультатПроверки.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |Переконайтеся, що
                    | - вибрано правильний вид програми для налаштування обміну;
                    | - вказано правильну адресу програми в Інтернеті.'"),
					НастройкиПодключения.ИмяПланаОбмена);
...
Процедура ЗагрузитьНастройкиXDTOКорреспондента(Параметры, АдресРезультата) Экспорт
	...
	Если Отказ Тогда
		...
		Если ПустаяСтрока(Результат.СообщениеОбОшибке) Тогда
			Результат.СообщениеОбОшибке = НСтр("ru='Не удалось получить параметры корреспондента.';uk='Не вдалося отримати параметри кореспондента.'");
		КонецЕсли;
	Иначе
...
Процедура УдалитьНастройкуСинхронизацииВКорреспонденте(НастройкиУдаления, Результат)
	...
	Если ВидТранспорта = Перечисления.ВидыТранспортаСообщенийОбмена.COM Тогда
		...
		Если УзелКорреспондента = Неопределено Тогда
			Результат.СообщениеОбОшибкеВКорреспонденте = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В программе-корреспонденте не найден узел плана обмена ""%1"" по коду ""%2"".';uk='У програмі-кореспонденті не знайдено вузол плану обміну ""%1"" по коду ""%2"".'"),
	...
	ИначеЕсли ВидТранспорта = Перечисления.ВидыТранспортаСообщенийОбмена.WS Тогда
		...
		Если ВерсииИнтерфейса.Найти("3.0.1.1") <> Неопределено Тогда
			WSПрокси = ОбменДаннымиСервер.ПолучитьWSПрокси_3_0_1_1(НастройкиПодключения);
		Иначе
			Результат.СообщениеОбОшибкеВКорреспонденте = НСтр("ru='Удаление настройки синхронизации на стороне программы-корреспондента из этой программы не поддерживается.';uk='Вилучення настройки синхронізації на стороні програми-кореспондента з цієї програми не підтримується.'");
			Результат.НастройкаУдаленаВКорреспонденте = Ложь;
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Результат.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для вида подключения ""%1"" не поддерживается удаление настройки синхронизации на стороне программы-корреспондента.';uk='Для виду підключення ""%1"" не підтримується видалення настройки синхронізації на стороні програми-кореспондента.'"),
			ВидТранспорта);
	КонецЕсли;
...
Процедура СоздатьОбновитьУзлыПланаОбмена(НастройкиПодключения)
	...
	Иначе
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Значення префікса програми ""%1"" не унікально (""%2""). Вже існує настройка синхронізації з таким самим префіксом.
                |Для продовження необхідно призначити в програмі ""%1"" унікальний префікс інформаційної бази, відмінний від поточного.'"),
	...
	РегистрыСведений.ОбщиеНастройкиУзловИнформационныхБаз.ОбновитьПрефиксы(
		...
		?(НастройкиПодключения.ИспользоватьПрефиксыДляНастройкиОбмена, НастройкиПодключения.ПрефиксИнформационнойБазыИсточника, ""),
...
Процедура ЗаполнитьНастройкиПодключенияИзXML(НастройкиПодключения, ИмяФайлаСтрокаXML, ЭтоФайл = Ложь) Экспорт
	...
	Если Не КорректныйФайлНастроек Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Файл не містить налаштувань підключення для вибраного обміну даними.
            |Обраний обмін ""%1"".
            |У файлі вказані параметри для обміну ""%2"".'"),
	...
	Если НастройкиПодключения.ВариантРаботыМастера = "ПродолжитьНастройкуОбменаДанными"
		...
		Если Не ПустаяСтрока(ПрефиксИБ)
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Префікс програми, вказаний при налаштуванні (""%1""), не відповідає префікса в цій програмі (""%2"").
                |Для продовження необхідно починати заново з іншої програми, і вказати коректний префікс (""%2"").'"),
	...
	Если НастройкиПодключения.ВерсияФорматаФайлаНастроекОбменаДанными = "1.0" Тогда
		...
		НастройкиПодключения.НаименованиеЭтойБазы    = НСтр("ru='Эта информационная база';uk='Дана інформаційна база'");
...
Процедура СчитатьДанныеВСтруктуру(СтруктураНастроек, ЧтениеXML)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Тогда
		ВызватьИсключение НСтр("ru='Ошибка разбора XML.';uk='Помилка розбору XML.'");
	КонецЕсли;
...
Функция НастройкиXDTOКорреспондентаИзXML(ИмяФайлаСтрокаXML, ЭтоФайл, УзелОбмена) Экспорт
	...
	Попытка
		УИД = Новый УникальныйИдентификатор(Header.Confirmation.From);
	Исключение
		ВызватьИсключение НСтр("ru='Идентификатор отправителя в файле с настройками EnterpriseData не соответствует формату.';uk='Ідентифікатор відправника у файлі з настройками EnterpriseData не відповідає формату.'");
	КонецПопытки;
