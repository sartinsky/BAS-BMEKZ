...
Процедура ВнешнееСоединениеВыполнитьДействияПоНастройкеНовогоОбменаДанными(Отказ, 
	...
	Попытка
		...
		Если Отказ Тогда
			ВызватьИсключение(НСтр("ru='При создании настройки синхронизации данных возникли ошибки.';uk='При створенні настройки синхронізації даних виникли помилки.'"));
		КонецЕсли;
	...
	Иначе
		...
		Иначе
			ВызватьИсключение НСтр("ru='Значение префикса первой информационной базы не уникально.
                ...
                |;uk='Значення префікса першої інформаційної бази не унікально.
                |В системі вже існує синхронізація даних інформаційної бази (програми) із зазначеним префіксом.'");
		КонецЕсли;
...
Процедура ВыполнитьЗагрузкуПараметровМастера(Отказ, СтрокаXML) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ПолеСтрокаСообщенияОбОшибке = НСтр("ru='Синхронизация данных с этой программой в режиме сервиса не предусмотрена.';uk='Синхронізація даних з цією програмою в режимі сервісу не передбачена.'");
