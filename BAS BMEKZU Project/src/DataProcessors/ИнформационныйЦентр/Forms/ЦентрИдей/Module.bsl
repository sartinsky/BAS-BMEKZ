...
Процедура ПроизвестиПоиск()
	...
	Если ПустаяСтрока(ТекстПоиска) Тогда 
		ТекстОшибки = НСтр("ru='Запрос должен быть заполнен';uk='Запит повинен бути заповнений'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ТекстПоиска");
		Возврат;
	КонецЕсли;
