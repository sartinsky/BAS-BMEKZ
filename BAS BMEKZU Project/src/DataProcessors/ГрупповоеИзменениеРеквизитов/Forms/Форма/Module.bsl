...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		...
		Элементы.ФормаИзменить.Заголовок = НСтр("ru='Выполнить';uk='Виконати'");
...
Процедура ВопросПерейтиКРазблокированиюРеквизитов(ВыбранныйРеквизит)
	...
	Если КонтекстныйВызов Тогда
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Перейти';uk='Перейти'"));
	...
	Иначе
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru='Разблокировать';uk='Розблокувати'"));
		ТекстВопроса = ПодставитьПараметрыВСтроку(
			НСтр("ru='Для того чтобы не допустить рассогласования данных в программе,
                ...
                |перевіривши всі місця використання обраних елементів в програмі.
                |
                |Розблокувати реквізит ""%1"" для зміни?'"),
...
Процедура ЗавершитьИзменениеОбъектов()
	...
	Если ОбработкаЗавершена Тогда
		ПоказатьОповещениеПользователя(НСтр("ru='Изменение реквизитов элементов';uk='Зміна реквізитів елементів'"), , 
			ПодставитьПараметрыВСтроку(НСтр("ru='Изменены элементы (%1).';uk='Змінено елементи (%1).'"), ТекущееСостояниеИзменения.КоличествоИзмененных));
...
Процедура НазадСервер()
	...
	Если Объект.ТипОперации = "ВыполнитьАлгоритм" Тогда
		Элементы.ФормаИзменить.Заголовок = НСтр("ru='Выполнить';uk='Виконати'");
		Элементы.ФормаИзменить.РасширеннаяПодсказка.Заголовок = НСтр("ru='Выполнить алгоритм';uk='Виконати алгоритм'");
	Иначе
		Элементы.ФормаИзменить.Заголовок = НСтр("ru='Изменить реквизиты';uk='Змінити реквізити'");
	КонецЕсли;
...
Процедура ИзменитьНаСервере(Знач ОстанавливатьИзменениеПриОшибке)
	...
	ПараметрыЗадания.Вставить("ВариантЗаписиОбъекта", ?(ОбработкаОбъект.ВариантЗаписиОбъекта = 1, "НеЗаписывать", "Записывать"));
	...
	Иначе
		...
		ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Групповое изменение элементов';uk='Групове редагування елементів'");
...
Процедура ОбновитьНадписьКоличествоВыбранных()
	...
	Если ЕстьНастроенныеОтборы() Тогда
		...
		ТекстНадписи = СтрокаСЧисломДляЛюбогоЯзыка(НСтр("ru=';%1 элемент;;%1 элемента;%1 элементов;%1 элемента';uk=';%1 елемент;;%1 елемента;%1 елементів;%1 елемента'"),
...
Функция ПредставлениеСвязейПараметровВыбора(СвязиПараметровВыбора, ОбъектМетаданных)
		...
		Если ВладелецРеквизитов <> Неопределено Тогда
			...
			Если Реквизит <> Неопределено Тогда
				...
				Если Не ПустаяСтрока(ПредставлениеТабличнойЧасти) Тогда
					ПредставлениеРеквизита = ПредставлениеРеквизита + " (" + НСтр("ru='таблица';uk='таблиця'") + " " 
						+ ПредставлениеТабличнойЧасти + ")";
				КонецЕсли;
	...
	Если СвязанныеРеквизиты.Количество() > 0 Тогда
		ШаблонПредставленияСвязей = НСтр("ru='Зависит от реквизитов: %1.';uk='Залежить від реквізитів: %1.'");
		Если СвязанныеРеквизиты.Количество() = 1 Тогда
			ШаблонПредставленияСвязей = НСтр("ru='Зависит от реквизита %1.';uk='Залежить від реквізиту %1.'");
		КонецЕсли;
