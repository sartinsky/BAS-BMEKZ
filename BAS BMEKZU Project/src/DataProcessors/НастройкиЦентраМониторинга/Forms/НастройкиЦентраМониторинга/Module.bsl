...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ИдентификаторЗадания") Тогда
		...
		ВывестиРезультатПолученияИдентификатора = Истина;
	...
	Иначе
		Элементы.ОтправлятьИнформациюОбОшибкахРасширеннаяПодсказка.Заголовок = СтрЗаменить(СодержимоеПодсказки,"%ДопИнфо"," " + НСтр("ru='на сервере платформы';uk='на сервері платформи'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьВидимостьЭлементов(ПараметрыВидимости)
	...
	Если ПараметрыВидимости.Статус = "Выполняется" Тогда
		Элементы.ОписаниеПрогресса.Заголовок = НСтр("ru='Выполняется получение идентификатора';uk='Виконується отримання ідентифікатора'");		
	...
	ИначеЕсли ПараметрыВидимости.Статус = "Выполнено" И РезультатВыполнения.Успешно Тогда
		Элементы.ОписаниеПрогресса.Заголовок = НСтр("ru='Идентификатор успешно получен';uk='Ідентифікатор успішно отримано'");		
	...
	ИначеЕсли ПараметрыВидимости.Статус = "Выполнено" И НЕ РезультатВыполнения.Успешно ИЛИ ПараметрыВидимости.Статус = "Ошибка" Тогда
		Если ПараметрыВидимости.Статус = "Ошибка" Тогда
			Пояснение = НСтр("ru='Ошибка при выполнении фонового задания.';uk='Помилка при виконанні фонового завдання.'");
		Иначе
			Пояснение = РезультатВыполнения.КраткоеПредставлениеОшибки;
		КонецЕсли;
		ШаблонЗаголовка = НСтр("ru='Не удалось получить идентификатор. %1 Подробнее см. в журнале регистрации';uk='Не вдалося отримати ідентифікатор. %1 Докладніше див. у журналі реєстрації'");
