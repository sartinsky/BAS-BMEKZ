...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.Сервис.СписокВыбора.Добавить("Другой", НСтр("ru='Другой сервис';uk='Інший сервіс'"));
	...
	Если Состояние <> Перечисления.СостоянияМиграцииПриложения.Выполняется Тогда
		...
		Наименование = СтрШаблон(НСтр("ru='%1 (Миграция приложения)';uk='%1 (Міграція програми)'"), Метаданные.Синоним);
...
Процедура ОткрытиеСтраницы(Страница)
	...
	Элементы.Отмена.Заголовок = НСтр("ru='Отмена';uk='Відмінити'");
...
Процедура СтраницаАдресСервисаОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Миграция приложения в сервис';uk='Міграція програми в сервіс'");
...
Функция СтраницаАдресСервисаДалее()
	...
	Если Не ЗначениеЗаполнено(АдресСервиса) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнен адрес сервиса';uk='Не заповнений адресу сервісу'"), , "АдресСервиса");
	...
	Если ЧастиАдреса.Количество() = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Некорректный адрес сервиса';uk='Некоректну адресу сервісу'"), , "АдресСервиса");
		Возврат Неопределено;
	КонецЕсли;
...
Процедура СтраницаИдентификационныеДанныеОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Аутентификация в сервисе';uk='Аутентифікація в сервісі'");
	
	Элементы.ДекорацияИдентификационныеДанные.Заголовок = СтрШаблон(НСтр("ru='Вход в сервис: %1';uk='Вхід в сервіс: %1'"), АдресСервиса);
...
Функция СтраницаИдентификационныеДанныеДалее()
	...
	Если Не ЗначениеЗаполнено(РегистрацияНаименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Полное имя (Ф.И.О.)""';uk='Не заповнено поле ""Повне ім''я (П. І. Б.)""'"), , "РегистрацияНаименование", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Логин) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Имя(логин)""';uk='Не заповнено поле ""Ім''я(логін)""'"), , "Логин", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(РегистрацияЭлектроннаяПочта) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Электронная почта""';uk='Не заповнено поле ""Електронна пошта""'"), , "РегистрацияЭлектроннаяПочта", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Пароль) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Пароль""';uk='Не заповнено поле ""Пароль""'"), , "Пароль", , Отказ);
	ИначеЕсли Пароль <> ПодтверждениеПароля Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Подтверждение пароля не совпадает с паролем';uk='Підтвердження паролю не збігається з паролем'"), , "ПодтверждениеПароля", , Отказ);
	КонецЕсли;
...
Процедура СтраницаПодтверждениеРегистрацииОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Подтверждение регистрации';uk='Підтвердження реєстрації'");
...
Процедура СтраницаВыборАбонентаОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Выбор абонента';uk='Вибір абонента'");
...
Функция СтраницаВыборАбонентаДалее()
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не выбран абонент';uk='Не вибраний абонент'"), , "КодАбонента");
	КонецЕсли;
...
Процедура СтраницаПраваПользователейОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Права пользователей';uk='Права користувачів'");
...
Функция СтраницаСозданиеПриложениеДалее()
	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не заполнено поле ""Наименование""';uk='Не заповнене поле ""Найменування""'"), , "Наименование", , Отказ);
	КонецЕсли;
...
Процедура СтраницаСозданиеПриложениеОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Параметры создания приложения';uk='Параметри створення програми'");
	Элементы.Далее.Заголовок = НСтр("ru='Начать миграцию';uk='Почати міграцію'");
...
Функция СтраницаОжиданиеДалее()
	...
	Если ЗначениеЗаполнено(Задание) Тогда
		Текст = НСтр("ru='Установлен монопольный режим, не закрывайте приложение до окончания миграции.';uk='Встановлено монопольний режим, не закривайте програму до закінчення міграції.'");
		Элементы.Далее.Доступность = Ложь;
	Иначе
		Текст = НСтр("ru='Не удалось установить монопольный режим, необходимо завершение сеансов всех пользователей.';uk='Не вдалося встановити монопольний режим, необхідно завершення сеансів всіх користувачів.'");
	КонецЕсли;
...
Процедура СтраницаОжиданиеОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Выполняется миграция...';uk='Виконується міграція...'");
	
	Элементы.Далее.Видимость = ТребуетсяМонопольныйРежим;
	Элементы.Далее.Заголовок = НСтр("ru='Завершить миграцию';uk='Завершити міграцію'");
	Элементы.Отмена.Видимость = Истина;
	Элементы.Назад.Видимость = Ложь;
	
	Строки = Новый Массив;
	Строки.Добавить(НСтр("ru='Адрес сервиса';uk='Адреса сервісу'") + ": ");
	Строки.Добавить(Новый ФорматированнаяСтрока(АдресСервиса, , , , АдресСервиса));
	Строки.Добавить(Символы.ПС);
	Строки.Добавить(СтрШаблон(НСтр("ru='Код абонента: %1';uk='Код абонента: %1'"), КодАбонента) + Символы.ПС);
	Строки.Добавить(СтрШаблон(НСтр("ru='Код приложения: %1';uk='Код програми: %1'"), КодПриложения) + Символы.ПС);
	Строки.Добавить(НСтр("ru='Контакты обслуживающей организации';uk='Контакти обслуговуючої організації'") + ":" + Символы.ПС);
	Строки.Добавить(Символы.Таб);
	Строки.Добавить(Символы.Таб + СтрШаблон(НСтр("ru='e-mail: %1';uk='e-mail: %1'"), ОбслуживающаяОрганизацияЭлектроннаяПочта) + Символы.ПС);
	Строки.Добавить(Символы.Таб);
	Строки.Добавить(Символы.Таб + СтрШаблон(НСтр("ru='тел.: %1';uk='тел.: %1'"), ОбслуживающаяОрганизацияТелефон) + Символы.ПС);
...
Процедура СтраницаРезультатОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Результат миграции';uk='Результат міграції'");
...
Процедура СтраницаОшибкаОткрытие(ЭтоПереходНазад)
	...
	Заголовок = НСтр("ru='Ошибка миграции';uk='Помилка міграції'");
...
Процедура ОтменитьПереход()
	...
	Комментарий = НСтр("ru='Переход в сервис отменен';uk='Перехід в сервіс скасовано'");
...
Функция СостояниеПерехода()
	...
	СостояниеПерехода.Вставить("ОсталосьВремени", НСтр("ru='Рассчитывается...';uk='Розраховується...'"));
	...
	Если (СостояниеПерехода.Состояние = Перечисления.СостоянияМиграцииПриложения.Выполняется 
		...
		Если ОставшеесяВремя <= 180 Тогда
			СостояниеПерехода.ОсталосьВремени = НСтр("ru='Несколько минут';uk='Кілька хвилин'");
		ИначеЕсли ОставшеесяВремя <= 3600 Тогда
			СостояниеПерехода.ОсталосьВремени = СтрШаблон(НСтр("ru='~ %1 мин.';uk='~ %1 хв.'"), Окр(ОставшеесяВремя / 60));
		Иначе
			СостояниеПерехода.ОсталосьВремени = СтрШаблон(НСтр("ru='~ %1 ч.';uk='~ %1 ч.'"), Окр(ОставшеесяВремя / 3600, 1));
		КонецЕсли;
	...
	СостояниеПерехода.Вставить("ТекстСообщенийОтправленоОбработано", СтрШаблон(НСтр("ru='%1 / %2';uk='%1 / %2'"), СостояниеПерехода.НомерОтправленногоСообщения, СостояниеПерехода.НомерПринятогоСообщения));
	СостояниеПерехода.Вставить("ТекстОбъектовВыгруженоЗагружено", СтрШаблон(НСтр("ru='%1 / %2';uk='%1 / %2'"), СостояниеПерехода.ВыгруженоОбъектов, СостояниеПерехода.ЗагруженоОбъектов));
	СостояниеПерехода.Вставить("ТекстОсталосьВыгрузитьЗагрузить", СтрШаблон(НСтр("ru='%1 / %2';uk='%1 / %2'"), СостояниеПерехода.ИзмененоОбъектов, СостояниеПерехода.ЗагрузитьОбъектов));
...
Процедура ОбновитьСтатусСопоставления(Форма)
	...
	Если НесопоставленоПользователей > 0 Тогда
		Заголовок = СтрШаблон(НСтр("ru='Пользователей из базы не сопоставлено: %1';uk='Користувачів з бази не зіставлено: %1'"), НесопоставленоПользователей);
	Иначе
		Заголовок = НСтр("ru='Все пользователи из базы сопоставлены';uk='Всі користувачі з бази зіставлені'");
	КонецЕсли;
...
Функция ВыбратьАбонента()
	...
	Если АбонентыПользователя.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru='Нет прав для добавления приложений абоненту.';uk='Немає прав для додавання додатків абоненту.'");
