...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ЭлектроннаяПочта) Тогда
		...
		Исключение
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ЭлектроннаяПочта", , Отказ);
			Возврат;
		КонецПопытки;
		Если Результат.Количество() > 1 Тогда
			ТекстОшибки = НСтр("ru='Можно ввести только один e-mail.';uk='Можна ввести тільки один e-mail.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "ЭлектроннаяПочта", , Отказ);
		КонецЕсли;
	КонецЕсли;
