		...
		Попытка
			...
			Если Не ДанныеКорреспондента.Свойство("ПараметрыИнформационнойБазы", Результат.ПараметрыКорреспондента) Тогда
				Результат.СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |;uk='Не підтримується версія 3.0.1.1 інтерфейсу ""ОбменДанными"".
                    |Для настройки підключення необхідно оновити програму ""%1"", або починати з неї.'"),
...
Процедура ПриНачалеСохраненияНастроекПодключения(НастройкиПодключения, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = КлючФоновогоЗадания(НастройкиПодключения.ИмяПланаОбмена,
		НСтр("ru='Настройка подключения к корреспонденту';uk='Настройка підключення до кореспонденту'"));

	Если ЕстьАктивныеФоновыеЗадания(КлючФоновогоЗадания) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Настройка подключения к корреспонденту для ""%1"" уже выполняется.';uk='Настройка підключення до кореспонденту для ""%1"" вже виконується.'"), НастройкиПодключения.ИмяПланаОбмена);
	КонецЕсли;
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Настройка подключения к корреспонденту: %1.';uk='Настройка підключення до кореспонденту: %1.'"), НастройкиПодключения.ИмяПланаОбмена);
...
Процедура ПриНачалеУдаленияНастройкиСинхронизации(НастройкиУдаления, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	Попытка
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отключение синхронизации данных ""%1"" с приложением ""%2"".';uk='Відключення синхронізації даних ""%1"" з додатком ""%2"".'"),
...
Функция ТаблицаПриложенийСинхронизацииДанных(Параметры)
	...
	Для Каждого НастройкаИзМенеджераСервиса Из НастройкиСинхронизацииИзМенеджераСервиса Цикл
		...
		Если СтрокаПриложение.КонечнаяТочкаКорреспондента.Пустая() Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru='Не найдена конечная точка корреспондента с идентификатором ""%1"".';uk='Не знайдена кінцева точка кореспондента з ідентифікатором ""%1"".'"),
				НастройкаИзМенеджераСервиса.КонечнаяТочкаКорреспондента);
		КонецЕсли;
