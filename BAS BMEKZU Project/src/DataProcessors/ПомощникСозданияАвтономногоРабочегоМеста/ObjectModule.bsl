...
Процедура СоздатьНачальныйОбразАвтономногоРабочегоМеста(
	...
	ВерсияБСП302 = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(СтандартныеПодсистемыСервер.ВерсияБиблиотеки(), "3.0.2.1") >= 0;
	Если ВерсияБСП302 Тогда
		МодульОбменДаннымиСлужебный = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиСлужебный");
		МодульОбменДаннымиСлужебный.ПроверитьКэшМеханизмаРегистрацииОбъектов();
	Иначе
		МодульОбменДаннымиВызовСервера = ОбщегоНазначения.ОбщийМодуль("ОбменДаннымиВызовСервера");
		МодульОбменДаннымиВызовСервера.ПроверитьКэшМеханизмаРегистрацииОбъектов();
	КонецЕсли;
...
Функция СформироватьНаименованиеПриложенияВСервисе()
	...
	Возврат СтрШаблон(НСтр("ru='%1 (приложение в Интернете)';uk='%1 (додаток в Інтернеті)'"), НаименованиеПриложения);
