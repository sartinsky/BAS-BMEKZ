...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоПодчиненныйУзелРИБ
		...
		СообщениеЖурнала = НСтр("ru='Обновление программы невозможно. В подчиненном узле информационной базы обновление получается из главного узла.';uk='Оновлення програми неможливо. У підпорядкованому вузлі інформаційної бази оновлення отримується з головного вузла.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru='<b>Обновление программы невозможно</b><br />В подчиненном узле информационной базы обновление получается из главного узла.';uk='<b>Оновлення програми неможливо</b><br />У підпорядкованому вузлі інформаційної бази оновлення отримується з головного вузла.'"),
			,
			СообщениеЖурнала);
		Элементы.КнопкаДалее.Видимость  = Ложь;
		Элементы.КнопкаОтмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
		Возврат;
	ИначеЕсли ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент()
		И Не ЭтоСценарийСообщениеОНерекомендуемойВерсииПлатформы(ЭтотОбъект) Тогда
		СообщениеЖурнала = НСтр("ru='Обновление программы невозможно. Обновление программы недоступно при работе в режиме веб-клиента.';uk='Оновлення програми неможливо. Оновлення програми недоступно при роботі в режимі веб-клієнта.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
		ОтобразитьВнутреннююОшибку(
			НСтр("ru='<b>Обновление программы невозможно</b><br />
                ...
                |;uk='<b>Оновлення програми неможливо</b><br />
                |Оновлення програми недоступно при роботі в режимі веб-клієнта.<br />
                |Відкрийте програму в тонкому або товстому клієнті і повторіть спробу.'"),
			,
			СообщениеЖурнала);
		Элементы.КнопкаДалее.Видимость  = Ложь;
		Элементы.КнопкаОтмена.Заголовок = НСтр("ru='Закрыть';uk='Закрити'");
	...
	ИначеЕсли (ЭтоСценарийРабочегоОбновления(ЭтотОбъект)
		...
		ТекстИсключения =
			НСтр("ru='Получение обновлений программы недоступно в текущем режиме работы.';uk='Отримання оновлень програми недоступне в поточному режимі роботи.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
	...
	Если ЭтоСценарийПереходаНаНовуюВерсиюПлатформы(ЭтотОбъект)
		...
		Если Не ЭтоАдминистраторСистемы Тогда
			...
			ВызватьИсключение НСтр("ru='Недостаточно прав для перехода на новую версию платформы.';uk='Недостатньо прав для переходу на нову версію платформи.'");
		КонецЕсли;
		
		ЭтотОбъект.АвтоЗаголовок = Ложь;
		ЭтотОбъект.Заголовок     = НСтр("ru='Переход на новую версию платформы';uk='Перехід на нову версію платформи'");
	...
	ИначеЕсли Не ЭтоСценарийРабочегоОбновления(ЭтотОбъект) Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неизвестный сценарий работы помощника обновления программы (""%1"").';uk='Невідомий сценарій роботи помічника оновлення програми (""%1"").'"),
			Сценарий);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаПолучениеИУстановка
		...
		Если ЗавершениеРаботы Тогда
			ТекстПредупреждения = НСтр("ru='Не завершена установка платформы.';uk='Не завершено встановлення платформи.'");
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Если ЗавершениеРаботы Тогда
		Если ЗначениеЗаполнено(ИДФоновогоЗадания) Тогда
			Отказ               = Истина;
			ТекстПредупреждения = НСтр("ru='Получение и установка обновлений не завершены.';uk='Отримання й встановлення оновлень не завершені.'");
		ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.ВыборРежимаОбновленияФайловыйРежим
			...
			Отказ               = Истина;
			ТекстПредупреждения = НСтр("ru='Не установлено обновление программы.';uk='Не встановлено оновлення програми.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДекорацияЗагрузитьДистрибутивКомпонентаПлатформыОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если ПустаяСтрока(URLСтраницыПлатформы) Тогда
		ПоказатьПредупреждение(, НСтр("ru='Адрес страницы новой версии платформы не определен.';uk='Адреса сторінки нової версії платформи не визначена.'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьНавигационнуюСсылку(Ссылка)
	...
	ИначеЕсли Ссылка = "open:DistribFolder" Тогда
		...
		ОбщегоНазначенияКлиент.ОткрытьПроводник(КонтекстОбновления.КаталогДистрибутиваПлатформы);
	...
	ИначеЕсли НРег(Лев(Ссылка, 28)) = "mailto:webits-info@bas-soft." Тогда
	...
	ИначеЕсли Ссылка = "open:debuglog" Тогда
		...
		ОбщегоНазначенияКлиент.ОткрытьФайлВПрограммеПросмотра(КонтекстОбновления.ПутьФайлаПротокола);
...
Процедура ОтправитьСообщениеВТехПоддержку(АдресПолучателя)
	...
	Иначе
		Тема = НСтр("ru='Интернет-поддержка. Обновление программы';uk='Інтернет-підтримка. Оновлення програми'");
	КонецЕсли;
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаНеУдалосьПодключитьсяКСервису Тогда
		...
		Сообщение = НСтр("ru='Ошибка при подключении сервиса автоматического обновления программы.';uk='Помилка при підключенні сервісу автоматичного оновлення програми.'")
	...
	ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаСообщениеСервиса Тогда
		...
		Если ПустаяСтрока(ПодробноеОписаниеОшибки) Тогда
			Сообщение = НСтр("ru='Сообщение при подключении сервиса автоматического обновления программы:';uk='Повідомлення про підключення сервісу автоматичного оновлення програми:'")
				+ Символы.ПС
		...
		Иначе
				...
				Если ПолучениеОбновленийПрограммыКлиентСервер.ФайлСуществует(КонтекстОбновления.ПутьФайлаПротокола) Тогда
					Вложения.Добавить(
						Новый Структура("Представление, ИмяФайла",
							НСтр("ru='Протокол установки платформы 1С_Предприятие.txt';uk='Протокол установки платформи 1С_Предприятие.txt'"),
			...
			Иначе
				...
				Сообщение = НСтр("ru='Не удалось обновить программу.';uk='Не вдалося оновити програму.'")
					+ Символы.ПС
					+ НСтр("ru='Причина:';uk='Причина:'")
					+ Символы.ПС
					+ ПодробноеОписаниеОшибки;
...
Процедура ОткрытьВнутреннийФайлИнструкции(ИмяМакета, ИмяФайла)
	...
	Попытка
		ОбщегоНазначенияКлиент.ОткрытьФайлВПрограммеПросмотра(ПутьФайлаИнструкции);
	Исключение
...
Функция УстановитьОбновлениеКонфигурации()
	...
	Для каждого ТекФайл Из ПараметрыУстановки.ФайлыОбновления Цикл
		СтрОписаниеФайловОбновлений = СтрОписаниеФайловОбновлений + Символы.ПС
			+ "  " + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Имя файла: %1, Выполнить обработчики обновления: %2;';uk='Ім''я файлу: %1, Виконати обробники оновлення: %2;'"),
	...
	СообщениеЖурналаРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Вызов интерфейса установки обновления (СтандартныеПодсистемы.ОбновлениеКонфигурации).
            |Файлы обновления: %1
            |Платформа: %2'
            |;uk='Виклик інтерфейсу встановлення оновлення (СтандартныеПодсистемы.ОбновлениеКонфигурации).
            |Файли оновлення: %1
            |Платформа: %2'"),
			СтрОписаниеФайловОбновлений,
...
Процедура ОтобразитьИнформациюОДоступномОбновлении(ЗаполнитьНачальныеНастройки = Ложь)
	...
	Иначе
		...
		ДоступноОбновлениеКонфигурации = Не ПустаяСтрока(ИнформацияОДоступномОбновлении.Конфигурация.Версия);
...
Процедура ОтобразитьСтраницуНоваяВерсияПлатформыУжеУстановленаНаКомпьютере()
	...
	Элементы.НадписьВерсияУстановлено.Заголовок =
		НСтр("ru='Версия';uk='Версія'") + " " + ОбновлениеПлатформы.Версия;
...
Процедура ОтобразитьКраткуюИнформациюОДоступномОбновлении()
	...
	Если Не ОбновлениеКомКонф.ДоступноОбновление И Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Если ЭтоСценарийРабочегоОбновления(ЭтотОбъект) Или ПустаяСтрока(ЗаголовокНетОбновления) Тогда
			ТекстСообщения = ТекстСообщения
				+ НСтр("ru='Обновление не требуется. Установлена актуальная версия программы.';uk='Оновлення не потрібне. Встановлена актуальна версія програми.'");
		Иначе
	...
	Иначе
		...
		Если ПрямоеОбновление Тогда
			ТекстСообщения = ТекстСообщения + НСтр("ru='Доступно обновление программы.';uk='Доступно оновлення програми.'");
		Иначе
			ТекстСообщения = ТекстСообщения
		...
		Если РазмерОбновления <> 0 Тогда
			ТекстСообщения = ТекстСообщения + ?(ПрямоеОбновление, " ", "<br />")
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Размер дистрибутива: <b>%1</b>';uk='Розмір дистрибутива: <b>%1</b>'"),
		...
		Если ОбновитьКонфигурацию Тогда
			Если ЭтоАдминистраторСистемы И Не ПустаяСтрока(ОбновлениеКомКонф.URLПорядокОбновления) Тогда
				ТекстСообщения = ТекстСообщения
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<br /><b>Внимание!</b> Для продолжения необходимо ознакомиться с информацией о <a href=""%1"">порядке обновления</a>.';uk='<br /><b>Увага!</b> Для продовження необхідно ознайомитися з інформацією про <a href=""%1"">порядок оновлення</a>.'"),
		...
		Если Не ЭтоАдминистраторСистемы Тогда
			ТекстСообщения = ТекстСообщения
				+ НСтр("ru='<br />Для обновления программы обратитесь к администратору.';uk='<br />Для оновлення програми зверніться до адміністратора.'");
		КонецЕсли;
...
Процедура ОтобразитьКомпонентКонфигурации()
	...
	Если ПустаяСтрока(ОбновлениеКомКонфигурации.Версия) Тогда
		...
		Элементы.ДекорацияЗаголовокОбновлениеКонфигурации.Заголовок = НСтр("ru='Обновление конфигурации';uk='Оновлення конфігурації'");
			...
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>Обновление не требуется. Текущая версия конфигурации: <b>%1</b><br />
                        |%2</body>'
                        |;uk='<body>Оновлення не потрібне. Поточна версія конфігурації: <b>%1</b><br />
                        |%2</body>'"),
	...
	Иначе
		...
		Элементы.ДекорацияЗаголовокОбновлениеКонфигурации.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>Обновление конфигурации, <b>%1</b></body>';uk='<body>Оновлення конфігурації, <b>%1</b></body>'"),
					ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРазмераФайла(
				...
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><b>Версия %1</b>   <a href=""%2"">Новое в версии</a><br />
                            ...
                            |%4</body>'
                            |;uk='<body><b>Версія %1</b> <a href=""%2"">Нове у версії</a><br />
                            |Поточна версія конфігурації: %3<br />
                            |%4</body>'"),
				...
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><b>Версия %1</b><br />
                            ...
                            |%3</body>'
                            |;uk='<body><b>Версія %1</b><br />
                            |Поточна версія конфігурації: %2<br />
                            |%3</body>'"),
						ОбновлениеКомКонфигурации.Версия,
						ИнтернетПоддержкаПользователей.ВерсияКонфигурации(),
						ИнтернетПоддержкаПользователей.СинонимКонфигурации()));
...
Процедура ОтобразитьКомпонентПлатформы()
	...
	Если Не ОбновлениеКомПл.ДоступноОбновление Тогда
		...
		Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
			ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<body>Обновление не требуется. Текущая версия платформы: <b>%1</b></body>';uk='<body>Оновлення не потрібне. Поточна версія платформи: <b>%1</b></body>'"),
					ПолучениеОбновленийПрограммыКлиентСервер.ТекущаяВерсияПлатформы1СПредприятие()));
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body>Обновление платформы, <b>%1</b></body>';uk='<body>Оновлення платформи, <b>%1</b></body>'"),
						ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеРазмераФайла(
							ОбновлениеКомПл.РазмерОбновления)));
		Иначе
			Элементы.ДекорацияЗаголовокОбновлениеПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
						НСтр("ru='<body>Обновление платформы</body>';uk='<body>Оновлення платформи</body>'"));
		КонецЕсли;
				...
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><b>Версия %1</b>   <a href=""%2"">Особенности перехода на новую версию платформы</a><br />
                            |Текущая версия платформы: %3</body>'
                            |;uk='<body><b>Версія %1</b> <a href=""%2"">Особливості переходу на нову версію платформи</a><br />
                            |Поточна версія платформи: %3</body>'"),
		...
		Иначе
			...
			Элементы.ДекорацияНомерВерсииПлатформы.Заголовок =
				ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='<body><b>Версия %1</b><br />
                            |Текущая версия платформы: %2</body>'
                            |;uk='<body><b>Версія %1</b><br />
                            |Поточна версія платформи: %2</body>'"),
						ОбновлениеКомПл.Версия,
						ПолучениеОбновленийПрограммыКлиентСервер.ТекущаяВерсияПлатформы1СПредприятие()));
			...
			Если ОбновитьКонфигурацию Тогда
				...
				Если ОбновлениеКомПл.ОбязательностьУстановки = 0 Тогда
					Элементы.ДекорацияПояснениеПлатформа.Заголовок =
						ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='<body>Для работы новой версии конфигурации требуется платформа не ниже версии <b>%1</b>.</body>';uk='<body>Для роботи нової версії конфігурації потрібна платформа не нижче версії <b>%1</b>.</body>'"),
...
Процедура НачатьПолучениеОбновлений()
	...
	Иначе
		...
		Состояние(, , НСтр("ru='Подключение к сервису обновлений';uk='Підключення до сервісу оновлень'"));
...
Функция ОписаниеФайловОбновленийНаСервере(
	...
	Если Не Пользователи.ЭтоПолноправныйПользователь(, Истина, Ложь) Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав.';uk='Недостатньо прав.'");
	КонецЕсли;
...
Процедура ЗагрузитьОбновлениеКонфигурации()
	...
	ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загрузка файла обновления конфигурации %1 (выполняется на сервере информационной базы).';uk='Завантаження файлу оновлення конфігурації %1 (виконується на сервері інформаційної бази).'"),
...
Процедура ПриПолученииФайлаОбновленияКонфигурации(ПолученныеФайлы, РезультатПолучения) Экспорт
	...
	ПолучениеОбновленийПрограммыКлиент.ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru='Файл обновления успешно загружен.';uk='Файл оновлення успішно завантажений.'"));
...
Процедура ИнициализироватьСтраницуПолучениеОбновления()
	...
	Иначе
		...
		Элементы.ДекорацияПолучениеДистрибутива.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполняется получение файлов обновления (%1). Получение файлов может занять от нескольких минут до';uk='Виконується отримання файлів оновлення (%1). Отримання файлів може зайняти від декількох хвилин до'")
					+ " " + НСтр("ru='нескольких часов в зависимости от размера обновления,';uk='декількох годин в залежності від розміру оновлення,'")
					+ " " + НСтр("ru='скорости подключения к Интернету и производительности компьютера.';uk='швидкості підключення до Інтернету і продуктивності комп''ютера.'"),
				РазмерОбновленияСтр(ЭтотОбъект));
	КонецЕсли;
...
Процедура НачатьПолучениеОбновленийВФоновомРежиме()
	...
	Попытка
		Задание = ФоновыеЗадания.Выполнить(
			...
			,
			НСтр("ru='Получение и установка обновлений программы';uk='Отримання та встановлення оновлень програми'"));
	Исключение
	...
	Если КонтекстОбновления = Неопределено Тогда
		Прогресс = 0;
		ТекущееДействие = НСтр("ru='Подготовка к получению обновления...';uk='Підготовка до отримання оновлення...'");
	КонецЕсли;
...
Процедура ОтобразитьСостояниеКонтекстаОбновления()
	...
	Если КонтекстОбновления.Свойство("ФайлыОбновленияПолучены") Тогда
		...
		ИначеЕсли КонтекстОбновления.ОбновитьПлатформу Тогда
			...
			// тогда отображается состояние установки платформы.
...
Функция СостояниеЗадания(Знач ИДЗадания)
	...
	Если Задание = Неопределено Тогда
		...
		Результат.Вставить(
			"ДопПараметры",
			Новый Структура("Сообщение, ИнформацияОбОшибке",
				НСтр("ru='Задание не найдено.';uk='Завдання не знайдено.'"),
				НСтр("ru='Задание не найдено.';uk='Завдання не знайдено.'")));
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		...
		СообщениеЖурнала = НСтр("ru='Задание отменено администратором системы.';uk='Завдання скасовано адміністратором системи.'");
		ПолучениеОбновленийПрограммы.ЗаписатьОшибкуВЖурналРегистрации(СообщениеЖурнала);
...
Процедура ОтобразитьПолучениеОбновленийЗавершено(Инициализировать = Ложь)
	...
	Иначе
		...
		Если ПараметрыАдминистрирования = Неопределено Тогда
			...
			ЗаголовокФормы = НСтр("ru='Установка обновления';uk='Встановлення оновлення'");
			Если ЭтоФайловаяИБ Тогда
	...
	Иначе
		...
		ОтобразитьВнутреннююОшибку(
			НСтр("ru='Не удалось установить обновление программы, т.к. не были введены
                ...
                |;uk='Не вдалося встановити оновлення програми, т. я. не були введені
                |коректні параметри адміністрування інформаційної бази.</body>'"),
				,
				ТекстСообщения);
	...
	Иначе
		...
		Если ЭтоФайловаяИБ Тогда
			...
			Если Инициализировать Тогда
				...
				Если Элементы.ПорядокОбновленияФайл.Видимость Тогда
					Элементы.НадписьПорядокОбновленияФайл.Заголовок =
						ИнтернетПоддержкаПользователейКлиентСервер.ФорматированныйЗаголовок(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='<body>Для продолжения необходимо ознакомиться с <a href=""%1"">порядком обновления</a>.</body>';uk='<body>Для продовження необхідно ознайомитися з <a href=""%1"">порядком оновлення</a>.</body>'"),
								ИнформацияОДоступномОбновлении.Конфигурация.URLПорядокОбновления));
				КонецЕсли;
			...
			Элементы.КнопкаДалее.Заголовок = ?(
				РежимОбновления = 0,
				НСтр("ru='Далее >';uk='Далі >'"),
				НСтр("ru='Готово';uk='Готово'"));
