...
Процедура ПровестиПлатежныеДокументы()
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		АдресХранилища = ПоместитьВоВременноеХранилище(Неопределено, Новый УникальныйИдентификатор);
		Обработки.ФормированиеПлатежныхДокументовПоНалогам.ПровестиДокументы(ПараметрыПроведения, АдресХранилища);
		Результат = Новый Структура("ЗаданиеВыполнено", Истина);
	Иначе
		НаименованиеЗадания = НСтр("ru='Создание платежных документов на уплату налогов';uk='Створення платіжних документів на сплату податків'");
		...
		Результат = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		Новый УникальныйИдентификатор,
		"Обработки.ФормированиеПлатежныхДокументовПоНалогам.ПровестиДокументы", 
		ПараметрыПроведения, 
		НаименованиеЗадания);
