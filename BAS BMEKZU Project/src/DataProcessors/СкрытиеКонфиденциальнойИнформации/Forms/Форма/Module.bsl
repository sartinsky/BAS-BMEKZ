...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru='Скрытие конфиденциальной информации';uk='Приховування конфіденційної інформації'");
	...
	Если ДеревоМетаданныхЗаполнено Тогда
		Если Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности Тогда
			...
			Если МакетОпределен Тогда
				Заголовок = НСтр("ru='Скрытие конфиденциальной информации - полные возможности';uk='Приховування конфіденційної інформації - повні можливості'");
			КонецЕсли;
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ru='Обычный вид';uk='Звичайний вигляд'");
		Иначе
			Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаЧастотныеВозможности;
			Заголовок = НСтр("ru='Скрытие конфиденциальной информации';uk='Приховування конфіденційної інформації'");
			Элементы.РасширенныеВозможности.Заголовок = НСтр("ru='Полные возможности';uk='Повні можливості'");
		КонецЕсли;
	Иначе
		Если ДеревоМетаданныхЗаполнено() Тогда
			Элементы.СтраницыВидФормы.ТекущаяСтраница = Элементы.СтраницаРасширенныеВозможности;
...
Процедура ЗначениеЧислоПриИзменении(Элемент)
	Если ЗначениеЧисло = 0 Тогда
		ТекстПредупреждения = НСтр("ru='Значение должно быть не нулевым.';uk='Значення повинне бути нульовим.'");
...
Процедура ВыполнитьСкрытие(Команда)
	...
	ТекстПредупреждения = НСтр("ru='Скрытие конфиденциальной информации необходимо выполнять на копии рабочей базы,
        ...
        |Продолжить?'
        |;uk='Приховування конфіденційної інформації необхідно виконувати на копії робочої бази,
        |всі зміни будуть виконані безповоротно.
        |Продовжити?'");
...
Процедура Подключаемый_ВыполнитьСкрытиеЗавершение()
	...
	Если РезультатЗапуска.Статус <> "Выполнено" Тогда
		...
		КонецЕсли;
	Иначе
		СообщитьПользователю(НСтр("ru='Скрытие конфиденциальной информации завершено.';uk='Приховування конфіденційної інформації завершено.'"));
	КонецЕсли;
...
Процедура СохранитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогСохранения.Фильтр = НСтр("ru='Файл настроек';uk='Файл настройок'") + "(*.xml)|*.xml";
...
Процедура ЗагрузитьНастройкиПродолжение(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если Подключено Тогда
		...
		ДиалогВыбора.Фильтр = НСтр("ru='Файл настроек';uk='Файл настройок'") + "(*.xml)|*.xml";
...
Процедура ЗаполнитьДеревоОбрабатываемыхОбъектов(СохраненныеНастройки, ОбработкаОбъект)
	...
	ОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	ОбрабатываемыеМетаданные = ОбработкаОбъект.ОбрабатываемыеМетаданные();
	ДеревоОбрабатываемыхОбъектов = РеквизитФормыВЗначение("ОбрабатываемыеОбъекты");
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Скрытие конфиденциальной информации';uk='Приховування конфіденційної інформації'");
