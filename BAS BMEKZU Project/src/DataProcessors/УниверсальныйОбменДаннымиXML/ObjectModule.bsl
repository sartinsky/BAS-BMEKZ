	...
	ИначеЕсли ФлагРежимОтладкиОбработчиков Тогда
		...
		Попытка
			...
			Если ЭтоВнешняяОбработка() Тогда
				...
				ВызватьИсключение
					НСтр("ru='Внешняя обработка отладки, загружаемая из файла на диске, не поддерживается.';uk='Зовнішня обробка налагодження, яка завантажується з файлу на диску, не підтримується.'");
...
Функция ЗаписьТекстаВоВременныйФайл(СписокВременныхФайлов)
	...
	Исключение
		ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000,
			ОписаниеОшибки(),
			НСтр("ru='Ошибка при создании временного файла для выгрузки данных';uk='Помилка при створенні тимчасового файлу для вивантаження даних'"));
		ВызватьИсключение;
	КонецПопытки;
...
Функция ЧтениеТекстаИзВременногоФайла(ИмяВременногоФайла)
	...
	Исключение
		ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000,
			ОписаниеОшибки(),
			НСтр("ru='Ошибка при открытии временного файла для переноса данных в файл обмена';uk='Помилка при відкритті тимчасового файлу для перенесення даних у файл обміну'"));
		ВызватьИсключение;
	КонецПопытки;
...
Процедура КомментарииКЗагрузкеОбъекта(НПП, ИмяПравила, Источник, ТипОбъекта, ГНПП = 0)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		Если НПП <> 0 Тогда
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка объекта № %1';uk='Завантаження об''єкту № %1'"), НПП);
		Иначе
			СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Загрузка объекта № %1';uk='Завантаження об''єкту № %1'"), ГНПП);
		КонецЕсли;
...
Процедура ВыгрузкаОбъектаВыборки(Объект, Правило, Свойства=Неопределено, ВходящиеДанные=Неопределено, ВыборкаДляВыгрузкиДанных = Неопределено)
	...
	Если ФлагКомментироватьОбработкуОбъектов Тогда
		...
		СтрокаСообщения = ПодставитьПараметрыВСтроку(НСтр("ru='Выгрузка объекта: %1';uk='Вивантаження об''єкту: %1'"), ПрОбъекта);
...
Процедура ВыполнитьДействияПослеЗавершенияЧтенияДанных() Экспорт
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Загружено объектов: %1';uk='Завантажено об''єктів: %1'"), мСчетчикЗагруженныхОбъектов), , Ложь, , , Истина);
...
Процедура ВыполнитьВыгрузку() Экспорт
	...
	ЗаписатьВПротоколВыполнения(ПодставитьПараметрыВСтроку(
		НСтр("ru='Выгружено объектов: %1';uk='Вивантажено об''єктів: %1'"), мСчетчикВыгруженныхОбъектов), , Ложь, , , Истина);
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(8,  НСтр("ru='Некорректно указано имя файла для выгрузки данных';uk='Некоректно вказано ім''я файлу для вивантаження даних'"));
