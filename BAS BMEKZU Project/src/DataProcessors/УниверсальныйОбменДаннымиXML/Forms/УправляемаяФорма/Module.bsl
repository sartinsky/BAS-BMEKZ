...
Процедура ПрочитатьПравилаОбмена(Команда)
	...
	Если Не ЭтоWindowsКлиент() И ПрямаяВыгрузка = 1 Тогда
		ПоказатьПредупреждение(,НСтр("ru='Прямое подключение к информационной базе поддерживается только в клиенте под управлением ОС Windows.';uk='Пряме підключення до інформаційної бази підтримується тільки в клієнті під управлінням ОС Windows.'"));
		Возврат;
	КонецЕсли;
