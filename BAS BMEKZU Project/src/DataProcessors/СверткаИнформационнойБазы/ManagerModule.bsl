...
Процедура СоздатьДокументыВводаОстатков(Знач ПараметрыОбработки, АдресХранилища) Экспорт
	...
	Попытка
		...
		Для Каждого ДанныеРегистра Из СпособыСверткиОбъектовМетаданных Цикл
			...
			ИначеЕсли ДанныеРегистра.ТипОбъектаМД = "РегистрРасчета" Тогда	// ИНАГРО
				СформироватьДокументыВводаОстатковПоРегистрамРасчета(ДанныеРегистра, ПараметрыОбработки);
	...
	Пока ВыборкаПоОрганизации.Следующий() Цикл
		...
		Пока ВыборкаПоСчетам.Следующий() Цикл
			...
			Если (НЕ ВыборкаПоСчетам.СуммаОстаток = 0) ИЛИ (НЕ ВыборкаПоСчетам.ВалютнаяСуммаОстаток = 0) ИЛИ (НЕ ВыборкаПоСчетам.КоличествоОстаток = 0) 
				...
				ИЛИ (НЕ ВыборкаПоСчетам.СуммаНУОстаток = 0) Тогда
				...
				ДокументОбъект.Комментарий = "*" + НСтр("ru='Сформирован обработкой свертки базы. ';uk='Сформований обробкою згортки бази. '", Локализация.КодЯзыкаИнформационнойБазы()) + Формат(ТекущаяДата(),"ДФ=dd.MM.yyyy");
				ДокументОбъект.Содержание = НСтр("ru='Ввод остатков по регистрам бухгалтерии: Хозрасчетный. Счет: ';uk='Введення залишків по регістрах бухгалтерії: Госпрозрахунковий. Рахунок: '", Локализация.КодЯзыкаИнформационнойБазы()) + ВыборкаПоСчетам.Счет.Код;
				...
				Пока ВыборкаДетали.Следующий() Цикл
					...
					Если ((ВыборкаПоСчетам.Счет.Вид =  ВидСчета.Активный) ИЛИ (ВыборкаПоСчетам.Счет.Вид =  ВидСчета.АктивноПассивный))
						И ((НЕ ВыборкаДетали.СуммаОстатокДт = 0) ИЛИ (НЕ ВыборкаДетали.ВалютнаяСуммаОстатокДт = 0) ИЛИ (НЕ ВыборкаДетали.КоличествоОстатокДт = 0)
 						ИЛИ (НЕ ВыборкаДетали.СуммаНУОстатокДт = 0) ) Тогда
						...
						НоваяЗапись.НалоговоеНазначениеДт = ВыборкаДетали.НалоговоеНазначение;
					...
					Если ((ВыборкаПоСчетам.Счет.Вид =  ВидСчета.Пассивный) ИЛИ (ВыборкаПоСчетам.Счет.Вид =  ВидСчета.АктивноПассивный))
						...
						ИЛИ (НЕ ВыборкаДетали.СуммаНУОстатокКт = 0) ) Тогда
 						НоваяЗапись = НаборЗаписей.Добавить();
						НоваяЗапись.Организация = ВыборкаПоОрганизации.Организация;
						НоваяЗапись.Активность = Ложь;
						НоваяЗапись.Период = ДокументОбъект.Дата;
						...
						НоваяЗапись.НалоговоеНазначениеКт = ВыборкаДетали.НалоговоеНазначение;
...
// Процедура формирует документы ввода остатков по переданному списку регистров расчета
//
Процедура СформироватьДокументыВводаОстатковПоРегистрамРасчета(СтрокаСписка, Параметры)  // ИНАГРО
	
	Если СтрокаСписка.СпособСвертки <> "Свернуть" Тогда
		Возврат;
	КонецЕсли;
	
	ТекущийРегистр = Метаданные.РегистрыРасчета.Найти(СтрокаСписка.ОбъектМД);
	...
	Запрос.Текст = 
		...
		|	ЛОЖЬ КАК Активность,
		|	&ПериодЗаписи КАК Период,
		|   &ПериодЗаписи КАК ПериодРегистрации,
		|   ТаблицаРегистра.ВидРасчета КАК ВидРасчета,
		|   ТаблицаРегистра.БазовыйПериодНачало КАК БазовыйПериодНачало,
		|   ТаблицаРегистра.БазовыйПериодКонец КАК БазовыйПериодКонец,";
	Если СтрокаСписка.ОбъектМД = "ИНАГРО_Начисления" Тогда
		Запрос.Текст = Запрос.Текст + "
		|   ТаблицаРегистра.ПериодДействия КАК ПериодДействия,
		|   ТаблицаРегистра.ПериодДействияНачало КАК ПериодДействияНачало,
		|   ТаблицаРегистра.ПериодДействияКонец КАК ПериодДействияКонец,";
	КонецЕсли;
	...
	Запрос.Текст = Запрос.Текст + "
								  |ИЗ
								  |РегистрРасчета." + ТекущийРегистр.Имя + " КАК ТаблицаРегистра"
								  + УсловиеИтогов;
	...
	СтруктураПараметров.Вставить("ТекущийРегистр", ТекущийРегистр);
	СтруктураПараметров.Вставить("ПериодСвертки", Параметры.ПериодСвертки);
	СтруктураПараметров.Вставить("ТипРегистра", "РегистрРасчета");
	СтруктураПараметров.Вставить("НаличиеИзмеренияОрганизация", НаличиеИзмеренияОрганизация);
	СтруктураПараметров.Вставить("ИмяИзмерения", ИмяИзмерения);
	СтруктураПараметров.Вставить("ОрганизацияДляЗаполненияШапкиДокумента", Параметры.ОрганизацияДляЗаполненияШапкиДокумента);
	...
	Если НЕ Результат.Пустой() Тогда
		Если Параметры.НаличиеИзмеренияОрганизация Тогда
			...
			Пока ВыборкаПоОрганизациям.Следующий() Цикл
				...
				ИначеЕсли Параметры.ТипРегистра = "РегистрРасчета" Тогда // ИНАГРО
					НаборЗаписей = РегистрыРасчета[Параметры.ТекущийРегистр.Имя].СоздатьНаборЗаписей();
				Иначе
...
Функция СформироватьШапкуДокументаВводаОстатков(ТекущийРегистр, ДатаСвертки, ТипРегистра, Организация = Неопределено)
	...
	ДокументОстатков.Комментарий = "*" + НСтр("ru='Сформирован обработкой свертки базы. ';uk='Сформований обробкою згортки бази. '", Локализация.КодЯзыкаИнформационнойБазы()) + Формат(ТекущаяДата(),"ДФ=dd.MM.yyyy");
	ДокументОстатков.Содержание = НСтр("ru='Ввод остатков по регистрам ';uk='Введення залишків по регістрах '", Локализация.КодЯзыкаИнформационнойБазы()) + ТипРегистра + ": " + ТекущийРегистр.Имя;
	...
	ИначеЕсли ТипРегистра = "РегистрРасчета" Тогда
		НоваяСтрока = ДокументОстатков.ТаблицаРегистровРасчета.Добавить();
	Иначе
...
Процедура УдалитьДокументыВводаОстатков(ПараметрыОбработки, АдресХранилища) Экспорт
	...
	Попытка
		...
		Запрос.Текст = 
			...
			|	И (ОперацияБух.Комментарий ПОДОБНО &КомментарийРус
			|	ИЛИ ОперацияБух.Комментарий ПОДОБНО &КомментарийУкр)";
		
		Запрос.УстановитьПараметр("КомментарийРус", "%*Сформирован обработкой свертки базы%");
		Запрос.УстановитьПараметр("КомментарийУкр", "%*Сформований обробкою згортки бази%");
			...
			Для Каждого ТекРегистрДвижений Из ДокументОбъект.Метаданные().Движения Цикл
				...
				Если ДокументОбъект.Движения[ТекРегистрДвижений.Имя].Количество() > 0 Тогда
					Для Каждого ТекДвижение Из ДокументОбъект.Движения[ТекРегистрДвижений.Имя] Цикл
						Если ТекРегистрДвижений.Имя = "ИНАГРО_ВзносыВФонды" 
							ИЛИ ТекРегистрДвижений.Имя = "ИНАГРО_Начисления"
							ИЛИ ТекРегистрДвижений.Имя = "ИНАГРО_Удержания"
							ИЛИ ТекРегистрДвижений.Имя = "ИНАГРО_Отражение"
							ИЛИ ТекРегистрДвижений.Имя = "ИНАГРО_РасчетСреднегоЗаработка" Тогда
							ТекДвижение.ПериодРегистрации = ДокументОбъект.Дата;
						Иначе
...
Процедура УдалитьДвиженияПоПодчиненномуРегистру(ПараметрыРегистра, ПараметрыОбработки, ПериодическийРегистрСведений = Истина)
	...
	ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ 10000
		...
		|ИЗ
		|	" + ПараметрыРегистра.ТипОбъектаМД + "."+ПараметрыРегистра.ОбъектМД+" КАК ДокументРег
		|ГДЕ
		|	ИСТИНА";
	
	Если ПараметрыОбработки.НаличиеИзмеренияОрганизация и ПараметрыОбработки.ТолькоВыбранныеОрганизации Тогда
		ТекстЗапроса = ТекстЗапроса + "
		|	И ДокументРег." + ПараметрыОбработки.ИмяИзмерения + " В (&ВыбранныеОрганизации)";
	КонецЕсли;
	
	// Если выбран способ свертки "Очистить", то удаляются все движения
	Если ПериодическийРегистрСведений И ПараметрыРегистра.СпособСвертки <> "Очистить" Тогда
		ТекстЗапроса = ТекстЗапроса + "
			|	И ДокументРег.Период < &ПериодСвертки";
	КонецЕсли;
	
	Если ПараметрыРегистра.ТипОбъектаМД = "РегистрРасчета" Тогда
		ТекстЗапроса = ТекстЗапроса + "
			|	И ДокументРег.ПериодРегистрации < &ПериодСвертки";
	КонецЕсли;
	...
	Пока Истина Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если ПараметрыРегистра.СпособСвертки <> "Не сворачивать" Тогда
				...
				ИначеЕсли ПараметрыРегистра.ТипОбъектаМД = "РегистрРасчета" Тогда  // ИНАГРО ++
					НаборЗаписей = РегистрыРасчета[ПараметрыРегистра.ОбъектМД].СоздатьНаборЗаписей();
...
Процедура ПроверитьНеобходимостьСнятияПометкиНаУдаление(ПараметрыРегистра, ПараметрыОбработки)
	...
	МассивТимовДокументовРегистаторов = НаборЗаписей.Отбор.Регистратор.ТипЗначения.Типы();
	
	Для Каждого ТипДокументаРегистратора из МассивТимовДокументовРегистаторов Цикл
...
Процедура УдалитьДвиженияПоСписку(Данные, ПараметрыОбработки)
	...
	ИначеЕсли Данные.ТипОбъектаМД = "РегистрРасчета" Тогда   // ИНАГРО ++
		
		ТекущийРегистр = Метаданные.РегистрыРасчета.Найти(Данные.ОбъектМД);
		
		Попытка
			
			УдалитьДвиженияПоПодчиненномуРегистру(Данные, ПараметрыОбработки, Ложь);
...
Функция ЕстьИзмерениеОрганизация(ДанныеРегистра)
	...
	ИначеЕсли ДанныеРегистра.ТипОбъектаМД = "РегистрРасчета" Тогда
		ТекущийРегистр = Метаданные.РегистрыРасчета.Найти(ДанныеРегистра.ОбъектМД);
...
Процедура ПередКомпоновкойОтчетаПоРегиструБухгалтерии(Настройки,ПараметрыОтчета)
	...
	ГруппаОстаткиДоСвертки.Заголовок     = НСтр("ru='Остатки до свертки';uk='Залишки до згортки'");
	...
	ГруппаОстаткиПослеСвертки.Заголовок     = НСтр("ru='Остатки после свертки';uk='Залишки після згортки'");
	...
	ГруппаПроверка.Заголовок     = НСтр("ru='Проверка';uk='Перевірка'");
...
Функция ОбновитьТекстЗаголовкаНаСервере(ПериодСвертки) Экспорт
	...
	ЗаголовокОтчета = НСтр("ru='Проверка созданных документов ';uk='Перевірка створених документів '") + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(ПериодСвертки, КонецГода(ПериодСвертки));
...
Процедура УстановитьДатуЗапретаИзмененияДанных(ПараметрыОбработки)
	...
	Если Не ПараметрыОбработки.ТолькоВыбранныеОрганизации Тогда
			...
			СообщениеЖурналаРегистрации = НСтр("ru='Установлена новая общая дата запрета изменения данных: ';uk='Установлена нова спільна дата заборони зміни даних: '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) + ДатаЗапрета;
	...
	Иначе
		...
		Для Каждого Орг Из ПараметрыОбработки.Организации Цикл
			...
			Если ОбновитьДатыЗапрета Тогда
				...
				СЖ = НСтр("ru='Установлена новая дата запрета изменения данных для организации ';uk='Установлена нова дата заборони зміни даних для організації '", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()) 
