...
Процедура ПриНачалеРегистрацииДанных(НастройкиРегистрации, ПараметрыОбработчика, ПродолжитьОжидание = Истина) Экспорт
	...
	КлючФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Регистрация данных для выгрузки (%1)';uk='Реєстрація даних для вивантаження (%1)'"),
...
Процедура ЗарегистрироватьДанныеДляВыгрузки(Параметры, АдресРезультата) Экспорт
	...
	Исключение
		...
		Результат.СообщениеОбОшибке = НСтр("ru='Возникла проблема при добавлении данных к выгрузке:';uk='Виникла проблема при додаванні даних до вивантаження:'") 
			+ Символы.ПС + КраткоеПредставлениеОшибки(Информация)
			+ Символы.ПС + НСтр("ru='Необходимо изменить условия отбора.';uk='Необхідно змінити умови відбору.'");
...
Процедура ВыгрузитьДанныеДляСопоставления(Параметры, АдресРезультата) Экспорт
	...
	Если Не Результат.ДанныеВыгружены
		...
		Результат.СообщениеОбОшибке = НСтр("ru='При выполнении выгрузки данных для сопоставления возникли ошибки (см. Журнал регистрации).';uk='При виконанні вивантаження даних для співставлення виникли помилки (див. Журнал реєстрації).'");
	КонецЕсли;
...
Функция РезультатАвтоматическогоСопоставленияДанных(Знач Корреспондент,
	...
	Если Результат.Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось загрузить данные из ""%1"" (этап автоматического сопоставления данных).';uk='Не вдалося завантажити дані з ""%1"" (етап автоматичного співставлення даних).'"),
	...
	Для Каждого СтрокаТаблицы Из ИнформацияСтатистики Цикл
		...
		Исключение
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка: тип ""%1"" не определен.';uk='Помилка: тип ""%1"" не визначений.'"), СтрокаТаблицы.ТипОбъектаСтрокой);
			Прервать;
		КонецПопытки;
