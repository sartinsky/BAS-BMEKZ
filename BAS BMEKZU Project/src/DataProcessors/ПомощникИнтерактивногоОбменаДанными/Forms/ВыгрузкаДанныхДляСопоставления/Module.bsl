...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НачальнаяВыгрузка Тогда
		ТекстПредупреждения = НСтр("ru='Отменить начальную выгрузку данных?';uk='Скасувати початкове вивантаження даних?'");
	Иначе
		ТекстПредупреждения = НСтр("ru='Отменить выгрузку данных?';uk='Скасувати вивантаження даних?'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьИспользованияФормы(Отказ = Ложь)
	...
	Если ОбменДаннымиПовтИсп.ЭтоУзелРаспределеннойИнформационнойБазы(Параметры.УзелОбмена) Тогда
		ТекстСообщения = НСтр("ru='Начальная выгрузка не поддерживается для узлов распределенных информационных баз.';uk='Початкове вивантаження не підтримується для вузлів розподілених інформаційних баз.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , , , Отказ);
		Возврат;
	КонецЕсли;
...
Функция Подключаемый_СтраницаЗапросПароля_ПриПереходеДалее(Отказ)
	...
	Если ПустаяСтрока(WSПароль) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Укажите пароль.';uk='Вкажіть пароль.'"), , "WSПароль", , Отказ);
	КонецЕсли;
