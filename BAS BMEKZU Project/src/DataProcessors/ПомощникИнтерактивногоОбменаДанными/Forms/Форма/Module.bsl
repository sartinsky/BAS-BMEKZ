...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли Параметры.Свойство("КодУзлаИнформационнойБазы") Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.УзелИнформационнойБазы) Тогда
			ВызватьИсключение НСтр("ru='Настройка обмена данными не найдена.';uk='Настройка обміну даними не знайдена.'");
		КонецЕсли;
	...
	Иначе
		...
		ВызватьИсключение НСтр("ru='Непосредственное открытие помощника не предусмотрено.';uk='Безпосереднє відкриття помічника не передбачено.'");
	...
	Если Не ОбменДаннымиПовтИсп.ЭтоУзелУниверсальногоОбменаДанными(Объект.УзелИнформационнойБазы) Тогда
		ВызватьИсключение НСтр("ru='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';uk='Для обраного вузла виконання обміну даними з настройкою не передбачено.'");
	КонецЕсли;
	...
	Если ВсеУзлы.Найти(Объект.УзелИнформационнойБазы) = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Для выбранного узла сопоставление данных не предусмотрено.';uk=' Для обраного вузла співставлення даних не передбачене.'");
	КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ПоказатьПодтверждениеЗакрытияПроизвольнойФормы(ЭтотОбъект, Отказ, ЗавершениеРаботы,
		НСтр("ru='Выйти из помощника?';uk='Вийти з помічника?'"), "ЗакрытьФормуБезусловно");
...
Процедура ПриЗакрытии(ЗавершениеРаботы)
	...
	Если ПолученоСообщениеДляСопоставленияДанных Тогда
		Если ЗакончитьСопоставлениеДанных
			И Не ПропуститьПолучениеДанных Тогда
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "ЗакрытиеФормыСопоставленияОбъектов" Тогда
		...
		Иначе
			...
			ПоказатьОповещениеПользователя(НСтр("ru='Сбор информации завершен';uk='Збір інформації завершений'"));
		КонецЕсли;
...
Процедура ОткрытьФормуСопоставления(Команда)
	...
	Если Не ТекущиеДанные.ИспользоватьПредварительныйПросмотр Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для типа данных нельзя выполнить сопоставление объектов.';uk='Для типу даних не можна виконати співставлення об''єктів.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗагрузкуДанныхДляСтрокиПродолжение(Знач ВыделенныеСтроки) 
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Загрузка данных завершена.';uk='Завантаження даних завершено.'"));
КонецПроцедуры
		...
		Если ЗакончитьСопоставлениеДанных Тогда
			Если ЗагрузитьСообщениеПослеСопоставления Тогда
				Элементы.ДекорацияПодсказкаПереходаИнформацииСтатистики.Заголовок =
					НСтр("ru='Нажмите кнопку ""Далее"" для завершения сопоставления данных и загрузки сообщения обмена.';uk='Натисніть кнопку ""Далі"" для завершення співставлення даних і завантаження повідомлень обміну.'");
			Иначе
				Элементы.ДекорацияПодсказкаПереходаИнформацииСтатистики.Заголовок =
					НСтр("ru='Нажмите кнопку ""Сохранить и закрыть"" для завершения сопоставления данных и выхода из помощника.';uk='Натисніть кнопку ""Зберегти і закрити"" для завершення співставлення даних і виходу з помічника.'");
			КонецЕсли;
		Иначе
			Элементы.ДекорацияПодсказкаПереходаИнформацииСтатистики.Заголовок =
                ...
                |;uk='Натисніть кнопку ""Зберегти і закрити"" для збереження результатів співставлення і виходу з помічника.
                |При наступному запуску помічника можна буде продовжити співставлення даних.'");
		КонецЕсли;
	Иначе
		Элементы.ДекорацияПодсказкаПереходаИнформацииСтатистики.Заголовок =
			НСтр("ru='Нажмите кнопку ""Далее"" для синхронизации данных.';uk='Натисніть кнопку ""Далі"" для синхронізації даних.'");
	КонецЕсли;
...
Процедура УстановитьЗаголовокФормы()
	...
	ШаблонЗаголовка = НСтр("ru='Обмен данными с ""%1""';uk='Обмін даними з ""%1""'");
	
	Если ПолученоСообщениеДляСопоставленияДанных Тогда
		ШаблонЗаголовка = НСтр("ru='Сопоставление данных ""%1""';uk='Співставлення даних ""%1""'");
	ИначеЕсли ПолучитьДанные И ОтправитьДанные Тогда
		ШаблонЗаголовка = НСтр("ru='Синхронизация данных с ""%1""';uk='Синхронізація даних з ""%1""'");
	ИначеЕсли ОтправитьДанные Тогда
		ШаблонЗаголовка = НСтр("ru='Отправка данных для ""%1""';uk='Відправка даних для ""%1""'");
	ИначеЕсли ПолучитьДанные Тогда
		ШаблонЗаголовка = НСтр("ru='Получение данных от ""%1""';uk='Отримання даних від ""%1""'");
	КонецЕсли;
...
Функция СостояниеДлительнойОперации(Знач ИдентификаторОперации, УзелОбмена)
	...
	Если Результат = "Failed" Тогда
		СтрокаСообщения = НСтр("ru='Ошибка в базе-корреспонденте: %1';uk='Помилка в базі-кореспонденті: %1'");
...
Процедура ОбработатьСтатусВыполненияФоновогоЗадания()
	...
	ИначеЕсли РезультатВыполненияФоновогоЗадания.Статус = "Отменено" Тогда
		СообщениеОбОшибке = НСтр("ru='Действие отменено пользователем.';uk='Дія скасована користувачем.'");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПроверитьПодключениеИСохранитьНастройки(Отказ)
	...
	Если Не ПропускатьСтраницуТранспорта Тогда
		...
		Попытка
			...
			СтруктураЗаписи.Вставить("WSЗапомнитьПароль", WSЗапомнитьПароль);
...
Функция Подключаемый_СтраницаОжиданияАнализаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru='Получение сообщения обмена во временный каталог';uk='Отримання повідомлення обміну в тимчасовий каталог'");
	...
	Если ДлительнаяОперацияЗавершена Тогда
		...
		Иначе
			...
			Если Не ЗначениеЗаполнено(Объект.ИмяФайлаСообщенияОбмена) Тогда
				...
				ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
				ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru='Получение файла с данными сообщения обмена во временный каталог';uk='Отримання файлу з даними повідомлення обміну в тимчасовий каталог'");
...
Процедура ЗавершениеПолученияДанныхВоВременныйКаталогНаСервере();
	...
	ШаблонСообщенияОбОшибке = НСтр("ru='Не удалось выполнить загрузку данных. Подробности см. в журнале регистрации';uk='Не вдалося виконати завантаження даних. Подробиці див. у журналі реєстрації'");
...
Функция Подключаемый_АнализДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru='Анализ данных сообщения обмена';uk='Аналіз дані повідомлення обміну'");
...
Процедура ЗавершениеАнализаДанныхНаСервере()
	...
	Если ЗафиксироватьОшибку Тогда
		...
		ТекстСообщения = ?(ЗначениеЗаполнено(СообщениеОбОшибке), СообщениеОбОшибке,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось загрузить данные из ""%1"" (этап анализа данных).';uk='Не вдалося завантажити дані з ""%1"" (етап аналізу даних).'"), Объект.УзелИнформационнойБазы));
...
Функция Подключаемый_СтраницаИнформацииСтатистики_ПриПереходеДалее(Отказ)
	...
	Сообщение = НСтр("ru='Не все данные сопоставлены. Наличие несопоставленных данных
                           ...
                           |Продолжить?'
                           |;uk='Не всі дані співставлені. Наявність неспівставлених даних
                           |може призвести до появи однакових елементів у списках (дублів).
                           |Продовжити?'");
...
Функция Подключаемый_ЗагрузкаДанных_ОбработкаДлительнойОперации(Отказ, ПерейтиДалее)
	...
	ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
	ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru='Загрузка данных из сообщения обмена';uk='Завантаження даних з повідомлення обміну'");
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не удалось зарегистрировать данные для выгрузки.';uk='Не вдалося зареєструвати дані для вивантаження.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриНачалеВыгрузкиДанных(Отказ)
	...
	Иначе
		...
		ПараметрыЗадания.ПараметрыМетода      = ПараметрыМетода;
		ПараметрыЗадания.НаименованиеЗадания  = НСтр("ru='Выгрузка данных в сообщение обмена';uk='Вивантаження даних в протокол обміну'");
...
Процедура ПриЗавершенииВыгрузкиДанных()
	...
	Иначе
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru='Не удалось выполнить обмен данными.';uk='Не вдалося виконати обмін даними.'"));
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗавершениеВыгрузкиНаСервере()
	...
	Если РезультатВыполненияМетода.Отказ
		И Не ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
		СообщениеОбОшибке = НСтр("ru='Не удалось выполнить отправку данных. Подробности см. в журнале регистрации';uk='Не вдалося виконати відправку даних. Подробиці див. у журналі реєстрації'");
	КонецЕсли;
