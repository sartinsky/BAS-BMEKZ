...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НовыйЭлементСписка = СписокВыбора.Добавить("СопоставленныеОбъекты", НСтр("ru='Сопоставленные данные';uk='Співставлені дані'"));
	ВариантыОтбораСтатусаСопоставления.Вставить(НовыйЭлементСписка.Значение, 
	...
	КонецЕсли;
	Заголовок = НСтр("ru='Сопоставление данных ""[ПредставлениеДанных]""';uk='Співставлення даних ""[ПредставлениеДанных]""'");
	...
	Элементы.Пояснение.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
        ...
        |;uk='Для співставлення даних ""%1""
        |с даними ""%2"" скористайтеся командою ""Співставити автоматично..."".
        |неспівставлені дані, що залишилися, можна співставити один з одним вручну.'"),
...
Процедура ВыполнитьПроверкуЗаданияПользовательскихПолей(Отказ, ПользовательскиеПоля)
	...
	ИначеЕсли ПользовательскиеПоля.Количество() > МаксимальноеКоличествоПользовательскихПолей() Тогда
		...
		СтрокаСообщения = НСтр("ru='Уменьшите количество полей (можно выбирать не более %1 полей)';uk='Зменшіть кількість полів (можна вибирати не більше %1 полів)'");
...
Процедура УстановитьСвязьИнтерактивно()
	...
	ТекстВопроса = НСтр("ru='Установить соответствие между выбранными объектами?';uk='Встановити відповідність між обраними об''єктами?'");
...
Процедура ЗавершениеФоновогоЗадания(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		ЗафиксироватьОшибку(НСтр("ru='Фоновое задание аварийно отменилось.';uk='Фонове завдання аварійно скасувалося.'"));
		ПерейтиНазад();
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(Отказ)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Сопоставление объектов';uk='Співставлення об''єктів'");
...
Процедура ЗафиксироватьОшибку(ПодробноеПредставлениеОшибки)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Помощник сопоставления объектов.Анализ данных';uk='Помічник співставлення об''єктів.Аналіз даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
