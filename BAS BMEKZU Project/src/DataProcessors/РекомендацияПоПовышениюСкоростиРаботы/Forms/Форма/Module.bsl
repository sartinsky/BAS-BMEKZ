...
Процедура ПриОткрытии(Отказ)
	...
	ТекстСообщения = НСтр("ru='На компьютере установлено %1 Гб оперативной памяти.
        ...
        |;uk='На комп''ютері %1 Гб оперативної пам''яті.
        |Для того щоб програма працювала швидше, 
        |рекомендується збільшити об''єм пам''яті до %2 Гб.'");
	
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ДоступныйОбъем, РекомендуемыйОбъем);
	
	ЗаголовокСообщения = НСтр("ru='Рекомендация по повышению скорости работы';uk='Рекомендація щодо підвищення швидкості роботи'");
	
	ПараметрыВопроса = СтандартныеПодсистемыКлиент.ПараметрыВопросаПользователю();
	ПараметрыВопроса.Заголовок = ЗаголовокСообщения;
	ПараметрыВопроса.Картинка = БиблиотекаКартинок.Предупреждение32;
	ПараметрыВопроса.Вставить("ТекстФлажка", НСтр("ru='Не показывать в течение двух месяцев';uk='Не показувати протягом двох місяців'"));
	
	Кнопки = Новый СписокЗначений;
	Кнопки.Добавить("ПродолжитьРаботу", НСтр("ru='Продолжить работу';uk='Продовжити роботу'"));
