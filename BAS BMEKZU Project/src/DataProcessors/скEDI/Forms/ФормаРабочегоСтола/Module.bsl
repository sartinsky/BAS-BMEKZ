...
Функция ПолучитьНаличиеВидаЭлектронногоДокументаДокументСВложениями(пОрганизация)
	...
	Запрос.УстановитьПараметр("Владелец", пОрганизация);
	ВыборкаРезультатаЗапроса = Запрос.Выполнить().Выбрать();
...
&НаСервереБезКонтекста
Процедура ПолучитьДанныеПоЭлектронномуДокументуДляОтображения(ЭлектронныйДокумент, знач ОсновнойЭлектронныйДокумент, пЗаголовокЗакладкиВариантыДокументов, пМассивСобытийПоЭлектронномуДокументу)
...
//
&НаКлиенте
Процедура ИнициализироватьПодписьИОтправкуЭлектронныхДокументов(ЖурналРабочегоСтола, Отправлять, ФормироватьНовыйВариантДокументов = Ложь, ОтправлятьТолькоСсылку = Ложь, ОтправлятьНаДоподписание = Ложь)
...
Процедура ОбновитьПослеВыполненияДействийСЭлектроннымиДокументамиЖурналПервичныеДокументы(КлючОбработанойЗаписи = Неопределено)
	...
	//ОбновитьДанныеГруппыИнформация(Элементы.СписокДокументов, СобытияПоЭлектронномуДокументу, СписокВариантыДокументов, ЗаголовокЗакладкиВариантыДокументов);
КонецПроцедуры
...
Процедура ОбновитьПослеВыполненияДействийСЭлектроннымиДокументамиЖурналДокументыНДС(КлючОбработанойЗаписи = Неопределено)
	...
	//ОбновитьДанныеГруппыИнформация(Элементы.СписокДокументовДокументыНДС, СобытияПоЭлектронномуДокументуДокументыНДС, СписокВариантыДокументовДокументыНДС, ЗаголовокЗакладкиВариантыДокументовДокументыНДС);
КонецПроцедуры
...
&НаКлиенте
Процедура ОбновитьДанныеГруппыИнформация(пЭлементСписокДокументов, пСобытияПоЭлектронномуДокументу, пСписокВариантыДокументов, пЗаголовокЗакладкиВариантыДокументов)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		ПолучитьДанныеПоЭлектронномуДокументуДляОтображения(ТекущиеДанные.ЭлектронныйДокумент, ТекущиеДанные.ОсновнойЭлектронныйДокумент, пЗаголовокЗакладкиВариантыДокументов_, пМассивСобытийПоЭлектронномуДокументу);
...
Процедура ЭкспортДанныхДокументаДокумента(ПараметрыСохраненияДокументов, ДополнительныеПараметры = Неопределено) Экспорт
	Если ТипЗнч(ПараметрыСохраненияДокументов) = Тип("Структура") Тогда
		...
		Если СохранятьВ_CADES <> Истина И СохранятьВ_P7S <> Истина И СохранятьИзображениеДокументаPDF <> Истина И СохранятьИзображениеДокументаPNG <> Истина Тогда
			Возврат;
		КонецЕсли;
		...
		ПолучитьМассивДанныхДляЭкспортаПоСпискуЭлектронныхДокументовНаСервере(КаталогСохранения, МассивФайлов, МассивАрхивов, МассивЭлектронныхДокументов, ЧтоСохраняем, СохранятьТелоДокумента, СохранятьИзображениеДокумента, СохранятьВложенныеФайлы, СохранятьВ_CADES, СохранятьВ_P7S, СохранятьИзображениеДокументаPDF, СохранятьИзображениеДокументаPNG);
...
&НаСервереБезКонтекста
Процедура ПолучитьМассивДанныхДляЭкспортаПоСпискуЭлектронныхДокументовНаСервере(КаталогСохранения, МассивФайлов, МассивАрхивов, МассивЭлектронныхДокументов, ЧтоСохраняем, СохранятьТелоДокумента, СохранятьИзображениеДокумента, СохранятьВложенныеФайлы, CADES, P7S, ИзображениеДокументаPDF, ИзображениеДокументаPNG)
	...
	Для Каждого ЭлектронныйДокумент Из МассивЭлектронныхДокументов Цикл
		...
		Если СодержаниеЭлектронногоДокумента <> Неопределено Тогда
			Если ЧтоСохраняем = "СписокЭлектронныхДокументов" Тогда
				ТекИмяФайла = Прав(Преф + Формат(ТекНомерДокумента, "ЧГ=0"), ДлинаПрефиксаФайлов) + " " + ЭлектронныйДокумент.ВидЭлектронногоДокумента.Наименование + " №" + СокрЛП(ЭлектронныйДокумент.НомерДокумента) + " від " + Формат(ЭлектронныйДокумент.ДатаДокумента, "ДФ=dd.MM.yyyy") + ".xml";
				...
				Если СохранятьТелоДокумента Тогда
					...
					Если ЗначениеЗаполнено(ТелоДокумента) Тогда
						...
						КонецЕсли;
						Если P7S Тогда
							ДополнитьМассивДанныхДляСохраненияВФорматеP7SНаСервере(КаталогСохранения, МассивФайлов, МассивАрхивов, ТекИмяФайла, ТелоДокумента);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				Если СохранятьИзображениеДокумента Тогда
					...
					Если ЗначениеЗаполнено(ИзображениеДокумента) Тогда
						...
						КонецЕсли;
						Если P7S Тогда
							ДополнитьМассивДанныхДляСохраненияВФорматеP7SНаСервере(КаталогСохранения, МассивФайлов, МассивАрхивов, ИзменитьРасширениеИмениФайла(ТекИмяФайла, ".pdf"), ИзображениеДокумента);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
				...
				Если ИзображениеДокументаPDF или ИзображениеДокументаPNG Тогда
					...
					Если ЗначениеЗаполнено(ТелоДокумента) Тогда
						...
						скEDI_ОбщегоНазначения.ПолучитьСодержимоеПоЭлектронномуДокументу(ЭлектронныйДокумент.Организация, ЭлектронныйДокумент, ТелоДокумента, ИзображениеДокументаPDF, ИзображениеДокументаPNG, пПредставлениеЭлектронногоДокументаPDF, пОтображениеМассивКартинок);
				...
				Если СохранятьВложенныеФайлы Тогда
					...
					Если ЗначениеЗаполнено(СодержаниеЭлектронногоДокумента.ВложениеДокумента1) Тогда
						...
						Если CADES Тогда
							...
							МассивФайлов.Добавить(Новый ОписаниеПередаваемогоФайла(ТекИмяФайлаВложения, СсылкаНаВложениеДокументаНаСервере));
						КонецЕсли;
						Если P7S Тогда
							ДополнитьМассивДанныхДляСохраненияВФорматеP7SНаСервере(КаталогСохранения, МассивФайлов, МассивАрхивов, ТекИмяФайлаВложения, ВложениеДокумента);
						КонецЕсли;
					КонецЕсли;
