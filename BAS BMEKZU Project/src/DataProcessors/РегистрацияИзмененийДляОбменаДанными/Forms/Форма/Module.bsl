...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если Не (ТипЗнч(ИсточникВыбора) = Тип("УправляемаяФорма")
...
Процедура ДобавитьРегистрациюВыделенныхОбъектов(БезУчетаАвторегистрации = Истина)
	...
	Текст = НСтр("ru='Зарегистрировать %1 для выгрузки на узле ""%2""?
                     ...
                     |
                     |Зміна реєстрації великої кількості об''єктів може зайняти тривалий час.'");
...
Процедура ФоновоеЗаданиеЗапуститьНаКлиенте(ПараметрыФоновогоЗадания)
	...
	Если Результат = Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru='При запуске фонового задания с целью изменения регистрации произошла ошибка.';uk='При запуску фонового завдання з метою зміни реєстрації сталася помилка.'");
...
Процедура СтраницаДлительнойОперации()
	...
	Если ВидДлительнойОперации Тогда
		СтатусОперации = НСтр("ru='Выполняется регистрация изменений. Пожалуйста, подождите.';uk='Виконується реєстрація змін. Будь ласка, зачекайте.'");
	Иначе
		СтатусОперации = НСтр("ru='Выполняется отмена регистрации изменений. Пожалуйста, подождите.';uk='Виконується скасування реєстрації змін. Будь ласка, зачекайте.'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	СообщениеОбОшибке = "";
	ТиповоеПредставлениеОшибки = НСтр("ru='При изменении регистрации произошла ошибка. Подробности см. в журнале регистрации';uk='При зміні реєстрації сталася помилка. Подробиці див. у журналі реєстрації'");
...
Функция ОбщийМодульДлительныеОперацииКлиент()
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение НСтр("ru='Общий модуль ""ДлительныеОперацииКлиент"" не найден.';uk='Загальний модуль ""ДлительныеОперацииКлиент"" не знайдено.'");
	КонецЕсли;
...
Функция ОбщийМодульДлительныеОперации()
	...
	Если ТипЗнч(Модуль) <> Тип("ОбщийМодуль") Тогда
		ВызватьИсключение НСтр("ru='Общий модуль ""ДлительныеОперации"" не найден.';uk='Загальний модуль ""ДлительныеОперации"" не знайдено.'");
	КонецЕсли;
...
Функция КонтрольСсылокДляВыбораЗапросом(Адрес)
	...
	Если ТипЗнч(Результат) = Тип("Массив") Тогда 
		...
		Если Результат.Колонки.Найти("Ссылка") = Неопределено Тогда
			Возврат НСтр("ru='В последнем результате запроса отсутствует колонка ""Ссылка""';uk='В останньому результаті запиту відсутня колонка ""Ссылка""'");
		КонецЕсли;
	Иначе		
