...
// Параметры:
	...
	Если ПланОбмена <> Неопределено
		...
		ПрефиксИмениПодписки = ПланОбмена.Имя + "Зарегистрировать";  
		ПрефиксИмениПодпискиИНАГРО = "ИНАГРО_" + ПланОбмена.Имя + "Зарегистрировать";  

		Для Каждого Подписка Из Метаданные.ПодпискиНаСобытия Цикл
			
			Если Не СтрНачинаетсяС(Подписка.Имя, ПрефиксИмениПодписки) И Не СтрНачинаетсяС(Подписка.Имя, ПрефиксИмениПодпискиИНАГРО) Тогда
				Продолжить;
			КонецЕсли;
