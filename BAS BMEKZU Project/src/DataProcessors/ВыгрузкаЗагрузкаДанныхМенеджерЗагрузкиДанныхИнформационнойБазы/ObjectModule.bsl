	...
	Для Каждого ОбъектМетаданных Из ТекущиеЗагружаемыеТипы Цикл
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='ВыгрузкаЗагрузкаДанных.ЗагрузкаОбъектаПропущена';uk='ВыгрузкаЗагрузкаДанных.ЗагрузкаОбъектаПропущена'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				...
				СтрШаблон(НСтр("ru='Загрузка данных объекта метаданных %1 пропущена, т.к. он включен в
                          ...
                          |список об''єктів метаданих, що виключаються з вивантаження і завантаження даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗагрузитьДанныеОбъектаИнформационнойБазы(Знач ОбъектМетаданных)
	...
	ЗаписьЖурналаРегистрации(
		НСтр("ru='ВыгрузкаЗагрузкаДанных.ЗагрузкаОбъектаМетаданных';uk='ВыгрузкаЗагрузкаДанных.ЗагрузкаОбъектаМетаданных'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,
		ОбъектМетаданных,
		,
		СтрШаблон(НСтр("ru='Начало загрузки данных объекта метаданных: %1';uk='Початок завантаження даних об''єкта метаданих: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			ОбъектМетаданных.ПолноеИмя()));
	...
	ЗаписьЖурналаРегистрации(
        ...
        |;uk='Закінчення завантаження даних об''єкта метаданих: %1
        |Завантажено об''єктів: %2'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если Не Отказ Тогда
		...
		Если ОбщегоНазначенияБТС.ЭтоНезависимыйНаборЗаписей(Объект.Метаданные()) Тогда
			...
			Исключение
				...
				Комментарий = СтрШаблон(НСтр("ru='Объекта метаданных %1 с представлением ""%2"" не загружен по причине: %3';uk='Об''єкта метаданих %1 з представленням ""%2"" не завантажений з причини: %3'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				                        Объект.Метаданные().ПолноеИмя(),
				                        Строка(Объект),
				                        ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				
				ЗаписьЖурналаРегистрации(
					НСтр("ru='ВыгрузкаЗагрузкаДанных.ЗагрузкаОбъектаМетаданных.Ошибка';uk='ВыгрузкаЗагрузкаДанных.ЗагрузкаОбъектаМетаданных.Ошибка'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
