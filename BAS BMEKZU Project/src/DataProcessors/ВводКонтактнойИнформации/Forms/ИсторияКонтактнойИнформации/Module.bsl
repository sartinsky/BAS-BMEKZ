...
Процедура ИсторияПередУдалением(Элемент, Отказ)
	...
	Если НЕ Отказ Тогда
		...
		ПоказатьВопрос(Оповещение, НСтр("ru='Удалить адрес действующий с';uk='Вилучити адреса чинна з'") + " " + Формат(ДействуетС, "ДЛФ=DD")+ "?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
	Отказ = Истина;
КонецПроцедуры
...
Процедура СформироватьДанные(ВводНовогоАдреса = Ложь)
	...
	Для Индекс = 0 По История.Количество() - 1 Цикл
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Не допускается ввод адресов с одинаковыми датами.';uk='Не допускається введення адрес з однаковими датами.'"),, "История[" + Строка(Индекс) + "].ДействуетС");
			Возврат;
		КонецЕсли;
	...
	Если НЕ ВводНовогоАдреса И ОтсутствуетНачальнаяДата Тогда
		ПоказатьПредупреждение(, НСтр("ru='Отсутствует адрес, действующий с даты начала ведения учета.';uk='Відсутня адреса, що діє з дати початку ведення обліку.'"));
		Возврат;
	КонецЕсли;
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='Начальное значение';uk='Початкове значення'"));
