...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(АдресФайла) Или Не ЭтоАдресВременногоХранилища(АдресФайла) Тогда
		ТекстИсключения = НСтр("ru='Не выбран файл для загрузки.
            ...
            |Відкрийте обробку з форми списку контрагентів за допомогою команди ""Завантажити"".'");
...
Функция ПодготовитьТабличныйДокументСервер(Знач АдресФайла, Знач РасширениеФайла, Знач ИдентификаторФормы, Знач ОписаниеКолонок)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Подготовка табличного документа к загрузке контрагентов.';uk='Підготовка табличного документа до завантаження контрагентів.'");
...
Функция ПодготовитьТаблицуДанныхДляЗагрузкиНаСервере()
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Подготовка таблицы данных при загрузке контрагентов.';uk='Підготовка таблиці даних при завантаженні контрагентів.'");
...
Процедура ЗагрузкаДанныхИзФайлаЗавершение(ДлительнаяОперация, ДополнительныеПараметры) Экспорт
	...
	Если ДлительнаяОперация.Статус = "Выполнено" Тогда
		...
		Если Результат.Ошибки <> Неопределено
			...
			ПоказатьОповещениеПользователя(НСтр("ru='При загрузке данных произошли ошибки.';uk='При завантаженні даних виникли помилки.'"));
			ОбщегоНазначенияКлиентСервер.СообщитьОшибкиПользователю(Результат.Ошибки)
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ru='Загрузка контрагентов произведена.';uk='Завантаження контрагентів проведено.'"));
			Закрыть();
		КонецЕсли;
	ИначеЕсли ДлительнаяОперация.Статус = "Ошибка" Тогда
		ВызватьИсключение ДлительнаяОперация.КраткоеПредставлениеОшибки;
...
Функция ПолучитьДанныеКонтрагента(Контрагент)
	...
	Возврат ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Контрагент, "КодПоЕДРПОУ, ИНН");
