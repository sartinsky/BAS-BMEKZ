...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ВидимостьРасписания Тогда 
		Элементы.УдалениеПомеченныхИспользование.Заголовок = НСтр("ru='Автоматически удалять помеченные объекты';uk='Автоматично вилучати відмічені об''єкти'");
	КонецЕсли;
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда // Временное решение для работы в мобильном клиенте, будет удалено в следующих версиях
		...
		Элементы.ГруппаПричины.Заголовок = НСтр("ru='Места использования';uk='Місця використання'");
	...
	Если КоличествоМожноУдалить = 0 Тогда
		...
		Если КоличествоВыделено = 1 Тогда
			Если ЕстьЗаписиРегистров Тогда
				ТекстОшибки = НСтр("ru='Удаление записи регистра выполняется из ее карточки.';uk='Вилучення запису регістра виконується з його картки.'");
...
Процедура ЗапуститьФоновоеЗаданиеКлиент(Режим)
	...
	Если ПараметрыМетода.ИскатьПомеченные Тогда
		Текст = НСтр("ru='Поиск помеченных на удаление объектов...';uk='Пошук відмічених для вилучення об''єктів,...'");
	Иначе
		...
		Иначе
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось заблокировать работу в программе по причине: 
                           |%1'
                           |;uk='Не вдалося заблокувати роботу в програмі через: 
                           |%1'"),
...
Функция ЗапуститьФоновоеЗадание(Знач ПараметрыМетода)
	...
	Иначе
		Элементы.НадписьДлительнаяОперация.Заголовок = НСтр("ru='Поиск объектов, помеченных на удаление...';uk='Пошук об''єктів, відмічених для вилучення...'");
		Элементы.НадписьДлительнаяОперация.ГоризонтальноеПоложениеВГруппе = ГоризонтальноеПоложениеЭлемента.Центр;
		Элементы.ФоновоеЗаданиеСостояние.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
	КонецЕсли;
	...
	НастройкиЗапуска.НаименованиеФоновогоЗадания = НСтр("ru='Удаление помеченных объектов (интерактивное)';uk='Вилучення відмічених об''єктів (інтерактивне)'");
...
Процедура ПослеЗавершенияФоновогоЗадания(Задание, ДополнительныеПараметры) Экспорт
	...
	Если Задание.Статус = "Выполнено" Тогда
		...
		Если Результат = Неопределено Тогда
			ВывестиОшибку(НСтр("ru='Не удалось прочитать результат удаления помеченных';uk='Не вдалося прочитати результат вилучення відмічених'"), "");
			Возврат;
		КонецЕсли;
	...
	Если РезультатВыполненияВФоне.УдалятьПомеченные Тогда
		...
		ИначеЕсли НеУдаленныеКоличество = 0 Тогда
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Удалено объектов: %1.';uk='Вилучено об''єктів: %1.'"),
		...
		Иначе
			...
			Результат.ОповещениеТекст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Удалено: %1, не удалено: %2.';uk='Вилучено: %1, не вилучено: %2.'"),
				Формат(УдаленныеКоличество, "ЧН=0; ЧГ="),
...
Процедура ВывестиОшибку(КраткоеПредставлениеОшибки, ПодробноеПредставлениеОшибки);
	Кратко = НСтр("ru='При удалении помеченных возникла ошибка:';uk='При вилученні позначених виникла помилка:'") + Символы.ПС + КраткоеПредставлениеОшибки;
