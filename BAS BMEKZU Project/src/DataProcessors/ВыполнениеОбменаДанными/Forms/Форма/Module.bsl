...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Синхронизация данных с ""%1""';uk='Синхронізація даних з ""%1""'"), НаименованиеКорреспондента);
...
Процедура ПроверитьПодключение()
	...
	Если Не ЕстьПодключение Тогда
		СообщениеОбОшибке = НСтр("ru='Не удалось подключиться к приложению в Интернете, по причине ""%1"".
            ...
            |;uk='Не вдалося підключитися до програми в Інтернеті, через ""%1"".
            |Переконайтеся, що:
            |- введено правильний пароль;
            |- зазначено коректну адресу для підключення;
            |- додаток доступний за зазначеною у настройках адресою;
            |- настройка синхронізації не була видалена в програмі в Інтернеті.
            |Повторіть спробу синхронізації.'");
	...
	ИначеЕсли Не НастройкаЗавершена Тогда
		СообщениеОбОшибкеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для продолжения необходимо перейти в программу ""%1"" и завершить в ней настройку синхронизации. Выполнение обмена данными отменено.';uk='Для продовження необхідно перейти в програму ""%1"" і завершити в ній настройку синхронізації. Виконання обміну даними скасовано.'"),
			НаименованиеКорреспондента);
		СинхронизацияДанныхОтключена = Истина;
	ИначеЕсли ПолученыДанныеДляСопоставления Тогда
		СообщениеОбОшибкеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для продолжения необходимо перейти в программу ""%1"" и выполнить загрузку сообщения для сопоставления данных. Выполнение обмена данными отменено.';uk='Для продовження необхідно перейти в програму ""%1"" і виконати завантаження повідомлення для співставлення даних. Виконання обміну даними скасовано.'"),
			НаименованиеКорреспондента);
		СинхронизацияДанныхОтключена = Истина;
	КонецЕсли;
	...
	ИначеЕсли ЕстьОшибки Тогда
		...
		Иначе
			...
			Если ВыводитьОписаниеОшибкиПользователю Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СообщениеОбОшибкеПользователю);
			КонецЕсли;
...
Функция ФоновоеЗаданиеЗапуститьНаСервере(ПараметрыЗадания, ОшибкаРасхожденияВерсийПриПолученииДанных, ПроверятьРасхождениеВерсий)
	...
	Если ПараметрыЗадания.ВыполнятьЗагрузку Тогда
		...
		ШаблонНаименования = НСтр("ru='Выполняется загрузка данных из %1';uk='Виконується завантаження даних з %1'");
		
	Иначе
		ШаблонНаименования = НСтр("ru='Выполняется выгрузка данных в %1';uk='Виконується вивантаження даних у %1'");
	КонецЕсли;
...
Процедура ФоновоеЗаданиеПолучитьРезультатНаСервере()
	...
	ТиповоеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось выполнить %1. Подробности см. в журнале регистрации';uk='Не вдалося виконати %1. Подробиці див. у журналі реєстрації'"),
		?(ФоновоеЗаданиеВыполняемоеДействие = 1, НСтр("ru='получение данных';uk='отримання даних'"), НСтр("ru='отправку данных';uk='відправлення даних'")));
