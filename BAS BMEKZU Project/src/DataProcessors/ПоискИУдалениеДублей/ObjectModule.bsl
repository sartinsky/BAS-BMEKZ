		...
		Если СтруктураПолейПодобия.Количество() > 0 Тогда
			...
			Если FuzzySearch = Неопределено Тогда
				Результат.ОписаниеОшибки = 
					НСтр("ru='Не удалось подключить внешнюю компоненту FuzzyStringMatchExtension
                               |из макета ""ОбщийМакет.КомпонентаПоискаСтрок""
                               |Подробнее см. в журнале регистрации.'
                               |;uk='Не вдалося підключити зовнішню компоненту FuzzyStringMatchExtension
                               |з макета ""ОбщийМакет.КомпонентаПоискаСтрок""
