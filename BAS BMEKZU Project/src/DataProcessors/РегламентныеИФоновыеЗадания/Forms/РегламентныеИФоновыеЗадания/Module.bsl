...
Процедура ТаблицаРегламентныеЗаданияПередУдалением(Элемент, Отказ)
	...
	ИначеЕсли Элемент.ТекущиеДанные.Предопределенное Тогда
		ПоказатьПредупреждение(, НСтр("ru='Невозможно удалить предопределенное регламентное задание.';uk='Неможливо вилучити напередвизначене регламентне завдання.'") );
	Иначе
		ПоказатьВопрос(
...
Процедура ВыполнитьРегламентноеЗаданиеВручную(Команда)
		...
		ИначеЕсли ПараметрыВыполнения.ПроцедураУжеВыполняется Тогда
			МассивСообщенийОбОшибках.Добавить(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |;uk='Процедура регламентного завдання ""%1""
                    |  уже виконується у фоновому завданні ""%2"", початому %3.'"),
					ТекущиеДанные.Наименование,
	...
	Если КоличествоОшибок > 0 Тогда
		ЗаголовокТекстаПроОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Задания выполнены с ошибками (%1 из %2)';uk='Завдання виконано з помилками (%1 из %2)'"),
...
Функция ЗагрузкаРегламентныхЗаданий(ИдентификаторЗадания)
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Формирование таблицы регламентных заданий';uk='Формування таблиці регламентних завдань'");
