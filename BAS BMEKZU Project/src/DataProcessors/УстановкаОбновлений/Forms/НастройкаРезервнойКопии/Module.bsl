...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		...
		ВызватьИсключение НСтр("ru='Создание резервной копии поддерживается только в операционной системе Windows.
                                    |В текущей операционной системе резервная копия делается самостоятельно.'
                                    |;uk='Створення резервної копії підтримується тільки в операційній системі Windows.
                                    |У поточній операційній системі резервна копія робиться самостійно.'");
