...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ОбновлениеКонфигурации.ЕстьПраваНаУстановкуОбновления() Тогда
		ВызватьИсключение НСтр("ru='Недостаточно прав для обновления конфигурации. Обратитесь к администратору.';uk='Недостатньо прав для оновлення конфігурації. Зверніться до адміністратора.'");
	ИначеЕсли ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		ВызватьИсключение НСтр("ru='Данная операция не доступна внешнему пользователю системы.';uk='Дана операція не доступна зовнішньому користувачеві системи.'");
	КонецЕсли;
	...
	Если ЭтоБазоваяВерсия Тогда
		ЗаголовокВариантаВыбора = "&" + НСтр("ru='Укажите файл обновления';uk='Зазначте файл оновлення'");
...
Процедура ПолеФайлОбновленияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Иначе
		...
		Диалог.Фильтр = НСтр("ru='Все файлы (*.cf*;*.cfu;*.cfe;*.zip)|*.cf*;*.cfu;*.cfe;*.zip|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации (*.cfu)|*.cfu|Файлы исправлений (*.cfe*;*.zip)|*.cfe*;*.zip';uk='Усі файли (*.cf*;*.cfu;*.cfe;*.zip)|*.cf*;*.cfu;*.cfe;*.zip|Файли поставки конфігурації (*.cf)|*.cf|Файли поставки оновлення конфігурації (*.cfu)|*.cfu|Файли виправлень (*.cfe*;*.zip)|*.cfe*;*.zip'");
	КонецЕсли;
...
Процедура НадписьОшибкаУстановкиИсправленийОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ОтборЖурнала.Вставить("СобытиеЖурналаРегистрации", НСтр("ru='Исправления.Установка';uk='Виправлення.Встановлення'"));
	...
	Для Каждого ПомещенныйФайл Из ПомещенныеФайлы Цикл
		...
		Попытка
			...
			Если Не ОбновлениеКонфигурации.ЭтоИсправление(УстановленноеРасширение) Тогда
				Расширение.Удалить();
				ВызватьИсключение НСтр("ru='Расширение не является патчем.';uk='Розширення не є патчем.'");
			КонецЕсли;
		...
		Исключение
			...
			ТекстОшибки = НСтр("ru='При установке исправления %1 возникла ошибка:
                |%2'
                |;uk='При встановленні виправлення %1 виникла помилка:
                |%2'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
				ИмяФайлаИсправления,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			ЗаписьЖурналаРегистрации(НСтр("ru='Исправления.Установка';uk='Виправлення.Встановлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ЕстьОшибки Тогда
		...
		Если УстановленоИсправлений > 0 Тогда
			ТекстНадписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Установлено исправлений: %1 из %2, изменения вступят в силу после перезапуска программы';uk='Встановлено виправлень: %1 з %2, зміни набудуть чинності після перезапуску програми'"),
...
Процедура ПослеПодтвержденияБезопасности(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = "Продолжить" Тогда
		...
		Для Каждого ИмяФайла Из ИменаВыбранныхФайлов Цикл
			Если СтрЗаканчиваетсяНа(ИмяФайла, ".cfe") Тогда
			...
			Иначе // Это файл обновления.
				...
				Иначе
					ВызватьИсключение НСтр("ru='Допустимо выбирать только один файл обновления.';uk='Припустимо вибирати тільки один файл оновлення.'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
