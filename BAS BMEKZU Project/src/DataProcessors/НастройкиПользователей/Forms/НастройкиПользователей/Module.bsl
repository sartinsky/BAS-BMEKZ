...
Процедура СкопироватьВсеНастройки(Команда)
	...
	КопируемыеНастройки.Добавить("ПерсональныеНастройкиЭлеватор", НСтр("ru='Персональные настройки (элеватор)';uk='Персональні настройки (елеватор)'")); // ИНАГРО
...
Процедура ОчиститьВсеНастройки(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Очистить все настройки у пользователя ""%1""?';uk='Очистити всі настройки у користувача ""%1""?'"), Строка(ПользовательСсылка));
...
Процедура ОчиститьНастройкиОтчетовИВнешнегоВида(Команда)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?';uk='Очистити всі настройки звітів і зовнішнього вигляду у користувача ""%1""?'"),
...
Функция ОбновлениеСпискаНастроек()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Обновление настроек пользователей';uk='Оновлення настройок користувачів'");
...
Процедура РассчитатьКоличествоНастроек()
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.НастройкиОтчетовСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Настройки отчетов (%1)';uk='Настройки звітів (%1)'"), КоличествоНастроек);
	Иначе
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ВнешнийВидСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Внешний вид (%1)';uk=' Зовнішній вигляд (%1)'"), КоличествоНастроек);
	Иначе
	...
	Если КоличествоНастроек <> 0 Тогда
		Элементы.ПрочиеНастройкиСтраница.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Прочие настройки (%1)';uk='Інші настройки (%1)'"), КоличествоНастроек);
	Иначе
...
Процедура СкопироватьНаСервере(ПользователиПриемник, ЛичныхНастроекОтчетов, Отчет)
		...
		Для Каждого Настройка Из Результат.МассивНастроек Цикл
			...
			Для Каждого Элемент Из Настройка Цикл
				...
				Если Элемент.Пометка Тогда
					...
					КлючОтчета = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Элемент.Значение, "/");
	...
	Иначе
		...
		Если Результат.МассивПерсональныхНастроекЭлеватор.Количество() <> 0 Тогда // ИНАГРО
			Обработки.НастройкиПользователей.СкопироватьНастройкиОтчетовИПерсональныеНастройки(ХранилищеОбщихНастроек,
				ПользовательИнформационнойБазы, ПользователиПриемник, Результат.МассивПерсональныхНастроекЭлеватор);
		КонецЕсли;
...
Процедура ОчиститьНаСервере(Пользователи = Неопределено, ВыбраннымПользователям = Ложь)
	...
	Если ВыбраннымПользователям Тогда
		...
		Если Результат.МассивПерсональныхНастроекЭлеватор.Количество() <> 0 Тогда // ИНАГРО
			Обработки.НастройкиПользователей.УдалитьНастройкиВыбраннымПользователям(Пользователи,
				Результат.МассивПерсональныхНастроекЭлеватор, "ХранилищеОбщихНастроек");
		КонецЕсли;
	...
	// ИНАГРО++
	Если Результат.МассивПерсональныхНастроекЭлеватор.Количество() <> 0 Тогда 
		Обработки.НастройкиПользователей.УдалитьВыбранныеНастройки(
			ПользовательИнформационнойБазы, Результат.МассивПерсональныхНастроекЭлеватор, "ХранилищеОбщихНастроек");
	КонецЕсли; 

	Если Результат.МассивПерсональныхНастроекЭлеватор.Количество() <> 0 Тогда
		
		Для Каждого ПрочиеПользовательскиеНастройки Из Результат.МассивПерсональныхНастроекЭлеватор Цикл
			ПользователиСлужебный.ПриУдаленииПрочихНастроекПользователя(
				СведенияОПользователе, ПрочиеПользовательскиеНастройки);
		КонецЦикла;
	КонецЕсли;
...
Процедура Подключаемый_ВыполнитьОбработкуОповещения()
	...
	ТекстОповещения     = НСтр("ru='Копирование настроек';uk='Копіювання настройок'");
	...
	Если Параметр.КопироватьВсе Тогда
		...
		Если МассивНастроек.Количество() = 7 Тогда
			ПояснениеОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Скопированы все настройки %1';uk='Скопійовано всі настройки %1'"), ПояснениеКомуСкопированыНастройки);
		Иначе
			ПояснениеОповещения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='%1 скопированы %2';uk='%1 скопійовані %2'"), ИменаНастроек, ПояснениеКомуСкопированыНастройки);
		КонецЕсли;
...
Процедура ОповеститьОбУдалении(КоличествоНастроек, НазваниеНастройки = Неопределено, КоличествоПользователей = Неопределено)
	...
	ПрописьПредмета = Формат(КоличествоНастроек, "ЧДЦ=0") + " "
		+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоНастроек,
			"", НСтр("ru='настройка,настройки,настроек,,,,,,0';uk='настройка,настройки,настройок,,,,,,0'"));
	
	Если КоличествоНастроек = 1
	   И КоличествоПользователей = Неопределено Тогда
		
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='""%1"" очищена пользователю ""%2""';uk='""%1"" очищена користувачеві ""%2""'"), НазваниеНастройки, Строка(ПользовательСсылка));
		
	ИначеЕсли КоличествоПользователей = Неопределено Тогда
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Очищено %1 пользователю ""%2""';uk='Очищено %1 користувачеві ""%2""'"), ПрописьПредмета, Строка(ПользовательСсылка));
	КонецЕсли;
	...
	Если КоличествоПользователей <> Неопределено Тогда
		...
		Если КоличествоНастроек = 1 Тогда
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='""%1"" очищена %2';uk='""%1"" очищена %2'"), НазваниеНастройки, ПояснениеКомуОчищатьНастройки);
		Иначе
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Очищено %1 %2';uk='Очищено %1 %2'"), ПрописьПредмета, ПояснениеКомуОчищатьНастройки);
		КонецЕсли;
...
Процедура СкопироватьНастройки()
	...
	Если ДеревоНастроек.ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(,
			НСтр("ru='Необходимо выбрать настройки, которые требуется скопировать.';uk='Необхідно вибрати настройки, які потрібно скопіювати.'"));
		Возврат;
...
Функция ВыделенныеНастройки()
	...
	МассивПерсональныхНастроекЭлеватор = Новый Массив;
	...
	Для Каждого ВыделенныйЭлемент Из ВыделенныеЭлементы Цикл
		...
		Если ВыделеннаяНастройка.Тип = "ПерсональныеНастройкиЭлеватор" Тогда // ИНАГРО
			МассивПерсональныхНастроекЭлеватор.Добавить(ВыделеннаяНастройка.Ключи);
			Продолжить;
		КонецЕсли;
	...
	Возврат Новый Структура("МассивНастроек, МассивПерсональныхНастроек, МассивПерсональныхНастроекЭлеватор, МассивВариантовОтчетов, МассивПользовательскихНастроек",
			МассивНастроек, МассивПерсональныхНастроек, МассивПерсональныхНастроекЭлеватор, МассивВариантовОтчетов, МассивПользовательскихНастроек);
КонецФункции
