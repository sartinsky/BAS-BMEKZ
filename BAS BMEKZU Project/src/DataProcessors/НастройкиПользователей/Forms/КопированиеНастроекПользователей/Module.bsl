...
Процедура ПользовательСсылкаПриИзменении(Элемент)
	...
	Если ПользовательСсылка <> Неопределено
		...
		ПоказатьПредупреждение(,НСтр("ru='У выбранного пользователя нет настроек, которые можно было бы
                ...
                |;uk='У обраного користувача немає настройок, які можна було б
                |скопіювати, виберіть іншого користувача.'"));
	...
	Для Каждого ОткрытоеОкно Из ОткрытыеОкна Цикл
		...
		Если СтрНайти(СтрокаВсеКопируемыеНастройки, ИмяОткрытойФормы) > 0 Тогда
			СтрокаОткрытыеФормы = ?(ЗначениеЗаполнено(СтрокаОткрытыеФормы),
				...
				НСтр("ru='Открытые окна';uk='Відкриті вікна'") + ":" + Символы.ПС + "- " + ОткрытоеОкно.Заголовок)
		КонецЕсли;
...
Процедура ВыбратьНастройкиПослеВыбора(Параметр, Контекст) Экспорт
	...
	Если Параметр.Свойство("ПерсональныеНастройкиЭлеватор") Тогда 	
		ВыбранныеНастройки.Вставить("ПерсональныеНастройкиЭлеватор", Параметр.ПерсональныеНастройкиЭлеватор); // ИНАГРО	
	КонецЕсли;
	...
	Иначе
		...
		Если Не НастройкиСкопированы Тогда
			...
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Настройки не были скопированы, так как у пользователя ""%1"" не было сохранено ни одной настройки.';uk='Настройки не були скопійовані, тому що в користувача ""%1"" не було збережено жодної настройки.'"),
...
Процедура СкопироватьВыбранныеНастройки(Отчет)
	...
	Если ВыбранныеНастройки.Свойство("ПерсональныеНастройкиЭлеватор") И 	
		 ВыбранныеНастройки.ПерсональныеНастройкиЭлеватор.Количество() > 0 Тогда // ИНАГРО
		Обработки.НастройкиПользователей.СкопироватьНастройкиОтчетовИПерсональныеНастройки(ХранилищеОбщихНастроек,
		Пользователь, Приемники, ВыбранныеНастройки.ПерсональныеНастройкиЭлеватор);
	КонецЕсли;
