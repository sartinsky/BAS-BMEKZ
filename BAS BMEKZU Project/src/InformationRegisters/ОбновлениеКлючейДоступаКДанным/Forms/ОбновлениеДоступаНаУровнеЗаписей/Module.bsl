...
Процедура ОтменитьОбновлениеПрогресса(Команда)
	...
	Если ЗначениеЗаполнено(ИдентификаторЗаданияОбновленияПрогресса) Тогда
		...
		Если АвтообновлениеПрогресса Тогда
			...
			Пояснение = НСтр("ru='Автообновление прогресса отключено';uk='Автооновлення прогресу вимкнено'");
		Иначе
			Пояснение = "";
		КонецЕсли;
		ПоказатьОповещениеПользователя(НСтр("ru='Обновление прогресса отменено';uk='Оновлення прогресу скасовано'"),,
			Пояснение);
	КонецЕсли;
...
Процедура ОбновитьСостояниеЗаданияОбновленияДоступа(Состояние = Неопределено)
	...
	Если ЗначениеЗаполнено(Состояние.ПоследнееЗавершениеОбновленияДоступа) Тогда
		...
		Если ЗначениеЗаполнено(ТекстОшибкиОбновленияДоступа) Тогда
			Если Состояние.ПоследнееЗавершениеСегодня Тогда
				Шаблон = НСтр("ru='<1>завершено</1> <2>с ошибкой</2> в %1 за %2';uk='<1>завершено</1> <2>з помилкою</2> у %1 за %2'");
			Иначе
				Шаблон = НСтр("ru='<1>завершено</1> <2>с ошибкой</2> %1 за %2';uk='<1>завершено</1> <2>з помилкою</2> %1 %2'");
			КонецЕсли;
		...
		Иначе
			Если Состояние.ПоследнееЗавершениеСегодня Тогда
				Шаблон = НСтр("ru='завершено в %1 за %2';uk='завершено %1 %2'")
			Иначе
				Шаблон = НСтр("ru='завершено %1 за %2';uk='завершено %1 %2'")
			КонецЕсли;
		КонецЕсли;
		Если Состояние.ПоследнееЗавершениеСегодня Тогда
	...
	Иначе
		ПоследнееЗавершение = "(" + ?(Состояние.ОбновлениеДоступаВыполняется,
			НСтр("ru='не завершалось';uk='не завершувалося'"), НСтр("ru='не запускалось';uk='не запускалося'")) + ")";
	КонецЕсли;
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполняется %1';uk='Виконується %1'"),
...
Функция ВремяВыполненияСтрокой(ВремяВСекундах)
	...
	Если ВсегоЧасов > 0 Тогда
		Шаблон = НСтр("ru='%3 ч %2 мин %1 сек';uk='%3 ч %2 хв %1 сек'");
		
	ИначеЕсли Минут > 0 Тогда
		Шаблон = НСтр("ru='%2 мин %1 сек';uk='%2 хв %1 сек'");
	Иначе
		Шаблон = НСтр("ru='%1 сек';uk=' %1 сек'");
	КонецЕсли;
...
Процедура ОбновитьПрогрессПослеПолученияДанных(Контекст)
	...
	Элементы.РегламентноеЗаданиеОтключено.Видимость =
		Не Элементы.ОбновлениеДоступаЗапрещено.Видимость
		И Контекст.Свойство("РегламентноеЗаданиеОтключено");
...
Функция НачатьОбновлениеПрогрессаНаСервере(Контекст, АдресРезультата, АдресХранимыхДанных,
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания =
		НСтр("ru='Управление доступом: Получение прогресса обновления доступа';uk='Управління доступом: Отримання прогресу оновлення доступу'");
	...
	Если ТекущаяКолонка = Неопределено
	 Или Не СтрНачинаетсяС(ТекущаяКолонка.Имя, "Списки") Тогда
		...
		ПоказатьПредупреждение(,
			НСтр("ru='Выберите колонку для сортировки';uk='Виберіть колонку для сортування'"));
		Возврат;
	КонецЕсли;
