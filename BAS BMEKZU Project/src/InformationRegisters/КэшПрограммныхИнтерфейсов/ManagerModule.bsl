		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось получить WS-определения по адресу 
                           ...
                           |%3'
                           |;uk='Не вдалося отримати WS-визначення за адресою 
                           |%1
                           |з причини:
                           |%2
                           |
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить WS-определения из кэша 
                       |по причине:
                       |%1'
                       |;uk='Не вдалося отримати WS-визначення з кешу 
                       |з причини:
                       |%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение ТекстОшибки;
...
Функция ПолучитьWSDL(Знач Адрес, Знач ИмяПользователя, Знач Пароль, Знач Таймаут, Знач ЗащищенноеСоединение = Неопределено)
	...
	Иначе
		ВызватьИсключение 
			НСтр("ru='Подсистема ""Получение файлов из интернета"" не доступна.';uk='Підсистема ""Отримання файлів з інтернету"" не доступна.'");
	КонецЕсли;
	...
	Если Не ОписаниеФайла.Статус Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить файл описания web-сервиса
                       |по причине:
                       |%1'
                       |;uk='Не вдалося отримати файл опису web-сервісу
                       |з причини:
                       |%1'"),
	...
	Исключение
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить файл описания web-сервиса
                       ...
                       |;uk='Не вдалося отримати файл опису web-сервісу
                       |з причини:
                       |Не вдалося отримати WS-визначення для збереження в кеші:
                       |%1'"),
			КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Если Определения.Сервисы.Количество() = 0 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить файл описания web-сервиса
                       ...
                       |;uk='Не вдалося отримати файл опису web-сервісу
                       |з причини:
                       |В отриманому файлі не міститься жодного опису сервісу.
                       |
                       |Можливо, адреса файлу опису вказано невірно:
                       |%1'"),
			Адрес);
	КонецЕсли;
