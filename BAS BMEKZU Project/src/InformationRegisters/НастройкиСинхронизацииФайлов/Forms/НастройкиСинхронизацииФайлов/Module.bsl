...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда // Временное решение для работы в мобильном клиенте, будет удалено в следующих версиях
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФормаДобавитьСинхронизациюФайлов", "Отображение", ОтображениеКнопки.Картинка);
...
Процедура ДеревоОбъектовМетаданныхПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru='Удаление настройки приведет к прекращению синхронизации 
        ...
        |;uk='Вилучення настройки призведе до припинення синхронізації 
        |за заданими в ній правилами. Продовжити?'");
		
	ОписаниеОповещения = Новый ОписаниеОповещения("УдалитьНастройкуЗавершение", ЭтотОбъект);
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Нет, НСтр("ru='Предупреждение';uk='Попередження'"));
		...
		Если Не НастройкаИмеетГлобальныеПравила Тогда
			...
			Если НайденныеНастройки.Количество() > 0 Тогда
				...
				Если ПравилоОтбора <> Неопределено И ПравилоОтбора.Отбор.Элементы.Количество() > 0 Тогда
					НоваяСтрокаТаблицы.ПравилоОтбора = ?(ПустаяСтрока(НайденныеНастройки[0].Наименование), НСтр("ru='Выбранные файлы';uk='Обрані файли'"), НайденныеНастройки[0].Наименование);
				Иначе
...
Процедура УстановитьНастройкиОтбора(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	Если ВыбранноеЗначение.ЕстьПравилоОтбора Тогда
		ОбновляемаяСтрока.ПравилоОтбора =
			?( ЗначениеЗаполнено(ВыбранноеЗначение.Наименование), ВыбранноеЗначение.Наименование, НСтр("ru='Выбранные файлы';uk='Обрані файли'"));
	Иначе
		ОбновляемаяСтрока.ПравилоОтбора = НСтр("ru='Все файлы';uk='Всі файли'");
	КонецЕсли;
...
Функция ОчиститьДанныеНастройки()
	...
	ПараметрыВыполненияВФоне.НаименованиеФоновогоЗадания = НСтр("ru='Подсистема Работа с файлам: Отключение синхронизации файлов с облачным сервисом';uk='Підсистема Робота з файлами: Відключення синхронізації файлів з хмарним сервісом'");
...
Процедура ЗапуститьРегламентноеЗадание()
	...
	Иначе
		...
		ПараметрыЗадания.НаименованиеФоновогоЗадания = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Запуск вручную: %1';uk='Запуск вручну: %1'"), РегламентноеЗаданиеМетаданные.Синоним);
