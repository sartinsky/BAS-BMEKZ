...
Функция ПолучитьСсылку(Наименование, ОперацияСсылка) Экспорт
	Если Наименование <> Неопределено Тогда 
		...
		КонецЕсли;
	Иначе
		Ссылка = Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000");
	КонецЕсли;
...
Функция ПолучитьСсылкуМногоКомментариев(СсылкаОперация)
	...
	Попытка
		ХешированиеДанных = Новый ХешированиеДанных(ХешФункция.SHA1);
		СлишкомМногоКомментариев = НСтр("ru='Слишком много комментариев';uk='Занадто багато коментарів'");
