...
Функция ПравилаКонвертацииСовместимыСТекущейВерсией(ИмяПланаОбмена, ОписаниеОшибки, ДанныеОПравилах) Экспорт
	...
	Если ИмяКонфигурацииИзПравил <> ИмяКонфигурацииИнформационнойБазы Тогда
		...
		ОписаниеОшибки.Вставить("ТекстОшибки",
            ...
            |;uk='Правила не можуть бути завантажені, так як вони призначені для програми ""%1"".
            |Слід використовувати правила з конфігурації або завантажити коректний комплект правил з файлу.'"));
	...
	Если РезультатСравнения <> 0 Тогда
		...
		Если РезультатСравнения < 0 Тогда
            ...
            |;uk='Синхронізація даних може працювати некоректно, так як завантажувані правила призначені для попередньої версії програми ""%1"" (%2).
            |Рекомендується використовувати правила з конфігурації або завантажити комплект правил, призначений для поточної версії програми (%3).'");
		...
		Иначе
            ...
            |;uk='Синхронізація даних може працювати некоректно, так як завантажувані правила призначені для більш нової версії програми ""%1"" (%2).
            |Рекомендується оновити версію програми або використовувати комплект правил, призначений для поточної версії програми (%3).'");
...
Процедура ЗагрузитьПравила(Отказ, Запись, АдресВременногоХранилища = "", ИмяФайлаПравил = "", ЭтоАрхив = Ложь) Экспорт
	...
	Если СписокПлановОбмена.Найти(Запись.ИмяПланаОбмена) = Неопределено Тогда
		НСтрока = НСтр("ru='План обмена %1 не используется для синхронизации данных, правила не обновлены.';uk='План обміну %1 не використовується для синхронізації даних, правила не оновлені.'");
