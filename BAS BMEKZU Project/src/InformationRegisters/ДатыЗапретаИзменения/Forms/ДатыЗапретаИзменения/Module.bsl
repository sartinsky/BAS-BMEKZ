...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru='Даты запрета загрузки данных прошлых периодов из других программ отключены в настройках.';uk='Дати заборони завантаження даних минулих періодів з інших програм відключені в настройках.'");
		...
		Элементы.УстановкаДатыЗапрета.СписокВыбора.НайтиПоЗначению("НетЗапрета").Представление =
			НСтр("ru='Не установлено';uk='Не встановлено'");
	...
	Иначе
		...
		Элементы.ИспользованиеДатЗапретаВыключеноНадпись.Заголовок =
			НСтр("ru='Даты запрета ввода и редактирования данных прошлых периодов отключены в настройках программы.';uk='Дати заборони введення і редагування даних минулих періодів відключені в настройках програми.'");
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ТекстВопроса = НСтр("ru='Заданные настройки дат запрета будут автоматически скорректированы.';uk='Задані настройки дат заборони будуть автоматично скориговані.'") 
		+ Символы.ПС + Символы.ПС + ТекстВопроса + Символы.ПС + Символы.ПС + НСтр("ru='Закрыть?';uk='Закрити?'");
...
Процедура УстановкаДатыЗапретаПриИзменении(Элемент)
	...
	Если ТекущаяУстановкаДатыЗапрета = "ПоПользователям"
	   И ВыбранноеЗначение = "ДляВсехПользователей" Тогда
		...
		ТекстВопроса = НСтр("ru='Отключить все даты запрета, кроме установленных для всех пользователей?';uk='Відключити всі дати заборони, крім встановлених для всіх користувачів?'");
	...
	ИначеЕсли (ТекущаяУстановкаДатыЗапрета = "ПоПользователям" Или ТекущаяУстановкаДатыЗапрета = "ДляВсехПользователей")
		...
		ТекстВопроса = НСтр("ru='Отключить все установленные даты запрета?';uk='Відключити всі встановлені дати заборони?'")
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Представление)
	   И Не ТекущиеДанные.БезДатыЗапрета Тогда
		...
		Если ДополнительныеПараметры.ДатыЗапретаДляВсехПользователей Тогда
			ТекстВопроса = НСтр("ru='Отключить даты запрета для всех пользователей?';uk='Відключити дати заборони для всіх користувачів?'");
		Иначе
			Если ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить даты запрета для ""%1""?';uk='Відключити дати заборони для ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить даты запрета для группы пользователей ""%1""?';uk='Відключити дати заборони для групи користувачів ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить даты запрета для внешнего пользователя ""%1""?';uk='Відключити дати заборони для зовнішнього користувача ""%1""?'"), ТекущиеДанные.Пользователь);
				
			ИначеЕсли ТипЗнч(ТекущиеДанные.Пользователь) = Тип("СправочникСсылка.ГруппыВнешнихПользователей") Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить даты запрета для группы внешних пользователей ""%1""?';uk='Відключити дати заборони для групи зовнішніх користувачів ""%1""?'"), ТекущиеДанные.Пользователь);
			Иначе
				ТекстВопроса = НСтр("ru='Отключить даты запрета?';uk='Відключити дати заборони?'")
			КонецЕсли;
		КонецЕсли;
	...
	Если ТекущиеДанные.ЭтоРаздел Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Раздел) Тогда
			Если ТекущиеДанные.ПолучитьЭлементы().Количество() > 0 Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить все настроенные даты запрета для раздела ""%1"" и его объектов?';uk='Відключити всі настроєні дати заборони для розділу ""%1"" та його об''єктів?'"), ТекущиеДанные.Раздел);
			Иначе
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отключить дату запрета для раздела ""%1""?';uk='Відключити дату заборони для розділу ""%1""?'"), ТекущиеДанные.Раздел);
			КонецЕсли;
		Иначе
			ТекстВопроса = НСтр("ru='Отключить общую дату запрета для всех разделов?';uk='Відключити загальну дату заборони для всіх розділів?'");
		КонецЕсли;
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Отключить дату запрета для объекта ""%1""?';uk='Відключити дату заборони для об''єкта ""%1""?'"), ТекущиеДанные.Объект);
	КонецЕсли;
		...
		Если ДатаЗапретаУстановлена(ТекущиеДанные, ТекущийПользователь)
		 Или ЭлементыРаздела.Количество() > 0 Тогда
			...
			ПоказатьВопрос(
				...
				ТекстВопроса, РежимДиалогаВопрос.ДаНет);
		Иначе
			ТекстСообщения = НСтр("ru='Для отключения даты запрета по конкретному объекту необходимо выбрать интересующий объект в одном из разделов.';uk='Для відключення дати заборони по конкретному об''єкту необхідно вибрати потрібний об''єкт в одному з розділів.'");
...
Процедура ДатыЗапретаПолноеПредставлениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Не ЗначениеНайдено Тогда
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='""%1"" уже есть в списке объектов';uk='""%1"" вже є в списку об''єктів'"), ВыбранноеЗначение));
		Возврат;
	КонецЕсли;
		...
		Если Не ЗаменитьОбъектВЗаписиПользователяНаСервере(
			...
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='""%1"" уже есть в списке объектов.
                    ...
                    |;uk='""%1"" вже є у списку об''єктів.
                    |Оновіть дані форми (клавіша F5).'"), ВыбранноеЗначение));
			Возврат;
		Иначе
			ОбновитьЗначенияПрочитанныхСвойств(
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Формат",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ДП='%1'", НСтр("ru='Без запрета';uk='Без заборони'")));
...
Процедура СпособУказанияДатыЗапретаОбработкаВыбораОбработчикОжидания()
	...
	Если ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ОбщаяДата" Тогда
		ТекстВопроса = НСтр("ru='Отключить даты запрета, установленные для разделов и объектов?';uk='Відключити дати заборони, встановлені для розділів і об''єктів?'");
	...
	ИначеЕсли ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ПоРазделам"
		...
		ТекстВопроса = НСтр("ru='Отключить даты запрета, установленные для объектов?';uk='Відключити дати заборони, встановлені для об''єктів?'");
	...
	ИначеЕсли ТекущийСпособ = "ПоРазделамИОбъектам" И ВыбранноеЗначение = "ПоОбъектам"
		...
		ТекстВопроса = НСтр("ru='Отключить даты запрета, установленные для разделов?';uk='Відключити дати заборони, встановлені для розділів?'");
...
Процедура ПользователиПолноеПредставлениеОбработкаВыбораОбработчикОжидания()
		...
		Если Не ЗаменитьПользователяНабораЗаписей(ТекущийПользователь, ВыбранноеЗначение, АдресУстановленныхБлокировок) Тогда
			ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='""%1"" уже есть в списке пользователей.
                    |Обновите данные формы (клавиша F5).'
                    |;uk='""%1"" вже є в списку користувачів.
                    |Оновіть дані форми (клавіша F5).'"), ВыбранноеЗначение));
			Возврат;
		КонецЕсли;
	...
	Иначе
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='""%1"" уже есть в списке пользователей.';uk='""%1"" вже є в списку користувачів.'"), ВыбранноеЗначение));
	КонецЕсли;
...
Процедура ПрочитатьДанныеПользователя(Форма, ТекущийСпособУказания = Неопределено, Данные = Неопределено)
		...
		Если НайденныеСтроки.Количество() > 0 Тогда
			Форма.Элементы.ТекущийПользовательПредставление.Заголовок =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Настройка для ""%1"":';uk='Настройка для ""%1"":'"), НайденныеСтроки[0].Представление);
		КонецЕсли;
	КонецЕсли;
	...
	Если Форма.ПоказыватьРазделыТекущегоПользователя Тогда
		Если Форма.ВсеРазделыБезОбъектов Тогда
			...
			Форма.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ru='Раздел';uk='Розділ'");
			Форма.Элементы.ДатыЗапрета.Отображение = ОтображениеТаблицы.Список;
			
		Иначе
			Форма.Элементы.ДатыЗапретаПолноеПредставление.Заголовок = НСтр("ru='Раздел, объект';uk='Розділ об''єкт'");
...
Процедура УстановитьВидимость()
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru='Разрешена загрузка из других программ любых данных прошлых периодов.';uk='Дозволене завантаження з інших програм будь-яких даних минулих періодів.'");
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ru='Даты запрета загрузки данных из других программ действуют одинаково для всех пользователей.';uk='Дати заборони завантаження даних з інших програм діють однаково для всіх користувачів.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ru='Персональная настройка дат запрета загрузки данных прошлых периодов из других программ для выбранных пользователей.';uk='Персональна настройка дат заборони завантаження даних минулих періодів з інших програм для вибраних користувачів.'");
		КонецЕсли;
	Иначе
		Если УстановкаДатыЗапрета = "НетЗапрета" Тогда
			РасширеннаяПодсказка = НСтр("ru='Разрешены ввод и редактирование любых данных прошлых периодов.';uk='Дозволене введення і редагування будь-яких даних минулих періодів.'");
		ИначеЕсли УстановкаДатыЗапрета = "ДляВсехПользователей" Тогда
			РасширеннаяПодсказка = НСтр("ru='Даты запрета ввода и редактирования данных прошлых периодов действуют одинаково для всех пользователей.';uk='Дати заборони введення і редагування даних минулих періодів діють однаково для всіх користувачів.'");
		Иначе
			РасширеннаяПодсказка = НСтр("ru='Персональная настройка дат запрета ввода и редактирования данных прошлых периодов для выбранных пользователей.';uk='Персональна настройка дат заборони введення і редагування даних минулих періодів для вибраних користувачів.'");
		КонецЕсли;
	КонецЕсли;
...
Процедура РедактироватьДатуЗапретаВФорме()
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru='Выделенные строки не заполнены.';uk='Виділені рядки не заповнені.'"));
		Возврат;
	КонецЕсли;
	...
	Если ОбновитьВыделение Тогда
		...
		ПоказатьПредупреждение(
			...
			НСтр("ru='Снято выделение с незаполненных строк.';uk='Знято виділення з незаповнених рядків.'"));
	Иначе
...
Процедура РедактироватьДатуЗапретаВФормеЗавершение(ВыделенныеСтроки) Экспорт
	...
	Иначе
		...
		ПараметрыФормы.Вставить("Объект", 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выбранные строки (%1)';uk='Вибрані рядки (%1)'"), ВыделенныеСтроки.Количество()));
	КонецЕсли;
...
Функция ТекстПредставленияОбщейДаты()
	...
	Возврат "<" + НСтр("ru='Общая дата для всех разделов';uk='Загальна дата для всіх розділів'") + ">";
		...
		Если ЭтоВсеПользователи(ТекущийПользователь) Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ни для одного раздела не были введены даты запрета, 
                    ...
                    |;uk='Ні для одного розділу не були введені дати заборони, 
                    |тому для всіх користувачів буде діяти більш проста настройка ""%1"".'"),
				СпособУказанияВБазеДанных);
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ни для одного раздела не были введены даты запрета, 
                    ...
                    |;uk='Ні для одного розділу не були введені дати заборони, 
                    |тому для ""%1"" буде діяти більш проста настройка ""%2"".'"),
		...
		Если ЭтоВсеПользователи(ТекущийПользователь) Тогда
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ни для одного объекта не были введены даты запрета, 
                    |поэтому для всех пользователей будет действовать более простая настройка ""%1"".'
                    |;uk='Ні для одного об''єкта не були введені дати заборони, 
                    |тому для всіх користувачів буде діяти більш проста настройка ""%1"".'"),
				СпособУказанияВБазеДанных);
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ТекстСообщенияВВыбранномРазделеДатыЗапретаДляОбъектовНеУстанавливаются(Раздел)
	...
	Возврат ?(Раздел <> РазделПустаяСсылка, 
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В разделе ""%1"" не предусмотрена установка дат запрета для отдельных объектов.';uk='В розділі ""%1"" не передбачено встановлення дат заборони для окремих об''єктів.'"), Раздел),
		НСтр("ru='Для установки дат запрета по отдельным объектам выберите один из разделов ниже и нажмите ""Подобрать"".';uk='Для встановлення дат заборони по окремих об''єктах виберіть один з розділів нижче і натисніть ""Підібрати"".'"));
