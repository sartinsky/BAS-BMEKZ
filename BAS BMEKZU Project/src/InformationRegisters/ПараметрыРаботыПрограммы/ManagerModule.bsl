...
Функция ИзмененияПараметраРаботыПрограммы(ИмяПараметра) Экспорт
	...
	Если Не ЭтоИзмененияПараметраРаботыПрограммы(ПоследниеИзменения) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для параметра работы программы ""%1"" не найдены изменения.';uk='Для параметра роботи програми ""%1"" не знайдені зміни.'"), ИмяПараметра)
...
Функция ЗагрузитьОбновитьПараметрыРаботыПрограммыВФоне(ОжидатьЗавершение, ИдентификаторФормы, СообщитьПрогресс) Экспорт
	...
	ПараметрыОперации.НаименованиеФоновогоЗадания = НСтр("ru='Фоновое обновление параметров работы программы';uk='Фонове оновлення параметрів роботи програми'");
	...
	Если Не ДоступноВыполнениеФоновыхЗаданий() Тогда
		ВызватьИсключение
			НСтр("ru='Обновление параметров работы программы не может быть выполнено,
                       ...
                       |;uk='Оновлення параметрів роботи програми не може бути виконано,
                       |т. я. підключені розширення конфігурації, модифікуючі права у ролях конфігурації.
                       |Для виконання оновлення необхідно відключити такі розширення.'");
	КонецЕсли;
	...
	Если Результат = Неопределено Или Результат.Статус = "Отменено" Тогда
		...
		КраткоеПредставлениеОшибки =
			НСтр("ru='Не удалось обновить параметры работы программы по причине:
                       ...
                       |;uk='Не вдалося оновити параметри роботи програми з причини:
                       |Фонове завдання, що виконує оновлення скасовано.'");
		...
		Иначе
			КраткоеПредставлениеОшибки =
				НСтр("ru='Не удалось обновить параметры работы программы по причине:
                           ...
                           |;uk='Не вдалося оновити параметри роботи програми з причини:
                           |Фонове завдання, що виконує оновлення не повернуло результат.'");
		КонецЕсли;
	   ...
	   И Не ВыполнятьОбновлениеБезФоновогоЗадания() Тогда
		ВызватьИсключение
			НСтр("ru='Не удалось обновить параметры работы программы по причине:
                       ...
                       |;uk='Не вдалося оновити параметри роботи програми з причини:
                       |Знайдені підключені розширення конфігурації.'");
	КонецЕсли;
	   ...
	   И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение
			НСтр("ru='Не удалось обновить параметры работы программы по причине:
                       ...
                       |;uk='Не вдалося оновити параметри роботи програми з причини:
                       |Оновлення неможливо виконати в області даних.'");
	КонецЕсли;
...
Процедура ПроверитьВозможностьОбновленияВМоделиСервиса(ИмяПараметра, НовоеЗначение, Операция)
	...
	ИмяСобытия = НСтр("ru='Параметры работы программы.Не выполнено обновление в неразделенном режиме';uk='Параметри роботи програми.Не виконано оновлення в нерозділеному режимі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
	Комментарий =
		НСтр("ru='1. Перешлите сообщение в техническую поддержку.
                   |2. Попытайтесь устранить проблему самостоятельно. Для этого
                   |запустите программу с параметром командной строки платформы
                   ...
                   |;uk='1. Надішліть повідомлення в технічну підтримку.
                   |2. Спробуйте вирішити проблему самостійно. Для цього
                   |запустіть програму з параметром командного рядка платформи
                   |""/С ЗапуститьОбновлениеИнформационнойбазы"" від імені користувача
                   |адміністратор сервісу, тобто в неподіленому режимі.
                   |
                   |Відомості про проблемний параметр:'");
	...
	ТекстОшибки =
		НСтр("ru='Параметры работы программы не обновлены в неразделенном режиме.
                   ...
                   |;uk='Параметри роботи програми не оновлені в нерозділеному режимі.
                   |Зверніться до адміністратора сервісу. Подробиці в журналі реєстрації.'");
