...
Процедура ОбновитьГруппыЗначенийДоступа(ЗначенияДоступа = Неопределено,
	...
	ТипыЗначенийСИзменениями = Новый Соответствие;
	...
	Если ЗначенияДоступа = Неопределено Тогда
		...
		Для Каждого ИмяТаблицы Из ЗначенияДоступаСГруппами.ИменаТаблицДляОбновления Цикл
			...
			ОбновитьГруппыЗначенияДоступа(МенеджерОбъекта.ПустаяСсылка(), ЕстьИзменения, ТипыЗначенийСИзменениями);
			
			Пока Выборка.Следующий() Цикл
				ОбновитьГруппыЗначенияДоступа(Выборка.Ссылка, ЕстьИзменения, ТипыЗначенийСИзменениями);
			КонецЦикла;
		КонецЦикла;
	...
	ИначеЕсли ТипЗнч(ЗначенияДоступа) = Тип("Массив") Тогда
		...
		Для каждого ЗначениеДоступа Из ЗначенияДоступа Цикл
			ОбновитьГруппыЗначенияДоступа(ЗначениеДоступа, ЕстьИзменения, ТипыЗначенийСИзменениями);
		КонецЦикла;
	Иначе
		ОбновитьГруппыЗначенияДоступа(ЗначенияДоступа, ЕстьИзменения, ТипыЗначенийСИзменениями);
	КонецЕсли;
	
	УправлениеДоступомСлужебный.ЗапланироватьОбновлениеЗависимыхСписковПоЗначениямСГруппами(
		ТипыЗначенийСИзменениями);
...
Процедура ОбновитьГруппыПустыхЗначенийДоступа() Экспорт
	...
	Для Каждого ИмяТаблицы Из ЗначенияДоступаСГруппами.ИменаТаблицДляОбновления Цикл
		...
		ОбновитьГруппыЗначенияДоступа(ПустаяСсылка, Неопределено, ТипыЗначенийСИзменениями);
	КонецЦикла;
...
//                    устанавливается Истина, иначе не изменяется.
//
Процедура ОбновитьГруппыЗначенияДоступа(ЗначениеДоступа, ЕстьИзменения, ТипыЗначенийСИзменениями)
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка при обновлении групп значений доступа.';uk='Помилка при оновленні груп значень доступу.'")
		+ Символы.ПС
		+ Символы.ПС;
		...
		Если СвойстваВидаДоступа.ТипГруппЗначений = Тип("Неопределено") Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У значения доступа ""%1""
                           ...
                           |типу ""%2""
                           |не вдалося прочитати реквізит Посилання по причині:
                           |%3'"),
			...
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У значения доступа ""%1""
                               ...
                               |з реквізитом ГруппаДоступа з причини:
                               |%3'"),
					Строка(ЗначениеДоступа),
					Строка(ТипЗначенияДоступа),
					КраткоеПредставлениеОшибки(Ошибка));
			...
			Иначе
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У значения доступа ""%1""
                               ...
                               |;uk='Значення доступу ""%1""
                               |типу ""%2""
                               |не вдалося прочитати реквізит ГруппаДоступа з причини:
                               |%3'"),
					Строка(ЗначениеДоступа),
					Строка(ТипЗначенияДоступа),
					КраткоеПредставлениеОшибки(Ошибка));
	...
	Если СвойстваВидаДоступа.ТипГруппЗначений <> Тип("Неопределено")
	   И (Объект = Неопределено Или Не Объект.ЭтоНовый()) Тогда
		...
		ТипыЗначенийСИзменениями.Вставить(ТипЗначенияДоступа, Истина);
	КонецЕсли;
