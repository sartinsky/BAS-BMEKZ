...
Процедура ОбновитьДанныеМонитора(Организация, Дата = Неопределено, ИнтервалОбновления = 0) Экспорт
	...
	Попытка
		...
		Если ТаблицаОстатковДС <> Неопределено Тогда 
			ЗаписатьДанныеРаздела(Перечисления.РазделыМонитораРуководителя.ОстаткиДенежныхСредств, Организация, Дата, ТаблицаОстатковДС);
		КонецЕсли;
		
		// Движение денежных средств
		Если ТаблицаДвиженийДС <> Неопределено Тогда 
			ЗаписатьДанныеРаздела(Перечисления.РазделыМонитораРуководителя.ДвижениеДенежныхСредств, Организация, Дата, ТаблицаДвиженийДС);
		КонецЕсли;
		
		// Продажи по контрагентам
		Если ТаблицаПродажиПоКонтрагентам <> Неопределено Тогда 
			ЗаписатьДанныеРаздела(Перечисления.РазделыМонитораРуководителя.ПродажиПоКонтрагентам, Организация, Дата, ТаблицаПродажиПоКонтрагентам);
		КонецЕсли;
		
		// Продажи по номенклатурным группам
		Если ТаблицаПродажиПоНомГруппам <> Неопределено Тогда 
			ЗаписатьДанныеРаздела(Перечисления.РазделыМонитораРуководителя.ПродажиПоНоменклатурнымГруппам, Организация, Дата, ТаблицаПродажиПоНомГруппам);
		КонецЕсли;
		
		// Задолженность покупателей
		Если ЗадолженностьПокупателей <> Неопределено Тогда 
			ЗаписатьДанныеРаздела(Перечисления.РазделыМонитораРуководителя.ЗадолженностьПокупателей, Организация, Дата, ЗадолженностьПокупателей);
		КонецЕсли;
		
		// Задолженность поставщикам
		Если ЗадолженностьПоставщикам <> Неопределено Тогда 
			ЗаписатьДанныеРаздела(Перечисления.РазделыМонитораРуководителя.ЗадолженностьПоставщикам, Организация, Дата, ЗадолженностьПоставщикам);
		КонецЕсли;
		
		// Задолженность покупателей просроченная
		Если ПросроченнаяЗадолженностьПокупателей <> Неопределено Тогда 
			ЗаписатьДанныеРаздела(Перечисления.РазделыМонитораРуководителя.ПросроченнаяЗадолженностьПокупателей, Организация, Дата, ПросроченнаяЗадолженностьПокупателей);
		КонецЕсли;
		
		// Задолженность поставщикам просроченная
		Если ПросроченнаяЗадолженностьПоставщикам <> Неопределено Тогда 
			ЗаписатьДанныеРаздела(Перечисления.РазделыМонитораРуководителя.ПросроченнаяЗадолженностьПоставщикам, Организация, Дата, ПросроченнаяЗадолженностьПоставщикам);
		КонецЕсли;
	Исключение
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "МониторРуководителя", НСтр("ru='Монитор руководителя';uk='Монітор керівника'"), Таблица);
