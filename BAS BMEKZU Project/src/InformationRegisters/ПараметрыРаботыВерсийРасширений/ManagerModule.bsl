	 ...
	 Или Параметры.ВерсияКонфигурации <> Метаданные.Версия Тогда
		ТекстОшибки =
			НСтр("ru='Не удалось обновить все параметры работы расширений, так как
                       ...
                       |;uk='Не вдалося оновити всі параметри роботи розширень, так як
                       |змінилося ім''я або версія конфігурації - потрібен перезапуск сеансу.'");
	КонецЕсли;
	 ...
	 Или Параметры.УстановленныеРасширения.Исправления <> ПараметрыСеанса.УстановленныеРасширения.Исправления Тогда
		ТекстОшибки =
			НСтр("ru='Не удалось обновить все параметры работы расширений,
                       ...
                       |;uk='Не вдалося оновити всі параметри роботи розширень,
                       |так як зазначений склад розширень відрізняється від поточного.'");
	КонецЕсли;
...
Процедура ОбновитьПараметрыРаботыРасширений(ПроверяемыеРасширения = Неопределено, НеподключенныеРасширения = "") Экспорт
	...
	Если ТипЗнч(Результат) <> Тип("Структура") Тогда
		ВызватьИсключение НСтр("ru='Фоновое задание подготовки расширений не вернуло результат.';uk='Фонове завдання підготовки розширень не повернуло результат.'");
	КонецЕсли;
