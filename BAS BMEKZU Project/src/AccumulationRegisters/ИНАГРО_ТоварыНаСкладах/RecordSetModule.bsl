...
Процедура КонтрольОстатков(ДокументОбъект, ИмяТабличнойЧасти, СтруктураШапкиДокумента, Отказ, Заголовок) Экспорт
	...
	ЕстьСерия               = Ложь;
	ЕстьКачество            = Ложь;
	ЕстьОрдерВТабЧасти      = ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("ПриходныйОрдер", МетаданныеДокумента, ИмяТабличнойЧасти);
	ЕстьСкладВТабЧасти      = ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("Склад", МетаданныеДокумента, ИмяТабличнойЧасти);
	ЕстьДокументРезерва     = ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("ДокументРезерва", МетаданныеДокумента, ИмяТабличнойЧасти);
	...
	ЕстьКоэффициент         = ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("Коэффициент", МетаданныеДокумента, ИмяТабличнойЧасти);
	ЕстьКлючСтроки          = ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("КлючСтроки", МетаданныеДокумента, ИмяТабличнойЧасти);
	...
	МассивПустыхСсылок.Добавить(Документы.ИНАГРО_ПриходныйОрдерНаТовары.ПустаяСсылка());
	...
	|   Док.ДокументРезерва                                    КАК ДокументРезерва,"), " 
	|   &ПриходныйОрдер                                        КАК ПриходныйОрдер,")) + "
	|	Док.Номенклатура.БазоваяЕдиницаИзмерения.Представление КАК ЕдиницаХраненияОстатковПредставление,"
	+ ?(ЕстьХарактеристика, "
	...
	,"") 
	+ ?(ЕстьКоэффициент, "
	|	ВЫРАЗИТЬ(СУММА(Док.Количество) КАК Число(15,3)) КАК ДокументКоличество,", "
	...
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ
	|	РегистрНакопления.ИНАГРО_ТоварыНаСкладах.Остатки(, Склад в (&СписокСкладов) И
	...
	+ ?(ЕстьКоэффициент,	"ВЫРАЗИТЬ(СУММА(Док.Количество) КАК Число(15,3))",
	"СУММА(Док.Количество)") + "
	|
	|ДЛЯ ИЗМЕНЕНИЯ РегистрНакопления.ИНАГРО_ТоварыНаСкладах.Остатки // Блокирующие чтение таблицы остатков регистра для разрешения коллизий многопользовательской работы
	|";
	...
	Пока Выборка.Следующий() Цикл
		...
		СОКРЛП(Выборка.НоменклатураПредставление)+ 
		" к получению на склад " + СокрЛП(Выборка.Склад) + ?(ЕстьДокументПередачи," по документу " + СтруктураШапкиДокумента.ДокументПередачи, "")
		+ " недостаточно.";
		
		ОбщегоНазначенияБПКлиентСервер.СообщитьОбОшибке(СтрокаСообщения + Символы.ПС + Символы.Таб +
		НСТр("ru=' Не хватает  ';uk=' Не вистачає  ' ") + ?(КоличествоОстаток > 0 , Выборка.ДокументКоличество - КоличествоОстаток,  Выборка.ДокументКоличество) + " " + Выборка.ЕдиницаХраненияОстатковПредставление +
		НСТр("ru='; Остаток ';uk='; Залишок  '") + КоличествоОстаток + " " + Выборка.ЕдиницаХраненияОстатковПредставление +
		НСТр("ru='; Требуется  ';uk='; Потрібно  '") + Выборка.ДокументКоличество + " " + Выборка.ЕдиницаХраненияОстатковПредставление, Отказ, Заголовок);
