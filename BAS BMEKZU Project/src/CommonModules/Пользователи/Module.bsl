...
//                             * АутентификацияСтандартная - Булево - аутентификация платформы.
	...
	Если Не ПользовательСуществует Тогда
		Если Не СоздатьНового Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Пользователь информационной базы ""%1"" не существует.';uk='Користувач інформаційної бази ""%1"" не існує.'"),
				ИмяИлиИдентификатор);
		КонецЕсли;
		ПользовательИБ = ПользователиИнформационнойБазы.СоздатьПользователя();
	Иначе
		Если СоздатьНового Тогда
			ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
				НСтр("ru='Невозможно создать пользователя информационной базы ""%1"", так как он уже существует.';uk='Неможливо створити користувача інформаційної бази ""%1"", так як він вже існує.'"),
				СтарыеСвойства.Имя,
		...
		Если ОбновляемыеСвойства.Свойство("СтарыйПароль")
		   И ТипЗнч(ОбновляемыеСвойства.СтарыйПароль) = Тип("Строка") Тогда
			...
			Если Не СтарыйПарольСовпадает Тогда
				ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
					НСтр("ru='При записи пользователя информационной базы ""%1"" старый пароль указан не верно.';uk='При запису користувача інформаційної бази ""%1"" старий пароль вказаний не вірно.'"),
					СтарыеСвойства.Имя,
					СтарыеСвойства.УникальныйИдентификатор);
			КонецЕсли;
		КонецЕсли;
	...
	Исключение
		ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("ru='Не удалось записать свойства пользователя информационной базы ""%1"" по причине:
                       |%2.'
                       |;uk='Не вдалося записати властивості користувача інформаційної бази ""%1"" з причини:
                       |%2.'"),
			ПользовательИБ.Имя,
	...
	Исключение
		ВызватьИсключение ОписаниеОшибкиПриЗаписиПользователяИБ(
			НСтр("ru='Не удалось удалить пользователя информационной базы ""%1"" по причине:
                       |%2.'
                       |;uk='Не вдалося вилучити користувача інформаційної бази ""%1"" з причини:
                       |%2.'"),
			ПользовательИБ.Имя,
	   ...
	   И ТипЗнч(Приемник) <> Тип("УправляемаяФорма") Тогда
		...
		ВызватьИсключение
			НСтр("ru='Недопустимое значение параметра Приемник или Источник
                       ...
                       |;uk='Неприпустиме значення параметра Приемник або Источник
                       |у процедурі СкопироватьСвойстваПользователяИБ загального модуля Пользователи.'");
	КонецЕсли;
				...
				Если ИдентификаторПользователяСервиса Тогда
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Пользователю сервиса с идентификатором ""%1""
                                   ...
                                   |;uk='Користувачу сервісу з ідентифікатором ""%1""
                                   |відповідає більше одного елемента в довіднику:'"),
						ТекущийНеоднозначныйИдентификатор);
				Иначе
					ОписаниеОшибки = ОписаниеОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Пользователю ИБ ""%1"" с идентификатором ""%2""
                                   |соответствует более одного элемента в справочнике:'
                                   |;uk='Користувачу ІБ ""%1"" з ідентифікатором ""%2""
                                   |відповідає більше одного елемента в довіднику:'"),
						ИмяДляВхода,
						ТекущийНеоднозначныйИдентификатор);
				КонецЕсли;
...
// Проверяет соответствие прав ролей назначению ролей, указанном 
			...
			Если НЕ СтарыйПарольСовпадает Тогда
				ОписаниеОшибки = ОписаниеОшибкиПриЗаписиПользователяИБ(
					НСтр("ru='При записи пользователя информационной базы
                               ...
                               |;uk='При запису користувача інформаційної бази
                               |%1,
