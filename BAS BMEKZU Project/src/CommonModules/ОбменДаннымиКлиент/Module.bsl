...
Процедура ОткрытьПодробноеОписаниеСинхронизации(СсылкаНаПодробноеОписание) Экспорт
	...
	Если ВРег(Лев(СсылкаНаПодробноеОписание, 4)) = "HTTP" Тогда
		...
		ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(СсылкаНаПодробноеОписание);
...
Процедура ОбработчикОткрытияФайлаИлиКаталога(Объект, ИмяСвойства, СтандартнаяОбработка = Ложь) Экспорт
	...
	ОбщегоНазначенияКлиент.ОткрытьПроводник(ПолноеИмяФайла);
...
//     ОповещениеЗавершения  - ОписаниеОповещения - Необязательное оповещение, которое будет вызвано со следующими
//                                                  параметрами:
//                                 Результат               - Строка, Неопределено - Выбранное значение (массив строк, если был
//                                                                                  множественный выбор), или Неопределено - если ничего не выбрано;
//                                 ДополнительныеПараметры - Неопределено.
//
//
...
Процедура ОткрытьИнструкциюКакИзменитьПарольСинхронизацииДанных(Знач АдресДляВосстановленияПароляУчетнойЗаписи) Экспорт
	...
	Если ПустаяСтрока(АдресДляВосстановленияПароляУчетнойЗаписи) Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru='Адрес инструкции для восстановления пароля учетной записи не задан.';uk='Адреса інструкції для відновлення пароля облікового запису не вказана.'"));
		
	Иначе
		
		ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку(АдресДляВосстановленияПароляУчетнойЗаписи);
