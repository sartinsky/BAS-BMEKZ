...
Функция ИмяСобытияУдалениеФайла()
	...
	Возврат НСтр("ru='Хранилище сертификатов.Удаление файла';uk='Сховище сертифікатів.Видалення файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ДобавитьПроверкаВходящихПараметров(Сертификат, ТипХранилища)
	...
	Если ТипЗнч(Сертификат) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru='Недопустимое значение параметра Сертификат (указан адрес, который не является адресом временного хранилища)';uk='Неприпустиме значення параметра Сертифікат (вказано адресу, який не є адресою тимчасового сховища)'"), 
			"ХранилищеСертификатов.Добавить");
	КонецЕсли;
