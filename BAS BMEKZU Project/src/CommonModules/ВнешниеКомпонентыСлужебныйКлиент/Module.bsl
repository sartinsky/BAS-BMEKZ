...
Процедура ПроверитьДоступностьКомпоненты(Оповещение, Контекст)
	...
	Если Информация.Состояние = "ОтключенаАдминистратором" Тогда 
		...
		Результат.ОписаниеОшибки = НСтр("ru='Отключена администратором.';uk='Відключено адміністратором.'");
	...
	ИначеЕсли Информация.Состояние = "НеНайдена" Тогда 
		...
		Иначе 
			Результат.ОписаниеОшибки = НСтр("ru='Компонента не найдена в хранилище.';uk='Компонента не знайдена у сховищі.'");
			ВыполнитьОбработкуОповещения(Оповещение, Результат);
		КонецЕсли;
	...
	Иначе 
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось подключить внешнюю компоненту ""%1"" на клиенте
                       ...
                       |з реєстру Windows
                       |з причини:
                       |Підключити компоненту з реєстру Windows можливо тільки в тонкій або товстій клієнтів Windows.'"),
		Контекст.Идентификатор);
	...
	Иначе 
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось подключить внешнюю компоненту ""%1"" на клиенте
                       ...
                       |з реєстру Windows
                       |з причини:
                       |Метод НачатьПодключениеВнешнейКомпоненты повернув Ложь.'"),
			Контекст.Идентификатор);
	...
	Иначе 
		...
		ПоказатьПредупреждение(Оповещение, 
			НСтр("ru='Загрузка внешней компоненты прервана
                       ...
                       |;uk='Завантаження зовнішньої компоненти перерванл
                       |з причини:
...
Процедура СохранитьКомпонентуВФайлПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		Диалог.Заголовок = НСтр("ru='Выберите файл для сохранения внешней компоненты';uk='Виберіть файл для збереження зовнішньої компоненти'");
		Диалог.Фильтр    = НСтр("ru='Файлы внешних компонент (*.zip)|*.zip|Все файлы (*.*)|*.*';uk='Файли зовнішніх компонент (*.zip)|*.zip|Всі файли (*.*)|*.*'");
...
Процедура СохранитьКомпонентуВФайлПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ПолученныеФайлы <> Неопределено 
		...
		ПоказатьОповещениеПользователя(НСтр("ru='Сохранение в файл';uk='Збереження у файл'"),,
			НСтр("ru='Внешняя компонента успешно сохранена в файл.';uk='Зовнішня компонента успішно збережена у файл.'"), БиблиотекаКартинок.Успешно32);
	КонецЕсли;
