...
//
// Параметры:
//  ПараметрыОтправкиПисьма  - Структура          - см. ПараметрыОтправкиПисьма().
	...
	Если ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
		ПоказатьПредупреждение(ОбработчикРезультата, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Проверка параметров учетной записи завершилась с ошибками:
                       |%1'
                       |;uk='Перевірку параметрів облікового запису завершено з помилками:
                       |%1'"), СообщениеОбОшибке ),,
			НСтр("ru='Проверка учетной записи';uk='Перевірка облікового запису'"));
	Иначе
		ПоказатьПредупреждение(ОбработчикРезультата, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Проверка параметров учетной записи завершилась успешно. %1';uk='Перевірка параметрів облікового запису завершилася успішно. %1'"),
			ДополнительноеСообщение),,
			НСтр("ru='Проверка учетной записи';uk='Перевірка облікового запису'"));
	КонецЕсли;
