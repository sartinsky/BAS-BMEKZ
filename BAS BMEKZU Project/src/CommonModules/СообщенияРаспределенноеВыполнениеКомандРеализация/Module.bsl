...
Функция ОтработатьПолучениеФайла(Сообщение) Экспорт
	...
	Событие = Событие + НСтр("ru='Запрос на передачу файла';uk='Запит на передачу файлу'",Локаль); 
    ...
    |;uk='Оброблений надійшовший запит на передачу файлу. Параметри запиту:
    |Call_ID: %1
    |File_ID: %2
    |Sender: %3
    |Recipient: %4
    |
    |Файл збережений під таким ім''ям:
    |%5
    |
    |Відповідь прикладного обробника: %6'",Локаль); 
...
Функция ОтработатьКвитанциюПередачиФайла(Сообщение) Экспорт
	...
	Если ТелоСообщения.Error Тогда
		...
		Ответ = РаспределенноеВыполнениеКомандПереопределяемый.ОбработатьОшибкуПередачиФайла(ИдентификаторВызова, 
 		...
 		Событие = Событие + НСтр("ru='Ошибка передачи файла';uk='Помилка передачі файлу'",Локаль); 
        ...
        |
        |Ответ прикладного обработчика: %6'
        |;uk='При передачі файлу виникла помилка. Параметри запиту:
        |Call_ID: %1
        |Sender: %2
        |Recipient: %3
        |CallCode: %4
	...
	Иначе
		...
		Событие = Событие + НСтр("ru='Квитанция о передаче файла';uk='Квитанція про передачу файлу'",Локаль); 
        ...
        |;uk='Оброблена квитанція на передачу файлу, що надійшла. Параметри запиту:
        |Call_ID: %1
        |Sender: %2
        |Recipient: %3
        |CallCode: %4
        |
        |Відповідь прикладного обробника: %5'",Локаль); 
...
Функция ОтработатьПрямойВызов(Сообщение) Экспорт
	...
	Событие = РаспределенноеВыполнениеКоманд.ИмяСобытияЖурналаРегистрации() + ".";
	Событие = Событие + НСтр("ru='Задание поставлено в очередь';uk='Завдання поставлено в чергу'",Локаль); 
	Комментарий = НСтр("ru='В очередь поставлено выполнение команды %1 (обработка %2, вызов %3).';uk='В чергу поставлено виконання команди %1 (обробка %2, виклик %3).'"); 
