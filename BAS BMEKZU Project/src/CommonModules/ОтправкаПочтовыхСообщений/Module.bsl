...
Функция ИмяФайлаПечатнойФормы(ОбъектПечати, НазваниеПечатнойФормы, Расширение)
	...
	Иначе
		...
		Шаблон = НСтр("ru='[НазваниеПечатнойФормы] [ПредставлениеОбъекта]';uk='[НазваниеПечатнойФормы] [ПредставлениеОбъекта]'");
	КонецЕсли;
...
Процедура ЗаполнитьТемуТелоПисьмаСКарточкойОрганизации(МассивОбъектов, ПараметрыПисьма, ТаблицаПечатныхФорм)
	...
	ТемаПисьма = СтрШаблон(НСтр("ru='Реквизиты %1';uk='Реквізити %1'"), ТекстНаименование);
	ТемаПисьмаИзменение = СтрШаблон(НСтр("ru='Внимание! Изменились реквизиты %1';uk='Увага! Змінилися реквізити %1'"), ТекстНаименование);
	...
	ВставитьТекстовыйУзел(ЭлементБлок, ПервыйДочерний, НСтр("ru='Новые реквизиты';uk='Нові реквізити'") + ":", Истина);
	ВставитьТекстовыйУзел(ЭлементБлок, ПервыйДочерний, "", Истина);
...
Процедура ЗаполнитьТемуТелоПисьма(МассивОбъектов, ПараметрыПисьма, КоллекцияПечатныхФорм);
	...
	ТекстПисьма = НСтр("ru='К письму приложены документы:';uk='До листа включені документи:'");
	...
	Для Каждого Объект Из МассивОбъектов Цикл
		...
		Иначе
			...
			Для Каждого ПечатнаяФорма Из СведенияОбъектаПечати.ПечатныеФормы Цикл
				ТекстПисьма = ТекстПисьма + Символы.ПС + СтрШаблон(НСтр("ru='- %1';uk='- %1'"), ПечатнаяФорма.Ключ.Представление);
			КонецЦикла;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ПутьКДанным) Тогда
			Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
				ТекстПисьма = ТекстПисьма + Символы.ПС + СтрШаблон(НСтр("ru='- %1';uk='- %1'"), ПечатнаяФорма[ПутьКДанным]);
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьТемуТелоПисьмаСДокументом(МассивОбъектов, ПараметрыПисьма, ПараметрыВывода, КоллекцияПечатныхФорм, КоличествоЭД)
	...
	Если ПараметрыВывода.Свойство("ДокументыКомплекта") Тогда
		Для каждого ОбъектПечати Из МассивОбъектов Цикл
			Если ПараметрыВывода.ДокументыКомплекта[ОбъектПечати] <> Неопределено Тогда
				Для каждого КлючИЗначение Из ПараметрыВывода.ДокументыКомплекта[ОбъектПечати] Цикл
	...
	Если Получатель <> Неопределено Тогда
		ПредставлениеПолучателя = ПолучитьПредставлениеСПрефиксом(НСтр("ru='для';uk='для'"),
			НаименованиеКонтрагента(Получатель));
	КонецЕсли;
	
	ПредставлениеОтправителя = "";
	Если Отправитель <> Неопределено Тогда
		ПредставлениеОтправителя = ПолучитьПредставлениеСПрефиксом(НСтр("ru='от';uk='від'"),
			НаименованиеКонтрагента(Отправитель));
	КонецЕсли;
	
	ТемаПисьма = СтрШаблон(НСтр("ru='Документы %1 %2';uk='Документи %1 %2'"),
		ПредставлениеПолучателя,
		ПредставлениеОтправителя);
	
	ТекстПисьма = СтрШаблон(НСтр("ru='К письму приложены документы %1 %2:';uk='До листа прикріплено документи %1 %2:'"),
		ПредставлениеПолучателя,
		ПредставлениеОтправителя);
	...
	Для Каждого Объект Из МассивОбъектов Цикл
		...
		Если ТаблицаСведенийОбъектовПечати <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(СведенияОбъектаПечати.СуммаДокумента) Тогда
				Шаблон = НСтр("ru='на сумму %1 %2';uk='на суму %1 %2'");
		...
		Иначе
			СведенияОбъектаПечати = КоллекцияПечатныхФорм.Найти(Объект, "Объект");
			Для Каждого ПечатнаяФорма Из СведенияОбъектаПечати.ПечатныеФормы Цикл
				ТекстПисьма = ТекстПисьма + Символы.ПС + СтрШаблон(НСтр("ru='- %1 %2';uk='- %1 %2'"),
		...
		Если ЗначениеЗаполнено(ПутьКДанным) Тогда
			Для Каждого ПечатнаяФорма Из КоллекцияПечатныхФорм Цикл
				...
				Если ПараметрыВывода.Свойство("ДокументыКомплекта") 
					...
					ОбъектДляПечати = ПараметрыВывода.ДокументыКомплекта[Объект][ПечатнаяФорма[ПутьКДанным]];
					
					Если ТаблицаСведенийОбъектовПечати <> Неопределено Тогда
						СведенияОбъектаПечати = ТаблицаСведенийОбъектовПечати.Найти(ОбъектДляПечати, "Ссылка");
						Если ЗначениеЗаполнено(СведенияОбъектаПечати.СуммаДокумента) Тогда
							Шаблон = НСтр("ru='на сумму %1 %2';uk='на суму %1 %2'");
					...
					КонецЕсли; 
				Иначе
					ОбъектДляПечати = Объект;
				КонецЕсли;
				
				ТекстПисьма = ТекстПисьма + Символы.ПС + СтрШаблон(НСтр("ru='- %1 %2';uk='- %1 %2'"),
					ОбщегоНазначенияБПВызовСервера.СформироватьЗаголовокДокумента(ОбъектДляПечати, ПечатнаяФорма[ПутьКДанным]),
					?(ПустаяСтрока(ТекстСуммаДокументаКомплекта), ТекстСуммаДокумента, ТекстСуммаДокументаКомплекта));
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	Если КоличествоВложений > 0 И КоличествоЭД > 0 Тогда
		...
		Для Индекс = НачальныйИндекс По КоличествоВложений - 1 Цикл
			ТекстПисьма = ТекстПисьма + Символы.ПС+ СтрШаблон(НСтр("ru='- %1';uk='- %1'"),
