	...
	Если Макет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,
                  ...
                  |;uk='Не вдалося сформувати дозвіл на використання зовнішньої компоненти, що
	...
	Если ТипМакета <> Метаданные.СвойстваОбъектов.ТипМакета.ДвоичныеДанные И ТипМакета <> Метаданные.СвойстваОбъектов.ТипМакета.ВнешняяКомпонента Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты:
                  ...
                  |;uk='Не вдалося сформувати дозвіл на використання зовнішньої компоненти:
                  |макет %1 не містить двійкових даних.'"), ИмяМакета);
	КонецЕсли;
	...
	Если ПустаяСтрока(ФайлМанифеста) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось сформировать разрешение на использование внешней компоненты,
                  ...
                  |;uk='Не вдалося сформувати дозвіл на використання зовнішньої компоненти, що
                  |поставляється в макеті %1: в архіві не виявлений файл MANIFEST.XML.'"), ИмяМакета);
	КонецЕсли;
...
Процедура ПриЗаписиСлужебныхДанных(Объект) Экспорт
	...
	Если РаботаВБезопасномРежиме.УстановленБезопасныйРежим() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Запись объекта %1 недоступна: установлен безопасный режим: %2.';uk='Запис об''єкту %1 недоступний: встановлений безпечний режим: %2.'"),
