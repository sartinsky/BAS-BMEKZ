	...
	Если ФайлОбъект.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Двоичные данные файла отсутствуют в регистре ДвоичныеДанныеФайла
                       |
                       |Ссылка на файл: ""%1"".'
                       |;uk='Двійкові дані файлу відсутні в регістрі ДвоичныеДанныеФайла
                       |
                       |Посилання на файл: ""%1"".'"),
	...
	Попытка
		...
		Для Каждого ФайлИсточник Из ФайлыИсточник Цикл
			...
			Если ПрисоединенныйФайл.ТипХраненияФайла = Перечисления.ТипыХраненияФайлов.ВИнформационнойБазе Тогда
				...
				// Возможно еще не отработала процедура переноса файлов из регистра УдалитьХранимыеФайлыВерсий в ДвоичныеДанныеФайлов
				Если ХранилищеФайла = Неопределено Тогда
					ХранилищеФайла = ХранилищеФайлаИзИнформационнойБазыДоОбновленияБСП24(ТекущаяВерсияОбъект.Ссылка);
				КонецЕсли; 
...
Функция ХранилищеФайлаИзИнформационнойБазыДоОбновленияБСП24(ФайлСсылка) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	УдалитьХранимыеФайлыВерсий.ВерсияФайла КАК Файл,
	|	УдалитьХранимыеФайлыВерсий.ХранимыйФайл КАК ДвоичныеДанныеФайла
	|ИЗ
	|	РегистрСведений.УдалитьХранимыеФайлыВерсий КАК УдалитьХранимыеФайлыВерсий
	|ГДЕ
	|	УдалитьХранимыеФайлыВерсий.ВерсияФайла = &ФайлСсылка";
