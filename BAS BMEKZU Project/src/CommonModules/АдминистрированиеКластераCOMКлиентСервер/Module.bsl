...
#Иначе
			ВызватьИсключение КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) + Символы.ПС + Символы.ПС
				+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В случае ошибки о несоответствии версии компоненты ""comcntr"" следует зарегистрировать ее на компьютере %1
                    |для учетной записи ОС Windows, от имени которой выполняется программа платформы. Например:
                    |regsvr32.exe ""%2\comcntr.dll""'
                    |;uk='У випадку помилки про невідповідність версії компоненти ""comcntr"" слід зареєструвати її на комп''ютері %1
                    |для облікового запису Windows, від імені якої виконується програма платформи. Наприклад:
...
Функция ПолучитьКластер(СоединениеСАгентомСервера, Знач ПортКластера, Знач ИмяАдминистратораКластера, Знач ПарольАдминистратораКластера)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='На рабочем сервере %1 не найден кластер %2';uk='На робочому сервері %1 не знайдено кластер %2'"),
...
Функция ПолучитьОписаниеИБ(СоединениеСАгентомСервера, Кластер, Знач ИмяВКластере)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В кластере серверов %1:%2 не найдена информационная база ""%3""';uk='У кластері серверів %1:%2 не знайдена інформаційна база ""%3""'"),
		Кластер.HostName,
		Формат(Кластер.MainPort, "ЧГ=0"),
		ИмяВКластере);
...
Функция ПолучитьПрофильБезопасности(СоединениеСАгентомСервера, Кластер, ИмяПрофиля)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В кластере серверов %1:%2 не найден профиль безопасности ""%3""';uk='У кластері серверів %1:%2 не знайдено профіль безпеки ""%3""'"),
		Кластер.HostName,
		Формат(Кластер.MainPort, "ЧГ=0"),
		ИмяПрофиля);
