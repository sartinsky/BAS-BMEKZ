...
//
// Функции 
// ДополнитьСтрокуСимволами
// ПреобразоватьТипXMLJ0100107
// ПреобразоватьТипXMLx1201002
// ПреобразоватьТипXMLx12012
// ПреобразоватьТипXMLОбщий
// ПреобразоватьТипXML
//
// взяты из модуля формы Отчета РегламентированныйОтчетМастерВыгрузкиРегламентированнойОтчетности
// доработки:
//
// 1. Функция ПреобразоватьТипXML -> дописать Экспорт
//
// 2. Найти
...
Функция ПреобразоватьТипXMLОбщий(Значение,Тип,Результат,ОписаниеОшибки = "")
	...
	ИначеЕсли  (ВРег(Тип) = ВРег("DGKontrOp"))
		...
		Попытка
			Если (Результат = "0") ИЛИ  (Результат = "1") ИЛИ  (Результат = "3") 
				ИЛИ Лев(Результат,2) = "20" Тогда
				// это допустимо
			ИначеЕсли СтрДлина(Результат) <> 3 Тогда
	 ...
	 ИЛИ ВРег(Тип) = ВРег("DGMonth0") Тогда
		...
		Попытка
			...
			Если    Найти(Результат, ".") > 0
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение не является целым числом от (%1) до 12 (номер месяца)';uk='Значення не є цілим числом від (%1) до 12 (номер місяця)'"), МинЗнач);					
			КонецЕсли;
		Исключение
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Значение не является целым числом от (%1) до 12 (номер месяца)';uk='Значення не є цілим числом від (%1) до 12 (номер місяця)'"), МинЗнач);					
		КонецПопытки;
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGsti") Тогда
		...
		Результат = СокрЛП(Формат(Значение,"ЧЦ=2; ЧДЦ=0; ЧН=0"));
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGDate")	
		...
		Попытка
			...
			Если    СтрДлина(Результат) <> 8 
				...
				ОписаниеОшибки = НСтр("ru='Значение не является датой (после 1900 года)';uk='Значення не є датою (після 1900 року)'");
			КонецЕсли;
		Исключение
			ОписаниеОшибки = НСтр("ru='Значение не является датой (после 1900 года)';uk='Значення не є датою (після 1900 року)'");
		КонецПопытки;			
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGDateEndKv") Тогда
		...
		Попытка
			...
			Если КонецДня(Контроль) <> КонецКвартала(Контроль) Тогда
				ОписаниеОшибки = НСтр("ru='Дата не является концом квартала';uk='Дата не є кінцем кварталу'");
			КонецЕсли;
		Исключение
			ОписаниеОшибки = НСтр("ru='Дата не является концом квартала';uk='Дата не є кінцем кварталу'");
		КонецПопытки;			
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGKATOTTG") 
		...
		ОписаниеОшибки = НСтр("ru='Значение не является 17-и значным числом';uk='Значення не є 17-и значним числом'");
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGOzn")
		...
		Попытка
			...
			Если    Найти(Результат, ".") > 0
				...
				ИЛИ Контроль > 1
				ИЛИ Цел(Контроль) <> Контроль Тогда
				ОписаниеОшибки = НСтр("ru='Значение не является целым числом от 0 до 1';uk='Значення не є цілим числом від 0 до 1'");
			КонецЕсли;
		Исключение
			ОписаниеОшибки = НСтр("ru='Значение не является целым числом от 0 до 1';uk='Значення не є цілим числом від 0 до 1'");
		КонецПопытки;
	...
	ИначеЕсли ВРег(Тип) = ВРег("CodPilg1406Column") 
		...
		Попытка
			...
			Если    Найти(Результат, ".") > 0
				ИЛИ Контроль < 0 
				ИЛИ Контроль > 99999999
				ИЛИ Цел(Контроль) <> Контроль 
				ИЛИ (Лев(Результат, 4) <> "1406" и Результат <> "99999999") Тогда
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGdecimal1")
		...
		Попытка
			...
			Если ВРег(Тип) = ВРег("DGKODR")
				...
				ОписаниеОшибки = НСтр("ru='Значение не является двузначным числом c одним знаком после запятой';uk='Значення не є двозначним числом c одним знаком після коми'");
			КонецЕсли;
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGpercentAlloc") Тогда
		...
		Попытка
			...
			Если   (Найти(Результат, ".") < СтрДлина(Результат)-2) 
				...
				ОписаниеОшибки = НСтр("ru='Значение не является числом c двумя знаками после запятой меньшее 9000000000';uk='Значення не є числом c двома знаками після коми менше 9000000000'");
			КонецЕсли;
		Исключение
			ОписаниеОшибки = НСтр("ru='Значение не является числом c двумя знаками после запятой';uk='Значення не є числом c двома знаками після коми'");
		КонецПопытки;
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGdecimal8")
		...
		Попытка
			Контроль = Число(Результат);
			Если   Найти(Результат, ".") <> СтрДлина(Результат) - 8  
				ИЛИ Цел(Контроль*100000000) <> Контроль*100000000 Тогда
	...
	ИначеЕсли ВРег(Тип) = ВРег("DGPNtypr") Тогда
		...
		ИначеЕсли СтрДлина(Результат) <> 2
			...
			ИЛИ (Лев(Результат, 1) = "1" И Найти("01234567", Прав(Результат, 1)) = 0) Тогда
				...
				ОписаниеОшибки = НСтр("ru='Значением может быть только одно из 01...17 или 20';uk='Значення може бути тільки одне з 01 ... 17 або 20'");					
...
Функция ВыгрузитьНалоговыйДокумент1СВСоответствиеЗначений(КешСхем, ИмяШаблона, СоответствиеЗначений, ТипДокументаMEDoc, ВидЭлектронногоДокумента, ОрганизацияEDI, Документ1С, ДанныеДокумента, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если ПолучитьПоказателиXSDСхемыИПоказателиXSDСхемы(КешСхем, ТипДокументаMEDoc, ЮрФизЛицо, ИмяШаблона, ПоказателиШапка, ПоказателиТело, ПоказателиТЧ1) Тогда
		...
		Иначе
			...
			//Для Каждого КлючИЗначение Из СоответствиеТаблица0 Цикл
			//	ЗначениеРеквизита = КлючИЗначение.Значение;
			//	ПоказательТип = ПоказателиШапка.Получить(КлючИЗначение.Ключ);
			//	Если ПоказательТип = Неопределено Тогда
			//		//СоответствиеТаблица0.Вставить(КлючИЗначение.Ключ, ЗначениеРеквизита);
			//	Иначе
			//		оо = "";
			//		ЗначениеРеквизита = скEDI_НалоговыеДокументы.ПреобразоватьТипXML(ЗначениеРеквизита, ПоказательТип, ИмяШаблона, оо);
