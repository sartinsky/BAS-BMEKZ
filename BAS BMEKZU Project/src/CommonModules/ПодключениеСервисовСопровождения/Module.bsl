...
//     *Идентификатор - Строка - идентификатор сервиса в системе Портал ИТС. Поле обязательно для заполнения,
...
// из сервисов Портала ИТС. Функцию необходимо использовать для определения
...
//
// Параметры:
//  Идентификаторы - Массив - содержит идентификаторы сервисов в системе Портал ИТС.
//
// Возвращаемое значение:
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Важное         = Ложь;
	Дело.Представление  = НСтр("ru='Подключение тестового периода';uk='Підключення тестового періоду'");
	...
	Дело.Представление  = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Подключение тестового периода (%1)';uk='Підключення тестового періоду (%1)'"),
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если ВыборкаДетальныеЗаписи.Подключен Тогда
			...
			Представление    = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Подключен %1';uk='Підключений %1'"),
		...
		Иначе
			...
			Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось подключить %1';uk='Не вдалося підключити %1'"),
				ВыборкаДетальныеЗаписи.НаименованиеТестовогоПериода);
		КонецЕсли;
...
Процедура ОтключитьИнтернетПоддержкуПользователей() Экспорт
	...
	Если Не ИнтернетПоддержкаПользователей.ДоступноПодключениеИнтернетПоддержки() Тогда
		ВызватьИсключение НСтр("ru='Отключение интернет поддержки пользователей не доступно.';uk='Відключення інтернет підтримки користувачів не доступно.'");
	КонецЕсли;
...
Процедура ОбновитьДанныеСервисовСопровождения() Экспорт
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru='Обновление данных идентификаторов сервисов сопровождения. Начало обновления.';uk='Оновлення даних ідентифікаторів сервісів супроводу. Початок оновлення.'"),
		Ложь);
	...
	ЗаписатьИнформациюВЖурналРегистрации(
		НСтр("ru='Обновление данных идентификаторов сервисов сопровождения. Успешно завершено.';uk='Оновлення даних ідентифікаторів сервісів супроводу. Успішно завершено.'"),
		Ложь);
...
Процедура СоздатьРегламентноеЗаданиеПроверкиПодключения(
	...
	Если Не РазрешеноПодключениеТестовыхПериодов() Тогда
		ВызватьИсключение НСтр("ru='Подключение тестового периода недоступно.';uk='Підключення тестового періоду недоступно.'");
	КонецЕсли;
	...
	Попытка
		...
		ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Создано регламентное задание для проверки запроса на подключение: %1';uk='Створено регламентне завдання для перевірки запиту на підключення: %1'"),
			ИдентификаторЗапроса);
	...
	Исключение
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%1'
            |;uk='Помилка при створенні регламентного завдання перевірки підключення тестових періодів:
            |%1'"),
...
Процедура УдалитьРегламентноеЗаданиеПроверкиПодключения(ИдентификаторЗапроса)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удалено регламентное задание проверки запроса на подключение: %1';uk='Вилучено регламентне завдання перевірки запиту на підключення: %1'"),
		ИдентификаторЗапроса);
...
//
// Параметры:
//  Идентификатор  - Строка - идентификатор сервиса в системе Портал ИТС.
//
// Возвращаемое значение:
...
//    *ИдентификаторСервисаСопровождения - Строка - идентификатор сервиса в системе Портал ИТС;
...
Функция ОбновитьКэшТестовыхПериодов(Знач Идентификаторы)
	...
	Если ЗначениеЗаполнено(НеКорректныеИдентификаторы) Тогда
		...
		РезультатОбновления.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Описания сервисов: %1, не добавлены.';uk='Описи сервісів: %1, не додані.'"),
			НеКорректныеИдентификаторы);
	...
	Если Не РезультатОперации.СодержаниеОтвета.ПодключениеДоступно Тогда
        ...
        |;uk='Підключення тестових періодів тимчасово недоступне,
        |виконується обробка запиту. Повторіть спробу пізніше.'");
		Возврат РезультатОбновления;
	КонецЕсли;
	...
	Если РезультатОбновления.Ошибка Тогда
		РезультатОбновления.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обнаружены некорректные идентификаторы сервисов:
                |%1'
                |;uk='Виявлені некоректні ідентифікатори сервісів:
                |%1'"),
			НеКорректныеИдентификаторы);
		Возврат РезультатОбновления;
	КонецЕсли;
...
//   Ключ       - Строка - идентификатор сервиса в системе Портал ИТС;
	...
	Если ИдентификаторыЗапросовНабор.Количество() = 0 Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не обнаружен идентификатор запроса на подключение.
                ...
                |Идентификатор: %2'
                |;uk='Не виявлено ідентифікатор запиту на підключення.
                |Сервіс супроводу: %1
                |Ідентифікатор: %2'"),
			СервисСопровождения,
			ИдентификаторЗапроса);
	...
	Если Не ПустаяСтрока(РезультатПроверки.ИмяОшибки) Тогда
		...
		ПодробнаяИнформацияОбОшибке          = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось проверить доступность сервиса тестовых периодов:
                ...
                |Причина:
                |%2'
                |;uk='Не вдалося перевірити доступність сервісу тестових періодів:
                |%1.
                |Причина:
                |%2'"),
			URLОперацииPing,
...
Функция ИнформацияОДоступныхТестовыхПериодах(ИдентификаторыСервисов)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Начало обновление кэша для сервисов: %1';uk='Початок оновлення кешу для сервісів: %1'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при получении доступных тестовых периодов:
                |%1'
                |;uk='Помилка при отриманні доступних тестових періодів:
                |%1'"),
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить доступные тестовые периоды.
                ...
                |Причина:
                |%2'
                |;uk='Не вдалося отримати доступні тестові періоди.
                |При отриманні інформації про доступні тестові періоди сервіс повернув помилку: %1.
                |Причина:
                |%2'"),
			URLОперации,
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Завершено обновление кэша для сервисов: %1';uk='Завершено оновлення кешу для сервісів: %1'"),
		СтрСоединить(ИдентификаторыСервисов, ","));
	ЗаписатьИнформациюВЖурналРегистрации(
		ТекстЖурналРегистрации,
		Ложь);
...
Функция ПрочитатьДанные_availableList(ТелоJSON)
	...
	//  descriptionUrl - ссылка на страницу сервиса на Портале ИТС;
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Получен ответ сервиса Подключения тестовых периодов:
            |%1'
            |;uk='Отримано відповідь сервісу Підключення тестових періодів:
            |%1'"),
		ТелоJSON);
...
Процедура ПодключитьТестовыйПериод(ПараметрыЗапроса, АдресХранилища) Экспорт
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Подключение тестового периода:
            ...
            |;uk='Підключення тестового періоду:
            |Сервіс супроводу: %1
            |Ідентифікатор тестового періоду: %2'"),
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при подключении тестового периода:
                |%1'
                |;uk='Помилка при підключенні тестового періоду:
                |%1'"),
			РезультатОтправки.СообщениеОбОшибке);
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось подключить тестовый период.
                ...
                |Причина:
                |%2'
                |;uk='Не вдалося підключити тестовий період.
                |При активації тестового періоду сервіс повернув помилку: %1.
                |Причина:
                |%2'"),
			URLОперации,
			РезультатОтправки.ИнформацияОбОшибке);
...
Функция СостояниеЗапросаНаПодключение(
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Проверка состояния запроса на подключение тестового периода:
            ...
            |Идентификатор тестового периода: %2'
            |;uk='Перевірка стану запиту на підключення тестового періоду:
            |Ідентифікатор запиту: %1
            |Ідентифікатор тестового періоду: %2'"),
		ИдентификаторЗапроса,
		ИдентификаторТестовогоПериода);
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		РезультатОперации.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при проверке запроса на подключение:
                |%1'
                |;uk='Помилка при перевірці запиту на підключення:
                |%1'"),
			РезультатОтправки.СообщениеОбОшибке);
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось проверить состояние запроса на подключение.
                ...
                |Причина:
                |%2'
                |;uk='Не вдалося перевірити стан запиту на підключення.
                |При перевірці запиту на підключення тестових періодів сервіс повернув помилку: %1.
                |Причина:
                |%2'"),
			URLОперации,
			РезультатОтправки.ИнформацияОбОшибке);
...
Процедура ОбработатьРезультатПроверкиПодключения(
	...
	ПодробноеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Ошибка при проверке статуса запроса на подключение тестового периода:
            ...
            |;uk='Помилка при перевірці статусу запиту на підключення тестового періоду:
            |Код помилки: %1
            |Інформація про помилку: %2'"),
	...
	Если РезультатПодключения.КодОшибки = "HAS_ACTIVE_OPTION_PACKAGES_BY_APPLICATION"
		...
		Если Подключен Тогда
			...
			// из другого сеанса, либо напрямую на Портале ИТС.
			РезультатПодключения.Подключен          = Истина;
...
Функция ТестовыйПериодПодключен(ИдентификаторТестовогоПериода, ПараметрыПодключения)
	...
	ТекстЖурналРегистрации = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
        ...
        |%1'
        |;uk='Перевірка активності тестового періоду:
        |%1'"),
		ИдентификаторТестовогоПериода);
	...
	Если Не ПустаяСтрока(РезультатОтправки.КодОшибки) Тогда
		...
		ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось проверить активность тестового периода.
                ...
                |Причина:
                |%2'
                |;uk='Не вдалося перевірити активність тестового періоду.
                |При перевірці активності тестового періоду сервіс повернув помилку: %1.
                |Причина:
                |%2'"),
			URLОперации,
			РезультатОтправки.ИнформацияОбОшибке);
...
Функция ДанныеАутентификации(URLОперации)
		...
		Иначе
			...
			Результат.ИнформацияОбОшибке =
				НСтр("ru='Ошибка аутентификации в сервисе.
                    |Подробнее см. в журнале регистрации.'
                    |;uk='Помилка аутентифікації в сервісі.
                    |Докладніше див. у журналі реєстрації.'");
			ПодробнаяИнформацияОбОшибке =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось вызвать операцию %1.
                        ...
                        |%2'
                        |;uk='Не вдалося викликати операцію %1.
                        |Не вдалося виконати аутентифікацію.
                        |%2'"),
					URLОперации,
	...
	Иначе
		...
		Если Результат.ДанныеАутентификации = Неопределено Тогда
			...
			Результат.ИнформацияОбОшибке =
				НСтр("ru='Для использования тестовых периодов, необходимо подключить Интернет-поддержку пользователей.';uk='Для використання тестових періодів, необхідно підключити Інтернет-підтримку користувачів.'");
			ЗаписатьИнформациюВЖурналРегистрации(
				Результат.ИнформацияОбОшибке,
				Истина);
		КонецЕсли;
...
Функция ХостСервисаПодключенияТестовыхПериодов(Домен)
	...
	// Пока этот сервис не используется, при необходимости указать корректный рабочий хост
	Возврат "unknown-host";
...
Функция ИмяСобытияЖурналаРегистрации()
	...
	Возврат НСтр("ru='Подключение тестовых периодов.';uk='Підключення тестових періодів.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Иначе
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось прочитать значение свойства %1.
                ...
                |;uk='Не вдалося прочитати значеняя властивості %1.
                |Некоректний тип значення властивості (%2).'"),
			ИмяСвойства,
