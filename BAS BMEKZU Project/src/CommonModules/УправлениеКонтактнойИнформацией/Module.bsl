...
Процедура ДобавитьКонтактнуюИнформацию(СсылкаИлиОбъект, ЗначениеИлиПредставление, ВидКонтактнойИнформации, Дата = Неопределено, Замещать = Истина) Экспорт
	...
	Если МетаданныеОбъекта = Неопределено
		...
		ВызватьИсключение НСтр("ru='Добавление контактной информации невозможно, у объекта нет таблицы с контактной информацией.';uk='Додавання контактної інформації неможливо, у об''єкта немає таблиці з контактною інформацією.'");;
	КонецЕсли;
...
//                    Для строки XML соответствующего XDTO пакету КонтактнаяИнформация или Адрес.
...
//
// Параметры:
//    КонтактнаяИнформация - Строка - Контактная информация в виде XML, соответствующего структуре
...
Функция ДоменАдресаКонтактнойИнформации(Знач КонтактнаяИнформация) Экспорт
	...
	Если УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВXML(КонтактнаяИнформация) Тогда
		КонтактнаяИнформацияСтруктурой = УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияВСтруктуруJSON(КонтактнаяИнформация,,, Ложь);
	ИначеЕсли УправлениеКонтактнойИнформациейКлиентСервер.ЭтоКонтактнаяИнформацияВJSON(КонтактнаяИнформация) Тогда
		КонтактнаяИнформацияСтруктурой = УправлениеКонтактнойИнформациейСлужебный.СтрокаJSONВСтруктуру(КонтактнаяИнформация);
	КонецЕсли;
...
Процедура СоздатьВТКонтактнаяИнформация(МенеджерВременныхТаблиц, МассивОбъектов, ТипыКонтактнойИнформации = Неопределено, ВидыКонтактнойИнформации = Неопределено, Дата = Неопределено) Экспорт
	...
	Для каждого ОбъектСКонтактнойИнформацией Из ОбъектыСГруппировкойПоТипам Цикл
		...
		Если МетаданныеОбъекта.ТабличныеЧасти.Найти("КонтактнаяИнформация") = Неопределено Тогда
			ВызватьИсключение  МетаданныеОбъекта.Имя + " " + НСтр("ru='не содержит контактную информацию.';uk='не містить контактну інформацію.'");
		КонецЕсли;
		...
		Если МетаданныеОбъекта.ТабличныеЧасти.КонтактнаяИнформация.Реквизиты.Найти("ДействуетС") <> Неопределено Тогда
			Если ТипЗнч(Дата) <> Тип("Дата") Тогда
				ВызватьИсключение НСтр("ru='Для получения контактной информации, хранящей историю изменений,
                    ...
                    |;uk='Для отримання контактної інформації, що зберігає історію змін,
                    |потрібно зазначати дату, з якої діє запис контактної інформації.'");
			КонецЕсли;
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Форма, Объект, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из Форма.КонтактнаяИнформацияОписаниеДополнительныхРеквизитов Цикл
		...
		Иначе
			...
			Если СвойствоВидаИнформации.РедактированиеТолькоВДиалоге И Не УправлениеКонтактнойИнформациейКлиентСервер.КонтактнаяИнформацияЗаполнена(Форма[ИмяРеквизита]) Тогда
				Представление = "";
			Иначе
...
//                * ТолькоНациональныйАдрес        - Булево - Истина, если возможен ввод только национальных адресов.
...
// Устанавливает свойства группы вида контактной информации.
Процедура УстановитьСвойстваГруппыВидаКонтактнойИнформации(Параметры) Экспорт
...
//        ** Регион           - Строка - текстовое представление региона РФ (только для национальных адресов).
//        ** КодРегиона       - Строка - код региона РФ (только для национальных адресов).
...
//        ** Улица            - Строка - текстовое представление улицы (только для национальных адресов).
...
Процедура ДобавитьКнопкуДополнительногоПоляКонтактнойИнформации(Знач Форма, Знач ИмяЭлементаДляРазмещения)
	...
	Описание = НСтр("ru='Добавить дополнительное поле контактной информации';uk='Додати додаткове поле контактної інформації'");
...
Функция СоздатьПолеВвода(Форма, Родитель, ИнформацияОВидеКИ, ИмяРеквизита, ИмяЭлементаДляРазмещения,ЭтоНовыйВидКИ = Ложь, ОбязательноеЗаполнение = Ложь)
	...
	Если ИнформацияОВидеКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Если Не ИнформацияОВидеКИ.ПометкаУдаления Тогда
			...
			ГруппаПодменюАдреса.Заголовок = НСтр("ru='Заполнить';uk='Заповнити'");
		КонецЕсли;
	...
	Для Каждого СтрокаКИ Из ВсеСтроки Цикл
		...
		Для Каждого Адрес Из НайденныеСтроки Цикл
			...
			Если Команда = Неопределено Тогда
				Команда = Форма.Команды.Добавить(ИмяКоманды);
				Команда.Подсказка = НСтр("ru='Скопировать адрес';uk='Скопіювати адресу'");
				Команда.Действие = "Подключаемый_КонтактнаяИнформацияВыполнитьКоманду";
				Команда.ИзменяетСохраняемыеДанные = Истина;
...
Функция ПолучитьТаблицуПоКодуЯзыкаИБ(Данные)
	...
	КонецЕсли; 
	ТребуетсяУкраинскийСиноним = (КодЯзыка = "uk");
