...
Процедура УстановитьУсловноеОформлениеПоляДата(ЭтотОбъект, 
	...
	ЭлементОформления.Представление = НСтр("ru='Представление даты: ""10.06.2012""';uk='Представлення дати: ""10.06.2012""'");
	...
	ЭлементОформления = УсловноеОформление.Элементы.Добавить();
	ЭлементОформления.Использование = Истина;
	ЭлементОформления.Представление = НСтр("ru='Представление даты сегодня: ""09:46""';uk='Представлення дати сьогодні: ""09:46""'");
		...
		Если ОтключитьАвторегистрацию.Количество() <> 0 Тогда
			...
			ОписаниеИсключения3 = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В составе плана обмена %1 не должно быть объектов с установленным признаком авторегистрации.
                    ...
                    |;uk='У складі плану обміну %1 не повинно бути об''єктів із установленою ознакою авторегістрації.
...
Процедура ПотребоватьПерезапускСеансаПоПричинеДинамическогоОбновленияВерсииПрограммы() Экспорт
	...
	ТекстОшибки = НСтр("ru='Версия программы обновлена, требуется перезапустить сеанс.';uk='Версія програми оновлена, потрібно перезапустити сеанс.'");
...
Процедура ВосстановитьПредопределенныеЭлементы() Экспорт
	...
	Если ПланыОбмена.ГлавныйУзел() <> Неопределено Тогда
		ВызватьИсключение НСтр("ru='Восстановление предопределенных элементов следует выполнять только в главном узле РИБ.
            ...
            |;uk='Відновлення напередвизначених елементів слід виконувати тільки в головному вузлі РІБ.
            |Потім виконати синхронізацію з підлеглими вузлами.'");
	КонецЕсли;
		...
		ИначеЕсли СтарыеСвойства.ИмяПредопределенныхДанных = ""
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недопустимо связывать с именем предопределенного элемент, помеченный на удаление:
                           |""%1"".'
                           |;uk='Неприпустимо пов''язувати з ім''ям напередвизначеного елемента, який відмічений для вилучення:
                           |""%1"".'"),
				Строка(Источник.Ссылка));
		КонецЕсли;
	КонецЕсли;
		...
		Если Счет.ВидыСубконто.Количество() > Индекс Тогда
			Если Счет.ВидыСубконто[Индекс].ВидСубконто <> Строка.ВидСубконто Тогда
				ЗаписьЖурналаРегистрации(
					НСтр("ru='Обмен данными.Отключение связи с главным узлом';uk='Обмін даними.Відключення зв''язку з головним вузлом'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,
					Счет.Метаданные(),
					Счет,
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='У счета ""%1"" субконто №%2 ""%3"" не совпадает с предопределенным субконто ""%4"".';uk='У рахунку ""%1"" субконто №%2 ""%3"" не збігається з напередвизначеним субконто ""%4"".'"),
						Строка(Счет),
						Индекс + 1,
...
Процедура ПередЗапускомПрограммы()
	...
	Если Метаданные.ВариантВстроенногоЯзыка <> Метаданные.СвойстваОбъектов.ВариантВстроенногоЯзыка.Русский Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Вариант встроенного языка конфигурации ""%1"" не поддерживается.
                       ...
                       |;uk='Варіант вбудованої мови конфігурації ""%1"" не підтримується.
	...
	Если ЗначениеЗаполнено(НеПоддерживаемаяВерсияПлатформы) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Режим совместимости конфигурации с версии %1 не поддерживается.
                       ...
                       |;uk='Режим сумісності конфігурації з версії %1 не підтримується.
                       |Для запуску встановіть в конфігурації режим сумісності ""Не використовувати"" при розробці версії на %2
                       |(або ""Версія %2"" при розробці на старіших версіях).'"),
	...
	Если ЗначениеЗаполнено(ПрофильИнформационнойБазы) Тогда
		...
		Если БезопасныйРежим() <> ПрофильИнформационнойБазы Тогда
			...
			Если Не ДоступенПривилегированныйРежим Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Невозможно установить параметры сеанса по причине: профиль безопасности %1 отсутствует в кластере серверов платформы, или для него запрещено использование в качестве профиля безопасности безопасного режима.
                        |
                        ...
                        |;uk='Неможливо встановити параметри сеансу через: профіль безпеки %1 відсутній в кластері серверів платформи, або для нього заборонено використання в якості профілю безпеки безпечного режиму.
                        |
		...
		Если Не ДоступенПривилегированныйРежим Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Невозможно установить параметры сеанса по причине: профиль безопасности %1 не содержит разрешения на установку привилегированного режима. Возможно, он был отредактирован через консоль кластера.
                    |
                    |Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).'
                    |;uk='Неможливо встановити параметри сеансу через: профіль безпеки %1 не містить дозволу на встановлення привілейованого режиму. Можливо, він був відредагований через консоль кластера.
                    |
                    |Для відновлення працездатності програми потрібно відключити використання профілю безпеки через консоль кластера і заново налаштувати профілі безпеки за допомогою інтерфейсу конфігурації (відповідні команди знаходяться в розділі настройок програми).'"),
				ПрофильИнформационнойБазы);
