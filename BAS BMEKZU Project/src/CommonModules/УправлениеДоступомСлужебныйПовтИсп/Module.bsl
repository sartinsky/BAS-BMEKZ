...
Функция РазрешенныйКлючДоступа() Экспорт
	...
	Если Не ЗначениеЗаполнено(СсылкаВБазеДанных) Тогда
		...
		РазрешенныйКлюч.Наименование = НСтр("ru='Разрешенный ключ доступа';uk='Дозволений ключ доступу'");
...
Функция РазрешенныйПустойНаборГруппДоступа() Экспорт
	...
	Если Не ЗначениеЗаполнено(СсылкаВБазеДанных) Тогда
		...
		РазрешенныйПустойНабор.Наименование = НСтр("ru='Разрешенный пустой набор групп доступа';uk='Дозволений порожній набір груп доступу'");
...
Функция РазмерностьКлючаДоступа() Экспорт
	...
	Если КоличествоПодобныхЭлементовВКоллекции(МетаданныеКлюча.Реквизиты, "Значение") <> 5 Тогда
		ВызватьИсключение
			НСтр("ru='В справочнике КлючиДоступа должно быть 5 реквизитов Значение* и не более.';uk='У довіднику КлючиДоступа повинно бути 5 реквізитів Значення* і не більше.'");
	КонецЕсли;
	...
	Если МетаданныеКлюча.ТабличныеЧасти.Найти("Шапка") = Неопределено
	 Или КоличествоПодобныхЭлементовВКоллекции(МетаданныеКлюча.ТабличныеЧасти.Шапка.Реквизиты, "Значение", 6) <> 5 Тогда
		ВызватьИсключение
			НСтр("ru='В справочнике КлючиДоступа должна быть табличная часть Шапка с 5 реквизитами Значение* и не более.';uk='У довіднику КлючиДоступа повинна бути таблична частина Шапка з 5 реквізитами Значення* і не більше.'");
	КонецЕсли;
	...
	Если КоличествоТабличныхЧастей < 1 Или КоличествоТабличныхЧастей > 12 Тогда
		ВызватьИсключение
			НСтр("ru='В справочнике КлючиДоступа должно быть от 1 до 12 табличных частей ТабличнаяЧасть*.';uk='У довіднику КлючиДоступа повинно бути від 1 до 12 табличних частин ТабличнаяЧасть*.'");
	КонецЕсли;
	...
	Если КоличествоРеквизитовТабличнойЧасти = 0 Тогда
		ВызватьИсключение
			НСтр("ru='В справочнике КлючиДоступа в табличных частях ТабличнаяЧасть*
                       ...
                       |;uk='У довіднику КлючиДоступа в табличних частинах ТабличнаяЧасть*
                       |припустимо тільки однакову кількість реквізитів Значення* і не більше 15.'");
	КонецЕсли;
		...
		Если Измерение.Имя = "Регистр" Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В отдельном регистре сведений %1
                           ...
                           |;uk='В окремому регістрі відомостей %1
                           |не повинно бути виміру %2.'"), ИмяРегистра, "Регистр");
		КонецЕсли;
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось получить имя предопределенного идентификатора объекта метаданных
                       ...
                       |""%1"".'
                       |;uk='Не вдалося отримати ім''я напередвизначеного ідентифікатора об''єкта метаданих
                       |так як не існує зазначений об''єкт метаданих:
                       |""%1"".'"),
