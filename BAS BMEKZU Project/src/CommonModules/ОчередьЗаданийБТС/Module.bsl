		...
		Пока Выборка.Следующий() Цикл 
			Попытка
				...
				Если ЗначениеЗаполнено(Выборка.Шаблон) И Выборка.СсылкаШаблона = Неопределено Тогда
					...
					ШаблонСообщения = НСтр("ru='На найден шаблон задания очереди с идентификатором %1';uk='На знайдено шаблон завдання черги з ідентифікатором %1'");
...
//
// Параметры:
//   Задание - СправочникОбъект.<ОчередьЗаданий>.
...
Процедура ЗапланироватьЗадание(Задание, Расписание, КоличествоПовторовПриАварийномЗавершении, ИнтервалПовтораПриАварийномЗавершении, ЧасовойПояс)
	...
	Если ЭтоЗаданиеОбласти Тогда
		...
		Исключение
			...
			ШаблонСообщения = НСтр("ru='Задание очереди с именем метода %1 в области %2 не запланировано по причине:
                |%3'
                |;uk='Задання черги з ім''ям методу %1 в області %2 не заплановано з причини:
                |%3'");
			...
			ЗаписьЖурналаРегистрации(
			    НСтр("ru='Очередь заданий.Планирование';uk='Черга завдань.Планування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			    УровеньЖурналаРегистрации.Ошибка,,,
				ТекстСообщения);
	...
	Попытка
		...
		ИначеЕсли Задание.СостояниеЗадания = Перечисления.СостоянияЗаданий.Выполняется Тогда
			...
			ЗаписьКонтроляВыполнения(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Задание.Ссылка, 
		...
		ИначеЕсли Расписание = Неопределено Или 
			...
			Если ЭтоЗаданиеОбласти Тогда
				Если Расписание = Неопределено И ЗначениеЗаполнено(Задание.Шаблон) Тогда // Задание по шаблону без расписания.
					...
					ШаблонСообщения = НСтр("ru='Для шаблон заданий очереди  %1 не найдено расписание';uk='Для шаблону завдань черги  %1 не знайдено розклад'");
	...
	Исключение
		...
		ШаблонСообщения = НСтр("ru='Задание очереди с именем метода %1 не запланировано по причине:
            |%2'
            |;uk='Задання черги з ім''ям методу %1 не заплановано з причини:
            |%2'");
	...
	Исключение
		...
		ШаблонКомментария =
			НСтр("ru='Не удалось выполнить обработчик %1 по причине:
                       |%2'
                       |;uk='Не вдалося виконати обробник %1 з причини:
                       |%2'");
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Очередь регламентных заданий.Завершено с ошибками';uk='Черга регламентних завдань.Завершено з помилками'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
...
Процедура ВыполнитьМетодКонфигурации(ИмяМетода, Параметры = Неопределено)
	...
	Попытка
		...
		Если Не(РазделительУстановлен) И РаботаВМоделиСервиса.ИспользованиеРазделителяСеанса() Тогда
			...
			ТекстСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='По завершении выполнения обработчика %1 не было выключено разделение сеанса.';uk='По завершенні виконання обробника %1 не був вимкнений поділ сеансу.'"),
				ИмяМетода);
...
Процедура ВыполнитьОбработчикЗадания(Шаблон, Псевдоним, Параметры)
	...
	Если ИмяМетода = Неопределено Тогда
		ШаблонСообщения = НСтр("ru='Метод %1 не разрешен к вызову через очередь заданий.';uk='Метод %1 не дозволено до виклику через чергу завдань.'");
