...
Функция НовыйНастройкиАвтоматическогоОбновления() Экспорт
	...
	Результат.Вставить("РежимАвтоматическойПроверкиНаличияОбновленийПрограммы", 0);
...
Функция URLОперацииСервисаОбновлений(Операция, Домен) Экспорт
...
Функция НовыйИнформацияОДоступномОбновленииКонфигурации() Экспорт
...
Функция НовыйИнформацияОДоступномОбновленииПлатформы() Экспорт
...
Процедура ЗаполнитьИнформациюОбОбновленииИзInfoResonseИзJSON(Результат, ТелоJSON) Экспорт
...
Процедура ЗаполнитьИнформациюОФайлахОбновленияИзUpdateResonseJSON(Результат, ТелоJSON) Экспорт
...
Функция НовыйКонтекстПолученияИУстановкиОбновлений(Параметры) Экспорт
	...
	Результат.Вставить("ТекущееДействие",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Подготовка к получению обновления...';uk='Підготовка до отримання оновлення...'"),
			КоличествоФайлов));
...
Процедура СоздатьКаталогиДляПолученияОбновления(Обновление, Контекст) Экспорт
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при создании каталога для сохранения дистрибутива конфигурации (%1).';uk='Помилка при створенні каталогу для збереження дистрибутива конфігурації (%1).'"),
		...
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось создать каталог %1 для сохранения дистрибутива конфигурации. %2';uk='Не вдалося створити каталог %1 для збереження дистрибутива конфігурації. %2'"),
			ВременныйКаталогОбновленийКонфигурации,
	...
	Исключение
		...
		СообщениеЖурнала =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при создании каталога (%1).';uk='Помилка при створенні каталогу (%1).'"),
		...
		Контекст.ИнформацияОбОшибке = СообщениеЖурнала;
		Контекст.Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось создать каталог %1. %2';uk='Не вдалося створити каталог %1. %2'"),
			КаталогИндексаФайлов,
			КраткоеПредставлениеОшибки(ИнфОшибка));
		Возврат;
	...
	Если Обновление.ФорматФайлаОбновления <> "zip" Тогда
		...
		Исключение
			...
			СообщениеЖурнала =
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при создании каталога дистрибутива конфигурации (%1).';uk='Помилка при створенні каталогу дистрибутива конфігурації (%1).'"),
	...
	Если Обновление.ФорматФайлаОбновления = "zip" Тогда
		...
		Если Не ФайлСуществует(Обновление.ПолноеИмяCFUФайлаВКаталогеДистрибутивов, Ложь) Тогда
			...
			Контекст.ИнформацияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Некорректный файл дистрибутива %1. Отсутствует файл обновления конфигурации %2.';uk='Некоректний файл дистрибутива %1. Відсутній файл оновлення конфігурації %2.'"),
			...
			Контекст.Сообщение = НСтр("ru='Дистрибутив не содержит файл обновления конфигурации.';uk='Дистрибутив не містить файл оновлення конфігурації.'");
			Возврат;
