...
Процедура ВключитьВерсионированиеОбъекта(ИмяОбъекта, Знач ВариантВерсионирования = Неопределено) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Неизвестный вариант версионирования: ""%1""';uk='Невідомий варіант версионування: ""%1""'"), ВариантВерсионирования);
...
Функция ПолучитьВерсионируемыеОбъекты()
	...
	Для Каждого Тип Из Метаданные.ОбщиеКоманды.ИНАГРО_ИсторияИзменений.ТипПараметраКоманды.Типы() Цикл // ИНАГРО
		Результат.Добавить(ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Тип));
	КонецЦикла;
...
Функция ВывестиРеквизитыПоРазобранномуОбъекту(ТЧОтчета, ВерсияОбъекта, СсылкаНаОбъект)
	...
	ВывестиТекстВОтчет(ТЧОтчета, Секция, "R1C2", НСтр("ru='Реквизиты';uk='Реквізити'"), 11, Истина);
