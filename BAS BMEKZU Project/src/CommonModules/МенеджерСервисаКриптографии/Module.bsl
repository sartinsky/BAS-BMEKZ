...
Функция ВызватьHTTPМетод(HTTPМетод, URL, ПараметрыЗапроса, СоответствиеПолейОтвета, Заголовки = Неопределено, ПараметрыПреобразования = Неопределено)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронная подпись в модели сервиса.Менеджер сервиса криптографии';uk='Електронний підпис моделі сервісу.Менеджер сервісу криптографії'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
			УровеньЖурналаРегистрации.Ошибка,,, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			
		Результат.Вставить("Выполнено", Ложь);
		Результат.Вставить("КодОшибки", "НеУдалосьУстановитьСоединение");
		Результат.Вставить("ОписаниеОшибки", НСтр("ru='Не удалось установить соединение с сервером.';uk='Не вдалося встановити з''єднання з сервером.'"));		
	КонецПопытки;
	...
	Иначе
		...
		Результат.Вставить("ОписаниеОшибки", НСтр("ru='Сервис временно недоступен. Обратитесь в службу поддержки или повторите попытку позже.';uk='Сервіс тимчасово недоступний. Зверніться в службу підтримки чи повторіть спробу пізніше.'"));
	КонецЕсли;
