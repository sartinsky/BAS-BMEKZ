...
// Возвращает идентификатор поставщика услуг "Портал ИТС"
...
// Устарела. Следует использовать одноименную функцию общего модуля
// ИнтернетПоддержкаПользователей или ИнтернетПоддержкаПользователейКлиент в
...
Функция ДоступноПодключениеИнтернетПоддержки() Экспорт
	...
	#Иначе
	Возврат ИнтернетПоддержкаПользователейКлиент.ДоступноПодключениеИнтернетПоддержки();
	#КонецЕсли
...
//			к серверам ИПП в доменной зоне bas-soft.eu.
//
Функция НастройкиСоединенияССерверами() Экспорт
	
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	Возврат ИнтернетПоддержкаПользователей.НастройкиСоединенияССерверами();
	#Иначе
...
Процедура ПроверитьURL(URL) Экспорт
	...
	Если ДоменХоста <> ".bas-soft.eu" И ДоменХоста <> ".1c.eu" Тогда	
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неизвестный хост (%1)';uk='Невідомий хост (%1)'"),
			СтруктураURI.Хост);
	...
	Пока КоличествоПеренаправлений < МаксКолвоПеренаправлений Цикл
		...
		Исключение
			...
			ПодробноеОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось загрузить содержимое (%1). %2';uk='Не вдалося завантажити вміст (%1). %2'"),
				URL,
			...
			Попытка
				...
				ОписаниеРезультатаДиагностики = НСтр("ru='Результаты диагностики соединения:';uk='Результати діагностики з''єднання:'")
			...
			Исключение
				...
				ОписаниеРезультатаДиагностики = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось выполнить диагностику соединения. %1';uk='Не вдалося виконати діагностику з''єднання. %1'"),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если Ответ.КодСостояния < 200 Или Ответ.КодСостояния >= 300 Тогда
		...
		ИначеЕсли Ответ.КодСостояния < 200
			...
			УстановитьОписаниеОшибки(
				...
				"ServerError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Некорректный ответ сервера (%1).';uk='Некоректна відповідь сервера (%1).'"),
					Ответ.КодСостояния),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ИначеЕсли Ответ.КодСостояния >= 400 И Ответ.КодСостояния < 500 Тогда
			...
			УстановитьОписаниеОшибки(
				...
				"ClientError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка (%1) при выполнении запроса к ресурсу.';uk='Помилка (%1) при виконанні запиту до ресурсу.'"),
		...
		Иначе
			...
			УстановитьОписаниеОшибки(
				...
				"ServerError",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Код ошибки: %1.';uk='Код помилки: %1.'"),
					Строка(Ответ.КодСостояния)),
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ПользовательскоеПредставлениеИнтервалаВерсий(ВерсияОТ, ВерсияДО) Экспорт
	...
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО <> "99.99.999.9999") Тогда
		...
		Иначе // Интервал версий
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Интервал %1...%2';uk='Інтервал %1...%2'"),
	...
	ИначеЕсли (ВерсияОТ <> "00.00.000.0000") И (ВерсияДО = "99.99.999.9999") Тогда // От версии и выше
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='От %1 и выше';uk='Від %1 та вище'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияОТ, Истина));
	ИначеЕсли (ВерсияОТ = "00.00.000.0000") И (ВерсияДО <> "99.99.999.9999") Тогда // До версии включительно
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='До версии %1 включительно';uk='До версії %1 включно'"),
			ПользовательскоеПредставлениеНомераВерсии(ВерсияДО, Истина));
...
Функция ПредставлениеДняНедели(НомерДняНедели) Экспорт
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Неопределено (%1)';uk='Невизначено (%1)'"),
		НомерДняНедели);

	Если НомерДняНедели = 1 Тогда
		Результат = НСтр("ru='Понедельник';uk='Понеділок'");
	ИначеЕсли НомерДняНедели = 2 Тогда
		Результат = НСтр("ru='Вторник';uk='Вівторок'");
	ИначеЕсли НомерДняНедели = 3 Тогда
		Результат = НСтр("ru='Среда';uk='Середа'");
	ИначеЕсли НомерДняНедели = 4 Тогда
		Результат = НСтр("ru='Четверг';uk='Четвер'");
	ИначеЕсли НомерДняНедели = 5 Тогда
		Результат = НСтр("ru='Пятница';uk='П''ятниця'");
	ИначеЕсли НомерДняНедели = 6 Тогда
		Результат = НСтр("ru='Суббота';uk='Субота'");
	ИначеЕсли НомерДняНедели = 7 Тогда
		Результат = НСтр("ru='Воскресенье';uk='Неділя'");
	КонецЕсли;
	...
	Если ТипЗнч(ОбъектХДТО) = Тип("ОбъектXDTO") Тогда
		...
		Исключение
			...
			ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
				...
				НСтр("ru='БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО';uk='БазовыйФункционал. Налагодження. ПредставлениеОбъектаХДТО'"), // ИдентификаторШага.
...
Функция СодержимоеТекстовогоФайла(ПолноеИмяФайла, КоличествоСтрок = 10) Экспорт
		...
		Исключение
			Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При чтении файла с именем %1 произошла ошибка:
                    |%2'
                    |;uk='При читанні файлу з ім''ям %1 сталася помилка:
                    |%2'"),
				ПолноеИмяФайла,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
	...
	#Иначе
		...
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При запуске в веб-клиенте невозможно показать содержимое файла [%1]';uk='При запуску у веб-клієнті неможливо показати вміст файлу [%1]'"),
			ПолноеИмяФайла);
...
Функция ЗавершитьРегистрациюРезультатаВыполненияОперации(КонтекстВыполнения, КодРезультата, ОписаниеРезультата, ВложенныйКонтекстВыполнения = Неопределено) Экспорт
	...
	Если ТипЗнч(ТекущийШагВыполнения.ВремяНачала) <> Тип("Число") Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не зарегистрировано начало шага выполнения %1, %2.';uk='Не зареєстрований початок кроку виконання %1, %2.'"),
...
Процедура ОтобразитьСостояниеПодключенияИПП(Форма) Экспорт
	...
	Если Форма.БИПДанныеАутентификации = Неопределено Тогда
		Элементы.ДекорацияЛогинИПП.Заголовок = НСтр("ru='Подключение к Интернет-поддержке не выполнено.';uk='Підключення до Інтернет-підтримки не виконано.'");
...
#Если Сервер Тогда	
	Сервер = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("RDI",
			"RDI_СерверАвторизации",
			"");
...
Функция ХостСервисовТехническойПоддержки(Домен) Экспорт
	...
	Возврат "portal-support.bas-soft.eu";
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьОшибкуВЖурналРегистрации(
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось отправить данные сообщения в службу технической поддержки.
                    ...
                    |Не вдалося перевірити доступність сервісу відправки повідомлень.
                    |%1'"),
				РезультатПроверки.ИнформацияОбОшибке));
		...
		Результат.СообщениеОбОшибке =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось отправить данные сообщения в службу технической поддержки.
                    ...
                    |%1'
                    |;uk='Не вдалося надіслати дані повідомлення в службу технічної підтримки.
                    |Не вдалося перевірити доступність сервісу.
                    |%1'"),
				РезультатПроверки.СообщениеОбОшибке);
...
Процедура УстановитьОписаниеОшибки(Результат, КодОшибки, СообщениеОбОшибке, ИнформацияОбОшибке, Перенаправления)
	...
	Если КодОшибки = "ConnectError" Тогда
		ДопСообщение = НСтр("ru='Ошибка при подключении к серверу.';uk='Помилка при підключенні до сервера.'");

	ИначеЕсли КодОшибки = "ServerError" Тогда
		ДопСообщение = НСтр("ru='На сервере возникла внутренняя ошибка при обработке запроса.';uk='На сервері виникла внутрішня помилка при обробці запиту.'");

	ИначеЕсли КодОшибки = "ClientError" Тогда
		ДопСообщение = НСтр("ru='Некорректный запрос.';uk='Некоректний запит.'");
