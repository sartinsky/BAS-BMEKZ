	...
	Если ПустаяСтрока(КаталогСкрипта) Тогда
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			...
			НСтр("ru='Обновление выполнено с очень старой версии программы. 
                       ...
                       |;uk='Оновлення виконано з дуже старої версії програми. 
                       |Дані журналу оновлення не були завантажені, але сам журнал 
                       |можна знайти в папці тимчасових файлів %temp% - 
                       |у папці виду 1Cv8Update.<хххххххх>(цифри).'"),
			, 
			Истина);
	...
	Иначе 
		ЖурналРегистрацииКлиент.ДобавитьСообщениеДляЖурналаРегистрации(СобытиеЖурналаРегистрации(),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Папка с журналом обновления расположена %1';uk='Папка з журналом оновлення розташована %1'"), КаталогСкрипта),
			,
			Истина);
...
#ИначеЕсли МобильныйКлиент Тогда
	...
	ВозможенЗапускОбновления = Ложь;
	ТекстСообщения = НСтр("ru='Обновление программы недоступно в мобильном клиенте.';uk='Оновлення програми недоступне в мобільному клієнті.'");
	
#Иначе
	
	Если Не ОбщегоНазначенияКлиентСервер.ЭтоWindowsКлиент() Тогда
		ВозможенЗапускОбновления = Ложь;
		ТекстСообщения = НСтр("ru='Обновление программы доступно только в клиенте под управлением ОС Windows.';uk='Оновлення програми доступно тільки в клієнті під управлінням ОС Windows.'");
	КонецЕсли;
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	Если ПараметрыПриложения["СтандартныеПодсистемы.ПредлагатьОбновлениеИнформационнойБазыПриЗавершенииСеанса"] = Истина Тогда
		...
		ПараметрыПредупреждения.ТекстПредупреждения  = НСтр("ru='Запланирована установка обновления';uk='Заплановано встановлення оновлень'");
...
Функция ВозможнаУстановкаОбновления(Параметры, ПараметрыАдминистрирования)
	...
	ИначеЕсли Параметры.РежимОбновления = 2 Тогда
		...
		Если Не ПоддерживаетсяПланировщикЗаданий() Тогда
			ПоказатьПредупреждение(, НСтр("ru='Планировщик заданий поддерживается только начиная с операционной системы версии 6.0 Vista.';uk='Планувальник завдань підтримується тільки починаючи з операційної системи версії 6.0 Vista.'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЕсли;
...
Функция ДатаОбновленияУказанаВерно(Параметры)
	...
	Если Параметры.ДатаВремяОбновления < ТекущаяДата Тогда
		ТекстСообщения = НСтр("ru='Обновление конфигурации может быть запланировано только на будущую дату и время.';uk='Оновлення конфігурації може бути заплановано тільки на майбутню дату й час.'");
	ИначеЕсли Параметры.ДатаВремяОбновления > ДобавитьМесяц(ТекущаяДата, 1) Тогда
		ТекстСообщения = НСтр("ru='Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';uk='Оновлення конфігурації може бути заплановано не пізніше, ніж через місяць щодо поточної дати.'");
	КонецЕсли;
...
Функция СформироватьФайлыСкриптаОбновления(Знач ИнтерактивныйРежим, Параметры, ПараметрыАдминистрирования)
	...
	Если ЭтоОтложенноеОбновление Тогда 
		...
		ОписаниеЗадачи = НСтр("ru='Обновление конфигурации системы';uk='Оновлення конфігурації системи'");
