...
Функция ПредставлениеКомпоненты(Идентификатор, Версия) Экспорт
	...
	Если ЗначениеЗаполнено(Версия) Тогда 
		ПредставлениеКомпоненты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (версии %2)';uk='%1 (версії %2)'"), 
			Идентификатор, 
			Версия);
	Иначе 
		ПредставлениеКомпоненты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 (последней версии)';uk='%1 (останньої версії)'"), 
			Идентификатор, 
			Версия);
	КонецЕсли;
...
Функция ИнформацияОКомпонентеИзФайла(АдресХранилищаФайла, ВыполнятьРазборИнфоФайла = Истина, 
	...
	Исключение
		ТекстОшибки = НСтр("ru='В файле отсутствует информация о компоненте.';uk='У файлі відсутня інформація про компоненту.'");
	...
	Для Каждого ЭлементАрхива Из ЧтениеАрхива.Элементы Цикл
		...
		Если ЭлементАрхива.Зашифрован Тогда
			...
			ТекстОшибки = НСтр("ru='ZIP-архив не должен быть зашифрован.';uk='ZIP-архів не повинен бути зашифрований.'");
		...
		Исключение
			...
			Результат.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при разборе файла %1
                           |%2'
                           |;uk='Помилка при розборі файлу %1
                           |%2'"),
	...
	Если Не НайденМанифест Тогда 
		ТекстОшибки = НСтр("ru='В архиве компоненты отсутствует обязательный файл MANIFEST.XML.';uk='В архіві компоненти відсутній обов''язковий файл MANIFEST.XML.'");
	...
	Если Не ДоступнаЗагрузкаСПортала() Тогда
		ВызватьИсключение 
			НСтр("ru='Обновление внешних компонент не доступно.
                       ...
                       |;uk='Оновлення зовнішніх компонент не доступно.
                       |Потрібна підсистема оновлення зовнішніх компонент бібліотеки інтернет підтримки.'");
	КонецЕсли;
...
Процедура НоваяКомпонентаСПортала(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	Если РезультатОперации.ДанныеВнешнихКомпонент.Количество() = 0 Тогда
		ТекстИсключения = НСтр("ru='На Портале ИТС внешняя компонента не обнаружена.';uk='На Порталі ІТС зовнішня компонента не виявлена.'");
		ЗаписьЖурналаРегистрации(НСтр("ru='Обновление внешних компонент';uk='Оновлення зовнішніх компонент'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если ЗначениеЗаполнено(КодОшибки) Тогда
		...
		Если КодОшибки = "ОтсутствуетКомпонента" Тогда 
			ИнформацияОбОшибке = НСтр("ru='В сервисе внешних компонент не обнаружена внешняя компонента';uk='У сервісі зовнішніх компонент не виявлена зовнішня компонента'");
		ИначеЕсли КодОшибки = "ОтсутствуетВерсия" Тогда
			ИнформацияОбОшибке = НСтр("ru='В сервисе внешних компонент не обнаружена требуемая версия внешней компоненты';uk='У сервісі зовнішніх компонент не виявлена необхідна версія зовнішньої компоненти'");
		ИначеЕсли КодОшибки = "ФайлНеЗагружен" Тогда 
			ИнформацияОбОшибке = НСтр("ru='При попытке загрузить файл внешней компоненты из сервиса, возникла ошибка';uk='При спробі завантажити файл зовнішньої компоненти з сервісу, сталася помилка'");
		ИначеЕсли КодОшибки = "АктуальнаяВерсия" Тогда
			ИнформацияОбОшибке = 
				НСтр("ru='Скорее всего произошла ошибка сервера при загрузке компоненты.
                           ...
                           |;uk='Швидше за все сталася помилка сервера при завантаженні компоненти.
                           |Отриманий код помилки: АктуальнаяВерсия, проте в ІБ компоненти не виявлено.'");
		КонецЕсли;
		
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При загрузке внешней компоненты %1 возникала ошибка:
                       |%2'
                       |;uk='При завантаженні зовнішньої компоненти %1 виникала помилка:
                       |%2'"),
	...
	Попытка
		...
		Объект.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Загружена с Портала ИТС. %1.';uk='Завантажити з Порталу ІТС. %1.'"),
			ТекущаяДатаСеанса());
