...
Функция ВыполнитьВФоне(Знач ИмяПроцедуры, Знач ПараметрыПроцедуры, Знач ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ЗапуститьНеВФоне И ПараметрыВыполнения.ЗапуститьВФоне Тогда
		ВызватьИсключение НСтр("ru='Параметры ""ВсегдаНеВФоне"" и ""ВсегдаВФоне""
            ...
            |;uk='Параметри ""ВсегдаНеВФоне"" і ""ВсегдаВФоне""
            |не можуть одночасно приймати значення Істина в ДлительныеОперации.ВыполнитьВФоне.'");
	КонецЕсли;
#Если ВнешнееСоединение Тогда
	...
	Если ПараметрыВыполнения.БезРасширений И ИнформационнаяБазаФайловая Тогда
		ВызватьИсключение НСтр("ru='Фоновое задание не может быть запущено с параметром ""БезРасширений""
            ...
            |;uk='Фонове завдання не може бути запущено з параметром ""БезРасширений""
            |у файловій інформаційній базі в ДлительныеОперации.ВыполнитьВФоне.'");
	КонецЕсли;
#КонецЕсли
...
#КонецЕсли
	...
	Если ВыполнитьБезФоновогоЗадания Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка выполнения';uk='Помилка виконання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Знач ИсключениеПриОшибке = Ложь, Знач ВыводитьПрогрессВыполнения = Ложь, 
	...
	Если Задание = Неопределено Тогда
		Пояснение = НСтр("ru='Операция не выполнена из-за аварийного завершения фонового задания.
            ...
            |;uk='Операція не виконана через аварійне завершення фонового завдання.
            |Фонове завдання не знайдено'") + ": " + Строка(ИдентификаторЗадания);
		...
		КонецЕсли;
		Результат.Статус = "Ошибка";
		Результат.КраткоеПредставлениеОшибки = НСтр("ru='Операция не выполнена из-за аварийного завершения фонового задания.';uk='Операція не виконана через аварійне завершення фонового завдання.'");
		Возврат Результат;
	КонецЕсли;
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		...
		Если ПараметрыСеанса.ОтмененныеДлительныеОперации.Найти(ИдентификаторЗадания) = Неопределено Тогда
			...
			Если Задание.ИнформацияОбОшибке <> Неопределено Тогда
				Результат.КраткоеПредставлениеОшибки   = НСтр("ru='Операция отменена администратором.';uk='Операція скасована адміністратором.'");
...
Процедура ВыполнитьПроцедуру(ИмяПроцедуры, ПараметрыПроцедуры)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Неверный формат параметра ИмяПроцедуры (переданное значение: %1)';uk='Неправильний формат параметра ИмяПроцедуры (передане значення: %1)'"), ИмяПроцедуры);
...
Функция ВыполнитьФоновоеЗадание(ПараметрыВыполнения, ИмяМетода, Параметры, Ключ, Наименование)
	...
	Если ТекущийРежимЗапуска() = Неопределено
		...
		Если ПараметрыВыполнения.ОжидатьЗавершение = Неопределено И Сеанс.ИмяПриложения = "BackgroundJob" Тогда
			ВызватьИсключение НСтр("ru='В файловой информационной базе невозможно одновременно выполнять более одного фонового задания';uk='У файловій інформаційній базі неможливо одночасно виконувати більше одного фонового завдання'");
		ИначеЕсли Сеанс.ИмяПриложения = "COMConnection" Тогда
			ВызватьИсключение НСтр("ru='В файловой информационной базе можно запустить фоновое задание только из клиентского приложения';uk='У файловій інформаційній базі можна запустити фонове завдання тільки з клієнтського додатка'");
		КонецЕсли;
