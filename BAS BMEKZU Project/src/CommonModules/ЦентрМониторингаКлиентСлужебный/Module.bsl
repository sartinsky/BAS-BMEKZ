...
Процедура ОповеститьЗапросНаПолучениеДампов() Экспорт
	ПоказатьОповещениеПользователя(НСтр("ru='Отчеты об ошибках';uk='Звіти про помилки'"),
			"e1cib/app/Обработка.НастройкиЦентраМониторинга.Форма.ЗапросНаСборИОтправкуОтчетовОбОшибках",
			НСтр("ru='Предоставьте отчеты о возникающих ошибках';uk='Надайте звіти про виникають помилки'"),
...
Процедура ОповеститьЗапросНаОтправкуДампов() Экспорт
	ПоказатьОповещениеПользователя(НСтр("ru='Отчеты об ошибках';uk='Звіти про помилки'"),
				...
				НСтр("ru='Отправьте отчеты о возникающих ошибках';uk='Відправте звіти про помилки, що виникають '"),
				БиблиотекаКартинок.Предупреждение32,
				СтатусОповещенияПользователя.Важное, "ЗапросНаОтправкуДампов");
КонецПроцедуры

Процедура ОповеститьЗапросКонтактнойИнформации() Экспорт
	ПоказатьОповещениеПользователя(НСтр("ru='Проблемы производительности';uk='Проблеми продуктивності'"),
				Новый ОписаниеОповещения(
						"ПриНажатииОповещенияЗапросКонтактнойИнформации",
						ЭтотОбъект, Истина),
				НСтр("ru='Сообщить о проблемах производительности';uk='Повідомити про проблеми продуктивності'"),
				БиблиотекаКартинок.Предупреждение32,
