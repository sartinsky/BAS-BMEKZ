...
Процедура УстановитьРасширенныйЗаголовокПриложения(ПриЗапуске = Ложь) Экспорт
	...
	Если ПараметрыКлиента.Свойство("РаботаСВнешнимиРесурсамиЗаблокирована")
		...
		ЗаголовокПриложения = НСтр("ru='[КОПИЯ]';uk='[КОПИЯ]'") + " " + ЗаголовокПриложения;
	КонецЕсли;
...
Процедура ПриНачалеРаботыСистемы(Знач ОповещениеЗавершения = Неопределено, НепрерывноеВыполнение = Истина) Экспорт
	...
	Если Не Результат.Статус Тогда
		ПараметрыФормы = Новый Структура("ОшибкаСЛК", Результат.Ошибка);
		ОткрытьФорму("Обработка.ИНАГРО_СЛК.Форма.ПараметрыСЛК", ПараметрыФормы);
		Возврат;
	КонецЕсли;
	// ИНАГРО --
...
#КонецЕсли
	...
	Если Предупреждения.Количество() = 0 Тогда
		Если Не ПараметрКлиента("ЗапрашиватьПодтверждениеПриЗавершенииПрограммы") Тогда
			Возврат;
		КонецЕсли;
		ТекстПредупреждения = НСтр("ru='Завершить работу с программой?';uk='Завершити роботу із програмою?'");
		Отказ = Истина;
	Иначе
		Отказ = Истина;
...
Функция ИнформацияДляПоддержки() Экспорт
	...
	Текст = НСтр("ru='Название программы: [НазваниеПрограммы] 
                       ...
                       |Версия Платформы: [ВерсияПлатформы] [РазрядностьПлатформы]; 
                       ...
                       |;uk='Назва програми: [НазваниеПрограммы] 
                       |Версія програми: [ВерсияПрограммы]; 
                       |Версія Платформи: [ВерсияПлатформы] [РазрядностьПлатформы]; 
                       |Версія Бібліотеки стандартних підсистем: [ВерсияБСП];
                       |Операційна система: [ОперационнаяСистема];
                       |Розмір оперативної пам''яті: [ОперативнаяПамять];
                       |Ім''я COM з''єднувача: [ИмяСОМСоединителя];
                       |Базова: [ЭтоБазоваяВерсияКонфигурации]
                       |Повноправний користувач: [ЭтоПолноправныйПользователь]
                       |Навчальна: [ЭтоУчебнаяПлатформа]
                       |Конфігурація змінена: [КонфигурацияИзменена]'") + Символы.ПС;
...
Процедура ПоказатьРезультатВыполнения(Форма, Результат) Экспорт
	...
	Если Результат.Свойство("ВыводПредупреждения") И Результат.ВыводПредупреждения.Использование Тогда
		...
		Если ЗначениеЗаполнено(ВыводПредупреждения.ТекстОшибок) Тогда
			...
			КонецЕсли;
			Кнопки.Добавить(0, НСтр("ru='Продолжить';uk='Продовжити'"));
...
Процедура ПриВыполненииСтандартныхПериодическихПроверок() Экспорт
	...
	ТекстСообщения =
		НСтр("ru='Версия программы обновлена (внесены изменения в конфигурацию информационной базы).
                   ...
                   |Для подальшої роботи рекомендується перезапустити  програму.
                   |Запустити знову?'");
...
Функция ИнтерактивнаяОбработкаПередНачаломРаботыСистемы(Параметры)
	...
	Иначе
		...
		Если Параметры.ОповещениеЗавершения = Неопределено Тогда
			...
			// как обработчика события, до открытия главного окна.
...
Процедура ДействияПередНачаломРаботыСистемыПослеОбработкиВходаСРазблокирующимКодом(Неопределен, Параметры) Экспорт
	...
	Попытка
		ОбновлениеИнформационнойБазыКлиент.НачатьОбновлениеИнформационнойБазы(Параметры);
	Исключение
		ОбработатьОшибкуПриЗапускеИлиЗавершении(Параметры, ИнформацияОбОшибке(), "Запуск", Истина);
	КонецПопытки;
	Если ИнтерактивнаяОбработкаПередНачаломРаботыСистемы(Параметры) Тогда
