...
Процедура ПолучитьМаркерБезопасностиПослеПолучения(Результат, ВходящийКонтекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		КонецЕсли;
	Иначе
		ТекстИсключения = НСтр("ru='Пользователь отказался от ввода пароля';uk='Користувач відмовився від введення пароля'");
	КонецЕсли;
...
Процедура РасшифроватьПереборомСертификатов(ВходящийКонтекст, ИнформацияОбОшибке = Неопределено)
	...
	Если ИнформацияОбОшибке = Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(ТекстИсключения) Тогда
			ТекстИсключения = НСтр("ru='Не удалось выполнить расшифровку сообщения';uk='Не вдалося виконати розшифровку повідомлення'");
		КонецЕсли;
		Попытка			
			ВызватьИсключение(ТекстИсключения);
		Исключение
...
Процедура РасшифроватьПослеВыполненияРасшифровки(ДлительнаяОперация, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		Если ТипЗнч(Результат.РезультатВыполнения) = Тип("Структура") Тогда
			...
			Иначе
				ВызватьИсключение(НСтр("ru='Неизвестный код возврата';uk='Невідомий код повернення'"));
			КонецЕсли;
		Иначе
...
Функция ПолучитьРезультатВыполненияВФоне(ДлительнаяОперация) Экспорт
	...
	Если ДлительнаяОперация = Неопределено Тогда
		Возврат ПодготовитьОтрицательныйРезультат(НСтр("ru='Вызов API сервиса криптографии не был завершен штатно.';uk='Виклик API сервісу криптографії не був завершений штатно.'"));
	КонецЕсли;
