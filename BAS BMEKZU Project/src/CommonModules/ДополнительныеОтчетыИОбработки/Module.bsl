...
Функция ПодключенныеОбъектыМетаданных(Вид) Экспорт
	...
	Для Каждого ТипИлиМетаданные Из МассивТиповИлиМетаданных Цикл
		...
		Если ОбъектМетаданных = ДополнительныеОтчетыИОбработкиКлиентСервер.ИдентификаторРабочегоСтола() Тогда
			...
			СтрокаТаблицы.Представление = НСтр("ru='Начальная страница';uk='Початкова сторінка'");
		Иначе
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	Если ВидыПодключаемыхКоманд.Найти("ЗаполнениеОбъектов", "Имя") = Неопределено Тогда
		...
		Вид.Заголовок   = НСтр("ru='Заполнить';uk='Заповнити'");
...
Процедура ПриОпределенииКомандПодключенныхКОбъекту(НастройкиФормы, Источники, ПодключенныеОтчетыИОбработки, Команды) Экспорт
	...
	Если ТипыЗаполнениеОбъекта.Количество() > 0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЗаполнениеОбъектов") Тогда
			...
			Команда.Представление = НСтр("ru='Дополнительные обработки заполнения...';uk='Додаткові обробки заповнення...'");
			Команда.Важность      = "СмТакже";
		Иначе
			Команда.Вид           = "КоманднаяПанель";
			Команда.Представление = НСтр("ru='Заполнение...';uk='Заповнення...'");
		КонецЕсли;
	...
	Если ТипыОтчетов.Количество() > 0 Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ВариантыОтчетов") Тогда
			Команда.Вид           = "Отчеты";
			Команда.Важность      = "СмТакже";
			Команда.Представление = НСтр("ru='Дополнительные отчеты...';uk='Додаткові звіти...'");
		Иначе
			Команда.Вид           = "КоманднаяПанель";
			Команда.Представление = НСтр("ru='Отчеты...';uk='Звіти...'");
		КонецЕсли;
		Команда.Порядок            = 50;
		Команда.Обработчик         = "ДополнительныеОтчетыИОбработкиКлиент.ОткрытьСписокКоманд";
		Команда.РежимЗаписи        = "Записывать";
	...
	Если ТипыСозданияСвязанныхОбъектов.Количество() > 0 Тогда
		...
		Иначе
			...
			Команда.Представление      = НСтр("ru='Создание связанных объектов...';uk='Створення зв''язаних об''єктів...'");
...
Процедура ПриПодключенииДопОтчета(Ссылка, ПараметрыОтчета, Результат, ПолучатьМетаданные) Экспорт
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ПараметрыОтчета.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Елемент ""%1"" не підключений, тому що підсистему ""%2"" відключено у настройках програми.
...
Процедура ПриПодключенииОтчета(Контекст) Экспорт
	...
	Если ТипЗнч(Ссылка) <> Тип("СправочникСсылка.ДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В процедуру ""%1"" не передан отчет';uk='В процедуру ""%1"" не переданий звіт'"),
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьДополнительныеОтчетыИОбработки") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Функціональна опція ""%2"" відключена в настройках програми.
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Объект ""%1"" не является дополнительным отчетом';uk='Об''єкт ""%1"" не є додатковим звітом'"), Строка(Ссылка));
	КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			Если Не ПодсистемаВключена Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" недоступен, т.к. дополнительные отчеты и обработки отключены в настройках программы.';uk='<Звіт ""%1"" недоступний, так як додаткові звіти і обробки відключена в настройках програми.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ЕстьПравоЧтения Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" недоступен, т.к. отсутствует право чтения дополнительных отчетов и обработок.';uk='<Звіт ""%1"" недоступний, оскільки відсутнє право читання додаткових звітів та обробок.'"),
					СтрокаТаблицы.Представление);
			ИначеЕсли Не ОтчетДоступенВМоделиСервиса Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" недоступен в модели сервиса.';uk='<Звіт ""%1"" недоступний в моделі сервісу.'"),
					СтрокаТаблицы.Представление);
			Иначе
...
Функция ПредставлениеРаздела(Раздел) Экспорт
	Если Раздел = ДополнительныеОтчетыИОбработкиКлиентСервер.ИдентификаторРабочегоСтола()
		...
		Возврат НСтр("ru='Начальная страница';uk='Початкова сторінка'");
	Иначе
...
Функция ПредставлениеОбъектаМетаданных(Объект) Экспорт
	Если ТипЗнч(Объект) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных") Тогда
		ОбъектМетаданных = Справочники.ИдентификаторыОбъектовМетаданных.ОбъектМетаданныхПоИдентификатору(Объект, Истина);
...
Процедура ВыгрузитьОтчетыИОбработкиВФайлы(ОтчетыИОбработки)
	...
	Если Не Выгрузка.Успех Тогда
		ТекстОшибки = СокрЛП(
			НСтр("ru='Не удалось выгрузить отчеты и обработки конфигурации во внешние файлы:';uk='Не вдалося вивантажити звіти і обробки конфігурації в зовнішні файли:'")
	...
	Пока ОбратныйИндекс > 0 Цикл
		...
		Иначе
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, НСтр("ru='Неподдерживаемый вид объектов метаданных: ""1""';uk='Непідтримуваний вид об''єктів метаданих: ""1""'"), СтрокаТаблицы.Вид);
			ОтчетыИОбработки.Удалить(ОбратныйИндекс);
			Продолжить;
		КонецЕсли;
		...
		Если ТекстСхемы = Неопределено Тогда
			ЗаписатьОшибку(СтрокаТаблицы.Ссылка, НСтр("ru='Не обнаружен файл ""%1"".';uk='Не знайдений файл ""%1"".'"), ПолноеИмяСхемыОбъекта);
			ОтчетыИОбработки.Удалить(ОбратныйИндекс);
			Продолжить;
		КонецЕсли;
		Если СтрокаТаблицы.Вид = "ОТЧЕТ" Тогда
		...
		Если Не СозданиеОбработки.Успех Тогда
			ТекстОшибки = НСтр("ru='Не удалось создать ""%1"" из внешнего файла ""%2"":%3';uk='Не вдалося створити ""%1"" із зовнішнього файлу ""%2"":%3'");
...
Функция ПакетныйЗапускКонфигуратора(Параметры, ПереданныеКомандыЗапуска)
	...
	Если Не ЗначениеЗаполнено(Параметры.ПутьККонфигурации) Тогда
		...
		Если ОткрытКонфигуратор() Тогда
			...
			Иначе
				Результат.Кратко = НСтр("ru='Для выгрузки модулей необходимо закрыть конфигуратор.';uk='Для вивантаження модулів необхідно закрити конфігуратор.'");
				Возврат Результат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Результат.Кратко = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось выгрузить конфигурацию в XML (код ошибки ""%1"")';uk='Не вдалося вивантажити конфігурацію в XML (код помилки ""%1"")'"),
		КодВозврата);
		...
		Если Не Результат.Успех Тогда
			Если Результат.ИмяОбъектаЗанято Тогда
				Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Имя ""%1"" занято объектами ""%2""';uk='Ім''я ""%1"" зайнято об''єктами ""%2""'"),
					ИмяОбъекта,
			...
			Исключение
				ЗаписьЖурналаРегистрации(
					...
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Ошибка при удалении задания ""%1"":%2';uk='Помилка при вилученні завдання ""%1"":%2'"),
...
Функция ЗарегистрироватьОбработку(Знач Объект, Знач ПараметрыРегистрации) Экспорт
	...
	Если РегистрационныеДанные.Вид = Перечисления.ВидыДополнительныхОтчетовИОбработок.ПечатнаяФорма
		...
		Результат.ТекстОшибки = НСтр("ru='Работа с печатными формами не поддерживается.';uk='Робота з друкованими формами не підтримується.'");
		Возврат Результат;
	КонецЕсли;
	...
	Для Каждого КомандаОписание Из РегистрационныеДанные.Команды Цикл
		...
		Если НЕ ЗначениеЗаполнено(КомандаОписание.ВариантЗапуска) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Для команды ""%1"" не определен способ запуска.';uk='Для команди ""%1"" не визначено спосіб запуску.'"), КомандаОписание.Представление));
		КонецЕсли;
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		...
		Исключение
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |%2'
                |;uk='Помилка при отриманні реєстраційних даних:
                |Помилка при вилученні тимчасового файлу ""%1"":
                |%2'"),
				РезультатРегистрации.ИмяОбъекта,
