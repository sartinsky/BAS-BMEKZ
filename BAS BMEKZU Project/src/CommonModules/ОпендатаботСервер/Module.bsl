...
#Область ОпендатаБотПолучениеРезультатаЗапроса
...
Функция ПолучитьРезультатПроверкиКлюча(Ключ) Экспорт
	...
	Если НЕ СтруктураВозврата.Ошибка Тогда
		...
		Если РезультатЧтениеJSON.Свойство("expiry_date") Тогда
			Если Не РезультатЧтениеJSON.expiry_date = "null" Тогда
...
Функция ПолучитьJSONНаСервере(СтруктураОтвета,Ресурс)
	...
	ИначеЕсли РезультатHTTP.КодСостояния = 403 Тогда 
		...
		СтруктураОтвета.Ошибка = Истина;
		СтруктураОтвета.ТекстОшибки = НСтр("ru='Ошибка 403 (Пинкод сервиса не указан или введен не корректно).';uk='Помилка 403 (Пінкод сервісу не вказано або введено не коректно).'");
		
	ИначеЕсли РезультатHTTP.КодСостояния = 404 Тогда 
		// Company not found
		СтруктураОтвета.Ошибка = Истина;
		СтруктураОтвета.ТекстОшибки = НСтр("ru='Ошибка 404 (Данные не найдены).';uk='Помилка 404 (Дані не знайдено).'");
...
Процедура ЗаполнитьПоставляемыеПрофилиПользователей(ОписанияПрофилей) Экспорт	
	...
	// Описание для заполнения профиля "Использование сервиса Опендатабот.Ризики".
	ОписаниеПрофиля = УправлениеДоступом.НовоеОписаниеПрофиляГруппДоступа();
	ОписаниеПрофиля.Идентификатор = "e0c51787-fac2-46a0-8caf-445bc4b0971b";
	ОписаниеПрофиля.Наименование  = НСтр("ru='Использование сервиса Опендатабот.Ризики';uk= 'Використання сервісу Опендатабот.Ризики'", Локализация.КодЯзыкаИнформационнойБазы());
...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = МодульРаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		...
		"opendatabot.com",
		443,
		НСтр("ru='Получение данных из сервиса Опендатабот.Ризики.';uk='Отримання даних із сервісу Опендатабот.Різики.'"));
