...
Функция ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	...
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		...
		ШаблонОшибки = НСтр("ru='Не удалось синхронизировать файл по адресу %2, т.к. сервер вернул HTTP код: %1. %3';uk='Не вдається синхронізувати файл за адресою %2, т. к. сервер повернув HTTP код: %1. %3'");
