			...
			Иначе // Считаем, что иначе это путь к файлу на диске.
				...
				Иначе
					...
					НСтр("ru='Не удалось добавить файл ""%1"" ни в один из томов, т.к. он отсутствует.
                        ...
                        |;uk='Не вдалося додати файл ""%1"" ні в один з томів, т. я. він відсутній.
                        |Можливо, файл вилучений антивірусною програмою.
...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка.Наименование                 = НСтр("ru='Поиск ссылок на несуществующие файлы в томах хранения';uk='Пошук посилань на неіснуючі файли у томах зберігання'");
	Проверка.Причины                      = НСтр("ru='Файл был физически удален или перемещен на диске вследствие работы антивирусных программ,
        ...
        |;uk='Файл був фізично видалений або переміщений на диску внаслідок роботи антивірусних програм,
        |ненавмисних дій адміністратора і. т. д.'");
	Проверка.Рекомендация                 = НСтр("ru='• Пометить файл в программе на удаление;
        ...
        |;uk='• Позначити файл у програмі на вилучення;
        |• Відновити файл на диску у томі з резервної копії.'");
...
Процедура ИзвлечьТекстИзФайлов() Экспорт
	...
	ЗаписьЖурналаРегистрации(
		...
		,
		,
		НСтр("ru='Начато регламентное извлечения текста';uk='Розпочато регламентне витягування тексту'"));
	...
	Для Каждого ФайлБезТекста Из ФайлыДляИзвлеченияТекста Цикл
			...
			ЗаписьЖурналаРегистрации(
				...
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось выполнить регламентное извлечение текста из файла
                               ...
                               |по причине:
                               |""%2"".'
                               |;uk='Не вдалося виконати регламентне вилучення тексту з файлу
                               |""%1""
                               |з причини:
                               |""%2"".'"),
					ИмяСРасширениемФайла,
		...
		Если УдалятьВременныйФайл Тогда
			...
			Исключение
				ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Извлечение текста';uk='Файли.Витягування тексту'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ЗаписатьРезультатИзвлеченияТекста(ФайлИлиВерсияСсылка, РезультатИзвлечения,
	...
	Если Не ПустаяСтрока(АдресВременногоХранилищаТекста) Тогда
		...
		МетаданныеФайла = Метаданные.НайтиПоТипу(ФайлИлиВерсияСсылка);
		ИспользованиеПолнотекстовогоПоиска = Метаданные.СвойстваОбъектов.ИспользованиеПолнотекстовогоПоиска.Использовать;
		Если МетаданныеФайла.ПолнотекстовыйПоиск = ИспользованиеПолнотекстовогоПоиска Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.ЭлектроннаяПодпись") Тогда
		...
		Обработчик.Комментарий =
			НСтр("ru='Перенос электронных подписей и сертификатов шифрования
                       ...
                       |;uk='Перенесення електронних підписів та сертифікатів шифрування
                       |з табличних частин в регістри відомостей.'");
		...
		Обработчик.Комментарий =
			НСтр("ru='Перенос электронных подписей с версий файлов на файлы.';uk='Перенесення електронних підписів з версій файлів на файли.'");
	...
	Обработчик.Комментарий =
			НСтр("ru='Перенос двоичных данных файлов в регистр сведений Двоичные данные файлов.';uk='Перенесення двійкових файлів даних в регістр відомостей Двійкові дані файлів.'");
	...
	Обработчик.Версия = "2.4.1.1";
	Обработчик.Комментарий =
			НСтр("ru='Перенос информации о наличии файлов в регистр сведений Наличие файлов.';uk='Перенесення інформації про наявність файлів в регістр відомостей Наявність файлів.'");
	...
	Обработчик.Версия = "2.4.1.1";
	Обработчик.Комментарий =
			НСтр("ru='Перенос информации о файлах в регистр сведений Сведения о файлах.';uk='Перенесення інформації про файли в регістр відомостей Відомості про файли.'");
	...
	Обработчик.Комментарий =
			НСтр("ru='Обновление универсальной даты и типа хранения элементов справочника Файлы.';uk='Оновлення універсальної дати і типу зберігання елементів довідника Файли.'");
	...
	Если ОбработатьПолученныеФайлы Тогда
		...
		Если ТипХраненияФайлов() = Перечисления.ТипыХраненияФайлов.ВТомахНаДиске Тогда
			...
			Если ДвоичныеДанные = Неопределено Тогда
				...
				ЗаписьЖурналаРегистрации(НСтр("ru='Файлы.Добавление файла в том';uk='Файли.Додавання файлу в тому'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		ИначеЕсли Не СтрЗаканчиваетсяНа(КлючИЗначение.Ключ, "ВерсииПрисоединенныхФайлов") И Не КлючИЗначение.Ключ ="ВерсииФайлов" Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при определении имен справочников для хранения версий файлов.
                           ...
                           |У власника файлів типу ""%1""
                           |зазначено ім''я довідника ""%2""
                           |без закінчення ""ВерсииПрисоединенныхФайлов"".'"),
				Строка(ТипВладельцаФайлов),
				Строка(КлючИЗначение.Ключ));
			...
			Если ОсновнойСправочникУказан Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при определении имен справочников для хранения версий файлов.
                               ...
                               |;uk='Помилка при визначенні імен довідників для зберігання версій файлів.
                               |У власника файлів типу ""%1""
                               |основний довідник версій вказаний більше одного разу.'"),
					Строка(ТипВладельцаФайлов),
					Строка(КлючИЗначение.Ключ));
			КонецЕсли;
			ОсновнойСправочникУказан = Истина;
...
Функция ИмяСправочникаХраненияВерсийФайлов(ВладелецФайлов, ИмяСправочника = "",
	...
	ШаблонПричиныОшибки = 
		НСтр("ru='У владельца версий файлов ""%1""
            ...
            |;uk='У власника версій файлів ""%1""
            |не вказаний основний довідник для зберігання версій файлів.'") + Символы.ПС;
...
Процедура УдалитьФайлВТоме(ИмяУдаляемогоФайла)
	...
	Если ФайлВременный.Существует() Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Файлы.Удаление файлов в томе';uk='Файли.Вилучення файлів в томі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Функция Кодировки() Экспорт
	...
	СписокКодировок.Добавить("utf-8_WithoutBOM", НСтр("ru='UTF-8 (Юникод UTF-8 без BOM)';uk='UTF-8 (Юнікод UTF-8 без BOM)'"));
...
Функция ОбновитьВерсиюФайла(ФайлСсылка,
	...
	Если СведенияОФайле.ХранитьВерсии Тогда
		ЗаголовокОшибки = НСтр("ru='Ошибка при записи новой версии присоединенных файлов.';uk='Помилка при записі нової версії приєднаних файлів.'");
		ОкончаниеОшибки = НСтр("ru='В этом случае запись версии файла невозможна.';uk='У цьому випадку запис версії файлу неможливий.'");
...
Процедура ФормаЭлементаПриСозданииНаСервере(Контекст, Отказ, СтандартнаяОбработка, Параметры, ТолькоПросмотр, НастраиватьОбъектФормы = Ложь) Экспорт
	...
	Если РасширениеФайлаВСписке Тогда
		...
		Если Не ЗначениеЗаполнено(Контекст.Кодировка) Тогда
			Контекст.Кодировка = НСтр("ru='По умолчанию';uk='По умовчанню'");
		КонецЕсли;
...
Функция ПроверитьИсключениеHTTP1С(Ответ, АдресСервера)
	...
	Если (Ответ.КодСостояния >= 400) И (Ответ.КодСостояния <= 599) Тогда
		...
		ШаблонОшибки = НСтр("ru='Не удалось синхронизировать файл по адресу %2, т.к. сервер вернул HTTP код: %1. %3';uk='Не вдається синхронізувати файл за адресою %2, тому що сервер повернув HTTP код: %1. %3'");
...
Функция ЗагрузитьФайлССервера(ПараметрыФайла, ЭтоФайл = Неопределено)
	...
	Если РезультатЗагрузки.Успешно И РезультатЗагрузки.АдресЗагруженногоФайла <> Неопределено Тогда
		...
		ТекстСообщения = НСтр("ru='Загружен файл из облачного сервиса: ""%1""';uk='Завантажений файл з хмарного сервісу: ""%1""'");
		СтатусДляЖурналаРегистрации = УровеньЖурналаРегистрации.Информация;
	Иначе
		ТекстСообщения = НСтр("ru='Не удалось загрузить файл ""%1"" из облачного сервиса по причине:';uk='Не вдалося завантажити файл ""%1"" з хмарного сервісу з причини:'") + " " + Символы.ПС + РезультатЗагрузки.ТекстОшибки;
			...
			Если НайденныйPropstat<>Неопределено Тогда
				Для каждого ДочернийУзелPropstat Из НайденныйPropstat.ДочерниеУзлы Цикл
					Если ДочернийУзелPropstat.ЛокальноеИмя = "quota-available-bytes" Тогда
						...
						ИнформацияОСвободномМесте = НСтр("ru='Свободное место : %1 Мб';uk='Вільне місце : %1 Мб'");
					...
					ИначеЕсли ДочернийУзелPropstat.ЛокальноеИмя = "quota-used-bytes" Тогда
						...
						ИнформацияОЗанятомМесте = НСтр("ru='Занято : %1 Мб';uk='Зайнято : %1 Мб'");
...
Процедура СкачатьДеревоФайловРекурсивно(ТекущиеСтрокиДереваФайлов, HttpАдрес, ПараметрыСинхронизации, Отказ=Ложь)
	...
	Попытка
		...
		Пока НайденныйResponse <> Неопределено Цикл
			...
			НоваяСтрокаДереваФайлов.ЭтоПапка = Ложь;
...
Процедура ВыполнитьСинхронизациюФайловОблачнымСервисом(ПараметрыСинхронизации)
	...
	Если Отказ = Истина Тогда
		...
		ТекстСобытия = НСтр("ru='Не удалось загрузить структуру файлов с сервера, синхронизация не выполнена.';uk='Не вдалося завантажити структуру файлів з сервера, синхронізація не виконана.'");
	...
	Для Каждого СтрокаТаблицы Из ТаблицаФайлов Цикл
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось синхронизировать файл ""%1"" по причине:';uk='Не вдається синхронізувати файл ""%1"" з причини:'"), Строка(СтрокаТаблицы.ФайлСсылка))
...
Функция УдалитьФайлВОблачномСервисе(Знач ПараметрыСинхронизации, Знач СтрокаТаблицы)
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстСобытия = НСтр("ru='Не удалено в базе: %1';uk='Не вилучено в базі: %1'");
		ЗаписатьВЖурналСобытийСинхронизацииФайлов(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСобытия, СтрокаТаблицы.Наименование), ПараметрыСинхронизации.УчетнаяЗапись);
		Возврат Ложь;
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |%2'
                |;uk='Не вдалося обробити том зберігання файлів: %1 з причини:
                |%2'"), 
	...
	Пока Результат.Количество() > 0 Цикл
		...
		Для Каждого СтрокаРезультата Из Результат Цикл
			...
			Если СтрокаРезультата.Владелец <> Неопределено Тогда
				УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Версия ""%1"" файла ""%2"" не существует в томе ""%3"".';uk='Версія ""%1"" файлу ""%2"" не існує в томі ""%3"".'"),
					СтрокаРезультата.Файл, СтрокаРезультата.Владелец, СтрокаРезультата.Том);
			Иначе
				УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Файл ""%1"" не существует в томе ""%2"".';uk='Файл ""%1"" не існує в томі ""%2"".'"),
...
Функция ТомДоступен(Том, ПредставлениеТома, Путь, ПараметрыПроверки)
	...
	Если ПустаяСтрока(Путь) Тогда
		...
		УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='У тома хранения файлов ""%1"" не задан путь к сетевому каталогу. Сохранение файлов в него невозможно.';uk='У тома зберігання файлів ""%1"" не зазначений шлях до мережного каталогу. Збереження файлів до нього неможливо.'"), 
			ПредставлениеТома);
	...
	Исключение
		...
		УточнениеПроблемы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Том хранения файлов ""%1"" недоступен по причине: 
                ...
                |;uk='Тому зберігання файлів ""%1"" недоступний через: 
                |%2
                |
                |Зазначений мережевий каталог міг бути відключений або до нього відсутні права доступу.
                |Неможлива робота з усіма файлами, що зберігаються в цьому томі.'"),
