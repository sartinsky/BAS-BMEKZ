	...
	Если РежимОбновленияДанных = "ПереходСДругойПрограммы" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Изменилось имя конфигурации на ""%1"".
                       ...
                       |Буде виконано перехід з іншої програми.'"),
			Метаданные.Имя);
	ИначеЕсли РежимОбновленияДанных = "ОбновлениеВерсии" Тогда
		Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Изменился номер версии конфигурации: с ""%1"" на ""%2"".
                       ...
                       |Буде виконано оновлення інформаційної бази.'"),
	...
	Если НеобходимоОбновлениеНеразделенныхДанныхИнформационнойБазы() Тогда
		...
		СообщениеАдминистраторуОбластиДанных =
			НСтр("ru='Вход в приложение временно невозможен в связи с обновлением на новую версию.
                       ...
                       |;uk='Вхід у прикладну програму тимчасово неможливий у зв''язку з оновленням на нову версію.
		...
		Если ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
			...
			Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
				...
				Возврат НСтр("ru='Вход в программу временно невозможен в связи с обновлением на новую версию.
                                ...
                                |Для завершення оновлення версії програми потрібні запуск в режимі товстого, тонкого або ВЕБ-клієнта'");
...
Функция КоличествоПотоковОбновленияИнформационнойБазы() Экспорт
	...
	Если РазрешеноМногопоточноеОбновление() Тогда
		...
		Если ПозицияПараметра > 0 Тогда
			...
			Исключение
				ТекстИсключения = НСтр("ru='Параметр запуска программы ""ЧислоПотоковОбновления"" необходимо указать в формате
                    |""ЧислоПотоковОбновления=Х"", где ""Х"" - максимальное количество потоков обновления.'
                    |;uk='Параметр запуску програми ""ЧислоПотоковОбновления"" необхідно зазначити у форматі
                    |""ЧислоПотоковОбновления=Х"", де ""Х"" - максимальна кількість потоків оновлення.'");
...
Функция ИтерацииОбновления()
	...
	Если ИтерацияОбновлениеОсновнойПодсистемы = Неопределено И ИмяОсновнойКонфигурации = "БиблиотекаСтандартныхПодсистем" Тогда
		ТекстСообщения = НСтр("ru='Файл поставки Библиотека стандартных подсистем не предназначен для создания
            ...
            |;uk='Файл постачання Бібліотека стандартних підсистем не призначений для створення
...
Функция ВыполнитьИтерациюОбновления(Знач ИтерацияОбновления, Знач Параметры) Экспорт
	...
	Если ТекущаяВерсияИБ <> "0.0.0.0"
		...
		Если ВыполняемыеОбработчики = Неопределено Тогда
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				ШаблонСообщения = НСтр("ru='Не найден план обновления конфигурации %1 с версии %2 на версию %3';uk='Не знайдено план оновлення конфігурації %1 з версії %2 на версію %3'");
			Иначе
				ШаблонСообщения = НСтр("ru='Не найден план обновления библиотеки %1 с версии %2 на версию %3';uk='Не знайдено план оновлення бібліотеки %1 з версії %2 на версію %3'");
			КонецЕсли;
	...
	Для Каждого Версия Из ВыполняемыеОбработчики.Строки Цикл
		...
		Иначе
			...
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.';uk='Виконується оновлення данних бібліотеки %3 з версії %1 на версію %2.'"), 
		...
		Иначе
			Если ИтерацияОбновления.ЭтоОсновнаяКонфигурация Тогда 
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполнено обновление информационной базы с версии %1 на версию %2.';uk='Виконано оновлення інформаційної бази з версії %1 на версію %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ);
			Иначе
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.';uk='Виконано оновлення данних бібліотеки %3 з версії %1 на версію %2.'"), 
					ТекущаяВерсияИБ, НоваяВерсияИБ, ИдентификаторБиблиотеки);
			КонецЕсли;
		КонецЕсли;
		ЗаписатьИнформацию(Сообщение);
...
Функция ОбновитьИнформационнуюБазуВФоне(УникальныйИдентификаторФормы, БлокировкаИБ) Экспорт
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Фоновое обновление информационной базы';uk='Фонове оновлення інформаційної бази'");
...
Функция ДокументОписаниеОбновлений(Знач Разделы) Экспорт
	...
	Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда 
		МакетОписаниеОбновлений = Метаданные.ОбщиеМакеты.Найти("ИНАГРО_ОписаниеИзмененийСистемыБСП");
	ИначеЕсли ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБЭМКЗУ() Тогда
		МакетОписаниеОбновлений = Метаданные.ОбщиеМакеты.Найти("ИНАГРО_ОписаниеИзмененийСистемыБЭМКЗ");
	Иначе
...
Процедура ПроверитьСвойстваОтложенногоОбработчика(Знач Обработчик, Знач РежимВыполненияОтложенныхОбработчиков, ТекстОшибок)
	...
	Если Обработчик.МонопольныйРежим = Истина Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='У отложенного обработчика ""%1""
            ...
            |не повинна бути встановлена ознака ""МонопольныйРежим"".'"), 
			Обработчик.Процедура);
		ЗаписатьОшибку(ТекстОшибки);
		ТекстОшибок = ТекстОшибок + ТекстОшибки + Символы.ПС;
	КонецЕсли;
...
Процедура ПроверитьУникальностьИдентификаторовОтложенныхОбработчиков(ИтерацииОбновления)
	...
	ТекстСообщения = НСтр("ru='Обнаружены отложенные обработчики обновления,
        ...
        |;uk='Виявлені відстрочені обробники оновлення,
        |у яких збігаються унікальні ідентифікатори. Наступні ідентифікатори не унікальні:'");
...
Функция ДобавитьПотокРегистрацииДанныхОтложенногоОбновления(ОписаниеПотоков, ОписаниеПроцедуры)
	...
	ШаблонНаименования = НСтр("ru='Регистрация данных обработчика обновления ""%1""';uk='Реєстрація даних обробника оновлення ""%1""'");
...
Функция ДобавитьПотокОбработчикаОтложенногоОбновления(ОписаниеПотоков, СведенияОбОбновлении)
	...
	Если ТипЗнч(ОбработчикОбновления) = Тип("СтрокаДереваЗначений") Тогда
		Если КонтекстОбработчика.ВыполнитьОбработчик Тогда
			ШаблонНаименования = НСтр("ru='Выполнение обработчика обновления ""%1""';uk='Виконання обробника оновлення ""%1""'");
			Поток = ОписаниеПотоков.Потоки.Добавить();
...
Процедура УстановитьКлючРегламентногоЗадания() Экспорт
	...
	Для Каждого Задание Из Задания Цикл
		Задание.Ключ = "9fa79867-46b8-4060-9694-eb85d6ecf1ea";
		Задание.Записать();
	КонецЦикла;
...
Функция ВозможноОперативноеОбновление(ПроверяемыеИтерацииОбновления = Неопределено) Экспорт
	...
	Если ПроцедурыОбработчиков.Количество() <> 0 Тогда
		ТекстСообщения = НСтр("ru='Следующие обработчики не поддерживают обновление без установки монопольного режима:';uk='Наступні обробники не підтримують оновлення без встановлення монопольного режиму:'");
	...
	Если ТранзакцияАктивнаНаНачалоВыполнения Тогда
		...
		Если ТранзакцияАктивна() Тогда
			...
			Исключение
                ...
                |Обробником оновлення було поглинуте виключення при активній зовнішній транзакції.
			...
			Исключение
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='У обработчика неправильно заполнено свойство Версия: ""%1"".
                               ...
                               |;uk='У обробника неправильно заповнена властивість Версія: ""%1"".
			...
			Если НЕ ЗначениеЗаполнено(ОписаниеОшибки)
			   ...
			   И Обработчик.Приоритет <> 0 Тогда
				...
				ОписаниеОшибки = НСтр("ru='У обработчика неправильно заполнено свойство Приоритет или
                                            ...
                                            |;uk='У обробника неправильно заповнена властивість Пріоритет або
		...
		Если Обработчик.РежимВыполнения <> ""
			...
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='У обработчика ""%1"" неправильно заполнено свойство РежимВыполнения.
                           ...
                           |;uk='У обробника ""%1"" неправильно заповнена властивість РежимВыполнения.
                           |Допустиме значення: ""Монопольно"", "" Відкладено"", ""Оперативно"".'"),
				Обработчик.Процедура);
		КонецЕсли;
		...
		Для каждого Область Из Макет.Области Цикл
			...
			Если ВесВерсии > ВесВерсииМетаданных Тогда
				ТекстИсключения = НСтр("ru='В общем макете ОписаниеИзмененийСистемы для одного из разделов изменений
                    ...
                    |;uk='У загальному макеті ОписаниеИзмененийСистемы для одного з розділів змін
                    |встановлена версія вище, ніж в метаданих. (%1, повинна бути %2)'");
...
Процедура ЗавершитьВыполнениеОтложенногоОбработчикаОбновления(КонтекстОбработчика, СведенияОбОбновлении)
	...
	Если ПараллельныйРежим
		...
		ШаблонОшибки = НСтр("ru='Не удалось выполнить обработчик обновления ""%1"". Подробнее в журнале регистрации.';uk='Не вдалося виконати оброблювач оновлення ""%1"". Докладніше в журналі реєстрації.'");
		...
		Если КоличествоЗапусковПроцедурыОбновления > 10000 Тогда // Защита от зацикливания.
			Если ПараметрыОбновления.ПараллельныйРежим
				...
				ТекстОшибки = НСтр("ru='Превышено допустимое количество запусков процедуры обновления.
                    ...
                    |;uk='Перевищена припустима кількість запусків процедури оновлення.
                    |Переконайтеся, що додаткові процедури обробки даних в головному вузлі
                    |повністю завершилися, виконайте синхронізацію даних та повторно
                    |запустіть виконання процедур обробки даних в даному вузлі.'");
			Иначе
				ТекстОшибки = НСтр("ru='Превышено допустимое количество запусков процедуры обновления.
...
//  ОбработчикОбновления - СтрокаДереваЗначений - обработчик обновления в виде строки дерева обработчиков.
//
Процедура ЗавершитьПроцедуруОбработкиДанных(КонтекстОбработчика, ОбработчикОбновления)
	...
	Для Каждого Обработчик Из СтрокаВерсия.Строки Цикл
		...
		ИначеЕсли ЗначениеЗаполнено(БлокируемыеОбъекты) И Не ЗначениеЗаполнено(ПроцедураПроверки) Тогда
			ТекстИсключения = НСтр("ru='У отложенного обработчика обновления ""%1""
                ...
                |заповнено список заблокованих об''єктів, але не встановлено властивість ""ПроцедураПроверки"".'");
...
Процедура ЗаполнитьДанныеОтложенногоОбработчика(ОписаниеПроцедуры, АдресРезультата) Экспорт
	...
	ШаблонСообщения = НСтр("ru='Выполняется процедура заполнения данных
        |""%1""
        |отложенного обработчика обновления
        |""%2"".'
        |;uk='Виконується процедура заповнення даних
	...
	Исключение
		...
		ШаблонОшибки = НСтр("ru='При вызове процедуры заполнения данных
            |""%1""
            |отложенного обработчика обновления
            |""%2""
            |произошла ошибка:
...
Процедура ОтметитьЗациклившиесяОбработчики(ДеревоОбработчиков)
	...
	Для каждого ОбработчикОбновления Из Выполняющиеся Цикл
		Если ОбработчикОбновления.ЧислоПопыток >= 2 Тогда
			ОбработчикОбновления.Статус = "Ошибка";
			ОбработчикОбновления.ИнформацияОбОшибке = НСтр("ru='Произошло зацикливание процедуры обработки данных. Выполнение прервано.';uk='Сталося зациклення процедури обробки даних. Виконання перервано.'");
		КонецЕсли;
	КонецЦикла;
