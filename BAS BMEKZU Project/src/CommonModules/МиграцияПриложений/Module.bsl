...
//   ЗавершитьМиграциюАвтоматически - Булево - если Истина, то будет выполнена попытка установить монопольный режим и завершить миграцию.
...
Процедура НачатьВыгрузку(АдресПриложения, Логин, Пароль, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически, ДополнительныеСвойства) Экспорт
	...
	Если Запись.Выбран() Тогда
		ВызватьИсключение НСтр("ru='Переход уже начался.';uk='Перехід вже почався.'");
	КонецЕсли;
	
	НачатьТранзакцию();
	Попытка
		НовыйУзел = ПланыОбмена.МиграцияПриложений.СоздатьУзел();
		НовыйУзел.Наименование = СтрШаблон(НСтр("ru='Миграция в %1 (%2)';uk='Міграція в %1 (%2)'"), Метаданные.Представление(), АдресПриложения);
		...
		ТехнологияСервисаИнтеграцияСБСП.ЗаписатьДанныеВБезопасноеХранилище(НовыйУзел.Ссылка, ПараметрыДоступа, "ПараметрыДоступа");
...
Процедура ОтменитьВыгрузку() Экспорт
	...
	Попытка
		...
		Если Не Запись.Выбран() Тогда
			ВызватьИсключение НСтр("ru='Переход не выполняется.';uk='Перехід не виконується.'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Запись.УзелОбмена) Тогда
		
			УстановитьПривилегированныйРежим(Истина);
			ПараметрыДоступа = ТехнологияСервисаИнтеграцияСБСП.ПрочитатьДанныеИзБезопасногоХранилища(Запись.УзелОбмена, "ПараметрыДоступа");
			...
			Исключение
				ШаблонСообщения = НСтр("ru='Отмена миграции на сервисе завершилась с ошибкой, обратитесь к администратору сервиса. 
                                        |Описание ошибки:
                                        |%1'
                                        |;uk='Скасування міграції на сервісі завершилося з помилкою, зверніться до адміністратора сервісу. 
                                        |Опис помилки:
                                        |%1'");
...
Процедура НачатьЗагрузку(ПользователиОбласти) Экспорт
	...
	Попытка
		...
		Если СостояниеЗагрузки.Выбран() Тогда
			ВызватьИсключение НСтр("ru='Загрузка уже выполняется.';uk='Завантаження вже виконується.'");
		КонецЕсли;
...
//
Процедура ЗагрузитьСообщение(Источник) Экспорт
	...
	Если Выборка.Следующий() И Выборка.ОчередьЗагрузки >= МаксимальнаяОчередьЗагрузки() Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru='Очередь загрузки больше %1!';uk='Черга завантаження більше %1!'"), МаксимальнаяОчередьЗагрузки());
	КонецЕсли;
	...
	Исключение
		...
		РегистрацияОшибки(НСтр("ru='Загрузка';uk='Завантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПредставлениеОшибки);
	КонецПопытки;
	
	Если Манифест.ИмяКонфигурации <> Метаданные.Имя Тогда
		ВызватьИсключение НСтр("ru='Имя конфигурации не совпадает.';uk='Ім''я конфігурації не збігається.'");
	КонецЕсли;
	
	Если Манифест.ВерсияКонфигурации <> Метаданные.Версия Тогда
		ВызватьИсключение НСтр("ru='Версия конфигурации не совпадает.';uk='Версія конфігурації не збігається.'");
	КонецЕсли;
...
//     * ТребуетсяПовторнаяОтправка - Булево - Истина если требуется отправить повторно данные, которые находятся сейчас в очереди.
//
...
Процедура ЗаданиеВыгрузка(ЗавершитьМиграцию = Ложь) Экспорт
	...
	Если Не СостояниеВыгрузки.Выбран() Тогда
		ПредставлениеОшибки = НСтр("ru='Состояние выгрузки не найдено.';uk='Стан вивантаження не знайдено.'");
		РегистрацияОшибки(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПредставлениеОшибки);
		РегламентныеЗаданияСервер.УдалитьЗадание(Метаданные.РегламентныеЗадания.МиграцияПриложенийВыгрузка);
		Возврат;
	КонецЕсли;
	...
	Если Не ЗначениеЗаполнено(УзелОбмена) Тогда
		ПредставлениеОшибки = НСтр("ru='Не найден узел обмена.';uk='Не знайдено вузол обміну.'");
	...
	УстановитьПривилегированныйРежим(Истина);
	ПараметрыДоступа = ТехнологияСервисаИнтеграцияСБСП.ПрочитатьДанныеИзБезопасногоХранилища(УзелОбмена, "ПараметрыДоступа");
	УстановитьПривилегированныйРежим(Ложь);
	
	Если ПараметрыДоступа = Неопределено Тогда
		ПредставлениеОшибки = НСтр("ru='Не найдены параметры подключения.';uk='Не знайдено параметри підключення.'");
		ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПредставлениеОшибки, ПредставлениеОшибки);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		Шаблон = НСтр("ru='Не удалось получить состояние загрузки, возможно сервис временно недоступен: %1 %1';uk='Не вдалося отримати стан завантаження, можливо сервіс тимчасово недоступний: %1 %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		Комментарий = СтрШаблон(Шаблон, Символы.ПС, ПредставлениеОшибки);
		РегистрацияПримечания(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Комментарий);
		...
		Если (ТекущаяУниверсальнаяДата() - ДатаПроверки) > (86400) Тогда
			ПредставлениеОшибки = НСтр("ru='Сервис недоступен 24 часа, выгрузка прервана.';uk='Сервіс недоступний 24 години, вивантаження перервана.'");
			ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПредставлениеОшибки, ПредставлениеОшибки);
		КонецЕсли;
		Возврат;
	КонецПопытки;
	...
	Если СостояниеЗагрузки.ИмяКонфигурации <> Метаданные.Имя Тогда
		ПредставлениеОшибки = НСтр("ru='Имя конфигурации не совпадает.';uk='Ім''я конфігурації не збігається.'");
		ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПредставлениеОшибки, ПредставлениеОшибки);
	КонецЕсли;
	...
	ИначеЕсли СостояниеВыгрузки.Состояние = Перечисления.СостоянияМиграцииПриложения.ОжиданиеЗагрузки Тогда
		...
		Попытка
			Если Не СостояниеЗагрузки.ВыполняетсяЗагрузка Тогда
				...
				СостояниеВыгрузки.Комментарий = НСтр("ru='Переход завершен.';uk='Перехід завершено.'");
	...
	Исключение
		...
		Иначе
			...
			РегистрацияОшибки(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
...
//
Функция ВыгрузитьДанныеВКаталог(КаталогВыгрузки, УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ВыгружатьНастройкиПользователей, ЗавершитьМиграциюАвтоматически) Экспорт
	...
	Если ВыгруженоОбъектов < ПараметрыВыгрузки.РазмерПорцииДанных Тогда
		Если ЗавершитьМиграциюАвтоматически Тогда
			...
			Исключение
				Комментарий = НСтр("ru='Не удалось установить монопольный режим.';uk='Не вдалося встановити монопольний режим.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				РегистрацияПримечания(НСтр("ru='Выгрузка';uk='Вивантаження'"), Комментарий);
			КонецПопытки;
		КонецЕсли;
...
Процедура ЗаданиеЗагрузка() Экспорт
	...
	Если Не СостояниеЗагрузки.Выбран() Тогда
		Комментарий = НСтр("ru='Состояние загрузки не найдено. Загрузка прервана';uk='Стан завантаження не знайдено. Завантаження перервано'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		РегистрацияОшибки(НСтр("ru='Загрузка';uk='Завантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Комментарий);
		// Самоудаление.
		УдалитьОчередь();
		РегламентныеЗаданияСервер.УдалитьЗадание(Метаданные.РегламентныеЗадания.МиграцияПриложенийЗагрузка);
		Возврат;
	...
	Если Не Выборка.Следующий() Тогда
		...
		Если (ТекущаяУниверсальнаяДата() - ОтсчетВремени) > (86400) Тогда
			ПредставлениеОшибки = НСтр("ru='Отсутствуют сообщения в течении 24 часов.';uk='Відсутні повідомлення протягом 24 годин.'");
	...
	Если Не Файл.Существует() Тогда
		ПодробноеПредставлениеОшибки = СтрШаблон(НСтр("ru='Файл сообщения %1 не найден.';uk='Файл повідомлення %1 не знайдено.'"), Выборка.ИмяФайла);
		КраткоеПредставлениеОшибки = НСтр("ru='Файл сообщения не найден.';uk='Файл повідомлення не знайдено.'");
	...
	Если ОбъектыДляОчистки = Неопределено Тогда
		...
		КонецЦикла;
		// Это самое начало
		Если ТехнологияСервисаИнтеграцияСБСП.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
			
			МодульРаботаВМоделиСервиса = ТехнологияСервисаИнтеграцияСБСП.ОбщийМодуль("УправлениеДоступом");
			МодульРаботаВМоделиСервиса.ОтключитьОбновлениеКлючейДоступа(Истина, Ложь);
	...
	Исключение
		...
		Иначе 
			РегистрацияОшибки(НСтр("ru='Загрузка';uk='Завантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
		...
		Если Результат.Успешно Тогда
			...
			Если Результат.ЭтоПоследнееСообщение Тогда
				...
				Если ТехнологияСервисаИнтеграцияСБСП.ПодсистемаСуществует("СтандартныеПодсистемы.УправлениеДоступом") Тогда
					...
					МодульРаботаВМоделиСервиса = ТехнологияСервисаИнтеграцияСБСП.ОбщийМодуль("УправлениеДоступом");
					МодульРаботаВМоделиСервиса.ОтключитьОбновлениеКлючейДоступа(Ложь);
...
Функция ЗагрузитьДанныеИзКаталога(КаталогЗагрузки, СоответствиеСсылок, ОбъектыДляОчистки) Экспорт
	...
	Если Манифест.Свойство("ОбъектыВыгруженныеПолностью", ОбъектыВыгруженныеПолностью) Тогда
		Шаблон = НСтр("ru='Не удалось выгрузить порциями объекты: %1.';uk='Не вдалося вивантажити порціями об''єкти: %1.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПроверитьСоставПланаОбмена() Экспорт
	...
	Для Каждого Состав Из Метаданные.ПланыОбмена.МиграцияПриложений.Состав Цикл
		Если Состав.АвтоРегистрация = АвтоРегистрацияИзменений.Разрешить Тогда
			ВызватьИсключение НСтр("ru='Некорректно настроен план обмена ""Миграция приложений"": авторегистрация должна быть выключена.';uk='Некоректно налаштовано план обміну ""Міграція програм"": автореєстрація повинна бути вимкнена.'");
		КонецЕсли;
	...
	Для Каждого ЗагружаемыйОбъект Из МиграцияПриложенийПовтИсп.ВыгружаемыеОбъекты() Цикл
		Если СоставПланаОбмена.Получить(ЗагружаемыйОбъект.Ключ) = Неопределено Тогда
			Шаблон = НСтр("ru='Некорректно настроен план обмена ""Миграция приложений"": в состав нужно добавить %1.';uk='Некоректно налаштовано план обміну ""Міграція програм"": до складу треба додати %1.'");
...
Функция АбонентыПользователя(ПараметрыДоступа) Экспорт
	...
	Попытка
		...
		КонецЦикла;
	Исключение
		ВызватьИсключение СтрШаблон(НСтр("ru='Неверный формат ответа внешнего программного интерфейса: ""%1""';uk='Неправильний формат відповіді зовнішнього програмного інтерфейсу: ""%1""'"), "ext/account/list");
	КонецПопытки;
...
Функция ВызватьМетодПрограммногоИнтерфейса(ПараметрыДоступа, ПараметрыМетода) Экспорт
	...
	Если Ответ.КодСостояния = 401 Тогда
		ВызватьИсключение НСтр("ru='Неверный логин или пароль.';uk='Невірний логін або пароль.'");
	ИначеЕсли Ответ.КодСостояния <> 200 Тогда
		Шаблон = НСтр("ru='""Не удалось выполнить запрос, код ошибки: %1';uk='""Не вдалося виконати запит, код помилки: %1'");
...
Функция СервисПоддерживаетМиграцию(ИмяСервера, АдресПрограммногоИнтерфейса, АдресРегистрации, АдресВосстановления, РегистрацияРазрешена) Экспорт
	...
	Исключение
		Шаблон = НСтр("ru='Сервис временно не доступен или не поддерживает миграцию,
                       |описание ошибки:
                       |%1'
                       |;uk='Сервіс тимчасово недоступний або не підтримує міграцію,
                       |опис помилки:
                       |%1'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(Шаблон, КраткоеПредставлениеОшибки(ИнформацияОбОшибке())));
		Возврат Ложь;
	КонецПопытки;
	
	Если Ответ.КодСостояния <> 200 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Сервис не поддерживает миграцию';uk='Сервіс не підтримує міграцію'"));
		Возврат Ложь;
	КонецЕсли;
...
//     * Роль - Строка - см. МиграцияПриложенийКлиентСервер.ПредставлениеРоли().
//
Функция ПользователиСервиса(ПараметрыДоступа) Экспорт
	...
	Для Каждого ДанныеПользователя Из РезультатВызова.user Цикл
		...
		Если ДанныеПользователя.login = "ivanoff.u3@yopmail.com"
			Или ДанныеПользователя.login = "petroff@yopmail.com" Тогда
			Продолжить;
		КонецЕсли;
...
Функция Зарегистрироваться(ИмяСервера, Наименование, Логин, ЭлектроннаяПочта, Пароль, Телефон) Экспорт
	...
	Если Ответ.КодСостояния <> 200 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(СтрШаблон(НСтр("ru='Не удалось выполнить запрос, код ответа: %1';uk='Не вдалося виконати запит, код відповіді: %1'"), Ответ.КодСостояния));
		Возврат Ложь;
	КонецЕсли;
	...
	Если Результат.error Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Результат.description);
		Возврат Ложь;
	КонецЕсли;
...
Функция Описание(ИдентификаторХранилища, ИдентификаторДанных) Экспорт
	...
	Иначе
		...
		Результат.Комментарий = СтрШаблон(НСтр("ru='Неподдерживаемый идентификатор данных: %1.';uk='Непідтримуваний ідентифікатор даних: %1.'"), ИдентификаторДанных);
	КонецЕсли;
...
Функция Загрузить(ОписаниеДанных) Экспорт
	...
	Если ОписаниеДанных.ИмяФайла = "message" Тогда
		...
		Исключение
			ИнформацияОбОшибке = ИнформацияОбОшибке();
			РегистрацияОшибки(НСтр("ru='Загрузка сообщения';uk='Завантаження повідомлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке));
			Результат.Успешно = Ложь;
	...
	Иначе
		...
		Результат.Успешно = Ложь;
		Результат.Комментарий = СтрШаблон(НСтр("ru='Загрузка файла ""%1"" не поддерживается.';uk='Завантаження файлу ""%1"" не підтримується.'"), ОписаниеДанных.ИмяФайла);
...
Функция СостояниеЗагрузкиИЗсервиса(ПараметрыДоступа)
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не удалось получить статус.';uk='Не вдалося отримати статус.'");
	КонецЕсли;
...
Функция ОтправитьСообщениеВСервис(ПараметрыДоступа, ИмяФайла)
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не удалось отправить сообщение.';uk='Не вдалося відправити повідомлення.'");
...
Функция ОтменитьЗагрузкуВСервисе(ПараметрыДоступа)
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не удалось отменить загрузку в сервисе.';uk='Не вдалося скасувати завантаження в сервісі.'");
	КонецЕсли;
...
Функция ПодтвердитьПовторнуюЗагрузкуВСервисе(ПараметрыДоступа)
	...
	Если Результат = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не удалось подтвердить повторную загрузку в сервисе.';uk='Не вдалося підтвердити повторне завантаження в сервісі.'");
	КонецЕсли;
...
Функция ВыгрузитьПользовательскиеДанные(УзелОбмена, НомерСообщения, ПоследнийОбъектМетаданных, ПараметрыВыгрузки)
	...
	ШаблонНачалоВыгрузки = НСтр("ru='Выгрузка объекта: %1';uk='Вивантаження об''єкту: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ШаблонОкончаниеВыгрузки = НСтр("ru='Выгружен объект: %1, Количество: %2, Длительность (сек): %3';uk='Вивантажений об''єкт: %1, Кількість: %2, Тривалість (сек): %3'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ВсегоОбъектов = 0;
	...
	Для Каждого ОбъектМетаданных Из ПорядокВыборкиМетаданных(ПоследнийОбъектМетаданных) Цикл
		...
		РегистрацияПримечания(НСтр("ru='Статистика выгрузки';uk='Статистика вивантаження'"), Комментарий);
...
Функция ВыгрузитьКонстанту(ОбъектМетаданных, Узел, НомерСообщения, ПараметрыВыгрузки)
	...
	|	ИСТИНА КАК Проверка
	|ИЗ
	|	" + ОбъектМетаданных.ПолноеИмя() + ".Изменения КАК Изменения
	|ГДЕ
	|	Изменения.Узел = &Узел
...
Функция ВыгрузитьСсылочныйТип(ОбъектМетаданных, Узел, НомерСообщения, ОсталосьВыгрузить, ПараметрыВыгрузки)
	...
	Запрос.УстановитьПараметр("Узел", Узел);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ " + Формат(ОсталосьВыгрузить, "ЧГ=0") + "
	|	Изменения.Ссылка КАК Ссылка
	|ИЗ
	|	" + ИмяТаблицы + ".Изменения КАК Изменения
	|ГДЕ
	|	Изменения.Узел = &Узел
	|	И Изменения.НомерСообщения ЕСТЬ NULL";
	...
	Пока Истина Цикл
		...
		Пока ВыборкаДанных.Следующий() Цикл
			...
			Если ЭтоРегистрБухгалтерии Тогда
				ДобавитьВспомогательныеДанные(СериализаторXDTO.ЗаписатьXDTO(Объект.Выгрузить()), ПараметрыВыгрузки);
			Иначе
				ДобавитьВспомогательныеДанные(СериализаторXDTO.ЗаписатьXDTO(Объект), ПараметрыВыгрузки);
			КонецЕсли;
			СериализаторXDTO.ЗаписатьXML(ЗаписьXML, Объект, "Object", НазначениеТипаXML.Явное);
...
//   Узел - ПланОбменаСсылка.Миграция Приложений.
//   ОсталосьВыгрузить - Число.
//
// Возвращаемое значение:
...
Функция ФильтрИзмененийНабораЗаписей(ОбъектМетаданных, Узел, ОсталосьВыгрузить)
	...
	Если Метаданные.РегистрыРасчета.Содержит(ОбъектМетаданных.Родитель()) Тогда
		ИмяТаблицы = "РегистрРасчета." + ОбъектМетаданных.Родитель().Имя + "." + ОбъектМетаданных.Имя
	Иначе
		ИмяТаблицы = ОбъектМетаданных.ПолноеИмя();
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого ПолеОтбора Из ПоляОтбора Цикл
			НаборЗаписей.Отбор[ПолеОтбора].Установить(Выборка[ПолеОтбора]);
		КонецЦикла;
	...
	Для Каждого КлючИЗначение Из СсылкиПоТипам Цикл
		...
		Для Каждого Ссылка Из КлючИЗначение.Значение Цикл
			...
			Если ПараметрыВыгрузки.ТипыСопоставляемыхОбщихДанныхПоПредопределенномуИмени[КлючИЗначение.Ключ] <> Неопределено
				...
				ВызватьИсключение СтрШаблон(НСтр("ru='Обнаружена ссылка на общие данные без поля поиска. Тип данных: %1';uk='Виявлене посилання на загальні дані без поля пошуку. Тип даних: %1'"), Объект.Метаданные().ПолноеИмя());
			КонецЕсли;
	...
	Для Каждого ПользовательИБ Из ПользователиИнформационнойБазы.ПолучитьПользователей() Цикл
		...
		Пока Истина Цикл
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				ШаблонОшибки = НСтр("ru='Выгрузка настройки %1 (пользователь: %2, ключ объекта: %3, ключ настроек: %4 ) пропущена по причине: %5';uk='Вивантаження настройки %1 (користувач: %2, ключ об''єкта: %3, ключ настройок: %4 ) пропущене через: %5'");
				...
				РегистрацияОшибки(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Комментарий);
				Продолжить;
			КонецПопытки;
...
Функция ПараметрыЗагрузки(КаталогЗагрузки, СоответствиеСсылок, ОбъектыДляОчистки)
	...
	Если ТаблицаДанных = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Отсутствует файл Data.xml';uk='Відсутній файл Data.xml'");
	КонецЕсли;
...
Функция ЗагрузитьПользовательскиеДанные(ПараметрыЗагрузки)
	...
	ШаблонНачалоЗагрузки = НСтр("ru='Загружается объект: %1';uk='Завантажується об''єкт: %1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	ШаблонОкончаниеЗагрузки = НСтр("ru='Загружен объект: %1, Количество: %2, Длительность (сек): %3';uk='Завантажений об''єкт: %1, Кількість: %2, Тривалість (сек): %3'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Для Каждого СтрокаДанных Из ПараметрыЗагрузки.ТаблицаДанных Цикл
		...
		РегистрацияПримечания(НСтр("ru='Статистика загрузки';uk='Статистика завантаження'"), Комментарий);
...
Функция ЗагрузитьСсылочныйТип(ОбъектМетаданных, Идентификатор, ПараметрыЗагрузки)
	...
	Пока ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		// Временный обход ошибки платформы: план видов расчета, план счетов и
		// возможно другие объекты не десериализуется в ОбъектXDTO.
...
Функция ЗагрузитьНаборЗаписей(ОбъектМетаданных, Идентификатор, ПараметрыЗагрузки)
	...
	Пока ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Если ЭтоРегистрБухгалтерии Тогда
			Объект = СериализаторXDTO.ПрочитатьXML(ЧтениеXML);
			ОбъектXDTO = СериализаторXDTO.ЗаписатьXDTO(Объект.Выгрузить());
			ЗаменитьСсылкиПриЗагрузке(ОбъектXDTO, ПараметрыЗагрузки);
			Объект.Загрузить(СериализаторXDTO.ПрочитатьXDTO(ОбъектXDTO));
			Если Объект.Количество() = 0 Тогда
				ЗаменитьСсылкиПриЗагрузке(Объект.Отбор.Регистратор.Значение, ПараметрыЗагрузки);
			Иначе
				Объект.Отбор.Регистратор.Значение = Объект[0].Регистратор;
			КонецЕсли;
		Иначе
...
Процедура ЗагрузитьНастройкиПользователей(ХранилищеНастроек, ИмяФайла, ПараметрыЗагрузки)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> "Настройки" Тогда
		ВызватьИсключение(НСтр("ru='Ошибка чтения XML. Неверный формат файла. Ожидается начало элемента ""Настройки"".';uk='Помилка читання XML. Неправильний формат файлу. Очікується початок елемента ""Настройка"".'"));
	КонецЕсли;
...
Процедура ПроверитьИспользованиеРазделениеСеанса()
	...
	Если Не РаботаВМоделиСервиса.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение НСтр("ru='Выполнение возможно только в разделенном сеансе.';uk='Виконання можливе тільки в розділеному сеансі.'");
	КонецЕсли;
...
Процедура ДобавитьВспомогательныеДанные(Значение, ПараметрыВыгрузки)
	...
	ИначеЕсли ПараметрыВыгрузки.ЗапрещенныеТипыОбщихДанных[ТипЗначения] <> Неопределено Тогда 
		...
		Если ЗначениеЗаполнено(Значение) Тогда
			ВызватьИсключение СтрШаблон(НСтр("ru='В объекте обнаружена недопустимая ссылка. Тип данных: %1';uk='В об''єкті виявлено неприпустиме посилання. Тип даних: %1'"), Значение.Метаданные().ПолноеИмя());
		КонецЕсли;
...
//						ОбъектМетаданных: ПланВидовРасчета - объект метаданных, для которого необходимо выполнить сопоставление.
...
Функция ТипПоляПоиска(ОбъектМетаданных, ИмяПоля)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru='Поле %1 не может использоваться в качестве поля естественного ключа объекта %2:
        ...
        |поле об''єкта не виявлено!'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		ИмяПоля,
		ОбъектМетаданных.ПолноеИмя());
	...
	Если Не Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если СпрОбъект.Пользователи.Найти(Выборка.Ссылка, "Пользователь") = Неопределено Тогда
				ПользовательОбъект.ДополнительныеСвойства.Вставить("СозданиеАдминистратора", НСтр("ru='Создание администратора области данных при миграции приложения.';uk='Створення адміністратора області даних при міграції програми.'"));
...
Процедура ЗарегистрироватьИзменения(Источник)
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользуетсяМиграцияПриложений") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСостояниеВыгрузки(СостояниеВыгрузки, СостояниеЗагрузки, УзелОбмена)
	...
	Если СостояниеЗагрузки.ЗавершеноСОшибками Тогда
		...
		СостояниеВыгрузки.Комментарий = НСтр("ru='При загрузке сообщения возникла ошибка:';uk='При завантаженні повідомлення виникла помилка:'") + Символы.ПС + СостояниеЗагрузки.ОписаниеОшибки;
	КонецЕсли;
...
Функция КоличествоИзмененных(Узел)
	...
	Для Каждого Состав Из МиграцияПриложенийПовтИсп.ВыгружаемыеОбъекты() Цикл
		...
		ИмяТаблицы = СтрЗаменить(Состав.Ключ.ПолноеИмя(), ".Перерасчет.", ".");
...
Процедура ПроверитьТипПрочитанногоОбъекта(Объект, ОбъектМетаданных)
	...
	Если ОбъектМетаданных <> Объект.Метаданные() Тогда
		ВызватьИсключение НСтр("ru='Тип прочитанного объекта не соответствует объявленному.';uk='Тип прочитаного об''єкта не відповідає оголошеному'");
	КонецЕсли;
...
Процедура ЗавершитьВыгрузкуСОшибкой(СостояниеВыгрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru='Выгрузка';uk='Вивантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура ЗавершитьЗагрузкуСОшибкой(СостояниеЗагрузки, ПодробноеПредставлениеОшибки, КраткоеПредставлениеОшибки)
	...
	РегистрацияОшибки(НСтр("ru='Загрузка';uk='Завантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), ПодробноеПредставлениеОшибки);
...
Процедура ЗавершитьЗагрузку(СостояниеЗагрузки)
	...
	Для Каждого ОписаниеПользователя Из СостояниеЗагрузки.Пользователи.Получить() Цикл
...
Процедура РегистрацияОшибки(Событие, Комментарий)
	...
	ИмяСобытия = СтрШаблон(НСтр("ru='Миграция приложений.%1';uk='Міграція прикладних програм.%1'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), Событие);
...
Процедура ПроверитьВыполняетсяЗагрузка()
	...
	Если Не Выборка.Следующий() Или ЗначениеЗаполнено(Выборка.ДатаЗавершения) Тогда
		ВызватьИсключение НСтр("ru='Загрузка сообщений не выполняется.';uk='Завантаження повідомлень не виконується.'");
	КонецЕсли;
...
Процедура ВызватьИсключениеНеизвестныйОбъектМетаданных(Знач ОбъектМетаданных)
	...
	ВызватьИсключение СтрШаблон(НСтр("ru='Неизвестный объект метаданных: %1';uk='Невідомий об''єкт метаданих: %1'"), ОбъектМетаданных.ПолноеИмя());
...
Процедура ЗарегистрироватьИзмененияПовторно(УзелОбмена, НомерСообщения)
	...
	Для Каждого Состав Из МиграцияПриложенийПовтИсп.ВыгружаемыеОбъекты() Цикл
		...
		ИмяТаблицы = СтрЗаменить(ОбъектМетаданных.ПолноеИмя(), ".Перерасчет.", ".");
