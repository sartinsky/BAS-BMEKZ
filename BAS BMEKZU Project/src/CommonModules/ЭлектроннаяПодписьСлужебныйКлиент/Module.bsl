...
Процедура НайтиУстановленныеПрограммыЦиклПродолжение(Менеджер, Контекст) Экспорт
	...
	Если Ошибка.НеУказанПуть Тогда
		ОбновитьЗначение(ОписаниеПрограммы.РезультатПроверкиНаКлиенте, НСтр("ru='Не указан путь к программе.';uk='Не вказаний шлях до програми.'"));
	...
	Если Не Подключено Тогда
		ТекстОшибки =
			НСтр("ru='В браузере требуется установить расширение
                       ...
                       |;uk='У браузері потрібно встановити розширення
		...
		Если Контекст.Программа <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Программа ""%1"" не доступна или не установлена на компьютере по причине:
                           |%2'
                           |;uk='Програма ""%1"" не доступна або не встановлена на комп''ютері з причини:
                           |%2'"),
				Контекст.Программа,
				...
				ТекстОшибки = ТекстОшибки + Символы.ПС + Символы.ПС
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Программу ""%1"" не удалось использовать по причине:
                                   |%2'
                                   |;uk='Програму ""%1"" не вдалося використати з причини:
                                   |%2'"),
						Ошибка.Программа,
						Ошибка.Описание);
			КонецЦикла;
...
Процедура СохранитьДанныеВместеСПодписьюПослеСохраненияФайлаДанных(Результат, Контекст) Экспорт
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru='При записи файла возникла ошибка:';uk='При запису файлу виникла помилка:'") + Символы.ПС + Результат.ОписаниеОшибки);
	...
	Если Контекст.НаСторонеКлиента Тогда
		...
		Если ЭтоXMLDSig Тогда
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					ЭлектроннаяПодписьСлужебныйКлиентСервер.ЗаголовокОшибкиПолученияДанных(Контекст.Операция)
					+ Символы.ПС + НСтр("ru='Внешняя компонента XMLDSig может использоваться только для подписания.';uk='Зовнішня компонента XMLDSig може використовуватися тільки для підписання.'"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
...
Процедура НачатьПроверкуПодписиПослеИнициализацииСертификата(СертификатКриптографии, Контекст) Экспорт
	...
	ПараметрыПодключения.ТекстПояснения = НСтр("ru='Для подписания XML необходима установка компоненты XMLDSig';uk='Для підписання XML необхідне встановлення компоненти XMLDSig'");
	...
	Иначе
		...
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда 
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ru='Операция невозможна. Требуется установка компоненты для HTTP-запросов.';uk='Операція неможлива. Потрібне встановлення компоненти для HTTP-запитів.'"));
		...
		Иначе 
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Операция невозможна. %1';uk='Операція неможлива. %1'"), Результат.ОписаниеОшибки));
...
Процедура ПослеПолученияИнформацииМодуляКриптографии(ИнформацияМодуляКриптографии, Контекст) Экспорт
	...
	Если ОписаниеПрограммы = Неопределено Тогда
		ЗавершитьОперациюСОшибкой(Контекст,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось определить тип криптопровайдера %1';uk='Не вдалося визначити тип криптопровайдера %1'"),
...
Процедура ПослеУстановкиСвойстваЗапретитьПользовательскийИнтерфейс(Контекст) Экспорт
	...
	Иначе
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Не установлен режим работы компоненты XMLDSig.';uk='Не встановлений режим роботи компоненти XMLDSig.'"));
	КонецЕсли;
...
Процедура НачатьПодписаниеСообщенияSOAP(Контекст)
	...
	Исключение
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';uk='Помилка виклику методу GetSignOIDFromCert компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Подписание_ПослеВыполненияGetSignOIDFromCert(OIDАлгоритмаПодписи, Параметры, Контекст) Экспорт
	...
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода GetSignOIDFromCert произошла ошибка:';uk='При виконанні методу GetSignOIDFromCert сталася помилка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Иначе
		...
		Исключение
			...
			ЗавершитьОперациюСОшибкой(
				Контекст,
				НСтр("ru='Ошибка вызова метода C14N компоненты XMLDSig.';uk='Помилка виклику методу C14N компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияC14N_ПодписываемыйТег(КаноникализированныйТекстXML, Параметры, Контекст) Экспорт
	...
	Если КаноникализированныйТекстXML = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода C14N произошла ошибка:';uk='При виконанні методу C14N сталася помилка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода Hash компоненты XMLDSig.';uk='Помилка виклику методу Hash компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияHash_ПодписываемыйТег(DigestValue, Параметры, Контекст) Экспорт
	...
	Если DigestValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода Hash произошла ошибка:';uk='При виконанні методу Hash сталася помилка:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Подписание_ПослеВыполненияC14N_SignedInfo(КаноникализированныйТекстXMLSignedInfo, Параметры, Контекст) Экспорт
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода Sign компоненты XMLDSig.';uk='Помилка виклику методу Sign компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Подписание_ПослеВыполненияSign(SignatureValue, Параметры, Контекст) Экспорт
	...
	Если SignatureValue = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода Sign произошла ошибка:';uk='При виконанні методу Sign сталася помилка:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура Проверка_ПослеВыполненияC14N_SignedInfo(КаноникализированныйТекстXMLSignedInfo, Параметры, Контекст) Экспорт
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';uk='Помилка виклику методу VerifySign компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура Проверка_ПослеВыполненияVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если ПодписьВерна = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru='При выполнении метода VerifySign произошла ошибка:';uk='При виконанні методу VerifySign сталася помилка:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Подпись не верна (SignatureValue некорректно.';uk='Підпис не вірний (SignatureValue некоректно.'"));
...
Процедура Проверка_ПослеВыполненияHash_ПодписываемыйТег(HashValue, Параметры, Контекст) Экспорт
	...
	Если Не ПодписьВерна Тогда
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).';uk='Підпис не вірний (SignatureValue коректно, відрізняється DigestValue).'"));
...
Процедура НачатьПолучениеТекстаОшибки(НачалоОписанияТекстаОшибки, Контекст)
	...
	Исключение
		...
		ЗавершитьОперациюСОшибкой(
			Контекст,
			НСтр("ru='Ошибка вызова метода GetLastError компоненты XMLDSig.';uk='Помилка виклику методу GetLastError компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
