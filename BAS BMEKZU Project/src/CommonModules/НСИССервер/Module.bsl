...
Процедура ОбновитьРегистр(ИмяРегистра, Синоним, Структура, Записи, ВыводитьПротокол, ВФоне)
	...
	Для Каждого Запись ИЗ Записи Цикл
		...
		// ИНАГРО ++
		Если СтруктураСтрока.Свойство("ПоЧасовая") Тогда
			ЗаписьВНаборе.ИНАГРО_ПоЧасовая = СтруктураСтрока.ПоЧасовая;
		КонецЕсли;
		// ИНАГРО --
	КонецЦикла;
	...
	Если ТаблицаЗаписей.Количество() > 0 тогда
		...
		КонецЕсли;
		Если ИмяРегистра = "ПределыСтраховыхВзносов" Тогда // ИНАГРО ++
			
			Для Каждого Запись из ТаблицаЗаписей Цикл
				Если Запись.ВидЕСВ = Перечисления.ВидыЕСВ.ОсновнаяЗарплата Тогда
					// ЕСВФОТБюджетники
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТБюджетники;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТИнвалиды50
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТИнвалиды50;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТИнвалиды
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТИнвалиды;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТРаботникиУТОГУТОС
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТРаботникиУТОГУТОС;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					...
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТРаботники;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТРаботникиСР
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТРаботникиСР;
					ТекущаяЗапись.Записать(Истина);
										
					// ЕСВФОТМобил
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТМобил;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТРаботникиЗаПределами
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТРаботникиЗаПределами;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТРаботникиИностранные
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТРаботникиИностранные;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТРаботникиФЛ
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТРаботникиФЛ;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТИнвалидыСР
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТИнвалидыСР;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					...
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВИнвалидыУТОГУТОС;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					...
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВРаботникиСР;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					...
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВРаботники;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					
				ИначеЕсли Запись.ВидЕСВ = Перечисления.ВидыЕСВ.ПоДоговорамГПХ Тогда
					
					// ЕСВФОТГПХ
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТГПХ;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					...
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВГПХ;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					
				ИначеЕсли Запись.ВидЕСВ = Перечисления.ВидыЕСВ.Больничные Тогда
					
					// ЕСВФОТБольничные
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТБольничные;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТБольничныеИнвалиды50
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТБольничныеИнвалиды50;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТБольничныеИнвалиды
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТБольничныеИнвалиды;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					...
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВБольничные;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					
				ИначеЕсли Запись.ВидЕСВ = Перечисления.ВидыЕСВ.Декретные Тогда
					
					// ЕСВФОТДекретные
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТДекретные;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТДекретныеИнвалиды50
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТДекретныеИнвалиды50;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					
					// ЕСВФОТДекретныеИнвалиды
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТДекретныеИнвалиды;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					...
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВФОТДекретныеУТОГ;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
					...
					ТекущаяЗапись = РегистрыСведений[ИмяРегистра].СоздатьМенеджерЗаписи();
					ЗаполнитьЗначенияСвойств(ТекущаяЗапись, Запись);
					ТекущаяЗапись.ИНАГРО_Налог = Справочники.Налоги.ЕСВДекретные;
					ТекущаяЗапись.МинимальнаяБаза = 0;
					ТекущаяЗапись.Записать(Истина);
...
Процедура ВыполнитьОбновлениеКлассификаторов(ВыводитьПротокол = Ложь, ВФоне = Ложь, СписокФайлов = Неопределено) Экспорт
	...
	Для Каждого Файл из РезультатЗапроса Цикл
		...
		Иначе	
			ЗаписатьФайлВБазу(ИмяФайла, ИмяФайла, ДвоичныеДанные, Версия, Тип, ВыводитьПротокол, ВФоне);
		КонецЕсли;	
...
Процедура ПолучитьСписокДополнительныхФайлов(ВыводитьПротокол, ВФоне) Экспорт
	...
	ФС = Новый Структура("ИмяФайла, Версия, Описание");
	...
	Для Каждого Файл из РезультатЗапроса Цикл
		...
		Если ОбщегоНазначения.РазделениеВключено() И ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() И СтрНайти(ИмяФайла, "РегламентированныйОтчет") > 0 Тогда
			Продолжить;
		КонецЕсли;	
