			...
			Исключение
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='При установке исправления ""%1"" произошла ошибка:
                               |
                               |%2'
                               |;uk='При встановленні виправлення ""%1"" сталася помилка:
                               |
			...
			Исключение
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось удалить исправление ""%1"" по причине:
                               |
                               |%2'
                               |;uk='Не вдалося видалити виправлення ""%1"" з причини:
                               |
                               |%2'"), Расширение.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
				ЗаписьЖурналаРегистрации(НСтр("ru='Исправления.Удаление';uk='Виправлення.Видалення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка())
					, УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
			КонецПопытки;
		КонецЦикла;
	КонецЕсли;
...
Функция ИсправленияИзменены() Экспорт
					...
					Исключение
						...
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='При записи исправления ""%1"" произошла ошибка:
                                       |
                                       |%2'
                                       |;uk='При запису виправлення ""%1"" сталася помилка:
                                       |
                                       |%2'"), Исправление.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
						ЗаписьЖурналаРегистрации(НСтр("ru='Исправления.Изменение';uk='Виправлення.Зміна'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
							УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
						ВызватьИсключение ТекстОшибки;
					КонецПопытки;
				КонецЕсли;
			...
			Исключение
				...
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось удалить отключенное исправление ""%1"" по причине:
                               |
                               |%2'
                               |;uk='Не вдалося видалити відключене виправлення ""%1"" з причини:
                               |
                               |%2'"), Расширение.Имя, КраткоеПредставлениеОшибки(ИнформацияОбОшибке));
				ЗаписьЖурналаРегистрации(НСтр("ru='Исправления.Удаление';uk='Виправлення.Видалення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					УровеньЖурналаРегистрации.Ошибка,,, ТекстОшибки);
	...
	Если Изменения.УдаленныеИсправления.Количество() > 0 Тогда
		Заголовок = НСтр("ru='Удалены устаревшие патчи';uk='Видалені застарілі патчі'");
	...
	Если Изменения.НеактивныеИсправления.Количество() > 0 Тогда
		Заголовок = НСтр("ru='Удалены неактивные патчи';uk='Видалені неактивні патчі'");
	...
	Если Изменения.ОтключенаЗащита.Количество() > 0 Тогда
		Заголовок = НСтр("ru='Отключены предупреждения об опасных действиях';uk='Відключені попередження про небезпечні дії'");
		Если ЗначениеЗаполнено(ОписаниеИзменений) Тогда
			ОписаниеИзменений = ОписаниеИзменений + Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Если Изменения.СнятБезопасныйРежим.Количество() > 0 Тогда
		Заголовок = НСтр("ru='Отключен безопасный режим';uk='Відключено безпечний режим'");
		Если ЗначениеЗаполнено(ОписаниеИзменений) Тогда
			ОписаниеИзменений = ОписаниеИзменений + Символы.ПС + Символы.ПС;
		КонецЕсли;
	...
	Результат = Новый Структура;
	Результат.Вставить("ЕстьИзменения", ИсправленияИзменены);
	Результат.Вставить("ОписаниеИзменений", ОписаниеИзменений);
