...
КонецПроцедуры
Процедура ПрочитатьЗаписьДляРедактированияВФормеНаТекущуюДату(Форма, ИмяРегистра, ВедущийОбъект) Экспорт
	...
	ПрочитатьЗаписьДляРедактированияВФормеПоСтруктуреНаТекущуюДату(Форма, ИмяРегистра, СтруктураВедущихОбъектов);
...
КонецПроцедуры
Процедура ПрочитатьЗаписьДляРедактированияВФормеПоСтруктуреНаТекущуюДату(Форма, ИмяРегистра, СтруктураВедущихОбъектов) Экспорт
	
	МенеджерЗаписи = МенеджерПоследнейЗаписиНаТекущуюДату(ИмяРегистра, СтруктураВедущихОбъектов);
...
КонецФункции
Функция МенеджерПоследнейЗаписиНаТекущуюДату(ИмяРегистра, СтруктураВедущихОбъектов) Экспорт
	...
	// Ищем последнюю запись на текущую дату
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ТекущаяДата", ОбщегоНазначенияБП.ПолучитьРабочуюДату());
	ПоляЗапроса = 
			"
			|	РегистрСведений.Период";
	Для каждого Измерение Из МетаданныеРегистра.Измерения Цикл
	...
	"
	|ИЗ
	|	РегистрСведений." + ИмяРегистра + ".СрезПоследних(&ТекущаяДата) КАК РегистрСведений";
	...
	Если НЕ ОбщегоНазначения.КоллекцииИдентичны(ТаблицаИсходногоНабора, ТаблицаНовогоНабора, ,ИменаКолонокИсключаемыхИзСравнения) Тогда
		...
		Для Каждого СтрокаТаблицаНовогоНабора Из ТаблицаНовогоНабора Цикл
			...
			Если СохранитьСтроку Тогда
				СохранитьЗаписьНабора(СтрокаТаблицаНовогоНабора, ИмяРегистра, СтруктураВедущихОбъектов, ДополнительныеСвойства)
			КонецЕсли; 
