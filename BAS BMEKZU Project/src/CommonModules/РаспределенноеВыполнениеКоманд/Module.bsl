...
Процедура ВыполнитьКомандуДополнительнойОбработки(ИдентификаторОбработки, ИдентификаторКоманды, ИдентификаторОперации, СообщитьМенеджеру = Ложь) Экспорт
	...
	Событие = Событие + НСтр("ru='Начало выполнения команды';uk='Початок виконання команди'"); 
	Комментарий = НСтр("ru='Начинаем выполнение команды %1 (обработка %2, вызов %3).';uk='Починаємо виконання команди %1 (обробка %2, виклик %3).'");
	...
	Если Выборка.Следующий() Тогда
		...
		Попытка
			...
			РезультатВыполнения = НСтр("ru='Команда выполнена успешно.';uk='Команда виконана успішно.'");
			
			Событие = ИмяСобытияЖурналаРегистрации() + ".";
			Событие = Событие + НСтр("ru='Завершение выполнения команды';uk='Завершення виконання команди'"); 
			Комментарий = НСтр("ru='Выполнение команды %1 (обработка %2, вызов %3) завершено.';uk='Виконання команди %1 (обробка %2, виклик %3) завершено.'");
			Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Комментарий, ИдентификаторКоманды, 
			ИдентификаторОбработки, ИдентификаторОперации);
			ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации.Информация,,, Комментарий);	
		...
		Исключение
			...
			Событие = НСтр("ru='Ошибка выполнения команды';uk='Помилка виконання команди'"); 
			Комментарий = НСтр("ru='При выполнении команды возникла ошибка';uk='При виконанні команди виникла помилка'"); 
	...
	Иначе
		ОшибкаВыполнения = Истина;
		Событие = НСтр("ru='Обработка не найдена';uk='Обробку не знайдено'"); 
		Комментарий = НСтр("ru='Не удалось найти внешнюю обработку по идентификатору';uk='Не вдалося знайти зовнішню обробку за ідентифікатором'"); 
...
Функция ВыполнитьПередачуФайлаПриложению(ИмяФайла, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ Указатель.Существует() Тогда
		Комментарий = НСтр("ru='Файл с указанным именем не найден на файловой системе.';uk='Файл із зазначеним ім''ям не знайдений на файловій системі.'",Локаль); 
	...
	Если НЕ ЗначениеЗаполнено(КодПолучателя) И НЕ ТипЗнч(КодПолучателя) = Тип("Число") Тогда
		Комментарий = НСтр("ru='Параметр КодПолучателя не содержит валидный код области данных.';uk='Параметр КодПолучателя не містить валідний код області даних.'",Локаль); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Процедура ВыслатьКвитанциюПередачиФайла(ИдентификаторВызова, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторВызова) ИЛИ НЕ ТипЗнч(ИдентификаторВызова) = Тип("УникальныйИдентификатор") Тогда
		Комментарий = НСтр("ru='Параметр ИдентификаторВызова не содержит валидный идентификатор.';uk='Параметр ИдентификаторВызова не містить валідний ідентифікатор.'",Локаль); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru='Распределенное выполнение команд';uk='Розподілене виконання команд'"); 
