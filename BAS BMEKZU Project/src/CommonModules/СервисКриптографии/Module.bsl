...
Функция Расшифровать(ЗашифрованныеДанные, Сертификат, ТипШифрования = "CMS", ПараметрыШифрования = Неопределено) Экспорт
	...
	Попытка
		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			ВызватьИсключение(НСтр("ru='Не удалось выполнить расшифровку сообщения';uk='Не вдалося виконати розшифровку повідомлення'"));
		КонецЕсли;
...
Функция РасшифроватьПереборомСертификатов(ЗашифрованныеДанные, ТипШифрования = "CMS", ПараметрыШифрования = Неопределено) Экспорт
	...
	Если СвойстваСообщения.Тип = "envelopedData" Тогда
		...
		КонецЦикла;			
	Иначе
		ВызватьИсключение(НСтр("ru='Недопустимое значение параметра ЗашифрованныеДанные - файл не является криптосообщением';uk='Неприпустиме значення параметра ЗашифрованныеДанные - файл не є криптоповідомленням'"));
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Идентификаторы) Тогда
		ВызватьИсключение(НСтр("ru='В хранилище отсутствуют сертификаты для расшифровки сообщения.';uk='У сховищі відсутні сертифікати для розшифровки повідомлення.'"));
	КонецЕсли;
...
Функция РасшифроватьБлок(ЗашифрованныеДанные, Получатель, КлючеваяИнформация, ПараметрыШифрования = Неопределено) Экспорт
	...
	Попытка
		...
		Если Не ЗначениеЗаполнено(Сертификат) Тогда
			ВызватьИсключение(НСтр("ru='Сертификат подписанта не найден в хранилище сертификатов.';uk='Сертифікат підписувача не знайдений у сховищі сертифікатів.'"));
		КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(Результат) Тогда
			ВызватьИсключение(НСтр("ru='Не удалось выполнить расшифровку блока данных';uk='Не вдалося виконати розшифровку блоку даних'"));
		КонецЕсли;
...
Функция ПолучитьСертификатыИзПодписи(Подпись) Экспорт
	...
	Иначе
		ВызватьИсключение(НСтр("ru='Параметр <Подпись> не является файлом подписи';uk='Параметр <Підпис> не є файлом підпису'"));
	КонецЕсли;
...
Функция ПолучитьДвоичныеДанныеСертификата(Сертификат)
	...
	Если Не ЗначениеЗаполнено(ДвоичныеДанныеСертификата) Тогда
		ВызватьИсключение(НСтр("ru='Не удалось извлечь двоичные данные сертификата';uk='Не вдалося витягти двійкові дані сертифіката'"));
	КонецЕсли;
...
Функция ВызватьHTTPМетод(Соединение, МетодHTTP, Запрос)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронная подпись в модели сервиса.Сервис криптографии.Выполнение запроса';uk='Електронний підпис моделі сервісу.Сервіс криптографії.Виконання запиту'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
...
Функция МаркерыБезопасности(ИдентификаторСертификата, ИспользоватьДлительныйМаркерБезопасности)
...
Процедура ЗашифроватьПроверкаВходящихПараметров(Данные, Получатели, ТипШифрования, ПараметрыШифрования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		ТипШифрования = "CMS",
		НСтр("ru='Недопустимое значение параметра ТипШифрования (неизвестный тип шифрования)';uk='Неприпустиме значення параметра ТипШифрования (невідомий тип шифрування)'"), 
		"СервисКриптографии.Зашифровать");
	...
	Если ТипЗнч(Данные) = Тип("Строка") Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(
				...
				НСтр("ru='Недопустимое значение параметра Данные (указан адрес, который не является адресом временного хранилища)';uk='Неприпустиме значення параметра Дані (зазначено адресу, яка не є адресою тимчасового сховища)'"), 
				"СервисКриптографии.Зашифровать");
	КонецЕсли;	
	...
	Для Каждого Элемент Из Данные Цикл
		...
		Если ТипЗнч(Элемент) = Тип("Строка") Тогда
			ОбщегоНазначенияКлиентСервер.Проверить(
				...
				СтрШаблон(НСтр("ru='Недопустимое значение параметра Данные[%1] (указан адрес, который не является адресом временного хранилища)';uk='Неприпустиме значення параметра Дані[%1] (зазначено адресу, яка не є адресою тимчасового сховища)'"), Индекс), 
				"СервисКриптографии.Зашифровать");
		КонецЕсли;
		Индекс = Индекс + 1;
	КонецЦикла;
...
Процедура РасшифроватьПроверкаВходящихПараметров(ЗашифрованныеДанные, ТипШифрования, ПараметрыШифрования)
	...
	Если ТипЗнч(ЗашифрованныеДанные) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru='Недопустимое значение параметра ЗашифрованныеДанные (указан адрес, который не является адресом временного хранилища)';uk='Неприпустиме значення параметра ЗашифрованныеДанные (зазначено адресу, яка не є адресою тимчасового сховища)'"), 
			"СервисКриптографии.Расшифровать");
	КонецЕсли;
...
Процедура РасшифроватьБлокПроверкаВходящихПараметров(ЗашифрованныеДанные, Получатель, КлючеваяИнформация, ПараметрыШифрования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru='Отсутствует одно и/или более обязательных свойств параметра КлючеваяИнформация (ephemeral_key|session_key|iv_data)';uk='Відсутня одна і/або більше обов''язкових властивостей параметра КлючеваяИнформация (ephemeral_key|session_key|iv_data)'"), 
		"СервисКриптографии.РасшифроватьБлок");
...
Процедура ХешированиеДанныхПроверкаВходящихПараметров(Данные, АлгоритмХеширования, ПараметрыХеширования)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru='Недопустимое значение параметра АлгоритмХеширования (неизвестный алгоритм хеширования)';uk='Неприпустиме значення параметра АлгоритмХеширования (невідомий алгоритм хешування)'"), 
		"СервисКриптографии.ХешированиеДанных");
...
Процедура ПодписатьПроверкаВходящихПараметров(Данные, Подписант, ТипПодписи, ПараметрыПодписания)
	...
	ОбщегоНазначенияКлиентСервер.Проверить(
		...
		НСтр("ru='Недопустимое значение параметра ТипПодписи (неизвестный тип подписи)';uk='Неприпустиме значення параметра ТипПодписи (невідомий тип підпису)'"), 
		"СервисКриптографии.Подписать");
...
Процедура ПроверитьПодписьПроверкаВходящихПараметров(Подпись, Данные, ТипПодписи, ПараметрыПодписания)
	...
	Если ТипЗнч(Подпись) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru='Недопустимое значение параметра Подпись (указан адрес, который не является адресом временного хранилища)';uk='Неприпустиме значення параметра Підпис (зазначено адресу, яка не є адресою тимчасового сховища)'"), 
			"СервисКриптографии.ПроверитьПодпись");
	КонецЕсли;
	...
	Если ТипПодписи = "GOST3410" Тогда 
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru='Для проверки подписи по ГОСТ Р 34.10-94 необходимо указание сертификата';uk='Для перевірки підпису по ГОСТ Р 34.10-94 необхідне зазначення сертифіката'"), 
			"СервисКриптографии.ПроверитьПодпись");
	КонецЕсли;
...
Процедура ПолучитьСвойстваКриптосообщенияПроверкаВходящихПараметров(Криптосообщение)
	...
	Если ТипЗнч(Криптосообщение) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru='Недопустимое значение параметра Криптосообщение (указан адрес, который не является адресом временного хранилища)';uk='Неприпустиме значення параметра Криптосообщение (зазначено адресу, яка не є адресою тимчасового сховища)'"), 
			"СервисКриптографии.ПолучитьСвойстваКриптосообщения");
	КонецЕсли;
...
Процедура ПолучитьВременныйПарольПроверкаВходящихПараметров(ИдентификаторСертификата, ПовторнаяОтправка, СпособДоставкиПаролей)
	...
	Если ТипЗнч(СпособДоставкиПаролей) = Тип("Строка") Тогда
		ОбщегоНазначенияКлиентСервер.Проверить(
			...
			НСтр("ru='Недопустимое значение параметра СпособДоставкиПаролей, допустимые значения ""phone"" или ""email""';uk='Неприпустиме значення параметра СпособДоставкиПаролей, припустимі значення ""phone"" або ""email""'"), 
			"СервисКриптографии.ПолучитьВременныйПароль");
	КонецЕсли;
...
Функция ИмяСобытияШифрование()
	...
	Возврат НСтр("ru='Сервис криптографии.Шифрование';uk='Сервіс криптографії.Шифрування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияШифрованиеБлока()
	
	Возврат НСтр("ru='Сервис криптографии.Шифрование блока';uk='Сервіс криптографії.Шифрування блоку'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияРасшифровка()
	
	Возврат НСтр("ru='Сервис криптографии.Расшифровка';uk='Сервіс криптографії.Розшифровка'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияРасшифровкаБлока()
	
	Возврат НСтр("ru='Сервис криптографии.Расшифровка блока';uk='Сервіс криптографії.Розшифровка блоку'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияАутентификация()
	
	Возврат НСтр("ru='Сервис криптографии.Аутентификация';uk='Сервіс криптографії.Аутентифікація'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияСвойстваКриптосообщения()
	
	Возврат НСтр("ru='Сервис криптографии.Свойства криптосообщения';uk='Сервіс криптографії.Властивості криптоповідомлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияПроверкаСертификата()
	
	Возврат НСтр("ru='Сервис криптографии.Проверка сертификата';uk='Сервіс криптографії.Перевірка сертифіката'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияСвойстваСертификата()
	
	Возврат НСтр("ru='Сервис криптографии.Свойства сертификата';uk='Сервіс криптографії.Властивості сертифіката'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияПроверкаПодписи()
	
	Возврат НСтр("ru='Сервис криптографии.Проверка подписи';uk='Сервіс криптографії.Перевірка підпису'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияХеширование()
	
	Возврат НСтр("ru='Сервис криптографии.Хеширование';uk='Сервіс криптографії.Хешування'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция ИмяСобытияПодписание()
	
	Возврат НСтр("ru='Сервис криптографии.Подписание';uk='Сервіс криптографії.Підписання'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
