	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "КодКАТОТТГ" Тогда
			Если НЕ (НЕ ЗначениеЗаполнено(Организация)) Тогда 
				...
				КодыОрганизации = РегистрыСведений.КодыОрганизации.СрезПоследних(ДатаЗначения, Данные);
				Если КодыОрганизации.Количество()> 0 Тогда
					Результат = КодыОрганизации[0].КодКАТОТТГ;
				КонецЕсли;
			КонецЕсли;	
			Если Результат <> Неопределено Тогда
				Значение = Результат;
		...
		ИначеЕсли ИмяПоказателя = "Индекс" Тогда
			Если ЗначениеЗаполнено(Организация) Тогда 
				...
				Если НайденныеСтроки.Количество() > 0 Тогда
					...
					АдресXDTO = Обработки.РасширенныйВводКонтактнойИнформации.АдресXMLВXDTO(НайденныеСтроки[0].ЗначенияПолей);
					Результат = Обработки.РасширенныйВводКонтактнойИнформации.ПочтовыйИндексАдреса(АдресXDTO);
...
Функция ПолучитьТелефонФаксОрганизацииПоПолямБП(Знач Организация, Знач ВидКИ = "Телефон") Экспорт
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		Результат = УправлениеКонтактнойИнформациейСлужебный.ДесериализацияТелефона(НайденныеСтроки[0].ЗначенияПолей).Состав;
		Возврат Результат;
...
//  Возврат "БПКОРП";
//
Функция ИДКонфигурации(basERP = ЛОЖЬ) Экспорт
	
	ИДКонфигурации =  Метаданные.Имя;
	Если    Найти(ВРег(ИДКонфигурации), ВРег("BASАгроБухгалтерия")) 
		ИЛИ Найти(ВРег(ИДКонфигурации), ВРег("BASБухгалтерияЭлеватораМельницыИКомбикормовогоЗавода")) 
		ИЛИ Найти(ВРег(ИДКонфигурации), ВРег("КомплексноеРешениеСельскохозяйственногоПредприятия"))> 0 Тогда
		Возврат "БСП";		
	ИначеЕсли Найти(ВРег(ИДКонфигурации), ВРег("БухгалтерияДляУкраины")) > 0
		ИЛИ Найти(ВРег(ИДКонфигурации), ВРег("BASБухгалтерия")) > 0
