	...
	Если Не РолиАдминистратораДоступны() Тогда
		Возврат КраткоеПредставлениеОшибкиАвторизацииПослеРегистрацииВЖурнале(
			НСтр("ru='Запуск от имени пользователя с правом Администрирование невозможен,
                       ...
                       |режим конфигурирования для этого использовать не следует.'
                       |;uk='Запуск від імені користувача з правом Адміністрування неможливий,
                       |так як він не зареєстрований у списку користувачів.
                       |
                       |Для ведення списку і настройки прав користувачів призначений список Користувачі
                       |режим конфігурування для цього використовувати не слід.'"),
			, РегистрироватьВЖурнале);
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка в процедуре ПользователиСлужебный.ОбработатьИнтерфейсРолей()
                       ...
                       |Невірне значення параметра Действие: ""%1"".'"),
			Действие);
	КонецЕсли;
...
Процедура ПроверитьНазначениеРолей(НазначениеРолей = Неопределено, ПроверитьВсе = Ложь, СписокОшибок = Неопределено) Экспорт
			...
			Для Каждого ОписаниеНазначения Из Назначение Цикл
				...
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%1"" указана более чем в одном назначении:
                               |%2, %3.'
                               |;uk='Роль ""%1"" вказана більш ніж в одному призначенні:
                               |%2, %3.'"),
	...
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.ТолькоДляВнешнихПользователей, ТекстОшибки,
		НСтр("ru='При проверке ролей только для внешних пользователей найдены ошибки:';uk='При перевірці ролей тільки для зовнішніх користувачів знайдені помилки:'"), СписокОшибок);
	
	ПроверитьСоставПравРолей(НедоступныеПрава, Назначение.СовместноДляПользователейИВнешнихПользователей, ТекстОшибки,
		НСтр("ru='При проверке ролей совместно для пользователей и внешних пользователей найдены ошибки:';uk='При перевірці ролей спільно для користувачів і зовнішніх користувачів знайдені помилки:'"), СписокОшибок);
	...
	Если ОбщегоНазначения.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ru='При проверке ролей для пользователей приложения найдены ошибки:';uk='При перевірці ролей для користувачів програми знайдені помилки:'"), СписокОшибок, НеразделенныеДанные);
	КонецЕсли;
	Если Не ОбщегоНазначения.РазделениеВключено() Или ПроверитьВсе Тогда
		...
		ПроверитьСоставПравРолей(НедоступныеПрава, Роли, ТекстОшибки,
			НСтр("ru='При проверке ролей для пользователей найдены ошибки:';uk='При перевірці ролей для користувачів знайдені помилки:'"), СписокОшибок);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.1.3.16";            // ИНАГРО
	Обработчик.НачальноеЗаполнение = Истина;   // ИНАГРО
...
Функция ПодсказкаДляНовогоПароля() Экспорт
	...
	Возврат
		НСтр("ru='Надежный пароль:
                   ...
                   |- містить будь-які 3 з 4-х типів символів: великі
                   |літери, малі літери, цифри, спеціальні символи;
                   |- не збігається з ім''ям (для входу).'");
...
Функция ПодсказкаПотребоватьСменуПароляПриВходе(ДляВнешнихПользователей) Экспорт
	...
	Подсказка.Добавить(НСтр("ru='Требования к длине и сложности пароля задаются отдельно.';uk='Вимоги до довжини і складності пароля задаються окремо.'"));
	Подсказка.Добавить(Символы.ПС);
	
	Если Не ЭтоПолноправныйПользователь Тогда
		Подсказка.Добавить(НСтр("ru='За подробностями обратитесь к администратору.';uk='За подробицями звертайтеся до адміністратора.'"));
		Возврат Новый ФорматированнаяСтрока(Подсказка);
	КонецЕсли;
	
	Подсказка.Добавить(НСтр("ru='См.';uk='Див.'"));
	Подсказка.Добавить(" ");
	
	Если Не ПользователиСлужебныйПовтИсп.Настройки().ОбщиеНастройкиВхода Тогда
		Подсказка.Добавить(НСтр("ru='Параметры информационной базы в конфигураторе в меню Администрирование.';uk='Параметри інформаційної бази в конфігураторі в меню Адміністрування.'"));
		Возврат Новый ФорматированнаяСтрока(Подсказка);
	КонецЕсли;
	
	Если ДляВнешнихПользователей Тогда
		Ссылка = НСтр("ru='Настройки входа внешних пользователей';uk='Настройка входу зовнішніх користувачів'");
	Иначе
		Ссылка = НСтр("ru='Настройки входа пользователей';uk='Настройки входу користувачів'");
	КонецЕсли;
	...
	Если Метаданные.Подсистемы.Найти("Администрирование") <> Неопределено Тогда
		...
		Подсказка.Добавить(НСтр("ru='в разделе Администрирование,
            ...
            |;uk='у розділі Адміністрування,
            |пункт Настройки прав і користувачів.'"));
	Иначе
		Подсказка.Добавить(".");
	КонецЕсли;
		...
		Попытка
			...
			Если Не ЗначениеЗаполнено(ИмяПользователя())
			   И ПользователиИнформационнойБазы.ПолучитьПользователей().Количество() = 0 Тогда
				...
				ВременныйАдминистраторИБ.Имя = НСтр("ru='Временный первый администратор';uk='Тимчасовий перший адміністратор'")
			...
			ВременныйПользовательИБ.Имя = НСтр("ru='Временный пользователь';uk='Тимчасовий користувач'")
				+ " (" + Строка(Новый УникальныйИдентификатор) + ")";
	...
	Если НЕ ОписаниеПользователяИБ.Свойство("Действие") Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |У параметрі ОписаниеПользователяИБ не зазначена властивість Действие.'"),
	   ...
	   И ОписаниеПользователяИБ.Действие <> "Удалить" Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |невірне значення ""%2"" властивості Действие.'"),
			ПользовательОбъект.Ссылка,
			...
			Если ТипПользователяСовпадаетСГруппой Тогда
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Пользователь ""%1"" не может быть включен в группу ""%2"",
                               ...
                               |тому що у групи стоїть ознака ""Всі користувачі заданого типу"".'"),
			...
			Иначе
				СообщениеПользователю = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Пользователь ""%1"" не может быть включен в группу ""%2"",
                               ...
                               |;uk='Користувач ""%1"" не може бути включений в групу ""%2"",
                               |тому що до складу її учасників входять тільки %3.'"),
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		...
		СтрокаОбъект = Формат(КоличествоПользователей, "ЧДЦ=0") + " "
			+ ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(КоличествоПользователей,
				"", НСтр("ru='пользователь,пользователя,пользователей,,,,,,0';uk='користувач,користувача,користувачів,,,,,,0'"));
...
Процедура ЗаполнитьСпискиНастроек(Параметры, АдресХранилища) Экспорт
	...
	Если Параметры.ПользовательИнформационнойБазы <> ИмяПользователя()
	   И Не ПравоДоступа("АдминистрированиеДанных", Метаданные) Тогда
		...
		ВызватьИсключение НСтр("ru='Недостаточно прав для получения настроек пользователя.';uk='Недостатньо прав для отримання налаштувань користувача.'");
	КонецЕсли;
...
Функция ТекстСообщенияТекущийПользовательНедоступенВСеансеБезРазделителей()
	...
	Возврат
		НСтр("ru='Недопустимое получение параметра сеанса ТекущийПользователь
                   ...
                   |у сеансі без зазначення всіх роздільників.'");
	...
	Если СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) = 1 И ЗначениеЗаполнено(ДобавляемыеНедоступныеРоли) Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка назначить пользователю %1 недоступную роль:
                       |%2.'
                       |;uk='Спроба призначити користувачеві %1 недоступну роль:
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	...
	ИначеЕсли СтрЧислоСтрок(ДобавляемыеНедоступныеРоли) > 1 Тогда
		ТекстРегистрацииДобавления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Попытка назначить пользователю %1 недоступные роли:
                       |%2.'
                       |;uk='Спроба призначити користувачеві %1 недоступні ролі:
                       |%2.'"),
			ПользовательИБ.ПолноеИмя, ДобавляемыеНедоступныеРоли);
	Иначе
	...
	Для Каждого ОписаниеРоли Из ОписаниеРолей Цикл
		...
		Для Каждого СвойстваДанных Из НеразделенныеДанные Цикл
			...
			Если ПравоДоступа("Изменение", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%1"" содержит право Изменение неразделенного объекта %2.';uk='Роль ""%1"" містить право Изменение неподільного об''єкта %2.'"),
			...
			Если ПравоДоступа("Добавление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%1"" содержит право Добавление неразделенного объекта %2.';uk='Роль ""%1"" містить право Добавление неподільного об''єкта %2.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
			...
			Если ПравоДоступа("Удаление", ОбъектМетаданных, Роль) Тогда
				ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Роль ""%1"" содержит право Удаление неразделенного объекта %2.';uk='Роль ""%1"" містить право Удаление неподільного об''єкта %2.'"),
					Роль, ОбъектМетаданных.ПолноеИмя());
				Если СписокОшибок = Неопределено Тогда
					ТекстОшибки = ТекстОшибки + Символы.ПС + ОписаниеОшибки;
				Иначе
	   ...
	   И БлокируемыеРеквизиты.Служебный <> ПользовательОбъект.Служебный Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Реквізит Служебный не допускається змінювати в підписках на події.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если БлокируемыеРеквизиты.Подготовлен <> ПользовательОбъект.Подготовлен Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Ошибка при записи пользователя ""%1"".
                       ...
                       |;uk='Помилка при записі користувача ""%1"".
                       |Реквізит Подготовлен не допускається змінювати в підписках на події.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
...
Процедура ПроверитьВходВПрограммуРазрешен(ОшибкаАвторизации)
	...
	ОшибкаАвторизации = ЗаголовокСообщенияАвторизацияНеВыполненаСПереводомСтроки()
		+ НСтр("ru='Ваша учетная запись отключена. Обратитесь к администратору.';uk='Ваш обліковий запис відключений. Зверніться до адміністратора.'");
...
// 
Функция ИНАГРО_ПолучениеТекущегоПользователя() Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		ВызватьИсключение
			НСтр("ru='Недопустимое получение параметра сеанса ТекущийПользователь"
"в сеансе без указания всех разделителей.';uk='Неприпустиме одержання параметра сеансу ТекущийПользователь"
			...
			Иначе
				...
				Если РезультатПользователи.Пустой() Тогда
					//Если Пользователи.ЭтоПолноправныйПользователь( , ОбщегоНазначенияПовтИсп.РежимРаботыПрограммы().Локальный, Ложь) Тогда
						...
						ПользовательПоНаименованию  = ИНАГРО_СсылкаПользователя12ПоПолномуНаименованию(ПолноеИмяПользователя);
...
"""%2""."
""
"Обратитесь к администратору.';uk='Авторизація не виконана. Робота системи буде завершена."
"Користувач: %1 не знайдений у довіднику ""Користувачі""."
""
"При спробі додавання користувача в довідник виникла помилка:"
"""%2""."
""
"Зверніться до адміністратора.'"),
					ИмяПользователя,
	...
	Если ТекущийПользователь = Неопределено Тогда
		...
		ТекстСообщенияОбОшибке = ТекстСообщенияПользовательНеНайденВСправочнике(ИмяПользователя) +
			НСтр("ru='"
"Возникла внутренняя ошибка при поиске пользователя.';uk='"
"Виникла внутрішня помилка при пошуку користувача.'");
		ВызватьИсключение ТекстСообщенияОбОшибке;
	КонецЕсли;
...
Функция ИНАГРО_СсылкаПользователя12ПоПолномуНаименованию(ПолноеИмя)
