...
Процедура ПроверитьВозможностьРаботыСВнешнимиРесурсами(РегламентноеЗадание, ЗависимостиЗаданий = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Приложение было перемещено.
            ...
            |;uk='Прикладну програму було переміщено.
...
Процедура РазрешитьРаботуСВнешнимиРесурсами(ПараметрыБлокировки = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Обсуждения") Тогда
		МодульПолучениеФайловИзИнтернетаКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ОбсужденияСлужебныйВызовСервера");
		МодульПолучениеФайловИзИнтернетаКлиентСервер.Разблокировать();
	КонецЕсли;
...
Процедура ЗапретитьРаботуСВнешнимиРесурсами() Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Обсуждения") Тогда
		МодульПолучениеФайловИзИнтернетаКлиентСервер = ОбщегоНазначения.ОбщийМодуль("ОбсужденияСлужебныйВызовСервера");
		МодульПолучениеФайловИзИнтернетаКлиентСервер.Заблокировать();
	КонецЕсли;
...
Функция УстановленаБлокировкаРаботыСВнешнимиРесурсами(ЭтоВызовПередНачаломРаботыСистемы = Ложь, ПоказатьФорму = Ложь) Экспорт
	...
	Если ЭтоФайловаяБаза <> ПараметрыБлокировки.ЭтоФайловаяБаза Тогда
		ТекстСообщения = 
			?(ЭтоФайловаяБаза, 
				НСтр("ru='Информационная база была перемещена из клиент-серверного режима работы в файловый.';uk='Інформаційна база була переміщена з клієнт-серверного режиму роботи в файловий.'"),
	...
	Если ЭтоФайловаяБаза Тогда
		...
		Если НайденныеФайлы.Количество() = 0 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В каталоге информационной базы отсутствует файл проверки %1.';uk='В каталозі інформаційної бази відсутній файл перевірки %1.'"), 
				ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИдентификаторБазы <> ПараметрыБлокировки.ИдентификаторБазы Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Идентификатор информационной базы в файле проверки %1 не соответствует идентификатору в текущей базе.';uk='Ідентифікатор інформаційної бази у файлі перевірки %1 не відповідає ідентифікатору в поточній базі.'"), 
				ИмяФайлаПроверки);
			УстановитьПризнакНеобходимостиПринятияРешенияОБлокировке(ПараметрыБлокировки, ТекстСообщения);
			Возврат Истина;
		КонецЕсли;
		...
		Если ИмяБазы <> СохраненноеИмяБазы
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Различаются имя информационной базы или имя компьютера.
                           ...
                           |Ім''я комп''ютера:
                           |Було <%3>, Стало <%4>
                           |Перевіряти ім''я сервера: <%5>'"),
				ПараметрыБлокировки.СтрокаСоединения, 
				СтрокаСоединения,
				СохраненноеИмяКомпьютера,
				ИмяКомпьютера,
...
Функция НастройкиПоУмолчанию()
	...
	НастройкиПодсистемы.Вставить("РасположениеКомандыСнятияБлокировки",
		НСтр("ru='Блокировку также можно снять позднее в разделе <b>Администрирование - Обслуживание</b>.';uk='Блокування також можна зняти пізніше у розділі <b>Адміністрування - Обслуговування</b>.'"));
