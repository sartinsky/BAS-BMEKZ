		...
		Если КомпонентаСодержитЕдинственныйКлассОбъектов Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось подключить внешнюю компоненту на клиенте
                           ...
                           |;uk='Не вдалося підключити зовнішню компоненту на клієнті
                           |%1
                           |через:
                           |Не допускається одночасно не вказувати і Ідентифікатор і ИдентификаторыСозданияОбъектов'"), 
	...
	Если Не МестоположениеКомпонентыКорректно(Контекст.Местоположение) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось подключить внешнюю компоненту ""%1"" на клиенте
                       ...
                       |;uk='Не вдалося підключити зовнішню компоненту ""%1"" на клієнті
                       |%2
                       |через:
                       |Не допустимо підключити компоненти з зазначеного місця.'"), 
	...
	Если Не МестоположениеКомпонентыКорректно(Контекст.Местоположение) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось установить внешнюю компоненту ""%1"" на клиенте
                       ...
                       |;uk='Не вдалося встановити зовнішню компоненту ""%1"" на клієнті
                       |%2
                       |з причини:
                       |Не допустимо встановлювати компоненти з зазначеного місця.'"), 
			Контекст.Идентификатор,
			Контекст.Местоположение);
	КонецЕсли;
		...
		Иначе 
			ТекстОшибки =  СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось подключить внешнюю компоненту ""%1"" на клиенте
                           ...
                           |%2
                           |з причини:
                           |Метод НачатьПодключениеВнешнейКомпоненты повернув Брехня.'"),
				Контекст.Идентификатор,
		...
		Если ПодключаемыйМодуль = Неопределено Тогда 
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось создать объект внешней компоненты ""%1"", подключенной на клиенте
                           ...
                           |по причине:
                           |%3'
                           |;uk='Не вдалося створити об''єкт зовнішньої компоненти ""%1"", підключеної на клієнті
                           |%2,
                           |з причини:
                           |%3'"),
				Контекст.Идентификатор,
			...
			Если ПодключаемыйМодуль = Неопределено Тогда 
				...
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось создать объект ""%1"" внешней компоненты ""%2"", подключенной на клиенте
                               ...
                               |по причине:
                               |%4'
                               |;uk='Не вдалося створити об''єкт ""%1"" зовнішньої компоненти ""%2"", підключеної на клієнті
                               |%3,
                               |з причини:
                               |%4'"),
...
Функция ПредставлениеМестоположенияКомпоненты(Местоположение)
	...
	Если СтрНачинаетсяС(Местоположение, "e1cib/") Тогда
		Возврат НСтр("ru='из хранилища внешних компонент';uk='зі сховища зовнішніх компонент'");
	Иначе 
		Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='из макета ""%1""';uk='з макета ""%1""'"),
			Местоположение);
	КонецЕсли;
...
Процедура ОткрытьПроводникПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru='Расширение для работы с файлами не установлено, открытие папки не возможно.';uk='Розширення для роботи з файлами не встановлено, відкриття папки не можливо.'");
	...
	Иначе 
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найдена папка, которую требуется открыть в проводнике:
                       |""%1""'
                       |;uk='Не знайдена книга, яку потрібно відкрити в провіднику:
                       |""%1""'"),
			ФайлИнфо.ПолноеИмя);
		ОткрытьПроводникОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
...
Процедура ОткрытьФайлВПрограммеПросмотраПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Если РасширениеПодключено Тогда
		...
		ФайлИнфо.НачатьПроверкуСуществования(Оповещение);
	Иначе
		ОписаниеОшибки = НСтр("ru='Расширение для работы с файлами не установлено, открытие файла невозможно.';uk='Розширення для роботи з файлами не встановлено, відкриття файлу неможливо.'");
	...
	Иначе 
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не найден файл, который требуется открыть:
                       |%1'
                       |;uk='Не знайдений файл, який потрібно відкрити:
                       |%1'"),
			ФайлИнфо.ПолноеИмя);
		ОткрытьФайлВПрограммеПросмотраОповеститьОбОшибке(ОписаниеОшибки, Контекст);
	КонецЕсли;
		...
		Если ПустаяСтрока(ФайлИнфо.Расширение) Тогда 
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Имя файла не содержит расширения:
                           |%1'
                           |;uk='Ім''я файлу не містить розширення:
                           |%1'"),
				ФайлИнфо.ПолноеИмя);
			ОткрытьФайлВПрограммеПросмотраОповеститьОбОшибке(ОписаниеОшибки, Контекст);
			Возврат;
		...
		Если ЭтоРасширениеИсполняемогоФайла(ФайлИнфо.Расширение) Тогда 
			ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Исполняемые файлы открывать запрещено:
                           |%1'
                           |;uk='Файли, що виконуються, відкривати заборонено:
                           |%1'"),
				ФайлИнфо.ПолноеИмя);
			ОткрытьФайлВПрограммеПросмотраОповеститьОбОшибке(ОписаниеОшибки, Контекст);
			Возврат;
...
Процедура ОткрытьНавигационнуюСсылкуПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Расширение для работы с файлами не установлено, переход по ссылке ""%1"" невозможен.';uk='Розширення для роботи з файлами не встановлено, перехід за посиланням ""%1"" неможливий.'"),
			НавигационнаяСсылка);
...
Процедура ЗапуститьПрограммуПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = 
			НСтр("ru='Расширение для работы с файлами не установлено, запуск программы невозможен.';uk='Розширення для роботи з файлами не встановлено, запуск програми неможливий.'");
...
Процедура ЗапуститьПрограммуПослеЗапускаПриложения(КодВозврата, Контекст) Экспорт 
	...
	Если ДождатьсяЗавершения Тогда 
		...
		Если КодВозврата = Неопределено Тогда 
			ОписаниеОшибки = 
				НСтр("ru='Произошла неизвестная ошибка при запуске программы.';uk='Сталася невідома помилка при запуску програми.'");
			ЗапуститьПрограммуОповеститьОбОшибке(ОписаниеОшибки, Контекст);
		Иначе 
...
Процедура СоздатьВременныйКаталогПослеПроверкиРасширенияРаботыСФайлами(РасширениеПодключено, Контекст) Экспорт
	...
	Иначе
		ОписаниеОшибки = 
			НСтр("ru='Расширение для работы с файлами не установлено, создание временного каталога невозможно.';uk='Розширення для роботи з файлами не встановлено, створення тимчасового каталогу неможливо.'");
