...
Процедура ВыполнитьКоманду(Знач Форма, Знач ИмяКоманды) Экспорт
	...
	ИначеЕсли СтрНачинаетсяС(ИмяКоманды, "КартаGoogleМеню") Тогда	
	...
	ИначеЕсли ТипКонтактнойИнформации = ПредопределенноеЗначение("Перечисление.ТипыКонтактнойИнформации.Телефон") Тогда
		...
		Если ПустаяСтрока(Параметры.НомерТелефона) Тогда
			Если МожноОтправлятьSMS Тогда
				ТекстПредупреждения = НСтр("ru='Для совершения звонка или отправки SMS требуется ввести номер телефона.';uk='Для здійснення дзвінка або відправки SMS потрібно ввести номер телефону.'");
			Иначе
				ТекстПредупреждения = НСтр("ru='Для совершения звонка требуется ввести номер телефона.';uk='Для здійснення дзвінка потрібно ввести номер телефону.'");
			КонецЕсли;
...
Процедура ПоказатьАдресНаКарте(Адрес, ИмяКартографическогоСервиса) Экспорт
	...
	Если СтрНайти(Адрес, "№") <> 0 Тогда
		// Google не умеет (на момент проверки функционала) корректно обрабатывать символ № в строке поиска
		МассивЧастейАдреса = СтрРазделить(Адрес, ",");
			
		Для Индекс = 0 По МассивЧастейАдреса.Количество()-1 Цикл
			ЧастьАдреса = МассивЧастейАдреса[Индекс];
			ПозицияСимволаНомер = СтрНайти(ЧастьАдреса, "№");
			Если ПозицияСимволаНомер <> 0 Тогда
				МассивЧастейАдреса[Индекс] = Прав(ЧастьАдреса, СтрДлина(ЧастьАдреса) - ПозицияСимволаНомер);
		    КонецЕсли;
		КонецЦикла; 
		
		АдресБезСимволаНомер = СтрСоединить(МассивЧастейАдреса, ",");
	Иначе
		АдресБезСимволаНомер = Адрес;
	КонецЕсли;
	АдресКодированный = URLEncode(АдресБезСимволаНомер);
