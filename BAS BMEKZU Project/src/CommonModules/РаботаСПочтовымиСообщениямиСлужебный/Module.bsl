...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.Взаимодействия") Тогда
		...
		Обработчик.Комментарий = НСтр("ru='Обновляет сведения о персональных учетных записях электронной почты.
            ...
            |;uk='Оновлює відомості про персональні облікові записи електронної пошти.
            |До завершення обробки список персональних облікових записів електронної пошти може бути неповним.'");
...
Функция ПодготовитьВложения(Вложения, НастройкиСохранения) Экспорт
	...
	Для Индекс = -Вложения.ВГраница() По 0 Цикл
		...
		Для Каждого ТипФайла Из ВыбранныеФорматыСохранения Цикл
			...
			Иначе
				ИмяФайлаДляАрхива = НСтр("ru='Документы';uk='Документи'") + ".zip";
			КонецЕсли;
...
Функция ПредставлениеПисьма(ТемаПисьма, ДатаПисьма)
	...
	ШаблонПредставления = НСтр("ru='%1 от %2';uk=' %1 від %2'");
	
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредставления,
		?(ПустаяСтрока(ТемаПисьма), НСтр("ru='<Без темы>';uk='<Без теми>'"), ТемаПисьма),
