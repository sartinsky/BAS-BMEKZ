...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия 		= "2.1.25.1";
	Обработчик.Процедура 	= "ОбновлениеИнформационнойБазыБП.ЗаполнитьПредопределенныеПВР_2_1_25_1";
	Обработчик.НачальноеЗаполнение = Ложь;

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия 		= "2.1.26.1";
	Обработчик.Процедура 	= "ОбновлениеИнформационнойБазыБП.ЗаполнитьПредопределенные_2_1_26_1";
	Обработчик.НачальноеЗаполнение = Ложь;

	Обработчик = Обработчики.Добавить();
	Обработчик.Версия 		= "2.1.27.1";
	Обработчик.Процедура 	= "ОбновлениеИнформационнойБазыБП.ЗаполнитьПредопределенные_2_1_27_1";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик = Обработчики.Добавить();   // Типовая 2.1.22.2
	Обработчик.Версия 		= "2.1.28.1";
	Обработчик.Процедура 	= "ОбновлениеИнформационнойБазыБП.ЗаполнитьПредопределенные_2_1_28_1";
	Обработчик.НачальноеЗаполнение = Истина;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия 		= "2.1.29.1";
	Обработчик.Процедура 	= "ОбновлениеИнформационнойБазыБП.ЗаполнитьПредопределенные_2_1_29_1";
	Обработчик.НачальноеЗаполнение = Истина;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия 		= "2.1.30.1";
	Обработчик.Процедура 	= "ОбновлениеИнформационнойБазыБП.ЗаполнитьПредопределенныеСтатьиНаборыНалДекл";
	Обработчик.НачальноеЗаполнение = Ложь;
	...
	Обработчик.Процедура 	= "ОбновлениеИнформационнойБазыБП.ЗаполнитьПредопределенные_2_1_30_2";
	Обработчик.НачальноеЗаполнение = Истина;
...
Процедура ЗаполнитьПредопределенныеПВР_2_1_25_1() Экспорт     

	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьПланВидовРасчета("ИНАГРО_Начисления",,"ДоплатаЗаВечерниеЧасы");  
	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьПланВидовРасчета("ИНАГРО_Начисления",,"ДоплатаЗаНочныеЧасы");  
	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьОбъект("РегистрСведений", "ИндексИнфляции");
	
КонецПроцедуры

Процедура ЗаполнитьПредопределенные_2_1_26_1() Экспорт     

	Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
		Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьОбъект("Справочник", "ИНАГРО_ЦелевоеНазначениеЗемель");
	КонецЕсли;
	
	ЗаполнитьКодификаторКАТОТТГ();
    ЗаменитьСтрокуНаСсылку();
	Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
		ЗаполнитьРеквизитыСпрУчастки();
	КонецЕсли;
	
КонецПроцедуры

Процедура ЗаполнитьКодификаторКАТОТТГ() Экспорт
    
    МакетXML = Справочники.ИНАГРО_КодификаторКАТОТТГ.ПолучитьМакет("НачальноеЗаполнение");
    
    ПостроительОтчета  = Новый ПостроительОтчета;                                    
    ПостроительОтчета.ИсточникДанных = Новый ОписаниеИсточникаДанных(МакетXML.Область(2,1,МакетXML.ВысотаТаблицы,МакетXML.ШиринаТаблицы));
    ПостроительОтчета.Выполнить();
    ТЗДанных = ПостроительОтчета.Результат.Выгрузить(); 
    
    Для каждого Строки Из ТЗДанных Цикл 
        
        СсылкаНаЭлемент = Неопределено;
        
        Если  ЗначениеЗаполнено(Строки.КодГруппа1) И НЕ ЗначениеЗаполнено(Строки.КодГруппа2) 
            И НЕ  ЗначениеЗаполнено(Строки.КодГруппа3) И НЕ ЗначениеЗаполнено(Строки.КодГруппа4) 
            И НЕ ЗначениеЗаполнено(Строки.Дополнительная) Тогда
            СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.КодГруппа1);
            Если СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.ПустаяСсылка() Тогда
                СоздатьЭлементСправочникаКодификаторКАТОТТГ(Строки.КодГруппа1, Строки.Наименование);
            КонецЕсли;
            
        ИначеЕсли ЗначениеЗаполнено(Строки.КодГруппа1) И  ЗначениеЗаполнено(Строки.КодГруппа2) 
            И НЕ ЗначениеЗаполнено(Строки.КодГруппа3) И НЕ ЗначениеЗаполнено(Строки.КодГруппа4) 
            И НЕ ЗначениеЗаполнено(Строки.Дополнительная) Тогда
            СсылкаНаЭлементРодителя = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.КодГруппа1);
            СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.КодГруппа2);
            Если СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.ПустаяСсылка() Тогда
                СоздатьЭлементСправочникаКодификаторКАТОТТГ(Строки.КодГруппа2, Строки.Наименование, СсылкаНаЭлементРодителя);
            КонецЕсли;
            
        ИначеЕсли ЗначениеЗаполнено(Строки.КодГруппа1) И  ЗначениеЗаполнено(Строки.КодГруппа2) 
            И ЗначениеЗаполнено(Строки.КодГруппа3) И НЕ ЗначениеЗаполнено(Строки.КодГруппа4) 
            И НЕ ЗначениеЗаполнено(Строки.Дополнительная) Тогда
            СсылкаНаЭлементРодителя = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.КодГруппа2);
            СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.КодГруппа3);
            Если СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.ПустаяСсылка() Тогда
                СоздатьЭлементСправочникаКодификаторКАТОТТГ(Строки.КодГруппа3, Строки.Наименование, СсылкаНаЭлементРодителя);
            КонецЕсли;
            
        ИначеЕсли ЗначениеЗаполнено(Строки.КодГруппа1) И ЗначениеЗаполнено(Строки.КодГруппа2) 
            И ЗначениеЗаполнено(Строки.КодГруппа3) И ЗначениеЗаполнено(Строки.КодГруппа4) 
            И НЕ ЗначениеЗаполнено(Строки.Дополнительная) Тогда
            СсылкаНаЭлементРодителя = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.КодГруппа3);
            СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.КодГруппа4);
            
            Если СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.ПустаяСсылка() Тогда
                СоздатьЭлементСправочникаКодификаторКАТОТТГ(Строки.КодГруппа4, Строки.Наименование, СсылкаНаЭлементРодителя); 
            КонецЕсли;
            
        ИначеЕсли ЗначениеЗаполнено(Строки.КодГруппа1) И ЗначениеЗаполнено(Строки.КодГруппа2) 
            И ЗначениеЗаполнено(Строки.КодГруппа3) И ЗначениеЗаполнено(Строки.КодГруппа4) 
            И  ЗначениеЗаполнено(Строки.Дополнительная) Тогда
            СсылкаНаЭлементРодителя = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.КодГруппа4);
            СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Строки.Дополнительная);
            Если СсылкаНаЭлемент = Справочники.ИНАГРО_КодификаторКАТОТТГ.ПустаяСсылка() Тогда
                СоздатьЭлементСправочникаКодификаторКАТОТТГ(Строки.Дополнительная, Строки.Наименование, СсылкаНаЭлементРодителя); 
            КонецЕсли;
...
Процедура СоздатьЭлементСправочникаКодификаторКАТОТТГ(Код, Наименование, Родитель = Неопределено)
    
    ТекущийОбъект = Справочники.ИНАГРО_КодификаторКАТОТТГ.СоздатьЭлемент();  
    Если Родитель <> Неопределено Тогда
        ТекущийОбъект.Родитель = Родитель;
    КонецЕсли;
...
Процедура ЗаменитьСтрокуНаСсылку() Экспорт
    
 	Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
	   //Изменение данных кода КАТЕТТО справочника "Участки"
	    Запрос = Новый Запрос;
	    Запрос.Текст = 
	    "ВЫБРАТЬ
	    |   Участки.Ссылка КАК Участки,
	    |   Участки.УдалитьКодКАТЕТТО КАК УдалитьКодКАТЕТТО
	    |ИЗ
	    |   Справочник.ИНАГРО_Участки КАК Участки
	    |ГДЕ
	    |   Участки.УдалитьКодКАТЕТТО <> """"";
	    
	    Выборка = Запрос.Выполнить().Выбрать();
	    
	    Пока Выборка.Следующий() Цикл
	        
	        СсылкаКодаКАТОТТГ = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Выборка.УдалитьКодКАТЕТТО);
	        Если СсылкаКодаКАТОТТГ <> Справочники.ИНАГРО_КодификаторКАТОТТГ.ПустаяСсылка() Тогда
	            ОбъектСправочника = Выборка.Участки.ПолучитьОбъект();
	            ОбъектСправочника.КодКАТЕТТО = СсылкаКодаКАТОТТГ;
    ...
    //Изменение данных кода КАТЕТТО справочника "ИНАГРО_ОбластиИРайоны"
    Запрос = Новый Запрос;
    Запрос.Текст = 
    "ВЫБРАТЬ
    |   ОбластиИРайоны.Ссылка КАК ОбластиИРайоны,
    |   ОбластиИРайоны.УдалитьКодКАТЕТТО КАК УдалитьКодКАТЕТТО
    |ИЗ
    |   Справочник.ИНАГРО_ОбластиИРайоны КАК ОбластиИРайоны
    |ГДЕ
    |   ОбластиИРайоны.УдалитьКодКАТЕТТО <> """"";
    ...
    Пока Выборка.Следующий() Цикл
        ...
        СсылкаКодаКАТОТТГ = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду(Выборка.УдалитьКодКАТЕТТО);
        Если СсылкаКодаКАТОТТГ <> Справочники.ИНАГРО_КодификаторКАТОТТГ.ПустаяСсылка() Тогда
            ОбъектСправочника = Выборка.ОбластиИРайоны.ПолучитьОбъект();
            ОбъектСправочника.КодКАТЕТТО = СсылкаКодаКАТОТТГ;
            ОбъектСправочника.Записать(); 
        КонецЕсли;
...
Процедура ЗаполнитьРеквизитыСпрУчастки() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ИНАГРО_Участки.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.ИНАГРО_Участки КАК ИНАГРО_Участки
	|ГДЕ
	|	ИНАГРО_Участки.ПометкаУдаления = ЛОЖЬ
	|	И ИНАГРО_Участки.ЭтоГруппа = ЛОЖЬ";	
	...
	Пока ВыборкаЭлементов.Следующий() Цикл
		...
		СправочникОбъект = ВыборкаЭлементов.Ссылка.ПолучитьОбъект();
		СправочникОбъект.ЕдиницаИзмеренияПлощади = Перечисления.ИНАГРО_ЕдиницаИзмеренияПлощади.Га;
		СправочникОбъект.ПроведенаНормативнаяОценка =  Истина;
...
Процедура ЗаполнитьПредопределенные_2_1_27_1() Экспорт
...
Процедура ЗаполнитьПредопределенные_2_1_28_1()  Экспорт

	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьОбъект("Справочник", "ВидыЛьготПоНДФЛ");
	
КонецПроцедуры	

Процедура ЗаполнитьПредопределенные_2_1_29_1()  Экспорт

	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьОбъект("РегистрСведений", "ИндексИнфляции");
	
	УдалитьСтрокуИНАГРО_КодификаторКАТОТТГ();
	
КонецПроцедуры

Процедура УдалитьСтрокуИНАГРО_КодификаторКАТОТТГ() Экспорт
    
	Если ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
		
        СсылкаКодаКАТОТТГ = Справочники.ИНАГРО_КодификаторКАТОТТГ.НайтиПоКоду("UA05060090020070449");  // Бабина Долина
        Если СсылкаКодаКАТОТТГ <> Справочники.ИНАГРО_КодификаторКАТОТТГ.ПустаяСсылка() Тогда
            ОбъектСправочника = СсылкаКодаКАТОТТГ.ПолучитьОбъект();
...
Процедура ЗаполнитьПредопределенные_2_1_30_2()  Экспорт

	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьОбъект("Справочник", "ИНАГРО_ВидыПраздников"); 
	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьОбъект("Справочник", "ИНАГРО_ПричиныНетрудоспособности"); 
	Обработки.НачальноеЗаполнениеИОбновлениеОбъектов.ЗаполнитьОбъект("РегистрСведений", "ИндексИнфляции");
