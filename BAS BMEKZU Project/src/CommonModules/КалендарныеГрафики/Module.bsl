...
Функция ДатыБлижайшихРабочихДней(График, НачальныеДаты, ПолучатьПредшествующие = Ложь, ВызыватьИсключение = Истина, 
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе 
			...
			КонецЕсли;
			Если ВызыватьИсключение Тогда
				ТекстСообщения = НСтр("ru='Невозможно определить ближайшую рабочую дату для даты %1, возможно, график работы не заполнен.';uk='Неможливо визначити найближчу робочу дату для дати %1, можливо, графік роботи не заповнено.'");
...
Процедура ПриДобавленииКлассификаторов(Классификаторы) Экспорт
	...
	Описатель.Наименование = НСтр("ru='Производственные календари';uk='Виробничі календарі'");
...
// составленный в соответствии с КЗОТ
//
...
Функция ДанныеКлассификатора() Экспорт
	...
	Если ДанныеФайлов = Неопределено Тогда
		ТекстСообщения = НСтр("ru='Не удалось получить данные календаря.
                               ...
                               |;uk='Не вдалося отримати дані календаря.
                               |Робота з класифікаторами не підтримується або відсутня.'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	
	Если Не ПустаяСтрока(ДанныеФайлов.КодОшибки) Тогда
		ИмяСобытия = НСтр("ru='Календарные графики.Получение файла классификатора';uk='Календарні графіки.Отримання файлу класифікатора'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru='Не удалось получить данные календаря.
                               ...
                               |;uk='Не вдалося отримати дані календаря.
                               |Отримані класифікатори не містять календарів.'");
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
	...
	Если СведенияФайла.Версия < ВерсияКалендарей() Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось обработать полученные данные календаря из-за конфликта версий.
                  ...
                  |;uk='Не вдалося обробити отримані дані календаря з-за конфлікту версій.
                  |Версія календарів
                  |- отриманого класифікатора %1, 
                  |- вбудованих в конфігурацію %2, 
                  |- завантаженого раніше класифікатора %3.'"),
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось обработать полученные данные календаря.
                  |%1.'
                  |;uk='Не вдалося обробити отримані дані календаря.
                  |%1.'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Если Не ОбщегоНазначения.РазделениеВключено() Тогда
		...
		Обработчик.Комментарий = НСтр("ru='Обновление графиков работы и др. данных, зависимых от производственных календарей.';uk='Оновлення графіків роботи та інших даних, залежних від виробничих календарів.'");
...
Процедура ПроверитьНачалоЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.НачалоЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ИмяСобытия = НСтр("ru='Календарные графики.Обработка файла классификатора';uk='Календарні графіки.Обробка файлу класифікатора'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный формат файла данных. Ожидается начало элемента %1';uk='Невірний формат файлу даних. Очікується початок елементу %1'"), 
			Имя);
...
Процедура ПроверитьКонецЭлемента(Знач ЧтениеXML, Знач Имя)
	...
	Если ЧтениеXML.ТипУзла <> ТипУзлаXML.КонецЭлемента Или ЧтениеXML.Имя <> Имя Тогда
		ИмяСобытия = НСтр("ru='Календарные графики.Обработка файла классификатора';uk='Календарні графіки.Обробка файлу класифікатора'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный формат файла данных. Ожидается конец элемента %1';uk='Невірний формат файлу даних. Очікується кінець елемента %1'"), 
			Имя);
		ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Ошибка, , , ТекстСообщения);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПроизводственныхКалендарейПриОбновлении()
	...
	Если ТаблицаДанных.Количество()=0 Тогда 
		ТаблицаКалендарей = Справочники.ПроизводственныеКалендари.ПроизводственныеКалендариИзМакета();
		Для каждого Календарь Из ТаблицаКалендарей Цикл
			КодКалендаря = Календарь.Code;
			...
			ДанныеПоУмолчанию = Справочники.ПроизводственныеКалендари.РезультатЗаполненияПроизводственногоКалендаряПоУмолчанию(
				КодКалендаря, НомерТекущегоГода);
				
			Если ТаблицаДанных.Количество()=0 Тогда 
				ТаблицаДанных =  ДанныеПоУмолчанию;
			Иначе
				Для каждого СтрокаДанныеПоУмолчанию Из ДанныеПоУмолчанию Цикл
					НоваяСтрока = ТаблицаДанных.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаДанныеПоУмолчанию);
				КонецЦикла; 
			КонецЕсли;
			
			ДанныеПоУмолчаниюСледующийГод = Справочники.ПроизводственныеКалендари.РезультатЗаполненияПроизводственногоКалендаряПоУмолчанию(
				КодКалендаря, НомерТекущегоГода+1);
				
			Для каждого СтрокаДанныеПоУмолчанию Из ДанныеПоУмолчаниюСледующийГод Цикл
				НоваяСтрока = ТаблицаДанных.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаДанныеПоУмолчанию);
			КонецЦикла; 
