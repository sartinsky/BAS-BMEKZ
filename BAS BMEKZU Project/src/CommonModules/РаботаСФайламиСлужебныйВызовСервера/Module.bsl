...
Функция ПодсчитатьКоличествоФайловВТомах()
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ЕСТЬNULL(КОЛИЧЕСТВО(ФайлыВТомах.Файл), 0) КАК КоличествоФайлов
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|	ГДЕ
	|		СведенияОФайлах.Файл.ТипХраненияФайла = ЗНАЧЕНИЕ(Перечисление.ТипыХраненияФайлов.ВТомахНаДиске)) КАК ФайлыВТомах
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ВерсииФайлов КАК ВерсииФайлов
	|		ПО ФайлыВТомах.Файл = ВерсииФайлов.Владелец
...
Процедура ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы(Параметры) Экспорт
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось обработать версию файла ""%1"" по причине:
                           |%2'
                           |;uk='Не вдалося обробити версію файлу ""%1"" з причини:
                           |%2'"),
				ВерсияСсылка,
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы не удалось обработать некоторые версии файлов (пропущены): %1';uk='Процедурі ПеренестиЭлектронныеПодписиСВерсийФайлоВНаФайлы не вдалося обробити деякі версії файлів (пропущені): %1'"),
			ПроблемныхОбъектов);
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Процедура ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы обработала очередную порцию версий файлов: %1';uk='Процедура ПеренестиЭлектронныеПодписиСВерсийФайловНаФайлы опрацювала чергову порцію версій файлів: %1'"),
				ОбъектовОбработано));
	КонецЕсли;
...
Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведенийДляТаблицы(ПараметрыОбновления, ПолноеИмяОбъектаМетаданных, ОбработкаЗавершена)
	...
	Если ОбъектМетаданных = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не указан объект для обработки электронных подписей и сертификатов шифрования.';uk='Не вказаний об''єкт для обробки електронних підписів та сертифікатів шифрування.'");
	КонецЕсли;
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось обработать объект: %1 по причине:
                       |%2'
                       |;uk='Не вдалося обробити об''єкт: %1 з причини:
                       |%2'"),
			ОбъектМетаданных,
			ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений не удалось обработать некоторые объекты (пропущены): %1';uk='Процедурі ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений не вдалося обробити деякі об''єкти (пропущені): %1'"),
			ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений обработала очередную порцию объектов: %1';uk='Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений опрацювала чергову порцію об''єктів: %1'"),
				ОбъектовОбработано));
	КонецЕсли;
