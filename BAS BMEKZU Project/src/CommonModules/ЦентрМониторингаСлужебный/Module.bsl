		...
		Иначе
			...
			Если АктивныеФоновыеЗадания.Количество() = 0 Тогда                                                                      
				...
				ФоновыеЗадания.Выполнить("ЦентрМониторингаСлужебный.ОтправитьТестовыйПакет",
					МассивПараметров,
					КлючФоновогоЗадания,
					НСтр("ru='Центр мониторинга: отправка тестового пакета';uk='Центр моніторингу: відправка тестового пакета'"));
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
		...
		Если НачатьСборИОтправкуОтчетовОбОшибках Тогда
			...
			Если АктивныеФоновыеЗадания.Количество() = 0 Тогда
				...
				ФоновыеЗадания.Выполнить("ЦентрМониторингаСлужебный.СобратьИОтправитьДампы",
					ПараметрыФоновогоЗадания,
					КлючФоновогоЗадания,
					НСтр("ru='Сбор и отправка отчетов об ошибках';uk='Збір та відправка звітів про помилки'"));
				КонецЕсли;
		Иначе
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Представление  = НСтр("ru='Предоставить отчеты об ошибках';uk='Надати звіти про помилки'");
		Дело.Количество     = 0;
		Дело.Подсказка      = НСтр("ru='Зарегистрированы аварийные завершения работы программы. Пожалуйста, расскажите нам об этой проблеме.';uk='Зареєстровані аварійні завершення роботи програми. Будь ласка, розкажіть нам про цю проблему.'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Важное         = Ложь;
		Дело.Владелец       = Раздел;
		Дело.Представление  = НСтр("ru='Отправить отчеты об ошибках';uk='Відправити звіти про помилки'");
		Дело.Количество     = 0;
		Дело.Подсказка      = НСтр("ru='Отчеты об аварийном завершении собраны и подготовлены. Пожалуйста, согласуйте их отправку.';uk='Звіти про аварійне завершення зібрані і підготовлені. Будь ласка, узгодьте їх відправку.'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Важное         = Истина;
		Дело.Владелец       = Раздел;
		Дело.Представление  = НСтр("ru='Сообщить о проблемах производительности';uk='Повідомити про проблеми продуктивності'");
		Дело.Количество     = 0;
		Дело.Подсказка      = НСтр("ru='Обнаружены проблемы производительности. Пожалуйста, расскажите нам об этих проблемах.';uk='Виявлені проблеми продуктивності. Будь ласка, розкажіть нам про ці проблеми.'");
...
Функция ПолучитьПараметрыПоУмолчанию()
	...
	ПараметрыКонстанты.Вставить("Сервер", "pult.bas-soft.eu");
...
Функция УстановитьПараметрыОтправки(Параметры)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Установка параметров отправки';uk='Центр моніторингу.Встановлення параметрів відправлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
...
Функция УстановитьПараметрыЦентраМониторинга(НовыеПараметры)
	...
	Исключение
		ОтменитьТранзакцию();
		Ошибка = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга.Установка параметров центра мониторинга';uk='Центр моніторингу.Встановлення параметрів центру моніторингу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, Метаданные.ОбщиеМодули.ЦентрМониторингаСлужебный,,Ошибка);
		ВызватьИсключение Ошибка;
	КонецПопытки;
КонецФункции
...
Функция НайтиКаталогДампов(КаталогНастроек, ТипДампа, ПрекратитьСборПолных) 
	...
	Если Файл.Существует() Тогда
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга';uk='Центр моніторингу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
			
			КаталогДампов.Путь = Неопределено;
			КаталогДампов.ОписаниеОшибки = НСтр("ru='Ошибка чтения файла настроек технологического журнала';uk='Помилка читання файлу настройок технологічного журналу'");
			Возврат КаталогДампов;
		КонецПопытки;
		...
		Если ПараметрыДампов <> Неопределено Тогда
			Если НЕ ПараметрыДампов.Свойство("location") ИЛИ НЕ ПараметрыДампов.Свойство("create") ИЛИ НЕ ПараметрыДампов.Свойство("type") Тогда
				Сообщение = НСтр("ru='Ошибка секции сбора дампов в файле настроек технологического журнала';uk='Помилка секції збирання дампів у файлі настройок технологічного журналу'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
				Сообщение = Сообщение + НСтр("ru='Регистрация дампов не возможна. Удалите файл или восстановите настройки.';uk='Реєстрація дампів не можлива. Вилучіть файл або відновіть настройки.'");
				ЧтениеXML.Закрыть();
				
				ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга';uk='Центр моніторингу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
				
				КаталогДампов.Путь = Неопределено;
				КаталогДампов.ОписаниеОшибки = НСтр("ru='Ошибка секции сбора дампов в файле настроек технологического журнала';uk='Помилка секції збирання дампів у файлі настройок технологічного журналу'");
				Возврат КаталогДампов;
			КонецЕсли;
		КонецЕсли;
	...
	Иначе
		...
		КонецЕсли;
		Если КаталогДампов.Путь = Неопределено Тогда
			КаталогДампов.ОписаниеОшибки = НСтр("ru='Ошибка создания файла настроек технологического журнала. Регистрация дампов не возможна.';uk='Помилка створення файлу настройок технологічного журналу. Реєстрація дампів не можлива.'");
		КонецЕсли;
	КонецЕсли;
	...
	ЧтениеXML.Закрыть();
		...
		Если ВРЕГ(ПервыйДочерний.ИмяУзла) = "CONFIG" Тогда
			...
			Попытка
				...
				ЗаписьXML.Закрыть();
			Исключение
				Сообщение = НСтр("ru='Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.';uk='Помилка запису файла настройок технологічного журналу. Реєстрація дампів не можлива.'");
				Сообщение = Сообщение + " """ +Файл.ПолноеИмя + """." + Символы.ПС;
				Сообщение = Сообщение + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
				
				ЗаписьЖурналаРегистрации(НСтр("ru='Центр мониторинга - регистрация дампов';uk='Центр моніторингу - реєстрація дампів'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Предупреждение,,,Сообщение);
				
				КаталогДампов.Путь = Неопределено;
				КаталогДампов.ОписаниеОшибки = НСтр("ru='Ошибка записи файла настроек технологического журнала. Регистрация дампов не возможна.';uk='Помилка запису файла настройок технологічного журналу. Реєстрація дампів не можлива.'");
				Возврат КаталогДампов;
			КонецПопытки;
		КонецЕсли;
	КонецЕсли;
...
Функция СоздатьФайлНастроекСбораДампов(ПутьКаталога, ТипДампа)
	...
	Попытка
		...
		ЗаписьXML.Закрыть();
	Исключение
		Сообщение = НСтр("ru='Ошибка создания файла настроек технологического журнала. Регистрация дампов не возможна.';uk='Помилка створення файлу настройок технологічного журналу. Реєстрація дампів не можлива.'");
...
Процедура СобратьИОтправитьДампы(СКлиентаНаСервере = Ложь, ИдентификаторЗадания = "") Экспорт
	...
	Если ПараметрыСбораИОтправкиДампов.ОтправлятьФайлыДампов = 0 Тогда
		УстановитьПривилегированныйРежим(Истина);
		УстановитьПараметрЦентраМониторинга("РезультатОтправки", НСтр("ru='Пользователь отказал в предоставлении дампов.';uk='Користувач відмовив у наданні дампів.'"));
		УстановитьПривилегированныйРежим(Ложь);
		ПрекратитьСборПолныхДампов();
		Возврат;
	КонецЕсли;
	...
	Если ТекущаяДатаСеанса() >= ПараметрыСбораИОтправкиДампов.ОкончаниеСбораДампов Тогда
		УстановитьПривилегированныйРежим(Истина);
		УстановитьПараметрЦентраМониторинга("РезультатОтправки", НСтр("ru='Сбор дампов прекращен по таймауту.';uk='Збір дампів припинений по таймауту.'"));	
		УстановитьПривилегированныйРежим(Ложь);
		ПрекратитьСборПолныхДампов();
		Возврат;
	КонецЕсли;
	...
	Если ПозицияРазделителя = 0 Тогда
		УстановитьПривилегированныйРежим(Истина);
		УстановитьПараметрЦентраМониторинга("РезультатОтправки", НСтр("ru='Не удалось определить букву диска';uk='Не вдалося визначити букву диска'"));
		УстановитьПривилегированныйРежим(Ложь);
		ПрекратитьСборПолныхДампов();
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыСбораИОтправкиДампов.СборПолныхДамповВключен[ИмяКомпьютера] = Истина Тогда
		...
		Если РезультатЗамера.Значение/1024 < ПараметрыСбораИОтправкиДампов.РезервМестаВключен
			И ТипДампа = "3" Тогда
			УстановитьПривилегированныйРежим(Истина);
			УстановитьПараметрЦентраМониторинга("РезультатОтправки", НСтр("ru='Недостаточно свободного места для хранения дампов. Сбор дампов будет отключен.';uk='Недостатньо вільного місця для зберігання дампів. Збір дампів буде відключений.'"));	
			УстановитьПривилегированныйРежим(Ложь);
			ПрекратитьСборПолныхДампов();
			Возврат;
		КонецЕсли;
	...
	Иначе
		...
		Если РезультатЗамера.Значение/1024 < ПараметрыСбораИОтправкиДампов.РезервМестаВыключен
			И ТипДампа = "3" Тогда
			УстановитьПривилегированныйРежим(Истина);
			УстановитьПараметрЦентраМониторинга("РезультатОтправки", НСтр("ru='Недостаточно свободного места для сбора дампов.';uk='Недостатньо вільного місця для збору дампів.'"));	
			УстановитьПривилегированныйРежим(Ложь);
			ПрекратитьСборПолныхДампов();
			Возврат;
		КонецЕсли;
	...
	Если Параметры.СпрашиватьПередОтправкой И Параметры.ТипДампа = "3" Тогда
		...
		Если Параметры.Свойство("ВариантыДампов") И Параметры.ВариантыДампов.Количество() Тогда
			...
			ШаблонЗапросДляОтправки = НСтр("ru='Для отправки подготовлены отчеты об ошибках (%1 шт.)
                                                                 ...
                                                                 |Отправить указанные файлы для анализа?'
                                                                 |;uk='Для відправки підготовлені звіти про помилки (%1 шт.)
                                                                 |Загальний обсяг даних: %2 МБ.
                                                                 |Надіслати зазначені файли для аналізу?'");
...
Функция СвободноеМестоНаДиске(БукваДиска, СКлиентаНаСервере)
	...
	Если ЗначениеЗаполнено(ПотокОшибок) Тогда 
		...
		РезультатЗапроса.ОписаниеОшибки = НСтр("ru='Ошибка при выполнении команды typeperf';uk='Помилка при виконанні команди typeperf'");
	Иначе 
		...
		Иначе
			РезультатЗапроса.Успешно = Ложь;
			РезультатЗапроса.ОписаниеОшибки = НСтр("ru='Не удалось разобрать результат typeperf';uk='Не вдалося розібрати результат typeperf'");
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтправитьТестовыйПакет(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	Если ОтправкаТестовогоПакетаВозможна(ПараметрыЦентраМониторинга, ДатаЗапуска) И ПараметрыВыполнения.ОтправкаТестовогоПакета
		...
		Исключение
			...
			РезультатВыполнения.КраткоеПредставлениеОшибки = НСтр("ru='Ошибка при формировании пакета.';uk='Помилка при формуванні пакета.'");
			Комментарий = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ЗаписьЖурналаРегистрации(
			НСтр("ru='Центр мониторинга - сформировать тестовый пакет для отправки';uk='Центр моніторингу - сформувати тестовий пакет для відправлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,
			,
			,
			Комментарий);
		...
		Попытка
			...
			Иначе
				РезультатВыполнения.Успешно = Ложь;
				РезультатВыполнения.КраткоеПредставлениеОшибки = НСтр("ru='Ошибка при отправке пакета.';uk='Помилка при відправленні пакета.'");
				Шаблон = НСтр("ru='Ошибка HTTP при отправке пакета. Код %1';uk='Помилка HTTP при відправленні пакета. Код %1'");
				Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Шаблон, HTTPОтвет.КодСостояния); 
				ЗаписьЖурналаРегистрации(
				НСтр("ru='Центр мониторинга - отправить тестовые данные мониторинга';uk='Центр моніторингу - відправити тестові дані моніторингу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
				Комментарий);
