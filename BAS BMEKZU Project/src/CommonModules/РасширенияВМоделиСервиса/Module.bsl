...
Функция ПоставляемоеРасширение(ИдентификаторИспользуемогоРасширения) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ВызватьИсключение НСтр("ru='Использование функции РасширенияВМоделиСервиса.ПоставляемоеРасширение()
                                ...
                                |;uk='Використання функції РасширенияВМоделиСервиса.ПоставляемоеРасширение()
                                |доступно тільки для сеансів з установленим поділом даних!'");
...
Функция ИспользуемоеРасширение(ПоставляемоеРасширение) Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.ДоступноИспользованиеРазделенныхДанных() Тогда
		...
		ВызватьИсключение НСтр("ru='Использование функции РасширенияВМоделиСервиса.ИспользуемоеРасширение()
                                |доступно только для сеансов с установленным разделением данных!'
                                |;uk='Використання функції РасширенияВМоделиСервиса.ИспользуемоеРасширение()
                                |доступно тільки для сеансів з установленим поділом даних!'");
...
Процедура УстановитьПоставляемоеРасширениеВОбластьДанных(Знач ОписаниеИнсталляции, Знач Ответственный) Экспорт
	...
	Попытка
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Поставляемые расширения.Инициирована установка поставляемого расширения в область данных';uk='Розширення, що поставляються. Ініційован встановлення розширення, що поставляється, в область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		...
		Если ОбщегоНазначения.СсылкаСуществует(ПоставляемоеРасширение) Тогда
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Установка в область данных';uk='Розширення, що поставляються. Встановлення в область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
		...
		Иначе
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Установка в область данных отложена';uk='Розширення, що поставляються. Встановлення в область даних відкладено'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				Строка(ОписаниеИнсталляции.Идентификатор),
				Строка(ОписаниеИнсталляции.Инсталляция));
...
Процедура УдалитьПоставляемоеРасширениеИзОбластиДанных(Знач ПоставляемоеРасширение) Экспорт
	...
	Попытка
		...
		Попытка
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Удаление из области данных';uk='Розширення, що поставляються. Видалення з області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,
				,
				ПоставляемоеРасширение,
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Ошибка удаления из области данных';uk='Розширення, що поставляються. Помилка видалення з області даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура ОбработатьОшибкуУстановкиРасширенияВОбластьДанных(Знач ПоставляемоеРасширение, Знач ИдентификаторИнсталляции, Знач ТекстИсключения) Экспорт
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Ошибка установки в область данных';uk='Розширення, що поставляються. Помилка встановлення в область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Ошибка,,
		ПоставляемоеРасширение,
...
Процедура АктуализацияИспользуемогоРасширения(Знач ИдентификаторИспользуемогоРасширения) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru='Расширение с идентификатором %1 не является поставляемым!';uk='Розширення з ідентифікатором %1 не є таким, що поставляється!'"),
...
Процедура ДоступныНовыеДанные(Знач Дескриптор, Загружать) Экспорт
	...
	Если Дескриптор.DataType = ИдентификаторВидаПоставляемыхДанных() Тогда
		...
		Если НЕ Загружать Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения отменена';uk='Розширення, що поставляються. Завантаження розширення скасовано'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,,
				НСтр("ru='Поставляемое расширение несовместимо с данной конфигурацией';uk='Розширення, що поставляється, несумісне з даною конфігурацією'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Совместимость);
...
Процедура ОбработатьПоставляемыеРасширения(Дескриптор, ПутьКФайлу)
	...
	ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения';uk='Розширення, що поставляються. Завантаження розширення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация,,,
		НСтр("ru='Инициирована загрузка поставляемого расширения';uk='Ініційоване завантаження розширення, що поставляється'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Манифест);
	...
	Если ПоставляемоеРасширение.ИдентификаторВерсии <> ОписаниеПоставляемогоРасширения.Версия Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Загрузка поставляемого расширения завершена';uk='Розширення, що поставляються. Завантаження розширення завершено'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Информация,,
			ПоставляемоеРасширение.Ссылка,
			НСтр("ru='Завершена загрузка поставляемого расширения';uk='Завершено завантаження  розширення, що поставляється'") + Символы.ПС + Символы.ВК + ОписаниеПоставляемогоРасширения.Манифест);
		...
		Для каждого ИнсталляцияРасширения Из ИспользуемыеРасширения Цикл
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='Поставляемые расширения.Запланирована актуализация используемого расширения';uk='Розширення, що поставляються. Запланована актуалізація використовуваного розширення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
				НСтр("ru='Область данных:';uk='Область даних:'") + ИнсталляцияРасширения.ОбластьДанных);
		...
		Для каждого ЭлементОчереди Из ОчередьИнсталляций Цикл
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Поставляемые расширения.Запланирована отложенная установка поставляемого расширения в область данных';uk='Розширення, що поставляються. Заплановано відкладене встановлення розширення в область даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
				УровеньЖурналаРегистрации.Информация,,
				ПоставляемоеРасширение.Ссылка,
...
Процедура ЗапроситьОбновлениеРасширений() Экспорт
	...
	Если ОбщегоНазначенияПовтИсп.РазделениеВключено() Тогда
		...
		Исключение
			...
			ОтменитьТранзакцию();
			ЗаписьЖурналаРегистрации(НСтр("ru='Запрос обновлений расширений';uk='Запит оновлення розширень'"), УровеньЖурналаРегистрации.Предупреждение, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
