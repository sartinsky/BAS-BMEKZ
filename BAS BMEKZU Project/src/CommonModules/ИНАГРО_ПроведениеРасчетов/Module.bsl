...
Функция РассчитатьСреднююПорядок100(Регистратор, КомментироватьРасчет = Ложь) Экспорт 
	...
	Иначе
		...
		Если Истина Тогда
			...
			|		СУММА(ВЫБОР КОГДА Начисления.ВидРасчета.ОбозначениеВТабелеУчетаРабочегоВремени = ЗНАЧЕНИЕ(Справочник.ИНАГРО_КлассификаторИспользованияРабочегоВремени.Командировка)
...
Процедура ОпределитьКодКатегорииЗЛ(СтрокаТЧ, Регистратор)
	...
	Если СхемаКатегорий = 3 Тогда
		...
		ИначеЕсли СтрокаТЧ.Налог = Справочники.Налоги.ЕСВФОТМобилИнвалиды Тогда		
			// Працівник-інвалід, призваний на строкову військову службу
