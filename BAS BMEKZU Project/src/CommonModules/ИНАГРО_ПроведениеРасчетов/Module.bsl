 ...
 // Функция возвращает дату изменения Порядка №100 расчета средней согласно Постановлению КМУ №1213 от 09.12.20 г.
//
	...
	Если ИмяРегистра = "ИНАГРО_Начисления" Тогда
		...
		Если НеобходимыеДанные.ОтработаноВремени Тогда
			...
			Иначе
				...
				//|ЕСТЬNULL(ВЫБОР
				//|		КОГДА Основной.ВидРасчета В (Значение(ПланВидовРасчета.ИНАГРО_Начисления.ПочасовойПростой), Значение(ПланВидовРасчета.ИНАГРО_Начисления.Простой), Значение(ПланВидовРасчета.ИНАГРО_Начисления.ПочасовойПростойПоСредней), Значение(ПланВидовРасчета.ИНАГРО_Начисления.ПочасовойПростойПоСредней))
				//|			ТОГДА 11 //ВремяПоТабелю.ВнутрисменныеЧасыОтклонение
				//|		ИНАЧЕ 
				|			ЕСТЬNULL(ВЫБОР	КОГДА ВремяВведенноеВЦеломЗаПериод.УчетПоТабелю
...
Функция ЧасоваяТарифнаяСтавкаРаботникаОрг(Знач Сотрудник, Знач ДатаАктуальности) Экспорт
	...
	|		&ДатаАктуальности,
	|		Сотрудник = &Сотрудник) КАК ОсновноеНачисление
	//|					И ВидНачисления = ЗНАЧЕНИЕ(Перечисление.ИНАГРО_ВидыНачисленийРаботникаОрганизации.Основное)) КАК ОсновноеНачисление
	|		ПО (ИСТИНА)
	|       И ОсновноеНачисление.ВидНачисления = ЗНАЧЕНИЕ(Перечисление.ИНАГРО_ВидыНачисленийРаботникаОрганизации.Основное)
    |       И ОсновноеНачисление.Действует	
	|//-----------------------
	...
	|) КАК ДанныеКалендаря
	|		ПО (ВЫБОР
	|				КОГДА РаботникиОрганизацииСрезПоследних.ГрафикРаботы.УчетНормыВремениПоОтдельномуГрафику
	|					ТОГДА ДанныеКалендаря.ГрафикРаботы = РаботникиОрганизацииСрезПоследних.ГрафикРаботы.ГрафикРаботыНормыВремени
...
Процедура АвтозаполнениеВзносы(Регистратор, Взносы, ЭтоВзносы, Сотрудники, ПоРегистратору = Ложь ) Экспорт
	...
	Если Истина Тогда
		...
		|		Работники.РаботаСНефиксированнымРабочимВременем	КАК РаботаСНефиксированнымРабочимВременем		
		|
	...
	Если НужноРассчитыватьКоэффициент Тогда
		...
		|	 КОГДА Основной.Сотрудник.ИНАГРО_ВидЗанятости <> ЗНАЧЕНИЕ(Перечисление.ВидыЗанятости.ОсновноеМестоРаботы)
		|	  ТОГДА 0
		|	 КОГДА ЕстьNULL(ДатыПриема.Период, ДАТАВРЕМЯ(1,1,1)) > НАЧАЛОПЕРИОДА(Основной.БазовыйПериодНачало, МЕСЯЦ)
		|	 ИЛИ ЕСТЬNULL(РаботникиОрганизацийНаНачалоПериода.ЗанимаемыхСтавок,0) = 0
		...
		|	  ТОГДА 0   
		|	 КОГДА Подразделения.РаботаСНефиксированнымРабочимВременем
        |	  ТОГДА 0		
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ИНАГРО_РаботникиОрганизаций.СрезПервых(
		|				,
		|				Организация = &парамОрганизация
		|					И Сотрудник В (&парамСотрудники)) КАК ДатыПриема
...
Функция ВизуализироватьФормулуРасчета(Объект, Режим = "Текст") Экспорт
	...
	ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаНачислений.НулеваяСумма Тогда
		...
		Иначе
			...
			|Результат = 0"
		КонецЕсли;
	ИначеЕсли СпособРасчета = Перечисления.СпособыРасчетаНачислений.ПоЧасовойТарифнойСтавкеУмножитьНаПроцент Тогда
		Если Режим = "Текст" Тогда
			СтрокаФормулы = "<"+Справочники.ИНАГРО_ПоказателиСхемМотивации.ОкладТариф.Наименование + "> * <Процент доплаты> * <Время в часах>"
		Иначе
			СтрокаФормулы = СтрокаФормулы + "
							|Результат = <FONT color=#0000ff>"+Справочники.ИНАГРО_ПоказателиСхемМотивации.ОкладТариф.Наименование + "</FONT> <FONT color=#FF0000> * </FONT> <FONT color=#0000ff>Процент доплаты </FONT> <FONT color=#FF0000> * </FONT> <FONT color=#0000ee>Время в часах</FONT>"			
		КонецЕсли;	
	...
	ИначеЕсли  СпособРасчета = Перечисления.СпособыРасчетаНачислений.КомпенсацияНевыплаченнойЗарплаты Тогда
		...
		Иначе
			СтрокаФормулы = СтрокаФормулы + "
							|Результат = <FONT color=#0000ff><Коэф.компенсации></FONT> <FONT color=#FF0000> * </FONT> <FONT color=#0000ff><Задолженность></FONT>"			
		КонецЕсли;
...
Функция РассчитатьСреднююПорядок100(Регистратор, КомментироватьРасчет = Ложь) Экспорт 
	...
	ОтпускаЗаСвойСчет = Новый Массив;
	ОтпускаЗаСвойСчет.Добавить(ПланыВидовРасчета.ИНАГРО_Начисления.ОтпускЗаСвойСчет);
	ОтпускаЗаСвойСчет.Добавить(ПланыВидовРасчета.ИНАГРО_Начисления.Простой);
	ОтпускаЗаСвойСчет.Добавить(ПланыВидовРасчета.ИНАГРО_Начисления.ПростойПоСредней);
	ОтпускаЗаСвойСчет.Добавить(ПланыВидовРасчета.ИНАГРО_Начисления.ПочасовойПростой);
	ОтпускаЗаСвойСчет.Добавить(ПланыВидовРасчета.ИНАГРО_Начисления.ПочасовойПростойПоСредней);
	ОтпускаЗаСвойСчет.Добавить(ПланыВидовРасчета.ИНАГРО_Начисления.ПростойСПриостановкойТрудДог); 
	ОтпускаЗаСвойСчет.Добавить(ПланыВидовРасчета.ИНАГРО_Начисления.ОтпускЗаСвойСчетВП); 
	ОтпускаЗаСвойСчет.Добавить(ПланыВидовРасчета.ИНАГРО_Начисления.МобилизацияБезОплаты);
	
	Запрос.УстановитьПараметр( "ОтпускЗаСвойСчет", ОтпускаЗаСвойСчет);
	мНеУчитыватьСовместителейДляСредних = Ложь;
	...
	Если ДатаНачалаСобытия < ИНАГРО_ПроведениеРасчетов.ДатаИзмененияПорядка100() Тогда
		...
		Если Истина Тогда
			...
			|	ГДЕ
			|		ОтпускаЗаСвойСчет.ВидРасчета В (&ОтпускЗаСвойСчет)
	...
	Иначе
		...
		Если НЕ ИспользуемыеВидыРасчета.Найти(ПланыВидовРасчета.ИНАГРО_СреднийЗаработок.ПоЗаработку) = Неопределено Тогда
		...
		Если ДатаНачалаСобытия >= ИНАГРО_ПроведениеРасчетов.ДатаИзмененияПорядка100_486() Тогда
			Если Истина Тогда
				Запрос.Текст = "ВЫБРАТЬ
				|	Премии.ВидРасчета,
				|	Премии.БазовыйПериодПремииНачало,
		...
		Если ДатаНачалаСобытия < ДатаИзмененияПорядка100_917() Тогда 
			...
			|						ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_СреднийЗаработок.ПоПремиямОтпуска), ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_СреднийЗаработок.ПоФиксПремиямОтпуска), 
			|						ЗНАЧЕНИЕ(ПланВидовРасчета.ИНАГРО_СреднийЗаработок.ПоГодовойПремии)) И Основной.КоличествоМесяцев > 1			
...
Функция ПолучитьВидУчетаВремени(ВидРасчета, СуммированныйУчетРабочегоВремени, ВидУчетаВремениДляСредней = Неопределено) Экспорт
	...
	ИначеЕсли	ВидРасчета.СпособРасчета = Перечисления.СпособыРасчетаНачислений.ПоМесячнойТарифнойСтавкеПоЧасам
		ИЛИ ВидРасчета.СпособРасчета = Перечисления.СпособыРасчетаНачислений.ПоЧасовойТарифнойСтавке 
		ИЛИ ВидРасчета.СпособРасчета = Перечисления.СпособыРасчетаНачислений.ПоЧасовойТарифнойСтавкеУмножитьНаПроцент Тогда	
...
Процедура ОпределитьКодКатегорииЗЛ(СтрокаТЧ, Регистратор)
	...
	Если СхемаКатегорий = 3 Тогда
		...
		ИначеЕсли СтрокаТЧ.Налог = Справочники.Налоги.ЕСВФОТМобилИнвалиды Тогда		
			// Особи з інвалідністю, яким нараховано виплати в межах середнього заробітку працівників, призваних на військову службу
			СтрокаТЧ.КодКатегорииЗЛ = 66;
		ИначеЕсли СтрокаТЧ.Налог = Справочники.Налоги.ЕСВФОТМобилТРО Тогда		
			// Громадяни України, що підписали контракт добровольця територіальної оборони
			СтрокаТЧ.КодКатегорииЗЛ = 75;
		ИначеЕсли СтрокаТЧ.Налог = Справочники.Налоги.ЕСВФОТМобилБезОбесп Тогда		
			// Наймані працівники, яких було призвано під час мобілізації на військову службу до Збройних Сил України, які не отримували доходів у вигляді грошового забезпечення у період мобілізації
			СтрокаТЧ.КодКатегорииЗЛ = 76; 
		КонецЕсли;	
	Иначе	
...
Процедура РассчитатьЗаписьРегистраРасчета(СтрокаДвижений,
	...
	Если СтрокаДвижений.Авторасчет Тогда
		...
		ИначеЕсли
			...
			//
		ИначеЕсли
			СпособРасчета = Перечисления.СпособыРасчетаНачислений.ПоЧасовойТарифнойСтавкеУмножитьНаПроцент Тогда
	...
	Если Истина Тогда 
		...
		Если Истина Тогда
			...
			Если НЕ ИспользуетсяНК Тогда
				...
				//|								ТОГДА СведенияОЛьготах.Льгота.РазмерНДФЛ * 
				//|                               ВЫБОР 
				//|                                    КОГДА (ДетиРаботников.КвоДетей - ЕстьNULL(ДетиРаботников.КвоДетейИнвалидов, 0)) < 0
				//|							            Тогда 0 
				//|								    ИНАЧЕ (ДетиРаботников.КвоДетей - ЕстьNULL(ДетиРаботников.КвоДетейИнвалидов, 0))
				//|									КОНЕЦ
			...
			КонецЕсли;
				...
				|	ЛьготыСотрудников.НалоговыйПериод КАК НалоговыйПериод,
				|	ЗНАЧЕНИЕ(Справочник.ВидыЛьготПоНДФЛ.Гр16912_16913б) КАК Льгота,
				|	СУММА(ЛьготыСотрудников.КвоЛьгот) КАК КвоЛьгот,
				|	СУММА(ЛьготыСотрудников.КвоПороговЛьгот) КАК КвоПороговЛьгот
				|ПОМЕСТИТЬ ВТЛьготыСотрудниковРазрешенныеДубли
				|ИЗ
				|	ВТЛьготыСотрудниковКоличество КАК ЛьготыСотрудников
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТЛьготыСотрудниковКоличество КАК ЛьготыСотрудниковДубли
				|		ПО ЛьготыСотрудников.Сотрудник = ЛьготыСотрудниковДубли.Сотрудник
				...
				|	ЛьготыСотрудников.Сотрудник КАК Сотрудник,
				|	ЛьготыСотрудников.НалоговыйПериод КАК НалоговыйПериод,
				|	ЛьготыСотрудников.Льгота КАК Льгота,
				|	ЛьготыСотрудников.КвоЛьгот КАК КвоЛьгот,
				|	ЛьготыСотрудников.КвоПороговЛьгот КАК КвоПороговЛьгот
				...
				|	ЛьготыСотрудников.КвоЛьгот,
				|	ЛьготыСотрудников.КвоПороговЛьгот
				|ИЗ
				...
				|	ЛьготыСотрудников.Сотрудник,
				|	ЛьготыСотрудников.НалоговыйПериод,
				|	МАКСИМУМ(ЛьготыСотрудников.КвоЛьгот) КАК КвоЛьгот,
				|	МАКСИМУМ(ЛьготыСотрудников.КвоПороговЛьгот) КАК КвоПороговЛьгот
				...
				|	ПО ЛьготыСотрудников.Сотрудник = ЛьготыСотрудниковМаксимум.Сотрудник
				|	 И ЛьготыСотрудников.НалоговыйПериод = ЛьготыСотрудниковМаксимум.НалоговыйПериод
				|	 И ЛьготыСотрудников.КвоЛьгот = ЛьготыСотрудниковМаксимум.КвоЛьгот
				|	 И ЛьготыСотрудников.КвоПороговЛьгот = ЛьготыСотрудниковМаксимум.КвоПороговЛьгот
				|СГРУППИРОВАТЬ ПО
				|	ЛьготыСотрудников.Сотрудник,
				|	ЛьготыСотрудников.НалоговыйПериод
				|";
