	...
	Попытка
		...
		Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(Задание.Метаданные().ПолноеИмя(), РаботаВМоделиСервиса.РазделительВспомогательныхДанных()) Тогда
			...
			Если ЗначениеЗаполнено(Задание.Шаблон)
				...
				ШаблонСообщения = НСтр("ru='На найден шаблон задания очереди  %1';uk='На знайдено шаблон завдання черги %1'");
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = НСтр("ru='Задание не удалось обновить
                                   |ОбластьДанных: %1
                                   |Имя метода: %2'
                                   |;uk='Завдання не вдалося оновити
                                   |ОбластьДанных: %1
                                   |Ім''я методу: %2'");
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Обновление очереди заданий';uk='Оновлення черги завдань'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
				УровеньЖурналаРегистрации.Ошибка,,,
				ТекстСообщения);
