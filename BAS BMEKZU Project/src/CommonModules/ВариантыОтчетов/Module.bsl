...
Процедура НастроитьОтчетВМодулеМенеджера(Настройки, ОтчетМетаданные) Экспорт
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |%1'
            |;uk='Неприпустиме значення параметра КлючВарианта у процедурі ВариантыОтчетов.НастроитьОтчетВМодулеМенеджера.
            |Не вдалося настроїти варіанти звіту з модуля менеджера з причини:
            |%1'"),
...
Функция ОписаниеОтчета(Настройки, Отчет) Экспорт
	...
	Если СтрокаОтчет = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Неприпустиме значення параметра Отчет у процедурі ВариантыОтчетов.ОписаниеОтчета.
            |Звіт ""%1"" не підключений до підсистеми ""%2"". Перевірте властивість ""Сховище варіантів"" у властивостях звіту.'"),
			Строка(Отчет),
	...
	Если СтрокаВариант = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Недопустимое значение параметра КлючВарианта в процедуре ВариантыОтчетов.ОписаниеВарианта:
                ...
                |;uk='Неприпустиме значення параметра КлючВарианта у процедурі ВариантыОтчетов.ОписаниеВарианта:
                |варіант ""%1"" відсутній у звіті ""%2"".'"),
			КлючВарианта,
...
Процедура СброситьПользовательскиеНастройки(Ключ, ТипыНастроек = Неопределено) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru='Некорректный тип параметра ""Отчет""';uk='Некоректний тип параметра ""Звіт""'");
	КонецЕсли;
		...
		Пока Истина Цикл
			...
			Исключение
				...
				ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, 
					НСтр("ru='В процессе выборки пользовательских настроек отчетов из системного хранилища возникла ошибка:';uk='У процесі вибірки користувацьких настройок звітів з системного сховища виникла помилка:'")
					+ Символы.ПС
					+ ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
...
Процедура ЗагрузитьПользовательские(ТаблицаВариантов = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ОпределенныеВарианты Цикл
		...
		Иначе
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вариант ""%1"" отчета ""%2"": не найден автор ""%3""';uk='Варіант ""%1"" звіту ""%2"": не знайдений автор ""%3""'"),
...
Функция ПодключитьОтчетИЗагрузитьНастройки(Параметры) Экспорт
	...
	Если НадоПрочитатьНастройки Тогда
		...
		Если ВариантНастроекКД = Неопределено Тогда
			Результат.ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Вариант ""%1"" (ключ ""%2"") не найден в схеме отчета ""%3"".';uk='Варіант ""%1"" (ключ ""%2"") не знайдений у схемі звіту ""%3"".'"),
		...
		Если ТипЗнч(МетаданныеОтчета) <> Тип("ОбъектМетаданных") Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось открыть отчет ""%1"".
                    ...
                    |;uk='Не вдалося відкрити звіт ""%1"".
                    |Можливо, було відключено розширення конфігурації з цим звітом.'"),
...
Функция СформироватьОтчет(Знач Параметры, Знач ПроверятьЗаполнение, Знач ПолучатьФлажокПустой) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru='Не удалось определить используемые таблицы:';uk='Не вдалося визначити використовувані таблиці:'");
	...
	Если СсылкиВариантов.Количество() > 0 Тогда
		...
		Для Каждого Ссылка Из СсылкиВариантов Цикл
			...
			Если ОтчетЗначение = Неопределено Тогда
				СтрокаТаблицы.Представление = НСтр("ru='<Недостаточно прав для работы с вариантом отчета>';uk='<Недостатньо прав для роботи з варіантом звіту>'");
			Иначе
...
Функция ДеревоПредопределенных(ФильтрПоТипуОтчетов = "Внутренний") Экспорт
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		Если СтрокаОтчет.ИспользуетСКД Тогда
			...
			Исключение
				ТекстОшибки = НСтр("ru='Не удалось прочитать схему отчета:';uk='Не вдалося прочитати схему звіту:'");
				ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			...
			Если СхемаКД <> Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ru='Не удалось прочитать список вариантов отчета:';uk='Не вдалося прочитати список варіантів звіту:'");
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, ТекстОшибки, ОтчетМетаданные);
				КонецПопытки;
			КонецЕсли;
			...
			Если ВариантыНастроек = Неопределено Тогда
				...
				Исключение
					ТекстОшибки = НСтр("ru='Не удалось прочитать список вариантов отчета из модуля менеджера:';uk='Не вдалося прочитати список варіантів звіту з модуля менеджера:'");
					ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
					ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки, ОтчетМетаданные);
				КонецПопытки;
			КонецЕсли;
			...
			Если ВариантыНастроек <> Неопределено Тогда
				Для Каждого ВариантНастроекКД Из ВариантыНастроек Цикл
					...
					Если РазрешеноИндексирование И ТипЗнч(ВариантНастроекКД) = Тип("ВариантНастроекКомпоновкиДанных") Тогда
						...
						Исключение
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Не удалось прочитать настройки варианта ""%1"":';uk='Не вдалося прочитати настройки варіанту ""%1"":'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru='Уменьшает количество быстрых настроек в пользовательских отчетах до 2 шт.';uk='Зменшує кількість швидких настройок в користувацьких звітах до 2 шт.'");
	...
	Если РазрешеноИндексированиеНеразделенныхДанных() Тогда
		...
		Обработчик.Комментарий   = НСтр("ru='Обновление индекса поиска отчетов, предусмотренных в программе.';uk='Оновлення індексу пошуку звітів, передбачених у програмі.'");
	КонецЕсли;
	...
	Обработчик.Комментарий     = НСтр("ru='Обновление индекса поиска отчетов, сохраненных пользователями.';uk='Оновлення індексу пошуку звітів, збережених користувачами.'");
	...
	Обработчик.Комментарий = НСтр("ru='Установка параметра ИсточникДанных в настройках вариантов универсального отчета.
        ...
        |;uk='Встановлення параметра ИсточникДанных у настройках варіантів універсального звіту.
        |Після завершення обробки, перейменування об''єктів метаданих не призведе до втрати збережених варіантів звітів'");
...
Процедура ПриОпределенииВидовПодключаемыхКоманд(ВидыПодключаемыхКоманд) Экспорт
	...
	Вид.Заголовок   = НСтр("ru='Отчеты';uk='Звіти'");
	Вид.Порядок     = 50;
	...
	Если ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")
	 Или ТипЗнч(Результат.Ссылка) = Тип("СправочникСсылка.ИдентификаторыОбъектовРасширений") Тогда
		...
		Если Не ПравоДоступа("Использование", Результат.Метаданные) Тогда
			Результат.ТекстОшибки = НСтр("ru='Недостаточно прав доступа';uk='Недостатньо прав доступу'");
			Возврат Результат;
		КонецЕсли;
		Попытка
			...
			Результат.Успех = Истина;
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось подключить отчет %1:';uk='Не вдалося підключити звіт %1:'"),
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(ДеревоВариантов, Отчет, ГруппироватьПоОтчетам)
	...
	Иначе
		...
		Если СтрокаОтчет = Неопределено Тогда
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Предупреждение, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Отчет ""%1"" не подключен к подсистеме.';uk='Звіт ""%1"" не підключений до підсистеми.'"), Отчет.Имя));
			Возврат;
		КонецЕсли;
	КонецЕсли;
	СтрокаОтчет.ГруппироватьПоОтчету = ГруппироватьПоОтчетам;
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Если Найденные.Количество() > 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Помилка реєстрації зміни імені варіанту звіту ""%1"":
                |Актуальне ім''я варіанту ""%2"" (старе ім''я ""%3"")
                |також значиться як старе ім''я ""%4"" (актуальне ім''я ""%5"").'"),
		...
		Если Найденные.Количество() > 2 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Помилка реєстрації змін імені варіанту звіту ""%1"":
                |Старе ім''я варіанту ""%2"" (актуальне ім''я ""%3"")
                |також значиться як старе ім''я 
                |варіанту звіту ""%4"" (актуальне ім''я ""%5"").'"),
				Строка(Изменение.Отчет),
				Изменение.СтароеИмяВарианта,
				Изменение.АктуальноеИмяВарианта,
...
Процедура ЗаписатьВЖурналЗапускПроцедуры(ИмяПроцедуры)
	...
	ЗаписьЖурналаРегистрации(ВариантыОтчетовКлиентСервер.НаименованиеПодсистемы(Неопределено),
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Запуск процедуры ""%1"".';uk='Запуск процедури ""%1"".'"), ИмяПроцедуры)); 
...
Процедура ЗаписатьВЖурналЗавершениеПроцедуры(ИмяПроцедуры, ИзмененоОбъектов = Неопределено)
	...
	Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Завершение процедуры ""%1"".';uk='Завершення процедури ""%1"".'"), ИмяПроцедуры);
	Если ИзмененоОбъектов <> Неопределено Тогда
		Текст = Текст + " " 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Изменено %1 объектов.';uk='Змінено %1 об''єктів.'"), ИзмененоОбъектов);
	КонецЕсли;
	ЗаписьЖурналаРегистрации(ВариантыОтчетовКлиентСервер.НаименованиеПодсистемы(Неопределено),
...
Процедура СократитьКоличествоБыстрыхНастроек(ВходящиеПараметры = Неопределено) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчетов Цикл
		...
		Если ИнформацияОбОшибке <> Неопределено Тогда // Возникла проблема.
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вариант ""%1"" отчета ""%2"":';uk='Варіант ""%1"" звіту ""%2"":'")
	...
	Если Ошибок > 0 Тогда
		ТекстОшибки = ПредставлениеПроцедуры + ":" + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось уменьшить количество быстрых настроек %1 отчетов.';uk='Не вдалося зменшити кількість швидких настройок %1 звітів.'"), Ошибок);
		ЗаписьЖурналаРегистрации(ВариантыОтчетовКлиентСервер.НаименованиеПодсистемы(Неопределено),
...
Процедура ОбновитьИндексПоиска(Кэш)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Кэш.ОбщиеДанные Тогда
			...
			Если Найденные.Количество() = 0 Тогда
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Вариант ""%1"" не найден для отчета ""%2""';uk='Варіант ""%1"" не знайдено для звіту ""%2""'"), 
		...
		Исключение
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось перестроить индекс поиска для варианта ""%1"" отчета ""%2"". Возможно, отчет неисправен.';uk='Не вдалося перебудувати індекс пошуку для ""%1"" звіту ""%2"". Можливо, звіт несправний.'"), 
				ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
...
Процедура ЗаписатьПараметрыВариантовОтчетов(Кэш)
	...
	КонецЕсли;
	ПредставлениеПроцедуры = НСтр("ru='Запись неразделенного кэша в регистр';uk='Запис неподіленого кеша в регістр'");
	ЗаписатьВЖурналЗапускПроцедуры(ПредставлениеПроцедуры);
...
Функция ПроиндексироватьСодержимоеСхемы(ВариантОбъект) Экспорт
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных")
		...
		Исключение
			ШаблонСообщения = НСтр("ru='Не удалось прочитать настройки пользовательского варианта отчета:
                |%1'
                |;uk='Не вдалося прочитати настройки інтерфейсу варіанту звіту:
                |%1'");
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		Если ТипЗнч(ВариантОбъект) = Тип("СправочникОбъект.ПредопределенныеВариантыОтчетов")
			...
			ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, 
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось прочитать настройки предопределенного варианта отчета ""%1"".';uk='Не вдалося прочитати настройки напередвизначеного варіанту звіту ""%1"".'"), ВариантОбъект.КлючЗамеров),
				ВариантОбъект.Ссылка);
		КонецЕсли;
		Возврат ТребуетсяЗапись; // Возникла проблема.
	КонецЕсли;
...
Функция УменьшитьКоличествоБыстрыхНастроек(ВариантОбъект, ОтчетОбъект)
	...
	Если ТипЗнч(НастройкиКД) <> Тип("НастройкиКомпоновкиДанных") Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Обнаружены пустые настройки пользовательского варианта ""%1"" отчета ""%2"".';uk='Виявлені порожні настройки користувацького варіанту ""%1"" звіту ""%2"".'"), 
			ВариантОбъект.КлючВарианта, ВариантОбъект.Отчет);
		ЗаписатьВЖурнал(УровеньЖурналаРегистрации.Ошибка, ТекстОшибки, ВариантОбъект.Ссылка);
	...
	Если ОтчетОбъект = Неопределено Тогда
		...
		Иначе
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось получить настройки отчета ""%1"":';uk='Не вдалося отримати  настройки звіту ""%1"":'") + Символы.ПС + Подключение.ТекстОшибки,
				ОтчетСсылка);
		...
		Для Каждого СтрокаТаблицы Из Найденные Цикл
			...
			ИначеЕсли Не НайденВПрограмме Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" не найден в программе>';uk='<Звіт ""%1"" не знайдений у програмі>'"),
					ИмяОтчета);
			ИначеЕсли Не ДоступенПоПравам Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Недостаточно прав для работы с отчетом ""%1"">';uk='<Недостатньо прав для роботи з звітом ""%1"">'"),
					ИмяОтчета);
			ИначеЕсли Не ДоступенПоОпциям Тогда
				СтрокаТаблицы.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='<Отчет ""%1"" отключен в настройках программы>';uk='<Звіт ""%1"" відключений в настройках програми>'"),
					ИмяОтчета);
			Иначе
