...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
		НСтр("ru='Заполнение справочника Программы электронной подписи и шифрования.';uk='Заповнення довідника Програми електронного підпису та шифрування.'");
	...
	Если ЗначениеЗаполнено(ПредставлениеОшибки) Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сертификат не найден на сервере по причине:
                       |%1'
                       |;uk='Сертифікат не знайдений на сервері з причини:
                       |%1'")
			+ Символы.ПС,
			ПредставлениеОшибки);
	Иначе
...
Функция СведенияОПодписиДляЖурналаРегистрации(ДатаПодписи, СвойстваПодписи, ЭтоОшибкаПодписания = Ложь) Экспорт
	...
	Иначе
		СведенияОПодписи = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Дата подписи: %1';uk='Дата підпису: %1'"), Формат(ДатаПодписи, "ДЛФ=DT")) + Символы.ПС;
	КонецЕсли;
...
Процедура ЗарегистрироватьПодписаниеДанныхВЖурнале(ЭлементДанных, ОписаниеОшибки = "") Экспорт
	...
	Если ЭтоОшибкаПодписания Тогда
		ИмяСобытия = НСтр("ru='Электронная подпись.Ошибка подписания данных';uk='Електронний підпис.Помилка підписання даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		
		СообщениеЖурналаРегистрации = СообщениеЖурналаРегистрации + "
		|
		|" + ОписаниеОшибки;
	Иначе
		ИмяСобытия = НСтр("ru='Электронная подпись.Подписание данных';uk='Електронний підпис.Підписання даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция СертификатИзДвоичныхДанных(ДанныеСертификата) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Электронная подпись.Удаление временного файла';uk='Електронний підпис.Вилучення тимчасового файлу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Процедура НастроитьПояснениеВводаПароля(Форма, ИмяЭлементаУсиленнаяЗащитаЗакрытогоКлюча = "", ИмяЭлементаПояснениеУсиленногоПароля = "") Экспорт
	...
	Если ЗначениеЗаполнено(ИмяЭлементаУсиленнаяЗащитаЗакрытогоКлюча) Тогда
		...
		Элемент.Заголовок = НСтр("ru='Ввод и сохранение пароля в программе';uk='Введення і збереження пароля в програмі'");
		Элемент.Подсказка =
			НСтр("ru='- Включается интерактивный режим работы программы электронной подписи,
                       ...
                       |- Отключается запрос пароля в форме программы платформы.
                       |
                       |Обязательно для закрытых ключей сертификатов, для которых в ОС включена усиленная защита.'
                       |;uk='- Включається інтерактивний режим роботи програми електронного підпису,
                       |  при якому вона запитує пароль і дозволяє його зберегти.
                       |- Відключається запит пароля у формі програми платформи.
                       |
                       |Обов''язково для закритих ключів сертифікатів, для яких в ОС включено посилений захист.'");
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(ИмяЭлементаПояснениеУсиленногоПароля) Тогда
		...
		Элемент.Подсказка =
			НСтр("ru='Пароль запрашивает программа электронной подписи, а не программа платформы,
                       |так как для выбранного сертификата включен режим ""Ввод и сохранение пароля в программе"".'
                       |;uk='Пароль запитує програма електронного підпису, а не програма платформи,
                       |так як для вибраного сертифіката включений режим ""Введення і збереження пароля в програмі"".'");
	КонецЕсли;
...
Функция ПроверитьПодпись(Знач КонвертSOAP, ДанныеАлгоритмаПодписания, МенеджерКриптографии) Экспорт
	...
	Иначе
		...
		Если ПодписьВерна Тогда
			ВызватьИсключение НСтр("ru='Подпись не верна (SignatureValue корректно, отличается DigestValue).';uk='Підпис не вірний (SignatureValue коректно, відрізняється DigestValue).'")
		Иначе
			ВызватьИсключение НСтр("ru='Подпись не верна (SignatureValue некорректно.';uk='Підпис не вірний (SignatureValue некоректно.'");
		КонецЕсли;
...
Функция ОбъектВнешнейКомпонентыXMLDSig()
	...
	Если ОбъектКомпоненты = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Не удалось подключить внешнюю компоненту XMLDSig.';uk='Не вдалося підключити зовнішню компоненту XMLDSig.'");
	КонецЕсли;
...
Функция СвойстваКриптопровайдера(МенеджерКриптографии)
	...
	Если ОписаниеПрограммы = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось определить тип криптопровайдера %1';uk='Не вдалося визначити тип криптопровайдера %1'"), ИмяКриптопровайдера);
	КонецЕсли;
...
Функция GetSignOIDFromCert(ОбъектКомпоненты, СертификатКриптографииBase64)
	...
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';uk='Помилка виклику методу GetSignOIDFromCert компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если OIDАлгоритмаПодписи = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка вызова метода GetSignOIDFromCert компоненты XMLDSig.';uk='Помилка виклику методу GetSignOIDFromCert компоненти XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция C14N(ОбъектКомпоненты, КонвертSOAP, XPath)
	...
	Попытка
		КаноникализированныйТекстXML = ОбъектКомпоненты.C14N(
			...
			XPath);
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода C14N компоненты XMLDSig.';uk='Помилка виклику методу C14N компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если КаноникализированныйТекстXML = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка вызова метода C14N компоненты XMLDSig.';uk='Помилка виклику методу C14N компоненти XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция Hash(ОбъектКомпоненты, КаноникализированныйТекстXML, OIDАлгоритмаХеширования, ТипКриптопровайдера)
	...
	Попытка
		DigestValue = ОбъектКомпоненты.Hash(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода Hash компоненты XMLDSig.';uk='Помилка виклику методу Hash компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если DigestValue = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка вызова метода Hash компоненты XMLDSig.';uk='Помилка виклику методу Hash компоненти XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция Sign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo, СертификатКриптографии, ПарольДоступаКЗакрытомуКлючу)
	...
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода Sign компоненты XMLDSig.';uk='Помилка виклику методу Sign компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если SignatureValue = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка вызова метода Sign компоненты XMLDSig.';uk='Помилка виклику методу Sign компоненти XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Функция VerifySign(ОбъектКомпоненты, КаноникализированныйТекстXMLSignedInfo, SignatureValue, СертификатКриптографииBase64, ТипКриптопровайдера)
	...
	Попытка
		ПодписьВерна = ОбъектКомпоненты.VerifySign(
			...
			ТипКриптопровайдера);
	Исключение
		ВызватьИсключение НСтр("ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';uk='Помилка виклику методу VerifySign компоненти XMLDSig.'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	
	Если ПодписьВерна = Неопределено Тогда
		ВызватьИсключение НСтр("ru='Ошибка вызова метода VerifySign компоненты XMLDSig.';uk='Помилка виклику методу VerifySign компоненти XMLDSig.'") + Символы.ПС + ОбъектКомпоненты.GetLastError();
	КонецЕсли;
...
Процедура ПроверитьАлгоритмПодписи(ОбъектКомпоненты, СертификатКриптографииBase64, ДанныеАлгоритмаПодписания)
	...
	Если OIDАлгоритмаПодписи <> ДанныеАлгоритмаПодписания.OIDАлгоритмаПодписи Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Алгоритм подписи сертификата криптографии отличается от алгоритма %1.';uk='Алгоритм підпису сертифіката криптографії відрізняється від алгоритму %1.'"),
