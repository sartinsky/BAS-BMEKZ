...
Процедура ВызватьИсключениеПриНаличииВРазделенныхДанныхСсылокНаНеразделенныеБезПоддержкиСопоставленияСсылок(Знач ОбъектМетаданных, Знач ИмяПоля, Знач МетаданныеСсылки, Знач ПриВыгрузке)
	...
	Если ПриВыгрузке Тогда
		...
		ТекстОшибки = ТекстОшибки + " " +
			НСтр("ru='(в качестве типа значения для объекта установлен составной тип данных,
                  ...
                  |;uk='(в якості типу значення для об''єкта встановлений складений тип даних,
                  |який може містити посилання як на поділені дані, так і на поділені,
                  |але при вивантаженні була діагностована спроба вивантаження посилання на неподілений об''єкт).'",Метаданные.ОсновнойЯзык.КодЯзыка);
	...
	ДополнениеОшибки = СтрШаблон(
		НСтр("ru='При этом неразделенный объект %1 не включен в состав типов общих данных,
              ...
              |для яких можливе виконання співставлення посилань при ви вантаженні та завантаженні.
              ...
              |полів, які однозначно визначають природний ключ об''єкта та включити об''єкт %1 у склад
              |типів загальних даних, для яких можливе виконання співставлення посилань при
	...
	Если Не ПриВыгрузке Тогда
		...
		ДополнениеОшибки = ДополнениеОшибки + + Символы.ПС + СтрШаблон(
			НСтр("ru='Если корректное сопоставление ссылок на неразделенные данные в ИБ, из которой выгружены
                  ...
                  |;uk='Якщо коректне співставлення посилань на нерозділені дані в ІБ, з якої вивантажені
