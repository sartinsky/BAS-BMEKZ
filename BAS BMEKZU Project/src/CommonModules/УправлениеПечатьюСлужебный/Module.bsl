...
Функция ИнициализироватьПечатнуюФорму(Макет) Экспорт
	...
	ИмяВременногоКаталога = ОбщегоНазначения.СоздатьВременныйКаталог();
...
//
Функция ПолучитьМакет(ДвоичныеДанныеМакета) Экспорт
	
	Расширение = ОпределитьРасширениеФайлаДанныхПоСигнатуре(ДвоичныеДанныеМакета);
	Если НЕ Расширение = "docx" Тогда
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", НСтр("ru='Макет шаблона офисного документа имеет не верный формат.';uk='Макет шаблону офісного документа має не вірний формат.'"));
		ВызватьИсключение(НСтр("ru='Ошибка анализа макета шаблона. Макет шаблона офисного документа имеет не верный формат.';uk='Помилка аналізу макета шаблону. Макет шаблону офісного документа має не вірний формат.'"));
	КонецЕсли;
...
Процедура ЗакрытьСоединение(ПечатнаяФорма) Экспорт
	...
	Попытка
		ОбщегоНазначения.УдалитьВременныйКаталог(ПечатнаяФорма.ИмяКаталога);
	Исключение
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("ru='Не удалось удалить временный каталог шаблона печатной формы по причине:';uk='Не вдалося видалити тимчасовий каталог шаблону друкованої форми з причини:'") + Символы.ПС 
...
Процедура РазобратьКонтейнерДокументDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	Исключение
		...
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("ru='Не удалось открыть файла шаблона по причине:';uk='Не вдалося відкрити файл шаблона з причини:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		...
		УдалитьФайлы(ПолноеИмяФайла);
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("ru='Не удалось выполнить разбор файла шаблона по причине:';uk='Не вдалося виконати розбір файлу шаблону з причини:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура СобратьКонтейнерДокументаDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	Исключение
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("ru='Не удалось создать файл документа по причине:';uk='Не вдалося створити файл документа з причини:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		ЗаписатьСобытияВЖурналРегистрации(СобытиеЖурналаРегистрации(), "Ошибка", ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение(НСтр("ru='Не удалось сформировать файл документа по причине:';uk='Не вдалося сформувати файл документу з причини:'") + Символы.ПС 
			+ КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура РазбитьТекстШаблонаНаОбласти(ЧтениеXML, СтруктураДокумента, ПараметрыАнализа)
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеКонцаРаздела(СтруктураРазбораXML, ЧтениеXML) Тогда
			...
			СнятьБлокирующийПоток = Истина;
			
		КонецЕсли;
		
		Если СнятьБлокирующийПоток И ТекущийУровень = УровеньТэгаБлок Тогда
			СнятьБлокирующийПоток = Ложь;
