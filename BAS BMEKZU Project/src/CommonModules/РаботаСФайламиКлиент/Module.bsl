...
Процедура ПоказатьПодтверждениеЗакрытияФормыСФайлами(Форма, Отказ, ЗавершениеРаботы, ВладелецФайлов,
	...
	ТекстВопроса = НСтр("ru='Один или несколько файлов заняты для редактирования.
                              |
                              |Продолжить?'
                              |;uk='Один або декілька файлів зайняті для редагування.
                              |
                              |Продовжити?'");
	...
	Если ВладелецФайлов.Пустая() Тогда
		...
		ПоказатьВопрос(ОбработчикОповещенияОЗакрытии,
			НСтр("ru='Данные еще не записаны. 
                ...
                |Перехід до ""Приєднані файли"" можливий тільки після запису даних.'"),
...
Процедура ОткрытьФормуНастройкиСканирования() Экспорт
	...
	Если СистемнаяИнформация.ТипПлатформы <> ТипПлатформы.Windows_x86
		...
		ТекстСообщения = НСтр("ru='Для сканирования необходимо использовать 32-битную версию программы  для ОС Windows.';uk='Для сканування необхідно використовувати 32-бітну версію програми для ОС Windows.'");
...
Процедура НапечататьФайлПриложением(ДанныеФайла, ИмяОткрываемогоФайла)
	...
	Если СтрНайти(РасширенияИсключения, " " + Расширение + ",") > 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Печать файлов данного типа не поддерживается.';uk='Друк файлів даного типу не підтримується.'"), Расширение);
		ПоказатьПредупреждение(, ТекстСообщения);
...
#Если ВебКлиент Тогда
	Если ПараметрыВыполнения.ДанныеФайла.Расширение <> "mxl" Тогда
		Текст = НСтр("ru='Необходимо сохранить файл на компьютер, после чего выполнить
            ...
            |;uk='Необхідно зберегти файл на комп''ютер, після чого виконати
            |друк за допомогою додатку, призначеного
            |для роботи з даним файлом.'");
