...
Функция ДобавитьВалютыПоКоду(Знач Коды) Экспорт
	...
	Результат = Справочники.Валюты.ДобавитьВалютыПоКоду(Коды);
...
Функция СформироватьСуммуПрописью(СуммаЧислом, Валюта, ВыводитьСуммуБезКопеек = Ложь, КодЯзыка = "ru") Экспорт
	...
	Если Метаданные.ОбщиеМодули.Найти("Локализация") = Неопределено Тогда
		ПараметрыПрописи = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Валюта, "ПараметрыПрописиНаРусском");
		Результат = ЧислоПрописью(Сумма, "ДП=Ложь", ПараметрыПрописи);
	Иначе	
	    МодульЛокализация = ОбщегоНазначения.ОбщийМодуль("Локализация");
		ПараметрыПрописи = МодульЛокализация.ПараметрыПрописи(Валюта, КодЯзыка);
		Результат = ЧислоПрописью(Сумма, "Л="+МодульЛокализация.ОпределитьКодЯзыкаДляФормат(КодЯзыка)+";ДП=Ложь", ПараметрыПрописи);
	КонецЕсли;
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ОбъектМетаданных = Метаданные.Обработки.Найти("НСИСЗагрузкаКурсовВалют");
	Если ОбъектМетаданных = Неопределено Тогда
		ОбъектМетаданных = Метаданные.Обработки.Найти("ЗагрузкаКурсовВалютФинансУА");
		Если ОбъектМетаданных = Неопределено Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Дело.Важное         = Истина;
		Дело.Форма          = ОбъектМетаданных.ПолноеИмя() + ".Форма";
...
Процедура ПреобразованиеСвязейВалют() Экспорт
	Справочники.Валюты.ПреобразованиеСвязейВалют();
КонецПроцедуры
...
Процедура ЗагрузитьАктуальныйКурсФинансУА(ПараметрыЗагрузки = Неопределено, АдресРезультата = Неопределено) Экспорт
	
	Если Метаданные.Обработки.Найти("ЗагрузкаКурсовВалютФинансУА") <> Неопределено Тогда
		Обработки["ЗагрузкаКурсовВалютФинансУА"].ЗагрузитьАктуальныйКурс(ПараметрыЗагрузки, АдресРезультата);
	КонецЕсли;
...
Функция Разрешения()
	...
	ИмяОбработки = "ЗагрузкаКурсовВалютФинансУА";
