...
Функция НайденЗапретИзмененияДанных(Знач ДанныеДляПроверки,
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка в функции НайденЗапретИзмененияДанных общего модуля ДатыЗапретаИзменения.';uk='Помилка у функції НайденЗапретИзмененияДанных загального модуля ДатыЗапретаИзменения.'")
		+ Символы.ПС
		+ Символы.ПС;
	...
	Для Каждого Строка Из ДанныеДляПроверки Цикл
		...
		Если СвойстваРаздела = Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В параметре ДанныеДляПроверки указан несуществующий раздел ""%1"".';uk='У параметрі ДанныеДляПроверки вказаний неіснуючий розділ ""%1"".'"),
				Строка.Раздел);
		КонецЕсли;
