Процедура ЗарегистрироватьПечатнуюФорму_Сервер() Экспорт
	...
	Попытка
		...
		Если ЕстьСправочникДополнительныеОтчетыИОбработки <> Неопределено Тогда
			лпПустаяСсылка = Справочники.ДополнительныеОтчетыИОбработки.ПустаяСсылка();
			МакетПечатнаяФорма = ПолучитьОбщийМакет("скEDI_ВнешняяПечатнаяФормаЭлектронногоДокумента_УФ");	
			ПечатнаяФормаСсылка = Справочники.ДополнительныеОтчетыИОбработки.НайтиПоНаименованию(НСтр("ru = 'Электронный документ'; uk = 'Електронний документ'"));
			Если ПечатнаяФормаСсылка = лпПустаяСсылка Тогда
				ПечатнаяФормаСсылка = Справочники.ДополнительныеОтчетыИОбработки.НайтиПоНаименованию("Электронный документ");
			КонецЕсли;	
			...
			Если ПечатнаяФормаСсылка = лпПустаяСсылка Тогда		
				...
				ПечатнаяФорма.ИмяОбъекта   = НСтр("ru = 'Электронный документ'; uk = 'Електронний документ'");
				ПечатнаяФорма.Наименование = НСтр("ru = 'Электронный документ'; uk = 'Електронний документ'");
				ПечатнаяФорма.Комментарий  = НСтр("ru = 'Печатная форма Электронного документа (FlyDoc)'; uk = 'Друкована форма Електронного документа (FlyDoc)'");
			...
			Иначе
				ПечатнаяФорма = ПечатнаяФормаСсылка.ПолучитьОбъект();
				ПечатнаяФорма.ИмяОбъекта   = НСтр("ru = 'Электронный документ'; uk = 'Електронний документ'");
				ПечатнаяФорма.Наименование = НСтр("ru = 'Электронный документ'; uk = 'Електронний документ'");
				ПечатнаяФорма.Комментарий  = НСтр("ru = 'Печатная форма Электронного документа (FlyDoc)'; uk = 'Друкована форма Електронного документа (FlyDoc)'");
				ПечатнаяФорма.Назначение.Очистить();
				ПечатнаяФорма.ХранилищеОбработки = Новый ХранилищеЗначения(МакетПечатнаяФорма);
			КонецЕсли;
			...
			Если ПечатнаяФормаСсылка = лпПустаяСсылка Тогда
				...
				НоваяКоманда.Представление 			= НСтр("ru = 'Электронный документ'; uk = 'Електронний документ'");  
		...
		ИначеЕсли ЕстьСправочникВнешниеОбработки <> Неопределено Тогда
			лпПустаяСсылка = Справочники.ВнешниеОбработки.ПустаяСсылка();
			МакетПечатнаяФорма = ПолучитьОбщийМакет("скEDI_ВнешняяПечатнаяФормаЭлектронногоДокумента");
			ПечатнаяФормаСсылка = Справочники.ВнешниеОбработки.НайтиПоНаименованию(НСтр("ru = 'Электронный документ'; uk = 'Електронний документ'"));
			Если ПечатнаяФормаСсылка = лпПустаяСсылка Тогда
				ПечатнаяФормаСсылка = Справочники.ВнешниеОбработки.НайтиПоНаименованию("Электронный документ");
			КонецЕсли;
			Если ПечатнаяФормаСсылка = лпПустаяСсылка Тогда
			...
			Иначе
				...
				ПечатнаяФорма.Наименование = НСтр("ru = 'Электронный документ'; uk = 'Електронний документ'");
				ПечатнаяФорма.Комментарий  = НСтр("ru = 'Печатная форма Электронного документа (FlyDoc)'; uk = 'Друкована форма Електронного документа (FlyDoc)'");
				ПечатнаяФорма.Принадлежность.Очистить();
				ПечатнаяФорма.ХранилищеВнешнейОбработки = Новый ХранилищеЗначения(МакетПечатнаяФорма);
			КонецЕсли;
			
			Если ИДКонфигурации = "УТП" 
				ИЛИ ИДКонфигурации = "УПП" 
				ИЛИ ИДКонфигурации = "УТ" Тогда
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Налоговая накладная'; uk = 'Податкова накладна'");
				НоваяСтрока.СсылкаОбъекта = Документы.НалоговаяНакладная.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Приложение 2 к НН'; uk = 'Додаток 2 до ПН'");
				НоваяСтрока.СсылкаОбъекта = Документы.Приложение2КНалоговойНакладной.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Счет на оплату покупателю'; uk = 'Рахунок на оплату покупцю'");
				НоваяСтрока.СсылкаОбъекта = Документы.СчетНаОплатуПокупателю.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Заказ покупателя'; uk = 'Замовлення покупця'");
				НоваяСтрока.СсылкаОбъекта = Документы.ЗаказПокупателя.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Реализация Товаров и услуг'; uk = 'Реалізація товарів і послуг'");
				НоваяСтрока.СсылкаОбъекта = Документы.РеализацияТоваровУслуг.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Счет на оплату поставщика'; uk = 'Рахунок на оплату постачальника'");
				НоваяСтрока.СсылкаОбъекта = Документы.СчетНаОплатуПоставщика.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Заказ поставщику'; uk = 'Замовлення постачальнику'");
				НоваяСтрока.СсылкаОбъекта = Документы.ЗаказПоставщику.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Поступление товаров и услуг'; uk = 'Надходження товарів і послуг'");
				НоваяСтрока.СсылкаОбъекта = Документы.ПоступлениеТоваровУслуг.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Акт сверки взаиморасчетов'; uk = 'Акт звірки взаєморозрахунків'");
				НоваяСтрока.СсылкаОбъекта = Документы.АктСверкиВзаиморасчетов.ПустаяСсылка();
				НоваяСтрока = ПечатнаяФорма.Принадлежность.Добавить();
				НоваяСтрока.ПредставлениеОбъекта = НСтр("ru = 'Договоры контрагентов'; uk = 'Договори контрагентів'");
