...
Функция МенеджерСервисаПоддерживаетУправлениеТарифамиВПриложении_1_0_1_2() Экспорт
	...
	Попытка
		...
		КонецЕсли;
	Исключение
		ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru='МенеджерСервисаПоддерживаетУправлениеТарифамиВПриложении_1_0_1_2';uk='МенеджерСервисаПоддерживаетУправлениетарифамивприложении_1_0_1_2'",КодЯзыка());
...
Процедура ЗарегистрироватьТарифицируемыеУслуги_ОбработчикОбновления() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru='Регистрация тарифицируемых услуг. Обработчик обновления';uk='Реєстрація тарифікованих послуг. Обробник оновлення'",КодЯзыка());
	...
	Попытка
		...
		ТекстИнформации = НСтр("ru='Услуги успешно отправлены в Менеджер сервиса';uk='Послуги успішно відправлені в Менеджер сервісу'");
		ЗаписьЖурналаРегистрации(ИмяСобытия, 
...
Процедура ЗарегистрироватьТарифицируемыеУслуги_РегламентноеЗадание() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru='Регистрация тарифицируемых услуг. Регламентное задание';uk='Реєстрація тарифікованих послуг. Регламентне завдання'",КодЯзыка());
...
Процедура ЗапроситьЛицензииУникальныхУслуг_РегламентноеЗадание() Экспорт
	...
	ИмяСобытия = ИмяСобытияТарификации() + НСтр("ru='Запрос лицензий уникальных услуг. Регламентное задание';uk='Запит ліцензій унікальних послуг. Регламентне завдання'",КодЯзыка());
	...
	Попытка
		...
		ТекстИнформации = НСтр("ru='Успех.';uk='Успіх.'");
		ЗаписьЖурналаРегистрации(ИмяСобытия, 
			УровеньЖурналаРегистрации.Информация,
			,
			,
...
Функция ИмяСобытияТарификации() Экспорт
	...
	Возврат НСтр("ru='Управление тарифами в модели сервиса.';uk='Управління тарифами моделі сервісу.'",КодЯзыка());
...
Процедура ПроверитьСтруктуруПоставщиковУслуг(ПоставщикиУслуг)
	...
	Для Каждого Поставщик Из ПоставщикиУслуг Цикл
		...
		Иначе
			ВызватьИсключение НСтр("ru='Имеется несколько поставщиков услуг с идентифкатором:';uk='Є кілька постачальників послуг з идентифкатором:'") + " " + Поставщик.Идентификатор;
		КонецЕсли;
	КонецЦикла;
