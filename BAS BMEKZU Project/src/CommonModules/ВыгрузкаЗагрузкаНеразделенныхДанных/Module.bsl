...
Процедура КонтрольСсылкиНаНеразделенныеДанныеПриВыгрузке(Контейнер, Знач ПроверяемаяСсылка, Знач ИсходныйОбъект, Знач ИсходныйОбъектМетаданных, Знач ИмяПоля)
	...
	ШаблонОшибки =
		НСтр("ru='Объект метаданных %1 включен в перечень объектов, для которых не требуется сопоставление ссылок при выгрузке / загрузке
              ...
              |%5
              |---------------------------------------------------------------------------------------------------------------------------'
              |;uk='Об''єкт метаданих %1 включений в перелік об''єктів, для яких не потрібне співставлення посилань при вивантаженні / завантаженні
              |даних (у переобумовленій процедурі 
              |ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхНеТребующихСопоставлениеСсылокПриЗагрузке(),
              |але при цьому для нього не забезпечуються вимоги відсутності посилань, які не співставляються, при вивантаженні.
              |
              |Посилання, які не зіставляються, виявлене при вивантаженні об''єкту %2, у якого в якості значення реквізиту %3
              |встановлене посилання на об''єкт %1, яке не зможе бути коректно співставлене при завантаженні даних.
              |Потрібно переглянути логіку використання об''єкта %1 і забезпечити для нього відсутність посилань, які не співставляються, 
              ...
              |%4
              |---------------------------------------------------------------------------------------------------------------------------
              |2. Серіалізація об''єкта посилання, яка не співставляється
              |---------------------------------------------------------------------------------------------------------------------------
              |%5
              |---------------------------------------------------------------------------------------------------------------------------'");
