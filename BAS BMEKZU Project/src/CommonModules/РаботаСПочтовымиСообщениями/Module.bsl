...
Функция ОтправитьПочтовоеСообщение(Знач УчетнаяЗапись, Знач ПараметрыОтправки,
	...
	Если ТипЗнч(УчетнаяЗапись) <> Тип("СправочникСсылка.УчетныеЗаписиЭлектроннойПочты")
		...
		ВызватьИсключение НСтр("ru='Учетная запись не заполнена или заполнена неправильно.';uk='Обліковий запис не заповнено або заповнено неправильно.'");
	КонецЕсли;
	...
	Если ПараметрыОтправки.ОшибочныеПолучатели.Количество() > 0 Тогда
		ТекстОшибки = НСтр("ru='Следующие почтовые адреса не были приняты почтовым сервером:';uk='Наступні поштові адреси не були прийняті поштовим сервером:'");
