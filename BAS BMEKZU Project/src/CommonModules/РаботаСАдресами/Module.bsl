...
// Возвращает сведения об адресах в виде структуру частей адреса 
//
// Параметры:
//   Адреса                 - Строка - адрес во внутреннем формате JSON или в XML, соответствующем XDTO пакету Адрес.
...
//                                     то получение идентификаторов может инициировать http запрос к веб-сервису orgaddress.
...
//       * ПроверитьАдрес   - Булево - Если Истина, то адрес будет проверен на соответствие. По умолчанию, Ложь.
//                                     Если в программу не загружены проверяемые адресные объекты,
//                                     то проверка адреса не может быть выполнена.
//
// Возвращаемое значение:
...
// Возвращает сведения об адресе в виде отдельных частей адреса и различных кодов (код региона и др.).
//
// Параметры:
...
//       * БезПредставлений - Булево - Если Истина, то поле Представление будет отсутствовать. По умолчанию, Ложь.
//       * КодыАдреса       - Булево - Если Истина, то результат содержит поля ИдентификаторАдресногоОбъекта, ИдентификаторДома
//                                     и структура с кодами адреса(Идентификаторы, ДополнительныеКоды).
//                                     Подробнее см. в возвращаемом значении структуры Идентификаторы и ДополнительныеКоды. По умолчанию, Ложь.
//                                     Если в адресе отсутствуют идентификаторы и в программу не загружены идентификаторы адресных объектов адреса,
//                                     то получение идентификаторов может инициировать http запрос к веб-сервису orgaddress.
//       * ПолныеНаименованияСокращений - Булево - Если Истина, то возвращается полное наименование адресных объектов.
...
//                                     Если в программу не загружены проверяемые адресные объекты,
//                                     то проверка адреса может инициировать http запрос к веб-сервису orgaddress.
//
// Возвращаемое значение:
...
//        * ТипАдреса        - Строка - основной тип адреса (только для адресов).
//                                      Варианты: "Муниципальный", "Административно-территориальный".
//        * Страна           - Строка - текстовое представление страны.
//        * КодСтраны        - Строка - код страны по ОКСМ (может быть пустой, если поле страна не заполнена).
...
//                                     171н.
//            ** Номер - Строка  - текстовое представление номера дома (только для адресов).
...
//        * ДополнительныеКоды  - Структура - Коды. Поле отсутствует, если дополнительный параметр КодыАдреса равен Ложь.
...
Функция АдресПоИдентификатору(ИдентификаторАдреса, ДополнительнаяИнформацияАдреса = Неопределено) Экспорт
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("НомерДома") И ЗначениеЗаполнено(ДополнительнаяИнформацияАдреса.НомерДома) Тогда
		...
		ОписаниеДома = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.НомерДома, НСтр("ru='Дом';uk='Будинок'"));
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("НомерСтроения") И ЗначениеЗаполнено(ДополнительнаяИнформацияАдреса.НомерСтроения) Тогда
		...
		ОписаниеСтроения = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.НомерСтроения, НСтр("ru='Корпус';uk='Корпус'"));
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("НомерОфиса") И ЗначениеЗаполнено(ДополнительнаяИнформацияАдреса.НомерОфиса) Тогда
		...
		ОписаниеПомещения = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.НомерОфиса, НСтр("ru='Офис';uk='Офіс'"));
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("АбонентскийЯщик") И ЗначениеЗаполнено(ДополнительнаяИнформацияАдреса.АбонентскийЯщик) Тогда
		...
		ОписаниеАбонентскийЯщик = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.АбонентскийЯщик, НСтр("ru='А/Я';uk='А/С'"));
...
//        ** Индекс           - Строка - Почтовый индекс (только для национальных адресов).
//        ** Регион           - Строка - Представление региона (только для национальных адресов).
//        ** КодРегиона       - Строка - Код региона (только для национальных адресов).
//        ** РегионСокращение - Строка - Сокращение региона (если СтарыйСоставПолей = Ложь).
//        ** Район            - Строка - Представление района (только для национальных адресов).
//        ** РайонСокращение  - Строка - Сокращение района (если СтарыйСоставПолей = Ложь).
//        ** Город            - Строка - Представление города (только для национальных адресов).
//        ** ГородСокращение  - Строка - Сокращение города (только для национальных адресов).
//        ** НаселенныйПункт  - Строка - Представление населенного пункта (только для национальных адресов).
//        ** НаселенныйПунктСокращение - Строка - сокращение населенного пункта (если СтарыйСоставПолей = Ложь).
//        ** Улица            - Строка - Представление улицы (только для национальных адресов).
//        ** УлицаСокращение  - Строка - Сокращение улицы (если СтарыйСоставПолей = Ложь).
//        ** ТипДома          - Строка - Тип дома см. РаботаСАдресамиКлиентСервер.ТипыОбъектовАдресацииАдреса.
//        ** Дом              - Строка - Представление дома (только для национальных адресов).
//        ** ТипКорпуса       - Строка - Тип корпуса см. РаботаСАдресамиКлиентСервер.ТипыОбъектовАдресацииАдреса.
//        ** Корпус           - Строка - Представление корпуса (только для национальных адресов).
//        ** ТипКвартиры      - Строка - Тип квартиры см. РаботаСАдресамиКлиентСервер.ТипыОбъектовАдресацииАдреса.
//        ** Квартира         - Строка - Представление квартиры (только для национальных адресов).
...
//        ** ТипКвартиры      - Строка - Тип квартиры см. РаботаСАдресамиКлиентСервер.ТипыОбъектовАдресацииАдреса.
//        ** Квартира         - Строка - Представление квартиры (только для национальных адресов).
//        ** АдресРФ          - Булево - Если Истина, то адрес национальный.
...
Функция ДополнительныеПравилаПреобразования() Экспорт
	...
	Для Каждого ДополнительныйАдресныйЭлемент Из РаботаСАдресамиКлиентСервер.ТипыОбъектовАдресацииАдреса() Цикл
...
Функция СтруктураАдресаВСтруктуруJSON(Знач КонтактнаяИнформация)
	...
	Если КонтактнаяИнформация.Свойство("Здание") 
		...
		И КонтактнаяИнформация.Здание.Свойство("Номер") Тогда
			...
			ОписаниеКонтактнойИнформации.HouseType = ?(КонтактнаяИнформация.Здание.Свойство("ТипЗдания"), КонтактнаяИнформация.Здание.ТипЗдания, НСтр("ru='Дом';uk='Будинок'"));
		
	КонецЕсли;
	
	Если КонтактнаяИнформация.Свойство("Корпус")И ЗначениеЗаполнено(КонтактнаяИнформация.Корпус) Тогда
		
		ТипКорпуса = ?(КонтактнаяИнформация.Свойство("ТипКорпуса"), КонтактнаяИнформация.ТипКорпуса, НСтр("ru='Корпус';uk='Корпус'"));
	...
	Если КонтактнаяИнформация.Свойство("Квартира") И ЗначениеЗаполнено(КонтактнаяИнформация.Квартира) Тогда
		...
		ТипКвартиры = ?(КонтактнаяИнформация.Свойство("ТипКвартиры"), КонтактнаяИнформация.ТипКвартиры, НСтр("ru='Квартира';uk='Квартира'"));
...
//       * ПроверитьАдрес   - Булево - Если Истина, то адрес будет проверен на соответствие.
//
// Возвращаемое значение:
//   Структура - сведения об адресе:
//        * Представление    - Строка - текстовое представление адреса по административно-территориальному делению.
...
Функция СведенияОбАдресеВВидеСтруктуры(Знач АдресСтрокой, ДополнительныеПараметры)
	...
	Результат = КонструкторПолейАдреса();
	...
	ПолученныйАдрес = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(АдресСтрокой, Перечисления.ТипыКонтактнойИнформации.Адрес);
	...
	Адрес.type = УправлениеКонтактнойИнформациейКлиентСервер.ТипКонтактнойИнформацииВСтроку(Перечисления.ТипыКонтактнойИнформации.Адрес);
	...
	Результат.КодСтраны   = КодСтраны(Адрес);
	...
	Для каждого ЧастьАдреса Из СоответствиеУровней Цикл
		...
		Результат.Вставить(ЧастьАдреса.Значение.Имя + "Сокращение", ТипКраткий);
		Результат.Вставить(ЧастьАдреса.Значение.Имя + "ТипКраткий", ТипКраткий);
	...
	Результат.НомерЗемельногоУчастка         = "";
	Результат.ИдентификаторЗемельногоУчастка = "";
...
Функция КонструкторПолейАдреса()
...
Функция КодСтраны(Знач Адрес)
	...
	Если НЕ ЗначениеЗаполнено(КодСтраны) Тогда
		...
		Если СведенияОСтране <> Неопределено Тогда
			КодСтраны = ?(СведенияОСтране <> Неопределено, СведенияОСтране.Код, "");
		КонецЕсли;
	КонецЕсли;
