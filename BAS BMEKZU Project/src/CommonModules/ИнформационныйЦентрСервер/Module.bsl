...
Функция ПолучитьОбщиеМакетыИнформационныхСсылок() Экспорт
	...
	ЯзыкЗаполнения =  ?((КодЛокализацииИБ = "uk_UA") или (КодЛокализацииИБ = "uk"),"uk","ru");
	ИмяМакета = ?(ЯзыкЗаполнения = "ru", "ИнформационныеСсылки_Общие", "ИнформационныеСсылки_Общие_uk");
	МассивМакетов.Добавить(ПолучитьОбщийМакет(ИмяМакета));
...
Функция ПолучитьПроксиНовостейСлужбыПоддержки() Экспорт
	...
	Если Не ТехнологияСервисаИнтеграцияСБСП.ПодсистемаСуществует("СтандартныеПодсистемы.РаботаВМоделиСервиса") Тогда
		ВызватьИсключение(НСтр("ru='Сервис новостей доступен только в модели сервиса.';uk='Сервіс новин доступний тільки в моделі сервісу.'"));
	КонецЕсли;
...
Функция СформироватьXMLСТехническимиПараметрами(ДополнительныеПараметры = Неопределено) Экспорт
	...
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru='Информационный центр.Отправка сообщения в техподдержку.Не удалось удалить временный файл технических параметров.';uk='Інформаційний центр.Відправлення повідомлення в техпідтримку.Не вдалося вилучити файл технічних параметрів.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()), 
