...
Функция НаселенныйПунктПоАдресу(ЗначениеАдресаJSON) Экспорт
	...
	ШаблонНаименованияГорода = НСтр("ru='%1. %2';uk='%1. %2'");
...
Процедура ПередЗаписьюИсторииКонтактнойИнформацииОбъекта(Объект) Экспорт
	...
	ИначеЕсли Объект.ИсторияКонтактнойИнформации.Количество() > 1 Тогда
		...
		Если ТаблицаИстории.Количество() <> Объект.ИсторияКонтактнойИнформации.Количество() Тогда
			ТекстСообщения = НСтр("ru='В истории контактной информации есть записи с одинаковым периодом и видом.';uk='В історії контактної інформації є записи з однаковим періодом і видом.'");
...
Процедура ПредставительКонтрагентаНачалоВыбора(Знач Контрагент, ДанныеВыбора, СтандартнаяОбработка) Экспорт
	...
	Пока РезультатЗапроса.Следующий() Цикл
		...
		Представление = (?(ПустаяСтрока(РезультатЗапроса.Должность),"",СокрЛП(РезультатЗапроса.Должность) + " ") + 
						 ?(ПустаяСтрока(РезультатЗапроса.Фамилия),"",СокрЛП(РезультатЗапроса.Фамилия) + " ") + 
						 ?(ПустаяСтрока(РезультатЗапроса.Имя),"",СокрЛП(РезультатЗапроса.Имя) + " ") + 
						 СокрЛП(РезультатЗапроса.Отчество));
			
		Если ЗначениеЗаполнено(Представление) Тогда 
			ДанныеВыбора.Добавить(Представление);
		КонецЕсли;
...
//
// Параметры:
//  Форма	 - УправляемаяФорма - Форма, которая содержит контактную информацию.
...
Процедура УстановитьАктуальноеЗначениеИсторииИзмененийАдреса(Форма, Элемент) Экспорт
	...
	Если КоличествоЗаписей > 0 Тогда
		...
		ДанныеСтроки = ЗначениеКонтактнойИнформацииФормы(Форма, Элемент.Имя);
...
Функция СтрокаСсылкиПоказатьНаКарте() Экспорт
	...
	СоставСтроки.Добавить(НСтр("ru='Показать на карте';uk='Показати на мапі'"));
