...
Функция ЗначенияРеквизитовОбъекта(Ссылка, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("Строка") Тогда 
		...
		Исключение
			...
			НСтр("ru='Неверный первый параметр Ссылка:
                       |%1'
                       |;uk='Невірний перший параметр Посилання:
		...
		Если Ссылка = Неопределено Тогда 
			...
			Если Не ПравоДоступа("Чтение", МетаданныеОбъекта) Тогда 
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Недостаточно прав для работы с таблицей ""%1""';uk='Недостатньо прав для роботи з таблицею ""%1""'"), ПолноеИмяОбъектаМетаданных);
			КонецЕсли;
	...
	Иначе // Если передана ссылка.
		...
		Исключение
			ВызватьИсключение НСтр("ru='Неверный первый параметр Ссылка: 
                                         ...
                                         |;uk='Невірний перший параметр Посилання: 
                                         |- Значення повинно бути посиланням або ім''ям напередвизначеного елемента'");	
		КонецПопытки;
		...
		Для Каждого Реквизит Из Реквизиты Цикл
			...
			Исключение 
				...
				Если Результат.Ошибка Тогда 
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Неверный второй параметр Реквизиты: %1';uk='Невірний другий параметр Реквізити: %1'"), Результат.ОписаниеОшибки);
				КонецЕсли;
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверный тип второго параметра Реквизиты: %1';uk='Невірний тип другого параметра Реквізити: %1'"), Строка(ТипЗнч(Реквизиты)));
	КонецЕсли;
...
Функция ЗначенияРеквизитовОбъектов(Ссылки, Знач Реквизиты, ВыбратьРазрешенные = Ложь) Экспорт
	...
	Если ПустаяСтрока(Реквизиты) Тогда 
		ВызватьИсключение НСтр("ru='Неверный второй параметр Реквизиты: 
                                     ...
                                     |;uk='Невірний другий параметр Реквізити: 
	...
	Исключение
		ВызватьИсключение НСтр("ru='Неверный первый параметр Ссылки: 
                                     ...
                                     |;uk='Невірний перший параметр Посилання: 
                                     |- Значення масиву повинні бути посиланнями'");
	КонецПопытки;
...
//           вспомогательными данными (ведущее измерение или попали в исключение ПриДобавленииИсключенийПоискаСсылок).
...
Функция МестаИспользования(Знач НаборСсылок, Знач АдресРезультата = "") Экспорт
	...
	Для Каждого МестоИспользования Из МестаИспользования Цикл
		...
		ИначеЕсли МетаданныеКонстант.Содержит(МестоИспользования.Метаданные) Тогда
			Представление = МестоИспользования.Метаданные.Представление() + " (" + НСтр("ru='константа';uk='константа'") + ")";
			
		ИначеЕсли МетаданныеПоследовательностей.Содержит(МестоИспользования.Метаданные) Тогда
			Представление = МестоИспользования.Метаданные.Представление() + " (" + НСтр("ru='последовательность';uk='послідовність'") + ")";
...
//     Обработки.ЗагрузкаКурсовВалют.ПолучитьМакет("КлассификаторВалют").ПолучитьТекст()).Данные;
//
Функция ПрочитатьXMLВТаблицу(Знач XML) Экспорт
	...
	Если Метаданные.ОбщиеМодули.Найти("Локализация") = Неопределено Тогда
		КодЯзыка = "ru";	
	Иначе	
	    МодульЛокализация = ОбщегоНазначения.ОбщийМодуль("Локализация");
		КодЯзыка = МодульЛокализация.КодЯзыкаИнформационнойБазы();
	КонецЕсли; 
	ИменаКолонокXML = СтрЗаменить(ИменаКолонок, "#", ?(КодЯзыка = "uk", "Ukr", ""));
...
Функция СократитьСтрокуКонтрольнойСуммой(Строка, МаксимальнаяДлина) Экспорт
	ОбщегоНазначенияКлиентСервер.Проверить(МаксимальнаяДлина >= 32, НСтр("ru='Параметр МаксимальнаяДлина не может быть меньше 32';uk='Параметр МаксимальнаяДлина не може бути менше 32'"),
	...
	Если Не СтрНачинаетсяС(СтрЗаменить(ПутьККаталогу, "/", "\"), СтрЗаменить(КаталогВременныхФайлов(), "/", "\")) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неверное значение параметра ПутьККаталогу в ОбщегоНазначения.УдалитьВременныйКаталог:
                       ...
                       |;uk='Невірне значення параметра ПутьККаталогу в ОбщегоНазначения.УдалитьВременныйКаталог:
                       |Каталог не є тимчасовим ""%1""'"), 
			ПутьККаталогу);
	КонецЕсли;
		...
		ЗаписьЖурналаРегистрации(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось удалить временный каталог ""%1"" по причине:
                    |%2'
                    |;uk='Не вдалося видалити тимчасовий каталог ""%1"" з причини:
                    |%2'"),
				ПутьККаталогу,
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось проверить доступность web-сервиса
                           ...
                           |%3'
                           |;uk='Не вдалося перевірити доступність web-сервісу
                           |%1
                           |з причини:
                           |%2
                           |
			...
			ЗаписьЖурналаРегистрации(НСтр("ru='WSПрокси';uk='WSПроксі'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
...
Функция ОбщиеПараметрыБазовойФункциональности() Экспорт
	...
	Если Не ПустаяСтрока(Минимальная)
		...
		ТекстСообщения = НСтр("ru='Минимальная версия платформы указана выше рекомендуемой.
            |Минимальная версия - ""%1"", рекомендуемая версия - ""%2"".'
            |;uk='Мінімальна версія платформи вказана вище рекомендованої.
            |Мінімальна версія - ""%1"", рекомендована версія - ""%2"".'");
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ТекстСообщения,
...
// Возвращает Истина, если эта информационная база подключена к Fresh.
//
// Возвращаемое значение:
		...
		Если Не Доступно Тогда
			...
			ВызватьИсключение
				НСтр("ru='Регламентное задание недоступно по функциональным опциям или
                           ...
                           |;uk='Регламентне завдання недоступне за функціональними опціями або
                           |не підтримує роботу в поточному режимі роботи програми.
                           |Виконання перервано. Завдання відключено.'");
		КонецЕсли;
	КонецЕсли;
...
//            Например, "http://bas-soft.eu/database".
//
// Пример: 
...
Функция ВидОбъектаПоТипу(ТипОбъекта) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неверный тип значения параметра (%1)';uk='Невірний тип значення параметра (%1)'"), Строка(ТипОбъекта));
...
Функция ПолучитьВерсииИнтерфейсаЧерезВнешнееСоединение(ВнешнееСоединение, Знач ИмяИнтерфейса) Экспорт
	...
	Исключение
		СтрокаСообщения = НСтр("ru='Корреспондент не поддерживает версионирование программных интерфейсов.
            |Описание ошибки: %1'
            |;uk='Кореспондент не підтримує версіонування програмних інтерфейсів.
            |Опис помилки: %1'");
...
Процедура СкопироватьСтрокиВБуферОбмена(ТабличнаяЧасть, ВыделенныеСтроки, Источник = Неопределено) Экспорт
	...
	Если ТаблицаЗначений.Колонки.Найти("Счет") <> Неопределено Тогда
		ТаблицаЗначений.Колонки.Счет.Имя = "СчетУчетаБУ";	
	КонецЕсли;
...
// Выполняет произвольный алгоритм на встроенном языке, предварительно устанавливая
...
Функция ПодключитьКомпонентуИзМакета(Идентификатор, ПолноеИмяМакета) Экспорт
	...
	Если Не МакетСуществует(ПолноеИмяМакета) Тогда 
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось подключить внешнюю компоненту ""%1"" на сервере
                       ...
                       |;uk='Не вдалося підключити зовнішню компоненту ""%1"" на сервері
                       |з %2
                       |через:
                       |Підключення на сервері не з макета заборонено'"),
			Идентификатор,
			ПолноеИмяМакета);
	КонецЕсли;
	...
	Если ПодключитьВнешнююКомпоненту(Местоположение, СимволическоеИмя) Тогда
		...
		Попытка
			...
			Если ПодключаемыйМодуль = Неопределено Тогда 
				ВызватьИсключение НСтр("ru='Оператор Новый вернул Неопределено';uk='Оператор Новый повернув Неопределено'");
			КонецЕсли;
		Исключение
			ПодключаемыйМодуль = Неопределено;
		...
		Если ПодключаемыйМодуль = Неопределено Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось создать объект внешней компоненты ""%1"", подключенной на сервере
                           ...
                           |по причине:
                           |%3'
                           |;uk='Не вдалося створити об''єкт зовнішньої компоненти ""%1"", підключеної на сервері
                           |з макета ""%2"",
                           |з причини:
                           |%3'"),
				Идентификатор,
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru='Подключение внешней компоненты на сервере';uk='Підключення зовнішньої компоненти на сервері'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Иначе
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось подключить внешнюю компоненту ""%1"" на сервере
                       ...
                       |з макета ""%2""
                       |через:
                       |Метод ПодключитьВнешнююКомпоненту повернув Хибність.'"),
			Идентификатор,
			Местоположение);
...
Функция Просклонять(Знач ФИО, Падеж, Результат, Пол = Неопределено) Экспорт
	...
	ВызватьИсключение НСтр("ru='Для вызова ОбщегоНазначения.Просклонять необходима подсистема «Склонение представлений объектов».';uk='Для виклику ОбщегоНазначения.Просклонять необхідна підсистема «Відмінювання представлень об''єктів».'");
...
Процедура ПроизвестиЗаменуВКонстанте(Результат, Знач МестоИспользования, Знач ПараметрыЗаписи, Знач ВнутренняяТранзакция = Истина)
	...
	Попытка
		Если ВнутренняяТранзакция Тогда
			...
			Исключение
				Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось заблокировать константу %1';uk='Не вдалося заблокувати константу %1'"), 
					ПредставлениеДанных);
		...
		Исключение
			...
			Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось записать %1 по причине: %2';uk='Не вдалося записати %1 через: %2'"), 
...
Процедура ПроизвестиЗаменуВОбъекте(Результат, Знач МестоИспользования, Знач ПараметрыВыполнения, Знач ВнутренняяТранзакция = Истина)
	...
	Попытка
		...
		Исключение
			СостояниеОперации = "ОшибкаЗаписи";
			ОписаниеОшибки = КраткоеПредставлениеОшибки(ИнформацияОбОшибке());
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось записать %1 по причине: %2';uk='Не вдалося записати %1 через: %2'"), 
				ПредставлениеДанных, ОписаниеОшибки);
			ВызватьИсключение;
		КонецПопытки;
	...
	Попытка
		...
		Если ВнутренняяТранзакция Тогда
			...
			Попытка
				Блокировка.Заблокировать();
			Исключение
				Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось заблокировать набор %1';uk='Не вдалося заблокувати набір %1'"), 
					ПредставлениеДанных);
				СостояниеОперации = "ОшибкаБлокировки";
				ВызватьИсключение;
			КонецПопытки;
	...
	Попытка
		...
		Для Каждого Ссылка Из Удаляемые Цикл
			...
			Попытка
				...
				КонецЕсли;
			Исключение
				ТекстОшибки = НСтр("ru='Ошибка удаления';uk='Помилка вилучення'")
					+ Символы.ПС
...
Процедура ЗаписатьОбъект(Знач Объект, Знач ПараметрыЗаписи)
	...
	Если СвойстваОбъекта.Иерархический Или СвойстваОбъекта.ВидыСубконто <> Неопределено Тогда 
		...
		Если Объект.Родитель = Объект.Ссылка Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='При записи ""%1"" возникает циклическая ссылка в иерархии.';uk='При запису ""%1"" виникає циклічне посилання в ієрархії.'"),
				Строка(Объект));
			КонецЕсли;
	...
	Если СвойстваОбъекта.Владельцы.Количество() > 1 И Объект.Владелец = Объект.Ссылка Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='При записи ""%1"" возникает циклическая ссылка в подчинении.';uk='При запису ""%1"" виникає циклічне посилання в підпорядкуванні.'"),
			Строка(Объект));
	КонецЕсли;
...
Процедура ЗаменитьСсылкуОднойТранзакцией(Результат, Знач Дубль, Знач ПараметрыВыполнения, Знач ТаблицаПоиска)
	...
	Исключение
		...
		Если СостояниеОперации = "ОшибкаБлокировки" Тогда
			...
			Ошибка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Не удалось заблокировать все места использования %1:';uk='Не вдалося заблокувати всі місця використання %1:'") 
...
Функция СерверныйМодульМенеджера(Имя)
	...
	Если Не ОбъектНайден Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Об''єкт метаданих ""%1"" не знайдено,
...
// на встроенном языке перед использованием его в операторах Выполнить и Вычислить
...
Процедура ПроверитьИмяПроцедурыКонфигурации(Знач ИмяПроцедуры)
	...
	Если ЧастиИмени.Количество() <> 2 И ЧастиИмени.Количество() <> 3 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяПроцедуры (передано значение: ""%1"") в ОбщегоНазначения.ВыполнитьМетодКонфигурации';uk='Неправильний формат параметра ИмяПроцедуры (передано значення: ""%1"") в ОбщегоНазначения.ВыполнитьМетодКонфигурации'"), ИмяПроцедуры);
	КонецЕсли;
	...
	Если ЧастиИмени.Количество() = 2 И Метаданные.ОбщиеМодули.Найти(ИмяОбъекта) = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неправильный формат параметра ИмяПроцедуры (передано значение: ""%1"") в ОбщегоНазначения.ВыполнитьМетодКонфигурации:
                ...
                |;uk='Неправильний формат параметра ИмяПроцедуры (передано значення: ""%1"") в ОбщегоНазначения.ВыполнитьМетодКонфигурации:
...
Функция НайтиОшибкуДоступностиРеквизитовОбъекта(ПолноеИмяОбъектаМетаданных, ПроверяемыеВыражения)
	...
	Если МетаданныеОбъекта = Неопределено Тогда 
		Возврат Новый Структура("Ошибка, ОписаниеОшибки", Истина, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка получения метаданных ""%1""';uk='Помилка отримання метаданих ""%1""'"), ПолноеИмяОбъектаМетаданных));
	КонецЕсли;
	...
	Для Каждого ТекущееВыражение Из ПроверяемыеВыражения Цикл
		...
		Если Не ПолеИсточникаСхемыЗапросаДоступно(Источник, ТекущееВыражение) Тогда 
			ТекстОшибки = ТекстОшибки + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='- Поле объекта ""%1"" не найдено';uk='- Поле об''єкта ""%1"" не знайдено'"), ТекущееВыражение);
		КонецЕсли;
