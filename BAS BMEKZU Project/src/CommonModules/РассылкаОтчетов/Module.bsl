...
Функция ВыполнитьРассылкуОтчетов(Рассылка, ПараметрыЖурнала = Неопределено, ДополнительныеНастройки = Неопределено) Экспорт
	...
	КонецЕсли;
	Если ФорматыПоУмолчанию.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru='Не установлены форматы по умолчанию.';uk='Не встановлені формати по умовчанню.'");
	КонецЕсли;
...
Функция ВыполнитьРассылку(Отчеты, ПараметрыДоставки, НаименованиеРассылки = "", ПараметрыЖурнала = Неопределено) Экспорт
	...
	Для Каждого СтрокаОтчет Из Отчеты Цикл
		...
		КонецЕсли;
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание, ТекстЖурнала, , "'" + Строка(СтрокаОтчет.Отчет) + "'");
		...
		Если ПараметрыДоставки.Персонализирована И НЕ ПараметрыОтчета.Персонализирован Тогда
			ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Отчет ""%1"" не может сформирован, так как в его настройках не указан отбор по получателю рассылки.';uk='Звіт ""%1"" не може сформований, так як в його настройках не зазначено відбір за одержувачем розсилки.'"),
		...
		Попытка
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Примечание,
				НСтр("ru='Отчет %1 успешно сформирован';uk='Звіт %1 успішно сформований'"), , "'"+ Строка(СтрокаОтчет.Отчет) +"'");
			НомерОтчета = НомерОтчета + 1;
		Исключение
			ЗаписьЖурнала(ПараметрыЖурнала, ,
				НСтр("ru='Отчет %1 не сформирован:';uk='Звіт %1 не сформований:'"), ИнформацияОбОшибке(), "'"+ Строка(СтрокаОтчет.Отчет) +"'");
		КонецПопытки;
	КонецЦикла;
	...
	Если ДеревоОтчетов.Строки.Найти(3, "Уровень", Истина) = Неопределено Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Предупреждение,
			НСтр("ru='Рассылка отчетов не выполнена, так как отчеты пустые или не сформированы из-за ошибок.';uk='Розсилка звітів не виконана, так як звіти порожні або не сформовані через помилки.'"), ,);
		ОбщегоНазначения.УдалитьВременныйКаталог(ПараметрыДоставки.КаталогВременныхФайлов);
		Возврат Ложь;
	КонецЕсли;
...
Процедура ФормаОтчетаДобавитьКоманды(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если КоличествоРассылокСОтчетом > 0 Тогда
		...
		КомандаРассылки.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Рассылки отчета (%1)';uk='Розсилання звіту (%1)'"), 
			КоличествоРассылокСОтчетом);
		КомандаРассылки.Подсказка = НСтр("ru='Открыть список рассылок, в которые включен отчет.';uk='Відкрити список розсилок, в які включений звіт.'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru='Отключает неиспользуемые регламентные задания рассылок отчетов.';uk='Відключає невикористовувані регламентні завдання розсилок звітів.'");
...
Процедура АктуализироватьСписокРегламентныхЗаданий(Параметры) Экспорт
	...
	Пока РассылкиОтчетов.Следующий() Цикл
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось обработать регламентное задание рассылки отчетов: %1 с типом %2 по причине: %3';uk='Не вдалося обробити регламентне завдання розсилки звітів: %1 з типом %2 з причини: %3'"),
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Процедуре АктуализироватьСписокРегламентныхЗаданий не удалось обработать некоторые регламентные задания рассылки отчетов (пропущены): %1';uk='Процедурі АктуализироватьСписокРегламентныхЗаданий не вдалося обробити деякі регламентні завдання розсилки звітів (пропущені): %1'"), 
				ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.РассылкиОтчетов,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Процедура АктуализироватьСписокРегламентныхЗаданий обработала очередную порцию рассылки отчетов: %1';uk='Процедура АктуализироватьСписокРегламентныхЗаданий опрацювала чергову порцію розсилки звітів: %1'"),
					ОбъектовОбработано));
	КонецЕсли;
	...
	Если Не ПравоДоступа("Чтение", Метаданные.Справочники.РассылкиОтчетов) Тогда
		ВызватьИсключение
			НСтр("ru='У текущего пользователя недостаточно прав для чтения рассылок отчетов.
                ...
                |;uk='У поточного користувача недостатньо прав для читання розсилок звітів.
                |Рекомендується відключити всі розсилки цього користувача або змінити автора його розсилок (на вкладці ""Розклад"").'");
	КонецЕсли;
	...
	Если Не Выборка.Следующий() Тогда
		ВызватьИсключение
			НСтр("ru='У текущего пользователя недостаточно прав для чтения этой рассылки.
                ...
                |;uk='У поточного користувача недостатньо прав для читання цієї розсилки.
                |Рекомендується змінити автора розсилки (на вкладці ""Розклад"").'");
	КонецЕсли;
	Если Не Выборка.ВыполнятьПоРасписанию Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='У рассылки отчетов ""%1"" отключен флажок ""Выполнять по расписанию""
                ...
                |;uk='У розсилки звітів ""%1"" відключений прапорець ""Виконувати за розкладом""
                |Рекомендується відключити відповідне регламентне завдання або перезаписати цю розсилку.'"),
			Строка(Рассылка));
	КонецЕсли;
...
Функция СформироватьСписокПолучателейРассылки(ПараметрыЖурнала, Рассылка) Экспорт
	...
	Если СписокПолучателей.Количество() = 0 Тогда
        ...
        |- Одержувачі позначені для вилучення;
        |- Виключені всі отримувачі (виняток має найвищий пріоритет; учасники виключених груп також виключаються з списку);
        |- Недостатньо прав доступу до довідника;
        |- Рекомендується перевірити коректність настройки облікового запису електронної пошти, від імені якої виконується розсилка.'");
...
Функция ИнициализироватьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ПерсонализацияВозможна, УникальныйИдентификаторФормы = Неопределено) Экспорт
	...
	Исключение
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось подключить и загрузить настройки отчета ""%1"".';uk='Не вдалося підключити і завантажити настройки звіту ""%1"".'"),
	...
	Если РассылкаОтчетовПовтИсп.ИсключаемыеОтчеты().Найти(Подключение.СсылкаОтчета) <> Неопределено Тогда
		ПараметрыОтчета.Ошибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Звіт ""%1"" не призначений для розсилання.
...
Процедура СформироватьИСохранитьОтчет(ПараметрыЖурнала, ПараметрыОтчета, ДеревоОтчетов, ПараметрыДоставки, ПолучательСсылка)
	...
	Для Каждого Формат Из ПараметрыОтчета.Форматы Цикл
		...
		Если Не ВременныйФайл.Существует() Тогда
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				ЗаголовокОшибки + Символы.ПС + НСтр("ru='Файл %3 не найден.';uk='Файл %3 не знайдено.'"), ,
				"'"+ СтрокаОтчет.Ключ +"'", 
				"'"+ ПараметрыФормата.Имя +"'",
...
Функция ПроверитьИДозаполнитьПараметрыВыполнения(ТаблицаОтчетов, ПараметрыДоставки, НаименованиеРассылки, ПараметрыЖурнала)
	...
	Если Недоступные.Количество() > 0 Тогда
		ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
			НСтр("ru='В рассылке есть недоступные отчеты (%1):%2';uk='В розсилці є недоступні звіти (%1):%2'"), ,
	...
	Если ПараметрыДоставки.ИспользоватьПапку Тогда
		...
		Иначе
			...
			Если Не ПравоДоступа Тогда
				...
				УстановитьПривилегированныйРежим(Ложь);
				ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
					НСтр("ru='Недостаточно прав для создания файлов в папке ""%1"".';uk='Недостатньо прав для створення файлів у папці ""%1"".'"), ,
					ПредставлениеПапки);
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;
	...
	Если ПараметрыДоставки.ИспользоватьСетевойКаталог Тогда
		Если Не ЗначениеЗаполнено(ПараметрыДоставки.СетевойКаталогWindows) 
			...
			ЗаписьЖурнала(ПараметрыЖурнала, УровеньЖурналаРегистрации.Ошибка,
				НСтр("ru='Сетевой каталог %1 не выбран, доставка в сетевой каталог отключена';uk='Мережевий каталог %1 не обраний, доставка в мережевий каталог відключена'"), ,
				ЗначениеПодстановки);
	...
	Если ПараметрыДоставки.ИспользоватьЭлектроннуюПочту Тогда
		...
		ЯзыкШаблона = Локализация.КодЯзыкаИнформационнойБазы();
		
		// Шаблон темы
		Если Не ПараметрыДоставки.Свойство("ШаблонТемы") Или Не ЗначениеЗаполнено(ПараметрыДоставки.ШаблонТемы) Тогда
			ПараметрыДоставки.Вставить("ШаблонТемы", РассылкаОтчетовКлиентСервер.ШаблонТемы(ЯзыкШаблона));
		КонецЕсли;
		
		// Шаблон сообщения
		Если Не ПараметрыДоставки.Свойство("ШаблонТекста") Или Не ЗначениеЗаполнено(ПараметрыДоставки.ШаблонТекста) Тогда
			ПараметрыДоставки.Вставить("ШаблонТекста", РассылкаОтчетовКлиентСервер.ШаблонТекста(ЯзыкШаблона));
	...
	ПараметрыДоставки.Вставить("КаталогВременныхФайлов", ОбщегоНазначения.СоздатьВременныйКаталог("RP"));
	...
	Если ПараметрыДоставки.Архивировать Тогда
		Если Не ПараметрыДоставки.Свойство("ИмяАрхива") Или Не ЗначениеЗаполнено(ПараметрыДоставки.ИмяАрхива) Тогда
			ПараметрыДоставки.Вставить("ИмяАрхива", РассылкаОтчетовКлиентСервер.ШаблонИмениАрхива(ЯзыкШаблона));
		КонецЕсли;
