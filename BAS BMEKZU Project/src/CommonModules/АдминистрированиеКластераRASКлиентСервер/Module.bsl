...
Функция ПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач ИмяПрофиля) Экспорт
	...
	Если ПрофилиБезопасности.Количество() <> 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В кластере серверов %1 не зарегистрирован профиль безопасности %2.';uk='У кластері серверів %1 не зареєстрований профіль безпеки %2.'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Процедура СоздатьПрофильБезопасности(Знач ПараметрыАдминистрированияКластера, Знач СвойстваПрофиляБезопасности) Экспорт
	...
	Если ПрофилиБезопасности.Количество() = 1 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В кластере серверов %1 уже зарегистрирован профиль безопасности %2.';uk='У кластері серверів %1 вже зареєстрований профіль безпеки %2.'"), ИдентификаторКластера, ИмяПрофиля);
	КонецЕсли;
...
Функция ИдентификаторИнформационнойБазы(Знач ИдентификаторКластера, Знач ПараметрыАдминистрированияКластера, Знач ПараметрыАдминистрированияИнформационнойБазы) Экспорт
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В кластере серверов %1 не зарегистрирована информационная база %2.';uk='У кластері серверів %1 не зареєстрована інформаційна база %2.'"), ИдентификаторКластера, ПараметрыАдминистрированияИнформационнойБазы.ИмяВКластере);
	КонецЕсли;
...
Функция ВыполнитьКоманду(Знач Шаблон, Знач ПараметрыАдминистрированияКластера, Знач ЗначенияПараметров = Неопределено)
	...
	#Если Сервер Тогда
		...
		Если БезопасныйРежим() <> Ложь Тогда
			ВызватьИсключение НСтр("ru='Внимание! Администрирование кластера невозможно в безопасном режиме.';uk='Увага! Адміністрування кластеру неможливо в безпечному режимі.'");
		КонецЕсли;
		
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			ВызватьИсключение НСтр("ru='Внимание! В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера.';uk='Увага! У моделі сервісу неприпустимо виконання прикладною інформаційною базою функцій адміністрування кластеру.'");
		КонецЕсли;
	...
	Если Не ФайлКлиента.Существует() Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Невозможно выполнить операцию администрирования кластера серверов по причине: файл %1 не найден.
                  ...
                  |- Для компьютеров с ОС Linux требуется установить пакет *-enterprise83-server*.'
                  |;uk='Неможливо виконати операцію адміністрування кластеру серверів через: файл %1 не знайдено.
                  |
                  ...
                  |- Для комп''ютерів з ОС Linux потрібно встановити пакет *-enterprise83-server*.'"),
