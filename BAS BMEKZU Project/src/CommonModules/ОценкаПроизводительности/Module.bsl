...
//
Функция СоздатьКлючевуюОперацию(ИмяКлючевойОперации, ЦелевоеВремя = 1, ВыполненаСОшибкой = Ложь, Длительная = Ложь) Экспорт
...
Процедура ЗафиксироватьДлительностьКлючевойОперации(Параметры)
	...
	Если ОценкаПроизводительностиСлужебный.ПодсистемаСуществует("СтандартныеПодсистемы.БазоваяФункциональность") Тогда
		МодульОбщегоНазначенияКлиентСервер = ОценкаПроизводительностиСлужебный.ОбщийМодуль("ОбщегоНазначенияКлиентСервер");
		КодОсновногоЯзыка = МодульОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
	Иначе
		КодОсновногоЯзыка = Метаданные.ОсновнойЯзык.КодЯзыка;
	КонецЕсли;	
	
	Если БезопасныйРежим() <> Ложь Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Работа подсистемы оценка производительности в безопасном режиме не поддерживается';uk='Робота підсистеми оцінка продуктивності в безпечному режимі не підтримується'",КодОсновногоЯзыка),
	...
	Если Не ЗначениеЗаполнено(ДатаНачалаКлючевойОперации) Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Невозможно зафиксировать замер с пустой датой начала';uk='Неможливо зафіксувати замір з порожньою датою початку'",КодОсновногоЯзыка),
			УровеньЖурналаРегистрации.Информация,,,Строка(КлючеваяОперация));
...
Процедура ЗаписатьЗамерыВремени(МассивЗамеров)
	...
	Если НаборЗаписей.Количество() > 0 Тогда
		...
		Исключение
			ЗаписьЖурналаРегистрации(НСтр("ru='Не удалось сохранить замеры производительности';uk='Не вдалося зберегти заміри продуктивності'",КодОсновногоЯзыка),
...
Процедура ВыгрузитьРезультаты(КаталогиЭкспорта, МассивыЗамеров, ТекДата, ПорядковыйНомерФайла)
	...
	ЗаписьXML.ЗаписатьАтрибут("version", ПространствоИмен, "1.0.0.3");
...
Функция ЗамерыВремениСОтборомПоПрофилю()
	...
	|	КлючевыеОперации.КлючеваяОперация.ВыполненаСОшибкой КАК ВыполненаСОшибкой,
...
Процедура ЗагрузитьФайлОценкиПроизводительности(ИмяФайла, АдресХранилища) Экспорт
	...
	Исключение
		...
		ШаблонСтроки = НСтр("ru='Не удалось распаковать архив %1 %2';uk='Не вдалося розпакувати архів %1 %2'");
...
Процедура ЗагрузитьФайлОценкиПроизводительностиApdexExport(ЧтениеXML, СуществующиеКлючевыеОперации, КлючевыеОперацииДляЗаписи, СырыеЗамерыДляЗаписи)
	...
	Пока ЧтениеXML.ТипУзла <> ТипУзлаXML.КонецЭлемента Цикл
        ...
        ВыполненаСОшибкой = КлючеваяОперация.runningError;
		...
		Если КлючеваяОперацияСсылка = Неопределено Тогда
			КлючеваяОперацияСсылка = СоздатьКлючевуюОперацию(КлючеваяОперацияИмя, ЦелевоеВремя, ВыполненаСОшибкой, Длительная);
