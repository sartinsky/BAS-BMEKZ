...
Процедура ВставитьДополнительнуюСтраницуНаСервере(Форма, ИмяТекущейСтраницыПанели = Неопределено) Экспорт
	...
	Иначе
		...
		Если МаксКолвоЭкз<>0 И Форма[Форма.мСтруктураМногостраничныхРазделов[ИмяТекущейСтраницыПанели]].Количество() >= МаксКолвоЭкз Тогда
			...
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='В разделе ""%1"" максимальное количество экземпляров страниц равно %2.';uk='В розділі ""%1"" максимальна кількість екземплярів сторінок дорівнює %2.'"), ЗаголовокСтраницы, МаксКолвоЭкз);
...
Процедура ВыполнитьОбновлениеИнформационнойБазы() Экспорт
	...
	УстановитьИспользованиеВстроенныхРегламентированныхОтчетов();
	
КонецПроцедуры

Процедура УстановитьИспользованиеВстроенныхРегламентированныхОтчетов()
	
	Если Метаданные.Константы.Найти("ИспользоватьВстроенныеРегламентированныеОтчеты") <> Неопределено Тогда
		Константы.ИспользоватьВстроенныеРегламентированныеОтчеты.Установить(Истина);                        
	КонецЕсли;
	
	Если НЕ РегламентированнаяОтчетностьВызовСервера.ИспользуютсяВстроенныеРегламентированныеОтчеты() Тогда
		...
		|ГДЕ
		|	Организации.ДатаНачалаИспользованияЗвит1С = ДАТАВРЕМЯ(1, 1, 1)";
		
		Выборка = Запрос.Выполнить().Выбрать();
		Пока Выборка.Следующий() Цикл
			
			//установка даты начала использования выполняется в модуле объекта
