	...
	Попытка
		...
		Для Каждого УслугаXDTO Из ОбщиеДанныеПоТарификации.Services Цикл 
			...
			Если ПоставщикУслуги.Пустая() Тогда 
				ВызватьИсключение НСтр("ru='Не найде поставщик услуги с кодом:';uk='Не знайдений постачальник послуги з кодом:'") + " " + УслугаXDTO.ServiceProviderID;
			КонецЕсли;
...
Процедура ОбновитьДанныеАбонентаПоТарификацииВПриложении(Знач ДанныеАбонентаПоТарификации) Экспорт
	...
	ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ru='Обновление данных по тарификации в области данных';uk='Оновлення даних по тарифікації в області даних'");
	...
	Попытка
		...
		Для Каждого ПодпискаНаУслугу Из ДанныеАбонентаПоТарификации.ServiceSubscriptions Цикл
			...
			Иначе
				ШаблонТекстаОшибки = НСтр("ru='Не найдена услуга с идентификатором ""%1"" и идентификатором поставщика ""%2""';uk='Не знайдена послуга з ідентифікатором ""%1"" і ідентифікатором постачальника ""%2""'");
...
Процедура УдалитьДанныеПоТарификацииВПриложениеАбонента(Знач УдаляемыеПодписки) Экспорт
	...
	Для Каждого Подписка Из УдаляемыеПодписки.SubscriptionsID Цикл
		...
		Исключение
			ОтменитьТранзакцию();
			ИнформацияПоОшибке = НСтр("ru='Проблема с удалением данных по подписке:';uk='Проблема з видаленням даних за підпискою:'") +
				" " +
				Строка(УдаляемыеПодписки.SubscriptionsID) +
				Символы.ПС +
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ru='Удаление данных по подписке';uk='Вилучення даних за підпискою'");
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Процедура ОбновитьДанныеПоЛицензиям(ОбновляемыеЛицензии) Экспорт
	...
	Исключение
		...
		ИнформацияПоОшибке = ИнформацияОбОшибке();
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ru='Обновление данных по лицензиям';uk='Оновлення даних по ліцензіях'");
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
...
Процедура УдалитьДанныеПоЛицензиям(УдаляемыеЛицензии) Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ИнформацияПоОшибке = ИнформацияОбОшибке();
			ИмяСобытия = ТарификацияСлужебный.ИмяСобытияТарификации() + НСтр("ru='Удаление данных по лицензиям';uk='Вилучення даних за ліцензіями'");
			ЗаписьЖурналаРегистрации(ИмяСобытия,
				УровеньЖурналаРегистрации.Ошибка,
				,
				,
