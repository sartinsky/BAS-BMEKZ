						...
						Если Выборка2.Следующий() Тогда
							...
							Если КонечнаяТочкаКорреспондента = Неопределено Тогда
								...
								ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru='Не определена конечная точка корреспондента для адреса ""%1"".';uk='Не визначена кінцева точка кореспондента для адреси ""%1"".'"),
									Выборка2.Адрес);
							КонецЕсли;
...
Процедура УдалитьНастройкуСинхронизации(ИмяПланаОбмена, КодУзлаКорреспондента) Экспорт
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Обработка сообщения удаления настройки синхронизации ""%1"" с приложением ""%2"".';uk='Обробка повідомлення видалити настройки синхронізації ""%1"" з додатком ""%2"".'"),
		...
		Если ПустаяСтрока(ПолучитьФункциональнуюОпцию("ПрефиксИнформационнойБазы")) Тогда
			...
			Если ПустаяСтрока(Префикс) Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                    ...
                    |;uk='Для продовження установки синхронізації даних необхідно
                    |задати префікс інформаційної бази в додатку ""%1"".'"),
					Метаданные.Синоним);
			КонецЕсли;
...
Процедура СоздатьНастройкуОбмена_3_0_1_1(НастройкиПодключения,
	...
	Если ПустаяСтрока(КодЭтогоУзла) Тогда
		// Код узла задается в обработчике обновления ИБ.
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Для предопределенного узла плана обмена ""%1"" не задан код.';uk='Для передобумовленого вузла плану обміну ""%1"" не заданий код.'"),
