...
Процедура ДобавитьОтборПоОрганизации(ПараметрыОтчета, КомпоновщикНастроек, ВПользовательскиеНастройки = Истина) Экспорт
	...
	СписокДоступныхОрганизаций.ЗагрузитьЗначения(ДоступныеОрганизации);
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
		...
		// Если среди доступных организаций, нет выбранной, то оставляем фильтр по доступным организаиям
...
Процедура ДобавитьОтборДляПоказателяРазница(ПараметрыОтчета, КомпоновщикНастроек, ВПользовательскиеНастройки = Истина) Экспорт
	...
	Если ПараметрыОтчета.ПоказательРазница Тогда 
		...
		ГруппаОтборИЛИ.Представление = "###Разница###";
		ГруппаОтборИЛИ.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтборИЛИ, "СальдоНаНачалоПериода.РазницаНачальныйОстатокДт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтборИЛИ, "СальдоНаНачалоПериода.РазницаНачальныйОстатокКт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтборИЛИ, "ОборотыЗаПериод.РазницаОборотДт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтборИЛИ, "ОборотыЗаПериод.РазницаОборотКт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтборИЛИ, "СальдоНаКонецПериода.РазницаКонечныйОстатокДт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
		БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(ГруппаОтборИЛИ, "СальдоНаКонецПериода.РазницаКонечныйОстатокКт", 0, ВидСравненияКомпоновкиДанных.НеРавно);
...
Процедура СформироватьОтчет(ПараметрыОтчета, АдресХранилища) Экспорт
	...
	// Определим необходимость вывода единиц измерения.
	Если ПараметрыОтчета.Свойство("ВыводитьЕдиницуИзмерения") Тогда
		ВыводитьЕдиницуИзмерения = ПараметрыОтчета.ВыводитьЕдиницуИзмерения;
	ИначеЕсли КомпоновщикНастроек.Настройки.ДополнительныеСвойства.Свойство("ВыводитьЕдиницуИзмерения") Тогда
...
Процедура ПриСохраненииПользовательскихНастроекНаСервере(ФормаОтчета, Настройки, СохранятьТолькоРеквизиты = Ложь, СохранятьВыбранныеПоля = Ложь) Экспорт
	...
	Если НЕ СохранятьТолькоРеквизиты Тогда
		...
		Если СохранятьВыбранныеПоля Тогда
		...
		Если СохранятьВыбранныеПоля Тогда
			ТекущиеНастройки.Выбор.ИдентификаторПользовательскойНастройки              = "Выбор";
		КонецЕсли;
...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(ФормаОтчета, Настройки, ЗагружатьТолькоРеквизиты = Ложь, ЗагружатьВыбранныеПоля = Ложь) Экспорт
	...
	Иначе 
		...
		Если Настройки.ДополнительныеСвойства.Свойство("ДанныеОтчета") Тогда
			...
			Для Каждого ЭлементСтруктуры Из ДополнительныеСвойства Цикл
				...
				Если ЭлементСтруктуры.Ключ = "ВидимостьПанелиНастроек" И НЕ ЕстьЭлементФормы(ФормаОтчета, "РазделыОтчета") Тогда
					...
					КонецЕсли;
				ИначеЕсли ЭлементСтруктуры.Ключ = "ОписаниеОтборовОтчета" Тогда
					
					// Проверяем, что есть текст для вывода.
					ВыводитьОписаниеОтборовОтчета = Не ПустаяСтрока(ЭлементСтруктуры.Значение);
					
					// Если отчет формируется при открытии, то не выводим представление отбора.
					ВыводитьОписаниеОтборовОтчета = ВыводитьОписаниеОтборовОтчета
					   И (Не ФормаОтчета.Параметры.Свойство("СформироватьПриОткрытии")
						Или ФормаОтчета.Параметры.СформироватьПриОткрытии <> Истина);
					  
					// Проверяем, что это не отчет-исключение.
					ВыводитьОписаниеОтборовОтчета = ВыводитьОписаниеОтборовОтчета
					   И (БухгалтерскиеОтчетыКлиентСервер.ПолучитьИдентификаторОбъекта(ФормаОтчета) <> "ОборотноСальдоваяВедомость");
					   
					// Проверяем, что есть, куда выводить.
					ВыводитьОписаниеОтборовОтчета = ВыводитьОписаниеОтборовОтчета
					   И ФормаОтчета.Элементы.Найти("Результат") <> Неопределено
					   И ФормаОтчета.Результат.ВысотаТаблицы = 0
					   И ФормаОтчета.Элементы.Результат.ОтображениеСостояния.ДополнительныйРежимОтображения = ДополнительныйРежимОтображения.Неактуальность;

					Если ВыводитьОписаниеОтборовОтчета Тогда
						...
						ОбластьОписания.Текст = НСтр("ru='Отбор:';uk= 'Відбір:'");
						ОбластьОписания = ФормаОтчета.Результат.Область(9, 2);
						ОбластьОписания.Шрифт = Новый Шрифт(ОбластьОписания.Шрифт, , 11);
						ОбластьОписания.Текст = ЭлементСтруктуры.Значение;
		...
		Если Не ЗагружатьТолькоРеквизиты Тогда
			...
			ТекущиеНастройки.Порядок.ИдентификаторПользовательскойНастройки            = "Порядок";
			ТекущиеНастройки.УсловноеОформление.ИдентификаторПользовательскойНастройки = "УсловноеОформление";
			Если ЗагружатьВыбранныеПоля Тогда
				ТекущиеНастройки.Выбор.ИдентификаторПользовательскойНастройки              = "Выбор";
			КонецЕсли;
			...
			Если ЗагружатьВыбранныеПоля Тогда
				ТекущиеНастройки.Выбор.ИдентификаторПользовательскойНастройки              = "";
			КонецЕсли;
			ОбновитьОписаниеОтборовОтчета(ФормаОтчета);
		КонецЕсли;		
	КонецЕсли;
	...
	Если ЕстьРеквизитФормы(ФормаОтчета, "ПолеОрганизация") Тогда
		ОбщегоНазначенияБПКлиентСервер.УстановитьЗначениеПолеОрганизация(
			ФормаОтчета.ПолеОрганизация, ФормаОтчета.Отчет.Организация, Ложь);
	КонецЕсли;
			...
			ИначеЕсли ТипЗнч(Отбор) = Тип("ГруппаЭлементовОтбораКомпоновкиДанных") Тогда
				Если Отбор.Представление = "###ОтборПоОрганизацииСОП###" Тогда
					Для Каждого ЭлементОтбора Из Отбор.Элементы Цикл
						...
						КонецЕсли;
					КонецЦикла;
				ИначеЕсли Отбор.Представление = "###Разница###" Тогда
				КонецЕсли;
...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если Параметры.Свойство("ВидРасшифровки") Тогда 
		...
		Если НЕ ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("Организация")Тогда
			ПользовательскиеНастройки.ДополнительныеСвойства.Вставить ("Организация", ДанныеОбъекта.Объект.Организация);
		КонецЕсли;
	...
	РегистрыУчетаВызовСервера.ПриСозданииНаСервере(Форма);
...
Функция ПолучитьСписокСчетовИсключаемыхИзРасчетаЗадолженности(Тип = 1) Экспорт
	...
	СписокСчетов = ХранилищеОбщихНастроек.Загрузить(ПолучитьЗначениеКлючаОбъектаПоТипу(Тип));
	Если СписокСчетов = Неопределено Тогда
		Если Тип=3 Тогда
			СписокСчетов = ПолучитьСчетаДРНеИспользуемыхПоУмолчанию();
		КонецЕсли; 
	КонецЕсли;
...
Процедура СохранитьСписокСчетовИсключаемыхИзРасчетаЗадолженности(СписокСчетов, Тип = 1) Экспорт
	...
	ИначеЕсли Тип = 3 Тогда
		Описание    = НСтр("ru='Счета исключаемые из расчета Доходов/Расходов';uk='Рахунки, що виключають із розрахунку Доходів/Витрат'", Локализация.КодЯзыкаИнформационнойБазы());
	КонецЕсли;
...
Функция ПолучитьЗначениеКлючаОбъектаПоТипу(Тип)
	...
	ИначеЕсли Тип = 3 Тогда
		КлючОбъекта = "Отчеты.СчетаИсключаемыеИзРасчетаДоходовРасходов";
	 КонецЕсли;
...
Функция ОписаниеОтборовОтчета(ПараметрыОтчета, КомпоновщикНастроек)
	...
	Если ПараметрыОтчета.Свойство("Подразделение") И ЗначениеЗаполнено(ПараметрыОтчета.Подразделение) Тогда
		ТекстПодразделение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Подразделение Равно ""%1""';uk= 'Підрозділ Дорівнює ""%1""'"), ПараметрыОтчета.Подразделение);
	КонецЕсли;
