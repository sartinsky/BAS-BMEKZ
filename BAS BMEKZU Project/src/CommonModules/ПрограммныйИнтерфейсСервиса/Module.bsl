...
Функция ПрочитанныеДанныеОтвета(Ответ)
	...
	Если ЗначениеЗаполнено(ТелоОтвета) Тогда
		...
		Исключение
			...
			Событие = ИмяСобытия() + "." + НСтр("ru='Ошибка чтения JSON';uk='Помилка читання JSON'",Локаль);
		...
		Для каждого Элемент Из РезультатОтвет Цикл
			...
			Исключение
				...
				Локаль = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
				Событие = ИмяСобытия() + "." + НСтр("ru='Ошибка преобразования JSON';uk='Помилка перетворення JSON'",Локаль);
				ЗаписьЖурналаРегистрации(Событие, УровеньЖурналаРегистрации.Ошибка,,, Комментарий);
				ВызватьИсключение;
			КонецПопытки; 
		КонецЦикла; 
...
Функция ИмяСобытия()
	...
	Локаль = ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка();
	Возврат НСтр("ru='Программный интерфейс сервиса';uk='Програмний інтерфейс сервісу'",Локаль);
