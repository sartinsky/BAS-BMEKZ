		...
		Если Реквизит <> Неопределено Тогда
			...
			Если ТипыРеквизита.Количество() <> 1 Тогда
				...
				СтрокаСообщения = НСтр("ru='Составной тип данных для значений по умолчанию не поддерживается.
                    |Реквизит ""%1"".'
                    |;uk='Складений тип даних для значень по умовчанню не підтримується.
                    |Реквізит ""%1"".'");
			...
			Если Не ОбщегоНазначения.ЭтоСправочник(ОбъектМетаданных) Тогда
				...
				СтрокаСообщения = НСтр("ru='Выбор значений по умолчанию поддерживается только для справочников.
                    |Реквизит ""%1"".'
                    |;uk='Вибір значень по умовчанню підтримується тільки для довідників.
                    |Реквізит ""%1"".'");
				СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СтрокаСообщения, Реквизит.ПолноеИмя());
				ВызватьИсключение СтрокаСообщения;
			КонецЕсли;
...
Процедура ПроверитьВозможностьВыполненияОбменов(ПриложениеВИнтернете = Ложь) Экспорт
	...
	Если Не ПравоДоступа("Просмотр", Метаданные.ОбщиеКоманды.Синхронизировать) Тогда
		...
		Если ПриложениеВИнтернете Тогда
			ВызватьИсключение НСтр("ru='Недостаточно прав для синхронизации данных с приложением в Интернете.';uk='Недостатньо прав для синхронізації даних з додатком в Інтернеті.'");
		Иначе
	...
	ИначеЕсли ОбновлениеИнформационнойБазы.НеобходимоОбновлениеИнформационнойБазы()
		...
		Если ПриложениеВИнтернете Тогда	
			ВызватьИсключение НСтр("ru='Приложение в Интернете находится в состоянии обновления.';uk='Прикладна програма в Інтернеті знаходиться в стані оновлення.'");
		Иначе
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий =
		НСтр("ru='Перенос настроек подключения обменов данными в новый регистр ""Настройки транспорта обмена данными"".';uk='Перенесення настройок підключення обмінів даними в новий регістр ""Настройки транспорту обміну даними"".'");
	...
	Обработчик.Комментарий =
		НСтр("ru='Заполнение вспомогательных настроек обмена данными в регистре ""Общие настройки узлов информационных баз"".';uk='Заповнення допоміжних установок обміну даними в регістрі ""Загальні настройки вузлів інформаційних баз"".'");
	...
	Обработчик.Версия = "3.0.1.91";
	Обработчик.Комментарий =
		НСтр("ru='Первоначальное заполнение настроек обмена данными XDTO.';uk='Первинне заповнення настройок обміну даними XDTO.'");
...
Функция ПолучитьСообщениеОбменаВоВременныйКаталогИзИнформационнойБазыКорреспондента(Отказ, УзелИнформационнойБазы, ВыводитьСообщения = Истина) Экспорт
	...
	Исключение
		Если ВыводитьСообщения Тогда
			Сообщение = НСтр("ru='Не удалось произвести обмен: %1';uk='Не вдалося виконати обмін: %1'");
...
Процедура ВыполнитьОбменДаннымиДляУзлаИнформационнойБазыЧерезФайлИлиСтроку(ПараметрыОбмена) Экспорт
	...
	Если Не НастройкаСинхронизацииЗавершена(ПараметрыОбмена.УзелИнформационнойБазы) Тогда
		...
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В ""%1"" настройка синхронизации данных с ""%2"" (идентификатор ""%3"") еще не завершена.';uk='У ""%1"" настройка синхронізації даних ""%2"" (ідентифікатор користувача ""%3"") ще не завершена.'"),
	...
	ИначеЕсли СтруктураНастроекОбмена.ПроизводитьВыгрузкуДанных Тогда
		...
		Если ЗначениеЗаполнено(ПсевдонимУзла) Тогда
			...
			Если УзелПсевдонимСуществует Тогда
				СтруктураНастроекОбмена.ТекущийУзелПланаОбменаКод = ПсевдонимУзла;
			КонецЕсли;
			Если ПараметрыПриемника.Свойство("ПоддерживаетсяСопоставлениеДанных") Тогда
				ЕстьПоддержкаСопоставления = ПараметрыПриемника.ПоддерживаетсяСопоставлениеДанных;
			КонецЕсли;
			Если ПараметрыПриемника.Свойство("НастройкаСинхронизацииДанныхЗавершена") Тогда
				НастройкаСинхронизацииДанныхЗавершена = ПараметрыПриемника.НастройкаСинхронизацииДанныхЗавершена;
			КонецЕсли;
		Иначе
			ПараметрыПриемникаПрокси = Прокси.GetIBParameters(
	...
	Если СтруктураНастроекОбменаВнешнееСоединение.Свойство("НастройкаСинхронизацииДанныхЗавершена") Тогда
		Если Не СообщениеДляСопоставленияДанных
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |;uk='Для продовження необхідно перейти в програму ""%1"" і завершити в ній настройку синхронізації.
                |Виконання обміну даними скасовано.'"),
	...
	Если СтруктураНастроекОбменаВнешнееСоединение.Свойство("ПолученоСообщениеДляСопоставленияДанных") Тогда
		Если Не СообщениеДляСопоставленияДанных
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |Выполнение обмена данными отменено.'
                |;uk='Для продовження необхідно перейти в програму ""%1"" і виконати завантаження повідомлення для співставлення даних.
                |Виконання обміну даними скасовано.'"),
				СтруктураНастроекОбменаВнешнееСоединение.УзелИнформационнойБазыНаименование);
			ЗаписьЖурналаРегистрацииОбменаДанными(СообщениеОбОшибке, СтруктураНастроекОбмена, Истина);
...
Процедура ВыполнитьСтандартнуюЗагрузкуИзмененийДляУзла(
	...
	ОбменДаннымиСлужебный.ОтключитьОбновлениеКлючейДоступа(Истина, Не ПараметрыРезервнойКопии.ВосстановленаРезервнаяКопия);
	Если ИспользоватьТранзакции Тогда
		НачатьТранзакцию();
	КонецЕсли;
	Попытка
		...
		Если РезультатВыполненияОбмена = Перечисления.РезультатыВыполненияОбмена.Ошибка Тогда
			ВызватьИсключение(НСтр("ru='В процессе получения данных возникли ошибки.';uk='В процесі отримання даних виникли помилки.'"));
		КонецЕсли;
		
		ОбменДаннымиСлужебный.ОтключитьОбновлениеКлючейДоступа(Ложь, Не ПараметрыРезервнойКопии.ВосстановленаРезервнаяКопия);
		Если ИспользоватьТранзакции Тогда
			ЗафиксироватьТранзакцию();
		КонецЕсли;
	Исключение
...
Функция СобытиеЖурналаРегистрацииУстановкаПодключенияКWebСервису() Экспорт
	...
	Возврат НСтр("ru='Обмен данными.Установка подключения к web-сервису';uk='Обмін даними. Встановлення підключення до web-сервісу'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдалениеОбменаДанными() Экспорт
	...
	Возврат НСтр("ru='Обмен данными.Удаление обмена данными';uk='Обмін даними.Видалення обміну даними'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииРегистрацияДанныхДляНачальнойВыгрузки() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Регистрация данных для начальной выгрузки';uk='Обмін даними.Реєстрація даних для початкової вивантаження'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхДляСопоставления() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Выгрузка данных для сопоставления';uk='Обмін даними.Вивантаження даних для співставлення'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииВыгрузкаДанныхВСервисПередачиФайлов() Экспорт
	...
	Возврат НСтр("ru='Обмен данными.Сервис передачи файлов.Выгрузка данных';uk='Обмін даними.Сервіс передачі файлів.Вивантаження даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
	
КонецФункции

Функция СобытиеЖурналаРегистрацииЗагрузкаДанныхИзСервисаПередачиФайлов() Экспорт
	
	Возврат НСтр("ru='Обмен данными.Сервис передачи файлов.Загрузка данных';uk='Обмін даними.Сервіс передачі файлів.Завантаження даних'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ВыполнитьКонтрольЗаписиНеразделенныхДанных(Знач Данные) Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		ТекстИсключения = НСтр("ru='Недостаточно прав для выполнения действия.';uk='Недостатньо прав для виконання дії.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура УстановитьПравилаОбменаЗагрузкиДанных(ОбработкаОбменаДаннымиXML, СтруктураНастроекОбмена)
	...
	Если ПравилаКонвертацииОбъектов = Неопределено Тогда
		...
		// Правила обмена должны быть указаны.
		НСтрока = НСтр("ru='Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.';uk='Не задано правила конвертації для плану обміну %1. Завантаження даних скасоване.'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтрока, СтруктураНастроекОбмена.ИмяПланаОбмена);
		ЗаписьЖурналаРегистрацииОбменаДанными(СтрокаСообщенияОбОшибке, СтруктураНастроекОбмена, Истина);
		ЗафиксироватьЗавершениеИнициализацииОбмена(СтруктураНастроекОбмена);
...
Процедура ПроверитьПодключениеОбработкиТранспортаСообщенийОбмена(Отказ,
	...
	Если Не ОбработкаОбъект.ПодключениеУстановлено() Тогда
		...
		СообщениеОбОшибке = ОбработкаОбъект.СтрокаСообщенияОбОшибке
			+ Символы.ПС + НСтр("ru='Техническую информацию об ошибке см. в журнале регистрации.';uk='Технічну інформацію про помилку див. у журналі реєстрації.'");
...
Функция WSПроксиДляУзлаИнформационнойБазы(УзелИнформационнойБазы,
	...
	Если Не ПустаяСтрока(МинимальнаяВерсия) Тогда
		Если МинимальнаяВерсия = "3.0.1.1"
			...
			СтрокаСообщенияОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Корреспондент не поддерживает требуемую версию %1 интерфейса ""ОбменДанными"".';uk='Кореспондент не підтримує необхідну версію %1 інтерфейсу ""ОбменДанными"".'"),
				МинимальнаяВерсия);
			Возврат Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Функция ЕстьПодключениеККорреспонденту(Знач Корреспондент,
		Знач СтруктураНастроек, СообщениеПользователю = "", НастройкаЗавершена = Истина, ПолученыДанныеДляСопоставления = Ложь) Экспорт
	...
	Если ВерсияКорреспондента_2_1_1_7
		Или ВерсияКорреспондента_3_0_1_1 Тогда
		
		WSПрокси = Неопределено;
		
		Если ВерсияКорреспондента_3_0_1_1 Тогда
			WSПрокси = ПолучитьWSПрокси_3_0_1_1(СтруктураНастроек, , СообщениеПользователю, 5);
		ИначеЕсли ВерсияКорреспондента_2_1_1_7 Тогда
		...
		Попытка
			...
			КодКорреспондента = ИдентификаторЭтогоУзлаДляОбмена(Корреспондент);
			...
			Если Не ЕстьПодключение
				И ЭтоПланОбменаXDTO(Корреспондент) Тогда
				
				Псевдоним = ПсевдонимПредопределенногоУзла(Корреспондент);
				
				Если ЗначениеЗаполнено(Псевдоним) Тогда
					КодКорреспондента = Псевдоним;
			...
			Если ЕстьПодключение
				И ВерсияКорреспондента_3_0_1_1 Тогда
				СтрокаСообщение = "";
				ПараметрыКорреспондентаXDTO = WSПрокси.GetIBParameters(
					ИмяПланаОбмена,
					КодКорреспондента,
					СтрокаСообщение);
					
				ПараметрыКорреспондента = СериализаторXDTO.ПрочитатьXDTO(ПараметрыКорреспондентаXDTO);
				
				Если ПараметрыКорреспондента.Свойство("НастройкаСинхронизацииДанныхЗавершена") Тогда
					НастройкаЗавершена = ПараметрыКорреспондента.НастройкаСинхронизацииДанныхЗавершена;
				КонецЕсли;
				
				Если ПараметрыКорреспондента.Свойство("ПолученоСообщениеДляСопоставленияДанных") Тогда
					ПолученыДанныеДляСопоставления = ПараметрыКорреспондента.ПолученоСообщениеДляСопоставленияДанных;
				КонецЕсли;
			КонецЕсли;
			...
			Если Не ЕстьПодключение
				...
				ВариантНастройки = "";
				Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ВариантНастройки", Корреспондент.Метаданные()) Тогда
					...
					Для Каждого ИмяСтарогоПланаОбмена Из ОбменДаннымиПовтИсп.ПланыОбменаБСП() Цикл
						...
						Если НастройкиСтарогоПланаОбмена.ИмяПланаОбменаДляПереходаНаНовыйОбмен = ИмяПланаОбмена Тогда
							...
							Если Не ВариантНастроек = Неопределено Тогда
								...
								ПараметрыКорреспондентаXDTO = WSПрокси.GetIBParameters(
									ИмяСтарогоПланаОбмена,
									КодКорреспондента,
									СообщениеПользователю);
									
								ПараметрыКорреспондента = СериализаторXDTO.ПрочитатьXDTO(ПараметрыКорреспондентаXDTO);
								
								ЕстьПодключение = ПараметрыКорреспондента.ПланОбменаСуществует И ПараметрыКорреспондента.УзелСуществует;
...
Функция РазличаютсяВерсииКорреспондента(ИмяПланаОбмена, КлючСообщенияЖурналаРегистрации, ВерсияВЭтойПрограмме,
	...
	Если ЗначениеЗаполнено(ВерсияВЭтойПрограмме) И ЗначениеЗаполнено(ВерсияВДругойПрограмме)
		...
		Если ВерсияВЭтойПрограммеБезНомераСборки <> ВерсияВДругойПрограммеБезНомераСборки Тогда
			...
			ШаблонСообщения = НСтр("ru='Синхронизация данных может быть выполнена некорректно, т.к. версия программы ""%1"" (%2) в правилах конвертации этой программы отличается от версии %3 в правилах конвертации в другой программе. Убедитесь, что загружены актуальные правила, подходящие для используемых версий обеих программ.';uk='Синхронізація даних може бути виконана некоректно, т. я. версія програми ""%1"" (%2) у правилах конвертації цієї програми відрізняється від версії %3 в правилах конвертації в іншій програмі. Переконайтеся, що завантажені актуальні правила, які підходять для  версій, що використовуються обох програм.'");
	...
	Для Каждого Раздел Из Разделы Цикл
		...
		Иначе
			...
			Дело.ПараметрыФормы = Новый Структура("Заголовок,ИмяМакета",
				НСтр("ru='Установка обновления';uk='Встановлення оновлення'"), "ИнструкцияКакВыполнитьУстановкуОбновленияВручную");
		КонецЕсли;
		Дело.Владелец       = Раздел;
