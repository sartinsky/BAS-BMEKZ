	...
	Для НомерСимвола = 1 По СтрДлина(ИсходнаяСтрока) Цикл
		...
		Иначе
			Если (КодСимвола >= 65 И КодСимвола <= 90)
				Или (КодСимвола = 1028) //"Є"
				Или (КодСимвола = 1108) // "є" 
				Или (КодСимвола = 1025) //"Ё"
				Или (КодСимвола = 1105) // "ё" 
				Или (КодСимвола >= 1010 И КодСимвола <= 1011) // "і""ї"
				Или (КодСимвола >= 1030 И КодСимвола <= 1031) // "І""Ї"
				Или (КодСимвола >= 1168 И КодСимвола <= 1169) // "ґ""Ґ"
...
#КонецЕсли	
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Копирование элементов ""%1"" не поддерживается';uk='Копіювання елементів ""%1"" не підтримується'"), ТипЭлемента);
