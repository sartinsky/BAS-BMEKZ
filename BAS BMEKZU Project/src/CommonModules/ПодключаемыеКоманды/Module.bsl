		...
		Если ИсточникиЧерезЗапятую = "" Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В формах отчетов, обработок и в общих формах
                    ...
                    |;uk='В формах звітів, обробок і в загальних формах
                    |при виклику процедури ""%1""
                    |потрібно явно вказувати параметр ""%2""'"),
				"ПодключаемыеКоманды.ПриСозданииНаСервере",
				"ПараметрыРазмещения.Источники");
		КонецЕсли;
	КонецЕсли;
...
Функция НастройкиПодключаемогоОбъекта(ПолноеИмя, НастройкиПрограммногоИнтерфейса = Неопределено) Экспорт
	...
	Исключение
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Не удалось прочитать настройки объекта %1 из модуля менеджера:';uk='Не вдалося прочитати настройки об''єкту %з 1 модуля менеджера:'"), ПолноеИмя);
		ТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		
		ЗаписьЖурналаРегистрации(
			НСтр("ru='Подключаемые команды';uk='Команди, які підключаються'",ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Для Каждого Настройка Из НастройкиПрограммногоИнтерфейса Цикл
		...
		Исключение
			ТекстОшибки = НСтр("ru='Не удалось зарегистрировать настройку программного интерфейса подключаемых объектов.
                ...
                |;uk='Не вдалося зареєструвати настройку програмного інтерфейсу об''єктів, що підключаються.
                |Ключ: ""%1"", опис типів: ""%2"", опис помилки: ""%3"".'");
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ТекстОшибки,
				Настройка.Ключ,
...
Процедура ПроверитьИмяВидаКоманд(ИмяВида)
	...
	Исключение
		ТекстОшибки = НСтр("ru='Имя вида команд ""%1"" не удовлетворяет требованиям именования переменных.';uk='Ім''я виду команд ""%1"" не задовольняє вимогам іменування змінних.'");
...
Процедура ВывестиКоманды(Форма, Команды, ПараметрыРазмещения)
	...
	Если КомандаЗаглушка = Неопределено Тогда
		...
		КомандаЗаглушка.Заголовок = НСтр("ru='(нет)';uk='(ні)'");
	КонецЕсли;
...
Функция ЗарегистрироватьПодменю(Элементы, СведенияОВсехПодменю, ИмяПодменю, ШаблонНового = Неопределено, КоманднаяПанель = Неопределено, ПодменюПоУмолчанию = Неопределено)
	...
	Если ЗначениеЗаполнено(ИмяПодменю) Тогда
		...
		Если Не Группы.Свойство("Важное") Тогда
			...
			ГруппаВажное.Заголовок = Подменю.Заголовок + " (" + НСтр("ru='Важное';uk='Важливе'") + ")";
		...
		Если Не Группы.Свойство("Обычное") Тогда
			...
			ГруппаПоУмолчанию.Заголовок = Подменю.Заголовок + " (" + НСтр("ru='Обычное';uk='Звичайне'") + ")";
		...
		Если Не Группы.Свойство("СмТакже") Тогда
			...
			ГруппаСмТакже.Заголовок = Подменю.Заголовок + " (" + НСтр("ru='См. также';uk='Див. також'") + ")";
...
Функция ОписаниеКоманды(ИмяКомандыВФорме, АдресНастроек) Экспорт
	...
	Если Команда = Неопределено Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Сведения о команде ""%1"" не найдены.';uk='Відомості про команду ""%1"" не знайдені.'"),
			ИмяКомандыВФорме);
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОписаниеКоманды.Обработчик) Тогда
			...
			Если КоличествоПодстрок = 2 Тогда
				...
				Если ОбъектМетаданныхОбщийМодуль = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Общий модуль ""%1"" не найден.';uk='Загальний модуль ""%1"" не знайдений.'"),
						ИмяМодуля);
				КонецЕсли;
