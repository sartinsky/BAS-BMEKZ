...
//
// Возвращаемое значение:
//  Строка - имя COM-класса для работы с платформой через COM-соединение.
//
...
Функция УстановитьВнешнееСоединениеСБазой(Параметры) Экспорт
	...
	#Иначе
		...
		КраткоеОписаниеОшибки = НСтр("ru='Прямое подключение к информационной базе доступно только на клиенте под управлением ОС Windows.';uk='Пряме підключення до інформаційної бази доступно тільки на клієнта під управлінням ОС Windows.'");
	#КонецЕсли
...
Процедура ПроверитьПараметр(Знач ИмяПроцедурыИлиФункции, Знач ИмяПараметра, Знач ЗначениеПараметра, 
	...
	Проверить(ТипЗнч(ИмяПроцедурыИлиФункции) = Тип("Строка"), 
		НСтр("ru='Недопустимое значение параметра ИмяПроцедурыИлиФункции';uk='Неприпустиме значення параметра ИмяПроцедурыИлиФункции'"), Контекст);
	Проверить(ТипЗнч(ИмяПараметра) = Тип("Строка"), 
		НСтр("ru='Недопустимое значение параметра ИмяПараметра';uk='Неприпустиме значення параметра ИмяПараметра'"), Контекст);
		
	ЭтоКорректныйТип = ЗначениеОжидаемогоТипа(ЗначениеПараметра, ОжидаемыеТипы);
	Проверить(ЭтоКорректныйТип <> Неопределено, 
		НСтр("ru='Недопустимое значение параметра ОжидаемыеТипы';uk='Неприпустиме значення параметра ОжидаемыеТипы'"), Контекст);
...
#Если ВебКлиент ИЛИ МобильныйКлиент Тогда
	ВызватьИсключение НСтр("ru='Запуск программ недоступен в веб-клиенте.';uk='Запуск програм недоступний у веб-клієнті.'");
#Иначе
...
#Если ВнешнееСоединение Тогда
		ВызватьИсключение 
			НСтр("ru='Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
                       ...
                       |;uk='Неприпустиме значення параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
                       |Повішення привілеїв системи не доступно зі зовнішнього з''єднання.'");
#КонецЕсли
...
#Если Сервер Тогда
		ВызватьИсключение 
			НСтр("ru='Недопустимое значение параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
                       ...
                       |;uk='Неприпустиме значення параметра ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами.
                       |Повішення привілеїв системи не доступно на сервері.'");
#КонецЕсли
			...
			Если ДождатьсяЗавершения Тогда
				ВызватьИсключение 
					НСтр("ru='Недопустимо одновременно устанавливать параметры 
                               ...
                               |;uk='Неприпустимо одночасно встановлювати параметри 
                               |- ПараметрыЗапускаПрограммы.ДождатьсяЗавершения і
                               |- ПараметрыЗапускаПрограммы.ВыполнитьСНаивысшимиПравами:
                               |Операційна система не дозволяє відстежувати від імені користувача процеси,
                               |запущені адміністратором.'");
			КонецЕсли;
...
#Если ВебКлиент Тогда
	ВызватьИсключение НСтр("ru='Выполнение диагностики соединения недоступно в веб-клиенте.';uk='Виконання діагностики з''єднання недоступне у веб-клієнті.'");
#Иначе
	
	Описание = Новый Массив;
	Описание.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='При обращении по URL: %1';uk='При зверненні по URL: %1'"), 
		URL));
...
#КонецЕсли
	...
	Журнал.Добавить(
		НСтр("ru='Журнал диагностики:
                   ...
                   |;uk='Журнал діагностики:
                   |Виконується перевірка доступності сервера.
                   |Опис діагностваної помилки див. у наступному повідомленні журналу.'"));
	Журнал.Добавить();
...
#КонецЕсли
	...
	Если СоединениеЧерезПрокси Тогда 
		...
		Описание.Добавить(
			НСтр("ru='Диагностика соединения не выполнена, т.к. настроен прокси-сервер.
                       ...
                       |;uk='Діагностика з''єднання не виконана, тому що налаштований проксі-сервер.
	...
	Иначе 
		...
		АдресСервераКонтрольнойПроверки = "bas-soft.eu";
		...
		Если РезультатДоступностиРесурса.Доступен Тогда 
			...
			Описание.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Выполнено обращение к несуществующему ресурсу на сервере %1
                           ...
                           |;uk='Виконано звернення до неіснуючого ресурсу на сервері %1
			...
			Если Не РезультатКонтрольнойПроверки.Доступен Тогда
				...
				Описание.Добавить(
					НСтр("ru='Отсутствует доступ в сеть интернет по причине:
                               ...
                               |;uk='Відсутній доступ в мережу інтернет через:
                               |- комп''ютер не підключений до інтернету;
                               |- неполадки у інтернет-провайдера;
                               |- підключення до інтернету блокує міжмережевий екран, 
                               |антивірусна програма або інше програмне забезпечення.'"));
			...
			Иначе 
				...
				Описание.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Сервер %1 не доступен по причине:
                               ...
                               |;uk='Сервер %1 не доступний через:
                               |- неполадки у інтернет-провайдера;
                               |- підключення до сервера блокує міжмережевий екран, 
                               |антивірусна програма або інше програмне забезпечення;
                               |- сервер відключений або на технічному обслуговуванні.'"),
					АдресСервераРесурса));
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	ЗаписьЖурналаРегистрации(
		НСтр("ru='Диагностика соединения';uk='Діагностика з''єднання'",КодОсновногоЯзыка()),
	...
	Если ТипЗнч(ОжидаемыеТипы) = Тип("Массив") Тогда
		...
		Для Каждого Тип Из ОжидаемыеТипы Цикл
			...
			Если Индекс > 10 Тогда
				Результат = Результат + ",... "
					+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='(всего %1 типов)';uk='(усього %1 типів)'"), ОжидаемыеТипы.Количество());
				Прервать;	
			КонецЕсли;	
		КонецЦикла;
		Возврат Результат;
...
Функция ПредставлениеТипа(Тип)
	...
	ИначеЕсли ТипЗнч(Тип) = Тип("ОписаниеТипов") Тогда
		...
		Возврат ?(СтрДлина(ТипСтрокой) > 150, Лев(ТипСтрокой, 150) + "..." 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='(всего %1 типов)';uk='(усього %1 типів)'"), Тип.Типы().Количество()), 
			ТипСтрокой);
	Иначе	
		ТипСтрокой = Строка(Тип);
		...
		Если СодержитНебезопасныеДействия(КомандаЗапуска) Тогда 
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Недопустимая строка команды
                           ...
                           |;uk='Неприпустимий рядок команди
                           |%1
                           |з причини:
                           |Рядок команди не повинний містити символи: ""$"", ""`"", ""|"", "";"", ""&"".'"),
				КомандаЗапуска);
		КонецЕсли;
		...
		Если КомандаЗапуска.Количество() > 0 Тогда 
			Если СодержитНебезопасныеДействия(КомандаЗапуска[0]) Тогда
				...
				НСтр("ru='Недопустимая команда или путь к исполняемому файлу
                           ...
                           |;uk='Неприпустима команда або шлях до виконуваного файлу
                           |%1
                           |з причини:
                           |Команди не повинна містити символи: ""$"", ""`"", ""|"", "";"", ""&"".'"),
				КомандаЗапуска[0]);
			КонецЕсли;
			Результат = МассивВСтрокуКоманды(КомандаЗапуска);
		Иначе
			ВызватьИсключение
				НСтр("ru='Ожидался что в первый элемент КомандаЗапуска будет командой или путем к исполняемому файлу.';uk='Очікувалось що в перший елемент КомандаЗапуска буде командою або шляхом до виконуваного файлу.'");
		КонецЕсли;
	Иначе 
		ВызватьИсключение 
			НСтр("ru='Ожидался что значение КомандаЗапуска будет <Строка> или <Массив>';uk='Очікувалось що значення КомандаЗапуска буде <Рядок> або <Масив>'");
	КонецЕсли;
...
Функция ТекстОшибкиПредопределенноеЗначениеНеНайдено(ПолноеИмяПредопределенного)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Предопределенное значение ""%1"" не найдено.';uk='Напередвизначене значення ""%1"" не знайдено.'"), ПолноеИмяПредопределенного);
...
Процедура УдалитьВременныйФайл(ПолноеИмяФайла)
	...
	Исключение
#Если Сервер Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru='Базовая функциональность';uk='Базова функціональність'",КодОсновногоЯзыка()),
...
#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Возврат НСтр("ru='Подключение проводится на сервере платформы в интернете.';uk='Підключення проводиться на сервері платформи в інтернеті.'");
	Иначе 
		Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
			Если КлиентПодключенЧерезВебСервер() Тогда 
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Подключение проводится из файловой базы на веб-сервере <%1>.';uk='Підключення проводиться з файлової бази на веб-сервері <%1>.'"), ИмяКомпьютера());
			Иначе 
				Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Подключение проводится из файловой базы на компьютере <%1>.';uk='Підключення проводиться з файлової бази на комп''ютері <%1>.'"), ИмяКомпьютера());
			КонецЕсли;
		Иначе
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Подключение проводится на сервере платформы <%1>.';uk='Підключення проводиться на сервері платформи <%1>.'"), ИмяКомпьютера());
		КонецЕсли;
	КонецЕсли;
#Иначе 
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Подключение проводится на компьютере (клиенте) <%1>.';uk='Підключення проводиться на комп''ютері (клієнта) <%1>.'"), ИмяКомпьютера());
#КонецЕсли
...
Функция ПроверитьДоступностьСервера(АдресСервера)
	...
	СостояниеДоступности = ?(Доступен, НСтр("ru='доступен';uk='доступний'"), НСтр("ru='не доступен';uk='не доступний'"));
	
	Журнал = Новый Массив;
	Журнал.Добавить(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Удаленный сервер %1 %2:';uk='Віддалений сервер %1 %2:'"), 
		АдресСервера, 
