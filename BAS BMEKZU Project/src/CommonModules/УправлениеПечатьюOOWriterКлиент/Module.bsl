...
#Если ВебКлиент Тогда
	ТекстУточнения = Символы.ПС + НСтр("ru='Для формирования этой печатной формы необходимо воспользоваться тонким клиентом.';uk='Для формування цієї друкованої форми необхідно скористатися тонким клієнтом.'");
#Иначе		
	ТекстУточнения = "";	
#КонецЕсли
	ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
        ...
        |;uk='Для виведення друкованих форм у форматі OpenOffice.org Writer потрібно, щоб на комп''ютері був встановлений пакет OpenOffice.org. %1
        |Технічні подробиці: %2'"),
