...
Процедура ПоказатьПредупреждениеОНеобходимостиРасширенияРаботыСФайлами(ОбработчикРезультата, ПредставлениеКоманды = "") Экспорт
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		...
		ТекстПредупреждения = НСтр("ru='Выполнение команды ""%1"" в мобильном клиенте не поддерживается.';uk='Виконання команди ""%1"" в мобільному клієнті не підтримується.'");
		
	ИначеЕсли Не КлиентПоддерживаетСинхронныеВызовы() Тогда
		ТекстПредупреждения = НСтр("ru='Выполнение команды ""%1"" в браузерах
            |Google Chrome и Mozilla Firefox не поддерживается.'
            |;uk='Виконання команди ""%1"" в браузерах
            |Google Chrome і Mozilla Firefox не підтримується.'");
	Иначе
...
Процедура ПередЗавершениемРаботыСистемы(Отказ, Предупреждения) Экспорт
	...
	ПредупреждениеПользователю.ТекстПредупреждения = НСтр("ru='Имеются занятые для редактирования файлы';uk='Є зайняті для редагування файли'");
...
Процедура ДействиеПриОткрытииФайлаВРабочемКаталоге(ОбработчикРезультата, ИмяФайлаСПутем, ДанныеФайла)
		   ...
		   И Параметры.РазмерВХранилищеФайлов <> Параметры.РазмерВРабочемКаталоге Тогда
			...
			Параметры.Вставить("Сообщение",
				НСтр("ru='Размер файла в рабочем каталоге на компьютере и его копии в программе отличается.
                           ...
                           |;uk='Розмір файлу у робочому каталозі на комп''ютері та його копії в програмі відрізняється.
                           |
                           |Взяти файл з програми і замінити їм існуючий на комп''ютері або
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Если ДанныеФайла.ВРабочемКаталогеНаЧтение = Ложь Тогда
			...
			Параметры.Вставить("Заголовок", НСтр("ru='В программе более новая версия файла';uk='У програмі більш нова версія файлу'"));
			...
			Параметры.Вставить("Сообщение",
				НСтр("ru='Файл в программе, отмеченный как занятый для редактирования,
                           ...
                           |;uk='Файл у програмі, зазначений як зайнятий для редагування,
                           |має більш пізню дату зміни (новіше), ніж його копія в робочому каталозі на комп''ютері.
                           |
                           |Взяти файл з програми і замінити існуючий на комп''ютері або
                           |відкрити існуючий?'"));
		Иначе
	...
	ИначеЕсли Параметры.ДатаИзмененияУниверсальнаяВРабочемКаталоге
		...
		Иначе
			...
			Параметры.Вставить("Заголовок", НСтр("ru='На компьютере более новая копия файла';uk='На комп''ютері нова копія файлу'"));
			...
			Параметры.Вставить(
				...
				НСтр("ru='Копия файла в рабочем каталоге на компьютере имеет более позднюю дату изменения (новее), чем в программе. Возможно, эта копия была отредактирована.
                           ...
                           |;uk='Копія файлу в робочому каталозі на комп''ютері має більш пізню дату зміни (новіша), ніж у програмі. Можливо, ця копія була відредагована.
                           |
                           |Відкрити існуючий файл на комп''ютері або замінити його на файл
                           |з програми з втратою змін і відкрити?'"));
		КонецЕсли;
	КонецЕсли;
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если НЕ РасширениеРаботыСФайламиПодключено() Тогда
			ТекстНапоминания = 
                ...
                |;uk='Зараз буде запропоновано відкрити або зберегти файл.
                |
                |1. Натисніть кнопку ""Зберегти"" (""Save"").
                |
                |2. Виберіть каталог для збереження файлу на комп''ютері і запам''ятайте його
                |(каталог знадобиться для редагування і розміщення файлу назад в програму).
                |
                |3. Для редагування файлу перейдіть у вибраний раніше каталог,
                |знайдіть там збережений файл і відкрийте його.'");
			...
			Если СтрНайти(СистемнаяИнформация.ИнформацияПрограммыПросмотра, "Firefox") <> 0 Тогда
				...
				|
				|"
				+ НСтр("ru='(По умолчанию браузер Mozilla Firefox автоматически сохраняет файлы в каталоге ""Мои документы"")';uk='(По умовчанню браузер Mozilla Firefox автоматично зберігає файли в каталозі ""Мої документи"")'");
			КонецЕсли;
			...
			ПараметрыНапоминания.Вставить("ТекстФлажка",
				НСтр("ru='Больше не показывать это сообщение';uk='Більше не показувати це повідомлення'"));
			ПараметрыНапоминания.Вставить("Заголовок",
				НСтр("ru='Получение файла для просмотра или редактирования';uk='Отримання файлу для перегляду або редагування'"));
...
Процедура ПроверитьПодписиПослеПодготовкиДанных(Данные, ДополнительныеПараметры)
	...
	Если ТипЗнч(Данные) = Тип("ДвоичныеДанные") Тогда
		АдресДанных = ПоместитьВоВременноеХранилище(Данные, ДополнительныеПараметры.Форма.УникальныйИдентификатор);
	Иначе
		АдресДанных = Данные;
	КонецЕсли;
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Ошибка при помещении файла
                               |""%1""
                               |в программу.'
                               |;uk='Помилка при розміщенні файлу
                               |""%1""
                               |в програму.'"),
					Файл.ПолноеИмя) );
			Продолжить;
...
Процедура ОбновитьСостояниеОСохраненииФайлов(Знач ВыбранныеФайлы,
	...
	Если ВыбранныеФайлы.Количество() > 1 Тогда
		Если ТекущаяПозиция = Неопределено Тогда
			ПоказатьОповещениеПользователя(НСтр("ru='Сохранение файлов';uk='Збереження файлів'"),, НСтр("ru='Сохранение файлов успешно завершено';uk='Збереження файлів успішно завершено'"));
		КонецЕсли;
	Иначе
		Если ТекущаяПозиция = Неопределено Тогда
			ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Файл ""%1"" (%2 Мб) сохранен.';uk='Файл ""%1"" (%2 Мб) збережений.'"),
				СохраняемоеИмяФайла,
				РазмерВМб);
			ПоказатьОповещениеПользователя(НСтр("ru='Сохранение файла';uk='Збереження файлу'"), , ТекстПояснения, БиблиотекаКартинок.Информация32);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПоместитьВыбранныеФайлыВХранилищеВебЗавершение(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	// До исправления 30163819
	Если ОбщегоНазначенияКлиентСервер.ЭтоМобильныйКлиент() Тогда
		
		ФайлМобильныйКлиент = Новый Файл(ВыбранноеИмяФайла);
		ИмяФайла = ФайлМобильныйКлиент.ПолучитьПредставлениеФайлаБиблиотекиМобильногоУстройства();
...
Процедура ЗакончитьРедактированиеСРасширением(ПараметрыВыполнения)
			...
			Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось поместить файл
                               ...
                               |
                               |Освободить файл?'
                               |;uk='Не вдалось помістити файл
                               |""%1"" (%2),
                               |в програму, оскільки він не існує в робочому каталозі на комп''ютері.
                               |
                               |Звільнити файл?'"),
			...
			Иначе
				СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не удалось поместить файл ""%1"",
                               ...
                               |
                               |Освободить файл?'
                               |;uk='Не вдалось помістити файл ""%1"",
                               |в програму, оскільки він не існує в робочому каталозі на комп''ютері.
                               |
                               |Звільнити файл?'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
	...
	Исключение
		ТекстОшибки = НСтр("ru='Не удалось поместить файл ""%1"" в рабочий каталог на компьютер,
            ...
            |;uk='Не вдалось помістити файл ""%1"" в робочий каталог на комп''ютер,
            |так як можливо він заблокований іншою програмою.'");
	...
	Иначе
		...
		Исключение
			...
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
                ...
                |
                |Повторить операцию?'
                |;uk='Не вдалось помістити файл з комп''ютера в програму з причини:
                |""%1"".
                |
                |Повторити операцію?'"),
...
Процедура ЗакончитьРедактированиеСРасширениемОбработкаИсключения(ИнформацияОбОшибке, ПараметрыВыполнения)
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Не удалось поместить файл ""%1""
                   ...
                   |Повторить операцию?'
                   |;uk='Не вдалось помістити файл ""%1""
                   |з диска на комп''ютері програму з причини
                   |""%2"".
                   |
                   |Повторити операцію?'"),
		Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
...
Процедура ОбновитьИзФайлаНаДиске(ОбработчикРезультата, ДанныеФайла, ИдентификаторФормы)
	...
	РаботаСФайламиСлужебныйКлиентСервер.ПроверитьВозможностьЗагрузкиФайла(ФайлНаДиске);
	...
	Если ПараметрыВыполнения.ВремяИзмененияВыбранное < ПараметрыВыполнения.ДатаФайлаВБазе Тогда // В хранилище более новый.
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Файл ""%1""
                       ...
                       |Операция прервана.'
                       |;uk='Файл ""%1""
                       |на диску на комп''ютері має більш пізню дату зміни (%2),
                       |ніж обраний файл (%3).
                       |
                       |Операція перервана.'"),
			Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
		...
		Если Не ПустаяСтрока(ПараметрыВыполнения.ПолныйПутьКФайлу) Тогда
			СтрокаПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Не удалось поместить файл ""%1"" 
                           ...
                           |Освободить файл?'
                           |;uk='Не вдалось помістити файл ""%1"" 
                           |в програму, так як він не існує на диску на комп''ютері:
                           |%2.
                           |
                           |Звільнити файл?'"),
				Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка),
...
Процедура ПолучитьФайлВерсииВЛокальныйКэшФайловЗапуск(ПараметрыВыполнения)
	...
	Если ПараметрыВыполнения.ПолноеИмяФайла = "" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru='Не удалось получить файл из программы в рабочий каталог на компьютере.';uk='Не вдалося отримати файл з програми в робочий каталог на комп''ютері.'"));
...
Процедура ПолучитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ru='Не удалось получить файл на компьютер из программы по причине:';uk='Не вдалося отримати файл на комп''ютер з програми з причини:'") + Символы.ПС);
...
Процедура ПолучитьПрисоединенныйФайлПослеПодключенияРасширения(РасширениеПодключено, Контекст) Экспорт
	...
	Если Не РасширениеПодключено Тогда
		...
		Результат.Вставить("ОписаниеОшибки", Контекст.ЗаголовокОшибки
			+ НСтр("ru='В браузере не установлено расширение для работы с файлами.';uk='У браузері не встановлено розширення для роботи з файлами.'"));
		ВыполнитьОбработкуОповещения(Контекст.Оповещение, Результат);
		Возврат;
	КонецЕсли;
...
Процедура ПолучитьПрисоединенныйФайлПослеПолученияРабочегоКаталога(Результат, Контекст) Экспорт
	...
	Действие.Вставить("ЗаголовокОшибки", Контекст.ЗаголовокОшибки
		+ НСтр("ru='Установка свойств файла не выполнено по причине:';uk='Встановлення властивостей файлу не виконана з причини:'"));
	ДействияСФайлом.Добавить(Действие);
...
Процедура ПоместитьПрисоединенныйФайл(Оповещение, ПрисоединенныйФайл, ИдентификаторФормы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Контекст.Вставить("ЗаголовокОшибки",
		НСтр("ru='Не удалось поместить файл с компьютера в программу по причине:';uk='Не вдалось помістити файл з комп''ютера в програму з причини:'") + Символы.ПС);
...
Процедура ОчиститьРабочийКаталогОбработчикЦикла(ПараметрыВыполнения)
			...
			Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
				ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Выполняется очистка рабочего каталога на компьютере.
                               ...
                               |;uk='Виконується очищення робочого каталогу на комп''ютері.
                               |
                               |У програмі відсутній файл
                               |""%1"".
                               |
                               |Вилучити його копію з робочого каталогу на комп''ютері?'"),
	...
	Если ПараметрыВыполнения.НомерШага = 2 Тогда
		...
		ФайлНаДиске.УстановитьТолькоЧтение(Ложь);
		Если ПараметрыВыполнения.ОчищатьВсе = Ложь Тогда
			ШапкаВопроса = НСтр("ru='Выполняется очистка рабочего каталога на компьютере при добавлении файла.';uk='Виконується очищення робочого каталогу на комп''ютері при додаванні файлу.'");
		Иначе
			ШапкаВопроса = НСтр("ru='Выполняется очистка рабочего каталога на компьютере.';uk='Виконується очищення робочого каталогу на комп''ютері.'");
		КонецЕсли;
		...
		Если ПараметрыВыполнения.ДанныеФайла.Владелец = ВРегистреПапка Тогда // Одна и та же папка.
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В рабочем каталоге на компьютере уже есть файл
                           ...
                           |;uk='В робочому каталозі на комп''ютері вже є файл
                           |""%1"",
                           |що відповідає іншому файлу в програмі.
                           |
                           |Код файлу в програмі: %2.
                           |Код файлу в робочому каталозі: %3.
                           |
                           |Рекомендується перейменувати один з файлів у програмі.'"),
				ПараметрыВыполнения.ПолноеИмяФайла,
		...
		Иначе
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='В рабочем каталоге на компьютере уже есть файл
                           ...
                           |;uk='В робочому каталозі на комп''ютері вже є файл
                           |""%1"",
                           |що відповіднає іншому файлу в програмі.
                           |
                           |Код файлу в програмі: %2.
                           |Код файлу в робочому каталозі: %3.
                           |
                           |Рекомендується вказати інший робочий каталог для однієї з папок у програмі.
                           |(У двох папок не повинно бути однакового робочого каталогу).'"),
				ПараметрыВыполнения.ПолноеИмяФайла,
				ПараметрыВыполнения.ДанныеФайла.КодФайла,
				ВРегистреКодФайла);
		КонецЕсли;
			...
			Если ДатаФайлаНаДиске > ДатаФайлаВБазе Тогда // В рабочем каталоге более новый (изменен пользователем со стороны).
				...
				Сообщение = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Дата изменения файла ""%1""
                               ...
                               |в робочому каталозі на комп''ютері більш пізня (новіше), ніж у програмі.
                               |Можливо, файл на комп''ютері був відредагований.'"),
					Строка(ПараметрыВыполнения.ДанныеФайла.Ссылка));
...
Процедура СохранитьКакСРасширениемПослеРасшифровки(ОписаниеДанных, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru='Файл успешно сохранен';uk='Файл успішно збережений'"), , ПолноеИмяФайла);
	...
	Если ПерсональныеНастройки.ПоказыватьПодсказкиПриРедактированииФайлов = Истина Тогда
		Если Не РасширениеРаботыСФайламиПодключено() Тогда
			ТекстНапоминания = 
                ...
                |;uk='Зараз буде запропоновано вибрати файл для того,
                |щоб помістити його в програму і закінчити редагування.
                |
                |Знайдіть потрібний файл у тому каталозі, який був
                |зазначений раніше при початку редагування.'");
			...
			ПараметрыНапоминания.Вставить("ТекстФлажка",
				НСтр("ru='Больше не показывать это сообщение';uk='Більше не показувати це повідомлення'"));
			ПараметрыНапоминания.Вставить("Заголовок",
				НСтр("ru='Помещение файла';uk='Розміщення файлу'"));
			СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
				ОбработчикРезультата, ТекстНапоминания, Кнопки, ПараметрыНапоминания);
			Возврат;
		КонецЕсли;
...
Процедура ПоказатьИнформациюФайлНеБылИзменен(ОбработчикРезультата)
	...
	Если ПерсональныеНастройки.ПоказыватьИнформациюЧтоФайлНеБылИзменен Тогда
		ТекстНапоминания = НСтр("ru='Версия не была создана, т.к. файл не изменен. Комментарий не сохранен.';uk='Версія не була створена, тому що файл не змінений. Коментар не збережений.'");
		...
		ПараметрыНапоминания.Вставить("ТекстФлажка",
			НСтр("ru='Больше не показывать это сообщение';uk='Більше не показувати це повідомлення'"));
		ПараметрыНапоминания.Вставить("Заголовок",
			НСтр("ru='Информация';uk='Інформація'"));
		СтандартныеПодсистемыКлиент.ПоказатьВопросПользователю(
			ОбработчикРезультата, ТекстНапоминания, Кнопки, ПараметрыНапоминания);
	Иначе
...
Процедура ЗакончитьРедактированиеПоСсылкамПослеУстановкиРасширения(РасширениеУстановлено, ПараметрыВыполнения) Экспорт
	...
	ПоказатьОповещениеПользователя(
	НСтр("ru='Завершить редактирование файлов';uk='Завершити редагування файлів'"),,
	СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	НСтр("ru='Завершено редактирование файлов (%1 из %2).';uk='Завершено редагування файлів (%1 з %2).'"),
	ПараметрыВыполнения.ДанныеФайлов.Количество(),
	ПараметрыВыполнения.МассивФайлов.Количество()),
	БиблиотекаКартинок.Информация32);
...
Функция ПроверитьЗанятыеФайлыПриЗавершенииРаботы()
	...
	ПараметрыПрикладнойФормыПредупреждения.Вставить("СообщениеЗаголовок",   НСтр("ru='Следующие файлы заняты для редактирования:';uk='Наступні файли зайняті для редагування:'"));
...
Процедура СравнитьФайлы(ИдентификаторФормы, ПервыйФайл, ВторойФайл, Расширение, ВладелецВерсии = Неопределено) Экспорт
	...
	Если Не РасширениеПоддерживается Тогда
		...
		НСтр("ru='Сравнение файлов поддерживается только для файлов следующих типов: 
            ...
            |;uk='Порівняння файлів підтримується лише для файлів наступних типів: 
            |  Текстовий документ (.txt)
            |  Документ формату RTF (.rtf) 
            |  Документ Microsoft Word (.doc, .docx) 
            |  Документ HTML (.html .htm) 
            |  Табличний документ (.mxl) 
...
Процедура СравнениеВерсийАвтомат(Результат, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ТекущийШаг = 5 Тогда
		Если ПараметрыВыполнения.Результат1 И ПараметрыВыполнения.Результат2 Тогда
			...
			КонецЕсли;
			ШаблонЗаголовкаФайла = НСтр("ru='Файл №%1 (%2)';uk='Файл №%1 (%2)'");
...
Процедура ОбработатьФайлПослеПолученияФайлов(ПолученныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПолученныеФайлы) <> Тип("Массив") Или ПолученныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("ru='Получение файла было отменено.';uk='Отримання файлу було скасовано.'"), Неопределено, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьФайлПослеПомещенияФайлов(ПомещенныеФайлы, Контекст) Экспорт
	...
	Если ТипЗнч(ПомещенныеФайлы) <> Тип("Массив") Или ПомещенныеФайлы.Количество() = 0 Тогда
		ОбработатьФайлПослеОшибки(НСтр("ru='Помещение файла было отменено.';uk='Приміщення файлу було скасовано.'"), Неопределено, Контекст);
		Возврат;
	КонецЕсли;
...
Процедура ПечатьФайлаСоШтампом(ТабличныйДокумент) Экспорт
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		...
		ПечатнаяФорма.СинонимМакета = НСтр("ru='Файл со штампом';uk='Файл зі штампом'");
