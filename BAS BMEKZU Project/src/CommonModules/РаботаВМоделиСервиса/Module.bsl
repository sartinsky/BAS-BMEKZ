...
Процедура УстановитьРазделениеСеанса(Знач Использование = Неопределено, Знач ОбластьДанных = Неопределено) Экспорт
	...
	Если НЕ СеансЗапущенБезРазделителей() Тогда
		ВызватьИсключение(НСтр("ru='Изменить разделение сеанса возможно только из сеанса запущенного без указания разделителей';uk='Змінити поділ сеансу можливо лише з сеансу, який запущено без зазначення роздільників'"));
	КонецЕсли;
...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	СтрокаПараметра.Описание = НСтр("ru='Внутренний адрес Менеджера сервиса';uk='Внутрішня адреса Менеджера сервісу'");
...
Процедура УстановитьИспользованиеПредопределенногоРегламентногоЗадания(ЗаданиеМетаданные, Использование) Экспорт
	...
	КонецЕсли;
	Если Задания.Количество() = 0 Тогда
		ШаблонСообщения = НСтр("ru='Не найдено задание в очереди для предопределенного задания с именем %1';uk='Не знайдено завдання у черзі для зумовленого завдання з ім''ям %1'");
		...
		Если ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ВерсияБТС, ТребуемаяВерсияБТС()) < 0 Тогда
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Для использования конфигурации в модели сервиса с текущей версией БСП требуется
                      ...
                      |;uk='Для використання конфігурації в моделі сервісу з поточною версією БСП необхідно
	...
	Если НЕ ОбменЧерезФС Тогда
		...
		Если Описание.Данные = Неопределено Тогда
			...
			Иначе
				...
				ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Добавление файла в хранилище. Не найден файл %1.';uk='Додавання файлу в сховище. Не знайдено файл %1.'"),
