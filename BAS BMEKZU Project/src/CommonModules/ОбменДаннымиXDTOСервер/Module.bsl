...
Функция ОбъектXDTOИзДанныхXDTO(
	...
	Для Каждого Свойство Из ТипXDTO.Свойства Цикл
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Неизвестный тип свойства <%1>. Тип объекта: %2';uk='Невідомий тип властивості <%1>. Тип об''єкта: %2'"), Свойство.Имя, Строка(ТипXDTO));
		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка формирования объекта XDTO: Тип свойства <%1>. Имя свойства: <%2>.';uk='Помилка формування об''єкта XDTO: Тип властивості <%1>. Ім''я властивості: <%2>.'"), ТипСвойства, Свойство.Имя)
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Если Не КомпонентыОбмена.ТолькоНастройкиXDTO Тогда
				...
				Если ВерсииФорматаОбменаМассив(КомпонентыОбмена.УзелКорреспондента).Найти(ФорматОбмена.Версия) = Неопределено Тогда
					СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Версия ""%1"" формата сообщения обмена ""%2"" не поддерживается.';uk='Версія ""%1"" формату повідомлення обміну ""%2"" не підтримується.'"),
				...
				Если ОтправительИзСообщения = Неопределено
					...
					СтрокаСообщения = НСтр("ru='Не найден узел обмена для загрузки данных. План обмена: %1, Идентификатор: %2';uk='Не знайдено вузол обміну для завантаження даних. План обміну: %1 Ідентифікатор: %2'");
...
Процедура ЗаписатьОбъектВИБ(КомпонентыОбмена, Объект, Тип, ЗаписатьОбъект = Ложь, Знач ОтправкаНазад = Ложь, УникальныйИдентификаторСтрокой = "") Экспорт
	...
	Если ОбщегоНазначения.РазделениеВключено()
		...
		Если Не ЭтоРазделенныйОбъектМетаданных Тогда
			...
			СтрокаСообщенияОбОшибке = НСтр("ru='Попытка изменения неразделенных данных (%1: %2) в разделенном режиме.';uk='Спроба зміни неподілених даних (%1: %2) в розділеному режимі.'");
...
Процедура КонвертацияСвойстваXDTOВЭлементСтруктуры(Источник, Свойство, Приемник, ИмяДляСвойстваСоставногоТипа = "")
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка чтения объекта XDTO, имя свойства: <%1>.';uk='Помилка читання об''єкта XDTO, ім''я властивості: <%1>.'"), ИмяСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
...
Процедура КонвертацияСвойстваСтруктурыОбъектаXDTO(
	...
	Исключение
		ПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Ошибка конвертации свойства объекта XDTO, имя свойства: <%2>.';uk='Помилка конвертації властивості об''єкта XDTO, ім''я властивості: <%2>.'"), ЗначениеСвойства)
			+ Символы.ПС + Символы.ПС + ПредставлениеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
		...
		Если ТипОбъекта = Тип("ОбъектXDTO") Тогда
			...
			Если КоллекцияСвойств.Количество() > 0 И КоллекцияСвойств.Получить("КлючевыеСвойства") <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(Номер) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1 №%2 от %3';uk='%1 №%2 від %3'"), Наименование, Номер, Дата);
				ИначеЕсли ЗначениеЗаполнено(Код) Тогда
					ПредставлениеОбъекта = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1, %2';uk='%1, %2'"), Наименование, Код);
				КонецЕсли;
			КонецЕсли;
