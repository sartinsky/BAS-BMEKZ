		...
		Если ПустаяСтрока(ExchangeFeature.ExchangeRole) Тогда
			Если ВерсияБСП243 Тогда
				ВызватьИсключение СтрШаблон(НСтр("ru='Не задано возвращаемое значение функции ИмяКонфигурацииИсточника() в модуле менеджера плана обмена %1';uk='Не задане значення що повертається для функції ИмяКонфигурацииИсточника() у модулі менеджера плану обміну %1'"),
					ИмяПланаОбмена);
			Иначе
				ВызватьИсключение СтрШаблон(НСтр("ru='Не задано значение свойства ИмяКонфигурацииИсточника в процедуре ПриПолученииНастроек() модуля менеджера плана обмена %1';uk='Не задано значення властивості ИмяКонфигурацииИсточника у процедурі ПриПолученииНастроек() модуля менеджера плану обміну %1'"),
					ИмяПланаОбмена);
			КонецЕсли;
		КонецЕсли;
