...
Функция GetFilePart(TransferId, PartNumber, PartData)
	...
	Если ИменаФайлов.Количество() = 0 Тогда
		...
		ШаблонСообщения = НСтр("ru='Не найден фрагмент %1 сессии передачи с идентификатором %2';uk='Не знайдено фрагмент %1 сесії передачі з ідентифікатором %2'");
	...
	Для НомерЧасти = 1 По PartQuantity Цикл
		...
		Если НайтиФайлы(ИмяФайла).Количество() = 0 Тогда
			ШаблонСообщения = НСтр("ru='Не найден фрагмент %1 сессии передачи с идентификатором %2.
                    ...
                    |;uk='Не знайдено фрагмент %1 сесії передачі з ідентифікатором %2.
...
Функция TestConnection(ИмяПланаОбмена, КодУзла, Результат)
	...
	Если ОбменДаннымиСервер.УзелПланаОбменаПоКоду(ИмяПланаОбмена, КодУзла) = Неопределено Тогда
		...
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='В ""%1"" не найдена настройка синхронизации данных ""%2"" с идентификатором ""%3"".';uk='У ""%1"" не знайдено настройку синхронізації даних ""%2"" з ідентифікатором ""%3"".'"),
...
Процедура ПроверитьБлокировкуИнформационнойБазыДляОбновления()
	...
	Если ЗначениеЗаполнено(ОбновлениеИнформационнойБазыСлужебный.ИнформационнаяБазаЗаблокированаДляОбновления()) Тогда
		...
		ВызватьИсключение НСтр("ru='Синхронизация данных временно недоступна в связи с обновлением приложения в Интернете.';uk='Синхронізація даних тимчасово недоступна у зв''язку з оновленням прикладної програми в Інтернеті.'");
...
Процедура ВыполнитьВыгрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	КлючФоновогоЗадания = КлючФоновогоЗаданияВыгрузкиЗагрузкиДанных(ИмяПланаОбмена,
		КодУзлаИнформационнойБазы,
		НСтр("ru='Выгрузка';uk='Вивантаження'"));
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Выгрузка данных через веб-сервис.';uk='Вивантаження даних через веб-сервіс.'");
	...
	ИначеЕсли ФоновоеЗадание.Статус = "Выполнено" Тогда
		...
		Возврат;
	Иначе
		Сообщение = НСтр("ru='Ошибка при выгрузке данных через веб-сервис.';uk='Помилка при вивантаженні даних через веб-сервіс.'");
...
Процедура ВыполнитьЗагрузкуДанныхВКлиентСерверномРежиме(ИмяПланаОбмена,
	...
	ПараметрыВыполнения = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru='Загрузка данных через веб-сервис.';uk='Завантаження даних через веб-сервіс.'");
	ПараметрыВыполнения.КлючФоновогоЗадания = КлючФоновогоЗадания;
	...
	Иначе
		...
		Сообщение = НСтр("ru='Ошибка при загрузке данных через веб-сервис.';uk='Помилка при завантаженні даних через веб-сервіс.'");
		Если ЗначениеЗаполнено(ФоновоеЗадание.ПодробноеПредставлениеОшибки) Тогда
			Сообщение = ФоновоеЗадание.ПодробноеПредставлениеОшибки;
		КонецЕсли;
...
Функция КлючФоновогоЗаданияВыгрузкиЗагрузкиДанных(ПланОбмена, КодУзла, Действие)
	...
	Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='ПланОбмена:%1 КодУзла:%2 Действие:%3';uk='ПланОбмена:%1 КодУзла:%2 Дія:%3'"),
		ПланОбмена,
		КодУзла,
		Действие);
...
Функция ЗарегистрироватьДанныеДляНачальнойВыгрузки(Знач ИмяПланаОбмена, Знач КодУзла, ДлительнаяОперация, ИдентификаторОперации, ТолькоСправочники)
	...
	Иначе
		...
		Если ТолькоСправочники Тогда
			...
			НаименованиеЗадания = НСтр("ru='Регистрация изменений справочников для начальной выгрузки.';uk='Реєстрація змін довідників для початкового вивантаження.'");
		Иначе
			ИмяМетода = "ОбменДаннымиСервер.ЗарегистрироватьВсеДанныеКромеСправочниковДляНачальнойВыгрузкиВФоне";
			НаименованиеЗадания = НСтр("ru='Регистрация изменений всех данных кроме справочников для начальной выгрузки.';uk='Реєстрація змін усіх даних крім довідників для початкового вивантаження.'");
		КонецЕсли;
		...
		Иначе
			...
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Ошибка при выполнении фонового задания: %1';uk='Помилка при виконанні фонового завдання: %1'"),
				НаименованиеЗадания);
		КонецЕсли;
