		...
		Если ПустаяСтрока(ИмяКонфигурацииИсточника) Тогда
			Если ВерсияБСП243 Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не задано возвращаемое значение функции ИмяКонфигурацииИсточника() в модуле менеджера плана обмена %1';uk='Не задане значення що повертається для функції ИмяКонфигурацииИсточника() у модулі менеджера плану обміну %1'"),
					ИмяПланаОбмена);
			Иначе
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Не задано значение свойства ИмяКонфигурацииИсточника в процедуре ПриПолученииНастроек() модуля менеджера плана обмена %1';uk='Не задано значення властивості ИмяКонфигурацииИсточника у процедурі ПриПолученииНастроек() модуля менеджера плану обміну %1'"),
					ИмяПланаОбмена);
			КонецЕсли;
		КонецЕсли;
...
Функция ВыполнитьДействиеСценарияОбменаДаннымиВПервойИнформационнойБазе(ИндексСтрокиСценария, СценарийОбменаДаннымиXDTO)
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Неизвестный режим обмена данными %1';uk='Невідомий режим обміну даними %1'"), Строка(РежимОбмена));
	КонецЕсли;
