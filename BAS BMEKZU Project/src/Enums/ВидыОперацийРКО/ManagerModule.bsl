...
Функция ПолучитьДоступныеЗначения(Отбор, СтрокаПоиска)
	...
	Если НЕ ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
		Исключения.Добавить(Перечисления.ВидыОперацийРКО.ИНАГРО_ВыплатаПаевПоВедомостям);
		Исключения.Добавить(Перечисления.ВидыОперацийРКО.ИНАГРО_ВыплатаПаяПайщику);
		Исключения.Добавить(Перечисления.ВидыОперацийРКО.ИНАГРО_ДругиеВыплаты);
		Исключения.Добавить(Перечисления.ВидыОперацийРКО.Удалить_ИНАГРО_ВыплатаЗаработнойПлатыРаботникам);
	КонецЕсли;
