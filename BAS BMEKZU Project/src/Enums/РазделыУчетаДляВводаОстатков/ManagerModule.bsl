...
// Функция возвращает список доступных значений
// Параметры:
//
// Отбор - ОтборКомпоновкиДанных - набор преобразуемых отборов.
// ЕстьНедоступные - Булево - проверяет есть ли недоступные.
//
// Возвращаемое значение:
// Структура - структура доступных значений
//
...
Функция ПолучитьДоступныеЗначения(Отбор, СтрокаПоиска)
	...
	Если НЕ ИНАГРО_ОбщийВызовСервераПовтИсп.ЕстьБСПУ() Тогда
		Исключения.Добавить(Перечисления.РазделыУчетаДляВводаОстатков.БиологическиеАктивы);
	КонецЕсли;
	
	ДоступныеЗначения = Новый СписокЗначений;
	ЕстьНедоступные   = Ложь;
	
	Для каждого ЗначениеПеречисления Из Метаданные.Перечисления.РазделыУчетаДляВводаОстатков.ЗначенияПеречисления Цикл
		...
		Если ТипЗнч(СтрокаПоиска) = Тип("Строка")
			...
			Продолжить;
		КонецЕсли;
		Ссылка = Перечисления.РазделыУчетаДляВводаОстатков[ЗначениеПеречисления.Имя];
		Если ТипЗнч(Отбор) = Тип("ПеречислениеСсылка.РазделыУчетаДляВводаОстатков")
