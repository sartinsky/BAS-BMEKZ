...
Функция СвойстваРазделовДатЗапрета() Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru='Ошибка в процедуре ПриЗаполненииРазделовДатЗапретаИзменения
                   ...
                   |;uk='Помилка в процедурі ПриЗаполненииРазделовДатЗапретаИзменения
	...
	Для Каждого Раздел Из Разделы Цикл
		Если Не ЗначениеЗаполнено(Раздел.Имя) Тогда
			ВызватьИсключение ЗаголовокОшибки + НСтр("ru='Имя раздела дат запрета не заполнено.';uk='Ім''я розділу дат заборони не заповнено.'");
		КонецЕсли;
		...
		Если РазделыДатЗапрета.Получить(Раздел.Имя) <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Имя раздела дат запрета ""%1"" уже определено.';uk='Ім''я розділу дат заборони ""%1"" вже визначено.'"),
				Раздел.Имя);
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(Раздел.Идентификатор) И Раздел.Имя <> "ОбщаяДата" Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Идентификатор раздела дат запрета ""%1"" не заполнен.';uk='Ідентифікатор розділу дат заборони ""%1"" не заповнений.'"),
				Раздел.Имя);
		КонецЕсли;
		...
		Если РазделыДатЗапрета.Получить(РазделСсылка) <> Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Идентификатор ""%1"" раздела дат запрета
                           ...
                           |;uk='Ідентифікатор користувача ""%1"" розділу дат заборони
                           |""%2"" вже використовується для розділу ""%3"".'"),
		...
		Если Не ЗначениеЗаполнено(Раздел.Представление) Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Представление раздела дат запрета ""%1"" не заполнено.';uk='Представлення розділу дат заборони ""%1"" не заповнено.'"),
				Раздел.Имя);
		КонецЕсли;
			...
			Если Не ОбщегоНазначения.ЭтоСсылка(Тип) Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип ""%1"" указан, как тип объектов для раздела дат запрета ""%2"".
                               ...
                               |;uk='Тип ""%1"" вказано, як тип об''єктів для розділу дат заборони ""%2"".
			...
			Если ТипыОбъектовДатЗапрета.Получить(Тип) = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='Тип объектов ""%1"" раздела дат запрета ""%2""
                               ...
                               |;uk='Тип об''єктів ""%1"" розділу дат заборони ""%2""
                               |не вказаний у властивості ""Тип"" плану видів характеристик ""Розділи дати заборони зміни"".'"),
					Строка(Тип), Раздел.Имя);
			КонецЕсли;
	...
	СвойстваРаздела.Вставить("Представление", НСтр("ru='Общая дата';uk='Загальна дата'"));
