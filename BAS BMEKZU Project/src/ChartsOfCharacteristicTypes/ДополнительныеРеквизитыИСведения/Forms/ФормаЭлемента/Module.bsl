...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ПараметрыЗаписи.Свойство("КогдаИмяУжеИспользуется")
		...
		Если ЗначениеЗаполнено(ТекстВопроса) Тогда
			Кнопки = Новый СписокЗначений;
			Кнопки.Добавить("ПродолжитьЗапись",            НСтр("ru='Продолжить';uk='Продовжити'"));
			Кнопки.Добавить("ВернутьсяКВводуИмени", НСтр("ru='Отмена';uk='Відмінити'"));
...
Процедура КомандаДалее(Команда)
	...
	Если ИндексСтраницы = 0
		...
		ТекстПредупреждения = НСтр("ru='Элемент не выбран.';uk='Елемент не вибраний.'");
		ПоказатьПредупреждение(, ТекстПредупреждения);
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСтраницуВыбора()
	...
	Если ЭтоДополнительноеСведение = 1 Тогда
		Элементы.ОбщиеРеквизитыНеВключенныеВНаборы.Заголовок = НСтр("ru='Только общие дополнительные сведения';uk='Тільки загальні додаткові відомості'");
	Иначе
		Элементы.ОбщиеРеквизитыНеВключенныеВНаборы.Заголовок = НСтр("ru='Только общие дополнительные реквизиты';uk='Тільки загальні додаткові реквізити'");
	КонецЕсли;
	...
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ОбщиеНаборыСвойств, "ОбщиеДополнительныеСведения", НСтр("ru='Общие дополнительные сведения';uk='Загальні додаткові відомості'"), Истина);
	ОбщегоНазначенияКлиентСервер.УстановитьПараметрДинамическогоСписка(
		ОбщиеНаборыСвойств, "ОбщиеДополнительныеРеквизиты", НСтр("ru='Общие дополнительные реквизиты';uk='Загальні додаткові реквізити'"), Истина);
...
Процедура ЗаписатьОбъект(ВариантТекстаВопроса, ИмяПроцедурыПродолжения, ДополнительныеПараметры = Неопределено, ПараметрыЗаписи = Неопределено)
	...
	Если ВариантТекстаВопроса = "ИзменениеПометкиУдаления" Тогда
		Если Модифицированность Тогда
			ТекстВопроса = НСтр("ru='Для %1 пометки удаления необходимо записать внесенные изменения. Записать данные?';uk='Для %1 позначки вилучення необхідно записати внесені зміни. Записати дані?'");
			Если Объект.ПометкаУдаления Тогда
				Действие = НСтр("ru='снятия';uk='зняття'");
			Иначе
				Действие = НСтр("ru='установки';uk='встановлення'");
			КонецЕсли;
...
Процедура РедактироватьФорматЗначенияЗавершение(Текст, Контекст) Экспорт
	...
	Если Текст <> Неопределено Тогда
		...
		ТекстПредупреждения = НСтр("ru='Следующие настройки формата автоматически не применяются в большинстве мест:';uk='Наступні настройки формату автоматично не застосовуються в більшості місць:'");
		...
		Для каждого Подстрока Из Массив Цикл
			Если СтрНайти(Подстрока, "ДП=") > 0 ИЛИ СтрНайти(Подстрока, "DE=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ru='представление пустой даты';uk='представлення порожньої дати'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ЧН=") > 0 ИЛИ СтрНайти(Подстрока, "NZ=") > 0 Тогда
				ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
					+ " - " + НСтр("ru='представление пустого числа';uk='представлення порожнього числа'");
				Продолжить;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДФ=") > 0 ИЛИ СтрНайти(Подстрока, "DF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ддд") > 0 ИЛИ СтрНайти(Подстрока, "ddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru='кратное название дня недели';uk='кратна назва дня тижня'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "дддд") > 0 ИЛИ СтрНайти(Подстрока, "dddd") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru='полное название дня недели';uk='повна назва дня тижня'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "МММ") > 0 ИЛИ СтрНайти(Подстрока, "MMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru='кратное название месяца';uk='кратна назва місяця'");
				КонецЕсли;
				Если СтрНайти(Подстрока, "ММММ") > 0 ИЛИ СтрНайти(Подстрока, "MMMM") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru='полное название месяца';uk='повна назва місяця'");
				КонецЕсли;
			КонецЕсли;
			Если СтрНайти(Подстрока, "ДЛФ=") > 0 ИЛИ СтрНайти(Подстрока, "DLF=") > 0 Тогда
				Если СтрНайти(Подстрока, "ДД") > 0 ИЛИ СтрНайти(Подстрока, "DD") > 0 Тогда
					ТекстПредупреждения = ТекстПредупреждения + Символы.ПС
						+ " - " + НСтр("ru='длинная дата (месяц прописью)';uk='довга дата (місяць прописом)'");
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
...
Процедура ОбновитьСоставЭлементовФормы(ТекстПредупреждения = "")
	...
	Иначе
		...
		Иначе
			...
			Элементы.ЗаполнятьОбязательно.Заголовок = НСтр("ru='Заполнять обязательно';uk='Заповнювати обов''язково'");
		КонецЕсли;
		УстановитьЗаголовкиГиперссылок();
	КонецЕсли;
	...
	Иначе
		...
		Если СвойстваВладельца.ЭтоДополнительноеСведение <> Истина Тогда
			ШаблонУточнения = НСтр("ru='Список значений общий с реквизитом ""%1""';uk='Список значень спільний  з реквізитом ""%1""'");
		Иначе
			ШаблонУточнения = НСтр("ru='Список значений общий со сведением ""%1""';uk='Список значень спільний з відомостю ""%1""'");
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СвойстваВладельца.НаборСвойств) Тогда
			ШаблонУточненияНабор = " " + НСтр("ru='набора ""%1""';uk='набору ""%1""'");
			...
			Иначе
				Если СписокНаборов.Количество() > 1 Тогда
					...
					СтрокаНаборы = ПользователиСлужебныйКлиентСервер.ПредметЦелогоЧисла(СписокНаборов.Количество(),
						"", НСтр("ru='набор,набора,наборов,,,,,,0';uk='набір,набору,наборів,,,,,,0'"));
...
Процедура ЗаполнитьСписокДействийПриДобавленииРеквизита()
	...
	Если ПереданныеПараметрыФормы.ЭтоДополнительноеСведение Тогда
		...
		СделатьПоОбразцу = НСтр("ru='Сделать копию сведения по образцу (с общим списком значений)
            ...
            |;uk='Зробити копію відомості за зразком (із спільним списком значень)
            |
	...
	Иначе
		...
		СделатьПоОбразцу = НСтр("ru='Сделать копию реквизита по образцу (с общим списком значений)
            ...
            |;uk='Зробити копію реквізиту за зразком (із спільним списком значень)
            |
...
Функция ИмяУжеИспользуется(Знач Имя, Знач ТекущееСвойство, Знач НаборСвойств, НовоеНаименование)
	...
	Если Выборка.ЭтоДополнительноеСведение Тогда
		ТекстВопроса = НСтр("ru='Существует дополнительное сведение с именем
                                  |""%1"".'
                                  |;uk='Існує додаткова відомість з ім''ям
                                  |""%1"".'");
	Иначе
		ТекстВопроса = НСтр("ru='Существует дополнительный реквизит с именем
                                  |""%1"".'
                                  |;uk='Існує додатковий реквізит з ім''ям
                                  |""%1"".'");
	КонецЕсли;
	...
	ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстВопроса + Символы.ПС + Символы.ПС
                                 ...
                                 |;uk='Рекомендується використовувати інше ім''я,
                                 |інакше програма може працювати некоректно.
                                 |
                                 |Створити нове ім''я та продовжити запис?'"),
		Имя);
...
Процедура УстановитьЗаголовокКнопкиФормата(Форма)
	...
	Если ПустаяСтрока(Форма.Объект.ФорматСвойства) Тогда
		ТекстЗаголовка = НСтр("ru='Формат по умолчанию';uk='Формат по умовчанню'");
	Иначе
		ТекстЗаголовка = НСтр("ru='Формат установлен';uk='Формат встановлено'");
	КонецЕсли;
