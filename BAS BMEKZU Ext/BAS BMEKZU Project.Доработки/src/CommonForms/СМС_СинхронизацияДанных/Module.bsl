...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НавигационнаяСсылка = "e1cib/app/ОбщаяФорма.СМС_СинхронизацияДанных";
...
Процедура УдалитьНастройкуСинхронизации(Команда)
	...
	Если МодельСервиса 
		...
		ПоказатьПредупреждение(, НСтр("ru='Для удаления настройки синхронизации данных перейдите в менеджер сервиса.
            ...
            |;uk='Для видалення настройки синхронізації даних перейдіть в менеджер сервісу.
...
Процедура НачальнаяВыгрузкаДанных(Команда)
	...
	Если Не НастройкаСинхронизацииЗавершена(ТекущиеДанные.УзелИнформационнойБазы) Тогда
		...
		ПоказатьВопрос(ОповещениеЗавершения,
            ...
            |;uk='Перед вивантаженням даних для зіставлення необхідно завершити настройку синхронізації.
            |Відкрити форму помічника настройки?'"),
			РежимДиалогаВопрос.ДаНет);
	ИначеЕсли Не ТекущиеДанные.ДоступнаИнтерактивнаяОтправка Тогда
		ПоказатьПредупреждение(, НСтр("ru='Для выбранного варианта настройки синхронизации выгрузка данных для сопоставления не поддерживается.';uk='Для вибраного варіанту настройки синхронізації вивантаження даних для співставлення не підтримується.'"));
		Возврат;
	Иначе
		ПараметрыПомощника = Новый Структура;
		ПараметрыПомощника.Вставить("УзелОбмена", ТекущиеДанные.УзелИнформационнойБазы);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='Не запускалась';uk='Не запускалася'");
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru='н/д';uk='н/д'"));
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СписокПриложений.СостояниеПредставление");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = НСтр("ru='Настройка не завершена';uk='Настройку не завершено'");
...
Процедура ОбработкаКомандыВыполненияСинхронизации(ИспользоватьДопОтборы = Ложь)
	...
	Если Не НастройкаСинхронизацииЗавершена(ТекущиеДанные.УзелИнформационнойБазы) Тогда
		...
		ПоказатьВопрос(ОповещениеЗавершения,
            ...
            |Открыть форму помощника настройки?'
            |;uk='Перед запуском синхронізації даних необхідно завершити її настройку.
            |Відкрити форму помічника настройки?'"),
			РежимДиалогаВопрос.ДаНет);
	ИначеЕсли ТекущиеДанные.ЗапускОбменаИзКорреспондента
		...
		ПоказатьПредупреждение(, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
            ...
            |;uk='Запуск синхронізації з ""%1"" з цієї програми не підтримується.
            |Перейдіть до ""%1"" і почніть синхронізацію з неї.'"), ТекущиеДанные.НаименованиеКорреспондента));
	Иначе
		ДополнительныеПараметры = Новый Структура;
	...
	Если ОбщегоНазначения.РазделениеВключено() Тогда
		Если ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
			...
			Если Не МодульОбменДаннымиВМоделиСервисаПовтИсп.СинхронизацияДанныхПоддерживается() Тогда
		 		ТекстСообщения = НСтр("ru='Возможность настройки синхронизации данных в данной программе не предусмотрена.';uk='Можливість настройки синхронізації даних у цій програмі не передбачено.'");
				Отказ = Истина;
			КонецЕсли;
		Иначе
			ТекстСообщения = НСтр("ru='В неразделенном режиме настройка синхронизации данных с другими программами недоступна.';uk='У нерозділеному режимі настройка синхронізації даних з іншими програмами недоступна.'");
			Отказ = Истина;
		КонецЕсли;
	Иначе
...
Функция СостояниеСинхронизацииДанных(СтрокаПриложение)
	...
	Если Не СтрокаПриложение.НастройкаЗавершена Тогда
		Состояние.Представление = НСтр("ru='Настройка не завершена';uk='Настройку не завершено'");
		Состояние.Картинка = 3;
		
		Если СтрокаПриложение.ПолученоСообщениеДляСопоставленияДанных Тогда
			Состояние.Представление = НСтр("ru='Настройка не завершена, получены данные для сопоставления';uk='Настройку не завершено, отримані дані для співставлення'");
		КонецЕсли;
	Иначе
		Если СтрокаПриложение.ДатаНачалаПоследнейЗагрузки > СтрокаПриложение.ДатаОкончанияПоследнейЗагрузки Тогда
			Состояние.Представление = НСтр("ru='Загрузка данных...';uk='Завантаження даних...'");
		...
		ИначеЕсли СтрокаПриложение.ДатаНачалаПоследнейВыгрузки > СтрокаПриложение.ДатаОкончанияПоследнейВыгрузки Тогда
			Состояние.Представление = НСтр("ru='Выгрузка данных...';uk='Вивантаження даних...'");
			Состояние.Картинка = 4;
		ИначеЕсли Не ЗначениеЗаполнено(СтрокаПриложение.ДатаПоследнегоЗапуска) Тогда
			Состояние.Представление = НСтр("ru='Не запускалась';uk='Не запускалася'");
			
			Если СтрокаПриложение.ПолученоСообщениеДляСопоставленияДанных Тогда
				Состояние.Представление = НСтр("ru='Получены данные для сопоставления';uk='Отримані дані для зіставлення'");
			КонецЕсли;
		Иначе
			Состояние.Представление = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Прошлый запуск: %1';uk='Минулий запуск: %1'"),
