...
&ИзменениеИКонтроль("скEDI_РегистрацияЭлектронногоДокументаПриЗаписи")
...
Процедура ПометитьНаУдалениеАктыРасчета(РеестрСсылка) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ ССЫЛКА ИЗ ДОКУМЕНТ.СМС_АКТРАСЧЕТА ГДЕ РеестрТТНВВоз=&РеестрСсылка И НЕ ПометкаУдаления");
...
Процедура СоздатьАктыРасчета(Источник) Экспорт
	...
	//del by Al
	//заказчик попросил регистрации документа в любом случае - совпадает веса или нет
	...
	|		КОГДА ЕСТЬNULL(скEDI_ЭлектронныеДокументы.ЭлектронныйДокумент, ЗНАЧЕНИЕ(Документ.скEDI_ЭлектронныйДокумент.ПустаяСсылка)) <> ЗНАЧЕНИЕ(документ.скEDI_ЭлектронныйДокумент.пустаяссылка)
	|			ТОГДА ВЫБОР
	|					КОГДА скEDI_ЭлектронныеДокументы.ЭлектронныйДокумент.ОсновнойЭлектронныйДокумент <> ЗНАЧЕНИЕ(документ.скEDI_ЭлектронныйДокумент.пустаяссылка)
	|						ТОГДА ВЫБОР
	|								КОГДА скEDI_ЭлектронныеДокументы.ЭлектронныйДокумент.ОсновнойЭлектронныйДокумент = скEDI_ЭлектронныеДокументы.ЭлектронныйДокумент.Ссылка
	|									ТОГДА ИСТИНА
	|								ИНАЧЕ ЛОЖЬ
	|							КОНЕЦ
	|					ИНАЧЕ ИСТИНА
	|				КОНЕЦ
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоОсновнойВариантЭД
	//DIN 28.09.2022
	|ИЗ
	|	Документ.СМС_АктРасчета КАК АктРасчета
	...
	//DIN 09.09.2022
	СписокТТН = РеестрСсылка.СписокТТН.Выгрузить();
	СписокТТН.Колонки.Добавить("ДоговорПоставщика",новый ОписаниеТипов("СправочникСсылка.ДоговорыКонтрагентов"));
	
	Для каждого Стр из СписокТТН Цикл 
		стр.ДоговорПоставщика = Стр.ТТН.ДоговорПоставщика;
	КонецЦикла;	
	
	СписокТТН.Сортировать("ДоговорПоставщика");
	
	ЗаполненныеАкты = Новый Массив;
	//DIN 10.09.2022
	...
	Для каждого Стр из СписокТТН Цикл 
		Если ДоговорПоставщикаТМП <> Стр.ТТН.ДоговорПоставщика Тогда 
			...
			Если НайденыйАкт <> Неопределено Тогда 
				...
				Если НайденыйАкт.ЕстьЭлектронныйДокумент И НайденыйАкт.ЭтоОсновнойВариантЭД  Тогда 
				//DIN 28.09.2022
					Продолжить;
				КонецЕсли;
				...
				//DIN 10.09.2022
				ЗаполненныеАкты.Добавить(НайденыйАкт.Ссылка);
				//DIN 10.09.2022
			...
			Иначе 
				АктРасчетаОбъект = Документы.СМС_АктРасчета.СоздатьДокумент();
				АктРасчетаОбъект.Дата = РеестрСсылка.Дата;
	...
	//DIN 10.09.2022
	Для Каждого АктСтарый из СтарыеАкты Цикл 
		Если ЗаполненныеАкты.Найти(АктСтарый.Ссылка) = Неопределено Тогда 
			АктСтарыйОбъект = АктСтарый.Ссылка.ПОлучитьОбъект();
			АктСтарыйОбъект.УстановитьПометкуУдаления(Истина);
		КонецЕсли;
	КонецЦикла;
	//DIN 10.09.2022
...
//added by Al
Функция ОпределитьДокументНакладнаяНаСервере(ПараметрыОтбора, ПараметрыДляФОрмыВыбора = Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("ВидЭлектронногоДокумента", ПараметрыОтбора.ЭлектронныйДокумент.ВидЭлектронногоДокумента);
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	
	Если Выборка.ЭтоПТУ_РТУ И Не ПараметрыДляФОрмыВыбора = Неопределено Тогда
		
		ПараметрыДляФОрмыВыбора.ТипДок = ?(ПараметрыОтбора.ЭлектронныйДокумент.ВхИсх = Перечисления.скEDI_ВхИсхЭлектронныйДокумент.Входящий, "ПоступлениеТоваровУслуг", "РеализацияТоваровУслуг");
		ПараметрыДляФОрмыВыбора.Контрагент = Справочники.Контрагенты.НайтиПоРеквизиту("КодПоЕДРПОУ",ПараметрыОтбора.ЭлектронныйДокумент.ЕДРПОУКонтрагента)
...
//added by Al
Функция ПроверитьСоответствиеКлючевыхРеквизитовДокументов(Док1С, ДокЭДО, ЭтоИсходящий) Экспорт
	 
	Возврат Док1С.Контрагент.КодПоЕДРПОУ  = ДокЭДО.ЕДРПОУКонтрагента И 
	        ?(ЭтоИсходящий, Истина, Док1С.ДатаВходящегоДокумента = ДокЭДО.ДатаДокумента И Док1С.НомерВходящегоДокумента  = ДокЭДО.НомерДокумента) И
			Док1С.СуммаДокумента 		   = ДокЭДО.СуммаДокумента   И
			//разрешим выбирать только те документы, которые пришли по обмену
			ЗначениеЗаполнено(Док1С.КлючДляОбмена);
